...
function note_print($note, $detail = NOTES_SHOW_FULL) {
    ...
    if ($detail & NOTES_SHOW_FOOT) {
        if (has_capability('eset/notes:manage', $systemcontext) && $note->publishstate == NOTES_STATE_SITE ||
            has_capability('eset/notes:manage', $context) &&
...
function core_notes_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (empty($course)) {
        // Site level profile.
        if (!has_capability('eset/notes:view', context_system::instance())) {
            // No cap, nothing to do.
            return false;
        }
    } else {
        if (!has_capability('eset/notes:view', context_course::instance($course->id))) {
            // No cap, nothing to do.
            return false;
        }
