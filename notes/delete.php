...
if (!has_capability('eset/notes:manage', $context)) {
...
if (data_submitted() && confirm_sesskey()) {
    ...
    $PAGE->navbar->add(fullname($user), new eset_url('/user/view.php', array('id' => $user->id, 'course' => $course->id)));
    $PAGE->navbar->add(get_string('notes', 'notes'),
                       new eset_url('/notes/index.php', array('user' => $user->id, 'course' => $course->id)));
    $PAGE->navbar->add(get_string('delete'));
    $PAGE->set_title($course->shortname . ': ' . $strnotes);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(get_string('deleteconfirm', 'notes'),
                          new eset_url('delete.php', $optionsyes),
                          new eset_url('index.php', $optionsno));
    echo '<br />';
