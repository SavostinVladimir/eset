...
$url = new eset_url('/notes/index.php');
if ($courseid != SITEID) {
...
require_capability('eset/notes:view', $coursecontext);
...
if ($userid && $course->id == SITEID) {
    ...
    $notesurl = new eset_url('/notes/index.php', array('user' => $userid));
    $PAGE->navbar->add(get_string('notes', 'notes'), $notesurl);
} else if ($course->id != SITEID) {
    $notenode = $PAGE->navigation->find('currentcoursenotes', null)->make_inactive();

    $notesurl = new eset_url('/notes/index.php', array('user' => $userid, 'course' => $courseid));
    $PAGE->navbar->add(get_string('notes', 'notes'), $notesurl);
    ...
    if (course_can_view_participants($coursecontext) || course_can_view_participants($systemcontext)) {
        $link = new eset_url('/user/index.php', array('id' => $course->id));
    }
}
...
if ($courseid != SITEID) {
    ...
    $addid = has_capability('eset/notes:manage', $context) ? $courseid : 0;
    $view = has_capability('eset/notes:view', $context);
    ...
    $view = has_capability('eset/notes:view', context_system::instance());
    ...
    if (!empty($userid)) {
        ...
        foreach ($courses as $c) {
            ...
            $viewcoursenotes = has_capability('eset/notes:view', $ccontext);
            if (has_capability('eset/notes:manage', $ccontext)) {
                $addid = $c->id;
            } else {
                $addid = 0;
            }
