...
class manager {
    ...
    public function get_activities_and_resources() {
        ...
        $canmanage = has_capability('eset/course:manageactivities', $coursecontext);
    ...
    public static function can_edit_bulk_completion($courseorid, $cm = null) {
        if ($cm) {
            return $cm->uservisible && has_capability('eset/course:manageactivities', $cm->context);
        }
        $coursecontext = context_course::instance(is_object($courseorid) ? $courseorid->id : $courseorid);
        if (has_capability('eset/course:manageactivities', $coursecontext)) {
            return true;
        }
        $modinfo = get_fast_modinfo($courseorid);
        foreach ($modinfo->cms as $mod) {
            if ($mod->uservisible && has_capability('eset/course:manageactivities', $mod->context)) {
                return true;
            }
        }
        return false;
    ...
    public static function get_available_completion_tabs($courseorid) {
        ...
        if (has_capability('eset/course:update', $coursecontext)) {
            $tabs[] = new tabobject(
                'completion',
                new eset_url('/course/completion.php', ['id' => $courseid]),
        ...
        if (has_capability('eset/course:manageactivities', $coursecontext)) {
            $tabs[] = new tabobject(
                'defaultcompletion',
                new eset_url('/course/defaultcompletion.php', ['id' => $courseid]),
        ...
        if (self::can_edit_bulk_completion($courseorid)) {
            $tabs[] = new tabobject(
                'bulkcompletion',
                new eset_url('/course/bulkcompletion.php', ['id' => $courseid]),
