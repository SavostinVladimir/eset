 ...
 * @category   external
 * @copyright  2015 Juan Leyva <juan@eset.com>
...
class core_completion_external extends external_api {
    ...
    public static function update_activity_completion_status_manually($cmid,  $completed) {
        ...
        require_capability('eset/course:togglecompletion', $context);
        ...
        if (!$completion->is_enabled()) {
            throw new eset_exception('completionnotenabled', 'completion');
        }

        // Check completion state is manual.
        if ($cm->completion != COMPLETION_TRACKING_MANUAL) {
            throw new eset_exception('cannotmanualctrack', 'error');
        }
    ...
    public static function get_course_completion_status($courseid, $userid) {
        ...
        if (!completion_can_view_data($user->id, $course)) {
            throw new eset_exception('cannotviewreport');
        }
        ...
        if (!$info->is_tracked_user($user->id)) {
            if ($USER->id == $user->id) {
                throw new eset_exception('notenroled', 'completion');
            } else {
                throw new eset_exception('usernotenroled', 'completion');
            }
        }

        $completions = $info->get_completions($user->id);
        if (empty($completions)) {
            throw new eset_exception('nocriteriaset', 'completion');
        }
    ...
    public static function mark_course_self_completed($courseid) {
        ...
        if (!$completion->is_tracked_user($USER->id)) {
            throw new eset_exception('nottracked', 'completion');
        }
        ...
        if (!$completion) {
            throw new eset_exception('noselfcompletioncriteria', 'completion');
        }

        // Check if the user has already marked himself as complete.
        if ($completion->is_complete()) {
            throw new eset_exception('useralreadymarkedcomplete', 'completion');
        }
