<?php
    ...
    // Designed to be redirected from eset/login/index.php
    ...
    $readmeurl = (new eset_url('/auth/shibboleth/README.txt'))->out();
...
/// If we can find the Shibboleth attribute, save it in session and return to main login page
    if (!empty($_SERVER[$pluginconfig->user_attribute])) {    // Shibboleth auto-login
        ...
        // returns true, the password is set to 'not cached' (AUTH_PASSWORD_NOT_CACHED) in the Eset DB. However,
    ...
    /// Check if the user has actually submitted login data to us
        ...
        if ($shibbolethauth->user_login($frm->username, $frm->password)
            ...
            /// Go to my-eset page instead of homepage if defaulthomepage enabled
            if (!has_capability('eset/site:config',context_system::instance()) and !empty($CFG->defaulthomepage) && $CFG->defaulthomepage == HOMEPAGE_MY and !isguestuser()) {
            ...
            // The Shibboleth user couldn't be mapped to a valid Eset user
