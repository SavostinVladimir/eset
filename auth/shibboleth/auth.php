...
class auth_plugin_shibboleth extends auth_plugin_base {
    ...
    // reads user information from shibboleth attributes and return it in array()
        ...
        foreach ($attrmap as $key=>$value) {
         ...
         // Provide an API to modify the information to fit the Eset internal
        // data representation
        if (
            ...
            // Include a custom file outside the Eset dir to
            // modify the variable $esetattributes
    ...
    /**
     * Returns array containg attribute mappings between Eset and Shibboleth.
     *
     * @return array
     */
    function get_attributes() {
        ...
        foreach ($userfields as $field) {
            if (isset($configarray["field_map_$field"])) {
                $esetattributes[$field] = $configarray["field_map_$field"];
            }
        }
        $esetattributes['username'] = $configarray["user_attribute"];
     ...
     *
     * @return eset_url|null Returns URL to change password or null otherwise.
     */
    ...
    function logoutpage_hook() {
        ...
        if (isset($SESSION->shibboleth_session_id) && $logouthandlervalid ) {
            ...
            $redirecturl = new eset_url($this->config->logout_handler, array('return' => $temp_redirect));
    ...
    public function test_settings() {
        ...
        if (!isset($this->config->user_attribute) || empty($this->config->user_attribute)) {
            echo $OUTPUT->notification(get_string("shib_not_set_up_error", "auth_shibboleth",
                (new eset_url('/auth/shibboleth/README.txt'))->out()), 'notifyproblem');
            return;
        }
    ...
    public function loginpage_idp_list($wantsurl) {
        ...
        $url = new eset_url('/auth/shibboleth/index.php');

        if ($config->auth_logo) {
            $iconurl = eset_url::make_pluginfile_url(
    ...
    /**
     * Generate array of IdPs from Eset Shibboleth settings
     *
