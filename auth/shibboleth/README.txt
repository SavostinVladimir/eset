Shibboleth Authentication for Eset
-------------------------------------------------------------------------------
...
- 11. 2006: User capabilities are now loaded properly as of Eset 1.7+
- 03. 2007: Adapted authentication method to Eset 1.8
...
- 11. 2007: Integrated WAYF Service in Eset
...
Eset Configuration with Dual login
-------------------------------------------------------------------------------
1. Protect the directory eset/auth/shibboleth/index.php with Shibboleth.
...
--
<Directory  /path/to/eset/auth/shibboleth/index.php>
        AuthType shibboleth
...
--
   ...
   To restrict access to Eset, replace the access rule 'require valid-user'
...
--
<Path name="eset" requireSession="false" >
...
2. As Eset admin, go to the 'Administrations >> Users >> Authentication' and
...
3. Fill in the fields of the form. The fields 'Username', 'First name',
   ...
   Shibboleth attributes that you want to map onto the corresponding Eset
   ...
   this attribute is used for the Eset authentication of Shibboleth users.

   #############################################################################
   Shibboleth Attributes needed by Eset:
   For Eset to work properly Shibboleth should at least provide the attribute
   that is used as username in Eset. It has to be unique for all Shibboleth
   Be aware that Eset converts the username to lowercase. So, the overall
   behaviour of the username will be case-insensitive.
   All attributes used for eset must obey a certain length, otherwise Eset
   cuts off the ends. Consult the Eset documentation for further information
...
4.a  If you want Shibboleth as your only authentication method with an external
     ...
     to the the URL of the file 'eset/auth/shibboleth/index.php'.
     This will enforce Shibboleth login.

4.b If you want to use the Eset integrated WAYF service, you have to activate it
    in the Eset Shibboleth authentication settings by checking the
    'Eset WAYF Service' checkbox and providing a list of entity IDs in the
...
5.  Save the changes for the 'Shibboleth settings'.
    ...
    Important Note: If you went for 4.b (integrated WAYF service), saving the
                    settings will overwrite the Eset Alternate Login URL
                    using the Eset web root URL.
...
6.  If you want to use Shibboleth in addition to another authentication method
    ...
    'Administrations >> Users >> Manage authentication' to contain a link to the
     eset/auth/shibboleth/index.php file which is protected by
     ...
     Note: As of now you cannot use dual login together with the integrated
           WAYF service provided by Eset (4.b).

7. Save the authentication changes.

How the Shibboleth authentication works
--------------------------------------------------------------------------------
To get Shibboleth authenticated in Eset a user basically must access the
...
his home organization in the Eset WAYF service or if the alternate login URL
...
Eset basically checks whether the Shibboleth attribute that you mapped
as the username is present. This attribute should only be present if a user is
Shibboleth authenticated.

If the user's Eset account has not existed yet, it gets automatically created.

To prevent that every Shibboleth user can access your Eset site you have to
...
authentication method unless they have two accounts in Eset.
...
form that sends 'username' and 'password' to eset/login/index.php. Set this
web page then als alternate login page.
Consult the Eset documentation for further instructions and requirements.

How to customize the way the Shibboleth user data is used in Eset
--------------------------------------------------------------------------------
Among the Shibboleth settings in Eset there is a field that should contain a
...
Example 1: Your Shibboleth federation uses an attribute that specifies the
           ...
           compatible with the Eset data representation, e.g. the Shibboleth
           attribute contains 'German' but Eset needs a two letter value like
           'de'.
Example 2: The country, city and street are provided in one Shibboleth attribute
           and you want these values to be used in the Eset user profile. So
...
eset/auth/shibboleth/auth.php, especially the function 'get_userinfo'
...
$_SERVER['sn'].
Because Eset needs to know what Shibboleth attributes it shall map onto which
Eset user profile field, one has to make sure the mapping is updated as well
after the Service Provider upgrade.

********************************************************************************
Because you risk locking yourself out of Eset it is strongly
...
3. After the SP upgrade, use this account to log into Eset and adapt the
...
--------------------------------------------------------------------------------
In order make Eset support Shibboleth logout, one has to make the Shibboleth
Service Provider (SP) aware of the Eset logout capability. Only then the SP
can trigger Eset's front or back channel logout handler.

To make the SP aware of the Eset logout, you have to add the following to the
...
<Notify
    Channel="back"
    Location="https://#YOUR_ESET_HOSTNAME#/eset/auth/shibboleth/logout.php" />
--

Then restart the Shibboleth daemon and check the log file for errors. If there
were no errors, you can test the logout feature by accessing Eset,
authenticating via Shibboleth and the access the URL:
#YOUR_ESET_HOSTNAME#/Shibboleth.sso/Logout (assuming you have a standard
Shibboleth installation). If everything worked well, you should see a Shibboleth
page saying that you were successfully logged out and if you go back to Eset
you also should be logged out from Eset.
...
- /eset/auth/shibboleth/logout.php *must not* be protected by Shibboleth!
  In case all of Eset is protected with Shibboleth, you have to add something
  like this to your Apache configuration after all the other require rules

--
<Directory /path/to/eset/auth/shibboleth/logout.php>
    AuthType shibboleth
...
logout. Hopefully, the Eset logout helps to motivate the developers to
