...
class auth_plugin_db extends auth_plugin_base {
    ...
    function db_init() {
        if ($this->is_configured() === false) {
            throw new eset_exception('auth_dbcantconnect', 'auth_db');
        }
    ...
    /**
     * Returns user attribute mappings between eset and the external database.
     *
     * @return array
     */
    function db_attributes() {
        ...
        foreach ($userfields as $field) {
            if (!empty($this->config->{"field_map_$field"})) {
                $esetattributes[$field] = $this->config->{"field_map_$field"};
            }
        }
        $esetattributes['username'] = $this->config->fielduser;
    ...
    /**
     * Synchronizes user from external db to eset user table.
     *
     * Sync should be done by using idnumber attribute, not username.
     * You need to pass firstsync parameter to function to fill in
     * idnumbers if they don't exists in eset user table.
     *
