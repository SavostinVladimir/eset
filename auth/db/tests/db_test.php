 ...
 * that are matching our four supported Eset database drivers.
 *
 * @package    auth_db
 * @category   phpunit
...
class auth_db_testcase extends advanced_testcase {
    ...
    public function test_clean_data() {
        ...
        // Get the user on eset user table.
    ...
    public function test_deleting_with_many_users() {
        ...
        // Set to delete from eset when missing from DB.
        set_config('removeuser', AUTH_REMOVEUSER_FULLDELETE, 'auth_db');
        $auth->config->removeuser = AUTH_REMOVEUSER_FULLDELETE;
        ...
        $esetuser = $DB->get_record('user', array('email' => $user->email, 'auth' => 'db'));
        $this->assertNotNull($esetuser);
        $this->assertEquals($user->username, $esetuser->username);
        ...
        $esetuser = $DB->get_record('user', array('id' => $esetuser->id));
        $this->assertFalse($auth->user_login($user->username, 'heslo'));
        $this->assertEquals(1, $esetuser->deleted);
