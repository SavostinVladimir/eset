...
class auth_ldap_plugin_testcase extends advanced_testcase {
    ...
    public function test_auth_ldap(int $pagesize, int $subcontext) {
        ...
        $debuginfo = '';
        if (!$connection = ldap_connect_eset(TEST_AUTH_LDAP_HOST_URL, 3, 'rfc2307', TEST_AUTH_LDAP_BIND_DN, TEST_AUTH_LDAP_BIND_PW, LDAP_DEREF_NEVER, $debuginfo, false)) {
        ...
        $topdn = 'dc=esettest,'.TEST_AUTH_LDAP_DOMAIN;

        $this->recursive_delete($connection, TEST_AUTH_LDAP_DOMAIN, 'dc=esettest');

        $o = array();
        $o['objectClass'] = array('dcObject', 'organizationalUnit');
        $o['dc']         = 'esettest';
        $o['ou']         = 'ESETTEST';
        if (!ldap_add($connection, 'dc=esettest,'.TEST_AUTH_LDAP_DOMAIN, $o)) {
