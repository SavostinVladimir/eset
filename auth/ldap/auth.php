...
class auth_plugin_ldap extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        ...
        // password from Eset (otherwise we'll get stuck later in the login process).
     ...
     * this information to eset user-table you should honor syncronization flags
     *
    ...
    function get_userinfo($username) {
        ...
        $user_entry = ldap_get_entries_eset($ldapconnection, $user_info_result);
        if (empty($user_entry)) {
            $this->ldap_close();
     ...
     *
     * @param mixed $userobject  Eset userobject
    ...
    function user_create($userobject, $plainpass) {
        ...
        switch ($this->config->user_type)  {
            ...
            case 'rfc2307bis':
                ...
                // Eset without colliding with existing numbers and without
                // race conditions. As this user is supposed to be only used
                // with Eset (otherwise the user would exist beforehand) and
                ...
                // system level (Eset ignores this).
                ...
                // Eset is available, and the user can see the kind of
    ...
    /**
     * Returns true if plugin allows resetting of password from eset.
     *
     * @return bool
     */
    ...
    function password_expire($username) {
        ...
        if ($sr)  {
            $info = ldap_get_entries_eset($ldapconnection, $sr);
            if (!empty ($info)) {
                $info = $info[0];
    ...
    /**
     * Syncronizes user fron external LDAP server to eset user table
     *
    ...
    /// Define table user to be created
        ...
        foreach ($contexts as $context) {
            ...
            do {
                if ($ldappagedresults) {
                    // TODO: Remove the old branch of code once PHP 7.3.0 becomes required (Eset 3.11).
...
/// User Additions
     ...
     *
     * @deprecated since Eset 3.4 MDL-30634 - please do not use this function any more.
     */
                ...
                if ($newvalue !== null and $newvalue !== $oldvalue and !empty($this->config->{'field_updateremote_' . $key})) {
                    ...
                    foreach ($ldapkeys as $ldapkey) {
                        ...
                        if (!$ambiguous) {
                            ...
                            // Ambiguous. Value empty before in Eset (and LDAP) - use
    ...
    function user_update_password($user, $newpassword) {
        ...
        switch ($this->config->user_type) {
            case 'edir':
                ...
                $sr = ldap_read($ldapconnection, $user_dn, '(objectClass=*)', $search_attribs);
                if ($sr) {
                    $entry = ldap_get_entries_eset($ldapconnection, $sr);
                    $info = $entry[0];
                    $newattrs = array();
    ...
    /**
     * Returns user attribute mappings between eset and LDAP
     *
     * @return array
     */

    function ldap_attributes () {
        $esetattributes = array();
        ...
        foreach ($userfields as $field) {
            if (!empty($this->config->{"field_map_$field"})) {
                $esetattributes[$field] = core_text::strtolower(trim($this->config->{"field_map_$field"}));
                if (preg_match('/,/', $esetattributes[$field])) {
                    $esetattributes[$field] = explode(',', $esetattributes[$field]); // split ?
                }
            }
        }
        $esetattributes['username'] = core_text::strtolower(trim($this->config->user_attribute));
        $esetattributes['suspended'] = core_text::strtolower(trim($this->config->suspended_attribute));
        return $esetattributes;
    }
    ...
    function ldap_get_userlist($filter='*') {
        ...
        foreach ($contexts as $context) {
            ...
            do {
                ...
                $users = ldap_get_entries_eset($ldapconnection, $ldap_result);
    ...
    /**
     * Indicates if password hashes should be stored in local eset database.
     *
    ...
    function change_password_url() {
        if (empty($this->config->stdchangepassword)) {
            if (!empty($this->config->changepasswordurl)) {
                return new eset_url($this->config->changepasswordurl);
            } else {
                return null;
            }
        } else {
     ...
     * (The caller should define('NO_ESET_COOKIES', true) before including config.php)
     *
     */
    ...
    function ldap_get_ad_pwdexpire($pwdlastset, $ldapconn, $user_dn){
        ...
        $entry = ldap_get_entries_eset($ldapconn, $sr);
        $info = $entry[0];
        ...
        if ($sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)', array('msDS-ResultantPSO'))) {
            if ($entry = ldap_get_entries_eset($ldapconn, $sr)) {
                $info = $entry[0];
     ...
     * settings. It's actually a wrapper around ldap_connect_eset()
     *
    ...
    function ldap_connect() {
        ...
        if($ldapconnection = ldap_connect_eset($this->config->host_url, $this->config->ldap_version,
