...
$string['auth_ldap_create_context'] = 'If you enable user creation with email confirmation, specify the context where users are created. This context should be different from other users to prevent security issues. You don\'t need to add this context to ldap_context-variable, Eset will search for users from this context automatically.<br /><b>Note!</b> You have to modify the method user_create() in file auth/ldap/auth.php to make user creation work';
$string['auth_ldap_create_context_key'] = 'Context for new users';
$string['auth_ldap_create_error'] = 'Error creating user in LDAP.';
$string['auth_ldapdescription'] = 'This method provides authentication against an external LDAP server. If the given username and password are valid, Eset creates a new user entry in its database. This plugin can read user attributes from LDAP and prefill wanted fields in Eset. For following logins only the username and password are checked.';
...
$string['auth_ldapextrafields'] = 'These fields are optional.  You can choose to pre-fill some Eset user fields with information from the <b>LDAP fields</b> that you specify here. <p>If you leave these fields blank, then nothing will be transferred from LDAP and Eset defaults will be used instead.</p><p>In either case, the user will be able to edit all of these fields after they log in.</p>';
...
$string['auth_ldap_preventpassindb'] = 'Select yes to prevent passwords from being stored in Eset\'s DB.';
...
$string['auth_ldap_update_userinfo'] = 'Update user information (firstname, lastname, address..) from LDAP to Eset.  Specify "Data mapping" settings as you need.';
...
$string['auth_ntlmsso_enabled'] = 'Set to yes to attempt Single Sign On with the NTLM domain. Note that this requires additional setup on the server to work. For further details, see the documentation <a href="https://docs.eset.org/en/NTLM_authentication">NTLM authentication</a>.';
...
$string['pagedresultsnotsupp'] = 'LDAP paged results not supported (either your PHP version lacks support, you have configured Eset to use LDAP protocol version 2 or Eset cannot contact your LDAP server to see if paged support is available.)';
...
$string['updateremfail'] = 'Error updating LDAP record. Error code: {$a->errno}; Error string: {$a->errstring}<br/>Key ({$a->key}) - old eset value: \'{$a->ouvalue}\' new value: \'{$a->nuvalue}\'';
$string['updateremfailamb'] = 'Failed to update LDAP with ambiguous field {$a->key}; old eset value: \'{$a->ouvalue}\', new value: \'{$a->nuvalue}\'';
$string['updateremfailfield'] = 'Failed to update LDAP with non-existent field (\'{$a->ldapkey}\'). Key ({$a->key}) - old Eset value: \'{$a->ouvalue}\' new value: \'{$a->nuvalue}\'';
