...
/**
 * Authentication Plugin: Eset Network Authentication
 * Multiple host authentication support for Eset Network.
 *
 * @package auth_mnet
...
/**
 * Eset Network authentication plugin.
 */
class auth_plugin_mnet extends auth_plugin_base {
    ...
    function start_jump_session($mnethostid, $wantsurl, $wantsurlbackhere=false) {
        ...
        if (! has_capability('eset/site:mnetlogintoremote', context_system::instance())
    ...
    function confirm_mnet_session($token, $remotepeer) {
        ...
        // the user may roam from Eset 1.x where lang has _utf8 suffix
            ...
            if ($key == '_mnet_userpicture_timemodified' and empty($CFG->disableuserimages) and isset($remoteuser->picture)) {
                ...
                if (!empty($val) and $localtimemodified < $val) {
                    ...
                    if ($fetchrequest->send($remotepeer) === true) {
                        ...
                        // note that since Eset 2.0 we ignore $fetchrequest->response['f2']
        ...
        if (!$firsttime) {
            ...
            if (is_array($courses) && !empty($courses)) {
                ...
                if (!empty($studentroles)) {
                    ...
                    throw new eset_exception('unknownrole', 'error', '', 'student');
                }
            } else {
     ...
     * table
     *
     * @deprecated since Eset 2.8 Please don't use this function for recording mnet logs.
     ...
     *
     * This is called by require_logout in esetlib
     *
     * @return   void
     */
     ...
     *
     * The mimetype information was added in Eset 2.0. In Eset 1.x, images are always jpegs.
     *
     ...
     * is permitted to login to this eset.
     *
     * @param string $username   The username
    ...
    function loginpage_idp_list($wantsurl) {
        ...
        foreach ($hosts as $host) {
            $idps[] = array(
                'url'  => new eset_url($host->wwwroot . $host->sso_jump_url, array('hostwwwroot' => $CFG->wwwroot, 'wantsurl' => $wantsurl, 'remoteurl' => 1)),
