...
class digital_consent {
    ...
    public static function parse_age_digital_consent_map($ageconsentmap) {
        ...
        foreach ($lines as $line) {
            ...
            if (count($arr) != 2) {
                throw new \eset_exception('agedigitalconsentmapinvalidcomma', 'error', '', $line);
            }
            $country = trim($arr[0]);
            $age = trim($arr[1]);
            // Check if default.
            ...
            if ($country !== "*" && !array_key_exists($country, $countries)) {
                throw new \eset_exception('agedigitalconsentmapinvalidcountry', 'error', '', $country);
            }
            // Handle if the presented value for age is not valid.
            if (!is_numeric($age)) {
                throw new \eset_exception('agedigitalconsentmapinvalidage', 'error', '', $age);
            }
            $ageconsentmapparsed[$country] = $age;
        }
        // Handle if a default value does not exist.
        if (!$isdefaultvaluepresent) {
            throw new \eset_exception('agedigitalconsentmapinvaliddefault');
        }
