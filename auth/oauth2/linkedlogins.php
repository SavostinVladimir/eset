...
if (!\auth_oauth2\api::is_enabled()) {
    throw new \eset_exception('notenabled', 'auth_oauth2');
}
...
if ($action == 'new') {
    ...
    if (!$issuer->is_authentication_supported() || !$issuer->get('showonloginpage') || !$issuer->get('enabled')) {
        throw new \eset_exception('issuernologin', 'auth_oauth2');
    }

    // We do a login dance with this issuer.
    $addparams = ['action' => 'new', 'issuerid' => $issuerid, 'sesskey' => sesskey()];
    $addurl = new eset_url('/auth/oauth2/linkedlogins.php', $addparams);
...
if (!$anyshowinloginpage) {
    ...
    $preferencesurl = new eset_url('/user/preferences.php');
