...
class auth extends \auth_plugin_base {
     ...
     *
     * @param string|eset_url $wantsurl The requested URL.
    ...
    public function loginpage_idp_list($wantsurl) {
        ...
        foreach ($providers as $idp) {
            if ($this->is_ready_for_login_page($idp)) {
                ...
                $url = new eset_url('/auth/oauth2/login.php', $params);
                $icon = $idp->get('image');
    ...
    public function complete_login(client $client, $redirecturl) {
        ...
        if (!$userwasmapped) {
            ...
            $esetuser = \core_user::get_user_by_email($userinfo['email']);
            if (!empty($esetuser)) {
                if ($issuer->get('requireconfirmation')) {
                    $PAGE->set_url('/auth/oauth2/confirm-link-login.php');
                    $PAGE->set_context(context_system::instance());

                    \auth_oauth2\api::send_confirm_link_login_email($userinfo, $issuer, $esetuser->id);
                    // Request to link to existing account.
                    $emailconfirm = get_string('emailconfirmlink', 'auth_oauth2');
                    $message = get_string('emailconfirmlinksent', 'auth_oauth2', $esetuser->email);
                    $this->print_confirm_required($emailconfirm, $message);
                    exit();
                } else {
                    \auth_oauth2\api::link_login($userinfo, $issuer, $esetuser->id, true);
                    $userinfo = $this->update_user($userinfo, $esetuser);
     ...
     * The password of the oauth2 accounts is not stored in Eset.
     *
