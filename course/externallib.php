...
class core_course_external extends external_api {
     ...
     * @since Eset 2.9 Options available
     * @since Eset 2.2
     */
    public static function get_course_contents_parameters() {
        return new external_function_parameters(
                array('courseid' => new external_value(PARAM_INT, 'course id'),
                      ...
                      ), 'Options, used since Eset 2.9', VALUE_DEFAULT, array())
                )
        );
    }
     ...
     * @param array $options Options for filtering the results, used since Eset 2.9
     * @return array
     * @since Eset 2.9 Options available
     * @since Eset 2.2
     */
    public static function get_course_contents($courseid, $options = array()) {
                    ...
                    switch ($name) {
                        ...
                        case 'modid':
                            ...
                            if (is_numeric($value)) {
                                ...
                                throw new eset_exception('errorinvalidparam', 'webservice', '', $name);
                            }
                            break;
                        case 'modname':
        ...
        if ($course->id != SITEID) {
            ...
            if (!file_exists($CFG->dirroot . '/course/format/' . $course->format . '/lib.php')) {
                throw new eset_exception('cannotgetcoursecontents', 'webservice', '', null,
        ...
        try {
            ...
            throw new eset_exception('errorcoursecontextnotvalid', 'webservice', '', $exceptionparam);
        }

        $canupdatecourse = has_capability('eset/course:update', $context);

        //create return value
        $coursecontents = array();

        if ($canupdatecourse or $course->visible
                or has_capability('eset/course:viewhiddencourses', $context)) {
            ...
            foreach ($sections as $key => $section) {
                ...
                if (empty($filters['excludemodules']) and !empty($modinfosections[$section->section])) {
                    foreach ($modinfosections[$section->section] as $cmid) {
                        ...
                        $canviewhidden = has_capability('eset/course:viewhiddenactivities',
                                    ...
                                    'contents' => new external_multiple_structure(
                                          new external_single_structure(
                                              array(
                                                  ...
                                                  'userid' => new external_value(PARAM_INT, 'User who added this content to eset'),
    ...
    public static function get_courses($options = array()) {
        ...
        foreach ($courses as $course) {
            ...
            if ($course->id != SITEID) {
                require_capability('eset/course:view', $context);
            }
            ...
            $courseadmin = has_capability('eset/course:update', $context);
            if ($courseadmin) {
    ...
    public static function create_courses_parameters() {
        $courseconfig = get_config('esetcourse'); //needed for many default values
        return new external_function_parameters(
            array(
    ...
    public static function create_courses($courses) {
        ...
        foreach ($params['courses'] as $course) {
            ...
            try {
                ...
                throw new eset_exception('errorcatcontextnotvalid', 'webservice', '', $exceptionparam);
            }
            require_capability('eset/course:create', $context);

            // Fullname and short name are required to be non-empty.
            if (trim($course['fullname']) === '') {
                throw new eset_exception('errorinvalidparam', 'webservice', '', 'fullname');
            } else if (trim($course['shortname']) === '') {
                throw new eset_exception('errorinvalidparam', 'webservice', '', 'shortname');
            }

            // Make sure lang is valid
            if (array_key_exists('lang', $course)) {
                if (empty($availablelangs[$course['lang']])) {
                    throw new eset_exception('errorinvalidparam', 'webservice', '', 'lang');
                }
                if (!has_capability('eset/course:setforcedlanguage', $context)) {
                    unset($course['lang']);
                }
            }
            ...
            if (array_key_exists('forcetheme', $course)) {
                if (!empty($CFG->allowcoursethemes)) {
                    if (empty($availablethemes[$course['forcetheme']])) {
                        throw new eset_exception('errorinvalidparam', 'webservice', '', 'forcetheme');
                    } else {
            ...
            if (!has_capability('eset/course:visibility', $context)) {
        ...
        foreach ($params['courses'] as $course) {
            ...
            try {
                ...
                if (array_key_exists('fullname', $course) && ($oldcourse->fullname != $course['fullname'])) {
                    require_capability('eset/course:changefullname', $context);
                    if (trim($course['fullname']) === '') {
                        throw new eset_exception('errorinvalidparam', 'webservice', '', 'fullname');
                    }
                }
                ...
                if (array_key_exists('shortname', $course) && ($oldcourse->shortname != $course['shortname'])) {
                    require_capability('eset/course:changeshortname', $context);
                ...
                if (array_key_exists('idnumber', $course) && ($oldcourse->idnumber != $course['idnumber'])) {
                    require_capability('eset/course:changeidnumber', $context);
                }

                // Check if user can change summary.
                if (array_key_exists('summary', $course) && ($oldcourse->summary != $course['summary'])) {
                    require_capability('eset/course:changesummary', $context);
                }
                ...
                if (array_key_exists('visible', $course) && ($oldcourse->visible != $course['visible'])) {
                    require_capability('eset/course:visibility', $context);
                }

                // Make sure lang is valid.
                if (array_key_exists('lang', $course) && ($oldcourse->lang != $course['lang'])) {
                    require_capability('eset/course:setforcedlanguage', $context);
                    if (empty($availablelangs[$course['lang']])) {
                        throw new eset_exception('errorinvalidparam', 'webservice', '', 'lang');
                    }
                }
                ...
                if ($e instanceof eset_exception) {
    ...
    public static function duplicate_course($courseid, $fullname, $shortname, $categoryid, $visible = 1, $options = array()) {
        ...
        if (! ($course = $DB->get_record('course', array('id'=>$params['courseid'])))) {
            throw new eset_exception('invalidcourseid', 'error');
        }
        ...
        if (!empty($params['options'])) {
            foreach ($params['options'] as $option) {
                ...
                if ($value !== 0 and $value !== 1) {
                    throw new eset_exception('invalidextparam', 'webservice', '', $option['name']);
                }
        ...
        require_capability('eset/course:create', $categorycontext);
        require_capability('eset/restore:restorecourse', $categorycontext);
        require_capability('eset/backup:backupcourse', $coursecontext);

        if (!empty($backupsettings['users'])) {
            require_capability('eset/backup:userinfo', $coursecontext);
            require_capability('eset/restore:userinfo', $categorycontext);
        }
        ...
        if ($foundcourses = $DB->get_records('course', array('shortname'=>$shortname))) {
            ...
            throw new eset_exception('shortnametaken', '', '', $foundcoursenamestring);
        }

        // Backup the course.

        $bc = new backup_controller(backup::TYPE_1COURSE, $course->id, backup::FORMAT_ESET,
        ...
        if (!file_exists($backupbasepath . "/eset_backup.xml")) {
            $file->extract_to_pathname(get_file_packer('application/vnd.eset.backup'), $backupbasepath);
        }
        ...
        if (!$rc->execute_precheck()) {
            ...
            if (is_array($precheckresults) && !empty($precheckresults['errors'])) {
                ...
                throw new eset_exception('backupprecheckerrors', 'webservice', '', $errorinfo);
            }
        }
    ...
    public static function import_course($importfrom, $importto, $deletecontent = 0, $options = array()) {
        ...
        if ($params['deletecontent'] !== 0 and $params['deletecontent'] !== 1) {
            throw new eset_exception('invalidextparam', 'webservice', '', $params['deletecontent']);
        }
        ...
        require_capability('eset/backup:backuptargetimport', $importfromcontext);
        require_capability('eset/restore:restoretargetimport', $importtocontext);

        $bc = new backup_controller(backup::TYPE_1COURSE, $importfrom->id, backup::FORMAT_ESET,
                    ...
                    new external_single_structure(
                        array(
                            'key' => new external_value(PARAM_ALPHA,
                                         ...
                                         ' - user must have \'eset/category:manage\' to search on idnumber,'.
                                         ...
                                         ' - user must have \'eset/category:manage\' or \'eset/category:viewhiddencategories\' to search on visible,'.
                                         '"theme" (string) only return the categories having this theme'.
                                         ' - user must have \'eset/category:manage\' to search on theme'),
            ...
            foreach ($params['criteria'] as $crit) {
                ...
                if (!isset($conditions[$key])) {
                    ...
                    switch ($key) {
                        ...
                        case 'idnumber':
                            if (has_capability('eset/category:manage', $context)) {
                                ...
                                throw new eset_exception('criteriaerror',
                                        'webservice', '', null,
                        ...
                        case 'visible':
                            if (has_capability('eset/category:viewhiddencategories', $context)) {
                                $value = clean_param($crit['value'], PARAM_INT);
                                $conditions[$key] = $value;
                                $wheres[] = $key . " = :" . $key;
                            } else {
                        ...
                        new external_single_structure(
                            array(
                                ...
                                'theme' => new external_value(PARAM_THEME,
                                        'the new category theme. This option must be enabled on eset',
                                        VALUE_OPTIONAL),
                        )
                    )
                )
        ...
        foreach ($params['categories'] as $category) {
            if ($category['parent']) {
                if (!$DB->record_exists('course_categories', array('id' => $category['parent']))) {
                    throw new eset_exception('unknowcategory');
                }
            ...
            self::validate_context($context);
            require_capability('eset/category:manage', $context);
                ...
                'categories' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'descriptionformat' => new external_format_value('description', VALUE_DEFAULT),
                            'theme' => new external_value(PARAM_THEME,
                                    'the category theme. This option must be enabled on eset', VALUE_OPTIONAL),
                        )
                    )
                )
            )
    ...
    public static function update_categories($categories) {
        ...
        foreach ($params['categories'] as $cat) {
            ...
            self::validate_context($categorycontext);
            require_capability('eset/category:manage', $categorycontext);
        ...
        foreach ($params['categories'] as $category) {
            ...
            if ($category['recursive']) {
                ...
                if ($deletecat->can_delete_full()) {
                    ...
                    throw new eset_exception('youcannotdeletecategory', '', '', $deletecat->get_formatted_name());
                }
            } else {
                ...
                if (!$newparentcat->id) {
                    throw new eset_exception('movecatcontentstoroot');
                }
    ...
    public static function delete_modules($cmids) {
        ...
        foreach ($params['cmids'] as $cmid) {
            ...
            $modcontext = context_module::instance($cm->id);
            require_capability('eset/course:manageactivities', $modcontext);
    ...
    public static function view_course($courseid, $sectionnumber = 0) {
        ...
        if (!empty($params['sectionnumber'])) {
            ...
            if (!$coursesection->uservisible) {
                require_capability('eset/course:viewhiddensections', $context);
            }
        }
    ...
    protected static function get_course_public_information(core_course_list_element $course, $coursecontext) {
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            $fileurl = eset_url::make_webservice_pluginfile_url($file->get_contextid(), $file->get_component(),
    ...
    public static function search_courses($criterianame,
        ...
        if ($params['criterianame'] == 'modulelist' or $params['criterianame'] == 'blocklist') {
            require_capability('eset/site:config', context_system::instance());
        }
    ...
    public static function get_course_module($cmid) {
        ...
        if (has_capability('eset/course:manageactivities', $context)) {
    ...
    public static function get_courses_by_field($field = '', $value = '') {
        ...
        foreach ($courses as $course) {
            $context = context_course::instance($course->id);
            $canupdatecourse = has_capability('eset/course:update', $context);
            $canviewhiddencourses = has_capability('eset/course:viewhiddencourses', $context);
    ...
    public static function edit_module($action, $id, $sectionreturn = null) {
        ...
        $contextarray = [
                'eset/course:update',
                'eset/course:manageactivities',
                'eset/course:activityvisibility',
                'eset/course:sectionvisibility',
                'eset/course:movesections',
                'eset/course:setcurrentsection',
        ];
        ...
        switch($action) {
            ...
            case 'show':
            case 'stealth':
                require_capability('eset/course:activityvisibility', $modcontext);
                ...
                break;
            case 'duplicate':
                require_capability('eset/course:manageactivities', $coursecontext);
                require_capability('eset/backup:backuptargetimport', $coursecontext);
                require_capability('eset/restore:restoretargetimport', $coursecontext);
                if (!course_allowed_module($course, $cm->modname)) {
                    throw new eset_exception('No permission to create that activity');
                }
    ...
    public static function get_recent_courses(int $userid = 0, int $limit = 0, int $offset = 0, string $sort = null) {
        ...
        if ($userid != $USER->id and !has_capability('eset/user:viewdetails', $usercontext)) {
            return array();
        }
    ...
    public static function toggle_activity_recommendation(string $area, int $id): array {
        ...
        require_capability('eset/course:recommendactivity', $context);
