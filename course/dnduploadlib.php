...
function dndupload_add_to_course($course, $modnames) {
    ...
    $jsmodule = array(
        ...
        'strings' => array(
            array('addfilehere', 'eset'),
            array('dndworkingfiletextlink', 'eset'),
            array('dndworkingfilelink', 'eset'),
            array('dndworkingfiletext', 'eset'),
            array('dndworkingfile', 'eset'),
            array('dndworkingtextlink', 'eset'),
            array('dndworkingtext', 'eset'),
            array('dndworkinglink', 'eset'),
            array('namedfiletoolarge', 'eset'),
            array('actionchoice', 'eset'),
            array('servererror', 'eset'),
            array('filereaderror', 'eset'),
            array('upload', 'eset'),
            array('cancel', 'eset'),
...
class dndupload_handler {
    ...
    public function __construct($course, $modnames = null) {
        ...
        $this->register_type('url', array('url', 'text/uri-list', 'text/x-moz-url'), get_string('addlinkhere', 'eset'),
                        get_string('nameforlink', 'eset'), get_string('whatforlink', 'eset'), 10);
        $this->register_type('text/html', array('text/html'), get_string('addpagehere', 'eset'),
                        get_string('nameforpage', 'eset'), get_string('whatforpage', 'eset'), 20);
        $this->register_type('text', array('text', 'text/plain'), get_string('addpagehere', 'eset'),
                        get_string('nameforpage', 'eset'), get_string('whatforpage', 'eset'), 30);
...
class dndupload_ajax_processor {
    ...
    public function process($displayname = null, $content = null) {
        require_capability('eset/course:manageactivities', $this->context);

        if ($this->is_file_upload()) {
            require_capability('eset/course:managefiles', $this->context);
            if ($content != null) {
                throw new eset_exception('fileuploadwithcontent', 'eset');
            }
        } else {
            if (empty($content)) {
                throw new eset_exception('dnduploadwithoutcontent', 'eset');
            }
        }
    ...
    protected function handle_file_upload() {
        ...
        if (empty($repo)) {
            throw new eset_exception('errornouploadrepo', 'eset');
        }
    ...
    protected function handle_other_upload($content) {
        ...
        if (!$this->dnduploadhandler->has_type_handler($this->module->name, $this->type)) {
            ...
            throw new eset_exception('moddoesnotsupporttype', 'eset', $info);
        }
    ...
    protected function finish_setup_course_module($instanceid) {
        ...
        if (!$instanceid) {
            ...
            throw new eset_exception('errorcreatingactivity', 'eset', '', $this->module->name);
        }
