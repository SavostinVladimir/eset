 ...
 */
class course_edit_form extends esetform {
    protected $course;
    protected $context;
    ...
    function definition() {
        ...
        $mform    = $this->_form;
        $PAGE->requires->yui_module('eset-course-formatchooser', 'M.course.init_formatchooser',
        ...
        if (!empty($course->id) and !has_capability('eset/course:changefullname', $coursecontext)) {
        ...
        if (!empty($course->id) and !has_capability('eset/course:changeshortname', $coursecontext)) {
        ...
        if (empty($course->id)) {
            if (has_capability('eset/course:create', $categorycontext)) {
                $displaylist = core_course_category::make_categories_list('eset/course:create');
            ...
            if (has_capability('eset/course:changecategory', $coursecontext)) {
                $displaylist = core_course_category::make_categories_list('eset/course:changecategory');
        ...
        if (!empty($course->id)) {
            if (!has_capability('eset/course:visibility', $coursecontext)) {
                $mform->hardFreeze('visible');
                $mform->setConstant('visible', $course->visible);
            }
        } else {
            if (!guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext)) {
                $mform->hardFreeze('visible');
        ...
        if (!empty($course->id) and !has_capability('eset/course:changeidnumber', $coursecontext)) {
        ...
        if (!empty($course->id) and !has_capability('eset/course:changesummary', $coursecontext)) {
        ...
        if (isset($course->format)) {
            ...
            if (!in_array($course->format, $courseformats)) {
                ...
                $formcourseformats[$course->format] = get_string('withdisablednote', 'eset',
        ...
        if ((empty($course->id) && guess_if_creator_will_have_course_capability('eset/course:setforcedlanguage', $categorycontext))
                || (!empty($course->id) && has_capability('eset/course:setforcedlanguage', $coursecontext))) {
        ...
        if ((empty($course->id) && guess_if_creator_will_have_course_capability('eset/course:renameroles', $categorycontext))
                || (!empty($course->id) && has_capability('eset/course:renameroles', $coursecontext))) {
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
                ((empty($course->id) && guess_if_creator_will_have_course_capability('eset/course:tag', $categorycontext))
                || (!empty($course->id) && has_capability('eset/course:tag', $coursecontext)))) {
