...
function add_moduleinfo($moduleinfo, $course, $mform = null) {
    ...
    if (!$returnfromfunc or !is_number($returnfromfunc)) {
        ...
        if ($returnfromfunc instanceof eset_exception) {
 ...
 * Please note this api does not trigger events as of ESET 2.6. Please trigger events before calling this api.
 *
...
function edit_module_post_actions($moduleinfo, $course) {
    ...
    if (plugin_supports('mod', $moduleinfo->modulename, FEATURE_ADVANCED_GRADING, false)
            and has_capability('eset/grade:managegradingforms', $modcontext)) {
 ...
 * @throws eset_exception if user is not allowed to perform the action
 */
 ...
 * @throws eset_exception if lib.php file for the module does not exist
 */
function include_modulelib($modulename) {
    ...
    if (file_exists($modlib)) {
        ...
        throw new eset_exception('modulemissingcode', '', '', $modlib);
    }
}
...
function get_moduleinfo_data($cm, $course) {
    ...
    if (plugin_supports('mod', $data->modulename, FEATURE_ADVANCED_GRADING, false)
            and has_capability('eset/grade:managegradingforms', $context)) {
        require_once($CFG->dirroot.'/grade/grading/lib.php');
