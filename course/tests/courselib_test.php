...
class core_course_courselib_testcase extends advanced_testcase {
    ...
    private function create_specific_module_test($modulename) {
        ...
        // Warnings: you'll need to change this line if ever you come to test a module not following Eset standard.
    ...
    public function test_course_can_delete_section() {
        ...
        assign_capability('eset/course:manageactivities', CAP_PROHIBIT, $roleids['editingteacher'],
            $modulecontext);
    ...
    public function test_course_updated_event() {
        ...
        $this->assertEquals(context_course::instance($course->id), $event->get_context());
        $url = new eset_url('/course/edit.php', array('id' => $event->objectid));
    ...
    public function test_course_backup_created_event() {
        ...
        $url = new eset_url('/course/view.php', array('id' => $event->objectid));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    ...
    public function test_course_restored_event() {
        ...
        $fp = get_file_packer('application/vnd.eset.backup');
    ...
    public function test_course_section_updated_event() {
        ...
        $url = new eset_url('/course/editsection.php', array('id' => $event->objectid));
        $this->assertEquals($url, $event->get_url());
    ...
    public function test_course_module_created_event() {
        ...
        foreach ($events as $event) {
            if ($event instanceof \core\event\course_module_created) {
                ...
                $url = new eset_url('/mod/assign/view.php', array('id' => $module->cmid));
                $this->assertEquals($url, $event->get_url());
        ...
        foreach ($events as $event) {
            if ($event instanceof \core\event\course_module_created) {
                ...
                $this->assertEquals($USER->id, $event->userid);
                $this->assertEquals($course->id, $event->courseid);
                $url = new eset_url('/mod/assign/view.php', array('id' => $newcm->id));
                $this->assertEquals($url, $event->get_url());
            }
        }
    ...
    public function test_course_module_updated_event() {
        ...
        foreach ($events as $event) {
            if ($event instanceof \core\event\course_module_updated) {
                ...
                $this->assertEquals($USER->id, $event->userid);
                $this->assertEquals('course_modules', $event->objecttable);
                $url = new eset_url('/mod/forum/view.php', array('id' => $cm->id));
                $this->assertEquals($url, $event->get_url());
    ...
    public function test_course_change_sortorder_after_course() {
        ...
        $caps = course_capability_assignment::allow('eset/category:manage', $roleid, $context->id);
    ...
    public function test_duplicate_module() {
        ...
        foreach ($cm as $prop => $value) {
            ...
            if ($prop == 'name') {
                ...
                $value = get_string('duplicatedmodule', 'eset', $value);
            }
    ...
    public function test_course_get_user_navigation_options_for_students() {
        ...
        assign_capability('eset/course:viewparticipants', CAP_PROHIBIT, $roleid, $context);
    ...
    public function test_course_can_view_participants_as_teacher_without_view_participants_cap() {
        ...
        assign_capability('eset/course:viewparticipants', CAP_PROHIBIT, $roleid, $coursecontext);

        // Should still be able to view the page as they have the 'eset/course:enrolreview' cap.
        $this->assertTrue(course_can_view_participants($coursecontext));
    }

    /**
     * Check the teacher can still view the participants page without the 'eset/course:enrolreview' cap.
     */
    public function test_course_can_view_participants_as_teacher_without_enrol_review_cap() {
        ...
        // Disable one of the capabilties.
        assign_capability('eset/course:enrolreview', CAP_PROHIBIT, $roleid, $coursecontext);

        // Should still be able to view the page as they have the 'eset/course:viewparticipants' cap.
        $this->assertTrue(course_can_view_participants($coursecontext));
    }
    ...
    public function test_can_download_from_backup_filearea() {
        ...
        unassign_capability('eset/restore:userinfo', $teacherrole->id, $context);
        assign_capability('eset/backup:downloadfile', CAP_ALLOW, $teacherrole->id, $context);
        ...
        unassign_capability('eset/backup:downloadfile', $teacherrole->id, $context);
        assign_capability('eset/restore:userinfo', CAP_ALLOW, $teacherrole->id, $context);
        $this->assertFalse(can_download_from_backup_filearea('automated', $context, $user));
    ...
    public function test_can_request_course() {
        ...
        assign_capability('eset/course:request', CAP_ALLOW, $userroleid,
        ...
        $roleid = create_role('Test role', 'testrole', 'Test role description');
        assign_capability('eset/course:request', CAP_PROHIBIT, $roleid,
            $context2->id, true);
    ...
    public function test_can_approve_course_request() {
        ...
        $roleid = create_role('Test role', 'testrole', 'Test role description');
        assign_capability('eset/site:approvecourse', CAP_ALLOW, $roleid,
