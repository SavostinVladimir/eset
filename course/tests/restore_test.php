...
class core_course_restore_testcase extends advanced_testcase {
    ...
    protected function backup_course($courseid, $userid = 2) {
        ...
        $packer = get_file_packer('application/vnd.eset.backup');

        $bc = new backup_controller(backup::TYPE_1COURSE, $courseid, backup::FORMAT_ESET, backup::INTERACTIVE_NO,
    ...
    public function test_async_restore_course_info_in_existing_course() {
        ...
        $c1 = $dg->create_course(['shortname' => 'SN', 'fullname' => 'FN', 'summary' => 'DESC', 'summaryformat' => FORMAT_ESET,
            'startdate' => $startdate]);
        ...
        $this->assertEquals(FORMAT_ESET, $c2->summaryformat);
    ...
    public function test_restore_idnumber_in_existing_course_without_permissions() {
        ...
        $roleid = $this->create_role_with_caps('eset/course:changeidnumber', CAP_PROHIBIT);
    ...
    public function test_restore_course_info_in_new_course() {
        ...
        $c1 = $dg->create_course(['shortname' => 'SN', 'fullname' => 'FN', 'startdate' => $startdate,
            'summary' => 'DESC', 'summaryformat' => FORMAT_ESET]);
        $backupid = $this->backup_course($c1->id);
    ...
    public function test_restore_course_shortname_in_existing_course_without_permissions() {
        ...
        $managers = get_archetype_roles('manager');
        $manager = array_shift($managers);
        $roleid = $this->create_role_with_caps('eset/course:changeshortname', CAP_PROHIBIT);
        $dg->role_assign($manager->id, $u1->id);
        $dg->role_assign($roleid, $u1->id);
        ...
        $this->assertEquals(FORMAT_ESET, $restored->summaryformat);
    }
    ...
    public function test_restore_course_fullname_in_existing_course_without_permissions() {
        ...
        $managers = get_archetype_roles('manager');
        $manager = array_shift($managers);
        $roleid = $this->create_role_with_caps('eset/course:changefullname', CAP_PROHIBIT);
        $dg->role_assign($manager->id, $u1->id);
        $dg->role_assign($roleid, $u1->id);
    ...
    public function test_restore_course_summary_in_existing_course_without_permissions() {
        ...
        $managers = get_archetype_roles('manager');
        $manager = array_shift($managers);
        $roleid = $this->create_role_with_caps('eset/course:changesummary', CAP_PROHIBIT);
        $dg->role_assign($manager->id, $u1->id);
        $dg->role_assign($roleid, $u1->id);
    ...
    public function test_restore_course_startdate_in_existing_course_without_permissions() {
        ...
        $managers = get_archetype_roles('manager');
        $manager = array_shift($managers);
        $roleid = $this->create_role_with_caps('eset/restore:rolldates', CAP_PROHIBIT);
        $dg->role_assign($manager->id, $u1->id);
        $dg->role_assign($roleid, $u1->id);
