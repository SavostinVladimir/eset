...
class core_course_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function test_create_categories() {
        ...
        $roleid = $this->assignUserCapability('eset/category:manage', $contextid);
        ...
        $this->unassignUserCapability('eset/category:manage', $contextid, $roleid);
    ...
    public function test_get_categories() {
        ...
        $category1data['descriptionformat'] = FORMAT_ESET;
        ...
        $roleid = $this->assignUserCapability('eset/category:manage', $context->id);
        $this->assignUserCapability('eset/category:viewhiddencategories', $context->id, $roleid);
        ...
        foreach ($categories as $category) {
            ...
            $this->assertEquals($category['description'], format_text($generatedcat->description, FORMAT_ESET, array('para' => false)));
        ...
        $this->unassignUserCapability('eset/category:viewhiddencategories', $context->id, $roleid);
        ...
        $this->expectException('eset_exception');
        $categories = core_course_external::get_categories(array(
            array('key' => 'id', 'value' => $category1->id),
            array('key' => 'idnumber', 'value' => $category1->idnumber),
            array('key' => 'visible', 'value' => 1)), 0);
    ...
    public function test_create_course_numsections() {
        ...
        // Set the required capabilities by the external function.
        $contextid = context_system::instance()->id;
        $roleid = $this->assignUserCapability('eset/course:create', $contextid);
        $this->assignUserCapability('eset/course:visibility', $contextid, $roleid);
    ...
    public function test_create_courses() {
        ...
        $roleid = $this->assignUserCapability('eset/course:create', $contextid);
        $this->assignUserCapability('eset/course:visibility', $contextid, $roleid);
        $this->assignUserCapability('eset/course:setforcedlanguage', $contextid, $roleid);
        ...
        $course2['summaryformat'] = FORMAT_ESET;
        ...
        foreach ($createdcourses as $createdcourse) {
            ...
            if ($createdcourse['shortname'] == $course2['shortname']) {
                ...
                throw new eset_exception('Unexpected shortname');
            }
        }

        // Call without required capability
        $this->unassignUserCapability('eset/course:create', $contextid, $roleid);
        $this->expectException('required_capability_exception');
    ...
    public function test_get_courses () {
        ...
        $coursedata['summaryformat'] = FORMAT_ESET;
        ...
        // Set the required capabilities by the external function.
        $context = context_system::instance();
        $roleid = $this->assignUserCapability('eset/course:view', $context->id);
        $this->assignUserCapability('eset/course:update',
        ...
        foreach ($courses as $course) {
            ...
            $this->assertEquals($course['summary'], format_text($dbcourse->summary, FORMAT_ESET, array('para' => false)));
    ...
    private function prepare_get_course_contents_test() {
        ...
        $roleid = $this->assignUserCapability('eset/course:view', $context->id);
        $this->assignUserCapability('eset/course:update', $context->id, $roleid);
        $this->assignUserCapability('mod/data:view', $context->id, $roleid);

        $conditions = array('course' => $course->id, 'section' => 2);
        $DB->set_field('course_sections', 'summary', 'Text with iframe <iframe src="https://eset.org"></iframe>', $conditions);
    ...
    public function test_duplicate_course() {
        ...
        $roleid = $this->assignUserCapability('eset/course:create', $categorycontext->id);
        $this->assignUserCapability('eset/course:view', $categorycontext->id, $roleid);
        $this->assignUserCapability('eset/restore:restorecourse', $categorycontext->id, $roleid);
        $this->assignUserCapability('eset/backup:backupcourse', $coursecontext->id, $roleid);
        $this->assignUserCapability('eset/backup:configure', $coursecontext->id, $roleid);
        // Optional capabilities to copy user data.
        $this->assignUserCapability('eset/backup:userinfo', $coursecontext->id, $roleid);
        $this->assignUserCapability('eset/restore:userinfo', $categorycontext->id, $roleid);
    ...
    public function test_update_courses() {
        ...
        // Set the required capabilities by the external function.
        $contextid = context_system::instance()->id;
        $roleid = $this->assignUserCapability('eset/course:update', $contextid);
        $this->assignUserCapability('eset/course:changecategory', $contextid, $roleid);
        $this->assignUserCapability('eset/course:changelockedcustomfields', $contextid, $roleid);
        $this->assignUserCapability('eset/course:changefullname', $contextid, $roleid);
        $this->assignUserCapability('eset/course:changeshortname', $contextid, $roleid);
        $this->assignUserCapability('eset/course:changeidnumber', $contextid, $roleid);
        $this->assignUserCapability('eset/course:changesummary', $contextid, $roleid);
        $this->assignUserCapability('eset/course:visibility', $contextid, $roleid);
        $this->assignUserCapability('eset/course:viewhiddencourses', $contextid, $roleid);
        $this->assignUserCapability('eset/course:setforcedlanguage', $contextid, $roleid);
        ...
        foreach ($courses as $course) {
            ...
            if ($course['id'] == $course2['id']) {
                ...
                $this->assertEquals(FORMAT_ESET, $courseinfo->summaryformat);
        ...
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        $this->unassignUserCapability('eset/course:update', $contextid, $roleid);
        ...
        $this->assignUserCapability('eset/course:update', $contextid, $roleid);
        $this->unassignUserCapability('eset/course:changecategory', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        ...
        $this->assignUserCapability('eset/course:changecategory', $contextid, $roleid);
        $this->unassignUserCapability('eset/course:changefullname', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('eset/course:changefullname', $contextid, $roleid);
        $this->unassignUserCapability('eset/course:changeshortname', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('eset/course:changeshortname', $contextid, $roleid);
        $this->unassignUserCapability('eset/course:changeidnumber', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('eset/course:changeidnumber', $contextid, $roleid);
        $this->unassignUserCapability('eset/course:changesummary', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->unassignUserCapability('eset/course:visibility', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $course1['summaryformat'] = FORMAT_ESET;
        $courses = array($course1);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        $updatedcoursewarnings = external_api::clean_returnvalue(core_course_external::update_courses_returns(),
                                                                    $updatedcoursewarnings);
        ...
        $this->unassignUserCapability('eset/course:changelockedcustomfields', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
    ...
    public function test_delete_modules() {
        ...
        $this->assignUserCapability('eset/course:manageactivities', $modcontext->id);

        // Assign capabilities to delete module 2.
        $modcontext = context_module::instance($module2->cmid);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('eset/course:manageactivities', $modcontext->id, $newrole);
    ...
    public function test_get_recent_courses() {
        ...
        $this->assignUserCapability('eset/user:viewdetails', $usercontext, $teacherroleid);
