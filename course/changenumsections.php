...
require_capability('eset/course:update', context_course::instance($course->id));
require_sesskey();
...
if ($desirednumsections > $maxsections) {
    ...
    \core\notification::warning(get_string('maxsectionslimit', 'eset', $maxsections));
    $increase = null;
    $insertsection = null;
    $numsections = 0;
...
if (isset($courseformatoptions['numsections']) && $increase !== null) {
    ...
    if ($insertsection) {
        ...
        require_capability('eset/course:movesections', context_course::instance($course->id));
    }
    $sections = [];
