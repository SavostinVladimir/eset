...
function build_mnet_logs_array($hostid, $course, $user=0, $date=0, $order="l.time ASC", $limitfrom='', $limitnum='',
    ...
    //if ($course->groupmode == SEPARATEGROUPS and !has_capability('eset/course:managegroups', context_course::instance($course->id))) {
...
//  extra - contains extra string to include in any link
    ...
    if (empty($course)) {
        throw new eset_exception('courseidnotfound');
    }
...
function print_course_request_buttons($context) {
    ...
    if (course_request::can_request($context)) {
        ...
        echo $OUTPUT->single_button(new eset_url('/course/request.php', $params),
            get_string('requestcourse'), 'get');
    }
    /// Print a button to manage pending requests
    if (has_capability('eset/site:approvecourse', $context)) {
        $disabled = !$DB->record_exists('course_request', array());
        echo $OUTPUT->single_button(new eset_url('/course/pending.php'), get_string('coursespending'), 'get', array('disabled' => $disabled));
    }
}
...
function can_edit_in_category($categoryid = 0) {
    ...
    return has_any_capability(array('eset/category:manage', 'eset/course:create'), $context);
}
...
function set_coursemodule_name($id, $name) {
    ...
    if (\core_text::strlen($module->name) > 255) {
        throw new \eset_exception('maximumchars', 'eset', '', 255);
    }
...
function course_delete_module($cmid, $async = false) {
    ...
    if (file_exists($modlib)) {
        ...
        throw new eset_exception('cannotdeletemodulemissinglib', '', '', null,
    ...
    if (!function_exists($deleteinstancefunction)) {
        throw new eset_exception('cannotdeletemodulemissingfunc', '', '', null,
    ...
    if (!$deleteinstancefunction($cm->instance)) {
        throw new eset_exception('cannotdeletemoduleinstance', '', '', null,
    ...
    if (!delete_mod_from_section($cm->id, $cm->section)) {
        throw new eset_exception('cannotdeletemodulefromsection', '', '', null,
...
function course_can_delete_section($course, $section) {
    ...
    if (!has_all_capabilities(array('eset/course:movesections', 'eset/course:update'), $context)) {
        return false;
    }
    ...
    if (!empty($modinfo->sections[$section])) {
        foreach ($modinfo->sections[$section] as $cmid) {
            if (!has_capability('eset/course:manageactivities', context_module::instance($cmid))) {
                return false;
            }
        }
    }
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    $editcaps = array('eset/course:manageactivities', 'eset/course:activityvisibility', 'eset/role:assign');
    $dupecaps = array('eset/backup:backuptargetimport', 'eset/restore:restoretargetimport');
    ...
    $hasmanageactivities = has_capability('eset/course:manageactivities', $modcontext);

    if (!isset($str)) {
        $str = get_strings(array('delete', 'move', 'moveright', 'moveleft',
            'editsettings', 'duplicate', 'modhide', 'makeavailable', 'makeunavailable', 'modshow'), 'eset');
        $str->assign         = get_string('assignroles', 'role');
        $str->groupsnone     = get_string('clicktochangeinbrackets', 'eset', get_string("groupsnone"));
        $str->groupsseparate = get_string('clicktochangeinbrackets', 'eset', get_string("groupsseparate"));
        $str->groupsvisible  = get_string('clicktochangeinbrackets', 'eset', get_string("groupsvisible"));
    }

    $baseurl = new eset_url('/course/mod.php', array('sesskey' => sesskey()));
    ...
    if ($hasmanageactivities) {
        $actions['update'] = new action_menu_link_secondary(
            new eset_url($baseurl, array('update' => $mod->id)),
            new pix_icon('t/edit', '', 'eset', array('class' => 'iconsmall')),
            $str->editsettings,
    ...
    if ($hasmanageactivities && $indent >= 0) {
        ...
        $actions['moveright'] = new action_menu_link_secondary(
            new eset_url($baseurl, array('id' => $mod->id, 'indent' => '1')),
            new pix_icon($rightarrow, '', 'eset', array('class' => 'iconsmall')),
            $str->moveright,
        ...
        $actions['moveleft'] = new action_menu_link_secondary(
            new eset_url($baseurl, array('id' => $mod->id, 'indent' => '-1')),
            new pix_icon($leftarrow, '', 'eset', array('class' => 'iconsmall')),
            $str->moveleft,
    ...
    if (has_capability('eset/course:activityvisibility', $modcontext)) {
        ...
        if ($displayedoncoursepage) {
            $actions['hide'] = new action_menu_link_secondary(
                new eset_url($baseurl, array('hide' => $mod->id)),
                new pix_icon('t/hide', '', 'eset', array('class' => 'iconsmall')),
                $str->modhide,
            ...
            $actions['show'] = new action_menu_link_secondary(
                new eset_url($baseurl, array('show' => $mod->id)),
                new pix_icon('t/show', '', 'eset', array('class' => 'iconsmall')),
                $str->modshow,
        ...
        if ($stealth) {
            ...
            $actions['hide'] = new action_menu_link_secondary(
                new eset_url($baseurl, array('hide' => $mod->id)),
                new pix_icon('t/unblock', '', 'eset', array('class' => 'iconsmall')),
                $str->makeunavailable,
            ...
            $actions[$action] = new action_menu_link_secondary(
                new eset_url($baseurl, array($action => $mod->id)),
                new pix_icon('t/block', '', 'eset', array('class' => 'iconsmall')),
                $str->makeavailable,
    ...
    if (has_all_capabilities($dupecaps, $coursecontext) &&
            plugin_supports('mod', $mod->modname, FEATURE_BACKUP_ESET2) &&
            course_allowed_module($mod->get_course(), $mod->modname)) {
        $actions['duplicate'] = new action_menu_link_secondary(
            new eset_url($baseurl, array('duplicate' => $mod->id)),
            new pix_icon('t/copy', '', 'eset', array('class' => 'iconsmall')),
            $str->duplicate,
    ...
    if ($hasmanageactivities && !$mod->coursegroupmodeforce) {
        if (plugin_supports('mod', $mod->modname, FEATURE_GROUPS, false)) {
            ...
            $actions[$actionname] = new action_menu_link_primary(
                new eset_url($baseurl, array('id' => $mod->id, 'groupmode' => $nextgroupmode)),
                new pix_icon($groupimage, '', 'eset', array('class' => 'iconsmall')),
                $grouptitle,
    ...
    // Assign.
    if (has_capability('eset/role:assign', $modcontext)){
        $actions['assign'] = new action_menu_link_secondary(
            new eset_url('/admin/roles/assign.php', array('contextid' => $modcontext->id)),
            new pix_icon('t/assignroles', '', 'eset', array('class' => 'iconsmall')),
            $str->assign,
    ...
    if ($hasmanageactivities) {
        $actions['delete'] = new action_menu_link_secondary(
            new eset_url($baseurl, array('delete' => $mod->id)),
            new pix_icon('t/delete', '', 'eset', array('class' => 'iconsmall')),
            $str->delete,
...
function course_get_cm_move(cm_info $mod, $sr = null) {
    ...
    if ($hasmanageactivities) {
        ...
        return html_writer::link(
            new eset_url($baseurl, array('copy' => $mod->id)),
            $OUTPUT->pix_icon($pixicon, $str->move, 'eset', array('class' => 'iconsmall', 'title' => '')),
 ...
 * the course format supports Eset course ajax features.
 *
 * @param string $format
 * @return stdClass
 */
...
function can_delete_course($courseid) {
    ...
    if (has_capability('eset/course:delete', $context)) {
        return true;
    }

    // hack: now try to find out if creator created this course recently (1 day)
    if (!has_capability('eset/course:create', $context)) {
        return false;
    }
...
function create_course($data, $editoroptions = NULL) {
    ...
    if (!empty($data->shortname)) {
        if ($DB->record_exists('course', array('shortname' => $data->shortname))) {
            throw new eset_exception('shortnametaken', '', '', $data->shortname);
        }
    }
    ...
    if (!empty($data->idnumber)) {
        if ($DB->record_exists('course', array('idnumber' => $data->idnumber))) {
            throw new eset_exception('courseidnumbertaken', '', '', $data->idnumber);
        }
    }
    ...
    if ($errorcode = course_validate_dates((array)$data)) {
        throw new eset_exception($errorcode);
    }
...
function update_course($data, $editoroptions = NULL) {
    ...
    if ($data->id == SITEID) {
        throw new eset_exception('invalidcourse', 'error');
    }
...
class course_request {
    ...
    public static function create($data) {
        ...
        if ($users = get_users_from_config($CFG->courserequestnotify, 'eset/site:approvecourse')) {
     ...
     * misconfigured), we check capabilities 'eset/site:approvecourse' and 'eset/course:changecategory'.
     *
     * @return bool
     */
    public function can_approve() {
        ...
        if ($category) {
            return has_capability('eset/site:approvecourse', $category->get_context());
        }
        ...
        return has_all_capabilities(['eset/site:approvecourse', 'eset/course:changecategory'], context_system::instance());
    }
     ...
     * hidden categories if he has capabilities 'eset/site:approvecourse' and
     * 'eset/course:changecategory'
     *
    ...
    public function approve() {
        ...
        $courseconfig = get_config('esetcourse');
        ...
        if (!empty($CFG->creatornewroleid) and !is_viewing($context, $user, 'eset/role:assign') and !is_enrolled($context, $user, 'eset/role:assign')) {
        ...
        $this->notify($user, $USER, 'courserequestapproved', get_string('courseapprovedsubject'), get_string('courseapprovedemail2', 'eset', $a), $course->id);
    ...
    public function reject($notice) {
        ...
        $this->notify($user, $USER, 'courserequestrejected', get_string('courserejectsubject'), get_string('courserejectemail', 'eset', $notice));
        $this->delete();
    }
...
function include_course_ajax($course, $usedmodules = array(), $enabledmodules = null, $config = null) {
    ...
    if (course_format_uses_sections($course->format)) {
        $PAGE->requires->yui_module('eset-course-dragdrop', 'M.course.init_section_dragdrop',
            array(array(
                'courseid' => $course->id,
        ...
        $PAGE->requires->yui_module('eset-course-dragdrop', 'M.course.init_resource_dragdrop',
            array(array(
                'courseid' => $course->id,
 ...
 * @return eset_url The url of course
 */
 ...
 * @throws eset_exception if user is not allowed to perform the action or module is not allowed in this course
 */
function create_module($moduleinfo) {
    ...
    foreach($mandatoryfields as $mandatoryfield) {
        if (!isset($moduleinfo->{$mandatoryfield})) {
            throw new eset_exception('createmodulemissingattribut', '', '', $mandatoryfield);
        }
    }
 ...
 * @throws eset_exception if current user is not allowed to update the module
 */
 ...
 * @param int $sr The section to link back to (used for creating the links)
 * @throws eset_exception if the plugin doesn't support duplication
...
function duplicate_module($course, $cm) {
    ...
    if (!plugin_supports('mod', $cm->modname, FEATURE_BACKUP_ESET2)) {
        throw new eset_exception('duplicatenosupport', 'error', '', $a);
    }

    // Backup the activity.

    $bc = new backup_controller(backup::TYPE_1ACTIVITY, $cm->id, backup::FORMAT_ESET,
    ...
    if ($newcmid) {
        ...
        $newname = get_string('duplicatedmodule', 'eset', $newcm->name);
...
function course_get_user_navigation_options($context, $course = null) {
    ...
    $options->blogs = !empty($CFG->enableblogs) &&
                        ...
                        && has_capability('eset/blog:view', $sitecontext);

    $options->notes = !empty($CFG->enablenotes) && has_any_capability(array('eset/notes:manage', 'eset/notes:view'), $context);
    ...
    if ($isfrontpage) {
        ...
        $options->badges = !empty($CFG->enablebadges) && has_capability('eset/badges:viewbadges', $sitecontext);
        $options->tags = !empty($CFG->usetags) && $isloggedin;
        $options->search = !empty($CFG->enableglobalsearch) && has_capability('eset/search:query', $sitecontext);
        ...
        $options->badges = !empty($CFG->enablebadges) && !empty($CFG->badges_allowcoursebadges) &&
                            has_capability('eset/badges:viewbadges', $context);
        // Add view grade report is permitted.
        $grades = false;

        if (has_capability('eset/grade:viewall', $context)) {
            $grades = true;
    ...
    if (\core_competency\api::is_enabled()) {
        $capabilities = array('eset/competency:coursecompetencyview', 'eset/competency:coursecompetencymanage');
...
function course_get_user_administration_options($course, $context) {
    ...
    $options->update = has_capability('eset/course:update', $context);
    $options->editcompletion = $CFG->enablecompletion &&
                               $course->enablecompletion &&
                               ($options->update || $hascompletiontabs);
    $options->filters = has_capability('eset/filter:manage', $context) &&
                        count(filter_get_available_in_context($context)) > 0;
    $options->reports = has_capability('eset/site:viewreports', $context);
    $options->backup = has_capability('eset/backup:backupcourse', $context);
    $options->restore = has_capability('eset/restore:restorecourse', $context);
    $options->copy = \core_course\management\helper::can_copy_course($course->id);
    $options->files = ($course->legacyfiles == 2 && has_capability('eset/course:managefiles', $context));

    if (!$isfrontpage) {
        $options->tags = has_capability('eset/course:tag', $context);
        $options->gradebook = has_capability('eset/grade:manage', $context);
        $options->outcomes = !empty($CFG->enableoutcomes) && has_capability('eset/course:update', $context);
        $options->badges = !empty($CFG->enablebadges);
        $options->import = has_capability('eset/restore:restoretargetimport', $context);
        $options->reset = has_capability('eset/course:reset', $context);
        $options->roles = has_capability('eset/role:switchroles', $context);
    } else {
...
function course_filter_courses_by_timeline_classification(
    ...
    if (!in_array($classification,
        ...
        throw new eset_exception($message);
    }
...
function course_can_view_participants($context) {
    $viewparticipantscap = 'eset/course:viewparticipants';
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $viewparticipantscap = 'eset/site:viewparticipants';
    }

    return has_any_capability([$viewparticipantscap, 'eset/course:enrolreview'], $context);
}
...
function can_download_from_backup_filearea($filearea, \context $context, stdClass $user = null) {
    ...
    switch ($filearea) {
        case 'course':
        case 'backup':
            $candownload = has_capability('eset/backup:downloadfile', $context, $user);
            break;
        case 'automated':
            ...
            $candownload = has_capability('eset/backup:downloadfile', $context, $user) &&
                           has_capability('eset/restore:userinfo', $context, $user);
            break;
        default:
            break;
...
function course_get_recent_courses(int $userid = null, int $limit = 0, int $offset = 0, string $sort = null) {
    ...
    if ($userid != $USER->id && isset($hiddenfields['lastaccess'])) {
        $recentcourses = array_filter($recentcourses, function($course) {
            ...
            // fields.
            return has_capability('eset/course:viewhiddenuserfields', $context);
        });
    }
