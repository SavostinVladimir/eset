...
$url = new eset_url('/course/management.php');
...
if ($courseid) {
    ...
    if (empty($topchildren)) {
        throw new eset_exception('cannotviewcategory', 'error');
    }
...
if (!core_course_category::has_capability_on_any(array('eset/category:manage', 'eset/course:create'))) {
    // The user isn't able to manage any categories. Lets redirect them to the relevant course/index.php page.
    $url = new eset_url('/course/index.php');
    if ($categoryid) {
        $url->param('categoryid', $categoryid);
    }
    redirect($url);
...
if ($category && !has_any_capability($capabilities, $systemcontext)) {
    ...
    navigation_node::override_active_url(new eset_url('/course/management.php', array('categoryid' => $category->id)));
    ...
    navigation_node::override_active_url(new eset_url('/course/management.php'));
}
if (!$issearching && $category !== null) {
    ...
    foreach ($parents as $parent) {
        $PAGE->navbar->add(
            ...
            new eset_url('/course/management.php', array('categoryid' => $parent->id))
        );
    }
...
if ($action !== false && confirm_sesskey()) {
    ...
    switch ($action) {
        ...
        case 'deletecategory':
            ...
            echo $renderer->heading(get_string('deletecategory', 'eset', $category->get_formatted_name()));

            if ($data = $mform->get_data()) {
                // The form has been submit handle it.
                if ($data->fulldelete == 1 && $category->can_delete_full()) {
                    $continueurl = new eset_url('/course/management.php');
                    ...
                    $continueurl = new eset_url('/course/management.php', array('categoryid' => $data->newparent));
            ...
            if ($bulkmovecourses) {
                ...
                try {
                    ...
                    if ($redirectback) {
                        ...
                        $redirectmessage = get_string('bulkmovecoursessuccess', 'eset', $a);
                    }
                ...
                if ($movecount > 1) {
                    ...
                    $notificationspass[] = get_string($movesuccessstrkey, 'eset', $a);
                } else if ($movecount === 1) {
                    $a = new stdClass;
                ...
                if ($for === 'thiscategory') {
                    ...
                    $categorieslist = core_course_category::make_categories_list('eset/category:manage');
                ...
                if ($category === null && count($categoryids) === 1) {
                    ...
                    redirect(new eset_url($PAGE->url, array('categoryid' => reset($categoryids))));
                }
            }
    }
    if ($redirectback) {
