...
//  Display the course home page.
    ...
    if ($switchrole > 0 && confirm_sesskey() &&
        has_capability('eset/role:switchroles', $context)) {
    ...
    $PAGE->set_other_editing_capability('eset/course:update');
    $PAGE->set_other_editing_capability('eset/course:manageactivities');
    $PAGE->set_other_editing_capability('eset/course:activityvisibility');
    if (course_format_uses_sections($course->format)) {
        $PAGE->set_other_editing_capability('eset/course:sectionvisibility');
        $PAGE->set_other_editing_capability('eset/course:movesections');
    }
    ...
    if ($PAGE->user_allowed_editing()) {
        if (($edit == 1) and confirm_sesskey()) {
            ...
            if ($course->id == SITEID) {
                ...
                $url = new eset_url($PAGE->url, array('notifyeditingon' => 1));
                redirect($url);
            }
        ...
        if (has_capability('eset/course:sectionvisibility', $context)) {
        ...
        if (!empty($section) && !empty($move) &&
                has_capability('eset/course:movesections', $context) && confirm_sesskey()) {
    ...
    if ($section and $section > 0 and course_format_uses_sections($course->format)) {
        ...
        $PAGE->set_title(get_string('coursesectiontitle', 'eset', array('course' => $course->fullname, 'sectiontitle' => $sectiontitle, 'sectionname' => $sectionname)));
    } else {
        $PAGE->set_title(get_string('coursetitle', 'eset', array('course' => $course->fullname)));
    }
