...
if (has_capability('eset/site:approvecourse', $context)) {
    ...
    $categories = core_course_category::make_categories_list('eset/site:approvecourse');
    if (!$categories) {
        require_capability('eset/site:approvecourse', $context);
    }
    $PAGE->set_context($context);
    $PAGE->set_url(new eset_url('/course/pending.php'));
}
...
if (!empty($approve) and confirm_sesskey()) {
    ...
    if ($courseid !== false) {
        if (has_capability('eset/course:update', context_course::instance($courseid))) {
            redirect(new eset_url('/course/edit.php', ['id' => $courseid, 'returnto' => 'pending']));
        } else {
            redirect(new eset_url('/course/view.php', ['id' => $courseid]));
        }
    } else {
...
/// Build a table of all the requests.
    ...
    foreach ($pending as $course) {
        ...
        $row[] = $OUTPUT->single_button(new eset_url($baseurl, array('approve' => $course->id, 'sesskey' => sesskey())), get_string('approve'), 'get') .
                 $OUTPUT->single_button(new eset_url($baseurl, array('reject' => $course->id)), get_string('rejectdots'), 'get');
