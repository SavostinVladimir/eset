...
class core_course_category implements renderable, cacheable_object, IteratorAggregate {
     ...
     *     'eset/category:viewhiddencategories' capability). Use with care!
    ...
    public static function get($id, $strictness = MUST_EXIST, $alwaysreturnhidden = false, $user = null) {
        if (!$id) {
            ...
            if ($strictness == MUST_EXIST) {
                throw new eset_exception('cannotviewcategory');
            }
            return null;
        }
    ...
    public static function user_top() {
        ...
        if (count($children) > 1) {
            ...
            // In this case user actually may not have capability 'eset/category:viewcourselist' on the top level.
            return self::top();
        }
    ...
    public static function create($data, $editoroptions = null) {
        ...
        $newcategory->descriptionformat = FORMAT_ESET;
        ...
        if (empty($data->name)) {
            throw new eset_exception('categorynamerequired');
        }
        if (core_text::strlen($data->name) > 255) {
            throw new eset_exception('categorytoolong');
        }
    ...
    public static function can_view_category($category, $user = null) {
        if (!$category->id) {
            return has_capability('eset/category:viewcourselist', context_system::instance(), $user);
        }
        $context = context_coursecat::instance($category->id);
        if (!$category->visible && !has_capability('eset/category:viewhiddencategories', $context, $user)) {
            return false;
        }
        return has_capability('eset/category:viewcourselist', $context, $user);
    }
    ...
    public static function can_view_course_info($course, $user = null) {
        ...
        if (!$course->visible) {
            ...
            if (!has_capability('eset/course:viewhiddencourses', $coursecontext, $user)) {
                return false;
            }
        }
        ...
        return has_capability('eset/category:viewcourselist', $categorycontext, $user);
    }
    ...
    private static function rebuild_coursecattree_cache_contents() : array {
        ...
        if (!$count) {
            // No categories found.
            // This may happen after upgrade of a very old eset version.
     ...
     * @param bool $checkvisibility if true, capability 'eset/course:viewhiddencourses' will be checked
     *     on not visible courses and 'eset/category:viewcourselist' on all courses
     * @return array array of stdClass objects
     */
    ...
    public static function has_manage_capability_on_any() {
        return self::has_capability_on_any('eset/category:manage');
    }
    ...
    public function get_view_link() {
        return new \eset_url('/course/index.php', [
            'categoryid' => $this->id,
        ]);
    }
     ...
     * 'eset/category:manage' in ALL child course categories AND
    ...
    public function can_delete_full() {
        ...
        foreach ($childcategories as $childcat) {
            ...
            if ((!$childcat->visible && !has_capability('eset/category:viewhiddencategories', $childcontext)) ||
                    !has_capability('eset/category:manage', $childcontext)) {
                return false;
            }
        }
    ...
    public function delete_full($showfeedback = true) {
        ...
        if ($courses = $DB->get_records('course', array('category' => $this->id), 'sortorder ASC')) {
            foreach ($courses as $course) {
                if (!delete_course($course, false)) {
                    throw new eset_exception('cannotdeletecategorycourse', '', '', $course->shortname);
                }
        ...
        if (!$cb->delete_contents($this->get_context())) {
            throw new eset_exception('errordeletingcontentfromcategory', 'contentbank', '', $this->get_formatted_name());
        }
        if (!question_delete_course_category($this, null)) {
            throw new eset_exception('cannotdeletecategoryquestions', '', '', $this->get_formatted_name());
        }
    ...
    public function move_content_targets_list() {
        ...
        if (!$this->is_uservisible() ||
                !has_capability('eset/category:manage', $context)) {
    ...
    public function can_change_parent($newparentcat) {
        if (!has_capability('eset/category:manage', $this->get_context())) {
            return false;
        }
        ...
        if ($newparentcat->id) {
            return has_capability('eset/category:manage', context_coursecat::instance($newparentcat->id));
        } else {
            return has_capability('eset/category:manage', context_system::instance());
        }
    }
    ...
    protected function change_parent_raw(core_course_category $newparentcat) {
        ...
        if (empty($newparentcat->id)) {
            ...
            if ($newparentcat->id == $this->id || in_array($this->id, $newparentcat->get_parents())) {
                // Can not move to itself or it's own child.
                throw new eset_exception('cannotmovecategory');
            }
    ...
    public function has_manage_capability() {
        ...
        return has_capability('eset/category:manage', $this->get_context());
    }
    ...
    public function can_create_course() {
        return $this->is_uservisible() && has_capability('eset/course:create', $this->get_context());
    }
    ...
    public function can_review_roles() {
        return $this->is_uservisible() && has_capability('eset/role:assign', $this->get_context());
    }
    ...
    public function can_review_cohorts() {
        return $this->is_uservisible() &&
            has_any_capability(array('eset/cohort:view', 'eset/cohort:manage'), $this->get_context());
    }
    ...
    public function can_review_filters() {
        return $this->is_uservisible() &&
                has_capability('eset/filter:manage', $this->get_context()) &&
    ...
    public function can_restore_courses_into() {
        return $this->is_uservisible() && has_capability('eset/restore:restorecourse', $this->get_context());
    }
