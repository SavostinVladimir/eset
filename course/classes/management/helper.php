...
class helper {
    ...
    public static function get_course_detail_array(\core_course_list_element $course) {
        ...
        $categoryurl = new \eset_url('/course/management.php', array('categoryid' => $course->category));
        ...
        if (has_capability('eset/site:accessallgroups', $course->get_context())) {
        ...
        if ($canaccess) {
            ...
            foreach ($rolecounts as $result) {
                ...
                $roledetails[] = \get_string('assignedrolecount', 'eset', $a);
            }
    ...
    public static function get_category_listitem_actions(\core_course_category $category) {
        ...
        $manageurl = new \eset_url('/course/management.php', array('categoryid' => $category->id));
        $baseurl = new \eset_url($manageurl, array('sesskey' => \sesskey()));
        $actions = array();
        // Edit.
        if ($category->can_edit()) {
            $actions['edit'] = array(
                'url' => new \eset_url('/course/editcategory.php', array('id' => $category->id)),
        ...
        if ($category->can_change_visibility()) {
            ...
            $actions['hide'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'hidecategory')),
                'icon' => new \pix_icon('t/hide', new \lang_string('hide')),
                'string' => new \lang_string('hide')
            );
            $actions['show'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'showcategory')),
        ...
        if ($category->can_change_sortorder()) {
            $actions['moveup'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'movecategoryup')),
                'icon' => new \pix_icon('t/up', new \lang_string('moveup')),
                'string' => new \lang_string('moveup')
            );
            $actions['movedown'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'movecategorydown')),
        ...
        if ($category->can_resort_subcategories() && $category->has_children()) {
            $actions['resortbyname'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'resortcategories', 'resort' => 'name')),
                'icon' => new \pix_icon('t/sort', new \lang_string('sort')),
                'string' => new \lang_string('resortsubcategoriesby', 'eset' , get_string('categoryname'))
            );
            $actions['resortbynamedesc'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'resortcategories', 'resort' => 'namedesc')),
                'icon' => new \pix_icon('t/sort', new \lang_string('sort')),
                'string' => new \lang_string('resortsubcategoriesbyreverse', 'eset', get_string('categoryname'))
            );
            $actions['resortbyidnumber'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'resortcategories', 'resort' => 'idnumber')),
                'icon' => new \pix_icon('t/sort', new \lang_string('sort')),
                'string' => new \lang_string('resortsubcategoriesby', 'eset', get_string('idnumbercoursecategory'))
            );
            $actions['resortbyidnumberdesc'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'resortcategories', 'resort' => 'idnumberdesc')),
                'icon' => new \pix_icon('t/sort', new \lang_string('sort')),
                'string' => new \lang_string('resortsubcategoriesbyreverse', 'eset', get_string('idnumbercoursecategory'))
            );
        }
        ...
        if (!empty($category->move_content_targets_list()) || $category->can_delete_full()) {
            $actions['delete'] = array(
                'url' => new \eset_url($baseurl, array('action' => 'deletecategory')),
        ...
        if ($category->can_review_roles()) {
            $actions['assignroles'] = array(
                'url' => new \eset_url('/admin/roles/assign.php', array('contextid' => $category->get_context()->id,
        ...
        if ($category->can_review_permissions()) {
            $actions['permissions'] = array(
                'url' => new \eset_url('/admin/roles/permissions.php', array('contextid' => $category->get_context()->id,
                    'returnurl' => $manageurl->out_as_local_url(false))),
        ...
        if ($category->can_review_permissions()) {
            $actions['checkroles'] = array(
                'url' => new \eset_url('/admin/roles/check.php', array('contextid' => $category->get_context()->id,
                    'returnurl' => $manageurl->out_as_local_url(false))),
        ...
        // Context locking.
        if (!empty($CFG->contextlocking) && has_capability('eset/site:managecontextlocks', $category->get_context())) {
            ...
            if (empty($parentcontext) || !$parentcontext->locked) {
                ...
                $actions['managecontextlock'] = [
                    'url' => new \eset_url('/admin/lock.php', [
        ...
        if ($category->can_review_cohorts()) {
            $actions['cohorts'] = array(
                'url' => new \eset_url('/cohort/index.php', array('contextid' => $category->get_context()->id)),
        ...
        if ($category->can_review_filters()) {
            $actions['filters'] = array(
                'url' => new \eset_url('/filter/manage.php', array('contextid' => $category->get_context()->id,
                    'return' => 'management')),
        ...
        if ($category->can_restore_courses_into()) {
            $actions['restore'] = array(
                'url' => new \eset_url('/backup/restorefile.php', array('contextid' => $category->get_context()->id)),
        ...
        if (\tool_recyclebin\category_bin::is_enabled()) {
            ...
            if ($categorybin->can_view()) {
                ...
                if (!$autohide || !empty($items)) {
                    ...
                    $actions['recyclebin'] = [
                       'url' => new \eset_url('/admin/tool/recyclebin/index.php', ['contextid' => $category->get_context()->id]),
    ...
    public static function get_course_listitem_actions(\core_course_category $category, \core_course_list_element $course) {
        ...
        if ($course->can_edit()) {
            $actions[] = array(
                'url' => new \eset_url('/course/edit.php', array('id' => $course->id, 'returnto' => 'catmanage')),
        ...
        if (self::can_copy_course($course->id)) {
            $actions[] = array(
                'url' => new \eset_url('/backup/copy.php', array('id' => $course->id, 'returnto' => 'catmanage')),
        ...
        if ($course->can_delete()) {
            $actions[] = array(
                'url' => new \eset_url('/course/delete.php', array('id' => $course->id)),
        ...
        if ($course->can_change_visibility()) {
            $actions[] = array(
                'url' => new \eset_url($baseurl, array('action' => 'hidecourse')),
                'icon' => new \pix_icon('t/hide', \get_string('hide')),
                'attributes' => array('data-action' => 'hide', 'class' => 'action-hide')
            );
            $actions[] = array(
                'url' => new \eset_url($baseurl, array('action' => 'showcourse')),
        ...
        if ($category->can_resort_courses()) {
            $actions[] = array(
                'url' => new \eset_url($baseurl, array('action' => 'movecourseup')),
                'icon' => new \pix_icon('t/up', \get_string('moveup')),
                'attributes' => array('data-action' => 'moveup', 'class' => 'action-moveup')
            );
            $actions[] = array(
                'url' => new \eset_url($baseurl, array('action' => 'movecoursedown')),
    ...
    public static function get_course_detail_actions(\core_course_list_element $course) {
        ...
        if ($course->can_review_enrolments()) {
            $actions['enrolledusers'] = array(
                'url' => new \eset_url('/user/index.php', array('id' => $course->id)),
        ...
        if ($course->can_restore()) {
            $actions['restore'] = array(
                'url' => new \eset_url('/backup/restorefile.php', array('contextid' => $course->get_context()->id)),
    ...
    public static function action_course_change_sortorder_up_one(\core_course_list_element $course,
        ...
        if (!$category->can_resort_courses()) {
            throw new \eset_exception('permissiondenied', 'error', '', null, 'core_course_category::can_resort');
        }
    ...
    public static function action_course_change_sortorder_after_course($courserecordorid, $moveaftercourseid) {
        ...
        if (!$category->can_resort_courses()) {
            ...
            throw new \eset_exception('nopermissions', 'error', $url, \get_string('resortcourses', 'eset'));
        }
    ...
    public static function action_course_show(\core_course_list_element $course) {
        if (!$course->can_change_visibility()) {
            throw new \eset_exception('permissiondenied', 'error', '', null,
    ...
    public static function action_category_change_sortorder_up_one(\core_course_category $category) {
        if (!$category->can_change_sortorder()) {
            throw new \eset_exception('permissiondenied', 'error', '', null, 'core_course_category::can_change_sortorder');
        }
    ...
    public static function action_category_hide(\core_course_category $category) {
        if (!$category->can_change_visibility()) {
            throw new \eset_exception('permissiondenied', 'error', '', null, 'core_course_category::can_change_visbility');
        }
        $category->hide();
        return true;
    }
    ...
    public static function action_category_move_courses_into(\core_course_category $oldcategory,
        ...
        if ($DB->record_exists_sql($sql, $params)) {
            ...
            throw new \eset_exception('coursedoesnotbelongtocategory');
        }
    ...
    public static function move_courses_into_category($categoryorid, $courseids = array()) {
        ...
        if (!$moveto->can_move_courses_out_of() || !$moveto->can_move_courses_into()) {
            throw new \eset_exception('cannotmovecourses');
        }
        ...
        foreach ($courseids as $id) {
            if (!isset($courses[$id])) {
                throw new \eset_exception('invalidcourseid');
            }
    ...
    public static function get_course_copy_capabilities(): array {
        return array('eset/backup:backupcourse', 'eset/restore:restorecourse', 'eset/course:view', 'eset/course:create');
    }
