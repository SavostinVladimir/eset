YUI.add('eset-course-management', function (Y, NAME) {
...
Console.NAME = 'eset-course-management';
...
Console.prototype = {
    ...
    initializer: function() {
        Y.log('Initialising course category management console', 'info', 'eset-course-management');
    ...
    initialiseCategories: function(listing) {
        ...
        if (!this.categoriesinit) {
            ...
            Y.log(count + ' categories being managed', 'info', 'eset-course-management');
        } else {
            Y.log(count + ' new categories being managed', 'info', 'eset-course-management');
        }
    },
    ...
    initialiseCourses: function() {
        ...
        Y.log(count + ' courses being managed', 'info', 'eset-course-management');
    },
    ...
    handleCourseDelegation: function(e) {
        ...
        if (course) {
            ...
            Y.log('Course with ID ' + courseid + ' could not be found for delegation', 'error', 'eset-course-management');
        }
    },
    ...
    handleCategoryDelegation: function(e) {
        ...
        if (category) {
            ...
            Y.log('Could not find category to delegate to.', 'error', 'eset-course-management');
        }
    },
    ...
    performAjaxAction: function(action, args, callback, context) {
        ...
        if (callback === null) {
            callback = function() {
                Y.log("'Action '" + action + "' completed", 'debug', 'eset-course-management');
            };
        }
...
DragDrop.NAME = 'eset-course-management-dd';
...
DragDrop.prototype = {
    ...
    dropHit: function(e) {
        ...
        if (!drag.test('.listitem-course')) {
            Y.log('It was not a course being dragged.', 'warn', 'eset-course-management');
            return false;
        }
        ...
        if (iscategory) {
            ...
            Y.log('Course dropped over unhandled target.', 'info', 'eset-course-management');
        }
    }
};
...
Item.NAME = 'eset-course-management-item';
...
Item.prototype = {
    ...
    moveup: function(transactionid, response, args) {
        ...
        if (outcome === false) {
            Y.log('AJAX request to move ' + this.get('itemname') + ' up failed by outcome.', 'warn', 'eset-course-management');
            return false;
        }
        node = this.get('node');
        ...
        if (previous) {
            ...
            this.updated(true);
            Y.log('Success: ' + this.get('itemname') + ' moved up by AJAX.', 'info', 'eset-course-management');
        } else {
    ...
    movedown: function(transactionid, response, args) {
        var node,
            ...
            outcome = this.checkAjaxResponse(transactionid, response, args);
        if (outcome === false) {
            Y.log('AJAX request to move ' + this.get('itemname') + ' down failed by outcome.', 'warn', 'eset-course-management');
            return false;
        }
        node = this.get('node');
        ...
        if (next) {
            ...
            this.updated(true);
            Y.log('Success: ' + this.get('itemname') + ' moved down by AJAX.', 'info', 'eset-course-management');
        } else {
    ...
    show: function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            hidebtn;
        if (outcome === false) {
            Y.log('AJAX request to show ' + this.get('itemname') + ' by outcome.', 'warn', 'eset-course-management');
            return false;
        }
        ...
        this.updated();
        Y.log('Success: ' + this.get('itemname') + ' made visible by AJAX.', 'info', 'eset-course-management');
    },
    ...
    markVisible: function() {
        ...
        Y.log('Marked ' + this.get('itemname') + ' as visible', 'info', 'eset-course-management');
        return true;
    },
    ...
    hide: function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            showbtn;
        if (outcome === false) {
            Y.log('AJAX request to hide ' + this.get('itemname') + ' by outcome.', 'warn', 'eset-course-management');
            return false;
        }
        this.markHidden();
        ...
        this.updated();
        Y.log('Success: ' + this.get('itemname') + ' made hidden by AJAX.', 'info', 'eset-course-management');
    },
    ...
    markHidden: function() {
        ...
        Y.log('Marked ' + this.get('itemname') + ' as hidden', 'info', 'eset-course-management');
        return true;
    },
...
Category.NAME = 'eset-course-management-category';
...
Category.prototype = {
    ...
    handle: function(action, e) {
        ...
        switch (action) {
            ...
            case 'select':
                ...
                break;
            default:
                Y.log('Invalid AJAX action requested of managed category.', 'warn', 'eset-course-management');
                return false;
        }
    },
    ...
    expand: function() {
        ...
        action.setAttribute('data-action', 'collapse').setAttrs({
            title: M.util.get_string('collapsecategory', 'eset', this.getName())
        });
    ...
    collapse: function() {
        ...
        action.setAttribute('data-action', 'expand').setAttrs({
            title: M.util.get_string('expandcategory', 'eset', this.getName())
        });
    ...
    loadSubcategories: function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            ...
            actionnode;
        if (outcome === false) {
            Y.log('AJAX failed to load sub categories for ' + this.get('itemname'), 'warn', 'eset-course-management');
            return false;
        }
        Y.log('AJAX loaded subcategories for ' + this.get('itemname'), 'info', 'eset-course-management');
    ...
    moveCourseTo: function(course) {
        var self = this;
        Y.use('eset-core-notification-confirm', function() {
            var confirm = new M.core.confirm({
                title: M.util.get_string('confirm', 'eset'),
                question: M.util.get_string('confirmcoursemove', 'eset', {
                    course: course.getName(),
                    category: self.getName()
                }),
                yesLabel: M.util.get_string('move', 'eset'),
                noLabel: M.util.get_string('cancel', 'eset')
            });
    ...
    completeMoveCourse: function(transactionid, response, args) {
        var outcome = this.checkAjaxResponse(transactionid, response, args),
            ...
            totals;
        if (outcome === false) {
            Y.log('AJAX failed to move courses into this category: ' + this.get('itemname'), 'warn', 'eset-course-management');
            return false;
        }
        course = managementconsole.getCourseById(args.courseid);
        if (!course) {
            Y.log('Course was moved but the course listing could not be found to reflect this', 'warn', 'eset-course-management');
            return false;
        }
        Y.log('Moved the course (' + course.getName() + ') into this category (' + this.getName() + ')',
            'debug', 'eset-course-management');
        this.highlight();
        if (course) {
    ...
    show: function(transactionid, response, args) {
        ...
        this.updated();
        Y.log('Success: category made visible by AJAX.', 'info', 'eset-course-management');
    },
    ...
    updateCourseVisiblity: function(courses) {
        var managementconsole = this.get('console'),
            key,
            course;
        Y.log('Changing categories course visibility', 'info', 'eset-course-management');
        try {
            for (key in courses) {
            ...
            Y.log('Error trying to update course visibility: ' + err.message, 'warn', 'eset-course-management');
        }
        return this;
    },
    ...
    updateChildVisibility: function(categories) {
        var managementconsole = this.get('console'),
            key,
            category;
        Y.log('Changing categories subcategory visibility', 'info', 'eset-course-management');
        try {
            ...
            Y.log('Error trying to update category visibility: ' + err.message, 'warn', 'eset-course-management');
        }
        return this;
    }
};
...
Course.NAME = 'eset-course-management-course';
...
Course.prototype = {
    ...
    handle: function(action, e) {
        ...
        switch (action) {
            ...
            case 'select':
                ...
                break;
            default:
                Y.log('Invalid AJAX action requested of managed course.', 'warn', 'eset-course-management');
                return false;
        }
    },
    ...
    moveAfterResponse: function(transactionid, response, args) {
        ...
        if (outcome === false) {
            ...
            Y.log('AJAX failed to move this course after the requested course', 'warn', 'eset-course-management');
            if (previous) {
        ...
        Y.log('AJAX successfully moved course (' + this.getName() + ')', 'info', 'eset-course-management');
        this.highlight();
    }
};
