YUI.add('eset-course-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for course sections and course modules.
 *
 * @module eset-course-dragdrop
 */
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex, 'debug', 'eset-course-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'eset-course-dragdrop');
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    setup_for_resource: function(baseselector) {
        Y.Node.all(baseselector).each(function(resourcesnode) {
            ...
            if (move) {
                ...
                move.replace(this.get_drag_handle(M.util.get_string('movecoursemodule', 'eset'),
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Eset.core_course.util.cm.getId(dragnode));
        params.sectionId = Y.Eset.core_course.util.section.getId(dropnode.ancestor(M.course.format.get_section_wrapper(Y), true));

        if (dragnode.next()) {
            params.beforeId = Number(Y.Eset.core_course.util.cm.getId(dragnode.next()));
        }
