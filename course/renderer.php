...
class core_course_renderer extends plugin_renderer_base {
     ...
     *
     * Please see http://docs.eset.org/dev/Courses_lists_upgrade_to_2.5
     *
    ...
    function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        if ($rendernonajaxcontrol) {
            ...
            if (!has_capability('eset/course:manageactivities', context_course::instance($course->id))
    ...
    public function course_section_cm_completion($course, &$completioninfo, cm_info $mod, $displayoptions = array()) {
        ...
        if ($completionicon) {
            ...
            if ($isediting || !$istrackeduser || !has_capability('eset/course:togglecompletion', $mod->context)) {
                ...
                $output .= html_writer::start_tag('form', array('method' => 'post',
                    'action' => new eset_url('/course/togglecompletion.php'),
    ...
    protected function course_section_cm_classes(cm_info $mod) {
        ...
        if ($mod->uservisible) {
            ...
            $accessiblebutdim = (!$mod->visible || $conditionalhidden) &&
                has_capability('eset/course:viewhiddenactivities', $mod->context);
            if ($accessiblebutdim) {
                $linkclasses .= ' dimmed';
    ...
    public function course_section_cm_availability(cm_info $mod, $displayoptions = array()) {
        ...
        $canviewhidden = has_capability('eset/course:viewhiddenactivities', $modcontext);
    ...
    public function course_section_cm_list($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        if ($ismoving) {
            $movingpix = new pix_icon('movehere', get_string('movehere'), 'eset', array('class' => 'movetarget'));
        ...
        if (!empty($moduleshtml) || $ismoving) {
            foreach ($moduleshtml as $modnumber => $modulehtml) {
                if ($ismoving) {
                    $movingurl = new eset_url('/course/mod.php', array('moveto' => $modnumber, 'sesskey' => sesskey()));
            ...
            if ($ismoving) {
                $movingurl = new eset_url('/course/mod.php', array('movetosection' => $section->id, 'sesskey' => sesskey()));
                $sectionoutput .= html_writer::tag('li',
                        html_writer::link($movingurl, $this->output->render($movingpix), array('title' => $strmovefull)),
                        array('class' => 'movehere'));
            }
     ...
     * @param eset_url $paginationurl url to view more or url to form links to the other pages in paging bar
    ...
    protected function course_name(coursecat_helper $chelper, core_course_list_element $course): string {
        ...
        $coursenamelink = html_writer::link(new eset_url('/course/view.php', ['id' => $course->id]),
        ...
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            if ($course->has_summary() || $course->has_course_contacts() || $course->has_course_overviewfiles()
                ...
                $url = new eset_url('/course/info.php', ['id' => $course->id]);
        ...
        if ($course->has_course_contacts()) {
            ...
            foreach ($course->get_course_contacts() as $coursecontact) {
                ...
                $name = implode(", ", $rolenames).': '.
                    html_writer::link(new eset_url('/user/view.php',
    ...
    protected function course_overview_files(core_course_list_element $course): string {
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            $url = eset_url::make_file_url("$CFG->wwwroot/pluginfile.php",
            ...
            if ($isimage) {
                ...
                $image = $this->output->pix_icon(file_file_icon($file, 24), $file->get_filename(), 'eset');
        ...
        if ($chelper->get_show_courses() == self::COURSECAT_SHOW_COURSES_EXPANDED_WITH_CAT) {
            if ($cat = core_course_category::get($course->category, IGNORE_MISSING)) {
                ...
                $content .= get_string('category').': '.
                    html_writer::link(new eset_url('/course/index.php', ['categoryid' => $cat->id]),
    ...
    protected function coursecat_subcategories(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($totalcount > count($subcategories)) {
            if ($paginationurl) {
                ...
                if ($viewmoreurl->compare(new eset_url('/course/index.php'), URL_MATCH_BASE)) {
    ...
    protected function coursecat_include_js() {
        ...
        $this->page->requires->yui_module('eset-course-categoryexpander',
                'Y.Eset.course.categoryexpander.init');
    }
    ...
    protected function coursecat_category_content(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($chelper->get_show_courses() > core_course_renderer::COURSECAT_SHOW_COURSES_COUNT) {
            ...
            if ($viewmoreurl = $chelper->get_courses_display_option('viewmoreurl')) {
                ...
                if ($viewmoreurl->compare(new eset_url('/course/index.php'), URL_MATCH_BASE)) {
                    $chelper->set_courses_display_option('viewmoreurl', new eset_url($viewmoreurl, array('categoryid' => $coursecat->id)));
                }
            }
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        $categoryname = html_writer::link(new eset_url('/course/index.php',
    ...
    protected function coursecat_tree(coursecat_helper $chelper, $coursecat) {
        ...
        if ($coursecat->get_children_count()) {
            ...
            $content .= html_writer::end_tag('div');
            $this->page->requires->strings_for_js(array('collapseall', 'expandall'), 'eset');
        }
    ...
    public function course_category($category) {
        ...
        if ($coursecat->can_create_course() || $coursecat->has_manage_capability()) {
            ...
            $managebutton = $this->single_button(new eset_url('/course/management.php',
        ...
        if (core_course_category::is_simple_site()) {
            ...
            if (count($categorieslist) > 1) {
                ...
                $select = new single_select(new eset_url('/course/index.php'), 'categoryid',
        ...
        $baseurl = new eset_url('/course/index.php');
        if ($coursecat->id) {
        ...
        if ($browse === 'courses' || !$coursecat->get_children_count()) {
            ...
            $coursedisplayoptions['paginationurl'] = new eset_url($baseurl, array('browse' => 'courses'));
            $catdisplayoptions['nodisplay'] = true;
            $catdisplayoptions['viewmoreurl'] = new eset_url($baseurl, array('browse' => 'categories'));
            $catdisplayoptions['viewmoretext'] = new lang_string('viewallsubcategories');
        } else if ($browse === 'categories' || !$coursecat->get_courses_count()) {
            $coursedisplayoptions['nodisplay'] = true;
            $catdisplayoptions['offset'] = $page * $perpage;
            $catdisplayoptions['paginationurl'] = new eset_url($baseurl, array('browse' => 'categories'));
            $coursedisplayoptions['viewmoreurl'] = new eset_url($baseurl, array('browse' => 'courses'));
            $coursedisplayoptions['viewmoretext'] = new lang_string('viewallcourses');
        } else {
            // we have a category that has both subcategories and courses, display pagination separately
            $coursedisplayoptions['viewmoreurl'] = new eset_url($baseurl, array('browse' => 'courses', 'page' => 1));
            $catdisplayoptions['viewmoreurl'] = new eset_url($baseurl, array('browse' => 'categories', 'page' => 1));
        }
        ...
        if ($coursecat->is_uservisible()) {
            ...
            if (has_capability('eset/course:create', $context)) {
                ...
                if ($coursecat->id) {
                    $url = new eset_url('/course/edit.php', array('category' => $coursecat->id, 'returnto' => 'category'));
                } else {
                    $url = new eset_url('/course/edit.php',
    ...
    public function coursecat_ajax() {
        ...
        if ($type === self::COURSECAT_TYPE_CATEGORY) {
            ...
            $chelper = new coursecat_helper();
            $baseurl = new eset_url('/course/index.php', array('categoryid' => $categoryid));
            $coursedisplayoptions = array(
                'limit' => $CFG->coursesperpage,
                'viewmoreurl' => new eset_url($baseurl, array('browse' => 'courses', 'page' => 1))
            );
            $catdisplayoptions = array(
                'limit' => $CFG->coursesperpage,
                'viewmoreurl' => new eset_url($baseurl, array('browse' => 'categories', 'page' => 1))
            );
    ...
    public function search_courses($searchcriteria) {
        ...
        if (!empty($searchcriteria)) {
            ...
            $displayoptions['paginationurl'] = new eset_url('/course/search.php', $searchcriteria);
                ...
                foreach ($courses as $course) {
                    ...
                    if ($showcategories && ($cat = core_course_category::get($course->category, IGNORE_MISSING))) {
                        $details = get_string('category').': '.
                                html_writer::link(new eset_url('/course/index.php', array('categoryid' => $cat->id)),
    ...
    protected function frontpage_remote_course(stdClass $course) {
        $url = new eset_url('/auth/mnet/jump.php', array(
    ...
    public function frontpage_available_courses() {
        ...
        if (!$totalcount && !$this->page->user_is_editing() && has_capability('eset/course:create', context_system::instance())) {
            // Print link to create a new course, for the 1st available category.
    ...
    public function add_new_course_button() {
        ...
        // Print link to create a new course, for the 1st available category.
        $output = $this->container_start('buttons');
        $url = new eset_url('/course/edit.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
        $output .= $this->single_button($url, get_string('addnewcourse'), 'get');
    ...
    public function frontpage_section1() {
        ...
        if (($section && (!empty($modinfo->sections[1]) or !empty($section->summary))) or $editing) {
            ...
            if (ismoving($SITE->id)) {
                ...
                $cancelcopyurl = new eset_url('/course/mod.php', ['cancelcopy' => 'true', 'sesskey' => sesskey()]);
            ...
            if ($editing && has_capability('eset/course:update', $context)) {
                $streditsummary = get_string('editsummary');
                $editsectionurl = new eset_url('/course/editsection.php', ['id' => $section->id]);
    ...
    protected function frontpage_news($forum) {
        ...
        if (isloggedin()) {
            ...
            $suburl = new eset_url('/mod/forum/subscribe.php', array('id' => $forum->id, 'sesskey' => sesskey()));
