...
M.course_dndupload = {
    ...
    add_status_div: function() {
        ...
        div.setContent(M.util.get_string($msgident, 'eset'));
    ...
    drag_type: function(e) {
        ...
        if (this.types_includes(e, 'Files')) {
            if (e.type != 'drop' || e._event.dataTransfer.files.length != 0) {
                ...
                return {
                    realtype: 'Files',
                    addmessage: M.util.get_string('addfilehere', 'eset'),
    ...
    add_preview_element: function(section) {
        ...
        preview.namespan.innerHTML = M.util.get_string('addfilehere', 'eset');
    ...
    file_handler_dialog: function(handlers, extension, file, section, sectionnumber) {
        ...
        content += '<p>'+M.util.get_string('actionchoice', 'eset', file.name)+'</p>';
        ...
        panel.addButton({
            label: M.util.get_string('upload', 'eset'),
            action: function(e) {
                e.preventDefault();
        ...
        panel.addButton({
            label: M.util.get_string('cancel', 'eset'),
            action: function(e) {
                e.preventDefault();
                panel.hide();
            },
    ...
    upload_file: function(file, section, sectionnumber, module) {
        ...
        if (this.maxbytes > 0 && file.size > this.maxbytes) {
            new M.core.alert({message: M.util.get_string('namedfiletoolarge', 'eset', {filename: file.name})});
            return;
        }
        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    ...
                    new M.core.alert({message: M.util.get_string('servererror', 'eset')});
                }
        ...
        try {
            ...
            resel.parent.removeChild(resel.li);
            new M.core.alert({message: M.util.get_string('filereaderror', 'eset', file.name)});
            return;
        }
    ...
    add_editing: function(elementid) {
        ...
        YUI().use('eset-course-coursebase', function(Y) {
