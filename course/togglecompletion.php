...
if ($courseid) {
    $PAGE->set_url(new eset_url('/course/togglecompletion.php', array('course'=>$courseid)));
    ...
    if ($user && $rolec) {
        ...
        echo $OUTPUT->header();
        $buttoncontinue = new single_button(new eset_url('/course/togglecompletion.php', array('course'=>$courseid, 'confirm'=>1, 'sesskey'=>sesskey())), get_string('yes'), 'post');
        $buttoncancel   = new single_button(new eset_url('/course/view.php', array('id'=>$courseid)), get_string('no'), 'get');
...
require_capability('eset/course:togglecompletion', context_module::instance($cmid));
