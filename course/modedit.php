...
$url = new eset_url('/course/modedit.php');
...
if (!empty($add)) {
    ...
    if ($data->section && $course->format != 'site') {
        ...
        $pageheading = get_string('addinganewto', 'eset', $heading);
    } else {
        $pageheading = get_string('addinganew', 'eset', $fullmodulename);
    }
    ...
    navigation_node::override_active_url(new eset_url('/course/modedit.php', array('update'=>$update, 'return'=>1)));
    ...
    if ($data->section && $course->format != 'site') {
        ...
        $pageheading = get_string('updatingain', 'eset', $heading);
    } else {
        $pageheading = get_string('updatinga', 'eset', $fullmodulename);
    }
    $navbaraddition = null;
...
$modesetform = "$CFG->dirroot/mod/$module->name/mod_form.php";
if (file_exists($modesetform)) {
    require_once($modesetform);
} else {
...
if ($mform->is_cancelled()) {
    if ($return && !empty($cm->id)) {
        ...
        $activityurl = new eset_url("/mod/$module->name/view.php", $urlparams);
    ...
    if (isset($fromform->submitbutton)) {
        $url = new eset_url("/mod/$module->name/view.php", array('id' => $fromform->coursemodule, 'forceview' => 1));
    ...
    $streditinga = get_string('editinga', 'eset', $fullmodulename);
