...
if ($class === 'section' && $field === 'move') {
    if (!$DB->record_exists('course_sections', array('course' => $course->id, 'section' => $id))) {
        throw new eset_exception('AJAX commands.php: Bad Section ID ' . $id);
    }

    require_capability('eset/course:movesections', $coursecontext);
    ...
    if (!$section = $DB->get_record('course_sections', array('course' => $course->id, 'section' => $sectionid))) {
        throw new eset_exception('AJAX commands.php: Bad section ID '.$sectionid);
    }
