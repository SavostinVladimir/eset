...
$url = new eset_url('/course/user.php', array('id'=>$id,'user'=>$user, 'mode'=>$mode));

$course = $DB->get_record('course', array('id'=>$id), '*', MUST_EXIST);
$user = $DB->get_record("user", array("id"=>$user, 'deleted'=>0), '*', MUST_EXIST);

if ($mode === 'outline' or $mode === 'complete') {
    $url = new eset_url('/report/outline/user.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$mode));
    redirect($url);
}
if ($mode === 'todaylogs' or $mode === 'alllogs') {
    $logmode = ($mode === 'todaylogs') ? 'today' : 'all';
    $url = new eset_url('/report/log/user.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$logmode));
    redirect($url);
}
if ($mode === 'stats') {
    $url = new eset_url('/report/stats/user.php', array('id'=>$user->id, 'course'=>$course->id));
    redirect($url);
}
if ($mode === 'coursecompletions' or $mode === 'coursecompletion') {
    $url = new eset_url('/report/completion/user.php', array('id'=>$user->id, 'course'=>$course->id));
    redirect($url);
}
...
if (has_capability('eset/user:viewuseractivitiesreport', $personalcontext) and !is_enrolled($coursecontext)) {
...
$anyreport  = has_capability('eset/user:viewuseractivitiesreport', $personalcontext);
...
} else if ($course->showgrades and $user->id == $USER->id and has_capability('eset/grade:view', $coursecontext)) {
    //ok - can view own grades
    $modes[] = 'grade';

} else if ($course->showgrades and has_capability('eset/grade:viewall', $personalcontext)) {
...
if (empty($modes)) {
    require_capability('eset/user:viewuseractivitiesreport', $personalcontext);
}
...
switch ($mode) {
    case "grade":
        ...
        if ($USER->id == $user->id) {
            ...
            $url = new eset_url('/course/user.php', array('id' => $id, 'user' => $user->id, 'mode' => $mode));
            ...
            if (!preg_match('/^user\d{0,}$/', $activenode->key)) { // No user name found.
                $userurl = new eset_url('/user/view.php', array('id' => $user->id, 'course' => $course->id));
                // Add the user name.
                $PAGE->navbar->add(fullname($user), $userurl, navigation_node::TYPE_SETTING);
            }
            $PAGE->navbar->add(get_string('report'));
            $gradeurl = new eset_url('/course/user.php', array('id' => $id, 'user' => $user->id, 'mode' => $mode));
