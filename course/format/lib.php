 ...
 *
 * If you are upgrading from Eset 2.3 start with copying the class format_legacy and renaming
...
abstract class format_base {
    ...
    public function get_max_sections() {
        $maxsections = get_config('esetcourse', 'maxsections');
     ...
     *    arguments in the esetform::addElement() method
     ...
     *
     * @param EsetQuickForm $mform form the elements are added to
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (!$forsection && empty($this->courseid)) {
            ...
            // EsetQuickForm_date_time_selector class, otherwise it's disabled by default.
            if (get_config('esetcourse', 'courseenddateenabled')) {
     ...
     *
     * @param stdClass|array $data return value from {@link esetform::get_data()} or array with data
    ...
    /**
     * Return an instance of esetform to edit a specified section
     *
    ...
    /**
     * Allows course format to execute code on eset_page::set_course()
     *
     * @param eset_page $page instance of page calling set_course
     */
    public function page_set_course(eset_page $page) {
    }

    /**
     * Allows course format to execute code on eset_page::set_cm()
     *
     * Current module can be accessed as $page->cm (returns instance of cm_info)
     *
     * @param eset_page $page instance of page calling set_cm
     */
    public function page_set_cm(eset_page $page) {
    }
     ...
     */
    public function get_renderer(eset_page $page) {
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if ($editable === null) {
            $editable = !empty($USER->editing) && has_capability('eset/course:update',
     ...
     * esetcourse/courseduration setting. Course formats like format_weeks for
    ...
    public function get_default_course_enddate($mform, $fieldnames = array()) {
        ...
        $courseduration = intval(get_config('esetcourse', 'courseduration'));
        if (!$courseduration) {
    ...
    public function section_action($section, $action, $sr) {
        ...
        if (!$this->uses_sections() || !$section->section) {
            ...
            throw new eset_exception('sectionactionnotsupported', 'core', null, s($action));
        }
        ...
        switch($action) {
            case 'hide':
            case 'show':
                require_capability('eset/course:sectionvisibility', $coursecontext);
...
class format_site extends format_base {
     ...
     */
    public function get_view_url($section, $options = array()) {
        return new eset_url('/', array('redirect' => 0));
    }

    /**
     * Returns the list of blocks to be automatically added on the site frontpage when eset is installed
     *
     * @return array of default blocks, must contain two keys BLOCK_POS_LEFT and BLOCK_POS_RIGHT
     *     each of values is an array of block names (for left and right side columns)
     */
