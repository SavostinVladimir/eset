...
abstract class format_section_renderer_base extends plugin_renderer_base {
    ...
    /**
     * @deprecated since Eset 3.0 MDL-48947 - Use format_section_renderer_base::section_edit_control_items() instead
     */
    ...
    protected function section_edit_control_items($course, $section, $onsectionpage = false) {
        ...
        if (!$isstealth && has_capability('eset/course:update', $coursecontext)) {
            if ($section->section > 0
            ...
            $controls['edit'] = array(
                'url'   => new eset_url('/course/editsection.php', array('id' => $section->id, 'sr' => $sectionreturn)),
                'icon' => 'i/settings',
                'name' => $streditsection,
                'pixattr' => array('class' => ''),
        ...
        if ($section->section) {
            $url = clone($baseurl);
            if (!$isstealth) {
                if (has_capability('eset/course:sectionvisibility', $coursecontext)) {
                ...
                if (!$onsectionpage) {
                    if (has_capability('eset/course:movesections', $coursecontext)) {
                        $url = clone($baseurl);
            ...
            if (course_can_delete_section($course, $section)) {
                ...
                $url = new eset_url('/course/editsection.php', array(
                    'id' => $section->id,
                    'sr' => $sectionreturn,
                    'delete' => 1,
                    'sesskey' => sesskey()));
    ...
    public function section_availability($section) {
        ...
        $canviewhidden = has_capability('eset/course:viewhiddensections', $context);
    ...
    protected function course_activity_clipboard($course, $sectionno = null) {
        ...
        if (ismoving($course->id)) {
            $url = new eset_url('/course/mod.php',
    ...
    protected function get_nav_links($course, $sections, $sectionno) {
        ...
        $canviewhidden = has_capability('eset/course:viewhiddensections', context_course::instance($course->id))
    ...
    public function print_multiple_section_page($course, $sections, $mods, $modnames, $modnamesused) {
        ...
        if ($this->page->user_is_editing() and has_capability('eset/course:update', $context)) {
    ...
    protected function change_number_sections($course, $sectionreturn = null) {
        $coursecontext = context_course::instance($course->id);
        if (!has_capability('eset/course:update', $coursecontext)) {
            return '';
        }
        ...
        if ($supportsnumsections) {
            ...
            if ($lastsection < $maxsections) {
                $straddsection = get_string('increasesections', 'eset');
                $url = new eset_url('/course/changenumsections.php',
            ...
            if ($course->numsections > 0) {
                ...
                $strremovesection = get_string('reducesections', 'eset');
                $url = new eset_url('/course/changenumsections.php',
                    array('courseid' => $course->id,
                          'increase' => false,
                          'sesskey' => sesskey()));
            ...
            // capabilities 'eset/course:update' and 'eset/course:movesections'.
            echo html_writer::start_tag('div', array('id' => 'changenumsections', 'class' => 'mdl-right'));
