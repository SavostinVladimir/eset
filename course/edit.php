...
if ($returnto === 'url' && confirm_sesskey() && $returnurl) {
    ...
    if (!empty($id)) {
        $returnurl = new eset_url($CFG->wwwroot . '/course/view.php', array('id' => $id));
    } else {
        $returnurl = new eset_url($CFG->wwwroot . '/course/');
    }
    if ($returnto !== 0) {
        switch ($returnto) {
            case 'category':
                $returnurl = new eset_url($CFG->wwwroot . '/course/index.php', array('categoryid' => $categoryid));
                break;
            case 'catmanage':
                $returnurl = new eset_url($CFG->wwwroot . '/course/management.php', array('categoryid' => $categoryid));
                break;
            case 'topcatmanage':
                $returnurl = new eset_url($CFG->wwwroot . '/course/management.php');
                break;
            case 'topcat':
                $returnurl = new eset_url($CFG->wwwroot . '/course/');
                break;
            case 'pending':
                $returnurl = new eset_url($CFG->wwwroot . '/course/pending.php');
                break;
        }
    }
}
...
if ($id) {
    ...
    require_capability('eset/course:update', $coursecontext);
    ...
    require_capability('eset/course:create', $catcontext);
...
if ($editform->is_cancelled()) {
    ...
    if (empty($course->id)) {
        ...
        if (!empty($CFG->creatornewroleid) and !is_viewing($context, NULL, 'eset/role:assign') and !is_enrolled($context, NULL, 'eset/role:assign')) {
        ...
        $courseurl = new eset_url('/course/view.php', array('id' => $course->id));
            ...
            foreach($instances as $instance) {
                if ($plugin = enrol_get_plugin($instance->enrol)) {
                    if ($plugin->get_manual_enrol_link($instance)) {
                        ...
                        $courseurl = new eset_url('/user/index.php', array('id' => $course->id, 'newcourse' => 1));
                        break;
                    }
                }
            }
...
if (!empty($course->id)) {
    ...
    $managementurl = new eset_url('/course/management.php');
    // These are the caps required in order to see the management interface.
    $managementcaps = array('eset/category:manage', 'eset/course:create');
