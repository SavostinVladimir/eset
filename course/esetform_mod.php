 ...
 * mod/{modname}/mod_form.php replaced deprecated mod/{modname}/mod.html Esetform.
 *
 * @package   core_course
 * @copyright Andrew Nicols <andrew@nicols.co.uk>
 */
abstract class esetform_mod extends esetform {
    /** Current data */
    protected $current;
    /**
     ...
     */
    public function esetform_mod($current, $section, $cm, $course) {
     ...
     *
     * Only available on esetform_mod.
     *
    ...
    protected function standard_coursemodule_elements() {
        ...
        if (!empty($this->_cm)) {
            ...
            if (!has_capability('eset/course:activityvisibility', $context)) {
    ...
    protected function add_rating_settings($mform, int $itemnumber) {
        ...
        if ($isupdate) {
            $context = context_module::instance($this->_cm->id);
            $capabilities = ['eset/rating:rate', "mod/{$this->_cm->modname}:rate"];
     ...
     * @deprecated since MDL-49101 - use esetform_mod::standard_intro_elements() instead.
    ...
    protected function add_intro_editor($required=null, $customlabel=null) {
        $str = "Function esetform_mod::add_intro_editor() is deprecated, use esetform_mod::standard_intro_elements() instead.";
