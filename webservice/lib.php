 ...
 * @copyright  2009 Jerome Mouneyrac <jerome@eset.com>
...
/**
 * WEBSERVICE_AUTHMETHOD_SESSION_TOKEN - token for embedded application (requires Eset session)
 */
 ...
 *
 * @package    core_webservice
 * @copyright  2010 Jerome Mouneyrac <jerome@eset.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice {
    /**
    ...
    public function authenticate_user($token) {
        ...
        if (!$token = $DB->get_record('external_tokens', array('token' => $token))) {
            //client may want to display login form => eset_exception
            throw new eset_exception('invalidtoken', 'webservice');
        }
        ...
        $hasmaintenanceaccess = has_capability('eset/site:maintenanceaccess', context_system::instance(), $user);
        if (!empty($CFG->maintenance_enabled) and !$hasmaintenanceaccess) {
            //this is usually temporary, client want to implement code logic  => eset_exception
            throw new eset_exception('sitemaintenance', 'admin');
        }
        ...
        if (empty($user->confirmed)) {
            ...
            $event->trigger();
            throw new eset_exception('usernotconfirmed', 'eset', '', $user->username);
        }
        ...
        if (!empty($auth->config->expiration) and $auth->config->expiration == 1) {
            ...
            if (intval($days2expire) < 0) {
                ...
                $event->trigger();
                throw new eset_exception('passwordisexpired', 'webservice');
            }
        }
    ...
    public function generate_user_ws_tokens($userid) {
        ...
        if (!is_siteadmin() && has_capability('eset/webservice:createtoken', context_system::instance(), $userid) && !empty($CFG->enablewebservices)) {
     ...
     *
     * @param string $token
     * @throws eset_exception if there is multiple result
     */
     ...
     *    (
     *       [0] => eset/course:managegroups
     *    )
     *
     *    [core_enrol_get_enrolled_users] => Array
     *    (
     *       [0] => eset/user:viewdetails
     *       [1] => eset/user:viewhiddendetails
     *       [2] => eset/course:useremail
     *       [3] => eset/user:update
     *       [4] => eset/site:accessallgroups
     *    )
     *  )
     * @param int $serviceid service id
     * @return array
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice_access_exception extends eset_exception {
...
abstract class webservice_server implements webservice_server_interface {
    ...
    protected function authenticate_user() {
        ...
        if ($this->authmethod == WEBSERVICE_AUTHMETHOD_USERNAME) {
            ...
            if (!$this->username) {
                throw new eset_exception('missingusername', 'webservice');
            }

            if (!$this->password) {
                throw new eset_exception('missingpassword', 'webservice');
            }
            ...
            if (!$auth->user_login_webservice($this->username, $this->password)) {
                ...
                throw new eset_exception('wrongusernamepassword', 'webservice');
            }
        ...
        if (empty($user->confirmed)) {
            ...
            $event->set_legacy_logdata(array(SITEID, '', '', '', get_string('wsaccessuserunconfirmed', 'webservice',
                $user->username) . " - ".getremoteaddr(), 0, $user->id));
            $event->trigger();
            throw new eset_exception('wsaccessuserunconfirmed', 'webservice', '', $user->username);
        }
    ...
    /**
     * Intercept some esetwssettingXXX $_GET and $_POST parameter
    ...
    protected function set_web_service_call_settings() {
        ...
        // Must be the same XXX ws parameter name as 'esetwssettingXXX'.
        ...
        foreach ($externalsettings as $name => $settingdata) {
            ...
            $wsparamname = 'esetwssetting' . $name;
    ...
    protected function get_virtual_method_code($function) {
        ...
        foreach ($function->parameters_desc->keys as $name => $keydesc) {
            ...
            if ($keydesc->required == VALUE_OPTIONAL) {
                ...
                throw new eset_exception('erroroptionalparamarray', 'webservice', '', $name);
                ...
                if ($keydesc instanceof external_value) {
                    ...
                    if (isset($keydesc->default) && is_array($keydesc->default) && empty($keydesc->default)) {
                        ...
                        throw new eset_exception('errornotemptydefaultparamarray', 'webservice', '', $name);
                    }
                }
            }
