...
class core_webservice_renderer extends plugin_renderer_base {
    ...
    public function admin_authorised_user_selector(&$options) {
        ...
        $actionurl = new eset_url('/' . $CFG->admin . '/webservice/service_users.php',
    ...
    public function admin_authorised_user_list($users, $serviceid) {
        ...
        foreach ($users as $user) {
            $modifiedauthoriseduserurl = new eset_url('/' . $CFG->admin . '/webservice/service_user_settings.php',
    ...
    public function admin_remove_service_function_confirmation($function, $service) {
        ...
        $formcontinue = new single_button(new eset_url('service_functions.php',
                                $optionsyes), get_string('remove'));
        $formcancel = new single_button(new eset_url('service_functions.php',
    ...
    public function admin_remove_service_confirmation($service) {
        ...
        $formcontinue = new single_button(new eset_url('service.php', $optionsyes),
                        get_string('delete'), 'post');
        $formcancel = new single_button(
                        new eset_url($CFG->wwwroot . "/" . $CFG->admin . "/settings.php", $optionsno),
    ...
    public function admin_delete_token_confirmation($token) {
        ...
        $formcontinue = new single_button(
                        new eset_url('/' . $CFG->admin . '/webservice/tokens.php', $optionsyes),
                        get_string('delete'));
        $formcancel = new single_button(
                        new eset_url('/' . $CFG->admin . '/settings.php', $optionsno),
                        get_string('cancel'), 'get');
    ...
    public function admin_service_function_list($functions, $service) {
        ...
        if (!empty($functions)) {
            ...
            foreach ($functions as $function) {
                ...
                //display remove function operation (except for build-in service)
                if (empty($service->component)) {
                    $removeurl = new eset_url('/' . $CFG->admin . '/webservice/service_functions.php',
        ...
        if (empty($service->component)) {
            ...
            $addurl = new eset_url('/' . $CFG->admin . '/webservice/service_functions.php',
    ...
    public function user_reset_token_confirmation($token) {
        ...
        $optionsyes = array('tokenid' => $token->id, 'action' => 'resetwstoken', 'confirm' => 1,
            'sesskey' => sesskey());
        $optionsno = array('section' => 'webservicetokens', 'sesskey' => sesskey());
        $formcontinue = new single_button(new eset_url($managetokenurl, $optionsyes),
                        get_string('reset'));
        $formcancel = new single_button(new eset_url($managetokenurl, $optionsno),
                        get_string('cancel'), 'get');
    ...
    public function user_webservice_tokens_box($tokens, $userid, $documentation = false) {
        ...
        if (!empty($tokens)) {
            foreach ($tokens as $token) {
                ...
                $userprofilurl = new eset_url('/user/view.php?id=' . $token->creatorid);
                ...
                if ($documentation) {
                    $doclink = new eset_url('/webservice/wsdoc.php',
...
EOF;
        ...
        $docurl = new eset_url('http://docs.eset.org/dev/Creating_a_web_service_client');
        ...
        $url = new eset_url($parenturl, $authparams); // Required
