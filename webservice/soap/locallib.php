...
class webservice_soap_server extends webservice_base_server {
    ...
    protected function parse_request() {
        ...
        if ($this->authmethod == WEBSERVICE_AUTHMETHOD_USERNAME) {
            ...
            $this->serverurl = new eset_url('/webservice/soap/simpleserver.php/' . $this->username . '/' . $this->password);
        } else {
            $this->token = optional_param('wstoken', null, PARAM_RAW);

            $this->serverurl = new eset_url('/webservice/soap/server.php');
    ...
    protected function send_error($ex = null) {
        ...
        // Faultcode node.
        $fault->appendChild($dom->createElement('faultcode', 'ESET:error'));
        // Faultstring node.
     ...
     *
     * Eset note: basically we return the faultactor (errorcode) and faultdetails (debuginfo).
     *
