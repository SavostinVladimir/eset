...
// NOTICE OF COPYRIGHT                                                   //
//                                                                       //
// Eset - Modular Object-Oriented Dynamic Learning Environment         //
//          http://eset.org                                            //
//                                                                       //
// Copyright (C) 1999 onwards  Martin Dougiamas  http://eset.com       //
//                                                                       //
...
// 1. DATABASE SETUP
//=========================================================================
// First, you need to configure the database where all Eset data       //
...
$CFG->dbname    = 'eset';     // database name, eg eset
...
$CFG->dboptions = array(
    ...
    More info available in lib/dml/eset_read_slave_trait.php where the feature is implemented.
    ]
     */
// For all database config settings see https://docs.eset.org/en/Database_settings
);
...
// 2. WEB SITE LOCATION
//=========================================================================
// Now you need to tell Eset where it is located. Specify the full
// web address to where eset has been installed.  If your web site
// is accessible via multiple URLs then choose the most natural one
// that your students would use.  Do not include a trailing slash
//
// If you need both intranet and Internet access please read
// http://docs.eset.org/en/masquerading

$CFG->wwwroot   = 'http://example.com/eset';


//=========================================================================
// 3. DATA FILES LOCATION
//=========================================================================
// Now you need a place where Eset can save uploaded files.  This
...
//
// - On Windows systems you might specify something like 'c:\esetdata'

$CFG->dataroot  = '/home/example/esetdata';
...
// created by Eset within the data directory.  The format is in
...
// standard location for the Eset admin pages.  You can work around this
// by renaming the admin directory in your installation, and putting that
// new name here.  eg "esetadmin".  This should fix all admin links in Eset.
...
//=========================================================================
//
// These are additional tweaks for which no GUI exists in Eset yet.
//
...
//
// Change the key pair lifetime for Eset Networking
...
// off non-Eset standard characters in usernames.
...
// only useful if the backup file has been created using Eset < 1.9.4 and the
...
//      $CFG->esetpageclass = 'eset_page';
//      $CFG->esetpageclassfile = "$CFG->dirroot/local/myplugin/mypageclass.php";
...
// comment out one the following options to enable it in Eset:
...
//         '/cachedir/' => '/var/www/eset/cache',    // for custom $CFG->cachedir locations
//         '/localcachedir/' => '/var/local/cache',    // for custom $CFG->localcachedir locations
//         '/tempdir/'  => '/var/www/eset/temp',     // for custom $CFG->tempdir locations
//         '/filedir'   => '/var/www/eset/filedir',  // for custom $CFG->filedir locations
//     );
//
...
//
// If this setting is set to true, then Eset will track the IP of the
...
//
// Enabling this will allow custom scripts to replace existing eset scripts.
...
// Some examples are:
//      http://my.eset.site/course/view.php
//      http://my.eset.site/index.php
//      http://my.eset.site/admin            (index.php implied)
// Custom scripts should not include config.php
// Warning: Replacing standard eset scripts may pose security risks and/or may not
...
//     LogFormat "%h %l %{ESETUSER}n %t \"%r\" %s %b \"%{Referer}i\" \"%{User-Agent}i\"" esetformat
// And in the part specific to your Eset install / virtualhost:
//     CustomLog "/your/path/to/log" esetformat
//
// Alternatively for other webservers such as nginx, you can instead have the username sent via a http header
// 'X-ESETUSER' which can be saved in the logfile and then stripped out before being sent to the browser:
...
//
// Email database connection errors to someone.  If Eset cannot connect to the
...
//
//     $CFG->tempdir = '/var/www/eset/temp';        // Directory MUST BE SHARED by all cluster nodes.
//     $CFG->cachedir = '/var/www/eset/cache';      // Directory MUST BE SHARED by all cluster nodes, locking required.
...
//
//     $CFG->backuptempdir = '/var/www/eset/backuptemp';  // Directory MUST BE SHARED by all cluster nodes.
//
...
//
// When Eset is about to perform an intensive operation it raises PHP's memory
...
//
// Eset 2.4 introduced a new cache API.
// The cache API stores a configuration file within the Eset data directory and
...
// webserver.
//
//     $CFG->altcacheconfigpath = '/var/www/shared/eset.cache.config.php
//
...
//
// As of version 2.4 Eset serves icons as SVG images if the users browser appears
// to support SVG.
...
//
// Use the following flag to set userid for noreply user. If not set then eset will
// create dummy user and use -ve value as user id.
//      $CFG->noreplyuserid = -10;
//
// As of version 2.6 Eset supports admin to set support user. If not set, all mails
// will be sent to supportemail.
//      $CFG->supportuserid = -20;
//
// Eset 2.7 introduces a locking api for critical tasks (e.g. cron).
...
// Since Eset 3.7 the output of al scheduled and adhoc tasks is stored in the database and it is possible to use an
...
//
// Eset 2.9 allows administrators to customise the list of supported file types.
...
//
// Font used in exported PDF files. When generating a PDF, Eset embeds a subset of
...
//
// Eset 3.7+ checks that cron is running frequently. If the time between cron runs
...
//
// Eset 3.9+ checks how old tasks are in the ad hoc queue and warns at 10 minutes
...
// see https://docs.eset.org/en/Session_handling#Read_only_sessions.
//
...
//
// The display of Eset's question bank is made up of a number of columns.
...
// for each post in the database when posts are created or updated. For posts that existed prior to a Eset 3.8 upgrade,
...
//     'eset-course-categoryexpander' => true,
// );
// $CFG->yuilogexclude = array(
//     'eset-core-notification' => true,
// );
//
...
// $CFG->phpunit_dataroot = '/home/example/phpu_esetdata';
...
//
// $CFG->behat_wwwroot = 'http://127.0.0.1/eset';
// $CFG->behat_prefix = 'bht_';
// $CFG->behat_dataroot = '/home/example/bht_esetdata';
//
// You can override default Eset configuration for Behat and add your own
// params; here you can add more profiles, use different Mink drivers than Selenium...
// These params would be merged with the default Eset behat.yml, giving priority
...
//       )
//   );
// You can also use the following config to override default Eset configuration for Behat.
...
// );
//
// All this page's extra Eset settings are compared against a white list of allowed settings
...
//           'behat_wwwroot' => 'http://127.0.0.1/eset',
//           'behat_dataroot' => '/home/example/bht_esetdata'
//       ),
//   );
//
...
//
// Mobile app Behat testing requires this option, pointing to a developer Eset app directory:
//   $CFG->behat_ionic_dirroot = '/where/I/keep/my/git/checkouts/esetapp';
//
...
// (Development->Profiling) built into Eset.
//      $CFG->pathtodot = '';
//
// Path to unoconv.
