...
class course_enrolment_manager {
    ...
    /**
     * The current eset_page object
     * @var eset_page
     */
    protected $esetpage = null;
     ...
     */
    public function __construct(eset_page $esetpage, $course, $instancefilter = null,
            $rolefilter = 0, $searchfilter = '', $groupfilter = 0, $statusfilter = -1) {
        $this->esetpage = $esetpage;
    ...
    /**
     * Returns the current eset page
     * @return eset_page
     */
    public function get_esetpage() {
        return $this->esetpage;
    }
    ...
    public function get_assignable_roles($otherusers = false) {
        ...
        if ($otherusers) {
            if (!is_array($this->_assignablerolesothers)) {
                ...
                list($courseviewroles, $ignored) = get_roles_with_cap_in_context($this->context, 'eset/course:view');
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        if (!is_siteadmin() and !array_key_exists($roleid, $this->get_assignable_roles())) {
            if (defined('AJAX_SCRIPT')) {
                throw new eset_exception('invalidrole');
            }
            return false;
        }
    ...
    public function assign_role_to_user($roleid, $userid) {
        require_capability('eset/role:assign', $this->context);
    ...
    public function add_user_to_group($user, $groupid) {
        require_capability('eset/course:managegroups', $this->context);
     ...
     * @return array
     */
    public function get_other_users_for_display(core_enrol_renderer $renderer, eset_url $pageurl, $sort, $direction, $page, $perpage) {
    ...
    public function get_users_for_display(course_enrolment_manager $manager, $sort, $direction, $page, $perpage) {
        $pageurl = $manager->get_esetpage()->url;
        ...
        $context    = $this->get_context();
        $canmanagegroups = has_capability('eset/course:managegroups', $context);

        $url = new eset_url($pageurl, $this->get_url_params());
        $extrafields = get_extra_user_fields($context);
     ...
     *
     * Please note that this function does not check capability for eset/coures:viewhiddenuserfields
     *
    ...
    private function prepare_user_for_display($user, $extrafields, $now) {
        $details = array(
            ...
            'userfullnamedisplay' => fullname($user, has_capability('eset/site:viewfullnames', $this->get_context())),
...
class enrol_user_button extends single_button {
     ...
     */
    public function __construct(eset_url $url, $label, $method = 'post') {
        static $count = 0;
        $count ++;
    ...
    public function require_yui_module($modules, $function, array $arguments = null, $galleryversion = null, $ondomready = false) {
        if ($galleryversion != null) {
            debugging('The galleryversion parameter to yui_module has been deprecated since Eset 2.3.', DEBUG_DEVELOPER);
        }
     ...
     * @param mixed $a
     */
    public function strings_for_js($identifiers, $component = 'eset', $a = null) {
        $string = new stdClass;
...
class enrol_ajax_exception extends eset_exception {
    /**
     * Constructor
...
abstract class enrol_bulk_enrolment_operation {
    ...
    /**
     * Returns a esetform used for this operation, or false if no form is required and the action
     * should be immediatly processed.
     *
     * @param eset_url|string $defaultaction
     * @param mixed $defaultcustomdata
     * @return enrol_bulk_enrolment_change_form|esetform|false
     */
