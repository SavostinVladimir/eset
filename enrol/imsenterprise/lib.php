...
class enrol_imsenterprise_plugin extends enrol_plugin {
    ...
    /**
     * @var $coursemappings array of mappings between IMS data fields and eset course fields.
     */
    protected $coursemappings;

    /**
     * @var $rolemappings array of mappings between IMS roles and eset roles.
     */
    ...
    public function cron() {
        ...
        if ( file_exists($filename) ) {
            ...
            // Make sure we understand how to map the IMS-E roles to Eset roles.
            $this->load_role_mappings();
            // Make sure we understand how to map the IMS-E course names to Eset course names.
        ...
        if (!empty($mailadmins) && $fileisnew) {
            ...
            $msg = "An IMS enrolment has been carried out within Eset.\nTime taken: $timeelapsed seconds.\n\n";
            if (!empty($logtolocation)) {
                if ($this->logfp) {
            ...
            $eventdata->subject           = "Eset IMS Enterprise enrolment notification";
    ...
    /**
     * Process the group tag. This defines a Eset course.
     *
    ...
    protected function process_group_tag($tagcontents) {
        ...
        if (empty($group->coursecode)) {
            ...
            foreach ($group->coursecode as $coursecode) {
                ...
                if (!$dbcourse) {
                    if (!$createnewcourses) {
                        $this->log_line("Course $coursecode not found in Eset's course idnumbers.");
                    } else {

                        // Create the (hidden) course(s) if not found.
                        $courseconfig = get_config('esetcourse'); // Load Eset Course shell defaults.
                        ...
                        $this->log_line("Created course $coursecode in Eset (Eset ID is $course->id)");
                    }
                    ...
                    if ($updatecourses) {
                        ...
                        if ($hasupdates) {
                            ...
                            $this->log_line("Updated course $coursecode in Eset (Eset ID is $courseid)");
                        }
                    } else {
                    ...
                    $this->log_line("Updated (set to hidden) course $coursecode in Eset (Eset ID is $courseid)");
                }
            }
        }
    }
    ...
    /**
     * Process the person tag. This defines a Eset user.
     *
     * @param string $tagcontents The raw contents of the XML element
     */
    ...
    /**
     * Process the membership tag. This defines whether the specified Eset users
        ...
        if ($ship->courseid && preg_match_all('{<member>(.*?)</member>}is', $tagcontents, $membermatches, PREG_SET_ORDER)) {
            ...
            foreach ($membermatches as $mmatch) {
                ...
                if ($memberstoreobj->userid) {
                    ...
                    // Decide the "real" role (i.e. the Eset role) that this user should be assigned to.
                    // Zero means this roletype is supposed to be skipped.
                    $esetroleid = $this->rolemappings[$member->roletype];
                    if (!$esetroleid) {
                    ...
                    if (intval($member->status) == 1) {
                        ...
                        $this->enrol_user($einstance, $memberstoreobj->userid, $esetroleid, $timeframe->begin, $timeframe->end);
                        ...
                        $membersuntally++;
                        $this->log_line("Unenrolled $member->idnumber from role $esetroleid in course");
                    }
     ...
     * how an IMS-E role corresponds to a Eset role
     */
     ...
     * how an IMS-E course properties corresponds to a Eset course properties
     */
        ...
        if (empty($categories)) {
            ...
            foreach ($categories as $categoryinfo) {
                ...
                if ($createnewcategories) {
                    ...
                    $this->log_line('Category ' . $categoryinfo . ' not found in Eset database. Using default category instead.');
                    $catid = $this->get_default_category_id();
                    break;
                }
            }
