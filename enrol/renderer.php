    ...
    public function user_roles_and_actions($userid, $roles, $assignableroles, $canassign, $pageurl) {
        ...
        foreach ($roles as $roleid=>$role) {
            if ($canassign and (is_siteadmin() or isset($assignableroles[$roleid])) and !$role['unchangeable']) {
                ...
                $url = new eset_url($pageurl, array('action'=>'unassign', 'roleid'=>$roleid, 'user'=>$userid));
        ...
        if (!empty($assignableroles) && $canassign) {
            ...
            if (!$hasallroles) {
                $url = new eset_url($pageurl, array('action' => 'assign', 'user' => $userid));
    ...
    public function user_groups_and_actions($userid, $groups, $allgroups, $canmanagegroups, $pageurl) {
        ...
        foreach($groups as $groupid=>$name) {
            if ($canmanagegroups and groups_remove_member_allowed($groupid, $userid)) {
                ...
                $url = new eset_url($pageurl, array('action'=>'removemember', 'group'=>$groupid, 'user'=>$userid));
        ...
        if ($canmanagegroups && (count($groups) < count($allgroups))) {
            $url = new eset_url($pageurl, array('action'=>'addmember', 'user'=>$userid));
...
class course_enrolment_table extends html_table implements renderable {
    ...
    public function set_fields($fields, $output) {
        ...
        $this->align = array();
        $url = $this->manager->get_esetpage()->url;
            ...
            if (is_array($label)) {
                ...
                foreach ($label as $n => $l) {
                    ...
                    if (!in_array($n, self::$sortablefields)) {
                        ...
                        $sorturl = new eset_url($url, array(self::SORTVAR => $n, self::SORTDIRECTIONVAR => $this->get_field_sort_direction($n)));
                ...
                if (!in_array($name, self::$sortablefields)) {
                    ...
                    $sorturl = new eset_url($url, array(self::SORTVAR => $name, self::SORTDIRECTIONVAR => $this->get_field_sort_direction($name)));
    ...
    public function initialise_javascript() {
        if (has_capability('eset/role:assign', $this->manager->get_context())) {
            $this->manager->get_esetpage()->requires->strings_for_js(array(
                'assignroles',
                'confirmunassign',
            ...
            $modules = array('eset-enrol-rolemanager', 'eset-enrol-rolemanager-skin');
            ...
            $this->manager->get_esetpage()->requires->yui_module($modules, $function, array($arguments));
        }
    }
    ...
    public function get_paging_bar() {
        if ($this->pagingbar == null) {
            $this->pagingbar = new paging_bar($this->totalusers, $this->page, $this->perpage, $this->manager->get_esetpage()->url, self::PAGEVAR);
        }
...
class course_enrolment_other_users_table extends course_enrolment_table {
    ...
    public function get_user_search_button() {
        ...
        $count++;
        $url = new eset_url('/admin/roles/assign.php', array('contextid'=>$this->manager->get_context()->id, 'sesskey'=>sesskey()));
        ...
        if ($count == 1) {
            $this->manager->get_esetpage()->requires->strings_for_js(array(
                    ...
                    'durationdays',
                    'enrolperiod'), 'enrol');
            $this->manager->get_esetpage()->requires->string_for_js('assignrole', 'role');

            $modules = array('eset-enrol-otherusersmanager', 'eset-enrol-otherusersmanager-skin');
            $function = 'M.enrol.otherusersmanager.init';
            $arguments = array(
                'courseId'=> $this->manager->get_course()->id,
                'ajaxUrl' => '/enrol/ajax.php',
                'url' => $this->manager->get_esetpage()->url->out(false));
            $this->manager->get_esetpage()->requires->yui_module($modules, $function, array($arguments));
        }
        return $control;
    }
