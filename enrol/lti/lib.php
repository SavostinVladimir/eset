...
class enrol_lti_plugin extends enrol_plugin {
    ...
    public function can_add_instance($courseid) {
        ...
        return has_capability('eset/course:enrolconfig', $context) && has_capability('enrol/lti:config', $context);
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if (!empty($data['requirecompletion'])) {
            ...
            $esetcontext = $DB->get_record('context', array('id' => $data['contextid']));
            if ($esetcontext->contextlevel == CONTEXT_MODULE) {
                $cm = get_coursemodule_from_id(false, $esetcontext->instanceid, 0, false, MUST_EXIST);
            } else {
                $cm = null;
            }
...
function enrol_lti_extend_navigation_course($navigation, $course, $context) {
    ...
    if (enrol_is_enabled('lti')) {
        ...
        if ($ltiplugin->can_add_instance($course->id)) {
            $url = new eset_url('/enrol/lti/index.php', array('courseid' => $course->id));
