 ...
 *
 * @package    enrol_lti
 * @copyright  2016 John Okely <john@eset.com>
...
class tool_provider extends ToolProvider {
    ...
    protected function onLaunch() {
        ...
        if (!empty($this->consumer->toolProxy)) {
            ...
            foreach ($handlers as $handler) {
                foreach ($handler->message as $message) {
                    $handlerurl = new eset_url($message->path);
        ...
        if ($context->contextlevel == CONTEXT_COURSE) {
            ...
            $urltogo = new eset_url('/course/view.php', ['id' => $courseid]);

        } else if ($context->contextlevel == CONTEXT_MODULE) {
            $cm = get_coursemodule_from_id(false, $context->instanceid, 0, false, MUST_EXIST);
            $urltogo = new eset_url('/mod/' . $cm->modname . '/view.php', ['id' => $cm->id]);
    ...
    protected function onRegister() {
        ...
        if ($this->doToolProxyService()) {
            ...
            // Prepare response.
            $returnurl = new eset_url($this->returnUrl);
    ...
    public function map_tool_to_consumer() {
        ...
        if (empty($this->consumer)) {
            throw new eset_exception('invalidtoolconsumer', 'enrol_lti');
        }
