...
class manage_table extends \table_sql {
    ...
    public function __construct($courseid) {
        ...
        $this->canconfig = has_capability('eset/course:enrolconfig', \context_course::instance($courseid));
    ...
    public function col_edit($tool) {
        ...
        $url = new \eset_url('/enrol/lti/index.php', array('sesskey' => sesskey(), 'courseid' => $this->courseid));

        if ($this->ltiplugin->can_delete_instance($instance)) {
            $aurl = new \eset_url($url, array('action' => 'delete', 'instanceid' => $instance->id));
        ...
        if ($this->ltienabled && $this->ltiplugin->can_hide_show_instance($instance)) {
            if ($instance->status == ENROL_INSTANCE_ENABLED) {
                $aurl = new \eset_url($url, array('action' => 'disable', 'instanceid' => $instance->id));
                $buttons[] = $OUTPUT->action_icon($aurl, new \pix_icon('t/hide', $strdisable, 'core',
                    array('class' => 'iconsmall')));
            } else if ($instance->status == ENROL_INSTANCE_DISABLED) {
                $aurl = new \eset_url($url, array('action' => 'enable', 'instanceid' => $instance->id));
        ...
        if ($this->ltienabled && $this->canconfig) {
            $linkparams = array(
                ...
                'returnurl' => new \eset_url('/enrol/lti/index.php', array('courseid' => $this->courseid))
            );
