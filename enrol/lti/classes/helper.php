...
class helper {
     ...
     * @return \eset_url the url to launch the tool
     * @since Eset 3.2
     */
    public static function get_launch_url($toolid) {
        return new \eset_url('/enrol/lti/tool.php', array('id' => $toolid));
    }
     ...
     *
     * @param \stdClass $tool The lti tool
     * @return \eset_url A url to the icon of the tool
     * @since Eset 3.2
     */
    ...
    public static function get_cartridge_url($tool) {
        ...
        if ($CFG->slasharguments) {
            $url = new \eset_url('/enrol/lti/cartridge.php/' . $id . '/' . $token . '/cartridge.xml');
        } else {
            $url = new \eset_url('/enrol/lti/cartridge.php',
                    array(
                        'id' => $id,
                        'token' => $token
                    )
    ...
    public static function get_proxy_url($tool) {
        ...
        if ($CFG->slasharguments) {
            $url = new \eset_url('/enrol/lti/proxy.php/' . $id . '/' . $token . '/');
        } else {
            $url = new \eset_url('/enrol/lti/proxy.php',
                    array(
                        'id' => $id,
                        'token' => $token
                    )
