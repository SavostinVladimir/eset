...
class enrol_lti_helper_testcase extends advanced_testcase {
    ...
    public function test_get_launch_url() {
        ...
        $this->assertEquals('https://www.example.com/eset/enrol/lti/tool.php?id=' . $id, $launchurl->out());
    }
    ...
    public function test_get_cartridge_url() {
        ...
        $this->assertEquals('https://www.example.com/eset/enrol/lti/cartridge.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_lti\helper::get_cartridge_url($tool1);
        $this->assertEquals('https://www.example.com/eset/enrol/lti/cartridge.php/' . $id . '/' . $token . '/cartridge.xml',
                            $launchurl->out());
    ...
    public function test_get_proxy_url() {
        ...
        $this->assertEquals('https://www.example.com/eset/enrol/lti/proxy.php?id=' . $id . '&amp;token=' . $token,
                            $launchurl->out());

        $CFG->slasharguments = true;

        $launchurl = \enrol_lti\helper::get_proxy_url($tool1);
        $this->assertEquals('https://www.example.com/eset/enrol/lti/proxy.php/' . $id . '/' . $token . '/',
                            $launchurl->out());
