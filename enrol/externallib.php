...
class core_enrol_external extends external_api {
            ...
            array (
                'coursecapabilities' => new external_multiple_structure(
                    new external_single_structure(
                        array (
                            'courseid' => new external_value(PARAM_INT, 'Course ID number in the Eset course table'),
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * groupid (integer) return only users in this group id. Requires \'eset/site:accessallgroups\' .
                            * onlyactive (integer) only users with active enrolments. Requires \'eset/course:enrolreview\' .
    ...
    public static function get_enrolled_users_with_capability($coursecapabilities, $options) {
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            if (!$coursecontext) {
                throw new eset_exception('cannotfindcourse', 'error', '', null,
            ...
            try {
                ...
                throw new eset_exception(get_string('errorcoursecontextnotvalid' , 'webservice', $exceptionparam));
            }
            ...
            if (!empty($groupid) && groups_is_member($groupid)) {
                require_capability('eset/site:accessallgroups', $coursecontext);
            }
            // The course:enrolereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('eset/course:enrolreview', $coursecontext);
            }

            // To see the permissions of others role:review capability is required.
            require_capability('eset/role:review', $coursecontext);
    ...
    public static function get_users_courses($userid, $returnusercount = true) {
        ...
        foreach ($courses as $course) {
            ...
            if (!$canviewlastaccess) {
                $canviewlastaccess = has_capability('eset/course:viewhiddenuserfields', $context);
            }
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * withcapability (string) return only users with this capability. This option requires \'eset/role:review\' on the course context.
                            * groupid (integer) return only users in this group id. If the course has groups enabled and this param
                                                ...
                                                This option requires \'eset/site:accessallgroups\' on the course context if the
                                                user doesn\'t belong to the group.
                            * onlyactive (integer) return only users with active enrolments and matching time restrictions. This option requires \'eset/course:enrolreview\' on the course context.
                            * userfields (\'string, string, ...\') return only the values of these user fields.
                            * limitfrom (integer) sql limit from.
    ...
    public static function get_enrolled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
                    'username'    => new external_value(PARAM_RAW, 'Username policy is defined in Eset security config', VALUE_OPTIONAL),
                    'firstname'   => new external_value(PARAM_NOTAGS, 'The first name(s) of the user', VALUE_OPTIONAL),
                    'lastname'    => new external_value(PARAM_NOTAGS, 'The family name of the user', VALUE_OPTIONAL),
                    'fullname'    => new external_value(PARAM_NOTAGS, 'The fullname of the user'),
    ...
    public static function get_course_enrolment_methods($courseid) {
        ...
        if (!core_course_category::can_view_course_info($course) && !can_access_course($course)) {
            throw new eset_exception('coursehidden');
        }
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            self::validate_context($context);
            require_capability('eset/role:assign', $context);
