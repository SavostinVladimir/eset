...
$canconfig = has_capability('eset/course:enrolconfig', $context);
            ...
            if ($plugin->can_delete_instance($instance)) {
                if ($confirm) {
                    if (enrol_accessing_via_instance($instance)) {
                        if (!$confirm2) {
                            $yesurl = new eset_url('/enrol/instances.php',
                                                     array('id' => $course->id,
                                                           'action' => 'delete',
...
$url = new eset_url('/enrol/instances.php', array('sesskey'=>sesskey(), 'id'=>$course->id));
foreach ($instances as $instance) {
    ...
    if ($canconfig) {
        if ($updowncount > 1) {
            $aurl = new eset_url($url, array('action'=>'up', 'instance'=>$instance->id));
        ...
        if ($updowncount < $icount) {
            $aurl = new eset_url($url, array('action'=>'down', 'instance'=>$instance->id));
        ...
        if ($plugin->can_delete_instance($instance)) {
            $aurl = new eset_url($url, array('action'=>'delete', 'instance'=>$instance->id));
        ...
        if (enrol_is_enabled($instance->enrol) && $plugin->can_hide_show_instance($instance)) {
            if ($instance->status == ENROL_INSTANCE_ENABLED) {
                $aurl = new eset_url($url, array('action'=>'disable', 'instance'=>$instance->id));
                ...
                $aurl = new eset_url($url, array('action'=>'enable', 'instance'=>$instance->id));
...
foreach (enrol_get_plugins(true) as $name=>$plugin) {
    if ($plugin->use_standard_editing_ui()) {
        if ($plugin->can_add_instance($course->id)) {
            ...
            $url = new eset_url('/enrol/editinstance.php', $params);
            $link = $url->out(false);
