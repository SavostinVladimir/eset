...
$PAGE->set_url(new eset_url('/enrol/ajax.php', array('id'=>$id, 'action'=>$action)));

$course = $DB->get_record('course', array('id'=>$id), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

if ($course->id == SITEID) {
    throw new eset_exception('invalidcourse');
}
...
switch ($action) {
    ...
    case 'unassign':
        ...
        if (!has_capability('eset/role:assign', $manager->get_context()) || !$manager->unassign_role_from_user($user, $role)) {
    ...
    case 'assign':
        ...
        if (!has_capability('eset/role:assign', $manager->get_context()) || !$manager->assign_role_to_user($roleid, $user->id)) {
    ...
    case 'searchotherusers':
        ...
        if (has_capability('eset/user:viewdetails', context_system::instance())) {
