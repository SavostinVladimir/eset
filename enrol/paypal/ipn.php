...
// Disable eset specific debug messages and any errors in output,
...
if (!enrol_is_enabled('paypal')) {
    ...
    throw new eset_exception('errdisabled', 'enrol_paypal');
}

/// Keep out casual intruders
if (empty($_POST) or !empty($_GET)) {
    http_response_code(400);
    throw new eset_exception('invalidrequest', 'core_error');
}
...
foreach ($_POST as $key => $value) {
    if ($key !== clean_param($key, PARAM_ALPHANUMEXT)) {
        throw new eset_exception('invalidrequest', 'core_error', '', null, $key);
    }
    if (is_array($value)) {
        throw new eset_exception('invalidrequest', 'core_error', '', null, 'Unexpected array param: '.$key);
    }
...
if (empty($data->custom)) {
    throw new eset_exception('invalidrequest', 'core_error', '', null, 'Missing request param: custom');
}
...
if (empty($custom) || count($custom) < 3) {
    throw new eset_exception('invalidrequest', 'core_error', '', null, 'Invalid value of the request param: custom');
}
...
if ($c->get_errno()) {
    throw new eset_exception('errpaypalconnect', 'enrol_paypal', '', array('url' => $paypaladdr, 'result' => $result),
        json_encode($data));
}
...
if (strlen($result) > 0) {
    if (strcmp($result, "VERIFIED") == 0) {          // VALID PAYMENT!
        ...
        if ($users = get_users_by_capability($context, 'eset/course:update', 'u.*', 'u.id ASC',
                                             '', '', '', '', false, true)) {
        ...
        throw new eset_exception('erripninvalid', 'enrol_paypal', '', null, json_encode($data));
    }
}
