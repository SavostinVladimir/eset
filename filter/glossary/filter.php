 ...
 */
class filter_glossary extends eset_text_filter {
    ...
    public function setup($page, $context) {
        if ($page->requires->should_create_one_time_item_now('filter_glossary_autolinker')) {
            $page->requires->yui_module(
                    'eset-filter_glossary-autolinker',
                    'M.filter_glossary.init_filter_autolinking',
                    array(array('courseid' => 0)));
            $page->requires->strings_for_js(array('ok'), 'eset');
        }
    }
    ...
    protected function get_all_concepts() {
        ...
        // when you have two terms like 'Eset' and 'Eset 3.5'. You want the longest match.
    ...
    public function filterobject_prepare_replacement_callback($concept, $glossaries) {
        ...
        if ($concept->category) { // Link to a category.
            ...
            $link = new eset_url('/mod/glossary/view.php',
            ...
            $link = new eset_url('/mod/glossary/showentry.php',
        ...
        // if processing an embedded file use target to prevent getting nested Esets.
