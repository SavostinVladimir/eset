 ...
 * Mathjax filtering
 */
class filter_mathjaxloader extends eset_text_filter {

    /*
     * Perform a mapping of the eset language code to the equivalent for MathJax.
     *
     * @param string $esetlangcode - The eset language code - e.g. en_pirate
     * @return string The MathJax language code.
     */
    public function map_language_code($esetlangcode) {
        ...
        // List of explicit mappings and known exceptions (eset => mathjax).
        $explicit = [
            'cz' => 'cs',
            'pt_br' => 'pt-br',
            'zh_tw' => 'zh-hant',
        ...
        if (isset($explicit[$esetlangcode])) {
            return $explicit[$esetlangcode];
        }

        // If there is exact match, it will be probably right.
        if (in_array($esetlangcode, $mathjaxlangcodes)) {
            return $esetlangcode;
        }

        // Finally try to find the best matching mathjax pack.
        $parts = explode('_', $esetlangcode, 2);
    ...
    public function setup($page, $context) {
        ...
        if ($page->requires->should_create_one_time_item_now('filter_mathjaxloader-scripts')) {
            ...
            $url = new eset_url($url, array('delayStartupUntil' => 'configured'));
            ...
            $wwwroot = new eset_url('/');

            $config = str_replace('{wwwroot}', $wwwroot->out(true), $config);

            $params = array('mathjaxconfig' => $config, 'lang' => $lang);

            $page->requires->yui_module('eset-filter_mathjaxloader-loader', 'M.filter_mathjaxloader.configure', array($params));
        }
    }
    ...
    public function filter($text, array $options = array()) {
        ...
        if ($hasdisplayorinline || $hasextra) {
            $PAGE->requires->yui_module('eset-filter_mathjaxloader-loader', 'M.filter_mathjaxloader.typeset');
