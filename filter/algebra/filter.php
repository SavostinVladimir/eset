...
/**
 * Eset - Filter for converting simple calculator-type algebraic
...
//-------------------------------------------------------------------------
// NOTE: This Eset text filter converts algebraic expressions delimited
...
//-------------------------------------------------------------------------
// You will then need to edit your eset/config.php to invoke mathml_filter.php
//-------------------------------------------------------------------------
...
function filter_algebra_image($imagefile, $tex= "", $height="", $width="", $align="middle") {
  ...
  if ($imagefile) {
    ...
    if (!file_exists("$CFG->dataroot/filter/algebra/$imagefile") && has_capability('eset/site:config', context_system::instance())) {
        $link = '/filter/algebra/algebradebug.php';
        $action = null;
    } else {
        $link = new eset_url('/filter/tex/displaytex.php', array('texexp'=>$tex));
...
class filter_algebra extends eset_text_filter {
...
//restrict filtering to forum 130 (Maths Tools on eset.org)
