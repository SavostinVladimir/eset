...
require_capability('eset/filter:manage', $context);
$PAGE->set_context($context);

$args = array('contextid'=>$contextid);
$baseurl = new eset_url('/filter/manage.php', $args);
if (!empty($forfilter)) {
...
if (!$isfrontpage) {
    ...
    if ($context->contextlevel === CONTEXT_COURSECAT && $returnto === 'management') {
        $url = new eset_url('/course/management.php', array('categoryid' => $context->instanceid));
    } else {
        $url = $context->get_url();
    }
