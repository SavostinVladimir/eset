...
class core_calendar_external extends external_api {
    ...
    public static function delete_calendar_events($events) {
        ...
        foreach ($params['events'] as $event) {
            ...
            if (!calendar_delete_event_allowed($eventobj)) {
                throw new eset_exception('nopermissions', 'error', '', get_string('deleteevent', 'calendar'));
            }
    ...
    public static function get_calendar_events($events = array(), $options = array()) {
        ...
        $hassystemcap = has_capability('eset/calendar:manageentries', context_system::instance());
        $warnings = array();
            ...
            if (!empty($params['events']['categoryids'])) {
                ...
                foreach ($catobjs as $catobj) {
                    if (isset($coursecategories[$catobj->id]) ||
                            has_capability('eset/category:manage', $catobj->get_context())) {
     ...
     * @since Eset 2.5
     * @throws eset_exception if user doesnt have the permission to create events.
     */
    ...
    public static function get_calendar_event_by_id($eventid) {
        ...
        if (!$event) {
            ...
            throw new \required_capability_exception($syscontext, 'eset/course:view', 'nopermission', '');
        }
    ...
    public static function update_event_start_day($eventid, $daytimestamp) {
        ...
        if (!$event) {
            throw new \eset_exception('Unable to find event with id ' . $eventid);
        }
    ...
    public static function get_calendar_access_information($courseid = 0) {
        ...
        return [
            'canmanageentries' => has_capability('eset/calendar:manageentries', $context),
            'canmanageownentries' => has_capability('eset/calendar:manageownentries', $context),
            'canmanagegroupentries' => has_capability('eset/calendar:managegroupentries', $context),
            'warnings' => [],
        ];
    }
     ...
     * @throws eset_exception If one or more of the dates provided does not convert to a valid timestamp.
     */
    public static function get_timestamps($datetimes) {
        ...
        foreach ($params['data'] as $key => $datetime) {
            ...
            try {
                ...
                throw new eset_exception('One or more of the dates provided were invalid');
            }
        }
