...
$url = new eset_url('/calendar/managesubscriptions.php');
if ($courseid != SITEID) {
...
navigation_node::override_active_url(new eset_url('/calendar/view.php', array('view' => 'month')));
$PAGE->set_url($url);
...
if ($usedefaultfilters) {
    ...
    if (!empty($types['category'])) {
        list($categoryinsql, $categoryparams) = $DB->get_in_or_equal(
                array_keys(\core_course_category::make_categories_list('eset/category:manage')), SQL_PARAMS_NAMED, 'category');
