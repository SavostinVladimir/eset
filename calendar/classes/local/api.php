...
class api {
    ...
    public static function get_action_events_by_timesort(
        ...
        if (is_null($timesortfrom) && is_null($timesortto)) {
            throw new \eset_exception("Must provide a timesort to and/or from value");
        }

        if ($limitnum < 1 || $limitnum > 50) {
            throw new \eset_exception("Limit must be between 1 and 50 (inclusive)");
        }
    ...
    public static function update_event_start_day(
        ...
        if ($min === false || $max === false) {
            throw new \eset_exception('The start day of this event can not be modified');
        }

        if ($min && $starttimestamp < $min[0]) {
            throw new \eset_exception($min[1]);
        }

        if ($max && $starttimestamp > $max[0]) {
            throw new \eset_exception($max[1]);
        }
