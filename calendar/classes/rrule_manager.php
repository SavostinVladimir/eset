...
class rrule_manager {
    ...
    protected function parse_rrule_property($prop) {
        ...
        switch ($property) {
            ...
            default:
                ...
                throw new eset_exception('errorrrule', 'calendar');
        }
    }
    ...
    protected function set_frequency($freq) {
        switch ($freq) {
            ...
            default:
                // We should never get here, something is very wrong.
                throw new eset_exception('errorrrulefreq', 'calendar');
        }
    }
    ...
    protected function get_day($daystring) {
        switch ($daystring) {
            ...
            default:
                // We should never get here, something is very wrong.
                throw new eset_exception('errorrruleday', 'calendar');
        }
    }
    ...
    protected function set_interval($intervalstr) {
        ...
        if ($interval < 1) {
            throw new eset_exception('errorinvalidinterval', 'calendar');
        }
    ...
    protected function set_bysecond($bysecond) {
        ...
        foreach ($seconds as $second) {
            if ($second < 0 || $second > 59) {
                throw new eset_exception('errorinvalidbysecond', 'calendar');
            }
    ...
    protected function set_byminute($byminute) {
        ...
        foreach ($minutes as $minute) {
            if ($minute < 0 || $minute > 59) {
                throw new eset_exception('errorinvalidbyminute', 'calendar');
            }
    ...
    protected function set_byhour($byhour) {
        ...
        foreach ($hours as $hour) {
            if ($hour < 0 || $hour > 23) {
                throw new eset_exception('errorinvalidbyhour', 'calendar');
            }
    ...
    protected function set_byday($byday) {
        ...
        foreach ($days as $day) {
            ...
            if (!in_array($suffix, $weekdays)) {
                throw new eset_exception('errorinvalidbydaysuffix', 'calendar');
            }
    ...
    protected function set_bymonthday($bymonthday) {
        ...
        foreach ($monthdays as $day) {
            ...
            if ($day < -31 || $day > 31 || $day == 0) {
                throw new eset_exception('errorinvalidbymonthday', 'calendar');
            }
    ...
    protected function set_byyearday($byyearday) {
        ...
        foreach ($yeardays as $day) {
            ...
            if ($day < -366 || $day > 366 || $day == 0) {
                throw new eset_exception('errorinvalidbyyearday', 'calendar');
            }
    ...
    protected function set_byweekno($byweekno) {
        ...
        foreach ($weeknumbers as $week) {
            ...
            if ($week < -53 || $week > 53 || $week == 0) {
                throw new eset_exception('errorinvalidbyweekno', 'calendar');
            }
    ...
    protected function set_bymonth($bymonth) {
        ...
        foreach ($months as $month) {
            // Valid values are 1 to 12.
            if ($month < 1 || $month > 12) {
                throw new eset_exception('errorinvalidbymonth', 'calendar');
            }
    ...
    protected function set_bysetpos($bysetpos) {
        ...
        foreach ($setposes as $pos) {
            ...
            if ($pos < -366 || $pos > 366 || $pos == 0) {
                throw new eset_exception('errorinvalidbysetpos', 'calendar');
            }
    ...
    protected function validate_rules() {
        ...
        if (!empty($this->until) && !empty($this->count)) {
            throw new eset_exception('errorhasuntilandcount', 'calendar');
        }
        ...
        if (!empty($this->bysetpos) && empty($this->bymonth) && empty($this->bymonthday) && empty($this->bysecond)
            ...
            throw new eset_exception('errormustbeusedwithotherbyrule', 'calendar');
        }
        ...
        foreach ($this->byday as $bydayrule) {
            if (!empty($bydayrule->value) && $this->freq != self::FREQ_MONTHLY && $this->freq != self::FREQ_YEARLY) {
                throw new eset_exception('errorinvalidbydayprefix', 'calendar');
            }
        }

        // The BYWEEKNO rule is only valid for YEARLY rules.
        if (!empty($this->byweekno) && $this->freq != self::FREQ_YEARLY) {
            throw new eset_exception('errornonyearlyfreqwithbyweekno', 'calendar');
        }
    }
    ...
    protected function generate_recurring_event_times($event) {
        ...
        if (empty($this->count)) {
            if ($this->until) {
                ...
                // Forever event. However, since there's no such thing as 'forever' (at least not in Eset),
