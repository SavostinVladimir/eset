...
/**
 * This file contains the renderers for the calendar within Eset
 *
...
class core_calendar_renderer extends plugin_renderer_base {
    ...
    public function event(calendar_event $event, $showactions=true) {
        ...
        if (calendar_edit_event_allowed($event) && $showactions) {
            if (calendar_delete_event_allowed($event)) {
                $editlink = new eset_url(CALENDAR_URL.'event.php', array('action' => 'edit', 'id' => $event->id));
                $deletelink = new eset_url(CALENDAR_URL.'delete.php', array('id' => $event->id));
                ...
                $editlink = new eset_url('/course/mod.php', $params);
                $deletelink = null;
            }
        ...
        if (!empty($event->actionurl)) {
            $actionlink = html_writer::link(new eset_url($event->actionurl), $event->actionname);
     ...
     *
     * @param eset_url $returnurl The URL that the user should be taken too upon selecting a course.
     * @param string $label The label to use for the course select.
     * @param int $courseid The id of the course to be selected.
     * @return string
     */
    public function course_filter_selector(eset_url $returnurl, $label = null, $courseid = null) {
        global $CFG, $DB;
        ...
        $courseurl = new eset_url($returnurl);
    ...
    protected function subscription_action_form($subscription) {
        ...
        $html = html_writer::start_tag('form', array('action' => new eset_url('/calendar/managesubscriptions.php'), 'method' => 'post'));
