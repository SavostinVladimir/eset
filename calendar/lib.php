 ...
 * It was introduced as part of Eset 2.0 and was created in order to provide a
...
class calendar_event {
     ...
     * @param bool $checkcapability If Eset should check the user can manage the calendar events for this call or not.
...
class calendar_information {
    ...
    public static function create($time, int $courseid, int $categoryid = null) : calendar_information {
        ...
        if ($courseid != SITEID && !empty($courseid)) {
            ...
            if (!$course->visible && !is_role_switched($course->id)) {
                require_capability('eset/course:viewhiddencourses', $calendar->context);
            }
            ...
            if ($this->categories === false) {
                ...
                foreach (\core_course_category::get_all() as $category) {
                    if (isset($coursecategories[$category->id]) ||
                            has_capability('eset/category:manage', $category->get_context(), $USER, false)) {
     ...
     * or throws a eset_exception if not
     *
    ...
    public function checkdate($defaultonow = true) {
        if (!checkdate($this->month, $this->day, $this->year)) {
            if ($defaultonow) {
                ...
                throw new eset_exception('invaliddate');
            }
        }
        return true;
    }
    ...
    public function add_sidecalendar_blocks(core_calendar_renderer $renderer, $showfilters=false, $view=null) {
        ...
        if (!has_capability('eset/block:view', $PAGE->context) ) {
            return;
        }
 ...
 * @throws eset_exception for an invalid id
 */
    ...
    switch ($type) {
        case 'frontpage':
            ...
            $nextlink = calendar_get_link_next(get_string('monthnext', 'calendar'), $urlbase, false, false, false, true,
                $nextmonthtime);
            $calendarlink = calendar_get_link_href(new \eset_url(CALENDAR_URL . 'view.php', array('view' => 'month')),
        ...
        case 'course':
            ...
            break;
        case 'upcoming':
            $calendarlink = calendar_get_link_href(new \eset_url(CALENDAR_URL . 'view.php', array('view' => 'upcoming')),
                false, false, false, $time);
            if (!empty($data['id'])) {
                $calendarlink->param('course', $data['id']);
            }
...
/**
 * Adds day, month, year arguments to a URL and returns a eset_url object.
 *
 * @param string|eset_url $linkbase
...
function calendar_get_link_href($linkbase, $d, $m, $y, $time = 0) {
    ...
    if (!($linkbase instanceof \eset_url)) {
        $linkbase = new \eset_url($linkbase);
    }
 ...
 * @param string|eset_url $linkbase The URL stub.
...
function calendar_get_link_previous($text, $linkbase, $d, $m, $y, $accesshide = false, $time = 0) {
    $href = calendar_get_link_href(new \eset_url($linkbase), $d, $m, $y, $time);
...
function calendar_set_filters(array $courseeventsfrom, $ignorefilters = false, stdClass $user = null) {
    ...
    $allgroupscaps = array('eset/site:accessallgroups', 'eset/calendar:manageentries');
...
function calendar_view_event_allowed(calendar_event $event) {
    ...
    if (has_capability('eset/calendar:manageentries', $sitecontext)) {
        return true;
    }

    if (!empty($event->groupid)) {
        // If it is a group event we need to be able to manage events in the course, or be in the group.
        if (has_capability('eset/calendar:manageentries', $event->context) ||
                has_capability('eset/calendar:managegroupentries', $event->context)) {
            return true;
        }
        ...
        throw new eset_exception('unknown event type');
    }
...
function calendar_edit_event_allowed($event, $manualedit = false) {
    ...
    if ($manualedit && !empty($event->modulename)) {
        ...
        return has_capability('eset/course:manageactivities', $context);
    }
    ...
    if (!empty($event->groupid)) {
        ...
        return $group && (
                has_capability('eset/calendar:manageentries', $event->context) ||
                (has_capability('eset/calendar:managegroupentries', $event->context)
                    && groups_is_member($event->groupid)));
    } else if (!empty($event->courseid)) {
        // If groupid is not set, but course is set, it's definitely a course event.
        return has_capability('eset/calendar:manageentries', $event->context);
        ...
        return (has_capability('eset/calendar:manageownentries', $event->context));
...
function calendar_get_default_courses($courseid = null, $fields = '*', $canmanage = false, int $userid = null) {
    ...
    if ((!empty($CFG->calendar_adminseesall) || $canmanage) &&
            has_capability('eset/calendar:manageentries', context_system::instance(), $userid)) {
    ...
    if ($courseid && $courseid != SITEID) {
        if (empty($courses[$courseid]) && has_capability('eset/calendar:manageentries', context_system::instance(), $userid)) {
    ...
    if ($event->timeduration) {
        ...
        if ($usermidnightstart == $usermidnightend) {
            ...
            if (!$showtime) {
                ...
                $url = calendar_get_link_href(new \eset_url(CALENDAR_URL . 'view.php', $linkparams), 0, 0, 0, $endtime);
            ...
            if ($now >= $usermidnightstart && $now < strtotime('+1 day', $usermidnightstart)) {
                ...
                $url = calendar_get_link_href(new \eset_url(CALENDAR_URL . 'view.php', $linkparams), 0, 0, 0, $starttime);
 ...
 * @param stdClass|int $user eset user object or id, null means current user
 */
...
function calendar_get_allowed_types(&$allowed, $course = null, $groups = null, $category = null) {
    ...
    $allowed->user = has_capability('eset/calendar:manageownentries', \context_system::instance());
    $allowed->groups = false;
    $allowed->courses = false;
    $allowed->categories = false;
    $allowed->site = has_capability('eset/calendar:manageentries', \context_course::instance(SITEID));
    ...
    if (!empty($course)) {
        ...
        if ($course->id != SITEID) {
            ...
            $allowed->user = has_capability('eset/calendar:manageownentries', $coursecontext);

            if (has_capability('eset/calendar:manageentries', $coursecontext)) {
                $allowed->courses = array($course->id => 1);
                $allowed->groups = $getgroupsfunc($course, $coursecontext, $USER);
            } else if (has_capability('eset/calendar:managegroupentries', $coursecontext)) {
                $allowed->groups = $getgroupsfunc($course, $coursecontext, $USER);
            }
        }
    }
...
function calendar_add_event_allowed($event) {
    ...
    switch ($event->eventtype) {
        case 'category':
            return has_capability('eset/category:manage', $event->context);
        case 'course':
            return has_capability('eset/calendar:manageentries', $event->context);
        case 'group':
            // Allow users to add/edit group events if -
...
/**
 * Add an iCalendar event to the Eset calendar.
 *
...
function calendar_add_icalendar_event($event, $unused = null, $subscriptionid, $timezone='UTC') {
    ...
    if ($updaterecord = $DB->get_record('event', array('uuid' => $eventrecord->uuid,
        ...
        // Compare iCal event data against the eset event to see if something has changed.
...
function calendar_get_icalendar($url) {
    ...
    if (!$calendar || $curl->info['http_code'] != 200 || !empty($curl->errorno)) {
        throw new \eset_exception('errorinvalidicalurl', 'calendar');
    }
...
function calendar_get_allowed_event_types(int $courseid = null) {
    ...
    if (!empty($courseid) && $courseid != SITEID) {
        ...
        $types['user'] = has_capability('eset/calendar:manageownentries', $context);
        calendar_internal_update_course_and_group_permission($courseid, $context, $types);
    }

    if (has_capability('eset/calendar:manageentries', \context_course::instance(SITEID))) {
        $types['site'] = true;
    }

    if (has_capability('eset/calendar:manageownentries', \context_system::instance())) {
        $types['user'] = true;
    }
    ...
    if ($types['course'] == false || $types['group'] == false) {
        if ($CFG->calendar_adminseesall && has_capability('eset/calendar:manageentries', context_system::instance())) {
            ...
            foreach ($courseswithgroups as $course) {
                ...
                if (has_capability('eset/calendar:manageentries', $context)) {
                    if (has_any_capability(['eset/site:accessallgroups', 'eset/calendar:managegroupentries'], $context)) {
...
function calendar_internal_update_course_and_group_permission(int $courseid, context $context, array &$types) {
    ...
    if (!$types['group'] && (has_capability('eset/calendar:manageentries', $context) ||
            has_capability('eset/calendar:managegroupentries', $context))) {
