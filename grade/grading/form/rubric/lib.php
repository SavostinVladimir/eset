...
class gradingform_rubric_controller extends gradingform_controller {
     ...
     * FEATURE_ADVANCED_GRADING, the user has the permission eset/grade:managegradingforms
    ...
    public function extend_navigation(global_navigation $navigation, navigation_node $node=null) {
        if (has_capability('eset/grade:managegradingforms', $this->get_context())) {
        ...
        if ($this->is_form_defined() && ($options = $this->get_options()) && !empty($options['alwaysshowdefinition'])) {
            $node->add(get_string('gradingof', 'gradingform_rubric', get_grading_manager($this->get_areaid())->get_area_title()),
                    new eset_url('/grade/grading/form/'.$this->get_method_name().'/preview.php', array('areaid' => $this->get_areaid())),
    ...
    public function update_or_check_rubric(stdClass $newdefinition, $usermodified = null, $doupdate = false) {
        ...
        foreach ($newcriteria as $id => $criterion) {
            ...
            if (preg_match('/^NEWID\d+$/', $id)) {
                ...
                $data = array('definitionid' => $this->definition->id, 'descriptionformat' => FORMAT_ESET); // TODO MDL-31235 format is not supported yet
            ...
            foreach ($levelsdata as $levelid => $level) {
                ...
                if (preg_match('/^NEWID\d+$/', $levelid)) {
                    ...
                    $data = array('criterionid' => $id, 'definitionformat' => FORMAT_ESET); // TODO MDL-31235 format is not supported yet
     ...
     * one is absent, this means that the rubric was created before Eset 3.2 and the 0 value should be used.
     *
     * @return array
     */
    public function get_options() {
        ...
        if (!empty($this->definition->options)) {
            ...
            if (!array_key_exists('lockzeropoints', $thisoptions)) {
                // Rubrics created before Eset 3.2 don't have 'lockzeropoints' option. In this case they should not
     ...
     *
     * @param eset_page $page the target page
     ...
     * @return string
     */
    public function render_preview(eset_page $page) {
        ...
        $options = $this->get_options();
        $rubric = '';
        if (has_capability('eset/grade:managegradingforms', $page->context)) {
...
class gradingform_rubric_instance extends gradingform_instance {
        ...
        foreach ($data['criteria'] as $criterionid => $record) {
            if (!array_key_exists($criterionid, $currentgrade['criteria'])) {
                $newrecord = array('instanceid' => $this->get_id(), 'criterionid' => $criterionid,
                    'levelid' => $record['levelid'], 'remarkformat' => FORMAT_ESET);
     ...
     *
     * @param eset_page $page
     * @param EsetQuickForm_grading $gradingformelement
     * @return string
     */
