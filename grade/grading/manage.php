...
if (!empty($shareform)) {
    require_capability('eset/grade:sharegradingforms', context_system::instance());
    ...
    if (!$confirmed) {
        ...
        echo $output->confirm(get_string('manageactionshareconfirm', 'core_grading', s($definition->name)),
            new eset_url($PAGE->url, array('shareform' => $shareform, 'confirmed' => 1)),
            $PAGE->url);
        echo $output->footer();
        die();
    } else {
        ...
        redirect(new eset_url($PAGE->url, array('message' => get_string('manageactionsharedone', 'core_grading'))));
    }
}
...
if (!empty($deleteform)) {
    ...
    if (!$confirmed) {
        ...
        echo $output->confirm(markdown_to_html(get_string('manageactiondeleteconfirm', 'core_grading', array(
            ...
            new eset_url($PAGE->url, array('deleteform' => $deleteform, 'confirmed' => 1)), $PAGE->url);
        echo $output->footer();
        die();
    } else {
        require_sesskey();
        $controller->delete_definition();
        redirect(new eset_url($PAGE->url, array('message' => get_string('manageactiondeletedone', 'core_grading'))));
    }
}
...
if (!empty($method)) {
    ...
    if ($controller->is_form_defined()) {
        ...
        echo $output->management_action_icon(new eset_url($PAGE->url, array('deleteform' => $definition->id)),
            get_string('manageactiondelete', 'core_grading'), 'b/edit-delete');
        // icon to save the form as a new template
        if (has_capability('eset/grade:sharegradingforms', context_system::instance())) {
            ...
            if ($allowshare) {
                echo $output->management_action_icon(new eset_url($PAGE->url, array('shareform' => $definition->id)),
        ...
        $pickurl = new eset_url('/grade/grading/pick.php', array('targetid' => $controller->get_areaid()));
