...
if ($targetcontroller->is_form_defined()) {
    redirect(new eset_url('/grade/grading/manage.php', array('areaid' => $targetid)));
}

list($context, $course, $cm) = get_context_info_array($targetmanager->get_context()->id);

require_login($course, true, $cm);
require_capability('eset/grade:managegradingforms', $context);

// user's capability in the templates bank
$canshare   = has_capability('eset/grade:sharegradingforms', context_system::instance());
$canmanage  = has_capability('eset/grade:managesharedforms', context_system::instance());

// setup the page
$PAGE->set_url(new eset_url('/grade/grading/pick.php', array('targetid' => $targetid)));
...
if ($pick) {
    ...
    if (!$sourcecontroller->is_shared_template() and !$sourcecontroller->is_own_form()) {
        ...
        // eset/grade:managegradingforms in the source area. so when users loose
        // their teacher role in a course, they can't access the course but they can
        // still copy the forms they have created there.
        throw new eset_exception('attempt_to_pick_others_form', 'core_grading');
    }
    if (!$sourcecontroller->is_form_defined()) {
        throw new eset_exception('form_definition_mismatch', 'core_grading');
    }
    ...
    if (!$confirmed) {
        ...
        echo $output->confirm(get_string('templatepickconfirm', 'core_grading',array(
            ...
            new eset_url($PAGE->url, array('pick' => $pick, 'confirmed' => 1)),
            $PAGE->url);
...
if ($remove) {
    ...
    if (!$sourcecontroller->is_shared_template()) {
        throw new eset_exception('attempt_to_delete_nontemplate', 'core_grading');
    }
    if (!$sourcecontroller->is_form_defined()) {
        throw new eset_exception('form_definition_mismatch', 'core_grading');
    }
    ...
    if ($canmanage or ($canshare and ($definition->usercreated == $USER->id))) {
        ...
        throw new eset_exception('no_permission_to_remove_template', 'core_grading');
    }
    if (!$confirmed) {
        echo $output->header();
        echo $output->confirm(get_string('templatedeleteconfirm', 'core_grading', s($definition->name)),
            new eset_url($PAGE->url, array('remove' => $remove, 'confirmed' => 1)),
            $PAGE->url);
        echo $output->box($sourcecontroller->render_preview($PAGE), 'template-preview-confirm');
        echo $output->footer();
        die();
...
foreach ($rs as $template) {
    ...
    $actions = array();
    if ($controller->is_shared_template()) {
        $actions[] = $output->pick_action_icon(new eset_url($PAGE->url, array('pick' => $template->id)),
            get_string('templatepick', 'core_grading'), 'i/valid', 'pick template');
        if ($canmanage or ($canshare and ($template->usercreated == $USER->id))) {
            //$actions[] = $output->pick_action_icon(new eset_url($PAGE->url, array('edit' => $template->id)),
            //    get_string('templateedit', 'core_grading'), 'i/edit', 'edit');
            $actions[] = $output->pick_action_icon(new eset_url($PAGE->url, array('remove' => $template->id)),
...
echo $output->single_button(
    new eset_url('/grade/grading/manage.php', array('areaid' => $targetid)),
    get_string('back'), 'get');
