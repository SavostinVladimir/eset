 ...
 * @copyright 2007 Eset Pty Ltd (http://eset.com)
...
$PAGE->set_url(new eset_url('/grade/report/user/index.php', array('id'=>$courseid)));
...
if (empty($userid)) {
    require_capability('eset/grade:viewall', $context);
...
} else if (has_capability('eset/grade:viewall', context_user::instance($userid)) and $course->showgrades) {
...
if (has_capability('eset/grade:viewall', $context)) { //Teachers will see all student reports
    ...
    $isseparategroups = ($course->groupmode == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $context));
    ...
    $showonlyactiveenrol = $showonlyactiveenrol || !has_capability('eset/course:viewsuspendedusers', $context);
    ...
    if (empty($userid)) {
        ...
        while ($userdata = $gui->next_user()) {
            ...
            $studentnamelink = html_writer::link(new eset_url('/user/view.php', array('id' => $report->user->id, 'course' => $courseid)), fullname($report->user));
