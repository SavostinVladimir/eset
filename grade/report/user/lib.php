...
class grade_report_user extends grade_report {
    ...
    public function __construct($courseid, $gpr, $context, $userid, $viewasuser = null) {
        ...
        if ($viewasuser) {
            ...
            $this->canviewhidden = has_capability('eset/grade:viewhidden', $coursecontext, $this->user->id);
        } else {
            $this->modinfo = $this->gtree->modinfo;
            $this->canviewhidden = has_capability('eset/grade:viewhidden', $coursecontext);
        }
    ...
    private function fill_table_recursive(&$element) {
        ...
        if ($type == 'item' or $type == 'categoryitem' or $type == 'courseitem') {
            ...
            if (!$hide) {
                ...
                **/
                $canviewall = has_capability('eset/grade:viewall', $this->context);
...
function gradereport_user_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $anyreport = has_capability('eset/user:viewuseractivitiesreport', $usercontext);
    ...
    if ($anyreport || $iscurrentuser) {
        ...
        if (has_capability('eset/grade:viewall', $coursecontext)) {
            ...
            if ($iscurrentuser && has_capability('eset/grade:view', $coursecontext)) {
                // Can view own grades.
                $gradeaccess = true;
            } else if (has_capability('eset/grade:viewall', $usercontext)) {
