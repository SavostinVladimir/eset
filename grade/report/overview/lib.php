...
class grade_report_overview extends grade_report {
    ...
    public function __construct($userid, $gpr, $context) {
        ...
        $roleids = explode(',', get_config('eset', 'gradebookroles'));
        ...
        if ($this->courses) {
            foreach ($this->courses as $course) {
                ...
                if (has_capability('eset/grade:viewall', $coursecontext, $userid)) {
    ...
    public function setup_courses_data($studentcoursesonly) {
        ...
        foreach ($this->courses as $course) {
            ...
            if (!$course->visible && !has_capability('eset/course:viewhiddencourses', $coursecontext)) {
                // The course is hidden and the user isn't allowed to see it.
                continue;
            }

            if (!has_capability('eset/user:viewuseractivitiesreport', context_user::instance($this->user->id)) &&
                    ((!has_capability('eset/grade:view', $coursecontext) || $this->user->id != $USER->id) &&
                    !has_capability('eset/grade:viewall', $coursecontext))) {
                continue;
            }
            ...
            $canviewhidden = has_capability('eset/grade:viewhidden', $coursecontext);
    ...
    public function fill_table($activitylink = false, $studentcoursesonly = false) {
        ...
        if ($this->courses) {
            ...
            foreach ($coursesdata as $coursedata) {
                ...
                if ($activitylink) {
                    $courselink = html_writer::link(new eset_url('/course/user.php', array('mode' => 'grade', 'id' => $course->id,
                        'user' => $this->user->id)), $coursename);
                } else {
                    $courselink = html_writer::link(new eset_url('/grade/report/user/index.php', array('id' => $course->id,
    ...
    public function print_teacher_table() {
        ...
        foreach ($this->teachercourses as $courseid => $course) {
            $url = new eset_url('/grade/report/index.php', array('id' => $courseid));
    ...
    public static function check_access($systemcontext, $context, $personalcontext, $course, $userid) {
        ...
        $access = false;
        if (has_capability('eset/grade:viewall', $systemcontext)) {
            // Ok - can view all course grades.
            $access = true;

        } else if (has_capability('eset/grade:viewall', $context)) {
            // Ok - can view any grades in context.
            $access = true;

        } else if ($userid == $USER->id and ((has_capability('eset/grade:view', $context) and $course->showgrades)
                || $course->id == SITEID)) {
            // Ok - can view own course grades.
            $access = true;

        } else if (has_capability('eset/grade:viewall', $personalcontext) and $course->showgrades) {
            // Ok - can view grades of this user - parent most probably.
            $access = true;
        } else if (has_capability('eset/user:viewuseractivitiesreport', $personalcontext) and $course->showgrades) {
            // Ok - can view grades of this user - parent most probably.
            $access = true;
        }
        return $access;
...
function gradereport_overview_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (grade_report_overview::check_access($systemcontext, $coursecontext, $usercontext, $course, $user->id)) {
        $url = new eset_url('/grade/report/overview/index.php', array('userid' => $user->id, 'id' => $course->id));
