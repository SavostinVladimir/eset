YUI.add('eset-gradereport_history-userselector', function (Y, NAME) {
 ...
 *
 * @module     eset-gradereport_history-userselector
 * @package    gradereport_history
 * @copyright  2013 NetSpot Pty Ltd (https://www.netspot.com.au)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @main       eset-gradereport_history-userselector
 */
...
Y.namespace('M.gradereport_history').UserSelector = Y.extend(USERSELECTOR, M.core.dialogue, {
    ...
    initializer: function() {
                    ...
                    '<div class="{{CSS.SEARCH}}" role="search">' +
                        '<form>' +
                            '<input type="text" class="{{CSS.SEARCHFIELD}}" ' +
                                'aria-label="{{get_string "search" "eset"}}" value="" />' +
                            '<input type="submit" class="{{CSS.SEARCHBTN}}"' +
                                'value="{{get_string "search" "eset"}}">' +
                        '</form>' +
        ...
        content = Y.Node.create(
            tpl({
                COMPONENT: COMPONENT,
                CSS: CSS,
                loadingIcon: M.util.image_url('i/loading', 'eset')
            })
        );
        ...
        this.get('boundingBox').one('.eset-dialogue-wrap').addClass('eset-dialogue-content');
    ...
    processSearchResults: function(tid, outcome, args) {
        ...
        if (!args.append) {
            if (totalUsers === 0) {
                bb.one(SELECTORS.RESULTSCOUNT).setHTML(M.util.get_string('noresults', 'eset'));
                content = '';
            } else {
                if (totalUsers === 1) {
