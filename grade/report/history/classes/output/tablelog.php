...
class tablelog extends \table_sql implements \renderable {
     ...
     * @param \eset_url $url url of the page where this table would be displayed.
    ...
    protected function define_table_columns() {
        ...
        foreach ($extrafields as $field) {
            if (get_string_manager()->string_exists($field, 'eset')) {
        ...
        if (!empty($this->gradeitems[$itemid])) {
            if ($history->itemtype === 'mod' && !$this->is_downloading()) {
                if (!empty($this->cms->instances[$history->itemmodule][$history->iteminstance])) {
                    ...
                    $url = new \eset_url('/mod/' . $history->itemmodule . '/view.php', array('id' => $cm->id));
    ...
    public function col_grader(\stdClass $history) {
        ...
        $profileurl = new \eset_url('/user/view.php', array('id' => $userid, 'course' => $this->courseid));
    ...
    protected function get_filters_sql_and_params() {
        ...
        if ($groupmode == SEPARATEGROUPS && !has_capability('eset/site:accessallgroups', $coursecontext)) {
