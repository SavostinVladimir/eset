...
class grade_report_grader extends grade_report {
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null) {
        ...
        $this->canviewhidden = has_capability('eset/grade:viewhidden', context_course::instance($this->course->id));
        ...
        $this->baseurl = new eset_url('index.php', array('id' => $this->courseid));
        ...
        $this->pbarurl = new eset_url('/grade/report/grader/index.php', array('id' => $this->courseid));

        $this->setup_groups();
        $this->setup_users();
        $this->setup_sortitemid();

        $this->overridecat = (bool)get_config('eset', 'grade_overridecat');
    }
    ...
    public function process_data($data) {
        ...
        $mygroups       = array();
        if ($this->groupmode == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $this->context)) {
            $separategroups = true;
        ...
        foreach ($data as $varname => $students) {
            ...
            foreach ($students as $userid => $items) {
                ...
                foreach ($items as $itemid => $postedvalue) {
                    ...
                    $gradeitem->update_final_grade($userid, $finalgrade, 'gradebook', $feedback, FORMAT_ESET);
    ...
    /**
     * Gets html toggle
     * @deprecated since Eset 2.4 as it appears not to be used any more.
     */
    ...
    public function get_left_rows($displayaverages) {
        ...
        if (get_capability_info('gradereport/singleview:view')) {
            $canseesingleview = has_all_capabilities(array('gradereport/singleview:view', 'eset/grade:viewall',
            'eset/grade:edit'), $this->context);
        }
        ...
        foreach ($this->users as $userid => $user) {
            ...
            $usercell->text = html_writer::link(
                    new eset_url('/user/view.php', ['id' => $user->id, 'course' => $this->course->id]),
            ...
            if ($canseeuserreport) {
                ...
                $url = new eset_url('/grade/report/'.$CFG->grade_profilereport.'/index.php',
            ...
            if ($canseesingleview) {
                ...
                $url = new eset_url('/grade/report/singleview/index.php',
    ...
    public function get_right_rows($displayaverages) {
        ...
        foreach ($this->gtree->get_levels() as $key => $row) {
            ...
            foreach ($row as $columnkey => $element) {
                ...
                if ($type == 'filler' or $type == 'fillerfirst' or $type == 'fillerlast') {
                    ...
                    if (get_capability_info('gradereport/singleview:view')) {
                        if (has_all_capabilities(array('gradereport/singleview:view', 'eset/grade:viewall',
                            'eset/grade:edit'), $this->context)) {

                            $strsingleview = get_string('singleview', 'grades', $element['object']->get_name());
                            $url = new eset_url('/grade/report/singleview/index.php', array(
                                'id' => $this->course->id,
                                'item' => 'grade',
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                // MDL-11274
                // Hide grades in the grader report if the current grader doesn't have 'eset/grade:viewhidden'
    ...
    protected function get_course_header($element) {
        ...
        if ($element['type'] == 'category') {
            ...
            $url = new eset_url($this->gpr->get_return_url(null, array('target' => $element['eid'], 'sesskey' => sesskey())));
     ...
     *
     * @deprecated since Eset 2.9 MDL-46662 - please do not use this function any more.
     */
    ...
    public function get_sort_arrows(array $extrafields = array()) {
        ...
        if (has_capability('eset/site:viewfullnames', $this->context)) {
        ...
        if (!empty($requirednames)) {
            foreach ($requirednames as $name) {
                $arrows['studentname'] .= html_writer::link(
                    new eset_url($this->baseurl, array('sortitemid' => $name)), $this->get_lang_string($name)
                );
        ...
        foreach ($extrafields as $field) {
            $fieldlink = html_writer::link(new eset_url($this->baseurl,
