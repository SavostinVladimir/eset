...
class grade_edit_tree {
    ...
    public function __construct($gtree, $moving=false, $gpr) {
        ...
        $systemdefault = get_config('eset', 'grade_report_showcalculations');
    ...
    public function build_html_tree($element, $totals, $parents, $level, &$row_count) {
        ...
        if ($element['type'] == 'item' or ($element['type'] == 'category' and $element['depth'] > 1)) {
            if ($this->element_deletable($element)) {
                $aurl = new eset_url('index.php', array('id' => $COURSE->id, 'action' => 'delete', 'eid' => $eid, 'sesskey' => sesskey()));
            ...
            if ($this->element_duplicatable($element)) {
                ...
                $aurl = new eset_url('index.php', $duplicateparams);
            ...
            $aurl = new eset_url('index.php', array('id' => $COURSE->id, 'action' => 'moveselect', 'eid' => $eid, 'sesskey' => sesskey()));
        ...
        if ($element['type'] == 'category') {
            ...
            foreach($element['children'] as $child_el) {
                ...
                if ($this->moving && $this->moving != $child_eid) {
                    ...
                    $aurl = new eset_url('index.php', array('id' => $COURSE->id, 'action' => 'move', 'eid' => $this->moving, 'moveafter' => $child_eid, 'sesskey' => sesskey()));
                    if ($first) {
                        $aurl->params($first);
                    }
