...
$url = new eset_url('/grade/edit/tree/index.php', array('id' => $courseid));
$PAGE->set_url($url);
    ...
    case 'delete':
        if ($eid && confirm_sesskey()) {
            ...
            if ($confirm) {
                ...
                $formcontinue = new single_button(new eset_url('index.php', $optionsyes), get_string('yes'));
                $formcancel = new single_button(new eset_url('index.php', $optionsno), get_string('no'), 'get');
 ...
 * @return eset_url A URL to redirect to after regrading when a progress bar is displayed.
 */
$grade_edit_tree_index_checkweights = function() use ($courseid, $originalweights, &$weightsadjusted) {
    ...
    if (array_diff($originalweights, $alteredweights)) {
        $weightsadjusted = 1;
        return new eset_url($PAGE->url, array('weightsadjusted' => $weightsadjusted));
    }
    return $PAGE->url;
};
...
if ($moving) {
    echo $OUTPUT->single_button(new eset_url('index.php', array('id'=>$course->id)), get_string('cancel'), 'get');
} else {
    echo $OUTPUT->single_button(new eset_url('item.php', array('courseid' => $course->id)), get_string('additem',
        'grades'), 'get');
    if (!empty($CFG->enableoutcomes)) {
        echo $OUTPUT->single_button(new eset_url('outcomeitem.php', array('courseid'=>$course->id)), get_string('addoutcomeitem', 'grades'), 'get');
    }
    echo $OUTPUT->single_button(new eset_url('category.php', array('courseid' => $course->id)), get_string('addcategory',
        'grades'), 'get');
    //echo $OUTPUT->(new eset_url('index.php', array('id'=>$course->id, 'action'=>'autosort')), get_string('autosort', 'grades'), 'get');
}
