...
if (isset($courseid) && ($scope  == 'custom')) {
    // custom scale
    $local_scope = true;
} elseif (($scope == 'global') && has_capability('eset/grade:manage', context_system::instance())) {
    // global scale
    $local_scope = false;
} else {
    ...
    while ( $csv_data = fgetcsv($handle, 8192, ';', '"')) { // if the line is over 8k, it won't work...
        ...
        if ($file_headers == '') {
            ...
            if ($error) {
                ...
                echo $OUTPUT->single_button(new eset_url('/grade/edit/outcome/import.php', array('courseid'=> $courseid)), get_string('back'), 'get');
                echo $OUTPUT->box_end();
                $fatal_error = true;
                break;
            }
        ...
        if ($scale) {
            ...
            if (!has_capability('eset/course:managescales', $context)) {
