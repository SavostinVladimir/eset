...
if ($courseid) {
    ...
    require_capability('eset/grade:manageoutcomes', $context);

    if (empty($CFG->enableoutcomes)) {
        redirect('../../index.php?id='.$courseid);
    }
    // This page doesn't exist on the navigation so map it to another
    navigation_node::override_active_url(new eset_url('/grade/edit/outcome/course.php', array('id'=>$courseid)));
} else {
    if (empty($CFG->enableoutcomes)) {
        redirect('../../../');
    }
...
switch ($action) {
    case 'delete':
        ...
        if (empty($outcome->courseid)) {
            require_capability('eset/grade:manage', context_system::instance());
        ...
        if(!$deleteconfirmed){
            ...
            echo $OUTPUT->header();
            $confirmurl = new eset_url('index.php', array(
...
$caneditsystemscales = has_capability('eset/course:managescales', $systemcontext);

if ($courseid) {

    $caneditcoursescales = has_capability('eset/course:managescales', $context);
    ...
    foreach($outcomes as $outcome) {
        ...
        if (empty($scale->id)) {   // hopefully never happens
            ...
            if (empty($scale->courseid)) {
                ...
                $caneditthisscale = has_capability('eset/course:managescales', $context);
            }
            if ($caneditthisscale) {
...
if ($outcomes = grade_outcome::fetch_all_global()) {
    ...
    foreach($outcomes as $outcome) {
        ...
        $buttons = "";
        if (has_capability('eset/grade:manage', context_system::instance())) {
            $buttons .= grade_button('edit', $courseid, $outcome);
        }
        if (has_capability('eset/grade:manage', context_system::instance()) and $outcome->can_delete()) {
            $buttons .= grade_button('delete', $courseid, $outcome);
        }
        $line[] = $buttons;
...
echo $OUTPUT->single_button(new eset_url('edit.php', array('courseid'=>$courseid)), $strcreatenewoutcome);
if ( !empty($outcomes_tables) ) {
    echo $OUTPUT->single_button(new eset_url('export.php', array('id'=>$courseid, 'sesskey'=>sesskey())),  get_string('exportalloutcomes', 'grades'));
}
...
function grade_print_scale_link($courseid, $scale, $gpr) {
    global $CFG, $OUTPUT;
    $url = new eset_url('/grade/edit/scale/edit.php', array('courseid' => $courseid, 'id' => $scale->id));
