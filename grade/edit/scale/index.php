...
if ($courseid) {
    ...
    require_capability('eset/course:managescales', $context);
...
switch ($action) {
    case 'delete':
        ...
        if (empty($scale->courseid)) {
            require_capability('eset/course:managescales', context_system::instance());
...
if ($scales = grade_scale::fetch_all_global()) {
    ...
    foreach($scales as $scale) {
        ...
        $buttons = "";
        if (has_capability('eset/course:managescales', context_system::instance())) {
            $buttons .= grade_button('edit', $courseid, $scale);
        }
        if (!$used and has_capability('eset/course:managescales', context_system::instance())) {
            $buttons .= grade_button('delete', $courseid, $scale);
        }
        $line[] = $buttons;
        $data[] = $line;
...
echo $OUTPUT->single_button(new eset_url('edit.php', array('courseid'=>$courseid)), $srtcreatenewscale);
