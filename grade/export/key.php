...
require_capability('eset/grade:export', $context);
...
if ($id and $delete) {
    if (!$confirm) {
        ...
        $formcontinue = new single_button(new eset_url('key.php', $optionsyes), get_string('yes'), 'get');
        $formcancel = new single_button(new eset_url('keymanager.php', $optionsno), get_string('no'), 'get');
...
$PAGE->navbar->add($strgrades, new eset_url('/grade/index.php', array('id'=>$courseid)));
$PAGE->navbar->add($strkeys, new eset_url('/grade/export/keymanager.php', array('id'=>$courseid)));
