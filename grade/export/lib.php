...
abstract class grade_export {
     ...
     * 2.8 this was simplified to just accept the data from the eset form.
     *
     * @access public
     * @param object $course
     * @param int $groupid
    ...
    public function display_preview($require_user_idnumber=false) {
        ...
        while ($userdata = $gui->next_user()) {
            ...
            foreach ($userprofilefields as $field) {
                ...
                echo '<td>' . format_text($fieldvalue, FORMAT_ESET, $formatoptions) . '</td>';
            }
            if (!$this->onlyactive) {
    ...
    public function get_export_params() {
        ...
        if (!is_array($this->displaytype)) {
            ...
            // Implode the grade display types array as eset_url function doesn't accept arrays.
    ...
    public function print_continue() {
        ...
        if (!$this->userkey) {
            ...
            $url = new eset_url('/grade/export/'.$this->plugin.'/export.php', $params);
     ...
     *
     * @return eset_url the url of grade publishing export.
     */
    public function get_export_url() {
        return new eset_url('/grade/export/'.$this->plugin.'/dump.php', $this->get_export_params());
    }
...
function export_verify_grades($courseid) {
    if (grade_needs_regrade_final_grades($courseid)) {
        throw new eset_exception('gradesneedregrading', 'grades');
    }
}
