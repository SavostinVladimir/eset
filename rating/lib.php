...
class rating implements renderable {
     ...
     * @return eset_url URL to view all of the ratings for the item this rating is for.
     */
    public function get_view_ratings_url($popup = false) {
        ...
        return new eset_url('/rating/index.php', $attributes);
    }
     ...
     * @param eset_url|string $returnurl The URL to return to.
     * @return eset_url can be used to rate the associated item.
     */
    public function get_rate_url($rating = null, $returnurl = null) {
        ...
        $url = new eset_url('/rating/rate.php', $args);
        return $url;
    }
...
class rating_manager {
    ...
    protected function generate_rating_settings_object($options) {
        ...
        $settings->permissions->view    = has_capability('eset/rating:view', $options->context);
        // Can view the aggregate of ratings of other people's items.
        $settings->permissions->viewany = has_capability('eset/rating:viewany', $options->context);
        // Can view individual ratings.
        $settings->permissions->viewall = has_capability('eset/rating:viewall', $options->context);
        // Can submit ratings.
        $settings->permissions->rate    = has_capability('eset/rating:rate', $options->context);
     ...
     *
     * @global eset_database $DB eset database object
     ...
     */
    public function initialise_rating_javascript(eset_page $page) {
        global $CFG;
 ...
 * @since     Eset 2.0
 */
class rating_exception extends eset_exception {
    /**
