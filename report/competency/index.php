...
if (empty($currentuser)) {
    $gradable = get_enrolled_users($context, 'eset/competency:coursecompetencygradable', $currentgroup, 'u.id', null, 0, 1);
    if (empty($gradable)) {
        $currentuser = 0;
    } else {
    ...
    $gradable = get_enrolled_users($context, 'eset/competency:coursecompetencygradable', $currentgroup, 'u.id');
    if (count($gradable) == 0) {
        $currentuser = 0;
...
$navurl = new eset_url('/report/competency/index.php', $urlparams);
$urlparams['user'] = $currentuser;
$urlparams['mod'] = $currentmodule;
$url = new eset_url('/report/competency/index.php', $urlparams);
...
echo $output->header();
$baseurl = new eset_url('/report/competency/index.php');
