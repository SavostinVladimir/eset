...
function report_insights_extend_navigation_course($navigation, $course, $context) {
    ...
    if (\core_analytics\manager::is_analytics_enabled() && has_capability('eset/analytics:listinsights', $context)) {

        $modelids = \core_analytics\manager::cached_models_with_insights($context);
        if (!empty($modelids)) {
            $url = new eset_url('/report/insights/insights.php', array('contextid' => $context->id));
