...
class report_loglive_table_log extends table_sql {
    ...
    public function col_fullnameuser($event) {
        ...
        if (!empty($logextra['realuserid'])) {
            ...
            $a->realusername = html_writer::link(new eset_url("/user/view.php", $params),
                $this->userfullnames[$logextra['realuserid']]);
            $params['id'] = $event->userid;
            $a->asusername = html_writer::link(new eset_url("/user/view.php", $params),
            ...
            $username = html_writer::link(new eset_url("/user/view.php", $params), $this->userfullnames[$event->userid]);
        } else {
            $username = '-';
        }
        return $username;
    ...
    public function col_relatedfullnameuser($event) {
        ...
        if (!empty($event->relateduserid) && isset($this->userfullnames[$event->relateduserid])) {
            ...
            return html_writer::link(new eset_url("/user/view.php", $params), $this->userfullnames[$event->relateduserid]);
        } else {
            return '-';
        }
    }
    ...
    public function col_ip($event) {
        ...
        $url = new eset_url("/iplookup/index.php?ip={$logextra['ip']}&user=$event->userid");
    ...
    public function update_users_and_courses_used() {
        ...
        if (!empty($courseids)) { // If all logs don't belog to site level then get course info.
            ...
            foreach ($courses as $courseid => $course) {
                $url = new eset_url("/course/view.php", array('id' => $courseid));
