...
class report_log_events_testcase extends advanced_testcase {
     ...
     *
     * It's not possible to use the eset API to simulate the viewing of log report, so here we
    ...
    public function test_report_viewed() {
        ...
        $url = new eset_url('/report/log/index.php', array('id' => $event->courseid));
     ...
     *
     * It's not possible to use the eset API to simulate the viewing of user log report, so here we
     * simply create the event and trigger it.
     */
    public function test_user_report_viewed() {
        ...
        $this->assertEventLegacyLogData($expected, $event);
        $this->assertEventContextNotUsed($event);
        $url = new eset_url('/report/log/user.php', array('course' => $course->id, 'id' => $user->id, 'mode' => 'today'));
        $this->assertEquals($url, $event->get_url());
    }
}
