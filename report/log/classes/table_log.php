...
class report_log_table_log extends table_sql {
     ...
     *
     * @deprecated since Eset 2.9 MDL-48595 - please do not use this function any more.
     */
    ...
    public function col_fullnameuser($event) {
        ...
        if (!empty($logextra['realuserid'])) {
            ...
            if (empty($this->download)) {
                ...
                $a->realusername = html_writer::link(new eset_url('/user/view.php', $params), $a->realusername);
                $params['id'] = $event->userid;
                $a->asusername = html_writer::link(new eset_url('/user/view.php', $params), $a->asusername);
            }
            ...
            if (empty($this->download)) {
                ...
                $username = html_writer::link(new eset_url('/user/view.php', $params), $username);
            }
        } else {
            $username = '-';
        }
    ...
    public function col_ip($event) {
        ...
        if (empty($this->download)) {
            $url = new eset_url("/iplookup/index.php?ip={$ip}&user={$event->userid}");
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if (!($this->filterparams->logreader instanceof logstore_legacy\log\store)) {
            ...
            if (!has_capability('eset/site:viewanonymousevents', $context)) {
                $joins[] = "anonymous = 0";
            }
        }
