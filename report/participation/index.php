...
$url = new eset_url('/report/participation/index.php', array('id'=>$id));
...
$baseurl = new eset_url('/report/participation/index.php', array(
    'id' => $course->id,
    'roleid' => $roleid,
...
if (!empty($instanceid) && !empty($roleid)) {
    ...
    foreach ($users as $u) {
        $data = array();
        $data[] = html_writer::link(new eset_url('/user/view.php', array('id' => $u->userid, 'course' => $course->id)),
            fullname($u, true));
    ...
    if ($perpage == SHOW_ALL_PAGE_SIZE) {
        $perpageurl = new eset_url($baseurl, array('perpage' => DEFAULT_PAGE_SIZE));
        ...
        $perpageurl = new eset_url($baseurl, array('perpage' => SHOW_ALL_PAGE_SIZE));
        echo html_writer::start_div('', array('id' => 'showall'));
