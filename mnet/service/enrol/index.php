...
$roamingusers = get_users_by_capability(context_system::instance(), 'eset/site:mnetlogintoremote', 'u.id');
if (empty($roamingusers)) {
    $capname = get_string('site:mnetlogintoremote', 'role');
    $url = new eset_url('/admin/roles/manage.php');
...
foreach ($hosts as $host) {
    $hostlink = html_writer::link(new eset_url($host->hosturl), s($host->hosturl));
    $editbtn  = $OUTPUT->single_button(new eset_url('/mnet/service/enrol/host.php', array('id'=>$host->id)),
