    ...
    public function get_remote_courses($mnethostid, $usecache=true) {
        ...
        if ($request->send($peer)) {
            ...
            foreach ($response as &$remote) {
                ...
                $course->summaryformat  = empty($remote['summaryformat']) ? FORMAT_ESET : (int)$remote['summaryformat'];
...
class mnetservice_enrol_potential_users_selector extends user_selector_base {
     ...
     *
     * Our users must have 'eset/site:mnetlogintoremote' capability assigned.
    ...
    public function find_users($search) {
        ...
        $userids = get_users_by_capability($systemcontext, 'eset/site:mnetlogintoremote', 'u.id');
