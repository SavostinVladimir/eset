...
function mnet_get_public_key($uri, $application=null) {
    ...
    if (empty($application)) {
        $application = $DB->get_record('mnet_application', array('name'=>'eset'));
    }
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Eset');
...
function mnet_sign_message($message, $privatekey = null) {
    ...
    <signedMessage>
        <Signature Id="EsetSignature" xmlns="http://www.w3.org/2000/09/xmldsig#">
            <SignedInfo>
 ...
 *     This is a URL associated with a eset networking peer when it
 ...
 *     them back to somewhere at the content provider(CP) eset (this eset)
...
function mnet_sso_apply_indirection ($jumpurl, $url) {
    ...
    if($urlparts) {
        if (isset($urlparts['path'])) {
            ...
            // 'localpart' to make it relative to eset's wwwroot
...
/**
 * Return an array of information about all eset's profile fields
...
function _mnet_field_helper(mnet_peer $peer, $key) {
    ...
    $defaults = explode(',', get_config('eset', 'mnetprofile' . $key . 'fields'));
