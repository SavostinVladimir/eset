...
class mnet_peer {
    ...
    var $force_theme        = 0;
    var $theme              = '';
    var $applicationid      = 1; // Default of 1 == Eset
    var $keypair            = array();
    var $error              = array();
    ...
    function bootstrap($wwwroot, $pubkey = null, $application) {
        ...
        if ($ip_address === $hostname && !preg_match('/^\d+\.\d+\.\d+.\d+$/',$hostname)) {
            throw new eset_exception('noaddressforhost', 'mnet', '', $hostname);
        }
        ...
        if (empty($this->application)) {
            $this->application = $DB->get_record('mnet_application', array('name'=>'eset'));
        }
