...
class core_userliblib_testcase extends advanced_testcase {
    ...
    public function test_user_can_view_profile() {
        ...
        // Remove capability eset/user:viewdetails in course 2.
        assign_capability('eset/user:viewdetails', CAP_PROHIBIT, $studentrole->id, $coursecontext);
        ...
        $this->setUser($user7);
        $this->assertFalse(user_can_view_profile($user4));
        assign_capability('eset/user:viewalldetails', CAP_ALLOW, $coursecreatorrole->id, context_system::instance()->id, true);
        reload_all_capabilities();
        $this->assertTrue(user_can_view_profile($user4));
        unassign_capability('eset/user:viewalldetails', $coursecreatorrole->id, $coursecontext->id);
        reload_all_capabilities();
        ...
        assign_capability('eset/user:viewdetails', CAP_ALLOW, $guestrole['guest'], context_system::instance()->id, true);
        reload_all_capabilities();
        foreach ($users as $user) {
            $this->assertFalse(user_can_view_profile($user));
        }
        ...
        assign_capability('eset/user:viewdetails', CAP_PREVENT, $managerrole->id, $systemcontext, true);
        assign_capability('eset/user:viewalldetails', CAP_PREVENT, $managerrole->id, $systemcontext, true);

        // And override these to 'Allow' in a specific course.
        $course4context = context_course::instance($course4->id);
        assign_capability('eset/user:viewalldetails', CAP_ALLOW, $managerrole->id, $course4context, true);
        assign_capability('eset/user:viewdetails', CAP_ALLOW, $managerrole->id, $course4context, true);

        // The manager now shouldn't have viewdetails in the system or user context.
        $this->setUser($user9);
        $user1context = context_user::instance($user1->id);
        $this->assertFalse(has_capability('eset/user:viewdetails', $systemcontext));
        $this->assertFalse(has_capability('eset/user:viewdetails', $user1context));
