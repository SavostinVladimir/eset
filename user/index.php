...
$bulkoperations = has_capability('eset/course:bulkmessaging', $context);
...
$perpageurl = new eset_url('/user/index.php', [
...
if ($bulkoperations) {
    ...
    if ($participanttable->get_page_size() < $participanttable->totalrows) {
        ...
        $label = get_string('selectalluserswithcount', 'eset', $participanttable->totalrows);
    ...
    $displaylist = array();
    if (!empty($CFG->messaging) && has_all_capabilities(['eset/site:sendmessage', 'eset/course:bulkmessaging'], $context)) {
        $displaylist['#messageselect'] = get_string('messageselectadd');
    }
    if (!empty($CFG->enablenotes) && has_capability('eset/notes:manage', $context) && $context->id != $frontpagectx->id) {
    ...
    foreach ($formats as $format) {
        if ($format->is_enabled()) {
            ...
            $url = new eset_url('bulkchange.php', $params);
