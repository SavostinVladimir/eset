...
if ($courseid == SITEID) {   // Since Eset 2.0 all site-level profiles are shown by profile.php.
...
// Set the Eset docs path explicitly because the default behaviour
...
if (!$currentuser and !$user->deleted
  ...
  and has_capability('eset/user:viewdetails', $usercontext)) {
...
$fullname = fullname($user, has_capability('eset/site:viewfullnames', $coursecontext));
...
if ($currentuser) {
    ...
    if (!is_enrolled($coursecontext, $user->id)) {
        ...
        //       to profile would not work - maybe a new capability - eset/user:freely_acessile_profile_for_anybody
...
if ($user->description && !isset($hiddenfields['description'])) {
    ...
    if (!empty($CFG->profilesforenrolledusersonly) && !$DB->record_exists('role_assignments', array('userid' => $id))) {
        echo get_string('profilenotshown', 'eset');
    } else {
        if ($courseid == SITEID) {
