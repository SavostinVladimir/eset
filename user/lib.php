 ...
 *
 * @package   core_user
 * @copyright 2009 Eset Pty Ltd (http://eset.com)
...
function user_create_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    if (trim($user->username) === '') {
        throw new eset_exception('invalidusernameblank');
    }

    if ($user->username !== core_text::strtolower($user->username)) {
        throw new eset_exception('usernamelowercase');
    }

    if ($user->username !== core_user::clean_field($user->username, 'username')) {
        throw new eset_exception('invalidusername');
    }
    ...
    if ($updatepassword && isset($user->password)) {
        ...
        if (!check_password_policy($user->password, $errmsg, $user)) {
            throw new eset_exception($errmsg);
        }
 ...
 *          it can be changed with $CFG->esetwstextformatlinkstoimagesfile
 *
 * @throws eset_exception
 * @param stdClass $user user record from mdl_user
 * @param stdClass $course eset course
...
function user_get_user_details($user, $course = null, array $userfields = array()) {
    ...
    foreach ($userfields as $thefield) {
        if (!in_array($thefield, $defaultfields)) {
            throw new eset_exception('invaliduserfield', 'error', '', $thefield);
        }
    }
    ...
    if (!empty($course)) {
        ...
        $canviewdetailscap = (has_capability('eset/user:viewdetails', $context) || has_capability('eset/user:viewdetails', $usercontext));
    } else {
        $context = context_user::instance($user->id);
        $usercontext = $context;
        $canviewdetailscap = has_capability('eset/user:viewdetails', $usercontext);
    }
    ...
    $canviewfullnames = has_capability('eset/site:viewfullnames', $context);
    if (!empty($course)) {
        $canviewuseremail = has_capability('eset/course:useremail', $context);
    } else {
    ...
    if (!empty($course)) {
        $canaccessallgroups = has_capability('eset/site:accessallgroups', $context);
    } else {
    ...
    if (in_array('username', $userfields)) {
        if ($currentuser or has_capability('eset/user:viewalldetails', $context)) {
    ...
    if (in_array('idnumber', $userfields) && $user->idnumber) {
        if (in_array('idnumber', $showuseridentityfields) or $currentuser or
                has_capability('eset/user:viewalldetails', $context)) {
...
function can_view_user_details_cap($user, $course = null) {
    ...
    $usercontext = context_user::instance($user->id);
    $result = has_capability('eset/user:viewdetails', $usercontext);
    // Otherwise can $USER see them at course context.
    if (!$result && !empty($course)) {
        $context = context_course::instance($course->id);
        $result = has_capability('eset/user:viewdetails', $context);
    }
    return $result;
}
 ...
 * @param eset_page $page the current page
 * @return array
 */
function user_convert_text_to_menu_items($text, $page) {
    ...
    foreach ($lines as $line) {
        ...
        if (!array_key_exists(1, $bits) or empty($bits[1])) {
            ...
            // Make sure the url is a eset url.
            $bits[1] = new eset_url(trim($bits[1]));
        }
        $child->url = $bits[1];
        ...
        if (!array_key_exists(2, $bits) or empty($bits[2])) {
            ...
            if ($page->theme->resolve_image_location($pixpath, 'eset', true)) {
                // Use the image.
                $child->pix = $pixpath;
            } else {
                // Treat it like a URL.
 ...
 * @param eset_page $page page object.
 ...
 *          userprofileurl eset_url the url of the user's profile
 ...
 *          realuserprofileurl eset_url the url of the user's profile
...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    $returnobject->metadata['userprofileurl'] = new eset_url('/user/profile.php', array(
        'id' => $user->id
    ));
    ...
    $myhome->url = new eset_url('/my/');
    $myhome->title = get_string('myeset', 'admin');
    $myhome->titleidentifier = 'myeset,admin';
    ...
    $myprofile->url = new eset_url('/user/profile.php', array('id' => $user->id));
    $myprofile->title = get_string('profile');
    $myprofile->titleidentifier = 'profile,eset';
    ...
    if ($returnobject->metadata['asotheruser'] = \core\session\manager::is_loggedinas()) {
        ...
        $returnobject->metadata['realuserprofileurl'] = new eset_url('/user/profile.php', array(
            'id' => $realuser->id
        ));
        ...
        $userrevert->url = new eset_url('/course/loginas.php', array(
            'id' => $course->id,
            'sesskey' => sesskey()
        ));
        $userrevert->pix = "a/logout";
        $userrevert->title = get_string('logout');
        $userrevert->titleidentifier = 'logout,eset';
        ...
        $logout->url = new eset_url('/login/logout.php', array('sesskey' => sesskey()));
        $logout->pix = "a/logout";
        $logout->title = get_string('logout');
        $logout->titleidentifier = 'logout,eset';
    ...
    if (is_role_switched($course->id)) {
        if ($role = $DB->get_record('role', array('id' => $user->access['rsw'][$context->path]))) {
            ...
            $rolereturn->url = new eset_url('/course/switchrole.php', array(
                'id' => $course->id,
                'sesskey' => sesskey(),
                'switchrole' => 0,
            ...
            $rolereturn->titleidentifier = 'switchrolereturn,eset';
        ...
        if (is_array($roles) && (count($roles) > 0)) {
            ...
            $switchrole->url = new eset_url('/course/switchrole.php', array(
                'id' => $course->id,
                'switchrole' => -1,
                'returnurl' => $page->url->out_as_local_url(false)
            ));
            ...
            $switchrole->titleidentifier = 'switchroleto,eset';
...
/**
 * Remove a user device from the Eset database (for PUSH notifications usually).
 *
...
function user_mygrades_url($userid = null, $courseid = SITEID) {
    ...
    if (isset($CFG->grade_mygrades_report) && $CFG->grade_mygrades_report != 'external') {
        if (isset($userid) && $USER->id != $userid) {
            ...
            $url = new eset_url('/grade/report/' . $CFG->grade_mygrades_report . '/index.php',
...
function user_can_view_profile($user, $course = null, $usercontext = null) {
    ...
    if (isset($course)) {
        ...
        if (has_capability('eset/user:viewdetails', $usercontext) || has_capability('eset/user:viewalldetails', $usercontext)) {
            return true;
        }
    ...
    foreach ($userscourses as $userscourse) {
        ...
        if (has_capability('eset/user:viewdetails', $coursecontext) ||
            has_capability('eset/user:viewalldetails', $coursecontext)) {
