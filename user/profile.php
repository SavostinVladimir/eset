...
if (isguestuser()) {     // Guests can never edit their profile.
    ...
    if ($currentuser) {
        $PAGE->set_blocks_editing_capability('eset/user:manageownblocks');
    } else {
        $PAGE->set_blocks_editing_capability('eset/user:manageblocks');
    }
}
...
if ($PAGE->user_allowed_editing()) {
    if ($reset !== null) {
        if (!is_null($userid)) {
            ...
            redirect(new eset_url('/user/profile.php', array('id' => $userid)));
        }
    ...
    $reseturl = new eset_url("$CFG->wwwroot/user/profile.php", array('edit' => 1, 'reset' => 1, 'id' => $userid));
    ...
    $url = new eset_url("$CFG->wwwroot/user/profile.php", $params);
