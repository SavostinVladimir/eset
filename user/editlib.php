...
function useredit_setup_preference_page($userid, $courseid) {
    ...
    if ($user->id == $USER->id) {
        ...
        if (!has_capability('eset/user:editownprofile', $systemcontext)) {
...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    $mform->addElement('text', 'esetnetprofile', get_string('esetnetprofile', 'user'));
    $mform->setType('esetnetprofile', PARAM_NOTAGS);
    $mform->addHelpButton('esetnetprofile', 'esetnetprofile', 'user');
    ...
    if (empty($USER->newadminuser)) {
        $mform->addElement('header', 'eset_picture', get_string('pictureofuser'));
        $mform->setExpanded('eset_picture', true);
    ...
    if (count($disabledusernamefields) > 0) {
        $mform->addElement('header', 'eset_additional_names', get_string('additionalnames'));
    ...
    if (core_tag_tag::is_enabled('core', 'user') and empty($USER->newadminuser)) {
        $mform->addElement('header', 'eset_interests', get_string('interests'));
        $mform->addElement('tags', 'interests', get_string('interestslist'),
            array('itemtype' => 'user', 'component' => 'core'));
        $mform->addHelpButton('interests', 'interestslist');
    }

    // Eset optional fields.
    $mform->addElement('header', 'eset_optional', get_string('optional', 'form'));
