...
abstract class user_selector_base {
    ...
    protected static $jsmodule = array(
                ...
                'requires'  => array('node', 'event-custom', 'datasource', 'json', 'eset-core-notification'),
                'strings' => array(
                    array('previouslyselectedusers', 'eset', '%%SEARCHTERM%%'),
                    array('nomatchingusers', 'eset', '%%SEARCHTERM%%'),
                    array('none', 'eset')
                ));
    ...
    public function get_selected_user() {
        if ($this->multiselect) {
            throw new eset_exception('cannotcallusgetselecteduser');
        }
        ...
        if (count($users) == 1) {
            ...
            throw new eset_exception('userselectortoomany');
        }
    }
