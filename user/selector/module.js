...
M.core_user.init_user_selector = function (Y, name, hash, extrafields, lastsearch) {
    ...
    var user_selector = {
        ...
        send_query : function(forceresearch) {
            ...
            this.listbox.setStyle('background','url(' + M.util.image_url('i/loading', 'eset') + ') no-repeat center center');
        },
        /**
        ...
        output_options : function(data) {
            ...
            if (!count) {
                var searchstr = (this.lastsearch != '') ? this.insert_search_into_str(M.util.get_string('nomatchingusers', 'eset'), this.lastsearch) : M.util.get_string('none', 'eset');
            ...
            if (this.get_option('preserveselected') && selectedusers) {
                this.output_group(this.insert_search_into_str(M.util.get_string('previouslyselectedusers', 'eset'), this.lastsearch), selectedusers, true, false);
            }
