...
class core_user_external extends external_api {
    ...
    public static function create_users_parameters() {
        ...
        $userfields = [
            ...
            'username' => new external_value(core_user::get_property_type('username'),
                'Username policy is defined in Eset security config.'),
    ...
    public static function create_users($users) {
        ...
        require_capability('eset/user:create', $context);
    ...
    public static function delete_users($userids) {
        ...
        // Ensure the current user is allowed to run this function.
        $context = context_system::instance();
        require_capability('eset/user:delete', $context);
        self::validate_context($context);
        ...
        foreach ($params['userids'] as $userid) {
            ...
            if (is_siteadmin($user)) {
                throw new eset_exception('useradminodelete', 'error');
            }
            if ($USER->id == $user->id) {
                throw new eset_exception('usernotdeletederror', 'error');
            }
    ...
    public static function update_users_parameters() {
        $userfields = [
            ...
            // General.
            'username' => new external_value(core_user::get_property_type('username'),
                'Username policy is defined in Eset security config.', VALUE_OPTIONAL, '', NULL_NOT_ALLOWED),
            'auth' => new external_value(core_user::get_property_type('auth'), 'Auth plugins include manual, ldap, etc',
    ...
    public static function update_users($users) {
        ...
        // Ensure the current user is allowed to run this function.
        $context = context_system::instance();
        require_capability('eset/user:update', $context);
        self::validate_context($context);
    ...
    public static function get_users($criteria = array()) {
        ...
        foreach ($params['criteria'] as $criteriaindex => $criteria) {
            ...
            if (array_key_exists($criteria['key'], $usedkeys)) {
                throw new eset_exception('keyalreadyset', '', '', null, 'The key ' . $criteria['key'] . ' can only be sent once');
            } else {
    ...
    public static function add_user_private_files($draftid) {
        ...
        require_capability('eset/user:manageownfiles', $context);

        $maxbytes = $CFG->userquota;
        $maxareabytes = $CFG->userquota;
        if (has_capability('eset/user:ignoreuserquota', $context)) {
    ...
    public static function add_user_device_parameters() {
        return new external_function_parameters(
            array(
                'appid'     => new external_value(PARAM_NOTAGS, 'the app id, usually something like com.eset.esetmobile'),
    ...
    /**
     * Add a user device in Eset database (for PUSH notifications usually).
     *
     * @throws eset_exception
     * @param string $appid The app id, usually something like com.eset.esetmobile.
    ...
    public static function add_user_device($appid, $name, $model, $platform, $version, $pushid, $uuid) {
        ...
        if ($userdevices = $DB->get_records('user_devices', array('uuid' => $params['uuid'],
            ...
            if (!$DB->insert_record('user_devices', $userdevice)) {
                throw new eset_exception("There was a problem saving in the database the device with key: " . $params['pushid']);
            }
        }
    ...
    public static function view_user_profile($userid, $courseid = 0) {
        ...
        if (!$currentuser and
                !has_capability('eset/user:viewdetails', $coursecontext) and
                !has_capability('eset/user:viewdetails', $usercontext)) {
            throw new eset_exception('cannotviewprofile');
        }
        ...
        if ($course->id == SITEID) {
            ...
            if (!$currentuser and !can_access_course($course, $user, '', true)) {
                throw new eset_exception('notenrolledprofile');
            }
    ...
    public static function get_user_preferences($name = '', $userid = 0) {
        ...
        if (empty($params['userid'])) {
            ...
            if ($user->id != $USER->id) {
                ...
                require_capability('eset/site:config', $context);
            }
        }
    ...
    public static function update_picture($draftitemid, $delete = false, $userid = 0) {
        ...
        if (!empty($CFG->disableuserimages)) {
            throw new eset_exception('userimagesdisabled', 'admin');
        }

        if (empty($params['userid']) or $params['userid'] == $USER->id) {
            $user = $USER;
            require_capability('eset/user:editownprofile', $context);
        } else {
            $user = core_user::get_user($params['userid'], '*', MUST_EXIST);
            core_user::require_active_user($user);
            $personalcontext = context_user::instance($user->id);

            require_capability('eset/user:editprofile', $personalcontext);
            if (is_siteadmin($user) and !is_siteadmin($USER)) {  // Only admins may edit other admins.
                throw new eset_exception('useradmineditadmin');
            }
        }
        ...
        if (is_mnet_remote_user($user) or !$userauth->can_edit_profile() or $userauth->edit_profile_url()) {
            throw new eset_exception('noprofileedit', 'auth');
        }
    ...
    public static function agree_site_policy() {
        ...
        try {
            ...
            if (!($e instanceof eset_exception) or $e->errorcode != 'sitepolicynotagreed') {
    ...
    public static function get_private_files_info($userid = 0) {
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            $usercontext = context_user::instance($USER->id);
            require_capability('eset/user:manageownfiles', $usercontext);
        } else {
            $user = core_user::get_user($params['userid'], '*', MUST_EXIST);
            core_user::require_active_user($user);
