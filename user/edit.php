...
// Create form.
$userform = new user_edit_form(new eset_url($PAGE->url, array('returnto' => $returnto)), array(
...
if ($returnto === 'profile') {
    if ($course->id != SITEID) {
        $returnurl = new eset_url('/user/view.php', array('id' => $user->id, 'course' => $course->id));
    } else {
        $returnurl = new eset_url('/user/profile.php', array('id' => $user->id));
    }
} else {
    $returnurl = new eset_url('/user/preferences.php', array('userid' => $user->id));
}
...
if ($userform->is_cancelled()) {
    ...
    if ($CFG->emailchangeconfirmation) {
        // Users with 'eset/user:update' can change their email address immediately.
        // Other users require a confirmation email.
        if (isset($usernew->email) and $user->email != $usernew->email && !has_capability('eset/user:update', $systemcontext)) {
            $a = new stdClass();
