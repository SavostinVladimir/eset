...
class core_user_renderer extends plugin_renderer_base {
    ...
    public function user_search($url, $firstinitial, $lastinitial, $usercount, $totalcount, $heading = null) {
        ...
        $content = html_writer::start_tag('form', array('action' => new eset_url($url)));
    ...
    public function user_list($userlist, $exclusivemode) {
        ...
        foreach ($userlist as $user) {
            ...
            if (user_can_view_profile($user)) {
                $profilelink = new eset_url('/user/view.php', array('id' => $user->id));
     ...
     * @deprecated since Eset 3.9 MDL-68612 - Please use participants_filter() instead.
     *
     * @param stdClass $course The course object.
     * @param context $context The context object.
     * @param array $filtersapplied Array of currently applied filters.
     * @param string|eset_url $baseurl The url with params needed to call up this page.
     * @return bool|string
     */
    public function unified_filter($course, $context, $filtersapplied, $baseurl = null) {
        ...
        $roleseditable = has_capability('eset/role:assign', $context);
        ...
        if (has_capability('eset/site:accessallgroups', $context) || $course->groupmode != SEPARATEGROUPS) {
        ...
        $canreviewenrol = has_capability('eset/course:enrolreview', $context);
        ...
        $hiddenfields = array();
        if (!has_capability('eset/course:viewhiddenuserfields', $context)) {
        ...
        if ($haslastaccess) {
            ...
            for ($i = 1; $i < 7; $i++) {
                ...
                $value = get_string('numdays', 'eset', $i);
            ...
            for ($i = 1; $i < 10; $i++) {
                ...
                $value = get_string('numweeks', 'eset', $i);
                $timeoptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $value);
            }
            // Months.
            for ($i = 2; $i < 12; $i++) {
                ...
                $value = get_string('nummonths', 'eset', $i);
                $timeoptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $value);
            }
            // Try a year.
            $timestamp = strtotime('-1 year', $now);
            if ($timestamp >= $minlastaccess) {
                $value = get_string('numyear', 'eset', 1);
                $timeoptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $value);
            }
            if (!empty($lastaccess0exists)) {
                $value = get_string('never', 'eset');
                $timeoptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $value);
            }
    ...
    protected function format_filter_option($filtertype, $criteria, $value, $label) {
        $optionlabel = get_string('filteroption', 'eset', (object)['criteria' => $criteria, 'value' => $label]);
                ...
                switch($key) {
                    case USER_FILTER_LAST_ACCESS:
                        ...
                        for ($i = 1; $i < 10; $i++) {
                            ...
                            $val = get_string('numweeks', 'eset', $i);
                            $filteroptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $val);
                        }
                        // Months.
                        for ($i = 2; $i < 12; $i++) {
                            ...
                            $val = get_string('nummonths', 'eset', $i);
                            $filteroptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $val);
                        }
                        // Try a year.
                        $timestamp = strtotime('-1 year', $now);
                        if ($timestamp >= $value) {
                            $val = get_string('numyear', 'eset', 1);
                            $filteroptions += $this->format_filter_option(USER_FILTER_LAST_ACCESS, $criteria, $timestamp, $val);
                        }
                        break;
