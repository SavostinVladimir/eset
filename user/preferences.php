...
if (!$currentuser) {
    ...
    $url = new eset_url('/user/preferences.php', array('userid' => $userid));
    $navbar = $PAGE->navbar->add(get_string('preferences', 'eset'), $url);
    ...
    if (!$PAGE->settingsnav->can_view_user_preferences($userid)) {
        throw new eset_exception('cannotedituserpreferences', 'error');
    }
} else {
