 ...
 * Editable by the profile owner if they have the eset/user:editownprofile capability
 * or any user with the eset/user:update capability.
 */
define('PROFILE_VISIBLE_ALL', '2');
/**
 * Visible to the profile owner or anyone with the eset/user:viewalldetails capability.
 * Editable by the profile owner if they have the eset/user:editownprofile capability
 * or any user with eset/user:viewalldetails and eset/user:update capabilities.
 */
define('PROFILE_VISIBLE_PRIVATE', '1');
/**
 * Only visible to users with the eset/user:viewalldetails capability.
 * Only editable by users with the eset/user:viewalldetails and eset/user:update capabilities.
 */
...
class profile_field_base {
    ...
    public function __construct($fieldid=0, $userid=0, $fielddata=null) {
        ...
        if ($CFG->debugdeveloper) {
            // In Eset 3.4 the new argument $fielddata was added to the constructor. Make sure that
    ...
    /**
     * Abstract method: Adds the profile field to the eset form class
     * @abstract The following methods must be overwritten by child classes
     * @param esetform $mform instance of the esetform class
     */
    ...
    public function display_data() {
        ...
        $options->para = false;
        return format_text($this->data, FORMAT_ESET, $options);
    }
    ...
    public function edit_field_set_locked($mform) {
        ...
        if ($this->is_locked() and !has_capability('eset/user:update', context_system::instance())) {
        ...
        switch ($this->field->visible) {
            ...
            case PROFILE_VISIBLE_PRIVATE:
                if ($this->is_signup_field() && (empty($this->userid) || isguestuser($this->userid))) {
                    ...
                    return has_capability('eset/user:viewalldetails', $context);
                }
            default:
                return has_capability('eset/user:viewalldetails', $context);
        }
    ...
    public function is_editable() {
        ...
        if ($this->userid == $USER->id && has_capability('eset/user:editownprofile', $systemcontext)) {
            return true;
        }

        if (has_capability('eset/user:update', $systemcontext)) {
            return true;
        }
...
/**
 * Adds code snippet to a eset form object for custom profile fields that
 * should appear on the signup page
 * @param esetform $mform eset form object
 */
