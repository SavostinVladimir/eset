YUI.add('eset-core_availability-form', function (Y, NAME) {
 ...
 *
 * @module eset-core_availability-form
 */
...
M.core_availability.form = {
    ...
    init: function(pluginParams) {
        ...
        // these controls as Behat incorrectly decides they're a esetform
        // textarea. IMO Behat should not know about esetforms at all and
        // should look purely at HTML elements on the page, but until it is
        // fixed to do this or fixed in some other way to only detect esetform
...
M.core_availability.plugin = {
     ...
     *
     * Errors are Eset language strings in format component:string, e.g.
...
M.core_availability.List = function(json, root, parentRoot) {
    ...
    this.node = Y.Node.create('<div class="availability-list"><h3 class="accesshide"></h3>' +
            ...
            '<div class="availability-none"><span class="px-3">' + M.util.get_string('none', 'eset') + '</span></div>' +
...
M.core_availability.List.prototype.clickAdd = function() {
    var content = Y.Node.create('<div>' +
            ...
            '<button type="button" class="btn btn-secondary">' + M.util.get_string('cancel', 'eset') +
            '</button></div></div>');
...
M.core_availability.DeleteIcon = function(toDelete) {
    this.span = Y.Node.create('<a class="d-inline-block col-form-label availability-delete px-3" href="#" title="' +
            M.util.get_string('delete', 'eset') + '" role="button">');
    var img = Y.Node.create('<img src="' + M.util.image_url('t/delete', 'core') +
            '" alt="' + M.util.get_string('delete', 'eset') + '" />');
