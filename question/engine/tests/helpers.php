...
class test_question_maker {
    ...
    public static function make_an_essay_question() {
        ...
        $essay->responsetemplateformat = FORMAT_ESET;
        $essay->graderinfo = '';
        $essay->graderinfoformat = FORMAT_ESET;
 ...
 *
 * Historically, tests like this were made because Eset did not have anything
 ...
 *    Eset used SimpleTest for unit tests and check_current_output os written in a
...
abstract class qbehaviour_walkthrough_test_base extends question_testcase {
    ...
    protected function save_quba(eset_database $db = null) {
        question_engine::save_questions_usage_by_activity($this->quba, $db);
    }

    protected function load_quba(eset_database $db = null) {
...
/**
 * Simple class that implements the {@link eset_recordset} API based on an
 * array of test data.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_test_recordset extends eset_recordset {
    protected $records;
