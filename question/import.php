...
if ($form = $import_form->get_data()) {
    ...
    if (!$result = $import_form->save_file('newfile', $importfile, true)) {
        throw new eset_exception('uploadproblem');
    }

    $formatfile = 'format/' . $form->format . '/format.php';
    if (!is_readable($formatfile)) {
        throw new eset_exception('formatnotfound', 'question', '', $form->format);
    }
    ...
    echo $OUTPUT->continue_button(new eset_url('edit.php', $params));
    echo $OUTPUT->footer();
    exit;
}
