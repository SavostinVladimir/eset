    ...
    public function readquestions ($lines) {
        ...
        foreach ($lines as $line) {
            ...
            if (preg_match("~^:TYPE:~i", $line)) {
                ...
                unset($question);
                $ignorerestofquestion = true;         // Question Type not handled by Eset.
                continue;
            }
            ...
            if (preg_match('~^:LAYOUT:(.*)~i', $line, $webctoptions)) {
                // Ignore  since layout in question_multichoice  is no more used in Eset.
            ...
            if (isset($question->qtype )&& 'calculated' == $question->qtype && preg_match('~^:TOLTYPE:percent~i', $line)) {
                // Percentage case is handled as relative in Eset.
