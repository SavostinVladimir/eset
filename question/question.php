...
$url = new eset_url('/question/question.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($makecopy) {
...
if ($cmid) {
    $questionbankurl = new eset_url('/question/edit.php', array('cmid' => $cmid));
} else {
    $questionbankurl = new eset_url('/question/edit.php', array('courseid' => $courseid));
}
...
if ($originalreturnurl) {
    ...
    $returnurl = new eset_url($originalreturnurl);
} else {
...
if ($id) {
    ...
    $addurl = new eset_url('/question/addquestion.php', $url->params());
...
$addpermission = has_capability('eset/question:add', $categorycontext);
...
if ($id) {
    ...
    $formeditable = true;
    require_capability('eset/question:add', $categorycontext);
}
...
if ($mform->is_cancelled()) {
    ...
    if (!empty($question->id)) {
        ...
        require_capability('eset/question:add', context::instance_by_id($contextid));
    ...
    if ($qtypeobj->finished_edit_wizard($fromform)) {
        ...
        $nexturl = new eset_url('/question/question.php', $nexturlparams);
        if ($cmid){
