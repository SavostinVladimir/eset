...
class delete_action_column extends menu_action_column_base {
    ...
    protected function get_url_icon_and_label(\stdClass $question): array {
        ...
        if ($question->hidden) {
            $url = new \eset_url($this->qbank->base_url(), array('unhide' => $question->id, 'sesskey' => sesskey()));
            return [$url, 't/restore', $this->strrestore];
        } else {
            $url = new \eset_url($this->qbank->base_url(), array('deleteselected' => $question->id, 'q' . $question->id => 1,
                    'sesskey' => sesskey()));
