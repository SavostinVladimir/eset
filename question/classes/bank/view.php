...
class view {
    ...
    /**
     * @var \eset_url base URL for the current page. Used as the
     * basis for making URLs for actions that reload the page.
     */
    protected $baseurl;

    /**
     * @var \eset_url used as a basis for URLs that edit a question.
     */
    ...
    public function __construct($contexts, $pageurl, $course, $cm = null) {
        ...
        $this->editquestionurl = new \eset_url('/question/question.php',
    ...
    protected function wanted_columns() {
        ...
        if (question_get_display_preference('qbshowtext', 0, PARAM_BOOL, new \eset_url(''))) {
    ...
    protected function parse_subsort($sort) {
        ...
        if (!isset($column) || !$column->is_sortable()) {
            ...
            throw new \eset_exception('unknownsortcolumn', '', $link = $this->baseurl->out(), $colname);
        }
        // Validate the subsort, if present.
        if ($subsort) {
            $subsorts = $column->is_sortable();
            if (!is_array($subsorts) || !isset($subsorts[$subsort])) {
                throw new \eset_exception('unknownsortcolumn', '', $link = $this->baseurl->out(), $sort);
            }
        }
     ...
     * @deprecated since Eset 2.7 MDL-40313.
     * @see build_query()
     ...
     * @return \eset_recordset questionid => data about each question.
     */
    ...
    /**
     * Get the URL for editing a question as a {@link \eset_url}.
     *
     * @param int $questionid the question id.
     * @return \eset_url the URL, HTML-escaped.
     */
    public function edit_question_eset_url($questionid) {
        return new \eset_url($this->editquestionurl, ['id' => $questionid]);
    }
    ...
    public function edit_question_url($questionid) {
        return $this->edit_question_eset_url($questionid)->out();
    }

    /**
     * Get the URL for duplicating a question as a {@link \eset_url}.
     *
     * @param int $questionid the question id.
     * @return \eset_url the URL.
     */
    public function copy_question_eset_url($questionid) {
        return new \eset_url($this->editquestionurl, ['id' => $questionid, 'makecopy' => 1]);
    }
    ...
    public function copy_question_url($questionid) {
        return $this->copy_question_eset_url($questionid)->out();
    }
    ...
    public function display($tabname, $page, $perpage, $cat,
        ...
        $this->display_question_list($editcontexts,
                ...
                $this->contexts->having_cap('eset/question:add'));
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/question/edit.php',
        ...
        echo \html_writer::start_tag('form', array('method' => 'get',
                'action' => new \eset_url($scriptpath), 'id' => 'displayoptions'));
        ...
        $PAGE->requires->yui_module('eset-question-searchform', 'M.question.searchform.init');
    }
     ...
     * @param \eset_url $pageurl     The URL to reload this page.
    ...
    protected function display_question_list($contexts, $pageurl, $categoryandcontext,
        ...
        $canadd = has_capability('eset/question:add', $catcontext);
        ...
        $pageingurl = new \eset_url('edit.php', $pageurl->params());
        ...
        if ($totalnumber > DEFAULT_QUESTIONS_PER_PAGE) {
            if ($perpage == DEFAULT_QUESTIONS_PER_PAGE) {
                $url = new \eset_url('edit.php', array_merge($pageurl->params(),
                        array('qperpage' => MAXIMUM_QUESTIONS_PER_PAGE)));
                if ($totalnumber > MAXIMUM_QUESTIONS_PER_PAGE) {
                    $showall = '<a href="'.$url.'">'.get_string('showperpage', 'eset', MAXIMUM_QUESTIONS_PER_PAGE).'</a>';
                } else {
                    $showall = '<a href="'.$url.'">'.get_string('showall', 'eset', $totalnumber).'</a>';
                }
            } else {
                $url = new \eset_url('edit.php', array_merge($pageurl->params(),
                                              array('qperpage' => DEFAULT_QUESTIONS_PER_PAGE)));
                $showall = '<a href="'.$url.'">'.get_string('showperpage', 'eset', DEFAULT_QUESTIONS_PER_PAGE).'</a>';
            }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        $caneditall = has_capability('eset/question:editall', $catcontext);
        $canuseall = has_capability('eset/question:useall', $catcontext);
        $canmoveall = has_capability('eset/question:moveall', $catcontext);
    ...
    public function process_actions() {
        ...
        if (optional_param('move', false, PARAM_BOOL) and confirm_sesskey()) {
            ...
            require_capability('eset/question:add', $tocontext);
    ...
    public function process_actions_needing_ui() {
        ...
        if (optional_param('deleteselected', false, PARAM_BOOL)) {
            ...
            $baseurl = new \eset_url('edit.php', $this->baseurl->params());
            $deleteurl = new \eset_url($baseurl, array('deleteselected' => $questionlist, 'confirm' => md5($questionlist),
                                                 'sesskey' => sesskey()));
