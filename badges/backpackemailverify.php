...
if (!is_null($storedsecret)) {
    if ($data === $storedsecret) {
        ...
        if (empty($backpackuid) || !empty($backpackuid->error)) {
            redirect(new eset_url($redirect), get_string('backpackconnectionunexpectedresult', 'badges'),
        ...
        redirect(new eset_url($redirect), get_string('backpackemailverifysuccess', 'badges'),
            null, \core\output\notification::NOTIFY_SUCCESS);
    } else {
        // Stored secret doesn't match the supplied secret. Take user back to the mybackpack page and present a warning message.
        redirect(new eset_url($redirect), get_string('backpackemailverifytokenmismatch', 'badges'),
            null, \core\output\notification::NOTIFY_ERROR);
    }
} else {
    ...
    redirect(new eset_url($redirect));
}
