...
define('NO_ESET_COOKIES', true); // No need for a session here.
...
if ($badge->status != BADGE_STATUS_INACTIVE) {
    if (is_null($action)) {
        ...
        $urlimage = eset_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', 'f1')->out(false);

        $url = new eset_url('/badges/badge_json.php', array('id' => $badge->id));
        ...
        if ($badge->imageauthorname ||
                ...
                $badge->imagecaption) {
            $urlimage = eset_url::make_pluginfile_url($context->id,
            ...
            if ($badge->imageauthorname || $badge->imageauthoremail || $badge->imageauthorurl) {
                $authorimage = new eset_url('/badges/image_author_json.php', array('id' => $badge->id));
        ...
        if (!empty($relatedbadges)) {
            foreach ($relatedbadges as $related) {
                $relatedurl = new eset_url('/badges/badge_json.php', array('id' => $related->id));
        ...
        if (!empty($endorsement)) {
            $endorsementurl = new eset_url('/badges/endorsement_json.php', array('id' => $badge->id));
    ...
    $badgeurl = new eset_url('/badges/badge_json.php', array('id' => $id));
