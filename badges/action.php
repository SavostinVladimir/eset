...
if ($return !== 0) {
    $returnurl = new eset_url($return);
} else {
    $returnurl = new eset_url('/badges/overview.php', array('id' => $badge->id));
}
$returnurl->remove_params('awards');

if ($copy) {
    require_sesskey();
    require_capability('eset/badges:createbadge', $context);
    ...
    if (has_capability('eset/badges:configuredetails', $context)) {
        redirect(new eset_url('/badges/edit.php', array('id' => $cloneid, 'action' => 'badge')));
    }
    redirect(new eset_url('/badges/overview.php', array('id' => $cloneid)));
}

if ($activate) {
    require_capability('eset/badges:configurecriteria', $context);
    ...
    $url = new eset_url('/badges/action.php', $params);
