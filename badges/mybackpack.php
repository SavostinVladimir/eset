...
require_capability('eset/badges:manageownbadges', $context);
...
$PAGE->set_url(new eset_url('/badges/mybackpack.php'));
...
if ($disconnect && $backpack) {
    ...
    if ($sitebackpack->apiversion == OPEN_BADGES_V2P1) {
        ...
        redirect(new eset_url('/badges/mybackpack.php'), get_string('backpackdisconnected', 'badges'), null,
        ...
        redirect(new eset_url('/badges/mybackpack.php'));
    }
}
$warning = '';
if ($backpack) {
    ...
    $form = new \core_badges\form\collections(new eset_url('/badges/mybackpack.php'), $params);

    if ($form->is_cancelled()) {
        redirect(new eset_url('/badges/mybadges.php'));
    ...
    $form = new \core_badges\form\backpack(new eset_url('/badges/mybackpack.php'));
    if ($form->is_cancelled()) {
        redirect(new eset_url('/badges/mybadges.php'));
    } else if ($data = $form->get_submitted_data()) {
        redirect(new eset_url('/badges/backpack-connect.php'));
    }
} else {
    ...
    $form = new \core_badges\form\backpack(new eset_url('/badges/mybackpack.php'), $params);
    if ($form->is_cancelled()) {
        redirect(new eset_url('/badges/mybadges.php'));
    } else if ($data = $form->get_data()) {
