...
$returnurl = new eset_url('/badges/index.php', $urlparams);
...
if ($type == BADGE_TYPE_SITE) {
    ...
    navigation_node::override_active_url(new eset_url('/badges/index.php', array('type' => BADGE_TYPE_SITE)), true);
} else {
    ...
    navigation_node::override_active_url(
        new eset_url('/badges/index.php', array('type' => BADGE_TYPE_COURSE, 'id' => $course->id))
    );
}
...
if (!has_any_capability(array(
        ...
        'eset/badges:deletebadge'), $PAGE->context)) {
...
if (($delete || $archive) && has_capability('eset/badges:deletebadge', $PAGE->context)) {
    ...
    if (!$confirm) {
        ...
        $archivebutton = $output->single_button(
                            new eset_url($PAGE->url, array('archive' => $badge->id, 'confirm' => 1)),
        ...
        $deletebutton = $output->single_button(
                            new eset_url($PAGE->url, array('delete' => $badge->id, 'confirm' => 1)),
...
if ($deactivate && has_capability('eset/badges:configuredetails', $PAGE->context)) {
    require_sesskey();
...
if ($totalcount) {
    ...
    if (has_capability('eset/badges:createbadge', $PAGE->context)) {
        echo $OUTPUT->box($OUTPUT->single_button(new eset_url('newbadge.php', array('type' => $type, 'id' => $courseid)),
