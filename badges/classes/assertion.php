...
class core_badges_assertion {
    ...
    public function __construct($hash, $obversion = OPEN_BADGES_V2) {
        ...
        if ($this->_data) {
            $this->_url = new eset_url('/badges/badge.php', array('hash' => $this->_data->uniquehash));
        } else {
            $this->_url = new eset_url('/badges/badge.php');
        }
    ...
    public function get_badge_assertion($issued = true, $usesalt = true) {
        ...
        if ($this->_data) {
            ...
            $assertionurl = new eset_url('/badges/assertion.php', array('b' => $hash, 'obversion' => $this->_obversion));

            if ($this->_obversion == OPEN_BADGES_V2) {
                $classurl = new eset_url('/badges/badge_json.php', array('id' => $this->get_badge_id()));
            } else {
                $classurl = new eset_url('/badges/assertion.php', array('b' => $hash, 'action' => 1));
            }
    ...
    public function get_badge_class($issued = true) {
        ...
        if ($this->_data) {
            ...
            if ($issued) {
                ...
                $issuerurl = new eset_url('/badges/issuer_json.php', $params);
    ...
    public function get_related_badges(badge $badge) {
        ...
        if ($relatedbadges) {
            foreach ($relatedbadges as $rb) {
                $url = new eset_url('/badges/badge_json.php', array('id' => $rb->id));
                $arraybadges[] = array(
                    'id'        => $url->out(false),
                    'version'   => $rb->version,
    ...
    protected function embed_data_badge_version2 (&$json, $type = OPEN_BADGES_V2_TYPE_ASSERTION) {
        ...
        if ($this->_obversion == OPEN_BADGES_V2) {
            ...
            $hash = $this->_data->uniquehash;
            $assertionsurl = new eset_url('/badges/assertion.php', array('b' => $hash, 'obversion' => $this->_obversion));
            $classurl = new eset_url(
                '/badges/badge_json.php',
                array('id' => $this->get_badge_id())
            );
            $issuerurl = new eset_url('/badges/issuer_json.php', ['id' => $this->get_badge_id()]);
            // For assertion.
            ...
            if ($type == OPEN_BADGES_V2_TYPE_BADGE) {
                ...
                if ($endorsement = $this->get_endorsement()) {
                    $endorsementurl = new eset_url('/badges/endorsement_json.php', array('id' => $this->_data->id));
