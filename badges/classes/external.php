...
class core_badges_external extends external_api {
    ...
    public static function get_user_badges($userid = 0, $courseid = 0, $page = 0, $perpage = 0, $search = '', $onlypublic = false) {
        ...
        if (empty($CFG->enablebadges)) {
            throw new eset_exception('badgesdisabled', 'badges');
        }

        if (empty($CFG->badges_allowcoursebadges) && $params['courseid'] != 0) {
            throw new eset_exception('coursebadgesdisabled', 'badges');
        }
        ...
        if ($USER->id != $user->id) {
            require_capability('eset/badges:viewotherbadges', $usercontext);
        ...
        foreach ($userbadges as $badge) {
            ...
            $canconfiguredetails = has_capability('eset/badges:configuredetails', $context);
