...
$navurl = new eset_url('/badges/index.php', array('type' => $badge->type));
...
if ($badge->type == BADGE_TYPE_COURSE) {
    ...
    $navurl = new eset_url('/badges/index.php', array('type' => $badge->type, 'id' => $badge->courseid));
...
require_capability('eset/badges:awardbadge', $context);

$url = new eset_url('/badges/award.php', array('id' => $badgeid, 'role' => $role));
$PAGE->set_url($url);
$PAGE->set_context($context);

// Set up navigation and breadcrumbs.
$strrecipients = get_string('recipients', 'badges');
$PAGE->navbar->add($badge->name, new eset_url('overview.php', array('id' => $badge->id)))->add($strrecipients);
...
if (empty($acceptedroles)) {
    echo $OUTPUT->header();
    $return = html_writer::link(new eset_url('recipients.php', array('id' => $badge->id)), $strrecipients);
...
if ($groupmode == SEPARATEGROUPS && empty($currentgroup) &&
    !has_capability('eset/site:accessallgroups', $context) && !is_siteadmin() ) {
    echo $OUTPUT->header();
...
if (count($acceptedroles) > 1) {
    ...
    if (!empty($selection)) {
        ...
        if (!$role) {
            $pageurl = new eset_url('/badges/award.php', array('id' => $badgeid));
            echo $OUTPUT->header();
            echo $OUTPUT->box($OUTPUT->single_select(new eset_url($pageurl), 'role', $select, '', array('' => 'choosedots'),
            ...
            $roleselect = $OUTPUT->single_select(new eset_url($pageurl), 'role', $select, $role, null, null,
...
if ($award && data_submitted() && has_capability('eset/badges:awardbadge', $context)) {
    require_sesskey();
    ...
    $recipientselector->set_existing_recipients($existingselector->find_users(''));
} else if ($revoke && data_submitted() && has_capability('eset/badges:revokebadge', $context)) {
    require_sesskey();
