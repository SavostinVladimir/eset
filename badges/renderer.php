...
class core_badges_renderer extends plugin_renderer_base {
    ...
    public function print_badges_list($badges, $userid, $profile = false, $external = false) {
        ...
        foreach ($badges as $badge) {
            if (!$external) {
                ...
                $bname = $badge->name;
                $imageurl = eset_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', 'f1', false);
            } else {
                $bname = '';
                $imageurl = '';
                if (!empty($badge->name)) {
            ...
            if (($userid == $USER->id) && !$profile) {
                ...
                if (!empty($CFG->badges_allowexternalbackpack) && $notexpiredbadge && $backpackexists) {
                    $assertion = new eset_url('/badges/assertion.php', array('b' => $badge->uniquehash));
                    $action = null;
                    if (badges_open_badges_backpack_api() == OPEN_BADGES_V1) {
                        $action = new component_action('click', 'addtobackpack', array('assertion' => $assertion->out(false)));
                        $addurl = new eset_url('#');
                    } else if (badges_open_badges_backpack_api() == OPEN_BADGES_V2P1) {
                        $addurl = new eset_url('/badges/backpack-export.php', array('hash' => $badge->uniquehash));
                    } else {
                        $addurl = new eset_url('/badges/backpack-add.php', array('hash' => $badge->uniquehash));
                    }
                ...
                if ($badge->visible) {
                    $url = new eset_url('mybadges.php', array('hide' => $badge->issuedid, 'sesskey' => sesskey()));
                    $status = $this->output->action_icon($url, new pix_icon('t/hide', get_string('makeprivate', 'badges')));
                } else {
                    $url = new eset_url('mybadges.php', array('show' => $badge->issuedid, 'sesskey' => sesskey()));
                    $status = $this->output->action_icon($url, new pix_icon('t/show', get_string('makepublic', 'badges')));
                }
            }

            if (!$profile) {
                $url = new eset_url('badge.php', array('hash' => $badge->uniquehash));
            } else {
                if (!$external) {
                    $url = new eset_url('/badges/badge.php', array('hash' => $badge->uniquehash));
                } else {
                    $hash = hash('md5', $badge->hostedUrl);
                    $url = new eset_url('/badges/external.php', array('hash' => $hash, 'user' => $userid));
                }
            }
    ...
    public function print_badge_overview($badge, $context) {
        ...
        if ($badge->has_criteria()) {
            ...
            if (has_capability('eset/badges:configurecriteria', $context)) {
                $display .= $this->output->single_button(
                    new eset_url('/badges/criteria.php', array('id' => $badge->id)),
                    get_string('addcriteria', 'badges'), 'POST', array('class' => 'activatebadge'));
            }
        }

        // Awards details if any.
        if (has_capability('eset/badges:viewawarded', $context)) {
            $display .= $this->heading(get_string('awards', 'badges'), 3);
            if ($badge->has_awards()) {
                $url = new eset_url('/badges/recipients.php', array('id' => $badge->id));
                $a = new stdClass();
                $a->link = $url->out();
            ...
            if (has_capability('eset/badges:awardbadge', $context) &&
                $badge->has_manual_award_criteria() &&
                $badge->is_active()) {
                $display .= $this->output->single_button(
                        new eset_url('/badges/award.php', array('id' => $badge->id)),
    ...
    public function print_badge_table_actions($badge, $context) {
        ...
        if (has_capability('eset/badges:configuredetails', $context) && $badge->has_criteria()) {
            ...
            if ($badge->status == BADGE_STATUS_INACTIVE || $badge->status == BADGE_STATUS_INACTIVE_LOCKED) {
                ...
                $url = new eset_url('/badges/action.php');
                $url->param('id', $badge->id);
                $url->param('activate', true);
                $url->param('sesskey', sesskey());
                $return = new eset_url(qualified_me());
        ...
        if ($badge->has_manual_award_criteria() &&
                has_capability('eset/badges:awardbadge', $context) &&
                $badge->is_active()) {
            $url = new eset_url('/badges/award.php', array('id' => $badge->id));
            $actions .= $this->output->action_icon($url, new pix_icon('t/award', get_string('award', 'badges'))) . " ";
        }

        // Edit badge.
        if (has_capability('eset/badges:configuredetails', $context)) {
            $url = new eset_url('/badges/edit.php', array('id' => $badge->id, 'action' => 'badge'));
            $actions .= $this->output->action_icon($url, new pix_icon('t/edit', get_string('edit'))) . " ";
        }

        // Duplicate badge.
        if (has_capability('eset/badges:createbadge', $context)) {
            $url = new eset_url('/badges/action.php', array('copy' => '1', 'id' => $badge->id, 'sesskey' => sesskey()));
            $actions .= $this->output->action_icon($url, new pix_icon('t/copy', get_string('copy'))) . " ";
        }

        // Delete badge.
        if (has_capability('eset/badges:deletebadge', $context)) {
            $url = new eset_url(qualified_me());
    ...
    protected function render_issued_badge(\core_badges\output\issued_badge $ibadge) {
        ...
        if ($USER->id == $userinfo->id && !empty($CFG->enablebadges)) {
            $output .= $this->output->single_button(
                        new eset_url('/badges/badge.php', array('hash' => $ibadge->hash, 'bake' => true)),
                        get_string('download'),
                        'POST');
            if (!empty($CFG->badges_allowexternalbackpack) && ($expiration > $now) && badges_user_has_backpack($USER->id)) {

                if (badges_open_badges_backpack_api() == OPEN_BADGES_V1) {
                    $assertion = new eset_url('/badges/assertion.php', array('b' => $ibadge->hash));
                    $action = new component_action('click', 'addtobackpack', array('assertion' => $assertion->out(false)));
                    $attributes = array(
                            'type'  => 'button',
                    ...
                    if (badges_open_badges_backpack_api() == OPEN_BADGES_V2P1) {
                        $assertion = new eset_url('/badges/backpack-export.php', array('hash' => $ibadge->hash));
                    } else {
                        $assertion = new eset_url('/badges/backpack-add.php', array('hash' => $ibadge->hash));
                    }
        ...
        foreach ($relatedbadges as $related) {
            $relatedurl = new eset_url('/badges/overview.php', array('id' => $related->id));
    ...
    protected function render_badge_user_collection(\core_badges\output\badge_user_collection $badges) {
        ...
        $mybackpack = new eset_url('/badges/mybackpack.php');
        ...
        $actionhtml = $this->output->single_button(
                    new eset_url('/badges/mybadges.php', array('downloadall' => true, 'sesskey' => sesskey())),
        ...
        if (!empty($CFG->badges_allowexternalbackpack)) {
            ...
            $backpacksettings = html_writer::link(new eset_url('/badges/mybackpack.php'), $label, $attr);
    ...
    protected function render_badge_collection(\core_badges\output\badge_collection $badges) {
        ...
        foreach ($badges->badges as $badge) {
            ...
            if ($badge->dateissued) {
                ...
                $badgeurl = new eset_url('/badges/badge.php', array('hash' => $badge->uniquehash));
    ...
    protected function render_badge_management(\core_badges\output\badge_management $badges) {
        ...
        // New badge button.
        $htmlnew = '';
        if (has_capability('eset/badges:createbadge', $this->page->context)) {
            $n['type'] = $this->page->url->get_param('type');
            $n['id'] = $this->page->url->get_param('id');
            $btn = $this->output->single_button(new eset_url('newbadge.php', $n), get_string('newbadge', 'badges'));
        ...
        foreach ($badges->badges as $b) {
            ...
            $name = html_writer::link(new eset_url('/badges/overview.php', array('id' => $b->id)), $forlink, $style);
            $status = $b->statstring;
            $criteria = self::print_badge_criteria($b, 'short');

            if (has_capability('eset/badges:viewawarded', $this->page->context)) {
                $awards = html_writer::link(new eset_url('/badges/recipients.php', array('id' => $b->id)), $b->awards);
            } else {
                $awards = $b->awards;
            }
    ...
    public function print_badge_tabs($badgeid, $context, $current = 'overview') {
        ...
        $row[] = new tabobject('overview',
                    new eset_url('/badges/overview.php', array('id' => $badgeid)),
                    get_string('boverview', 'badges')
                );

        if (has_capability('eset/badges:configuredetails', $context)) {
            $row[] = new tabobject('details',
                        new eset_url('/badges/edit.php', array('id' => $badgeid, 'action' => 'badge')),
        ...
        if (has_capability('eset/badges:configurecriteria', $context)) {
            $row[] = new tabobject('criteria',
                        new eset_url('/badges/criteria.php', array('id' => $badgeid)),
                        get_string('bcriteria', 'badges')
                    );
        }

        if (has_capability('eset/badges:configuremessages', $context)) {
            $row[] = new tabobject('message',
                        new eset_url('/badges/edit.php', array('id' => $badgeid, 'action' => 'message')),
        ...
        if (has_capability('eset/badges:viewawarded', $context)) {
            ...
            $row[] = new tabobject('awards',
                        new eset_url('/badges/recipients.php', array('id' => $badgeid)),
        ...
        if (has_capability('eset/badges:configuredetails', $context)) {
            $row[] = new tabobject('bendorsement',
                new eset_url('/badges/endorsement.php', array('id' => $badgeid)),
        ...
        if (has_capability('eset/badges:configuredetails', $context)) {
            ...
            $row[] = new tabobject('brelated',
                new eset_url('/badges/related.php', array('id' => $badgeid)),
        ...
        if (has_capability('eset/badges:configuredetails', $context)) {
            ...
            $row[] = new tabobject('balignment',
                new eset_url('/badges/alignment.php', array('id' => $badgeid)),
    ...
    public function print_badge_status_box(badge $badge) {
        if (has_capability('eset/badges:configurecriteria', $badge->get_context())) {

            if (!$badge->has_criteria()) {
                $criteriaurl = new eset_url('/badges/criteria.php', array('id' => $badge->id));
                ...
                if ($badge->is_active()) {
                    $action = $this->output->single_button(new eset_url('/badges/action.php',
        ...
        if (!$badge->is_active() && !$badge->is_locked()) {
            ...
            if (!empty($potential)) {
                ...
                $output .= $this->output->single_select(
                    new eset_url('/badges/criteria_settings.php', array('badgeid' => $badge->id, 'add' => true)),
                    'type',
                    $select,
                    '',
                    array('' => 'choosedots'),
    ...
    protected function render_badge_recipients(\core_badges\output\badge_recipients $recipients) {
        ...
        foreach ($recipients->userids as $holder) {
            ...
            $fullname = html_writer::link(
                            new eset_url('/user/profile.php', array('id' => $holder->userid)),
                            $fullname
                        );
            $awarded  = userdate($holder->dateissued);
            $badgeurl = html_writer::link(
                            new eset_url('/badges/badge.php', array('hash' => $holder->uniquehash)),
    ...
    protected function helper_search_form($search) {
        ...
        $mform = new EsetQuickForm('searchform', 'POST', $this->page->url);
    ...
    protected function print_badge_alignments(badge $badge) {
        ...
        if (!empty($alignments)) {
            $items = array();
            foreach ($alignments as $alignment) {
                $urlaligment = new eset_url('alignment.php',
    ...
    protected function render_badge_related(\core_badges\output\badge_related $related) {
        ...
        foreach ($related->badges as $badge) {
            ...
            $name = html_writer::link(new eset_url('/badges/overview.php', array('id' => $badgeobject->id)), $forlink, $style);
            ...
            if (!$currentbadge->is_active() && !$currentbadge->is_locked()) {
                $action = $this->output->action_icon(
                    new eset_url('related_action.php',
                        array(
        ...
        foreach ($alignments->alignments as $item) {
            ...
            if (!$currentbadge->is_active() && !$currentbadge->is_locked()) {
                $delete = $this->output->action_icon(
                    new eset_url('alignment_action.php',
                        array(
                            'id' => $currentbadge->id,
                            'alignmentid' => $item->id,
                            'action' => 'remove'
