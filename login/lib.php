...
/**
 *
 * Login library file of login/password related Eset functions.
 *
 * @package    core
 * @subpackage lib
...
function core_login_process_password_reset($username, $email) {
    ...
    if ($user and !empty($user->confirmed)) {
        ...
        if (!$userauth->can_reset_password() or !is_enabled_auth($user->auth)
          or !has_capability('eset/user:changeownpassword', $systemcontext, $user->id)) {
...
function core_login_get_return_url() {
    ...
    if ($urltogo == ($CFG->wwwroot . '/')) {
        ...
        // Go to my-eset page instead of site homepage if defaulthomepage set to homepage_my.
