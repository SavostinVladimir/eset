...
if (defined('BEHAT_SITE_RUNNING') && BEHAT_SITE_RUNNING) {
    ...
    if ($wantsurl !== '') {
        $SESSION->wantsurl = (new eset_url($wantsurl))->out(false);
    }
}
...
if ($user !== false or $frm !== false or $errormsg !== '') {
    ...
    // Handles the case of another Eset site linking into a page on this site
...
if ($anchor && isset($SESSION->wantsurl) && strpos($SESSION->wantsurl, '#') === false) {
    $wantsurl = new eset_url($SESSION->wantsurl);
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if (empty($user->confirmed)) {       // This account was never confirmed
            ...
            $resendconfirmurl = new eset_url('/login/index.php',
                [
    ...
    /// Currently supported only for ldap-authentication module
        ...
        redirect(new eset_url(get_login_url(), array('testsession'=>$USER->id)));
...
if (!empty($CFG->alternateloginurl)) {
    $loginurl = new eset_url($CFG->alternateloginurl);
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new eset_url('/login/index.php'));
}
...
if (isloggedin() and !isguestuser()) {
    ...
    $logout = new single_button(new eset_url('/login/logout.php', array('sesskey'=>sesskey(),'loginpage'=>1)), get_string('logout'), 'post');
    $continue = new single_button(new eset_url('/'), get_string('cancel'), 'get');
