...
if ($reset and confirm_sesskey()) {
    ...
    echo format_text(get_string('choosereadme', 'theme_'.$theme->name), FORMAT_ESET);
    echo $OUTPUT->box_end();
    ...
    $urlconfirm = new eset_url('/theme/index.php', array('confirmation' => 1, 'choose' => $choose));
    redirect($urlconfirm);
...
if (!empty($CFG->enabledevicedetection) && empty($device)) {
    ...
    foreach ($devices as $thedevice) {
        ...
        if ($themename) {
            ...
            if (empty($themename)) {
                ...
                // not the current one.
                $screenshoturl = new eset_url('/theme/image.php',
            ...
            if ($thedevice != 'default' && !$themelocked) {
                ...
                $unsetthemeurl = new eset_url('/theme/index.php',
        ...
        $deviceurl = new eset_url('/theme/index.php', array('device' => $thedevice, 'sesskey' => sesskey()));
    ...
    foreach ($themes as $themename => $themedir) {
        ...
        // Link to the screenshot, now mandatory - the image path is hardcoded because we need image from other themes,
        // not the current one.
        $screenshotpath = new eset_url('/theme/image.php',
            array('theme' => $themename, 'image' => 'screenshot', 'component' => 'theme'));
        ...
        if (!$themelocked) {
            if (($ischosentheme) && ($device != 'default')) {
                ...
                $setthemeurl = new eset_url('/theme/index.php',
...
echo $OUTPUT->single_button(new eset_url('index.php', $params), get_string('themeresetcaches', 'admin'));
