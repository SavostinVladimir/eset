...
class base_block_testcase extends advanced_testcase {
     ...
     *
     * @param eset_recordset $rs Recordset to convert
     * @return array Array indexed by number (0, 1, 2, ...)
     */
    protected static function recordset_to_indexed_array(eset_recordset $rs) {
        $results = [];
        foreach ($rs as $rec) {
            $results[] = $rec;
        }
    ...
    public function test_get_doc_url() {
        ...
        $expected = new eset_url('/course/view.php', ['id' => $course->id], 'inst' . $blockid);
        ...
        $expected = new eset_url('/', ['redirect' => 0], 'inst' . $block2id);
        ...
        $expected = new eset_url('/mod/page/view.php', ['id' => $page->cmid], 'inst' . $block3id);
        ...
        $expected = new eset_url('/course/view.php', ['id' => $course->id], 'inst' . $block4id);
        $this->assertEquals($expected, $area->get_doc_url($doc));
        $this->assertEquals($expected, $area->get_context_url($doc));
        ...
        $expected = new eset_url('/course/view.php', ['id' => $course->id], 'inst' . $block5id);
        $this->assertEquals($expected, $area->get_doc_url($doc));
        $this->assertEquals($expected, $area->get_context_url($doc));
    }
    ...
    public function test_get_doc_icon() {
        ...
        $this->assertEquals('eset', $result->get_component());
    }
}
