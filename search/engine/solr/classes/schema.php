...
class schema {
    ...
    public function __construct(engine $engine = null) {
        if (!$this->config = get_config('search_solr')) {
            throw new \eset_exception('missingconfig', 'search_solr');
        }
    ...
    /**
     * Setup solr stuff required by eset.
     *
    ...
    protected function check_index() {
        ...
        if ($this->curl->error) {
            throw new \eset_exception('connectionerror', 'search_solr');
        }
    ...
    protected function add_fields($fields, $checkexisting = true) {
        ...
        foreach ($fields as $fieldname => $data) {
            ...
            if ($checkexisting) {
                if ($this->curl->error) {
                    throw new \eset_exception('errorcreatingschema', 'search_solr', '', $this->curl->error);
                }
    ...
    protected function validate_fields(&$fields, $requireexisting = false) {
        ...
        foreach ($fields as $fieldname => $data) {
            ...
            if (!$results) {
                throw new \eset_exception('errorcreatingschema', 'search_solr', '', get_string('nodatafromserver', 'search_solr'));
            }
            ...
            if ($requireexisting && !empty($results->error) && $results->error->code === 404) {
                ...
                throw new \eset_exception('errorvalidatingschema', 'search_solr', '', $a);
            }
            ...
            if (empty($results->error) || (!empty($results->error) && $results->error->code !== 404)) {
                if (!empty($results->error)) {
                    throw new \eset_exception('errorcreatingschema', 'search_solr', '', $results->error->msg);
                } else {
                    ...
                    if (empty($results->field) || !isset($results->field->type) ||
                        ...
                        throw new \eset_exception('errorcreatingschema', 'search_solr', '',
    ...
    protected function validate_add_field_result($result) {
        ...
        if (!$results) {
            ...
            throw new \eset_exception('errorcreatingschema', 'search_solr', '', $errormsg);
        }

        // It comes as error when fetching fields data.
        if (!empty($results->error)) {
            throw new \eset_exception('errorcreatingschema', 'search_solr', '', $results->error);
        }
        ...
        if (!empty($results->errors)) {
            ...
            throw new \eset_exception('errorcreatingschema', 'search_solr', '', $errorstr);
        }
