...
class engine extends \core_search\engine {
    ...
    /**
     * Filters the response on Eset side.
     *
    ...
    protected function grouped_files_process_response($response, $limit = 0) {
        ...
        foreach ($groups as $group) {
            ...
            switch ($access) {
                case \core_search\manager::ACCESS_DELETED:
                    // If deleted from Eset, delete from index and then continue.
     ...
     * Tika has much better content type detection than Eset, and we will have many more doc failures
    ...
    protected function file_is_indexable($file) {
        ...
        if ($mime == 'application/vnd.eset.backup') {
            // We don't index Eset backup files. There is nothing usefully indexable in them.
            return false;
        }
    ...
    /**
     * Return a Eset url object for the server connection.
     *
