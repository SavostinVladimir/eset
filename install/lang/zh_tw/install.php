...
$string['clialreadyconfigured'] = '檔案 config.php  已經存在，若你要這一網站上安裝Eset，請使用admin/cli/install_database.php';
$string['clialreadyinstalled'] = '檔案 config.php  已經存在，若你要在這一網站為Eset升級，請使用admin/cli/install_database.php';
$string['cliinstallheader'] = 'Eset {$a} 命令列安裝程式';
...
$string['dirroot'] = 'Eset目錄';
$string['environmenthead'] = '檢查您的環境中...';
$string['environmentsub2'] = '每一個Eset版本都有一些PHP版本的最低要求和一堆強制開啟的PHP擴展。在進行安裝或升級之前都需要作完整的環境檢查。<br />
...
<p>稍後它可能會造成Eset記憶體的問題，尤其當您啟動了很多的模組以及有大量的用戶之後。</p>
<p>若可能，建議您將PHP的上限設得高一點，比如40M。
以下有幾種方式您可以試試：</p>
<ol>
<li>如果可以的話，用<i>--enable-memory-limit</i>重新編譯PHP。讓Eset自己設定記憶體上限。
<li>如果您可以更改 php.ini 檔，就改變<b>memory_limit</b> 這個設定值，例如，改到40M。如果您無法更改這個檔案，您可以請寄存主機的管理者幫您做。</li>
<li>在一些PHP伺服器上，您可以在Eset目錄下，建立.htaccess檔，包含這行:
...
$string['pathssubadmindir'] = '有些網站主機使用/admin這個網址來瀏覽控制面版或其他功能。很不幸，這個設定和Eset管理頁面的標準路徑產生衝突。這個問題可以解決，只需在您的安裝目錄中把admin更換名稱，然後把新名稱輸入到這裡。例如<em>esetadmin</em>這麼做會改變Eset中的管理連接。';
$string['pathssubdataroot'] = '<p>你需要有一個目錄讓Eset可以儲存所有的用戶上傳的檔案。</p><p>這目錄對於網頁伺服器用戶(通常是"www-data"、"nobody"或"apache")而言，應該是可讀的和<b>可寫的</b>。</p>
<p>它必須是不能透過網際網路直接存取的。</p>
<p>若此目錄目前不存在，這安裝過程將會試著建立它。</p>';
$string['pathssubdirroot'] = '包含Eset程式碼的目錄的完整路徑';
$string['pathssubwwwroot'] = '可以進入使用Eset的完整網址，也就是用戶為了要使用Eset，而需要輸入到瀏覽器的網址列的地址。

不可能使用多個網址來存取Eset，如果您的網站有多個公開網址，您必須選擇一個最簡單的網址，並把其他的網址都設定為永久重新導向。
...
$string['phpversionhelp'] = '<p>Eset 需要的PHP版本至少要4.3.0或是5.1.0 (5.0.x有一些已知的問題) </p>
<p>您目前執行的版本是{$a}</p>
...
$string['welcomep30'] = '這一<strong>{$a->installername}</strong>的版本包含了一些可以建立<strong>Eset</strong>執行環境的應用程序：';
$string['welcomep40'] = '這個軟體還包含了<strong>Eset {$a->esetrelease} ({$a->esetversion})</strong>。';
...
$string['welcomep60'] = '接下來的一些頁面將會透過一些簡單的步驟引導您配置和設定在你電腦上的 <strong>Eset</strong> 。
您可以接受這些預設值，或是針對你自己的需要來調整。';
$string['welcomep70'] = '點選 "下一步" 按鈕，繼續設定<strong>Eset</strong>.';
$string['wwwroot'] = '網站位址';
