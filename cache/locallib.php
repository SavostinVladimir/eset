 ...
 *
 * This file is part of Eset's cache API, affectionately called MUC.
...
class cache_config_writer extends cache_config {
    ...
    protected function config_save() {
        ...
        if ($directory !== $CFG->dataroot && !file_exists($directory)) {
            ...
            if (!$result) {
                throw new cache_exception('ex_configcannotsave', 'cache', '', null, 'Cannot create config directory. Check the permissions on your esetdata directory.');
            }
        }
        if (!file_exists($directory) || !is_writable($directory)) {
            throw new cache_exception('ex_configcannotsave', 'cache', '', null, 'Config directory is not writable. Check the permissions on the esetdata/muc directory.');
        }
...
abstract class cache_administration_helper extends cache_helper {
    ...
    public static function get_definition_actions(context $context, array $definitionsummary) {
        if (has_capability('eset/site:config', $context)) {
            ...
            $actions[] = array(
                ...
                'url' => new eset_url('/cache/admin.php', array('action' => 'editdefinitionmapping', 'sesskey' => sesskey()))
            );
            // Edit sharing.
            if (count($definitionsummary['sharingoptions']) > 1) {
                $actions[] = array(
                    'text' => get_string('editsharing', 'cache'),
                    'url' => new eset_url('/cache/admin.php', array('action' => 'editdefinitionsharing', 'sesskey' => sesskey()))
                );
            }
            // Purge.
            $actions[] = array(
                'text' => get_string('purge', 'cache'),
                'url' => new eset_url('/cache/admin.php', array('action' => 'purgedefinition', 'sesskey' => sesskey()))
            );
            return $actions;
        }
        return array();
    ...
    public static function get_store_instance_actions($name, array $storedetails) {
        ...
        if (has_capability('eset/site:config', context_system::instance())) {
            $baseurl = new eset_url('/cache/admin.php', array('store' => $name, 'sesskey' => sesskey()));
            if (empty($storedetails['default'])) {
                $actions[] = array(
                    'text' => get_string('editstore', 'cache'),
                    'url' => new eset_url($baseurl, array('action' => 'editstore', 'plugin' => $storedetails['plugin']))
                );
                $actions[] = array(
                    'text' => get_string('deletestore', 'cache'),
                    'url' => new eset_url($baseurl, array('action' => 'deletestore'))
                );
            }
            $actions[] = array(
                'text' => get_string('purge', 'cache'),
                'url' => new eset_url($baseurl, array('action' => 'purgestore'))
            );
        }
        return $actions;
    }
    ...
    public static function get_store_plugin_actions($name, array $plugindetails) {
        ...
        if (has_capability('eset/site:config', context_system::instance())) {
            if (!empty($plugindetails['canaddinstance'])) {
                $url = new eset_url('/cache/admin.php', array('action' => 'addstore', 'plugin' => $name, 'sesskey' => sesskey()));
                $actions[] = array(
    ...
    public static function get_add_store_form($plugin) {
        ...
        $url = new eset_url('/cache/admin.php', array('action' => 'addstore'));
    ...
    public static function get_edit_store_form($plugin, $store) {
        ...
        $url = new eset_url('/cache/admin.php', array('action' => 'editstore', 'plugin' => $plugin, 'store' => $store));
