...
class core_cache_renderer extends plugin_renderer_base {
    ...
    public function definition_summaries(array $definitionsummaries, context $context) {
        ...
        $url = new eset_url('/cache/admin.php', array('action' => 'rescandefinitions', 'sesskey' => sesskey()));
     ...
     * @param eset_url $editurl
     * @return string HTML
     */
    public function mode_mappings($applicationstore, $sessionstore, $requeststore, eset_url $editurl) {
        $table = new html_table();
        $table->colclasses = array(
            'mode',
            'mapping',
    ...
    public function lock_summaries(array $locks) {
        ...
        foreach ($locks as $lock) {
            ...
            if ($lock['uses'] === 0 && !$lock['default']) {
                $url = new eset_url('/cache/admin.php', array('lock' => $lock['name'], 'action' => 'deletelock', 'sesskey' => sesskey()));
        ...
        $url = new eset_url('/cache/admin.php', array('action' => 'newlockinstance', 'sesskey' => sesskey()));
