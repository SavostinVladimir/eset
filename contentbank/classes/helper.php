...
class helper {
    ...
    public static function get_page_ready(\context $context, string $title, bool $internal = false): void {
        ...
        $cburl = new \eset_url('/contentbank/index.php', ['contextid' => $context->id]);
        ...
        switch ($context->contextlevel) {
            case CONTEXT_COURSE:
                ...
                \navigation_node::override_active_url(new \eset_url('/course/view.php', ['id' => $courseid]));
                $PAGE->navbar->add($title, $cburl);
                $PAGE->set_pagelayout('incourse');
                break;
            case CONTEXT_COURSECAT:
                $coursecat = $context->instanceid;
                \navigation_node::override_active_url(new \eset_url('/course/index.php', ['categoryid' => $coursecat]));
                $PAGE->navbar->add($title, $cburl);
