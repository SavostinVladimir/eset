...
require_capability('eset/contentbank:access', $context);

$statusmsg = optional_param('statusmsg', '', PARAM_ALPHANUMEXT);
$errormsg = optional_param('errormsg', '', PARAM_ALPHANUMEXT);

$returnurl = new \eset_url('/contentbank/index.php', ['contextid' => $context->id]);
...
$PAGE->set_url(new \eset_url('/contentbank/view.php', ['id' => $id]));
...
if ($contenttype->can_manage($content)) {
    ...
    if ($contenttype->can_upload()) {
        $actionmenu->add_secondary_action(new action_menu_link(
            new eset_url('/contentbank/upload.php', ['contextid' => $context->id, 'id' => $content->get_id()]),
...
if ($contenttype->can_download($content)) {
    ...
    $actionmenu->add_secondary_action(new action_menu_link(
        new eset_url($contenttype->get_download_url($content)),
