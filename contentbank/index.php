...
if (has_capability('eset/contentbank:useeditor', $context)) {
    ...
    if (!empty($editabletypes)) {
        // Editor base URL.
        $editbaseurl = new eset_url('/contentbank/edit.php', ['contextid' => $contextid]);
        $toolbar[] = [
            'name' => get_string('add'),
...
if (has_capability('eset/contentbank:upload', $context)) {
    // Don' show upload button if there's no plugin to support any file extension.
    $accepted = $cb->get_supported_extensions_as_string($context);
    if (!empty($accepted)) {
        $importurl = new eset_url('/contentbank/upload.php', ['contextid' => $contextid]);
        $toolbar[] = [
