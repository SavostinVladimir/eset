...
// increase or decrease available memory - we need to make sure eset
...
// Fake some settings so that we can use selected functions from esetlib.php, weblib.php and filelib.php.
global $CFG;
$CFG = new stdClass();
...
$CFG->docroot              = 'https://docs.eset.org';
...
if (!empty($memlimit) and $memlimit != -1) {
    if (get_real_size($memlimit) < get_real_size($minrequiredmemory)) {
        ...
        echo "Eset requires at least {$minrequiredmemory}B of PHP memory.<br />";
...
if (is_null($CFG->dataroot)) {
    ...
    while(is_dataroot_insecure()) {
        ...
        $CFG->dataroot = dirname($parrent).DIRECTORY_SEPARATOR.'esetdata';
    }
...
if ($config->stage == INSTALL_SAVE) {
    ...
    $database = eset_database::get_driver_instance($config->dbtype, 'native');
