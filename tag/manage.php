...
if (!$tagcoll) {
    ...
    $url = new eset_url($manageurl, array('action' => 'colladd'));
...
$otherfields = '';
if ($filter !== '') {
    $otherfields = html_writer::link(new eset_url($PAGE->url, ['filter' => null]),
        get_string('resetfilter', 'tag'));
}

$data = [
    'action' => new eset_url('/tag/manage.php'),
...
if ($perpage == SHOW_ALL_PAGE_SIZE) {
    ...
    $url = new eset_url($PAGE->url, $params);
