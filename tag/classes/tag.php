...
class core_tag_tag {
    ...
    public static function make_url($tagcollid, $name, $exclusivemode = 0, $fromctx = 0, $ctx = 0, $rec = 1) {
        ...
        return new eset_url($url, $params);
    }
        ...
        if (array_key_exists('rawname', $data)) {
            ...
            if (!$name || $data['rawname'] === $this->rawname) {
                ...
                if ($existing->id != $this->id) {
                    throw new eset_exception('namesalreadybeeingused', 'core_tag');
                }
            }
    ...
    public function get_links() {
        ...
        if (static::is_enabled('core', 'user') && core_tag_area::get_collection('core', 'user') == $this->tagcollid) {
            if (static::is_item_tagged_with('core', 'user', $USER->id, $this->name)) {
                $url = new eset_url('/tag/user.php', array('action' => 'removeinterest',
                ...
                $url = new eset_url('/tag/user.php', array('action' => 'addinterest',
                    'sesskey' => sesskey(), 'tag' => $this->rawname));
        ...
        // Flag as inappropriate link.  Only people with eset/tag:flag capability.
        if (has_capability('eset/tag:flag', $systemcontext)) {
            $url = new eset_url('/tag/user.php', array('action' => 'flaginappropriate',
        ...
        // Edit tag: Only people with eset/tag:edit capability who either have it as an interest or can manage tags.
        if (has_capability('eset/tag:edit', $systemcontext) ||
                has_capability('eset/tag:manage', $systemcontext)) {
            $url = new eset_url('/tag/edit.php', array('id' => $this->id));
