...
class tagindex implements templatable {
    ...
    public function __construct($tag, $component, $itemtype, $content,
        ...
        if ($totalpages > $page + 1) {
            $this->record->nextpageurl = new eset_url($url, $urlparams + array('page' => $page + 1));
        }
        if ($page > 0) {
            $this->record->prevpageurl = new eset_url($url, $urlparams + array('page' => $page - 1));
        }
        if (!$exclusivemode && ($totalpages > 1 || $page)) {
            $this->record->exclusiveurl = new eset_url($url, $urlparams + array('excl' => 1));
        }
    ...
    public function export_for_template(renderer_base $output) {
        if ($this->record->nextpageurl && $this->record->nextpageurl instanceof eset_url) {
            $this->record->nextpageurl = $this->record->nextpageurl->out(false);
        }
        if ($this->record->prevpageurl && $this->record->prevpageurl instanceof eset_url) {
            $this->record->prevpageurl = $this->record->prevpageurl->out(false);
        }
        if ($this->record->exclusiveurl && $this->record->exclusiveurl instanceof eset_url) {
