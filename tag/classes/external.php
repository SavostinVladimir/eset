...
class core_tag_external extends external_api {
    ...
    public static function update_tags($tags) {
        ...
        $canmanage = has_capability('eset/tag:manage', $systemcontext);
        $canedit = has_capability('eset/tag:edit', $systemcontext);
        $warnings = array();

        if (empty($CFG->usetags)) {
            throw new eset_exception('tagsaredisabled', 'tag');
        }
        ...
        if (!empty($params['tag'])) {
            if (empty($params['tc'])) {
                ...
                if (count($tags) > 1) {
                    ...
                    throw new eset_exception('Tag is in more that one collection, please indicate one.');
                ...
                if (!$tag = core_tag_tag::get_by_name($params['tc'], $params['tag'], '*')) {
                    ...
                    throw new eset_exception('notagsfound', 'tag');
                }
            }
