...
if ($tagname) {
    ...
    if (!$tagcollid) {
        ...
        if (count($tags) > 1) {
            ...
            redirect(new eset_url('/tag/search.php', array('query' => $tagname)));
        } else if (count($tags) == 1) {
            $tag = reset($tags);
        }
    } else {
        if (!$tag = core_tag_tag::get_by_name($tagcollid, $tagname, '*')) {
            redirect(new eset_url('/tag/search.php', array('tc' => $tagcollid, 'query' => $tagname)));
        }
    }
} else if ($tagid) {
...
$buttons = '';
if (has_capability('eset/tag:manage', context_system::instance())) {
    $buttons .= $OUTPUT->single_button(new eset_url('/tag/manage.php'),
