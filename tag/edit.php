...
//Editing a tag requires eset/tag:edit capability
$systemcontext   = context_system::instance();
require_capability('eset/tag:edit', $systemcontext);
...
if ($tagname) {
    ...
    if (!$tagcollid) {
        ...
        if (count($tags) > 1) {
            ...
            redirect(new eset_url('/tag/search.php', array('tag' => $tagname)));
        } else if (count($tags) == 1) {
            $tag = reset($tags);
        }
    } else {
        if (!$tag = core_tag_tag::get_by_name($tagcollid, $tagname, '*')) {
            redirect(new eset_url('/tag/search.php', array('tagcollid' => $tagcollid)));
        }
    }
} else if ($tagid) {
...
if (empty($tag)) {
    redirect(new eset_url('/tag/search.php'));
}

$PAGE->set_url($tag->get_view_url());
$PAGE->set_subpage($tag->id);
$PAGE->set_context($systemcontext);
$PAGE->set_blocks_editing_capability('eset/tag:editblocks');
...
if ($tagform->is_cancelled()) {
    redirect($returnurl ? new eset_url($returnurl) : $tag->get_view_url());
} else if ($tagnew = $tagform->get_data()) {
    // If new data has been sent, update the tag record.
    $updatedata = array();

    if (has_capability('eset/tag:manage', $systemcontext)) {
...
navigation_node::override_active_url(new eset_url('/tag/search.php'));
