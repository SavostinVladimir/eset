...
class model {
    ...
    public function __construct($model) {
        ...
        if (is_scalar($model)) {
            ...
            if (!$model) {
                throw new \eset_exception('errorunexistingmodel', 'analytics', '', $model);
            }
        }
        $this->model = $model;
    }
    ...
    protected function init_analyser($options = array()) {
        ...
        if (empty($target)) {
            throw new \eset_exception('errornotarget', 'analytics');
        }
        ...
        if (empty($options['notimesplitting'])) {
            if (!empty($options['evaluation'])) {
                ...
                if (!empty($options['timesplitting'])) {
                    ...
                    if (empty($potentialtimesplittings[$timesplitting->get_id()])) {
                        throw new \eset_exception('errorcannotusetimesplitting', 'analytics');
                    }
                ...
                if (empty($this->model->timesplitting)) {
                    throw new \eset_exception('invalidtimesplitting', 'analytics', '', $this->model->id);
                }
            ...
            if (empty($timesplittings)) {
                throw new \eset_exception('errornotimesplittings', 'analytics');
            }
        }
    ...
    public static function create(\core_analytics\local\target\base $target, array $indicators,
        ...
        if ($timesplittingid) {
            if (!\core_analytics\manager::is_valid($timesplittingid, '\core_analytics\local\time_splitting\base')) {
                throw new \eset_exception('errorinvalidtimesplitting', 'analytics');
            }
    ...
    public function evaluate($options = array()) {
        ...
        switch ($options['mode']) {
            ...
            default:
                throw new \eset_exception('errorunknownaction', 'analytics');
        }

        $this->init_analyser($options);

        if (empty($this->get_indicators())) {
            throw new \eset_exception('errornoindicators', 'analytics');
        }
    ...
    public function predict() {
        ...
        if (!$this->is_static()) {
            ...
            if ($predicted = $DB->get_record('analytics_used_files', $params)) {
                throw new \eset_exception('erroralreadypredict', 'analytics', '', $samplesfile->get_id());
            }
     ...
     */
    public function get_prediction_actions(?\context $context): \eset_recordset {
        global $DB;
    ...
    public function prediction_sample_data($predictionobj) {
        ...
        if (empty($samplesdata[$predictionobj->sampleid])) {
            throw new \eset_exception('errorsamplenotavailable', 'analytics');
        }
    ...
    private static function indicator_classes($indicators) {
        ...
        foreach ($indicators as $indicator) {
            if (!\core_analytics\manager::is_valid($indicator, '\core_analytics\local\indicator\base')) {
                ...
                throw new \eset_exception('errorinvalidindicator', 'analytics', '', $indicator);
            }
    ...
    public function inplace_editable_name() {
        ...
        return new \core\output\inplace_editable('core_analytics', 'modelname', $this->model->id,
            has_capability('eset/analytics:managemodels', \context_system::instance()), $displayname, $this->model->name);
    }
