...
class manager {
    ...
    public static function check_can_manage_models() {
        require_capability('eset/analytics:managemodels', \context_system::instance());
    }
    ...
    public static function check_can_list_insights(\context $context, bool $return = false) {
        ...
        if ($context->contextlevel === CONTEXT_USER && $context->instanceid == $USER->id) {
            $capability = 'eset/analytics:listowninsights';
        } else {
            $capability = 'eset/analytics:listinsights';
        }
    ...
    public static function get_predictions_processor($predictionclass = false, $checkisready = true) {
        ...
        if (!isset(self::$predictionprocessors[$checkisready][$predictionclass])) {
            ...
            if ($checkisready) {
                ...
                if ($isready !== true) {
                    throw new \eset_exception('errorprocessornotready', 'analytics', '', $isready);
                }
            }
     ...
     * @todo MDL-65086 This will be deleted in Eset 3.11
    ...
    public static function get_prediction($predictionid, $requirelogin = false) {
        ...
        if (!$predictionobj = $DB->get_record('analytics_predictions', array('id' => $predictionid))) {
            throw new \eset_exception('errorpredictionnotfound', 'analytics');
        }

        $context = \context::instance_by_id($predictionobj->contextid, IGNORE_MISSING);
        if (!$context) {
            throw new \eset_exception('errorpredictioncontextnotavailable', 'analytics');
        }
    ...
    /**
     * Used to be used to add models included with the Eset core.
     *
     * @deprecated Deprecated since Eset 3.7 (MDL-61667) - Use lib/db/analytics.php instead.
     * @todo Remove this method in Eset 3.11 (MDL-65186).
     * @return void
     */
    ...
    /**
     * Return the list of all the models declared anywhere in this Eset installation.
     *
