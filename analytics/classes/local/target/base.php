...
abstract class base extends \core_analytics\calculable {
    ...
    public function prediction_actions(\core_analytics\prediction $prediction, $includedetailsaction = false,
        ...
        if ($this->link_insights_report() && $includedetailsaction) {
            ...
            $predictionurl = new \eset_url('/report/insights/prediction.php', array('id' => $predictionid));
     ...
     *
     * Feel free to overwrite if you need to but keep in mind that eset/analytics:listinsights
     * or eset/analytics:listowninsights capability is required to access the list of insights.
     *
    ...
    public function get_insights_users(\context $context) {
        if ($context->contextlevel === CONTEXT_USER) {
            if (!has_capability('eset/analytics:listowninsights', $context, $context->instanceid)) {
                $users = [];
            } else {
            ...
            $users = get_enrolled_users($context, 'eset/analytics:listinsights');
        } else {
            $users = get_users_by_capability($context, 'eset/analytics:listinsights');
        }
        return $users;
    }
    ...
    public function get_insight_context_url($modelid, $context) {
        return new \eset_url('/report/insights/insights.php?modelid=' . $modelid . '&contextid=' . $context->id);
    }
     ...
     * @param  \stdClass    $user
     * @param  \eset_url  $insighturl
     * @return string[]                     The plain text message and the HTML message
     */
    public function get_insight_body(\context $context, string $contextname, \stdClass $user, \eset_url $insighturl): array {
        global $OUTPUT;
