...
class blog_entry implements renderable {
            ...
            if ($associations = $associations = $DB->get_records('blog_association', array('blogid' => $this->id))) {
                ...
                foreach ($associations as $key => $assocrec) {
                    ...
                    if ($context->contextlevel == CONTEXT_COURSE) {
                        ...
                        $associations[$key]->url = $assocurl = new eset_url('/course/view.php',
                    ...
                    if ($context->contextlevel == CONTEXT_MODULE) {
                        ...
                        $associations[$key]->url = new eset_url('/mod/' . $modinfo->name . '/view.php',
                                                                  array('id' => $context->instanceid));
                        $associations[$key]->text = $instancename;
     ...
     * @param esetform  $form              Used for attachments.
    ...
    public function can_user_edit($userid=null) {
        ...
        if ($this->userid == $userid && has_capability('eset/blog:create', $sitecontext)) {
    ...
    public function can_user_view($targetuserid) {
        ...
        if (empty($CFG->enableblogs) || !has_capability('eset/blog:view', $sitecontext)) {
        ...
        if ($this->publishstate == 'draft' && !has_capability('eset/blog:viewdrafts', $sitecontext)) {
     ...
     *
     * @return array This function returns an array ideal for sending to esets'
...
class blog_listing {
    ...
    public function get_entry_fetch_sql($count=false, $sort='lastmodified DESC', $userid = false) {
        ...
        if (has_capability('eset/user:readuserblogs', context_system::instance())) {
            // Don't add permission constraints.

        } else if (!empty($this->filters['user'])
                   && has_capability('eset/user:readuserblogs',
    ...
    public function print_entries() {
        ...
        if (has_capability('eset/blog:create', $sitecontext)) {
            ...
            if (empty($userid) || (!empty($userid) && $userid == $USER->id)) {
                ...
                $addurl = new eset_url("$CFG->wwwroot/blog/edit.php");
