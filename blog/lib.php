 ...
 * the capability eset/blog:create, or if they have the capability
 * eset/blog:manageentries.
 *
...
function blog_user_can_edit_entry($blogentry) {
    ...
    if (has_capability('eset/blog:manageentries', $sitecontext)) {
        return true; // Can edit any blog entry.
    }

    if ($blogentry->userid == $USER->id && has_capability('eset/blog:create', $sitecontext)) {
...
function blog_user_can_view_user_entry($targetuserid, $blogentry=null) {
    ...
    if ($blogentry && $blogentry->publishstate == 'draft' && !has_capability('eset/blog:viewdrafts', $sitecontext)) {
    ...
    switch ($CFG->bloglevel) {
        ...
        default:
            ...
            return has_capability('eset/user:readuserblogs', $personalcontext);
        break;
 ...
 * for new entries not yet copied into Eset.
...
function blog_delete_external_entries($externalblog) {
    global $DB;
    require_capability('eset/blog:manageexternal', context_system::instance());
 ...
 *
 * @param eset_page $page The page to load for (normally $PAGE)
 * @param stdClass $userid Load for a specific user
 * @return array An array of options organised by type.
 */
function blog_get_all_options(eset_page $page, stdClass $userid = null) {
    global $CFG, $DB, $USER;
    ...
    if (!empty($CFG->enableblogs)
        ...
        && has_capability('eset/blog:view', context_system::instance())) {

        $options[CONTEXT_SYSTEM] = array('viewsite' => array(
            'string' => get_string('viewsiteentries', 'blog'),
            'link' => new eset_url('/blog/index.php')
        ));
    }
...
function blog_get_options_for_user(stdClass $user=null) {
    ...
    $sitecontext = context_system::instance();
    $canview = has_capability('eset/blog:view', $sitecontext);
    ...
    if (!$iscurrentuser && $canview && ($CFG->bloglevel >= BLOG_SITE_LEVEL)) {
        ...
        $options['userentries'] = array(
            ...
            'link' => new eset_url('/blog/index.php', array('userid' => $user->id))
        );
    } else {
        ...
        if (has_capability('eset/blog:create', $sitecontext)) {
            // We can add to our own blog.
            $options['add'] = array(
                'string' => get_string('addnewentry', 'blog'),
                'link' => new eset_url('/blog/edit.php', array('action' => 'add'))
            );
        }
    }
    if ($canview && $CFG->enablerssfeeds) {
        $options['rss'] = array(
            ...
            'link' => new eset_url(rss_get_url($sitecontext->id, $USER->id, 'blog', 'user/'.$user->id))
        );
    }
...
function blog_get_options_for_course(stdClass $course, stdClass $user=null) {
    ...
    if (has_capability('eset/blog:create', $sitecontext)) {
        ...
        $options['courseadd'] = array(
            ...
            'link' => new eset_url('/blog/edit.php', array('action' => 'add', 'courseid' => $course->id))
        );
    }
...
function blog_get_options_for_module($module, $user=null) {
    ...
    if (has_capability('eset/blog:view', $sitecontext)) {
        ...
        if ($CFG->bloglevel >= BLOG_SITE_LEVEL) {
            ...
            $options['moduleview'] = array(
                ...
                'link' => new eset_url('/blog/index.php', array('modid' => $module->id))
            );
        }
        // View MY entries about this module.
        $options['moduleviewmine'] = array(
            'string' => get_string('viewmyentriesaboutmodule', 'blog', $modulename),
            'link' => new eset_url('/blog/index.php', array('modid' => $module->id, 'userid' => $USER->id))
        );
        if (!empty($user) && ($CFG->bloglevel >= BLOG_SITE_LEVEL)) {
    ...
    if (has_capability('eset/blog:create', $sitecontext)) {
        ...
        $options['moduleadd'] = array(
            ...
            'link' => new eset_url('/blog/edit.php', array('action' => 'add', 'modid' => $module->id))
        );
    }
    // Cache the options.
 ...
 * 4. url:     The eset_url object used as the base for add and view links
...
function blog_get_headers($courseid=null, $groupid=null, $userid=null, $tagid=null, $tag=null, $modid=null, $entryid=null,
    ...
    $blogurl = new eset_url('/blog/index.php');
    ...
    if (!in_array($action, array('edit', 'add'))) {
        ...
        // Append Search info.
        if (!empty($search) && has_capability('eset/blog:search', $sitecontext)) {
...
function blog_comment_validate($commentparam) {
    ...
    $sitecontext = context_system::instance();
    return has_capability('eset/blog:view', $sitecontext) &&
...
function core_blog_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $url = new eset_url("/blog/index.php", array('userid' => $user->id));
    if (!empty($course)) {
...
function blog_get_tagged_posts($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    ...
    $content = '';
    if (empty($CFG->enableblogs) || !has_capability('eset/blog:view', $systemcontext)) {
        ...
        if (!empty($blogs)) {
            ...
            foreach ($blogs as $blog) {
                ...
                $url = new eset_url('/blog/index.php', array('entryid' => $blog->id));
                $subject = html_writer::link($url, $subject, array('class' => $class));

                $fullname = fullname($user);
                if (user_can_view_profile($user)) {
                    $profilelink = new eset_url('/user/view.php', array('id' => $blog->userid));
            ...
            $allblogsurl = new eset_url('/blog/index.php', $urlparams);
...
function blog_validate_access($courseid, $modid, $groupid, $entryid, $userid) {
    ...
    if (!$userid && has_capability('eset/blog:view', $sitecontext) && $CFG->bloglevel > BLOG_USER_LEVEL) {
        if ($entryid) {
    ...
    if (!empty($groupid)) {
        ...
        if (groups_get_course_groupmode($course) == SEPARATEGROUPS &&
                !has_capability('eset/site:accessallgroups', $coursecontext)) {
    ...
    if (!empty($userid)) {
        ...
        if ($USER->id == $userid) {
            ...
            if (!has_capability('eset/blog:view', $sitecontext) || !blog_user_can_view_user_entry($userid)) {
                print_error('cannotviewcourseblog', 'blog');
            }
        }
    }
