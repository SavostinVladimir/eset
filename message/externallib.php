...
class core_message_external extends external_api {
                ...
                'messages' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            'textformat' => new external_format_value('text', VALUE_DEFAULT, FORMAT_ESET),
                        )
                    )
                )
            )
    ...
    public static function send_instant_messages($messages = array()) {
        ...
        $context = context_system::instance();
        self::validate_context($context);
        require_capability('eset/site:sendmessage', $context);

        // Ensure the current user is allowed to delete message for everyone.
        $candeletemessagesforallusers = has_capability('eset/site:deleteanymessage', $context);
        ...
        foreach ($params['messages'] as $message) {
            ...
            if ($success) {
                ...
                // See http://docs.eset.org/dev/Errors_handling_in_web_services#When_to_send_a_warning_on_the_server_side .
                $resultmsg['msgid'] = -1;
    ...
    public static function delete_contacts($userids, $userid = 0) {
        ...
        $capability = 'eset/site:manageallmessaging';
    ...
    public static function get_conversation_members(int $userid, int $conversationid, bool $includecontactrequests = false,
        ...
        if (!\core_message\api::is_user_in_conversation($params['userid'], $params['conversationid'])) {
            throw new eset_exception('You are not a member of this conversation.');
        }
    ...
    public static function message_search_users($userid, $search, $limitfrom = 0, $limitnum = 0) {
        ...
        if (($USER->id != $params['userid']) && !has_capability('eset/site:readallmessages', $systemcontext)) {
     ...
     * @return stdClass
     * @throws \eset_exception if the messaging feature is disabled on the site.
     * @since 3.2
     */
    ...
    public static function get_conversation(
        ...
        if ($conversation) {
            ...
            throw new \eset_exception('errorconversationdoesnotexist', 'message');
        }
    }
    ...
    public static function get_conversation_messages(int $currentuserid, int $convid, int $limitfrom = 0, int $limitnum = 0,
        ...
        if (($USER->id != $params['currentuserid']) && !has_capability('eset/site:readallmessages', $systemcontext)) {
            throw new eset_exception('You do not have permission to perform this action.');
        }
        ...
        if (!\core_message\api::is_user_in_conversation($params['currentuserid'], $params['convid'])) {
            throw new eset_exception('User is not part of conversation.');
        }
    ...
    public static function search_contacts($searchtext, $onlymycourses = false) {
        ...
        if ($params['searchtext'] === '') {
            throw new eset_exception('querystringcannotbeempty');
        }
    ...
    public static function get_messages($useridto, $useridfrom = 0, $type = 'both', $read = true,
        ...
        if (!empty($useridto)) {
            if (core_user::is_real_user($useridto)) {
                ...
                throw new eset_exception('invaliduser');
            }
        }
        ...
        if ($useridto != $USER->id and $useridfrom != $USER->id and
             !has_capability('eset/site:readallmessages', $context)) {
            throw new eset_exception('accessdenied', 'admin');
        }
        ...
        if ($messages = message_get_messages($useridto, $useridfrom, $notifications, $read, $sort, $limitfrom, $limitnum)) {
            $canviewfullname = has_capability('eset/site:viewfullnames', $context);
    ...
    public static function mark_all_notifications_as_read($useridto, $useridfrom, $timecreatedto = 0) {
        ...
        if ($useridto != $USER->id and $useridfrom != $USER->id and
            // The deleteanymessage cap seems more reasonable here than readallmessages.
             !has_capability('eset/site:deleteanymessage', $context)) {
            throw new eset_exception('accessdenied', 'admin');
        }
    ...
    public static function get_unread_conversations_count($useridto) {
        ...
        if ($useridto != $USER->id and !has_capability('eset/site:readallmessages', $context)) {
            throw new eset_exception('accessdenied', 'admin');
        }
    ...
    public static function delete_conversations_by_id($userid, array $conversationids) {
        ...
        foreach ($params['conversationids'] as $conversationid) {
            if (\core_message\api::can_delete_conversation($user->id, $conversationid)) {
                ...
                throw new eset_exception("You do not have permission to delete the conversation '$conversationid'");
            }
        }
    ...
    public static function delete_message($messageid, $userid, $read = true) {
        ...
        if (\core_message\api::can_delete_message($user->id, $params['messageid'])) {
            ...
            throw new eset_exception('You do not have permission to delete this message');
        }
    ...
    protected static function validate_preferences_permissions($userid) {
        ...
        if ($user->id == $USER->id) {
            ...
            require_capability('eset/user:editownmessageprofile', $systemcontext);
        } else {
            // Teachers, parents, etc.
            $personalcontext = context_user::instance($user->id);
            require_capability('eset/user:editmessageprofile', $personalcontext);
        }
        return $user;
    }
    ...
    public static function get_user_message_preferences($userid = 0) {
        ...
        $providers = array_filter(message_get_providers_for_user($user->id), function($provider) {
            return $provider->component === 'eset';
        });
     ...
     * @return array
     * @throws eset_exception if messaging is disabled or if the user cannot perform the action.
     */
    ...
    public static function get_member_info(
        ...
        if (($USER->id != $referenceuserid) && !has_capability('eset/site:readallmessages', $systemcontext)) {
            throw new eset_exception('You do not have permission to perform this action.');
        }
     ...
     * @throws eset_exception if the current user cannot perform this action.
     */
    ...
    public static function delete_message_for_all_users(int $messageid, int $userid) {
        ...
        if (core_message\api::can_delete_message_for_all_users($user->id, $params['messageid'])) {
            ...
            throw new eset_exception('You do not have permission to delete this message for everyone.');
        }
