...
class core_message_externallib_testcase extends externallib_advanced_testcase {
    ...
    public function test_send_instant_messages_no_capability() {
        ...
        $this->unassignUserCapability('eset/site:sendmessage', $contextid, $userrole->id);
    ...
    public function test_block_user_when_ineffective() {
        ...
        assign_capability('eset/site:messageanyuser', CAP_ALLOW, $authenticateduser->id, context_system::instance(), true);
    ...
    public function test_get_messages() {
        ...
        message_post_message($user1, $user2, 'some random text 1', FORMAT_ESET);
        message_post_message($user1, $user3, 'some random text 2', FORMAT_ESET);
        message_post_message($user2, $user3, 'some random text 3', FORMAT_ESET);
        message_post_message($user3, $user2, 'some random text 4', FORMAT_ESET);
        message_post_message($user3, $user1, 'some random text 5', FORMAT_ESET);
        ...
        $eventdata->modulename        = 'eset';
        $eventdata->component         = 'enrol_paypal';
        $eventdata->name              = 'paypal_enrolment';
        $eventdata->userfrom          = get_admin();
        $eventdata->userto            = $user1;
        $eventdata->subject           = "Eset: PayPal payment";
        ...
        $message->contexturl        = (string)new eset_url('/course/view.php', array('id' => $course->id));
    ...
    public function test_delete_message() {
        ...
        $m1to2 = message_post_message($user1, $user2, 'some random text 1', FORMAT_ESET);
        $m2to3 = message_post_message($user2, $user3, 'some random text 3', FORMAT_ESET);
        $m3to2 = message_post_message($user3, $user2, 'some random text 4', FORMAT_ESET);
        $m3to4 = message_post_message($user3, $user4, 'some random text 4', FORMAT_ESET);
    ...
    /**
     * Tests searching users as another user having the 'eset/user:viewdetails' capability.
     */
    ...
    /**
     * Tests searching users as another user without the 'eset/user:viewdetails' capability.
     */
    ...
    public function test_get_unread_conversations_count() {
        ...
        $this->send_message($user1, $user4, 'Hey mate, you see the new messaging UI in Eset?');
    ...
    public function test_get_user_message_preferences() {
        ...
        // Set a couple of preferences to test.
        set_user_preference('message_provider_eset_instantmessage_loggedin', 'email', $user);
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email', $user);
        ...
        foreach ($prefs['preferences']['components'] as $component) {
            foreach ($component['notifications'] as $prefdata) {
                if ($prefdata['preferencekey'] != 'message_provider_eset_instantmessage') {
                    continue;
                }
                foreach ($prefdata['processors'] as $processor) {
