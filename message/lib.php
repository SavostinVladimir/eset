...
function message_output_fragment_processor_settings($args = []) {
    ...
    if (!isset($args['type'])) {
        throw new eset_exception('Must provide a processor type');
    }

    if (!isset($args['userid'])) {
        throw new eset_exception('Must provide a userid');
    }
...
function core_message_can_edit_message_profile($user) {
    global $USER;
    if ($user->id == $USER->id) {
        return has_capability('eset/user:editownmessageprofile', context_system::instance());
    } else {
        $personalcontext = context_user::instance($user->id);
        if (!has_capability('eset/user:editmessageprofile', $personalcontext)) {
            return false;
        }
        if (isguestuser($user)) {
            return false;
