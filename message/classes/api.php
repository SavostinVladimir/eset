 ...
 * @copyright  2016 Mark Nelson <markn@eset.com>
...
class api {
    ...
    public static function message_search_users(int $userid, string $search, int $limitfrom = 0, int $limitnum = 20) : array {
        ...
        if (empty($CFG->messaging)) {
            throw new \eset_exception('disabled', 'message');
        }
    ...
    public static function get_conversations($userid, $limitfrom = 0, $limitnum = 20, int $type = null,
        ...
        if (!is_null($type) && !in_array($type, [self::MESSAGE_CONVERSATION_TYPE_INDIVIDUAL,
            ...
            throw new \eset_exception("Invalid value ($type) for type param, please see api constants.");
        }
        ...
        foreach ($conversations as $conversation) {
            ...
            $conv->candeletemessagesforallusers = has_capability('eset/site:deleteanymessage',  $contexttodeletemessageforall);
    ...
    public static function get_conversation(
        ...
        $canreadallmessages = has_capability('eset/site:readallmessages', $systemcontext);
        if (($USER->id != $userid) && !$canreadallmessages) {
            throw new \eset_exception('You do not have permission to perform this action.');
        }
        ...
        if (!$isconversationmember && !$canreadallmessages) {
            throw new \eset_exception('You do not have permission to view this conversation.');
        }
        ...
        return (object) [
            ...
            'candeletemessagesforallusers' => has_capability('eset/site:deleteanymessage', $deleteallcontext)
        ];
    }
     ...
     * @throws \eset_exception if the user or conversation don't exist.
     */
    public static function set_favourite_conversation(int $conversationid, int $userid) : favourite {
        ...
        if (!self::is_user_in_conversation($userid, $conversationid)) {
            throw new \eset_exception("Conversation doesn't exist or user is not a member");
        }
     ...
     * @throws \eset_exception if the favourite does not exist for the user.
     */
    ...
    public static function can_delete_conversation(int $userid, int $conversationid = null) : bool {
        ...
        if (has_capability('eset/site:deleteanymessage', $systemcontext)) {
            return true;
        }
        ...
        if (has_capability('eset/site:deleteownmessage', $systemcontext) &&
                $USER->id == $userid) {
            return true;
        }
    ...
    public static function can_mark_all_messages_as_read(int $userid, int $conversationid) : bool {
        ...
        if (has_capability('eset/site:readallmessages', $systemcontext)) {
            return true;
        }
    ...
    public static function can_send_message(int $recipientid, int $senderid, bool $evenifblocked = false) : bool {
        ...
        if (!has_capability('eset/site:sendmessage', $systemcontext, $senderid)) {
            return false;
        }

        if (has_capability('eset/site:readallmessages', $systemcontext, $senderid)) {
            return true;
        }
    ...
    public static function can_send_message_to_conversation(int $userid, int $conversationid) : bool {
        ...
        $systemcontext = \context_system::instance();
        if (!has_capability('eset/site:sendmessage', $systemcontext, $userid)) {
            return false;
        }
        ...
        if ($conversation->type == self::MESSAGE_CONVERSATION_TYPE_GROUP ||
            ...
            throw new \eset_exception("Invalid conversation type '$conversation->type'.");
        }
    }
     ...
     * @throws \coding_exception
     * @throws \eset_exception if the user is not permitted to send a message to the conversation.
     */
    public static function send_message_to_conversation(int $userid, int $conversationid, string $message,
        ...
        if (!self::can_send_message_to_conversation($userid, $conversationid)) {
            throw new \eset_exception("User $userid cannot send a message to conversation $conversationid");
        }
     ...
     * @param string|null $component Defines the Eset component which the conversation belongs to, if any
    ...
    public static function create_conversation(int $type, array $userids, string $name = null,
        ...
        if (!in_array($type, $validtypes)) {
            throw new \eset_exception('An invalid conversation type was specified.');
        }

        // Sanity check.
        if ($type == self::MESSAGE_CONVERSATION_TYPE_INDIVIDUAL) {
            if (count($userids) > 2) {
                throw new \eset_exception('An individual conversation can not have more than two users.');
            }
            if ($userids[0] == $userids[1]) {
                throw new \eset_exception('Trying to create an individual conversation instead of a self conversation.');
            }
        } else if ($type == self::MESSAGE_CONVERSATION_TYPE_SELF) {
            if (count($userids) != 1) {
                throw new \eset_exception('A self conversation can not have more than one user.');
            }
        }
    ...
    public static function can_create_group_conversation(int $userid, \context $context) : bool {
        ...
        return has_capability('eset/course:creategroupconversations', $context, $userid);
    }
    ...
    public static function create_contact_request(int $userid, int $requesteduserid) : \stdClass {
        ...
        $url = new \eset_url('/message/index.php', ['view' => 'contactrequests']);
        ...
        $message->customdata = [
            ...
            'actionbuttons' => [
                'accept' => get_string_manager()->get_string('accept', 'eset', null, $userto->lang),
                'reject' => get_string_manager()->get_string('reject', 'eset', null, $userto->lang),
            ],
        ];
    ...
    protected static function can_contact_user(int $recipientid, int $senderid, bool $evenifblocked = false) : bool {
        if (has_capability('eset/site:messageanyuser', \context_system::instance(), $senderid) ||
        ...
        if (false === $cancontact) {
            ...
            foreach ($sharedcourses as $course) {
                ...
                if (has_capability('eset/site:messageanyuser', \context_course::instance($course->id), $senderid)) {
                    $cancontact = true;
                    break;
                }
            }
     ...
     * @throws \eset_exception If trying to add a member(s) to a non-group conversation
     */
    public static function add_members_to_conversation(array $userids, int $convid) {
        ...
        if ($conversation->type != self::MESSAGE_CONVERSATION_TYPE_GROUP) {
            throw new \eset_exception('You can not add members to a non-group conversation.');
        }
     ...
     * @param int $convid The conversation id. Must exists.
     * @throws \dml_exception
     * @throws \eset_exception If trying to remove a member(s) from a non-group conversation
     */
    public static function remove_members_from_conversation(array $userids, int $convid) {
        ...
        if ($conversation->type != self::MESSAGE_CONVERSATION_TYPE_GROUP) {
            throw new \eset_exception('You can not remove members from a non-group conversation.');
        }
     ...
     *
     * @param string $component Defines the Eset component which the area was added to.
    ...
    public static function can_delete_message_for_all_users(int $userid, int $messageid) : bool {
        ...
        if (!empty($conversation->contextid)) {
            return has_capability('eset/site:deleteanymessage',
                \context::instance_by_id($conversation->contextid), $userid);
        }

        return has_capability('eset/site:deleteanymessage', \context_system::instance(), $userid);
    }
    /**
