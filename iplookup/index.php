...
if ($user) {
    if ($user = $DB->get_record('user', array('id'=>$user, 'deleted'=>0))) {
        ...
        if (has_capability('eset/user:viewdetails', context_user::instance($user->id))) {
...
if (empty($CFG->googlemapkey3)) {
    ...
    if (is_https()) {
        $PAGE->requires->js(new eset_url('https://maps.googleapis.com/maps/api/js', array('key'=>$CFG->googlemapkey3, 'sensor'=>'false')));
    } else {
        $PAGE->requires->js(new eset_url('http://maps.googleapis.com/maps/api/js', array('key'=>$CFG->googlemapkey3, 'sensor'=>'false')));
    }
