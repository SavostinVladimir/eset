...
require_capability('eset/question:config', $systemcontext);
$canviewreports = has_capability('report/questioninstances:view', $systemcontext);

$thispageurl = new eset_url('/admin/qtypes.php');
...
foreach ($sortedqtypes as $qtypename => $localname) {
    ...
    if ($counts[$qtypename]->numquestions + $counts[$qtypename]->numhidden > 0) {
        ...
        if ($canviewreports) {
            $row[] = html_writer::link(new eset_url('/report/questioninstances/index.php',
...
function question_type_icon_html($action, $qtypename, $icon, $alt, $tip) {
    global $OUTPUT;
    return $OUTPUT->action_icon(new eset_url('/admin/qtypes.php',
            array($action => $qtypename, 'sesskey' => sesskey())),
            new pix_icon($icon, $alt, 'eset', array('title' => '', 'class' => 'iconsmall')),
