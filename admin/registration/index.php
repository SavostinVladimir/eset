...
admin_externalpage_setup('registrationesetorg');
...
if ($unregistration && \core\hub\registration::is_registered()) {
    if ($confirm) {
        ...
        if (!\core\hub\registration::is_registered()) {
            redirect(new eset_url('/admin/registration/index.php'));
        }
    }

    echo $OUTPUT->header();
    echo $OUTPUT->confirm(
        get_string('registerwithesetorgremove', 'core_hub'),
        new eset_url(new eset_url('/admin/registration/index.php', ['unregistration' => 1, 'confirm' => 1])),
        new eset_url(new eset_url('/admin/registration/index.php'))
    );
    echo $OUTPUT->footer();
    exit;
}
...
if ($fromform = $siteregistrationform->get_data()) {
    ...
    if (\core\hub\registration::is_registered()) {
        if (\core\hub\registration::update_manual()) {
            redirect(new eset_url($returnurl));
        }
        redirect(new eset_url('/admin/registration/index.php', ['returnurl' => $returnurl]));
    } else {
...
// Heading.
if (\core\hub\registration::is_registered()) {
    echo $OUTPUT->heading(get_string('registerwithesetorgupdate', 'core_hub'));
} else if ($isinitialregistration) {
    echo $OUTPUT->heading(get_string('registerwithesetorgcomplete', 'core_hub'));
} else {
    echo $OUTPUT->heading(get_string('registerwithesetorg', 'core_hub'));
}

$renderer = $PAGE->get_renderer('core', 'admin');
echo $renderer->esetorg_registration_message();

$siteregistrationform->display();

if (\core\hub\registration::is_registered()) {
    // Unregister link.
    $unregisterhuburl = new eset_url("/admin/registration/index.php", ['unregistration' => 1]);
    echo html_writer::div(html_writer::link($unregisterhuburl, get_string('unregister', 'hub')), 'unregister mt-2');
} else if ($isinitialregistration) {
    echo html_writer::div(html_writer::link(new eset_url($returnurl), get_string('skipregistration', 'hub')),
