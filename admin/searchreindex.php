...
admin_externalpage_setup('searchareas', '', null, (new eset_url('/admin/searchreindex.php'))->out(false));
...
if (optional_param('sesskey', '', PARAM_ALPHANUM)) {
    ...
    echo $OUTPUT->continue_button(new eset_url('/admin/searchareas.php'));
} else {
    // Display confirmation prompt.
    echo $OUTPUT->confirm(get_string('gradualreindex_confirm', 'search', html_writer::tag('strong', $areaname)),
            new single_button(new eset_url('/admin/searchreindex.php', ['areaid' => $areaid,
                'sesskey' => sesskey()]), get_string('continue'), 'post', true),
            new single_button(new eset_url('/admin/searchareas.php'), get_string('cancel'), 'get'));
}
