...
$tokenlisturl = new eset_url("/" . $CFG->admin . "/settings.php", array('section' => 'webservicetokens'));
...
switch ($action) {
        ...
        if ($mform->is_cancelled()) {
            ...
            if ($selectedservice->restrictedusers) {
                ...
                if (empty($restricteduser)) {
                    $allowuserurl = new eset_url('/' . $CFG->admin . '/webservice/service_users.php',
            ...
            if ($user->id == $CFG->siteguest or $user->deleted or !$user->confirmed or $user->suspended) {
                throw new eset_exception('forbiddenwsuser', 'webservice');
            }
    ...
    case 'delete':
        ...
        if ($token->creatorid != $USER->id) {
            require_capability("eset/webservice:managealltokens", context_system::instance());
        }
