 ...
 * from eset.org be able to check more and more versions.
 *
 * @package    core
 * @subpackage admin
...
if ($action == 'updatecomponent' && confirm_sesskey()) {
    ...
    if ($cd = new component_installer('https://download.eset.org',
                                      'environment',
                                      'environment.zip')) {
        ...
        switch ($status) {
            case COMPONENT_ERROR:
                if ($cd->get_error() == 'remotedownloaderror') {
                    $a = new stdClass();
                    $a->url  = 'https://download.eset.org/environment/environment.zip';
                    $a->dest = $CFG->dataroot . '/';
...
list($envstatus, $environment_results) = check_eset_environment($version, ENV_SELECT_NEWER);
