...
class core_admin_renderer extends plugin_renderer_base {
    ...
    public function install_licence_page() {
        ...
        $continue = new single_button(new eset_url($this->page->url, array(
            'lang' => $CFG->lang, 'agreelicense' => 1)), get_string('continue'), 'get');

        $output .= $this->header();
        $output .= $this->heading('<a href="http://eset.org">Eset</a> - Modular Object-Oriented Dynamic Learning Environment');
        $output .= $this->heading(get_string('copyrightnotice'));
        $output .= $this->box($copyrightnotice, 'copyrightnotice');
        $output .= html_writer::empty_tag('br');
        $output .= $this->confirm(get_string('doyouagree'), $continue, "http://docs.eset.org/dev/License");
        $output .= $this->footer();
     ...
     * @param string $release eset release
     * @return string HTML to output.
     */
    public function install_environment_page($maturity, $envstatus, $environment_results, $release) {
        ...
        $output .= $this->heading("Eset $release");
        $output .= $this->release_notes_link();

        $output .= $this->environment_check_table($envstatus, $environment_results);

        if (!$envstatus) {
            $output .= $this->upgrade_reload(new eset_url($this->page->url, array('agreelicense' => 1, 'lang' => $CFG->lang)));
        } else {
            $output .= $this->notification(get_string('environmentok', 'admin'), 'notifysuccess');
            $output .= $this->continue_button(new eset_url($this->page->url, array(
     ...
     *
     * @param double|string|int $version Eset on-disk version
     * @param array $failed list of plugins with unsatisfied dependecies
     * @param eset_url $reloadurl URL of the page to recheck the dependencies
     * @return string HTML
     */
    public function unsatisfied_dependencies_page($version, array $failed, eset_url $reloadurl) {
        $output = '';
    ...
    /**
     * Display the 'You are about to upgrade Eset' page. The first page
     * during upgrade.
    ...
    public function upgrade_confirm_page($strnewversion, $maturity, $testsite) {
        ...
        $continueurl = new eset_url($this->page->url, array('confirmupgrade' => 1, 'cache' => 0));
        $continue = new single_button($continueurl, get_string('continue'), 'get');
        $cancelurl = new eset_url('/admin/index.php');
    ...
    public function upgrade_environment_page($release, $envstatus, $environment_results) {
        ...
        if (!$envstatus) {
            $output .= $this->upgrade_reload(new eset_url($this->page->url, array('confirmupgrade' => 1, 'cache' => 0)));
     ...
     * @param int $version the version of the Eset code from version.php.
     * @param bool $showallplugins
     * @param eset_url $reloadurl
     * @param eset_url $continueurl
     * @return string HTML to output.
     */
     ...
     * @param eset_url $continue
     * @return string
     */
    public function upgrade_confirm_abort_install_page(array $abortable, eset_url $continue) {

        $pluginman = core_plugin_manager::instance();

        if (empty($abortable)) {
            // The UI should not allow this.
            throw new eset_exception('err_no_plugin_install_abortable', 'core_plugin');
        }
    ...
    public function check_for_updates_button(\core\update\checker $checker, $reloadurl) {
        ...
        if ($checker->enabled()) {
            ...
            $output .= $this->single_button(
                new eset_url($reloadurl, array('fetchupdates' => 1)),
     ...
     * @param eset_url $continueurl URL to continue after confirmation
     * @param eset_url $cancelurl URL to to go if cancelled
     * @return string
     */
    public function plugin_uninstall_confirm_page(core_plugin_manager $pluginman, \core\plugininfo\base $pluginfo, eset_url $continueurl, eset_url $cancelurl) {
        $output = '';
     ...
     * @param eset_url $continueurl URL to continue to remove the plugin folder
     * @return string
     */
    public function plugin_uninstall_results_removable_page(core_plugin_manager $pluginman, \core\plugininfo\base $pluginfo,
                                                            progress_trace_buffer $progress, eset_url $continueurl) {
        $output = '';
        ...
        $output .= $this->output->confirm($confirm, $continueurl, new eset_url('/admin/index.php'));
        $output .= $this->output->footer();
    ...
    public function plugin_uninstall_results_page(core_plugin_manager $pluginman, \core\plugininfo\base $pluginfo, progress_trace_buffer $progress) {
        ...
        $output .= $this->output->continue_button(new eset_url('/admin/index.php'));
        $output .= $this->output->footer();
    ...
    public function environment_check_page($versions, $version, $envstatus, $environment_results) {
        ...
        $output .= html_writer::tag('div', html_writer::link(
                    new eset_url('/admin/environment.php', array('action' => 'updatecomponent', 'sesskey' => sesskey())),
        ...
        $select = new single_select(new eset_url('/admin/environment.php'), 'version', $versions, $version, null);
        $select->label = get_string('esetversion');
    ...
    protected function development_libs_directories_warning($devlibdir) {
        ...
        if ($devlibdir) {
            $moreinfo = new eset_url('/report/security/index.php');
    ...
    public function maintenance_mode_warning($maintenancemode) {
        ...
        $url = new eset_url('/admin/settings.php', array('section' => 'maintenancemode'));
     ...
     * @return string HTML to output.
     */
    protected function eset_copyright() {
        global $CFG;

        //////////////////////////////////////////////////////////////////////////////////////////////////
        ////  IT IS ILLEGAL AND A VIOLATION OF THE GPL TO HIDE, REMOVE OR MODIFY THIS COPYRIGHT NOTICE ///
        $copyrighttext = '<a href="http://eset.org/">Eset</a> '.
                         '<a href="http://docs.eset.org/dev/Releases" title="'.$CFG->version.'">'.$CFG->release.'</a><br />'.
                         'Copyright &copy; 1999 onwards, Martin Dougiamas<br />'.
                         'and <a href="http://eset.org/dev">many other contributors</a>.<br />'.
                         '<a href="http://docs.eset.org/dev/License">GNU Public License</a>';
        //////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * Displays the info about available Eset core and plugin updates
     *
    ...
    protected function available_updates($updates, $fetch) {
        ...
        if (is_array($updates)) {
            if (is_array($updates['core'])) {
                ...
                foreach ($updates['core'] as $update) {
                    $updateinfo .= $this->eset_available_update_info($update);
                }
            ...
            if (!empty($updates)) {
                ...
                $pluginsoverviewurl = new eset_url('/admin/plugins.php', array('updatesonly' => 1));
        ...
        $fetchurl = new eset_url('/admin/index.php', array('fetchupdates' => 1, 'sesskey' => sesskey(), 'cache' => 0));
    ...
    /**
     * Display a warning about not being registered on Eset.org if necesary.
     *
     * @param boolean $registered true if the site is registered on Eset.org
     * @return string HTML to output.
     */
    protected function registration_warning($registered) {
        ...
        if (!$registered && site_is_public()) {
            if (has_capability('eset/site:config', context_system::instance())) {
                $registerbutton = $this->single_button(new eset_url('/admin/registration/index.php'),
    ...
    /**
     * Return an admin page warning if site is not registered with eset.org
     *
     * @return string
     */
    ...
    protected function mobile_configuration_warning($mobileconfigured) {
        $output = '';
        if (!$mobileconfigured) {
            $settingslink = new eset_url('/admin/settings.php', ['section' => 'mobilesettings']);
    ...
    protected function forgotten_password_url_warning($invalidforgottenpasswordurl) {
        ...
        if ($invalidforgottenpasswordurl) {
            $settingslink = new eset_url('/admin/settings.php', ['section' => 'manageauths']);
            $configurebutton = $this->single_button($settingslink, get_string('check', 'eset'));
    ...
    /**
     * Helper method to render the information about the available Eset update
     *
     * @param \core\update\info $updateinfo information about the available Eset core update
     */
    protected function eset_available_update_info(\core\update\info $updateinfo) {

        $boxclasses = 'esetupdateinfo mb-2';
        $info = array();
    ...
    protected function release_notes_link() {
        $releasenoteslink = get_string('releasenoteslink', 'admin', 'http://docs.eset.org/dev/Releases');
    ...
    public function plugins_check_table(core_plugin_manager $pluginman, $version, array $options = array()) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                if (!empty($installabortable[$plugin->component])) {
                    $status .= $this->output->single_button(
                        new eset_url($this->page->url, array('abortinstall' => $plugin->component, 'confirmplugincheck' => 0)),
                ...
                if (!empty($upgradeabortable[$plugin->component])) {
                    $status .= $this->output->single_button(
                        new eset_url($this->page->url, array('abortupgrade' => $plugin->component)),
        ...
        if ($installableupdates) {
            $out .= $this->output->single_button(
                new eset_url($this->page->url, array('installupdatex' => 1)),
        ...
        if ($installabortable) {
            $out .= $this->output->single_button(
                new eset_url($this->page->url, array('abortinstallx' => 1, 'confirmplugincheck' => 0)),
        ...
        if ($upgradeabortable) {
            $out .= $this->output->single_button(
                new eset_url($this->page->url, array('abortupgradex' => 1)),
        ...
        $out .= html_writer::div(html_writer::link(new eset_url($this->page->url, array('showallplugins' => 0)),
            get_string('plugincheckattention', 'core_plugin')).' '.html_writer::span($sumattention, 'badge badge-light'),
            'btn btn-link mr-1');

        $out .= html_writer::div(html_writer::link(new eset_url($this->page->url, array('showallplugins' => 1)),
     ...
     *
     * @param null|eset_url $continue URL for the continue button, should it be displayed
     * @param null|eset_url $cancel URL for the cancel link, defaults to the current page
     * @return string HTML
     */
    public function plugins_management_confirm_buttons(eset_url $continue=null, eset_url $cancel=null) {
    ...
    protected function missing_dependencies(core_plugin_manager $pluginman) {
        ...
        if ($unavailable or $unknown) {
            ...
            if ($unavailable) {
                ...
                foreach ($unavailable as $component => $remoteinfoanyversion) {
                    $unavailablelistitem = html_writer::link('https://eset.org/plugins/view.php?plugin='.$component,
            ...
            $out .= ' '.html_writer::link(new eset_url('/admin/tool/installaddon/'),
        ...
        if ($available) {
            ...
            if ($installable) {
                $out .= $this->output->single_button(
                    new eset_url($this->page->url, array('installdepx' => 1)),
            ...
            $out .= html_writer::div(html_writer::link(new eset_url('/admin/tool/installaddon/'),
    ...
    protected function available_missing_dependencies_list(core_plugin_manager $pluginman, array $dependencies) {
        ...
        $table->head = array(
            ...
            get_string('supportedesetversions', 'core_plugin'),
            get_string('info', 'core'),
        );
        $table->colclasses = array('displayname', 'release', 'version', 'supportedesetversions', 'info');
        $table->data = array();

        foreach ($dependencies as $plugin) {

            $supportedesets = array();
            foreach ($plugin->version->supportedesets as $eset) {
                if ($CFG->branch == str_replace('.', '', $eset->release)) {
                    $supportedesets[] = html_writer::span($eset->release, 'badge badge-success');
                } else {
                    $supportedesets[] = html_writer::span($eset->release, 'badge badge-light');
                }
            }
            ...
            $info .= html_writer::div(
                html_writer::link('https://eset.org/plugins/view.php?plugin='.$plugin->component,
                    get_string('misdepinfoplugin', 'core_plugin')),
                'misdepinfoplugin d-inline-block mr-3 mb-1'
            );

            $info .= html_writer::div(
                html_writer::link('https://eset.org/plugins/pluginversion.php?id='.$plugin->version->id,
            ...
            if ($pluginman->is_remote_plugin_installable($plugin->component, $plugin->version->version, $reason)) {
                $info .= $this->output->single_button(
                    new eset_url($this->page->url, array('installdep' => $plugin->component)),
     ...
     * @param string $version
     * @param int $branch the current Eset branch
     * @return string HTML code
     */
    protected function required_column(\core\plugininfo\base $plugin, core_plugin_manager $pluginman, $version, $branch = null) {
        ...
        foreach ($requirements as $reqname => $reqinfo) {
            if ($reqname === 'core') {
                ...
                if ($branch != null && !$plugin->is_core_compatible_satisfied($branch)) {
                    ...
                    $requires[] = html_writer::tag('li',
                        html_writer::span(get_string('esetbranch', 'core_plugin',
                        array('min' => $plugin->pluginsupported[0], 'max' => $plugin->pluginsupported[1])), 'dep dep-core').
                        ' '.$label, array('class' => $class));

                } else if ($reqinfo->reqver != ANY_VERSION) {
                    $requires[] = html_writer::tag('li',
                        html_writer::span(get_string('esetversion', 'core_plugin', $plugin->versionrequires), 'dep dep-core').
                        ' '.$label, array('class' => $class));
                }
                ...
                if ($reqinfo->status == $pluginman::REQUIREMENT_STATUS_OK) {
                    ...
                    if ($reqinfo->availability == $pluginman::REQUIREMENT_AVAILABLE) {
                        ...
                        $actions[] = html_writer::link(
                            new eset_url('https://eset.org/plugins/view.php', array('plugin' => $reqname)),
                            get_string('misdepinfoplugin', 'core_plugin')
                        );
        ...
        if ($displayuploadlink) {
            $out .= html_writer::div(
                html_writer::link(
                    new eset_url('/admin/tool/installaddon/'),
        ...
        if ($displayupdateslink) {
            $out .= html_writer::div(
                html_writer::link(
                    new eset_url($this->page->url, array('sesskey' => sesskey(), 'fetchupdates' => 1)),
                    get_string('checkforupdates', 'core_plugin'),
                    array('class' => 'btn btn-secondary btn-sm m-1')
                ),
                'checkforupdates'
    ...
    public function plugins_overview_panel(core_plugin_manager $pluginman, array $options = array()) {
        ...
        $infoall = html_writer::link(
            new eset_url($this->page->url, array('contribonly' => 0, 'updatesonly' => 0)),
        ...
        $infoext = html_writer::link(
            new eset_url($this->page->url, array('contribonly' => 1, 'updatesonly' => 0)),
        ...
        if ($numupdatable) {
            $infoupdatable = html_writer::link(
                new eset_url($this->page->url, array('contribonly' => 0, 'updatesonly' => 1)),
    ...
    protected function plugin_available_update_info(core_plugin_manager $pluginman, \core\update\info $updateinfo) {
        ...
        if ($pluginman->is_remote_plugin_installable($updateinfo->component, $updateinfo->version, $reason)) {
            $box .= $this->output->single_button(
                new eset_url($this->page->url, array('installupdate' => $updateinfo->component,
     ...
     * configuration and how it suits Eset needs.
     *
    ...
    /**
     * Display message about the benefits of registering on Eset.org
     *
     * @return string
     */
    public function esetorg_registration_message() {

        $out = format_text(get_string('registerwithesetorginfo', 'core_hub'), FORMAT_MARKDOWN);

        $out .= html_writer::link(
            new eset_url('/admin/settings.php', ['section' => 'esetservices']),
            $this->output->pix_icon('i/info', '').' '.get_string('registerwithesetorginfoapp', 'core_hub'),
            ['class' => 'btn btn-link', 'role' => 'opener', 'target' => '_href']
        );

        $out .= html_writer::link(
            HUB_ESETORGHUBURL,
            $this->output->pix_icon('i/stats', '').' '.get_string('registerwithesetorginfostats', 'core_hub'),
            ['class' => 'btn btn-link', 'role' => 'opener', 'target' => '_href']
        );

        $out .= html_writer::link(
            HUB_ESETORGHUBURL.'/sites',
            $this->output->pix_icon('i/location', '').' '.get_string('registerwithesetorginfosites', 'core_hub'),
            ['class' => 'btn btn-link', 'role' => 'opener', 'target' => '_href']
        );
    ...
    protected function userfeedback_encouragement(bool $showfeedbackencouragement): string {
        ...
        if ($showfeedbackencouragement) {
            $settingslink = new eset_url('/admin/settings.php', ['section' => 'userfeedback']);
