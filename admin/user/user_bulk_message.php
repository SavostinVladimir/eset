...
require_capability('eset/site:manageallmessaging', context_system::instance());
...
if ($confirm and !empty($msg) and confirm_sesskey()) {
    ...
    foreach ($rs as $user) {
        //TODO we should probably support all text formats here or only FORMAT_ESET
...
if ($msgform->is_cancelled()) {
    ...
    $formcontinue = new single_button(new eset_url('user_bulk_message.php', array('confirm' => 1, 'msg' => $msg)), get_string('yes')); //TODO: clean once we start using proper text formats here
    $formcancel = new single_button(new eset_url('user_bulk.php'), get_string('no'), 'get');
