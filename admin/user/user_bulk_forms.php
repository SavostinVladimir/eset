 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_bulk_action_form extends esetform {
    ...
    public function get_actions(): array {
        ...
        $actions = [];
        if (has_capability('eset/user:update', $syscontext)) {
            $actions['confirm'] = new action_link(
                new eset_url('/admin/user/user_bulk_confirm.php'),
                get_string('confirm'));
        }
        if (has_capability('eset/site:readallmessages', $syscontext) && !empty($CFG->messaging)) {
            $actions['message'] = new action_link(
                new eset_url('/admin/user/user_bulk_message.php'),
                get_string('messageselectadd'));
        }
        if (has_capability('eset/user:delete', $syscontext)) {
            $actions['delete'] = new action_link(
                new eset_url('/admin/user/user_bulk_delete.php'),
                get_string('delete'));
        }
        $actions['displayonpage'] = new action_link(
                new eset_url('/admin/user/user_bulk_display.php'),
                get_string('displayonpage'));

        if (has_capability('eset/user:update', $syscontext)) {
            $actions['download'] = new action_link(
                new eset_url('/admin/user/user_bulk_download.php'),
                get_string('download', 'admin'));
        }

        if (has_capability('eset/user:update', $syscontext)) {
            $actions['forcepasswordchange'] = new action_link(
                new eset_url('/admin/user/user_bulk_forcepasswordchange.php'),
                get_string('forcepasswordchange'));
        }
        if (has_capability('eset/cohort:assign', $syscontext)) {
            $actions['addtocohort'] = new action_link(
                new eset_url('/admin/user/user_bulk_cohortadd.php'),
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_bulk_form extends esetform {
