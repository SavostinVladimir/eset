...
$pageurl = new eset_url('/admin/plugins.php', $pageparams);
...
if ($uninstall) {
    ...
    if (is_null($pluginfo)) {
        throw new eset_exception('err_uninstalling_unknown_plugin', 'core_plugin', '', array('plugin' => $uninstall),
    ...
    if (!$pluginman->can_uninstall_plugin($pluginfo->component)) {
        throw new eset_exception('err_cannot_uninstall_plugin', 'core_plugin', '',
            array('plugin' => $pluginfo->component),
            'core_plugin_manager::can_uninstall_plugin() returned false');
    }

    if (!$confirmed) {
        $continueurl = new eset_url($PAGE->url, array('uninstall' => $pluginfo->component, 'sesskey' => sesskey(), 'confirm' => 1, 'return'=>$return));
        ...
        if ($pluginman->is_plugin_folder_removable($pluginfo->component)) {
            $continueurl = new eset_url($PAGE->url, array('delete' => $pluginfo->component, 'sesskey' => sesskey(), 'confirm' => 1));
...
if ($delete and $confirmed) {
    ...
    // Make sure we know the plugin.
    if (is_null($pluginfo)) {
        throw new eset_exception('err_removing_unknown_plugin', 'core_plugin', '', array('plugin' => $delete),
    ...
    if (!is_null($pluginfo->versiondb)) {
        throw new eset_exception('err_removing_installed_plugin', 'core_plugin', '',
    ...
    // Make sure the folder is within Eset installation tree.
    if (strpos($pluginfo->rootdir, $CFG->dirroot) !== 0) {
        throw new eset_exception('err_unexpected_plugin_rootdir', 'core_plugin', '',
            array('plugin' => $pluginfo->component, 'rootdir' => $pluginfo->rootdir, 'dirroot' => $CFG->dirroot),
            'plugin root folder not in the eset dirroot');
    }
