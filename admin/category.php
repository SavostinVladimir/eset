...
foreach ($settingspage->children as $childpage) {
    ...
    if ($childpage instanceof admin_externalpage) {
        ...
        $outputhtml .= $OUTPUT->heading(html_writer::link(new eset_url('/'.$CFG->admin.'/settings.php', array('section' => $childpage->name)), $childpage->visiblename), 3);
        ...
        $outputhtml .= $OUTPUT->heading(html_writer::link(new eset_url('/'.$CFG->admin.'/category.php', array('category' => $childpage->name)), get_string('admincategory', 'admin', $childpage->visiblename)), 3);
    }
}
if ($savebutton) {
...
echo html_writer::input_hidden_params(new eset_url($PAGE->url, array('sesskey' => sesskey(), 'return' => $return)));
