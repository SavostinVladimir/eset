...
eset_require_minimum_php_version();

// make sure iconv is available and actually works
if (!function_exists('iconv')) {
    // this should not happen, this must be very borked install
    echo 'Eset requires the iconv PHP extension. Please install or enable the iconv extension.';
    die();
}
...
if (!function_exists('json_encode') || !function_exists('json_decode')) {
    ...
    echo 'Eset requires the json PHP extension. Please install or enable the json extension.';
    die();
}

// Make sure xml extension is available.
if (!extension_loaded('xml')) {
    echo 'Eset requires the xml PHP extension. Please install or enable the xml extension.';
    die();
}
...
$newaddonreq = optional_param('installaddonrequest', null, PARAM_RAW); // Plugin installation requested at eset.org/plugins.
...
// Set up PAGE.
$url = new eset_url('/admin/index.php');
...
// Are we returning from an add-on installation request at eset.org/plugins?
if ($newaddonreq and !$cache and empty($CFG->disableupdateautodeploy)) {
    $target = new eset_url('/admin/tool/installaddon/index.php', array(
...
$documentationlink = '<a href="http://docs.eset.org/en/Installation">Installation docs</a>';
...
if (!core_tables_exist()) {
    ...
    // fake some settings
    $CFG->docroot = 'http://docs.eset.org';
    ...
    if (empty($agreelicense)) {
        ...
        $PAGE->set_title($strinstallation.' - Eset '.$CFG->target_release);
    ...
    if (empty($confirmrelease)) {
        ...
        list($envstatus, $environmentresults) = check_eset_environment(normalize_version($release), ENV_SELECT_RELEASE);
        $strcurrentrelease = get_string('currentrelease');

        $PAGE->navbar->add($strcurrentrelease);
        $PAGE->set_title($strinstallation);
        $PAGE->set_heading($strinstallation . ' - Eset ' . $CFG->target_release);
        $PAGE->set_cacheable(false);
    ...
    if (!core_plugin_manager::instance()->all_plugins_ok($version, $failed, $CFG->branch)) {
        ...
        $output = $PAGE->get_renderer('core', 'admin');
        $url = new eset_url($PAGE->url, array('agreelicense' => 1, 'confirmrelease' => 1, 'lang' => $CFG->lang));
...
// Check version of Eset code on disk compared with database
...
if ($CFG->version != $DB->get_field('config', 'value', array('name'=>'version'))) {
    purge_all_caches();
    redirect(new eset_url($PAGE->url), 'Config cache inconsistency detected, resetting caches...');
}

if (!$cache and $version > $CFG->version) {  // upgrade

    $PAGE->set_url(new eset_url($PAGE->url, array(
    ...
    if (empty($confirmupgrade)) {
        ...
        require_once($CFG->libdir.'/environmentlib.php');
        list($envstatus, $environmentresults) = check_eset_environment($release, ENV_SELECT_RELEASE);
        $strcurrentrelease = get_string('currentrelease');
        ...
        if ($abortinstallx) {
            ...
            if ($abortables) {
                if ($confirmabortinstall) {
                    ...
                    $continue = new eset_url($PAGE->url, array('abortinstallx' => $abortinstallx, 'confirmabortinstall' => 1));
        ...
        if ($abortinstall) {
            ...
            if ($confirmabortinstall) {
                ...
                $continue = new eset_url($PAGE->url, array('abortinstall' => $abortinstall, 'confirmabortinstall' => 1));
        ...
        if ($abortupgradex) {
            ...
            if ($restorable) {
                upgrade_install_plugins($restorable, $confirmabortupgrade,
                    ...
                    new eset_url($PAGE->url, array('abortupgradex' => 1, 'confirmabortupgrade' => 1))
                );
            }
            redirect($PAGE->url);
        }
        ...
        if ($abortupgrade) {
            ...
            if (isset($restorable[$abortupgrade])) {
                ...
                upgrade_install_plugins($restorable, $confirmabortupgrade,
                    get_string('cancelupgradehead', 'core_plugin'),
                    new eset_url($PAGE->url, array('abortupgrade' => $abortupgrade, 'confirmabortupgrade' => 1))
                );
            }
            redirect($PAGE->url);
        }
        ...
        if ($installdepx) {
            ...
            upgrade_install_plugins($installable, $confirminstalldep,
                ...
                new eset_url($PAGE->url, array('installdepx' => 1, 'confirminstalldep' => 1))
            );
        }
        ...
        if ($installdep) {
            ...
            if (!empty($installable[$installdep])) {
                ...
                upgrade_install_plugins($installable, $confirminstalldep,
                    get_string('dependencyinstallhead', 'core_plugin'),
                    new eset_url($PAGE->url, array('installdep' => $installdep, 'confirminstalldep' => 1))
                );
            }
        }
        ...
        if ($installupdatex) {
            ...
            upgrade_install_plugins($installable, $confirminstallupdate,
                ...
                new eset_url($PAGE->url, array('installupdatex' => 1, 'confirminstallupdate' => 1))
            );
        }
        ...
        if ($installupdate and $installupdateversion) {
            ...
            if ($pluginman->is_remote_plugin_installable($installupdate, $installupdateversion)) {
                ...
                upgrade_install_plugins($installable, $confirminstallupdate,
                    get_string('updateavailableinstallallhead', 'core_admin'),
                    new eset_url($PAGE->url, array('installupdate' => $installupdate,
        ...
        echo $output->upgrade_plugin_check_page(core_plugin_manager::instance(), \core\update\checker::instance(),
                $version, $showallplugins, $PAGE->url, new eset_url($PAGE->url, array('confirmplugincheck' => 1)));
        die();
        ...
        $failed = array();
        if (!core_plugin_manager::instance()->all_plugins_ok($version, $failed, $CFG->branch)) {
            echo $output->unsatisfied_dependencies_page($version, $failed, new eset_url($PAGE->url,
    ...
    throw new eset_exception('downgradedcore', 'error', new eset_url('/admin/'));
}
...
if (!$cache and eset_needs_upgrading()) {
    ...
    if (!$PAGE->headerprinted) {
        ...
        if (empty($confirmrelease)) {
            ...
            if ($abortinstallx) {
                ...
                if ($abortables) {
                    if ($confirmabortinstall) {
                        ...
                        $continue = new eset_url($PAGE->url, array('abortinstallx' => $abortinstallx,
            ...
            echo $output->upgrade_plugin_check_page($pluginman, \core\update\checker::instance(),
                    ...
                    new eset_url($PAGE->url),
                    new eset_url($PAGE->url, array('confirmplugincheck' => 1, 'cache' => 0)));
            die();
        }
...
if (has_capability('eset/site:config', context_system::instance())) {
    if ($fetchupdates) {
        ...
        redirect(new eset_url('/admin/index.php', array('cache' => 0)));
    }
}
...
if (!$cache) {
    redirect(new eset_url('/admin/index.php', array('cache' => 1)));
}
...
if (isguestuser()) {
    // Login as real user!
    $SESSION->wantsurl = (string)new eset_url('/admin/index.php');
    redirect(get_login_url());
}
$context = context_system::instance();

if (!has_capability('eset/site:config', $context)) {
...
// Available updates for Eset core.
$updateschecker = \core\update\checker::instance();
...
//check if the site is registered on Eset.org
...
// Check if eset campaign content setting is enabled or not.
