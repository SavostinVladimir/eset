<?php
/**
 * A service browser for remote Esets
 *
 * This script 'remotely' executes the reflection methods on a remote Eset,
...
$esetapplicationid = $DB->get_field('mnet_application', 'id', array('name' => 'eset'));

$url = new eset_url('/admin/mnet/testclient.php');
$PAGE->set_url($url);
...
foreach ($hosts as $id => $host) {
    ...
    $newurl = new eset_url($url, array('hostid' => $host->id));
    echo '<p>' . html_writer::link($newurl, $host->wwwroot) . '</p>';
}

if (!empty($hostid) && array_key_exists($hostid, $hosts)) {
    $host = $hosts[$hostid];
    if ($host->applicationid != $esetapplicationid) {
        echo $OUTPUT->notification(get_string('notesetapplication', 'mnet'));
    }
    ...
    foreach ($services as $id => $servicedata) {
        ...
        $newurl = new eset_url($url, array('hostid' => $host->id, 'servicename' => $servicedata['name']));
        $table->data[] = array(
            $servicedata['name'],
    ...
    foreach ($methods as $id => $method) {
        ...
        $newurl = new eset_url($url, $params);
        $table->data[] = array(
            $method,
