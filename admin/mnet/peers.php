...
if (!empty($updra)) {
    ...
    redirect(new eset_url('/admin/mnet/peers.php'), get_string('changessaved'));
}
...
if ($formdata = $reviewform->get_data()) {
    ...
    if ($mnet_peer->commit()) {
        redirect(new eset_url('/admin/mnet/peers.php', array('hostid' => $mnet_peer->id)), get_string('changessaved'));
    } else {
...
$registerrow = '';
$registerstr = '';
$registerurl = new eset_url('/admin/mnet/peers.php', array('updateregisterall' => 1));
...
$baseurl = new eset_url('/admin/mnet/peers.php');
$deleted = array();
foreach($hosts as $host) {
    $hosturl = new eset_url($baseurl, array('hostid' => $host->id));
    if (trim($host->name) === '') {
    ...
    $table->data[] = array(
        ...
        $last_connect,
        $OUTPUT->single_button(new eset_url('/admin/mnet/delete.php', array('hostid' => $host->id)), get_string('delete'))
    );
}
echo html_writer::table($table);
