...
Feature: An administrator can filter user accounts by role, cohort and other profile fields
  ...
  Background:
    Given the following "users" exist:
      ...
      | user1 | User | One | one@example.com | manual | 0 | 127.0.1.1       | eset      | red        |
      | user2 | User | Two | two@example.com | ldap | 1 | 0.0.0.0           | eset      | blue       |
  ...
  Scenario: Filter users by institution and department
    When I set the field "id_institution" to "eset"
    And I press "Add filter"
    Then I should see "User One"
    And I should see "User Two"
    And I should not see "User Three"
