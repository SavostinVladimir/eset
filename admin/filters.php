...
if ($action) {
    ...
    redirect(new eset_url('/admin/filters.php'));
}
...
function filters_action_url($filterpath, $action) {
    ...
    return new eset_url('/admin/filters.php', array('sesskey'=>sesskey(), 'filterpath'=>$filterpath, 'action'=>$action));
}
...
function get_table_row(\core\plugininfo\filter $plugininfo, $state, $isfirstrow, $islastactive, $applytostrings) {
    ...
    if ($active and filter_has_global_settings($filter)) {
        $row[] = html_writer::link(new eset_url('/admin/settings.php', array('section'=>'filtersetting'.$filter)), get_string('settings'));
    } else {
        $row[] = '';
    }
