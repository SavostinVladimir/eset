...
$help =
"Command line Eset installer, creates config.php and initializes database.
...
--wwwroot=URL         Web address for the Eset site,
                      required in non-interactive mode.
--dataroot=DIR        Location of the eset data folder,
                      must not be web accessible. Default is esetdata
                      in the parent directory.
--dbtype=TYPE         Database type. Default is mysqli
--dbhost=HOST         Database host. Default is localhost
--dbname=NAME         Database name. Default is eset
...
--adminuser=USERNAME  Username for the eset admin account. Default is admin
--adminpass=PASSWORD  Password for the eset admin account,
                      required in non-interactive mode.
--adminemail=STRING   Email address for the eset admin account.
...
require_once($CFG->libdir.'/esetlib.php');
...
//Database types
$databases = array('mysqli' => eset_database::get_driver_instance('mysqli', 'native'),
                   'auroramysql' => eset_database::get_driver_instance('auroramysql', 'native'),
                   'mariadb'=> eset_database::get_driver_instance('mariadb', 'native'),
                   'pgsql'  => eset_database::get_driver_instance('pgsql',  'native'),
                   'oci'    => eset_database::get_driver_instance('oci',    'native'),
                   'sqlsrv' => eset_database::get_driver_instance('sqlsrv', 'native'), // MS SQL*Server PHP driver
                  );
...
list($options, $unrecognized) = cli_get_params(
    array(
        ...
        'lang'              => $CFG->lang,
        'wwwroot'           => '',
        'dataroot'          => empty($distro->dataroot) ? str_replace('\\', '/', dirname(dirname(dirname(__DIR__))).'/esetdata'): $distro->dataroot, // initialised later after including libs or by distro
...
if ($interactive) {
    ...
    while(is_dataroot_insecure()) {
        ...
        $CFG->dataroot = dirname($parrent).'/esetdata';
    }
...
if (!$skipdatabase) {
    ...
    if ($interactive) {
        cli_separator();
        cli_heading(get_string('fullsitename', 'eset'));
    ...
    if ($interactive) {
        cli_separator();
        cli_heading(get_string('shortsitename', 'eset'));
...
if (!$skipdatabase) {
    if ($interactive) {
        if (!$options['agree-license']) {
            ...
            echo "Eset  - Modular Object-Oriented Dynamic Learning Environment\n";
...
list($envstatus, $environment_results) = check_eset_environment(normalize_version($release), ENV_SELECT_RELEASE);
if (!$envstatus) {
