...
admin_externalpage_setup($pagename, '', null, new eset_url('/admin/repositoryinstance.php'));

$baseurl = new eset_url("/$CFG->admin/repositoryinstance.php", array('sesskey'=>sesskey()));

$parenturl = new eset_url("/$CFG->admin/repository.php", array(
    'sesskey'=>sesskey(),
    'action'=>'edit',
));
...
if (!empty($edit) || !empty($new)) {
    ...
    $continueurl = new eset_url($baseurl, array(
        'type' => $type,
        'delete' => $delete,
        'sure' => 'yes',
    ));
    $continueanddownloadurl = new eset_url($continueurl, array(
        'downloadcontents' => 1
    ));
