...
/**
 * Eset Mobile admin tool external functions tests.
 *
 * @package    tool_mobile
 * @category   external
...
class tool_mobile_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_public_config() {
        ...
        list($authinstructions, $notusedformat) = external_format_text($authinstructions, FORMAT_ESET, $context->id);
    ...
    public function test_get_autologin_key() {
        ...
        $this->setUser($user);
        $service = $DB->get_record('external_services', array('shortname' => ESET_OFFICIAL_MOBILE_SERVICE));
        ...
        core_useragent::instance(true, 'Mozilla/5.0 (Linux; Android 7.1.1; Moto G Play Build/NPIS26.48-43-2; wv) ' .
                'AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/71.0.3578.99 Mobile Safari/537.36 EsetMobile');
    ...
    public function test_call_external_functions() {
        ...
        $service = $webservicemanager->get_external_service_by_shortname(ESET_OFFICIAL_MOBILE_SERVICE);
        $token = external_generate_token_for_current_user($service);
