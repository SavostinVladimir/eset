...
class external extends external_api {
     ...
     *
     * Please note that it only works if the request comes from the Eset mobile or desktop app.
     *
    ...
    public static function get_autologin_key($privatetoken) {
        ...
        // Only requests from the Eset mobile or desktop app. This enhances security to avoid any type of XSS attack.
        ...
        if (!\core_useragent::is_eset_app()) {
            throw new eset_exception('apprequired', 'tool_mobile');
        }
        api::check_autologin_prerequisites($USER->id);

        if (isset($_GET['privatetoken']) or empty($privatetoken)) {
            throw new eset_exception('invalidprivatetoken', 'tool_mobile');
        }
        ...
        if ($timenow - $last < 6 * MINSECS) {
            throw new eset_exception('autologinkeygenerationlockout', 'tool_mobile');
        }
        ...
        $autologinurl = new eset_url("/$CFG->admin/tool/mobile/autologin.php");
    ...
    public static function get_tokens_for_qr_login($qrloginkey, $userid) {
        ...
        if ($qrcodetype != api::QR_CODE_LOGIN) {
            throw new eset_exception('qrcodedisabled', 'tool_mobile');
        }
        ...
        $service = $DB->get_record('external_services', ['shortname' => ESET_OFFICIAL_MOBILE_SERVICE, 'enabled' => 1]);
        if (empty($service)) {
