...
class api {
    ...
    /** @var string URL of the Eset Apps Portal */
    const ESET_APPS_PORTAL_URL = 'https://apps.eset.com';
    ...
    const DEFAULT_ANDROID_APP_ID = 'com.eset.esetmobile';
    /** @var string Default iOS app id */
    const DEFAULT_IOS_APP_ID = '633359593';

    /**
     * Returns a list of Eset plugins supporting the mobile app.
     *
    ...
    public static function get_public_config() {
        ...
        list($authinstructions, $notusedformat) = external_format_text($CFG->auth_instructions, FORMAT_ESET, $context->id);
        list($maintenancemessage, $notusedformat) = external_format_text($CFG->maintenance_message, FORMAT_ESET, $context->id);
        $settings = array(
            'wwwroot' => $CFG->wwwroot,
        ...
        $url = new eset_url("/$CFG->admin/tool/mobile/launch.php");
        $settings['launchurl'] = $url->out(false);

        // Check that we are receiving a eset_url object, themes can override get_logo_url and may return incorrect values.
        if (($logourl = $OUTPUT->get_logo_url()) && $logourl instanceof eset_url) {
            $settings['logourl'] = clean_param($logourl->out(false), PARAM_URL);
        }
        if (($compactlogourl = $OUTPUT->get_compact_logo_url()) && $compactlogourl instanceof eset_url) {
    ...
    public static function get_config($section) {
        ...
        $context = context_system::instance();
        $isadmin = has_capability('eset/site:config', $context);
        ...
        if (empty($section) or $section == 'gradessettings') {
            ...
            // The previous function may return eset_url instances or plain string URLs.
            if ($settings->mygradesurl instanceof eset_url) {
    ...
    public static function check_autologin_prerequisites($userid) {
        ...
        if (!$CFG->enablewebservices or !$CFG->enablemobilewebservice) {
            throw new eset_exception('enablewsdescription', 'webservice');
        }

        if (!is_https()) {
            throw new eset_exception('httpsrequired', 'tool_mobile');
        }

        if (has_capability('eset/site:config', context_system::instance(), $userid) or is_siteadmin($userid)) {
            throw new eset_exception('autologinnotallowedtoadmins', 'tool_mobile');
        }
    }

    /**
     * Creates an auto-login key for the current user, this key is restricted by time and ip address.
     * This key is used for automatically login the user in the site when the Eset app opens the site in a mobile browser.
     *
     ...
     * This key is used for automatically login the user in the site when the user scans a QR code in the Eset app.
     *
     * @return string the key
     * @since Eset 3.9
     */
    ...
    public static function get_potential_config_issues() {
        ...
        if ((int) $CFG->userquota === PHP_INT_MAX) {
            // In old Eset version was a text so was possible to have numeric values > PHP_INT_MAX.
    ...
    public static function generate_login_qrcode(stdClass $mobilesettings) {
        ...
        $urlscheme = !empty($mobilesettings->forcedurlscheme) ? $mobilesettings->forcedurlscheme : 'esetmobile';
    ...
    /**
     * Gets Eset app plan subscription information for the current site as it is returned by the Apps Portal.
     *
    ...
    public static function get_subscription_information() : ?array {
        ...
        // Eset Apps portal or the Sites registration database.
        $credentials = [];
        ...
        // Ask the Eset Apps Portal for the subscription information.
        $curl = new curl();
        $curl->setopt(array('CURLOPT_TIMEOUT' => 10, 'CURLOPT_CONNECTTIMEOUT' => 10));

        $serverurl = static::ESET_APPS_PORTAL_URL . "/lib/ajax/service-nologin.php";
        ...
        if ($curlerrno = $curl->get_errno()) {
            ...
            debugging("Unexpected response from the Eset Apps Portal server, CURL error number: $curlerrno");
            return null;
        } else if ($info['http_code'] != 200) {
            // Unexpected error from server.
            debugging('Unexpected response from the Eset Apps Portal server, HTTP code:' . $info['httpcode']);
            return null;
        } else if (!empty($wsresponse[0]['error'])) {
            // Unexpected error from Eset Apps Portal.
            debugging('Unexpected response from the Eset Apps Portal server:' . json_encode($wsresponse[0]));
            return null;
        } else if (empty($wsresponse[0]['data'])) {
            debugging('Unexpected response from the Eset Apps Portal server:' . json_encode($wsresponse));
            return null;
        }
