...
$urlscheme         = optional_param('urlscheme', 'esetmobile', PARAM_NOTAGS); // The URL scheme the app supports.
...
if (!preg_match('/^[a-zA-Z][a-zA-Z0-9-\+\.]*$/', $urlscheme)) {
    throw new eset_exception('Invalid parameter: the value of urlscheme isn\'t valid. ' .
...
if (!empty($oauthsso) && is_enabled_auth('oauth2')) {
    $wantsurl = new eset_url('/admin/tool/mobile/launch.php',
        array('service' => $serviceshortname, 'passport' => $passport, 'urlscheme' => $urlscheme, 'confirmed' => $confirmed));
    $oauthurl = new eset_url('/auth/oauth2/login.php',
...
if (empty($SESSION->justloggedin) and
    ...
    throw new eset_exception('pluginnotenabledorconfigured', 'tool_mobile');
}
...
$siteadmin = has_capability('eset/site:config', context_system::instance(), $USER->id);
...
if ($confirmed or $isios) {
    ...
    if ($confirmed) {
        ...
        echo $OUTPUT->single_button(new eset_url('/course/'), get_string('courses'));
        echo $OUTPUT->box_end();
    }
