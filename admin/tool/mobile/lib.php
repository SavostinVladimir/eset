...
/**
 * Generate the app download url to promote eset mobile.
 *
 * @return eset_url|void App download eset_url object or return if setuplink is not set.
 */
function tool_mobile_create_app_download_url() {
    ...
    $downloadurl = new eset_url($mobilesettings->setuplink);

    // Do not update the URL if it is a custom one (we may break it completely).
    if ($mobilesettings->setuplink != 'https://download.eset.org/mobile') {
        return $downloadurl;
    }
...
function tool_mobile_user_has_token($userid) {
    ...
    $sql = "SELECT 1
             ...
             WHERE t.externalserviceid = s.id
               AND s.enabled = 1
               AND s.shortname IN ('eset_mobile_app', 'local_mobile')
               AND t.userid = ?";
 ...
 *
 * Used add a section about the eset mobile app.
 *
...
function tool_mobile_standard_footer_html() {
    ...
    if (!empty($CFG->enablemobilewebservice) && $url = tool_mobile_create_app_download_url()) {
        $output .= html_writer::link($url, get_string('getesetonyourmobile', 'tool_mobile'));
    }
    return $output;
}
