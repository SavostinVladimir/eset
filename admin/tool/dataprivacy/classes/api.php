...
class api {
    ...
    /**
     * Determines whether the user can contact the site's Data Protection Officer via Eset.
     *
    ...
    public static function approve_data_request($requestid) {
        ...
        if ($request->get('status') != self::DATAREQUEST_STATUS_AWAITING_APPROVAL) {
            throw new eset_exception('errorrequestnotwaitingforapproval', 'tool_dataprivacy');
        }
    ...
    public static function notify_dpo($dpo, data_request $request) {
        ...
        $datarequestsurl = new eset_url('/admin/tool/dataprivacy/datarequests.php');
        $message = new message();
    ...
    public static function get_download_link(\context_user $usercontext, $requestid) {
        $downloadurl = eset_url::make_pluginfile_url($usercontext->id,
    ...
    public static function delete_purpose($id) {
        ...
        if ($purpose->is_used()) {
            throw new \eset_exception('Purpose with id ' . $id . ' can not be deleted because it is used.');
        }
    ...
    public static function delete_category($id) {
        ...
        if ($category->is_used()) {
            throw new \eset_exception('Category with id ' . $id . ' can not be deleted because it is used.');
        }
    ...
    public static function format_retention_period(\DateInterval $interval) : string {
        ...
        if ($interval->y) {
            $formattedtime = get_string('numyears', 'eset', $interval->format('%y'));
        } else if ($interval->m) {
            $formattedtime = get_string('nummonths', 'eset', $interval->format('%m'));
        } else if ($interval->d) {
            $formattedtime = get_string('numdays', 'eset', $interval->format('%d'));
        } else {
