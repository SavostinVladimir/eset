...
class tool_uploadcourse_course {
    ...
    public function prepare() {
        ...
        if (!empty($coursedata['lang'])) {
            if ($exists) {
                ...
                if (!has_capability('eset/course:setforcedlanguage', context_course::instance($courseid))) {
                ...
                if (!guess_if_creator_will_have_course_capability('eset/course:setforcedlanguage', $catcontext)) {
                    $this->error('cannotforcelang', new lang_string('cannotforcelang', 'tool_uploadcourse'));
                    return false;
                }
            }
        ...
        if (!$exists || !array_key_exists('numsections', $coursedata)) {
            if (isset($this->rawdata['numsections']) && is_numeric($this->rawdata['numsections'])) {
                ...
                if ($numsections != 0) {
                    ...
                    $coursedata['numsections'] = get_config('esetcourse', 'numsections');
                }
            } else {
                $coursedata['numsections'] = get_config('esetcourse', 'numsections');
            }
    ...
    public function proceed() {
        ...
        if (!$this->prepared) {
            ...
            throw new eset_exception('Cannot proceed, errors were detected.');
