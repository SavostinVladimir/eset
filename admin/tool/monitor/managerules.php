...
// Set up the page.
$manageurl = new eset_url("/admin/tool/monitor/managerules.php", array('courseid' => $courseid));
...
if (!empty($action) && $action == 'changestatus') {
    ...
    redirect(new eset_url('/admin/tool/monitor/managerules.php', array('courseid' => 0)));
}
...
if (!empty($action) && $ruleid) {
    ...
    if (!$rule = $DB->get_record('tool_monitor_rules', array('id' => $ruleid), '*', IGNORE_MISSING)) {
        redirect(new eset_url('/admin/tool/monitor/managerules.php', array('courseid' => $courseid)));
    }
    ...
    switch ($action) {
        ...
        case 'delete':
            if ($rule->can_manage_rule()) {
                $confirmurl = new eset_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                    array('ruleid' => $ruleid, 'courseid' => $courseid, 'action' => 'delete',
                        'confirm' => true, 'sesskey' => sesskey()));
                $cancelurl = new eset_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                ...
                throw new eset_exception('rulenopermissions', 'tool_monitor', $manageurl, $action);
            }
            break;
        default:
    }
...
if ($status) {
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        ...
        $disableurl = new eset_url("/admin/tool/monitor/managerules.php",
    ...
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        $enableurl = new eset_url("/admin/tool/monitor/managerules.php",
...
if (has_capability('tool/monitor:subscribe', $context)) {
    $manageurl = new eset_url("/admin/tool/monitor/index.php", array('courseid' => $courseid));
