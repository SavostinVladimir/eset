...
class renderable extends \table_sql implements \renderable {
     ...
     * @param \eset_url $url url where this table is displayed.
     * @param int $courseid course id.
     * @param int $perpage Number of rules to display per page.
     */
    public function __construct($uniqueid, \eset_url $url, $courseid = 0, $perpage = 100) {
    ...
    public function col_course(\tool_monitor\rule $rule) {
        ...
        if (empty($courseid)) {
            ...
            return \html_writer::link(new \eset_url('/course/view.php', array('id' => $courseid)), $coursename);
        }
    }
    ...
    public function col_manage(\tool_monitor\rule $rule) {
        ...
        if ($this->hassystemcap || ($rule->courseid != 0)) {
            $editurl = new \eset_url($CFG->wwwroot. '/admin/tool/monitor/edit.php', array('ruleid' => $rule->id,
        ...
        $copyurl = new \eset_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
        ...
        if ($this->hassystemcap || ($rule->courseid != 0)) {
            $deleteurl = new \eset_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php', array('ruleid' => $rule->id,
