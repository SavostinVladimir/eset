...
admin_externalpage_setup('tool_policy_managedocs', '', ['policyid' => $policyid, 'versionid' => $versionid],
    new eset_url('/admin/tool/policy/editpolicydoc.php'));
...
if ($makecurrent) {
    ...
    if ($confirm) {
        ...
        redirect(new eset_url('/admin/tool/policy/managedocs.php'));
    }
    ...
    echo $output->confirm(
        ...
        new eset_url($PAGE->url, ['makecurrent' => $makecurrent, 'confirm' => 1]),
        new eset_url('/admin/tool/policy/managedocs.php')
    );
    echo $output->footer();
    die();
}
...
if ($inactivate) {
    ...
    echo $output->confirm(
        ...
        new eset_url($PAGE->url, ['inactivate' => $inactivate, 'confirm' => 1]),
        new eset_url('/admin/tool/policy/managedocs.php')
    );
    echo $output->footer();
    die();
...
if ($delete) {
    ...
    echo $output->confirm(
        ...
        new eset_url($PAGE->url, ['delete' => $delete, 'confirm' => 1]),
        new eset_url('/admin/tool/policy/managedocs.php')
    );
    echo $output->footer();
    die();
