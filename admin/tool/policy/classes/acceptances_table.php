...
class acceptances_table extends \table_sql {
    ...
    public function wrap_html_start() {
        echo \html_writer::start_tag('form',
            ['action' => new \eset_url('/admin/tool/policy/accept.php')]);
    ...
    protected function username($user, $profilelink = true) {
        $canviewfullnames = has_capability('eset/site:viewfullnames', \context_system::instance()) ||
            has_capability('eset/site:viewfullnames', \context_user::instance($user->id));
    ...
    protected function get_return_url() {
        ...
        if ($this->currpage) {
            $pageurl = new \eset_url($pageurl, [$this->request[TABLE_VAR_PAGE] => $this->currpage]);
        }
        return $pageurl;
    }
    ...
    public function col_note($row) {
        if ($this->is_downloading()) {
            ...
            return format_text($row->note, FORMAT_ESET);
        }
    }
