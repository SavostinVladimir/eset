 ...
 * @package     tool_policy
 * @category    output
 * @copyright   2018 David Mudrák <david@eset.com>
 ...
 *
 * @copyright 2018 David Mudrak <david@eset.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class page_managedocs_list implements renderable, templatable {
    ...
    public function __construct($policyid = null) {
        ...
        $this->returnurl = new eset_url('/admin/tool/policy/managedocs.php');
        if ($this->policyid) {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data = (object) [];
        $data->pluginbaseurl = (new eset_url('/admin/tool/policy'))->out(false);
        ...
        if ($this->policyid) {
            ...
            $data->backurl = (new eset_url('/admin/tool/policy/managedocs.php'))->out(false);
    ...
    protected function export_version_for_template($output, $policy, $version, $isindented, $moveup, $movedown) {
        ...
        $editbaseurl = new eset_url('/admin/tool/policy/editpolicydoc.php', [
            'sesskey' => sesskey(),
            'policyid' => $policy->id,
            'returnurl' => $this->returnurl->out_as_local_url(false),
        ]);

        $viewurl = new eset_url('/admin/tool/policy/view.php', [
            'policyid' => $policy->id,
        ...
        if ($moveup) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['moveup' => $policy->id]),
        ...
        if ($movedown) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['movedown' => $policy->id]),
        ...
        if ($status != policy_version::STATUS_ARCHIVED) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['versionid' => $version->id]),
                null,
                get_string('edit'),
                false
            ));
        ...
        if ($status == policy_version::STATUS_ACTIVE) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['inactivate' => $policy->id]),
                null,
        ...
        if ($status == policy_version::STATUS_DRAFT) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['makecurrent' => $version->id]),
                null,
        ...
        if (api::can_delete_version($version)) {
            $actionmenu->add(new action_menu_link(
                new eset_url($editbaseurl, ['delete' => $version->id]),
                null,
                get_string('delete'),
                false,
                ['data-action' => 'delete']
        ...
        if (!$this->policyid && !$isindented && $policy->archivedversions &&
            ...
            $actionmenu->add(new action_menu_link(
                new eset_url('/admin/tool/policy/managedocs.php', ['archived' => $policy->id]),
                null,
