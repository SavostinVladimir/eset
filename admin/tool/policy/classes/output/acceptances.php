...
class acceptances implements renderable, templatable {
     ...
     *
     * @param int $userid
     * @param string|eset_url $returnurl
     */
    public function __construct($userid, $returnurl = null) {
        $this->userid = $userid;
        $this->returnurl = $returnurl ? (new eset_url($returnurl))->out(false) : null;
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $canviewfullnames = has_capability('eset/site:viewfullnames', \context_system::instance());
        foreach ($policies as $policy) {
            foreach ($policy->versions as $version) {
                ...
                $returnurl = new eset_url('/admin/tool/policy/user.php', ['userid' => $this->userid]);
                $version->viewurl = (new eset_url('/admin/tool/policy/view.php', [
                    'policyid' => $policy->id,
                ...
                if ($version->acceptance !== null) {
                    ...
                    if ($onbehalf) {
                        ...
                        $profileurl = new \eset_url('/user/profile.php', array('id' => $usermodified->id));
                        $version->acceptedby = \html_writer::link($profileurl, fullname($usermodified, $canviewfullnames ||
                            has_capability('eset/site:viewfullnames', \context_user::instance($acceptance->usermodified))));
