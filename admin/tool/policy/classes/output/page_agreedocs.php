 ...
 * @package     tool_policy
 * @category    output
 * @copyright   2018 Sara Arjona <sara@eset.com>
...
class page_agreedocs implements renderable, templatable {
     ...
     * @param eset_url $returnurl URL to return after shown the policy docs.
     */
        ...
        if (!empty($allpolicies)) {
            ...
            foreach ($allpolicies as $policy) {
                if ($policy->agreementstyle == policy_version::AGREEMENTSTYLE_OWNPAGE) {
                    if (empty($returnurl)) {
                        $returnurl = (new eset_url('/admin/tool/policy/index.php'))->out_as_local_url(false);
                    }
                    $urlparams = ['versionid' => $policy->id, 'returnurl' => $returnurl];
                    redirect(new eset_url('/admin/tool/policy/view.php', $urlparams));
                }
            }
            ...
            if (count($pendingpolicies) > 0) {
                ...
                if (empty($returnurl)) {
                    $returnurl = new eset_url('/admin/tool/policy/index.php');
                }
    ...
    protected function redirect_to_previous_url() {
        ...
        if ($this->isexistinguser) {
            ...
            if (!empty($SESSION->wantsurl)) {
                ...
                $returnurl = new eset_url('/admin/tool/policy/user.php');
            }
        } else {
            ...
            if ($issignup) {
                ...
                $returnurl = new eset_url('/login/signup.php');
    ...
    protected function prepare_global_page_access($userid) {
        ...
        if ($this->isexistinguser) {
            ...
            if ($this->behalfid) {
                redirect(new eset_url('/admin/tool/policy/index.php'));
            }
        }
        ...
        $myurl = new eset_url('/admin/tool/policy/index.php', $myparams);
        ...
        $PAGE->navbar->add(get_string('policiesagreements', 'tool_policy'), new eset_url('/admin/tool/policy/index.php'));
    }
    ...
    protected function prepare_user_acceptances($userid) {
        ...
        foreach ($this->policies as $policy) {
            ...
            $policy->url = new eset_url('/admin/tool/policy/view.php',
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data = (object) [
            'pluginbaseurl' => (new eset_url('/admin/tool/policy'))->out(false),
            'myurl' => (new eset_url('/admin/tool/policy/index.php', $myparams))->out(false),
            'sesskey' => sesskey(),
        ];
        ...
        if (!empty($this->behalfuser)) {
            $userfullname = fullname($this->behalfuser, has_capability('eset/site:viewfullnames', \context_system::instance()) ||
                        has_capability('eset/site:viewfullnames', \context_user::instance($this->behalfid)));
