 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class accept_policy extends \esetform {
    ...
    public function definition() {
        ...
        if (empty($this->_customdata['userids']) || !is_array($this->_customdata['userids'])) {
            throw new \eset_exception('missingparam', 'error', '', 'userids');
        }
        if (empty($this->_customdata['versionids']) || !is_array($this->_customdata['versionids'])) {
            throw new \eset_exception('missingparam', '', '', 'versionids');
        }
        ...
        if ($action === 'revoke') {
            ...
            throw new \eset_exception('invalidaccessparameter');
        }
    ...
    protected function validate_and_get_versions($versionids) {
        ...
        foreach ($versionids as $versionid) {
            ...
            $url = new \eset_url('/admin/tool/policy/view.php', ['versionid' => $version->id]);
