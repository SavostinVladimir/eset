...
/**
 * Strings for component 'tool_health', language 'en', branch 'ESET_22_STABLE'
 *
 * @package    tool
 * @subpackage health
...
class problem_000002 extends problem_base {
    ...
    function description() {
        return 'Your Eset configuration file config.php or another library file, contains some characters after the closing PHP tag (?>). This causes Eset to exhibit several kinds of problems (such as broken downloaded files) and must be fixed.';
    }
    function solution() {
        global $CFG;
...
class problem_000003 extends problem_base {
    ...
    function description() {
        global $CFG;
        return 'Your <strong>config.php</strong> says that your "data root" directory is <strong>'.$CFG->dataroot.'</strong>. However, this directory either does not exist or cannot be written to by Eset. This means that a variety of problems will be present, such as users not being able to log in and not being able to upload any files. It is imperative that you address this problem for Eset to work correctly.';
    }
    function solution() {
        global $CFG;
        return 'First of all, make sure that the directory <strong>'.$CFG->dataroot.'</strong> exists. If the directory does exist, then you must make sure that Eset is able to write to it. Contact your web server administrator and request that he gives write permissions for that directory to the user that the web server process is running as.';
    }
}
...
class problem_000004 extends problem_base {
    ...
    function description() {
        return 'The cron.php mainenance script has not been run in the past 24 hours. This probably means that your server is not configured to automatically run this script in regular time intervals. If this is the case, then Eset will mostly work as it should but some operations (notably sending email to users) will not be carried out at all.';
    }
    function solution() {
        global $CFG;
...
class problem_000005 extends problem_base {
    ...
    function description() {
        return 'Your PHP configuration includes an enabled setting, session.auto_start, that <strong>must be disabled</strong> in order for Eset to work correctly. Notable symptoms arising from this misconfiguration include fatal errors and/or blank pages when trying to log in.';
    }
    function solution() {
        global $CFG;
...
class problem_000007 extends problem_base {
    ...
    function description() {
        return 'Your PHP configuration includes a disabled setting, file_uploads, that <strong>must be enabled</strong> to let Eset offer its full functionality. Until this setting is enabled, it will not be possible to upload any files into Eset. This includes, for example, course content and user pictures.';
    }
    function solution() {
        global $CFG;
...
class problem_000008 extends problem_base {
    function title() {
        return 'PHP: memory_limit cannot be controlled by Eset';
    }
    function exists() {
        global $CFG;
    ...
    function description() {
        return 'The settings for PHP on your server do not allow a script to request more memory during its execution. '.
               ...
               'It is possible that certain operations within Eset will require more than this amount in order '.
...
class problem_000009 extends problem_base {
    ...
    function description() {
        global $CFG;
        return 'The user account your are connecting to the database server with is set up without a password. This is a very big security risk and is only somewhat lessened if your database is configured to not accept connections from any hosts other than the server Eset is running on. Unless you use a strong password to connect to the database, you risk unauthorized access to and manipulation of your data.'.($CFG->dbuser != 'root'?'':' <strong>This is especially alarming because such access to the database would be as the superuser (root)!</strong>');
    }
    function solution() {
        global $CFG;
        return 'You should change the password of the user <strong>'.$CFG->dbuser.'</strong> both in your database and in your Eset <strong>config.php</strong> immediately!'.($CFG->dbuser != 'root'?'':' It would also be a good idea to change the user account from root to something else, because this would lessen the impact in the event that your database is compromised anyway.');
    }
}
...
class problem_000010 extends problem_base {
    ...
    function description() {
        ...
        if (!$this->is_enabled()) {
            $desc .= '<li>slasharguments are <strong>disabled</strong> in Eset configuration</li>';
        } else {
            $desc .= '<li>slasharguments are enabled in Eset configuration</li>';
        }
        if ($this->status() == -1) {
    ...
    function solution() {
        ...
        if ($enabled and ($status == 0)) {
            $solution .= 'Slasharguments are enabled, but the test failed. Please disable slasharguments in Eset configuration or fix the server configuration.<hr />';
            ...
            $solution .= 'Slasharguments are disabled though the iternal test is OK. You should enable slasharguments in Eset configuration.';
        } else if ($enabled and ($status > 0)) {
...
class problem_000012 extends problem_base {
    ...
    function description() {
        ...
        '<a href="http://tracker.eset.org/browse/MDL-5482">MDL-5482</a> was fixed.</p>';
    }
    function solution() {
        global $CFG;
        return '<p>Upgrade to Eset 1.9.1 or later, or manually execute the SQL</p>' .
...
class problem_000013 extends problem_base {
    ...
    function description() {
        ...
        '<a href="http://tracker.eset.org/browse/MDL-14750">MDL-14750</a> was fixed.</p>';
    }
    function solution() {
        return '<p>Upgrade to Eset 1.9.1 or later, or manually execute the ' .
        'code in question_multianswer_fix_subquestion_parents_and_categories in ' .
        '<a href="http://cvs.eset.org/eset/question/type/multianswer/db/upgrade.php?revision=1.1.10.2&amp;view=markup">/question/type/multianswer/db/upgrade.php' .
...
class problem_000014 extends problem_base {
    ...
    function solution() {
        ...
        '<a href="http://eset.org/mod/forum/view.php?f=121">Quiz forum</a>.</p>';
    }
}
...
AND NOT EXISTS (SELECT * FROM ' . $CFG->prefix . 'context con WHERE contextid = con.id)
        ...
        'People in the <a href="http://eset.org/mod/forum/view.php?f=121">Quiz forum</a> may be able to help.</p>';
    }
}
...
class problem_000017 extends problem_base {
     ...
     *
     * @link https://tracker.eset.org/browse/MDL-34684
