...
class category_bin extends base_bin {
    ...
    public function store_item($course) {
        ...
        // For recycle bin we want to ensure that backup files are always stored in Eset file
        ...
        // own preferences and 100% separate from ESET_AUTOMATED.
        ...
        if (!isset($result['backup_destination'])) {
            throw new \eset_exception('Failed to backup activity prior to deletion.');
        }
        ...
        if (!$file->get_contenthash()) {
            throw new \eset_exception('Failed to backup activity prior to deletion (invalid file).');
        }
        ...
        if (!$fs->create_file_from_storedfile($filerecord, $file)) {
            ...
            throw new \eset_exception("Failed to copy backup file to recyclebin.");
        }
    ...
    public function restore_item($item) {
        ...
        if (empty($files)) {
            throw new \eset_exception('Invalid recycle bin item!');
        }

        if (count($files) > 1) {
            throw new \eset_exception('Too many files found!');
        }
        ...
        if (!$course) {
            throw new \eset_exception("Could not create course to restore into.");
        }
        ...
        if (!$controller->execute_precheck()) {
            ...
            if (!empty($results['errors'])) {
                ...
                echo $OUTPUT->continue_button(new \eset_url('/course/index.php', array('categoryid' => $this->_categoryid)));
                echo $OUTPUT->footer();
                exit();
            }
        }
