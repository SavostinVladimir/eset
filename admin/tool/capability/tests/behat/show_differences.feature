...
Feature: show only differences between roles for selected capabilities
  ...
  Scenario: Compare identical roles
    When I set the following fields to these values:
      | Capability: | eset/course:changefullname, eset/course:changeshortname, eset/course:changeidnumber, eset/course:changesummary |
  ...
  Scenario: Compare different roles
    ...
    And I should see "eset/course:changefullname" in the "comparisontable" "table"
    And I should see "eset/course:changeshortname" in the "comparisontable" "table"
    And I should not see "eset/course:changesummary" in the "comparisontable" "table"

  Scenario: Compare different roles but comparing capabilities that are equals on both
    When I set the following fields to these values:
      | Capability: | eset/course:changeidnumber, eset/course:changesummary |
      | Roles:      | Student, Studentdf                                        |
    And I set the field "Show differences only" to "1"
    And I click on "Get the overview" "button"
    Then I should see "There are no differences to show between selected roles in this context"
  ...
  Scenario: Compare all roles without selecting specific role
    When I set the following fields to these values:
      | Capability: | eset/course:changefullname, eset/site:config |
    And I set the field "Show differences only" to "1"
    And I click on "Get the overview" "button"
    Then I should not see "eset/site:config" in the "comparisontable" "table"
    And I should see "eset/course:changefullname" in the "comparisontable" "table"
