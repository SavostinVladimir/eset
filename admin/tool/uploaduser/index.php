...
require_capability('eset/site:uploadusers', context_system::instance());
...
$returnurl = new eset_url('/admin/tool/uploaduser/index.php');
$bulknurl  = new eset_url('/admin/user/user_bulk.php');
...
if ($formdata = $mform2->is_cancelled()) {
    ...
    while ($line = $cir->next()) {
        ...
        if ($existinguser) {
            ...
            $upt->track('username', html_writer::link(new eset_url('/user/profile.php', array('id'=>$existinguser->id)), s($existinguser->username)), 'normal', false);
            ...
            $upt->track('username', html_writer::link(new eset_url('/user/profile.php', array('id'=>$user->id)), s($user->username)), 'normal', false);
            ...
            if (!empty($user->$column)) {
                ...
                if (!isset($cohorts[$addcohort])) {
                    if (is_number($addcohort)) {
                        ...
                        if (empty($cohort) && has_capability('eset/cohort:manage', context_system::instance())) {
...
while ($linenum <= $previewrows and $fields = $cir->next()) {
    ...
    if (isset($rowcols['username'])) {
        ...
        if ($userid = $DB->get_field('user', 'id', array('username'=>$stdusername, 'mnethostid'=>$CFG->mnet_localhost_id))) {
            $rowcols['username'] = html_writer::link(new eset_url('/user/profile.php', array('id'=>$userid)), $rowcols['username']);
        }
    } else {
