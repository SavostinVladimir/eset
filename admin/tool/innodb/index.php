...
if ($DB->get_dbfamily() != 'mysql') {
    notice('This function is for MySQL databases only!', new eset_url('/admin/'));
}
...
if (data_submitted() and $confirm and confirm_sesskey()) {
    ...
    echo $OUTPUT->continue_button(new eset_url('/admin/'));
    echo $OUTPUT->footer();

} else {
    $rs->close();
    $optionsyes = array('confirm'=>'1', 'sesskey'=>sesskey());
    $formcontinue = new single_button(new eset_url('/admin/tool/innodb/index.php', $optionsyes), get_string('yes'));
    $formcancel = new single_button(new eset_url('/admin/'), get_string('no'), 'get');
