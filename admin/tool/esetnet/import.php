...
/**
 * This is the main endpoint which EsetNet instances POST to.
 *
 * EsetNet instances send the user agent to this endpoint via a form POST.
 * Then:
 * 1. The POSTed resource information is put in a session store for cross-request access.
 * 2. This page makes a GET request for admin/tool/esetnet/index.php (the import confirmation page).
...
if (!empty($_POST)) {
    ...
    redirect(new eset_url('/admin/tool/esetnet/index.php', ['id' => $importinfo->get_id()]));

}

// Invalid or missing POST data. Show an error to the user.
print_error('missinginvalidpostdata', 'tool_esetnet');
