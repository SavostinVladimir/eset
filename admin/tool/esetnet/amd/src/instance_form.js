 ...
 * plugin & Eset 3.9 functions. The 3.9 Functions have a base understanding that certain
 ...
 *
 * @module     tool_esetnet/instance_form
 * @package    tool_esetnet
 * @copyright  2020 Mathew May <mathew.solutions>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

define(['tool_esetnet/validator',
        'tool_esetnet/selectors',
        'core/loadingicon',
        'core/templates',
        'core/notification',
        'jquery'],
    ...
    /**
     * Given a user wishes to see the EsetNet profile url form transition them there.
     *
     * @method chooserNavigateToMnet
     * @param {HTMLElement} showEsetNet The chooser's area for ment
     ...
     */
    var chooserNavigateToMnet = function(showEsetNet, footerData, carousel, modal) {
        showEsetNet.innerHTML = '';

        // Add a spinner.
        var spinnerPromise = LoadingIcon.addIconToContainer(showEsetNet);
        ...
        $.when(
            ...
            transitionPromise
                ...
                Templates.replaceNodeContents(showEsetNet, footerData.customcarouseltemplate, '');
                return;
        }).catch(Notification.exception);

        // We apply our handlers in here to minimise plugin dependency in the Chooser.
        registerListenerEvents(showEsetNet);
        ...
        modal.setFooter(Templates.render('tool_esetnet/chooser_footer_close_mnet', {}));
    };

    /**
     * Given a user no longer wishes to see the EsetNet profile url form transition them from there.
     *
    ...
    var footerClickListener = function(e, footerData, modal) {
        if (e.target.matches(Selectors.action.showEsetNet) || e.target.closest(Selectors.action.showEsetNet)) {
            e.preventDefault();
            const carousel = $(modal.getBody()[0].querySelector(Selectors.region.carousel));
            const showEsetNet = carousel.find(Selectors.region.esetNet)[0];

            chooserNavigateToMnet(showEsetNet, footerData, carousel, modal);
        }
