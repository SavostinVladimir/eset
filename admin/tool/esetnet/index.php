...
/**
 * Landing page for all imports from EsetNet.
 *
...
use tool_esetnet\local\import_backup_helper;
...
if ($importinfo->get_config()->course) {
    require_capability('eset/course:manageactivities', context_course::instance($importinfo->get_config()->course));
}
...
if ($cancel) {
    if (!empty($importinfo->get_config()->course)) {
        $url = new \eset_url('/course/view.php', ['id' => $importinfo->get_config()->course]);
    } else {
        $url = new \eset_url('/');
    }
    redirect($url);
    ...
    if (strtolower($importinfo->get_resource()->get_extension()) == 'mbz') {
        ...
        $url = new \eset_url('/backup/restorefile.php', [
    ...
    if (!is_null($importinfo->get_config()->course) && !is_null($importinfo->get_config()->section)) {
        redirect(new \eset_url('/admin/tool/esetnet/options.php', ['id' => $id]));
    }

    if (is_null($importinfo->get_config()->course)) {
        redirect(new \eset_url('/admin/tool/esetnet/select.php', ['id' => $id]));
    }
}
...
$PAGE->set_title(get_string('addingaresource', 'tool_esetnet'));
$PAGE->set_heading(get_string('addingaresource', 'tool_esetnet'));
$url = new eset_url('/admin/tool/esetnet/index.php');
$PAGE->set_url($url);
...
echo $OUTPUT->header();
echo $renderer->render_from_template('tool_esetnet/import_confirmation', $context);
echo $OUTPUT->footer();
