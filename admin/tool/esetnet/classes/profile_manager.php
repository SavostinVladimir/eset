...
/**
 * Class for handling interaction with the esetnet profile.
 *
 * @package    tool_esetnet
 * @copyright  2020 Adrian Greeve <adrian@eset.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class profile_manager {
     ...
     * @return esetnet_user_profile or null.
     */
    public static function get_esetnet_user_profile(int $userid): ?esetnet_user_profile {
        global $CFG;
        // Check for official profile.
        if (self::official_profile_exists()) {
            $user = \core_user::get_user($userid, 'esetnetprofile');
            try {
                $userprofile = $user->esetnetprofile ? $user->esetnetprofile : '';
                return (isset($user)) ? new esetnet_user_profile(s($userprofile), $userid) : null;
        ...
        foreach ($profilefields as $key => $field) {
            if ($field->get_category_name() == self::get_category_name()
                ...
                try {
                    return new esetnet_user_profile(s($field->display_data()), $userid);
                } catch (\eset_exception $e) {
                    // If an exception is thrown, means there isn't a valid profile set. No need to log exception.
                    return null;
                }
    ...
    /**
     * Save the esetnet profile.
     *
     * @param esetnet_user_profile $esetnetprofile The esetnet profile to save.
     */
    public static function save_esetnet_user_profile(esetnet_user_profile $esetnetprofile): void {
        global $CFG, $DB;
        // Do some cursory checks first to see if saving is possible.
        if (self::official_profile_exists()) {
            // All good. Let's save.
            $user = \core_user::get_user($esetnetprofile->get_userid());
            $user->esetnetprofile = $esetnetprofile->get_profile_name();
        ...
        require_once($CFG->dirroot . '/user/profile/lib.php');
        \profile_save_custom_fields($esetnetprofile->get_userid(),
                [$fielddata->shortname => $esetnetprofile->get_profile_name()]);
    }

    /**
     * Checks to see if the required user profile fields and categories are in place. If not it regenerates them.
     *
     * @param  stdClass $fielddata The esetnet profile field.
     * @return stdClass The same esetnet profile field, with any necessary updates made.
     */
     ...
     *
     * @return stdClass the esetnet profile table object. False if no record found.
     */
    ...
    public static function official_profile_exists(): bool {
        ...
        if (isset($usertablecolumns['esetnetprofile'])) {
            return true;
        }
        return false;
    }
     ...
     *
     * @return string The category used to hold the eset net profile field.
     */
    public static function get_category_name(): string {
        return get_config('tool_esetnet', 'profile_category');
    }

    /**
     * Sets the a unique category to hold the eset net user profile.
     *
    ...
    private static function set_category_name(string $categoryname): string {
        ...
        do {
            ...
            if ($category > 0) {
                ...
                set_config('profile_category', $attemptname, 'tool_esetnet');
    ...
    public static function create_user_profile_category(): int {
        ...
        $data->name = self::set_category_name(get_string('pluginname', 'tool_esetnet'));
    ...
    /**
     * Sets a unique name to be used for the eset net profile.
     *
    ...
    private static function set_profile_field_name(string $fieldname): string {
        ...
        do {
            ...
            if ($profilefield > 0) {
                ...
                set_config('profile_field_name', $attemptname, 'tool_esetnet');
                $foundfieldname = true;
            }
    ...
    /**
     * Gets the unique profile field used to hold the eset net profile.
     *
     * @return string The profile field name being used on this site.
     */
    public static function get_profile_field_name(): string {
        return get_config('tool_esetnet', 'profile_field_name');
    }


    /**
     * Create a user profile field to hold the esetnet profile information.
     *
    ...
    public static function create_user_profile_text_field(int $categoryid): void {
        ...
        // Add our esetnet profile field.
        $profileclass = new \profile_define_text();
        $data = (object) [
            'shortname' => self::set_profile_field_name('mnetprofile'),
            'name' => get_string('mnetprofile', 'tool_esetnet'),
            'datatype' => 'text',
            'description' => get_string('mnetprofiledesc', 'tool_esetnet'),
            'descriptionformat' => 1,
    ...
    /**
     * Given our $esetnetprofile let's cURL the domains' WebFinger endpoint
     *
     * @param esetnet_user_profile $esetnetprofile The esetnet profile to get info from.
     * @return array [bool, text, raw]
     */
    public static function get_esetnet_profile_link(esetnet_user_profile $esetnetprofile): array {
        $domain = $esetnetprofile->get_domain();
        $username = $esetnetprofile->get_username();

        // Assumption: All EsetNet instance's will contain a WebFinger validation script.
        ...
        if (!$errno) {
            ...
            if ($info['http_code'] >= 400) {
                if ($info['http_code'] === 404) {
                    ...
                    return [
                        'result' => false,
                        'message' => get_string('profilevalidationfail', 'tool_esetnet'),
                    ];
                } else {
                    // There was some other error that was not a missing account.
                    return [
                        'result' => false,
                        'message' => get_string('profilevalidationerror', 'tool_esetnet'),
                    ];
                }
            }
            ...
            return [
                'result' => true,
                'message' => get_string('profilevalidationpass', 'tool_esetnet'),
