...
class remote_resource {
     ...
     * @throws \eset_exception if the file cannot be downloaded.
     */
    public function download_to_requestdir(): array {
        ...
        // In future, use a timeout (download and/or connection) controlled by a tool_esetnet setting.
        $downloadtimeout = 30;

        $result = $this->curl->download_one($this->url->get_value(), null, ['filepath' => $fullpathwithname,
            'timeout' => $downloadtimeout]);
        if ($result !== true) {
            throw new \eset_exception('errorduringdownload', 'tool_esetnet', '', $result);
        }
