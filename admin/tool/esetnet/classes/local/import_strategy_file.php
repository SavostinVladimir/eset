 ...
 *
 * The import_strategy_file objects contains the setup steps needed to prepare a resource for import as a file into Eset. This
...
class import_strategy_file implements import_strategy {
     ...
     * @throws \eset_exception if the file size means the upload limit is exceeded for the user.
     */
    public function import(remote_resource $resource, \stdClass $user, \stdClass $course, int $section): \stdClass {
        ...
        if ($this->size_exceeds_upload_limit($size, $useruploadlimit)) {
            throw new \eset_exception('uploadlimitexceeded', 'tool_esetnet', '', ['filesize' => $size,
        ...
        if ($this->size_exceeds_upload_limit($localsize, $useruploadlimit)) {
            throw new \eset_exception('uploadlimitexceeded', 'tool_esetnet', '', ['filesize' => $localsize,
                'uploadlimit' => $useruploadlimit]);
        }
