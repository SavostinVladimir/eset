...
class external extends external_api {
    ...
    public static function verify_webfinger(string $profileurl, int $course, int $section) {
        ...
        try {
            $mnetprofile = new esetnet_user_profile($params['profileurl'], $USER->id);
        } catch (\Exception $e) {
            return [
                'result' => false,
        ...
        $userlink = profile_manager::get_esetnet_profile_link($mnetprofile);

        // There were no problems verifying the account so lets store it.
        if ($userlink['result'] === true) {
            profile_manager::save_esetnet_user_profile($mnetprofile);
        ...
        if ($arrcourses = \core_course_category::search_courses(array('search' => $params['searchvalue']))) {
            foreach ($arrcourses as $course) {
                if (has_capability('eset/course:manageactivities', \context_course::instance($course->id))) {
                    ...
                    $viewurl = new \eset_url('/admin/tool/esetnet/options.php', $options);
