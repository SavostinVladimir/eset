...
/**
 * Page to select WHAT to do with a given resource stored on EsetNet.
 *
 ...
 */
use tool_esetnet\local\import_handler_registry;
use tool_esetnet\local\import_processor;
use tool_esetnet\local\import_info;
use tool_esetnet\local\import_strategy_file;
use tool_esetnet\local\import_strategy_link;
...
require_capability('eset/course:manageactivities', context_course::instance($config->course));
...
if ($cancel) {
    redirect(new eset_url('/course/view.php', ['id' => $config->course]));
}
...
if ($import && $module) {
    ...
    redirect(new eset_url('/course/view.php', ['id' => $course->id]));
}
...
$PAGE->set_url(new eset_url('/admin/tool/esetnet/options.php'));
...
echo $OUTPUT->header();
echo $PAGE->get_renderer('core')->render_from_template('tool_esetnet/import_options_select', $context);
echo $OUTPUT->footer();
