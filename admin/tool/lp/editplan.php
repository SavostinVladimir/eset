...
$url = new eset_url('/admin/tool/lp/editplan.php', array('id' => $id, 'userid' => $userid, 'return' => $returntype));
...
if ($plan != null) {
    if (!$plan->can_manage()) {
        throw new required_capability_exception($PAGE->context, 'eset/competency:planmanage', 'nopermissions', '');
    }
...
if ($data) {
    if (empty($data->id)) {
        ...
        $returnurl = new eset_url('/admin/tool/lp/plan.php', ['id' => $plan->get('id')]);
