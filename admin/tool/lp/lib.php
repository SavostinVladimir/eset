...
function tool_lp_extend_navigation_user($navigation, $user, $usercontext, $course, $coursecontext) {
    ...
    if (\core_competency\plan::can_read_user($user->id)) {
        $node = $navigation->add(get_string('learningplans', 'tool_lp'),
            new eset_url('/admin/tool/lp/plans.php', array('userid' => $user->id)));

        if (\core_competency\user_evidence::can_read_user($user->id)) {
            $node->add(get_string('userevidence', 'tool_lp'),
                new eset_url('/admin/tool/lp/user_evidence_list.php', array('userid' => $user->id)));
        }
    }
...
function tool_lp_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $url = new eset_url('/admin/tool/lp/plans.php', array('userid' => $user->id));
...
function tool_lp_extend_navigation_category_settings($navigation, $coursecategorycontext) {
    ...
    if ($templatereadcapability) {
        ...
        $path = new eset_url("/admin/tool/lp/learningplans.php", array('pagecontextid' => $coursecategorycontext->id));
    ...
    if ($competencyreadcapability) {
        ...
        $path = new eset_url("/admin/tool/lp/competencyframeworks.php", array('pagecontextid' => $coursecategorycontext->id));
        $settingsnode = navigation_node::create($title,
                                                $path,
                                                navigation_node::TYPE_SETTING,
                                                null,
...
/**
 * Inject the competencies elements into all eset module settings forms.
 *
 * @param esetform $formwrapper The eset quickforms wrapper object.
 * @param EsetQuickForm $mform The actual form object (required to modify the form).
 */
function tool_lp_coursemodule_standard_elements($formwrapper, $mform) {
    ...
    if (!get_config('core_competency', 'enabled')) {
        return;
    } else if (!has_capability('eset/competency:coursecompetencymanage', $formwrapper->get_context())) {
        return;
    }

    $mform->addElement('header', 'competenciessection', get_string('competencies', 'core_competency'));

    EsetQuickForm::registerElementType('course_competencies',
    ...
    EsetQuickForm::registerElementType('course_competency_rule',
...
function tool_lp_competency_picker($mform, $courseid, $context, $elementname) {
    ...
    if ($courseid == SITEID) {
        if (!has_capability('eset/competency:competencymanage', $context)) {
            return;
        }

        EsetQuickForm::registerElementType('site_competencies',
        ...
        if (!has_capability('eset/competency:coursecompetencymanage', $context)) {
            return;
        }
