...
class page_helper {
     ...
     *
     * @param  eset_url $url The current page.
     ...
     */
    public static function setup_for_course(eset_url $url, $course, $subtitle = '') {
        global $PAGE;
        ...
        $returnurl = new eset_url('/admin/tool/lp/coursecompetencies.php', array('courseid' => $course->id));
     ...
     *               - Return URL
     */
    public static function setup_for_template($pagecontextid, eset_url $url, $template = null, $subtitle = '',
                                              $returntype = null) {
        global $PAGE, $SITE;
        ...
        $templatesurl = new eset_url('/admin/tool/lp/learningplans.php', array('pagecontextid' => $pagecontextid));
        $templateurl = null;
        if ($template) {
            $templateurl = new eset_url('/admin/tool/lp/templatecompetencies.php', [
     ...
     */
    public static function setup_for_plan($userid, eset_url $url, $plan = null, $subtitle = '', $returntype = null) {
        global $PAGE, $USER;

        // Check that the user is a valid user.
        $user = core_user::get_user($userid);
        if (!$user || !core_user::is_real_user($userid)) {
            throw new \eset_exception('invaliduser', 'error');
        }

        $context = context_user::instance($user->id);

        $plansurl = new eset_url('/admin/tool/lp/plans.php', array('userid' => $userid));
        $planurl = null;
        if ($plan) {
            $planurl = new eset_url('/admin/tool/lp/plan.php', array('id' => $plan->get('id')));
        }
     ...
     *               - Return URL (main plan page)
     */
    public static function setup_for_user_evidence($userid, eset_url $url, $evidence = null, $subtitle = '', $returntype = null) {
        global $PAGE, $USER;
        ...
        $evidencelisturl = new eset_url('/admin/tool/lp/user_evidence_list.php', array('userid' => $userid));
        $evidenceurl = null;
        if ($evidence) {
            $evidenceurl = new eset_url('/admin/tool/lp/user_evidence.php', array('id' => $evidence->get('id')));
        }
    ...
    public static function setup_for_framework($id, $pagecontextid, $framework = null, $returntype = null) {
        ...
        $url = new eset_url("/admin/tool/lp/editcompetencyframework.php", array('id' => $id, 'pagecontextid' => $pagecontextid));
        if ($returntype) {
            $url->param('return', $returntype);
        }
        $frameworksurl = new eset_url('/admin/tool/lp/competencyframeworks.php', array('pagecontextid' => $pagecontextid));
        ...
        if (empty($id)) {
            ...
            if ($returntype == 'competencies') {
                $frameworksurl = new eset_url('/admin/tool/lp/competencies.php', array(
                    'pagecontextid' => $pagecontextid,
     ...
     */
    public static function setup_for_competency($pagecontextid, eset_url $url, $framework, $competency = null, $parent = null) {
        global $PAGE, $SITE;
        ...
        $frameworksurl = new eset_url('/admin/tool/lp/competencyframeworks.php', ['pagecontextid' => $pagecontextid]);
        $PAGE->navigation->override_active_url($frameworksurl);
        ...
        $returnurl = new eset_url('/admin/tool/lp/competencies.php', $returnurloptions);
