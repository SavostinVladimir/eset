...
require_capability('eset/competency:templatemanage', $context);

// We keep the original context in the URLs, so that we remain in the same context.
$url = new eset_url("/admin/tool/lp/edittemplate.php", [
    'id' => $id,
...
if ($data) {
    if (empty($data->id)) {
        ...
        $returnurl = new eset_url('/admin/tool/lp/templatecompetencies.php', [
