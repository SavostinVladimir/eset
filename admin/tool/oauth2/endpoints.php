...
if ($mform && $mform->is_cancelled()) {
    redirect(new eset_url('/admin/tool/oauth2/endpoints.php', ['issuerid' => $issuerid]));
} else if ($action == 'edit') {
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $continueurl = new eset_url('/admin/tool/oauth2/endpoints.php', $continueparams);
        $cancelurl = new eset_url('/admin/tool/oauth2/endpoints.php');
        echo $OUTPUT->header();
        $strparams = [ 'issuer' => s($issuer->get('name')), 'endpoint' => s($endpoint->get('name')) ];
    ...
    $addurl = new eset_url('/admin/tool/oauth2/endpoints.php', ['action' => 'edit', 'issuerid' => $issuerid]);
