...
class tool_log_setting_managestores extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        $url = new eset_url('/admin/tool/log/stores.php', array('sesskey' => sesskey()));
        $printed = array();
        foreach ($allstores as $store => $unused) {
            ...
            if (isset($enabled[$store])) {
                $aurl = new eset_url($url, array('action' => 'disable', 'store' => $store));
                ...
                if (isset($available[$store])) {
                    $aurl = new eset_url($url, array('action' => 'enable', 'store' => $store));
                    $hideshow = "<a href=\"$aurl\">";
            ...
            if ($isenabled) {
                if ($updowncount > 1) {
                    $aurl = new eset_url($url, array('action' => 'up', 'store' => $store));
                    $updown .= "<a href=\"$aurl\">";
                ...
                if ($updowncount < $storecount) {
                    $aurl = new eset_url($url, array('action' => 'down', 'store' => $store));
                    $updown .= "<a href=\"$aurl\">";
