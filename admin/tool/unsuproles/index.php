...
if ($action === 'delete') {
    ...
    //show confirmation
    echo $OUTPUT->header();
    $yesurl = new eset_url($PAGE->url, array('roleid'=>$roleid, 'contextlevel'=>$contextlevel, 'action'=>'delete', 'confirm'=>1, 'sesskey'=>sesskey()));
...
if (!$problems) {
    ...
    foreach ($problems as $problem) {
        ...
        $edit = array();
        $aurl = new eset_url('/admin/roles/define.php', array('roleid'=>$problem->roleid, 'action'=>'edit'));
        $edit[] = html_writer::link($aurl, $OUTPUT->pix_icon('t/edit', get_string('edit')));
        $aurl = new eset_url($PAGE->url, array('roleid'=>$problem->roleid, 'contextlevel'=>$problem->contextlevel, 'action'=>'delete'));
