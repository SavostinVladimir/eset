...
class tool_customlang_utils {
    ...
    protected static function dump_strings($lang, $component, $strings) {
        ...
        if ($lang !== clean_param($lang, PARAM_LANG)) {
            throw new eset_exception('Unable to dump local strings for non-installed language pack .'.s($lang));
        }
        ...
        if (!$filename = self::get_component_filename($component)) {
            throw new eset_exception('Unable to find the filename for the component '.s($component));
        }
        ...
        if (!$f = fopen($filepath, 'w')) {
            throw new eset_exception('Unable to write '.s($filepath));
        }
        fwrite($f, <<<EOF
<?php
...
EOF
     ...
     * @return string|boolean filename eg 'eset.php' or 'workshop.php', false if not found
     */
...
class tool_customlang_menu implements renderable {
     ...
     *
     * @return array (string)key => (object)[->(string)title ->(eset_url)url ->(string)method]
     */
     ...
     * @param eset_url $url action handler
     * @param string $method form method
     */
    public function add_item($key, $title, eset_url $url, $method) {
...
class tool_customlang_translator implements renderable {
    ...
    public function __construct(eset_url $handler, $lang, $filter, $currentpage = 0) {
        global $DB;
