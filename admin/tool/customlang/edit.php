...
admin_externalpage_setup('toolcustomlang', '', null,
    new eset_url('/admin/tool/customlang/edit.php', array('lng' => $lng)),
...
if ($filter->is_cancelled()) {
    ...
    redirect(new eset_url($PAGE->url, array('p'=>0)));
}
...
if ($translatorsubmitted) {
    ...
    if ($checkin === false) {
        $nexturl = new eset_url($PAGE->url, array('p' => $currentpage));
    } else {
        $nexturl = new eset_url('/admin/tool/customlang/index.php', array('action'=>'checkin', 'lng' => $lng, 'sesskey'=>sesskey()));
    }
