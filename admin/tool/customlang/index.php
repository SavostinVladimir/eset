...
if ($action === 'checkout') {
    ...
    echo $output->continue_button(new eset_url('/admin/tool/customlang/edit.php', array('lng' => $lng)), 'get');
    echo $output->footer();
    exit;
}
    ...
    if (!$confirm) {
        ...
        if ($numofmodified != 0) {
            ...
            echo $output->confirm(get_string('confirmcheckin', 'tool_customlang'),
                                  new eset_url($PAGE->url, array('action'=>'checkin', 'lng'=>$lng, 'confirm'=>1)),
                                  new eset_url($PAGE->url, array('lng'=>$lng)));
        } else {
            echo $output->heading(get_string('modifiedno', 'tool_customlang', $numofmodified), 3);
            echo $output->continue_button(new eset_url($PAGE->url, array('lng' => $lng)));
        }
        echo $output->footer();
        die();
...
if (has_capability('tool/customlang:edit', context_system::instance())) {
    $menu['checkout'] = array(
        ...
        'url'       => new eset_url($PAGE->url, array('action' => 'checkout', 'lng' => $lng)),
        'method'    => 'post',
    );
    if ($numofmodified != 0) {
        $menu['checkin'] = array(
            'title'     => get_string('checkin', 'tool_customlang'),
            'url'       => new eset_url($PAGE->url, array('action' => 'checkin', 'lng' => $lng)),
            'method'    => 'post',
        );
    }
}
