...
/**
 * Fetches language packages from download.eset.org server
 *
 * Language packages are available at https://download.eset.org/langpack/
...
if (empty($CFG->langotherroot)) {
    throw new eset_exception('missingcfglangotherroot', 'tool_langimport');
}
...
if (!empty($CFG->skiplangupgrade)) {
    ...
    echo $OUTPUT->single_button(new eset_url($PAGE->url, array('purgecaches' => 1)), get_string('purgestringcaches', 'tool_langimport'));
    echo $OUTPUT->footer();
    die;
}
...
$uninstallurl = new eset_url('/admin/tool/langimport/index.php', array('mode' => DELETION_OF_SELECTED_LANG));
$updateurl = null;
if ($remote) {
    $updateurl = new eset_url('/admin/tool/langimport/index.php', array('mode' => UPDATE_ALL_LANG));
}
$installurl = new eset_url('/admin/tool/langimport/index.php', array('mode' => INSTALLATION_OF_SELECTED_LANG));
...
$PAGE->requires->strings_for_js(array('uninstallconfirm', 'uninstall', 'selectlangs', 'noenglishuninstall'),
                                'tool_langimport');
$PAGE->requires->yui_module('eset-core-languninstallconfirm',
