...
$returnurl = new \eset_url('/admin/tool/analytics/index.php');
$params = array('id' => $id, 'action' => $action);
$url = new \eset_url('/admin/tool/analytics/model.php', $params);
...
switch ($action) {
    ...
    case 'evaluate':
        ...
        if ($model->is_static()) {
            throw new eset_exception('errornostaticevaluate', 'tool_analytics');
        }

        if ($onlycli) {
            throw new eset_exception('erroronlycli', 'tool_analytics');
        }
    ...
    case 'log':
        ...
        if ($model->is_static()) {
            throw new eset_exception('errornostaticlog', 'tool_analytics');
        }
    ...
    case 'exportdata':
        ...
        if ($model->is_static() || !$model->is_trained()) {
            throw new eset_exception('errornoexport', 'tool_analytics');
        }
