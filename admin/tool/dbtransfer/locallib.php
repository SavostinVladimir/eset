 ...
 * @param eset_database $sourcedb
 * @param eset_database $targetdb
 * @param progress_trace $feedback
 * @return void
 */
function tool_dbtransfer_transfer_database(eset_database $sourcedb, eset_database $targetdb, progress_trace $feedback = null) {
    core_php_time_limit::raise();
 ...
 * @param eset_database $target
 * @param progress_trace $feedback
 * @return void
 * @throws Exception on conversion error
 */
function tool_dbtransfer_rebuild_target_log_actions(eset_database $target, progress_trace $feedback = null) {
    global $DB, $CFG;
    ...
    $DB = $target;
    try {
        $DB->delete_records('log_display', array('component'=>'eset'));
...
function tool_dbtransfer_get_drivers() {
    ...
    $files = new RegexIterator(new DirectoryIterator("$CFG->libdir/dml"), '|^.*_eset_database\.php$|');
    $drivers = array();

    foreach ($files as $file) {
        $matches = null;
        preg_match('|^([a-z0-9]+)_([a-z]+)_eset_database\.php$|', $file->getFilename(), $matches);
        if (!$matches) {
            continue;
        }
        $dbtype = $matches[1];
        ...
        $targetdb = eset_database::get_driver_instance($dbtype, $dblibrary, false);
