...
if ($action) {
    ...
    if ($areaid) {
        ...
        if ($area === false) {
            throw new eset_exception('invalidrequest');
        }
    }
    ...
    if (in_array($action, ['delete', 'indexall', 'reindexall', 'deleteall'])) {
        ...
        if ($indexingenabled) {
            ...
            if (!optional_param('confirm', 0, PARAM_INT)) {
                ...
                $actionurl = new eset_url('/admin/searchareas.php', $actionparams);
                $cancelurl = new eset_url('/admin/searchareas.php');
                echo $OUTPUT->header();
                ...
                redirect(new eset_url('/admin/searchareas.php'));
            }
        }
        ...
        // Invalid action.
        throw new eset_exception('invalidaction');
    }
}
...
if (isset($searchmanager) && $indexingenabled) {
    ...
    $params = (object) [
        'url' => (new eset_url("/admin/settings.php?section=manageglobalsearch#admin-searchindexwhendisabled"))->out(false)
    ];
...
foreach ($searchareas as $area) {
    ...
    if ($area->is_enabled()) {
        $columns[] = $OUTPUT->action_icon(admin_searcharea_action_url('disable', $areaid),
            new pix_icon('t/hide', get_string('disable'), 'eset', array('title' => '', 'class' => 'iconsmall')),
        ...
        if ($areasconfig && $indexingenabled) {
            ...
            if ($area->supports_get_document_recordset()) {
                $actions[] = $OUTPUT->pix_icon('i/reload', '') . html_writer::link(
                        new eset_url('searchreindex.php', ['areaid' => $areaid]),
        ...
        $columns[] = $OUTPUT->action_icon(admin_searcharea_action_url('enable', $areaid),
            new pix_icon('t/show', get_string('enable'), 'eset', array('title' => '', 'class' => 'iconsmall')),
...
function admin_searcharea_action_url($action, $areaid = false) {
    ...
    return new eset_url('/admin/searchareas.php', $params);
}
