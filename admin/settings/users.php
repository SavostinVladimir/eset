...
if ($hassiteconfig
 or has_capability('eset/user:create', $systemcontext)
 or has_capability('eset/user:update', $systemcontext)
 or has_capability('eset/user:delete', $systemcontext)
 or has_capability('eset/role:manage', $systemcontext)
 or has_capability('eset/role:assign', $systemcontext)
 or has_capability('eset/cohort:manage', $systemcontext)
 or has_capability('eset/cohort:view', $systemcontext)) { // Speedup for non-admins, add all caps used on this page.


    // Stuff under the "accounts" subcategory.
    $ADMIN->add('accounts', new admin_externalpage('editusers', new lang_string('userlist','admin'), "$CFG->wwwroot/$CFG->admin/user.php", array('eset/user:update', 'eset/user:delete')));
    $ADMIN->add('accounts', new admin_externalpage('userbulk', new lang_string('userbulk','admin'), "$CFG->wwwroot/$CFG->admin/user/user_bulk.php", array('eset/user:update', 'eset/user:delete')));
    $ADMIN->add('accounts', new admin_externalpage('addnewuser', new lang_string('addnewuser'), "$CFG->wwwroot/user/editadvanced.php?id=-1", 'eset/user:create'));
    ...
    $ADMIN->add('accounts', new admin_externalpage('profilefields', new lang_string('profilefields','admin'), "$CFG->wwwroot/user/profile/index.php", 'eset/site:config'));
    $ADMIN->add('accounts', new admin_externalpage('cohorts', new lang_string('cohorts', 'cohort'), $CFG->wwwroot . '/cohort/index.php', array('eset/cohort:manage', 'eset/cohort:view')));
    ...
    if ($ADMIN->fulltree) {
        ...
        $temp->add(new admin_setting_configmultiselect('hiddenuserfields', new lang_string('hiddenuserfields', 'admin'),
                   new lang_string('confighiddenuserfields', 'admin'), array(),
                       array('description' => new lang_string('description'),
                             ...
                             'esetnetprofile' => new lang_string('esetnetprofile', 'user'),
        ...
        // with eset/site:viewuseridentity).
    ...
    $ADMIN->add('roles', new admin_externalpage('defineroles', new lang_string('defineroles', 'role'), "$CFG->wwwroot/$CFG->admin/roles/manage.php", 'eset/role:manage'));
    $ADMIN->add('roles', new admin_externalpage('assignroles', new lang_string('assignglobalroles', 'role'), "$CFG->wwwroot/$CFG->admin/roles/assign.php?contextid=".$systemcontext->id, 'eset/role:assign'));
    $ADMIN->add('roles', new admin_externalpage('checkpermissions', new lang_string('checkglobalpermissions', 'role'), "$CFG->wwwroot/$CFG->admin/roles/check.php?contextid=".$systemcontext->id, array('eset/role:assign', 'eset/role:safeoverride', 'eset/role:override', 'eset/role:manage')));
...
if ($hassiteconfig) {
    ...
    $url = new eset_url('/admin/settings.php?section=supportcontact');
    $url = $url->out();
