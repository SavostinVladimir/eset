...
switch ($action) {
    case 'delete':
        ...
        if (!$confirmed) {
            ...
            $formcontinue = new single_button(new eset_url($baseurl, $optionsyes), get_string('yes'));
            $formcancel = new single_button(new eset_url($baseurl), get_string('no'), 'get');
...
echo $OUTPUT->single_button(new eset_url($defineurl, array('action' => 'add')), get_string('addrole', 'core_role'), 'get');
