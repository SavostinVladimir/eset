...
if (!in_array($action, array('add', 'export', 'edit', 'reset', 'view'))) {
    throw new eset_exception('invalidaccess');
}
if ($action != 'add') {
...
$baseurl = new eset_url('/admin/roles/define.php', array('action'=>$action, 'roleid'=>$roleid));
$manageurl = new eset_url('/admin/roles/manage.php');
if ($return === 'manage') {
    $returnurl = $manageurl;
} else {
    $returnurl = new eset_url('/admin/roles/define.php', array('action'=>'view', 'roleid'=>$roleid));;
}

admin_externalpage_setup('defineroles', '', array('action' => $action, 'roleid' => $roleid),
    new eset_url('/admin/roles/define.php'));

// Check access permissions.
$systemcontext = context_system::instance();
require_capability('eset/role:manage', $systemcontext);
...
if (optional_param('savechanges', false, PARAM_BOOL) && confirm_sesskey() && $definitiontable->is_submission_valid()) {
    ...
    if ($action === 'add') {
        redirect(new eset_url('/admin/roles/define.php', array('action'=>'view', 'roleid'=>$definitiontable->get_role_id())));
    } else {
        redirect($returnurl);
    }
}
...
if ($action === 'view') {
    ...
    $url = new eset_url('/admin/roles/define.php', array('action'=>'edit', 'roleid'=>$roleid, 'return'=>'define'));
    echo $OUTPUT->single_button(new eset_url($url), get_string('edit'));
    $url = new eset_url('/admin/roles/define.php', array('action'=>'reset', 'roleid'=>$roleid, 'return'=>'define'));
    echo $OUTPUT->single_button(new eset_url($url), get_string('resetrole', 'core_role'));
    $url = new eset_url('/admin/roles/define.php', array('action'=>'export', 'roleid'=>$roleid));
    echo $OUTPUT->single_button(new eset_url($url), get_string('export', 'core_role'));
