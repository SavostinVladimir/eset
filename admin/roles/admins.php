...
if (optional_param('add', false, PARAM_BOOL) and confirm_sesskey()) {
    if ($userstoadd = $potentialadmisselector->get_selected_users()) {
        ...
        echo $OUTPUT->header();
        $yesurl = new eset_url('/admin/roles/admins.php', array('confirmadd'=>$user->id, 'sesskey'=>sesskey()));
    ...
    if ($userstoremove = $admisselector->get_selected_users()) {
        ...
        if ($USER->id == $user->id) {
            ...
            echo $OUTPUT->header();
            $yesurl = new eset_url('/admin/roles/admins.php', array('confirmdel'=>$user->id, 'sesskey'=>sesskey()));
