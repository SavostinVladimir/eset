...
$url = new eset_url('/admin/roles/check.php', array('contextid' => $contextid));
...
if (!has_any_capability(array('eset/role:assign', 'eset/role:safeoverride', 'eset/role:override', 'eset/role:manage'), $context)) {
...
// Admins (people with eset/role:manage) can run this report for any user.
...
if (!is_null($reportuser)) {
    ...
    if (!empty($roleassignments)) {
        ...
        foreach ($roleassignments as $ra) {
            ...
            if (has_capability('eset/role:manage', $systemcontext)) {
                $rolename = html_writer::link(new eset_url('/admin/roles/define.php',
...
if ($context->contextlevel > CONTEXT_USER) {
    ...
    if ($returnurl) {
        $backurl = new eset_url($returnurl);
    } else {
