...
$url = new eset_url('/admin/roles/permissions.php', array('contextid' => $contextid));
...
require_capability('eset/role:review', $context);

navigation_node::override_active_url($url);
$pageurl = new eset_url($url);
if ($returnurl) {
...
$allowoverrides     = has_capability('eset/role:override', $context);
$allowsafeoverrides = has_capability('eset/role:safeoverride', $context);
    ...
    if (isset($overridableroles[$roleid]) && ($prevent || $unprohibit)) {
        ...
        if ($prevent) {
            if ($confirm && data_submitted() && confirm_sesskey()) {
                ...
                $continueurl = new eset_url($PAGE->url,
...
if ($overridableroles) {
    $overrideurl = new eset_url('/admin/roles/override.php', array('contextid' => $context->id));
