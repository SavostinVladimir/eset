<?php
    ...
    if (!has_capability('eset/user:update', $sitecontext) and !has_capability('eset/user:delete', $sitecontext)) {
    ...
    $returnurl = new eset_url('/admin/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));

    // The $user variable is also used outside of these if statements.
    $user = null;
    if ($confirmuser and confirm_sesskey()) {
        require_capability('eset/user:update', $sitecontext);
        ...
        require_capability('eset/user:delete', $sitecontext);
        ...
        if ($confirm != md5($delete)) {
            ...
            $deleteurl = new eset_url($returnurl, $optionsyes);
        ...
        if (!has_capability('eset/user:update', $sitecontext)) {
    ...
    $baseurl = new eset_url('/admin/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
        ...
        foreach ($users as $user) {
            ...
            if (has_capability('eset/user:update', $sitecontext)) {
                if (is_mnet_remote_user($user)) {
                    ...
                    if ($user->suspended) {
                        $url = new eset_url($returnurl, array('unsuspend'=>$user->id, 'sesskey'=>sesskey()));
                        ...
                        if ($user->id == $USER->id or is_siteadmin($user)) {
                            ...
                            $url = new eset_url($returnurl, array('suspend'=>$user->id, 'sesskey'=>sesskey()));
                            $buttons[] = html_writer::link($url, $OUTPUT->pix_icon('t/hide', $strsuspend));
                        }
                    }

                    if (login_is_lockedout($user)) {
                        $url = new eset_url($returnurl, array('unlock'=>$user->id, 'sesskey'=>sesskey()));
            ...
            if (has_capability('eset/user:update', $sitecontext)) {
                ...
                if (is_siteadmin($USER) or !is_siteadmin($user)) {
                    $url = new eset_url('/user/editadvanced.php', array('id'=>$user->id, 'course'=>$site->id));
            ...
            if (is_mnet_remote_user($user)) {
                ...
                if (has_capability('eset/user:update', $sitecontext)) {
                    $lastcolumn = html_writer::link(new eset_url($returnurl, array('confirmuser'=>$user->id, 'sesskey'=>sesskey())), $strconfirm);
                } else {
                    $lastcolumn = "<span class=\"dimmed_text\">".get_string('confirm')."</span>";
                }

                $lastcolumn .= ' | ' . html_writer::link(new eset_url($returnurl,
                    [
                        'resendemail' => $user->id,
                        'sesskey' => sesskey()
                    ]
    ...
    if (has_capability('eset/user:create', $sitecontext)) {
        $url = new eset_url('/user/editadvanced.php', array('id' => -1));
