...
$env       = optional_param('env', 'filepicker', PARAM_ALPHA);  // Opened in editor or esetform
    ...
    case 'download':
        ...
        if ((is_array($accepted_types) and in_array('*', $accepted_types)) or $accepted_types == '*') {
            ...
            if (!in_array(mimeinfo('type', $saveas_filename), $mimetypes)) {
                throw new eset_exception('invalidfiletype', 'repository', '', get_mimetype_description(array('filename' => $saveas_filename)));
            }
        }

        // We have two special repository type need to deal with
        // local and recent plugins don't added new files to eset, just add new records to database
        ...
        if (empty($sourcekey)) {
            throw new eset_exception('missingsourcekey', 'repository');
        }

        // Check that the sourcekey matches.
        if (sha1($source . repository::get_secret_key() . sesskey()) !== $sourcekey) {
            throw new eset_exception('sourcekeymismatch', 'repository');
        }
        ...
        if ($allowexternallink and $linkexternal === 'yes' and ($repo->supported_returntypes() & FILE_EXTERNAL)) {
            ...
            if ($repo->has_eset_files()) {
                $file = repository::get_eset_file($reference);
            ...
            if ($usefilereference || $usecontrolledlink) {
                if ($repo->has_eset_files()) {
                    $sourcefile = repository::get_eset_file($reference);
                ...
                if (repository::draftfile_exists($itemid, $saveas_path, $saveas_filename)) {
                    ...
                    $event['newfile']->url = eset_url::make_draftfile_url($itemid, $saveas_path, $unused_filename)->out();

                    $event['existingfile'] = new stdClass;
                    $event['existingfile']->filepath = $saveas_path;
                    $event['existingfile']->filename = $saveas_filename;
                    $event['existingfile']->url      = eset_url::make_draftfile_url($itemid, $saveas_path, $saveas_filename)->out();
                } else {

                    $storedfile = $fs->create_file_from_reference($record, $repo_id, $reference);
                    $event = array(
                        'url'=>eset_url::make_draftfile_url($storedfile->get_itemid(), $storedfile->get_filepath(), $storedfile->get_filename())->out(),
                ...
                die;
            } else if ($repo->has_eset_files()) {
                // Some repository plugins (local, user, coursefiles, recent) are hosting eset
                // internal files, we cannot use get_file method, so we use copy_to_area method

                // If the eset file is an alias we copy this alias, otherwise we copy the file
