...
/**
 * This file contains classes used to manage the repository plugins in Eset
 *
 * @since Eset 2.0
        ...
        if (!$existingtype) {
            ...
            if (empty($instanceoptionnames)) {
                ...
                if (empty($this->_options['pluginname'])) {
                    // when eset trying to install some repo plugin automatically
 ...
 *
 * To create repository plugin, see: {@link http://docs.eset.org/dev/Repository_plugins}
...
abstract class repository implements cacheable_object {
    /**
     * Timeout in seconds for downloading the external file into eset
     * @deprecated since Eset 2.7, please use $CFG->repositorygetfiletimeout instead
     */
    const GETFILE_TIMEOUT = 30;

    /**
     * Timeout in seconds for syncronising the external file size
     * @deprecated since Eset 2.7, please use $CFG->repositorysyncfiletimeout instead
     */
    const SYNCFILE_TIMEOUT = 1;

    /**
     * Timeout in seconds for downloading an image file from external repository during syncronisation
     * @deprecated since Eset 2.7, please use $CFG->repositorysyncimagetimeout instead
     */
    ...
    /**
     * Parses the eset file reference and returns an instance of stored_file
     *
     * @param string $reference reference to the eset internal file as retruned by
     *        {@link repository::get_file_reference()} or {@link file_storage::pack_reference()}
     * @return stored_file|null
     */
    public static function get_eset_file($reference) {
    ...
    /**
     * This function is used to copy a eset file to draft area.
     *
    ...
    public function copy_to_area($source, $filerecord, $maxbytes = -1, $areamaxbytes = FILE_AREA_MAX_BYTES_UNLIMITED) {
        ...
        if ($this->has_eset_files() == false) {
            throw new coding_exception('Only repository used to browse eset files can use repository::copy_to_area()');
        }
        ...
        $stored_file = self::get_eset_file($source);
        ...
        if (repository::draftfile_exists($draftitemid, $new_filepath, $new_filename)) {
            ...
            $event['newfile']->url = eset_url::make_draftfile_url($draftitemid, $new_filepath, $unused_filename)->out();
            $event['existingfile'] = new stdClass;
            $event['existingfile']->filepath = $new_filepath;
            $event['existingfile']->filename = $new_filename;
            $event['existingfile']->url = eset_url::make_draftfile_url($draftitemid, $new_filepath, $new_filename)->out();
            return $event;
        } else {
            $fs->create_file_from_storedfile($filerecord, $stored_file);
            $info = array();
            ...
            $info['url'] = eset_url::make_draftfile_url($draftitemid, $new_filepath, $new_filename)->out();
        ...
        foreach ($records as $record) {
            ...
            if (!$args['onlyvisible'] || ($repository->is_visible() && !$repository->disabled)) {
                ...
                if ($record->repositorytype == 'coursefiles') {
                    ...
                    $capability = $capability && has_capability('eset/course:managefiles', $current_context);
                }
    ...
    public function get_reference_details($reference, $filestatus = 0) {
        if ($this->has_eset_files()) {
            ...
            if (empty($fileinfo)) {
                if ($filestatus == 666) {
                    if (is_siteadmin() || ($context && has_capability('eset/course:managefiles', $context))) {
     ...
     * Invoked at ESET/repository/repository_ajax.php
     *
     ...
     * - copy the file to the eset system account
     ...
     * (either copy or a reference) is already in eset and it is being picked
    ...
    public static function move_to_filepool($thefile, $record) {
        ...
        if (repository::draftfile_exists($record->itemid, $record->filepath, $record->filename)) {
            ...
            $event['newfile']->url = eset_url::make_draftfile_url($draftitemid, $record->filepath, $new_filename)->out();

            $event['existingfile'] = new stdClass;
            $event['existingfile']->filepath = $record->filepath;
            $event['existingfile']->filename = $old_filename;
            $event['existingfile']->url      = eset_url::make_draftfile_url($draftitemid, $record->filepath, $old_filename)->out();
            return $event;
        }
        if ($file = $fs->create_file_from_pathname($record, $thefile)) {
            ...
            return array(
                'url'=>eset_url::make_draftfile_url($file->get_itemid(), $file->get_filepath(), $file->get_filename())->out(),
                'id'=>$file->get_itemid(),
    ...
    public static function display_instances_list($context, $typename = null) {
        ...
        if ($admin) {
            $baseurl = new eset_url('/'.$CFG->admin.'/repositoryinstance.php', array('sesskey'=>sesskey()));
            $output .= $OUTPUT->heading(get_string('siteinstances', 'repository'));
        } else {
            $baseurl = new eset_url('/repository/manage_instances.php', array('contextid'=>$context->id, 'sesskey'=>sesskey()));
        }
    ...
    /**
     * Does this repository used to browse eset files?
     *
     * @return bool
     */
    public function has_eset_files() {
        return false;
    }
     ...
     * to eset. This function is not called for eset repositories, the function
    ...
    public function get_file($url, $filename = '') {
        ...
        if ($result !== true) {
            throw new eset_exception('errorwhiledownload', 'repository', '', $result);
        }
        return array('path'=>$path, 'url'=>$url);
    }

    /**
     * Downloads the file from external repository and saves it in eset filepool.
    ...
    public function import_external_file_contents(stored_file $file, $maxbytes = 0) {
        if (!$file->is_external_file()) {
            ...
            return;
        } else if ($this->has_eset_files()) {
            // files that are references to local files are already in eset filepool
            ...
            if (!$file->get_status() && $contentexists) {
                // we already have the content in eset filepool and it was synchronised recently.
                ...
                try {
                    ...
                    if (isset($fileinfo['path'])) {
                        ...
                        throw new eset_exception('errorwhiledownload', 'repository', '', '');
                    }
                } catch (Exception $e) {
                    if ($contentexists) {
    ...
    public final function can_be_edited_by_user() {
        ...
        if ($repocontext->contextlevel == CONTEXT_USER && $repocontext->instanceid != $USER->id) {
            ...
            return false;
        } else if ($repocontext->contextlevel == CONTEXT_MODULE && !has_capability('eset/course:update', $repocontext)) {
            // We need to have permissions on the course to edit the instance.
            return false;
        } else if ($repocontext->contextlevel == CONTEXT_SYSTEM && !has_capability('eset/site:config', $repocontext)) {
     ...
     * contains_private_data() to repositories which are external to Eset and shouldn't be accessed
     * to by the users having the capability to 'login as' someone else. For instance, the repository
     * 'Private files' is not considered as private because it's part of Eset.
     *
     ...
     *
     * See details on {@link http://docs.eset.org/dev/Repository_plugins}
     *
    ...
    /**
     * For oauth like external authentication, when external repository direct user back to eset,
    ...
    /**
     * function which is run when the type is created (eset administrator add the plugin)
     *
    ...
    /**
     * Edit/Create Admin Settings Eset form
     *
     * @param esetform $mform Eset form (passed by reference)
    ...
    public static function type_config_form($mform, $classname = 'repository') {
        ...
        if (empty($instnaceoptions)) {
            ...
            // it can be empty, then eset will look for instance name from language string
    ...
    /**
     * Validate Admin Settings Eset form
     *
     * @static
     * @param esetform $mform Eset form (passed by reference)
    ...
    /**
     * Edit/Create Instance Settings Eset form
     *
     * @param esetform $mform Eset form (passed by reference)
     */
     ...
     *
     * @param esetform $mform eset form
     ...
     * @throws eset_exception if for any reason file can not be updated (file does not exist, target already exists, etc.)
     */
    public static function update_draftfile($draftid, $filepath, $filename, $updatedata) {
        ...
        if (!$file = $fs->get_file($usercontext->id, 'user', 'draft', $draftid, $filepath, $filename)) {
            if ($filename === '.') {
                throw new eset_exception('foldernotfound', 'repository');
            } else {
                throw new eset_exception('filenotfound', 'error');
            }
        }
        if (!$file->is_directory()) {
            ...
            if ($updatedata['filepath'] !== $filepath || $updatedata['filename'] !== $filename) {
                ...
                if ($fs->file_exists($usercontext->id, 'user', 'draft', $draftid, $updatedata['filepath'], $updatedata['filename'])) {
                    throw new eset_exception('fileexists', 'repository');
                }
     ...
     * @deprecated since Eset 2.6 MDL-42016 - please do not use this function any more.
     */
     ...
     * repositories (see {@link repository::has_eset_files()}), references to
     * internal files are updated immediately when source is modified.
     *
     * Referenced files may optionally keep their content in Eset filepool (for
     ...
     *
     * @deprecated since Eset 3.2, 3.1.1, 3.0.5
     * @return bool
     */
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class repository_exception extends eset_exception {
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class repository_instance_form extends esetform {
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class repository_type_form extends esetform {
    /** @var stdClass repository instance */
    protected $instance;
     ...
     * Validate eset form data
     *
     * @param array $data eset form data
     * @param array $files
     * @return array errors
     */
    public function validation($data, $files) {
...
function repository_download_selected_files($context, string $component, string $filearea, $itemid, array $files) {
    ...
    if ($newfile =
        ...
        $return = new stdClass();
        $return->fileurl = eset_url::make_draftfile_url($newdraftitemid, '/', $zippedfile)->out();
