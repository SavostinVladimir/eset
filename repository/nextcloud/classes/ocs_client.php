...
class ocs_client extends rest {
     ...
     * @throws \eset_exception when trying to construct a eseturl
     */
    public function __construct(client $oauthclient) {
        ...
        $this->ocsendpoint = new \eset_url($ocsendpoint);
    ...
    public function get_api_functions() {
        return [
            'create_share' => [
                ...
                'method' => 'post',
                'args' => [
                    'path' => PARAM_TEXT, // Could be PARAM_PATH, we really don't want to enforce a Eset understanding of paths.
                    'shareType' => PARAM_INT,
    ...
    /**
     * In POST requests, Eset's REST API assumes that params are
     ...
     * Curl assumes the content type to be `multipart/form-data` then, but the Eset REST API tries to put
