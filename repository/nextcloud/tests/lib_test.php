...
class repository_nextcloud_lib_testcase extends advanced_testcase {
    ...
    protected function setUp() {
        ...
        $instance = $generator->create_instance([
            ...
            'controlledlinkfoldername' => 'Esetfiles',
...
JSON;
    ...
    public function test_logout() {
        ...
        $mock->expects($this->exactly(2))->method('get_login_url')->will($this->returnValue(new eset_url('url')));
    ...
    public function test_send_file_errors() {
        ...
        $mock->expects($this->once())->method('is_dir')->with('Esetfiles')->willReturn(false);
        ...
        $mock->expects($this->once())->method('mkcol')->with(
            $webdavprefix . 'Esetfiles')->willReturn(400);
        $this->expectException(\repository_nextcloud\request_exception::class);
...
XML;
        ...
        // Checks that setting for foldername are used.
        $mock->expects($this->once())->method('is_dir')->with('Esetfiles')->willReturn(true);
        ...
        $this->set_private_property($mock, 'ocsclient');
        $this->repo->expects($this->once())->method('move_file_to_folder')->with('/merged (3).txt', 'Esetfiles',
