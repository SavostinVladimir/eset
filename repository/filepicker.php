 ...
 * @copyright  2009 Dongsheng Cai <dongsheng@eset.com>
...
$home_url = new eset_url('/repository/draftfiles_manager.php', $params);

$params['savepath'] = $savepath;
$params['repo_id'] = $repo_id;
$url = new eset_url("/repository/filepicker.php", $params);
...
case 'search':
    ...
    try {
        ...
        $purl = new eset_url($url, array('search_paging' => 1, 'action' => 'search', 'repo_id' => $repo_id));
        ...
        if (!empty($list['upload'])) {
            ...
            if (!empty($list['path'])) {
                foreach ($list['path'] as $p) {
                    ...
                    $pathurl = new eset_url($url, array(
                        'p'=>$p['path'],
                        'action'=>'list',
                        'draftpath'=>$draftpath,
                        'savepath'=>$savepath
            ...
            if (!empty($list['page'])) {
                ...
                $pagingurl = new eset_url("/repository/filepicker.php?action=list&itemid=$itemid&ctx_id=$contextid&repo_id=$repo_id&course=$courseid&sesskey=".  sesskey());
