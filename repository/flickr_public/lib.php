...
class repository_flickr_public extends repository {
    ...
    public function license4eset ($license_id) {
        $license = array(
            '0' => 'allrightsreserved',
            '1' => 'cc-nc-sa',
            '2' => 'cc-nc',
    ...
    public function get_file($photoid, $file = '') {
        ...
        if (!@getimagesize($source)) {
            throw new eset_exception('cannotdownload', 'repository');
        }
        ...
        if (!empty($this->usewatermarks)) {
            $img = new eset_image($path);
            $img->watermark($copyright, array(10,10), array('ttf'=>true, 'fontsize'=>12))->saveas($path);
        }

        return array('path'=>$path, 'author'=>$info['owner']['realname'], 'license'=>$this->license4eset($info['license']));
    }

    /**
     * Add Instance settings input to Eset form
     * @param object $mform
     */
    ...
    /**
     * is run when eset administrator add the plugin
     */
