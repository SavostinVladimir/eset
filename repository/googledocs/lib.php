...
class repository_googledocs extends repository {
     ...
     *
     * @param eset_url $overrideurl - Use this url instead of the repo callback.
    ...
    public function print_login_popup($attr = null) {
        ...
        $url = new eset_url($client->get_login_url());
                ...
                if (isset($gfile->fileExtension)) {
                    ...
                    switch ($type){
                        case 'document':
                            ...
                            if ($ext === 'rtf') {
                                // Eset user 'text/rtf' as the MIME type for RTF files.
    ...
    public function get_file($reference, $filename = '') {
        ...
        if ($source->exportformat == 'download') {
            $params = ['alt' => 'media'];
            $sourceurl = new eset_url($base . '/files/' . $source->id, $params);
            $source = $sourceurl->out(false);
        } else {
            $params = ['mimeType' => $source->exportformat];
            $sourceurl = new eset_url($base . '/files/' . $source->id . '/export', $params);
            $types = get_mimetypes_array();
    ...
    public function send_file($storedfile, $lifetime=null , $filter=0, $forcedownload=false, array $options = null) {
        ...
        if (empty($options['offline']) && !empty($info) && $info->is_writable() && !empty($source->usesystem)) {
            ...
            $url = eset_url::make_pluginfile_url($storedfile->get_contextid(),
                                                   $storedfile->get_component(),
                                                   $storedfile->get_filearea(),
                                                   $storedfile->get_itemid(),
                                                   $storedfile->get_filepath(),
    ...
    protected function prevent_writers_from_sharing_file(\repository_googledocs\rest $client, $fileid) {
        // We don't want anyone but Eset to change the sharing settings.
        $params = [
            'fileid' => $fileid
        ];
        $update = [
    ...
    public function reference_file_selected($reference, $context, $component, $filearea, $itemid) {
        ...
        // Copy the file so we get a snapshot file owned by Eset.
