...
if ($confirm) {
    ...
    if (repository_onedrive::import_skydrive_files()) {
        ...
        redirect(new eset_url('/admin/repository.php'), $mesg, null, \core\output\notification::NOTIFY_SUCCESS);
    } else {
        $mesg = get_string('skydrivefilesnotimported', 'repository_onedrive');
        redirect(new eset_url('/admin/repository.php'), $mesg, null, \core\output\notification::NOTIFY_ERROR);
    }
} else {
    $continueurl = new eset_url('/repository/onedrive/importskydrive.php', ['confirm' => true]);
    $cancelurl = new eset_url('/admin/repository.php');
    echo $OUTPUT->header();
