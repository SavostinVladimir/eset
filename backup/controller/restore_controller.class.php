...
class restore_controller extends base_controller {
    ...
    protected $format; // Format of backup (eset, imscc)
    ...
    public function __construct($tempdir, $courseid, $interactive, $mode, $userid, $target,
        ...
        // If format is not eset2, set to conversion needed
        if ($this->format !== backup::FORMAT_ESET) {
            $this->set_status(backup::STATUS_REQUIRE_CONV);

        // Else, format is eset2, load plan, apply security and set status based on interactivity
        } else {
            // Load plan
            $this->load_plan();
...
// checksumable interface methods
    ...
    /**
     * Converts from current format to backup::ESET format
     */
    public function convert() {
        ...
        if (!convert_helper::to_eset2_format($this->get_tempdir(), $this->format, $this->get_logger())) {
        ...
        $this->format = backup::FORMAT_ESET;
...
// Protected API starts here
    ...
    protected function load_plan() {
        // First of all, we need to introspect the eset_backup.xml file
