 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class copy_form extends \esetform {

    /**
     * Build form for the course copy settings.
     *
     * {@inheritDoc}
     * @see \esetform::definition()
     */
    public function definition() {
        ...
        if (!empty($copies)) {
            $progresslink = new \eset_url('/backup/copyprogress.php?', array('id' => $course->id));
        ...
        $requiredcapabilities = array(
            'eset/restore:createuser', 'eset/backup:userinfo', 'eset/restore:userinfo'
        );
        ...
        if (!empty($roles) && has_capability('eset/restore:createuser', $coursecontext)) {
            $rolearray = array();
