...
/**
 * This file contains the generic esetform bridge for the backup user interface
...
/**
 * Base esetform bridge
 *
 * Ahhh the mighty esetform bridge! Strong enough to take the weight of 682 full
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class base_esetform extends esetform {
        ...
        if ($setting->get_visibility() == backup_setting::VISIBLE) {
            ...
            switch ($setting->get_status()) {
                case backup_setting::LOCKED_BY_PERMISSION:
                    $icon = ' '.$OUTPUT->pix_icon('i/permissionlock', get_string('lockedbypermission', 'backup'), 'eset',
                            array('class' => 'smallicon lockedicon permissionlock'));
                    break;
                case backup_setting::LOCKED_BY_CONFIG:
                    $icon = ' '.$OUTPUT->pix_icon('i/configlock', get_string('lockedbyconfig', 'backup'), 'eset',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                case backup_setting::LOCKED_BY_HIERARCHY:
                    $icon = ' '.$OUTPUT->pix_icon('i/hierarchylock', get_string('lockedbyhierarchy', 'backup'), 'eset',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                default:
                    $icon = '';
     ...
     *
     * @param string $elementname
     * @return HTML_QuickForm_input|EsetQuickForm_group
     */
    ...
    public function display() {
        ...
        $PAGE->requires->yui_module('eset-backup-backupselectall', 'M.core_backup.backupselectall',
                array($modnames));
        $PAGE->requires->strings_for_js(array('select', 'all', 'none'), 'eset');
