 ...
 *  - initialise_stage_form : To get a backup_esetform instance for the stage
 *
 * @package   core_backup
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class backup_ui_stage_initial extends backup_ui_stage {
     ...
     * @param base_esetform $m
     * @return int The number of changes
     */
    public function process(base_esetform $m = null) {
    ...
    /**
     * Initialises the backup_esetform instance for this stage
     *
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        switch ($type) {
            case 'activity':
                ...
                $restorerul = new eset_url('/backup/restorefile.php', array('contextid' => $modcontext->id));
                break;
            case 'course':
            default:
                $coursecontext = context_course::instance($courseid);
                $restorerul = new eset_url('/backup/restorefile.php', array('contextid' => $coursecontext->id));
        }
