YUI.add('eset-backup-backupselectall', function (Y, NAME) {

/**
 * Adds select all/none links to the top of the backup/restore/import schema page.
 *
 * @module eset-backup-backupselectall
 */
...
M.core_backup.backupselectall = function(modnames) {
    ...
    var html_generator = function(classname, idtype, heading, extra) {
        ...
        return '<div class="' + classname + '">' +
                    '<div class="fitem fitem_fcheckbox backup_selector">' +
                        ...
                        '<div class="felement">' +
                            '<a id="backup-all-' + idtype + '" href="#">' + M.util.get_string('all', 'eset') + '</a> / ' +
                            '<a id="backup-none-' + idtype + '" href="#">' + M.util.get_string('none', 'eset') + '</a>' +
                            extra +
                        '</div>' +
                    '</div>' +
                '</div>';
    ...
    var html = html_generator('include_setting section_level', 'included', M.util.get_string('select', 'eset'),
            ' (<a id="backup-bytype" href="#">' + M.util.get_string('showtypes', 'backup') + '</a>)');
    if (withuserdata) {
        html += html_generator('normal_setting', 'userdata', M.util.get_string('select', 'eset'));
    }
