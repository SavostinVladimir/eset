...
require_once($CFG->dirroot . '/backup/eset2/backup_plan_builder.class.php');
...
class core_backup_renderer extends plugin_renderer_base {
    ...
    public function backup_details($details, $nextstageurl) {
        ...
        $html .= $this->backup_detail_pair(get_string('esetversion', 'backup'),
                html_writer::tag('span', $details->eset_release, array('class' => 'eset_release')).
                html_writer::tag('span', '['.$details->eset_version.']', array('class' => 'eset_version sub-detail')));
        $html .= $this->backup_detail_pair(get_string('backupversion', 'backup'),
                html_writer::tag('span', $details->backup_release, array('class' => 'eset_release')).
                html_writer::tag('span', '['.$details->backup_version.']', array('class' => 'eset_version sub-detail')));
     ...
     *
     * @param eset_url $nextstageurl URL to send user to
     ...
     * @return string HTML code to display
     */
    public function backup_details_unknown(eset_url $nextstageurl) {
     ...
     * @return string
     */
    public function course_selector(eset_url $nextstageurl, $wholecourse = true, restore_category_search $categories = null,
     ...
     * @return string
     */
    public function import_course_selector(eset_url $nextstageurl, import_course_search $courses = null) {
    ...
    public function render_backup_files_viewer(backup_files_viewer $viewer) {
        ...
        foreach ($files as $file) {
            ...
            $fileurl = eset_url::make_pluginfile_url(
                $file->get_contextid(),
                $file->get_component(),
                $file->get_filearea(),
                null,
            ...
            $restoreurl = new eset_url('/backup/restorefile.php', $params);
            ...
            if ($params['filearea'] == 'automated') {
                if (!has_capability('eset/restore:viewautomatedfilearea', $viewer->currentcontext)) {
                    $restorelink = '';
                }
        ...
        if ($canmanagebackups) {
            $html .= $this->output->single_button(
                new eset_url('/backup/backupfilesedit.php', array(
     ...
     *
     * @param int $userid The Eset user id.
     * @param \context $context The Eset context for these restores.
    ...
    public function copy_progress_viewer(int $userid, int $courseid): string {
        ...
        foreach ($copies as $copy) {
            $sourceurl = new \eset_url('/course/view.php', array('id' => $copy->sourceid));
