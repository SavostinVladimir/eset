 ...
 *  - initialise_stage_form : To get a restore_esetform instance for the stage
 *
 * @package   core_backup
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
    ...
    public function get_progress_bar() {
        ...
        while ($stage > 0) {
            ...
            if ($stage < $currentstage && $currentstage < restore_ui::STAGE_COMPLETE) {
                ...
                // $item['link'] = new eset_url($PAGE->url, array('restore' => $this->get_restoreid(), 'stage' => $stage));
            }
...
class restore_ui_stage_confirm extends restore_ui_independent_stage implements file_progress {
    ...
    protected function extract_file_to_dir($source) {
        ...
        $fb = get_file_packer('application/vnd.eset.backup');
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        $prevstageurl = new eset_url('/backup/restorefile.php', array('contextid' => $this->contextid));
        $nextstageurl = new eset_url('/backup/restore.php', array(
        ...
        } else if ($format !== backup::FORMAT_ESET) {
...
class restore_ui_stage_destination extends restore_ui_independent_stage {
    ...
    public function __construct($contextid) {
        ...
        $url = new eset_url($PAGE->url, array(
            'filepath' => $this->filepath,
            'contextid' => $this->contextid,
            'stage' => restore_ui::STAGE_DESTINATION));
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        if ($format === backup::FORMAT_ESET) {
            // Standard Eset 2 format, let use get the type of the backup.
        ...
        if ($context->contextlevel == CONTEXT_COURSE and has_capability('eset/restore:restorecourse', $context)) {
...
class restore_ui_stage_settings extends restore_ui_stage {
     ...
     *
     * @param base_esetform $form
     * @return bool|int
     */
    public function process(base_esetform $form = null) {
     ...
     *
     * @return backup_esetform|base_esetform|restore_settings_form
     * @throws coding_exception
     */
...
class restore_ui_stage_process extends restore_ui_stage {
    ...
    public function process(base_esetform $form = null) {
        if (optional_param('cancel', false, PARAM_BOOL)) {
            redirect(new eset_url('/course/view.php', array('id' => $this->get_ui()->get_controller()->get_courseid())));
        }
...
class restore_ui_stage_complete extends restore_ui_stage_process {
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        $html .= $renderer->continue_button(new eset_url('/course/view.php', array(
