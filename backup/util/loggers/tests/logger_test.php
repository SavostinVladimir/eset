...
class backup_logger_testcase extends basic_testcase {
    ...
    function test_error_log_logger() {
        ...
        $message = 'This log exists because you have run Eset unit tests: Ignore it';
    ...
    function test_database_logger() {
        // Instantiate with date and level output (and with specs from the global eset "log" table so checks will pass
        $now = time();
        $datecol = 'time';
        $levelcol = 'action';
        $messagecol = 'info';
    ...
    function test_file_logger() {
        ...
        if (!check_dir_exists(dirname($file), true, true)) {
            throw new eset_exception('error_creating_temp_dir', 'error', dirname($file));
        }
