    ...
    public static function check_security($backup_controller, $apply) {
        ...
        switch ($mode) {
            case backup::MODE_IMPORT:
                if (!has_capability('eset/backup:backuptargetimport', $coursectx, $userid)) {
                    $a = new stdclass();
                    $a->userid = $userid;
                    $a->courseid = $courseid;
                    $a->capability = 'eset/backup:backuptargetimport';
        ...
        // Now, enforce 'eset/backup:userinfo' to 'users' setting, applying changes if allowed,
        ...
        $hasusercap   = has_capability('eset/backup:userinfo', $coursectx, $userid);
        ...
        if (!$hasusercap) { // If user has not the capability
            ...
            if (!$apply && $prevvalue) { // Cannot apply changes and the value is set, throw exception
                ...
                $a->setting = 'users';
                $a->value = $prevvalue;
                $a->capability = 'eset/backup:userinfo';
        ...
        // Now, enforce 'eset/backup:anonymise' to 'anonymise' setting, applying changes if allowed,
        // else throwing exception
        ...
        $hasanoncap  = has_capability('eset/backup:anonymise', $coursectx, $userid);
        ...
        if (!$hasanoncap) { // If user has not the capability
            ...
            if (!$apply && $prevvalue) { // Cannot apply changes and the value is set, throw exception
                ...
                $a->setting = 'anonymize';
                $a->value = $prevvalue;
                $a->capability = 'eset/backup:anonymise';
                throw new backup_controller_exception('backup_setting_value_wrong_for_capability', $a);
        ...
        if ($mode != backup::MODE_IMPORT) {
            $hasconfigcap = has_capability('eset/backup:configure', $coursectx, $userid);
            if (!$hasconfigcap) {
