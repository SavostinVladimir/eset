    ...
    public static function check_security($restore_controller, $apply) {
        ...
        switch ($type) {
            case backup::TYPE_1COURSE :
                $typecapstocheck['eset/restore:restorecourse'] = $coursectx;
                break;
            case backup::TYPE_1SECTION :
                $typecapstocheck['eset/restore:restoresection'] = $coursectx;
                break;
            case backup::TYPE_1ACTIVITY :
                $typecapstocheck['eset/restore:restoreactivity'] = $coursectx;
                break;
            default :
        ...
        switch ($mode) {
            case backup::MODE_IMPORT:
                if (!has_capability('eset/restore:restoretargetimport', $coursectx, $userid)) {
                    $a = new stdclass();
                    $a->userid = $userid;
                    $a->courseid = $courseid;
                    $a->capability = 'eset/restore:restoretargetimport';
        ...
        // Now, enforce 'eset/restore:userinfo' to 'users' setting, applying changes if allowed,
        ...
        $hasusercap   = has_capability('eset/restore:userinfo', $coursectx, $userid);
        ...
        if (!$hasusercap && $prevvalue) { // If user has not the capability and setting is enabled
            ...
            if (!$apply) { // Cannot apply changes, throw exception
                ...
                $a->setting = 'users';
                $a->value = $prevvalue;
                $a->capability = 'eset/restore:userinfo';
        ...
        if ($mode != backup::MODE_IMPORT) {
            $hasconfigcap = has_capability('eset/restore:configure', $coursectx, $userid);
            if (!$hasconfigcap) {
        ...
        if ($type == backup::TYPE_1COURSE) {
            ...
            $hasrolldatescap = has_capability('eset/restore:rolldates', $coursectx, $userid);
            if (!$hasrolldatescap) {
            ...
            $haschangefullnamecap = has_capability('eset/course:changefullname', $coursectx, $userid);
            ...
            // the setting so that they cannot change it.
            $haschangeshortnamecap = has_capability('eset/course:changeshortname', $coursectx, $userid);
            ...
            $hasupdatecap = has_capability('eset/course:update', $coursectx, $userid);
            if (!$hasupdatecap) {
            ...
            // the setting so that they cannot change it.
            $hasmanageenrolcap = has_capability('eset/course:enrolconfig', $coursectx, $userid);
