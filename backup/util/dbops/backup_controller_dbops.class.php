...
abstract class backup_controller_dbops extends backup_dbops {
     ...
     * Used by get_eset_backup_information()
     */
    ...
    /**
     * Get details information for main eset_backup.xml file, extracting it from
     ...
     */
    public static function get_eset_backup_information($backupid,
            \core\progress\base $progress = null) {

        // Start tracking progress if required (for load_controller).
        if ($progress) {
            $progress->start_progress('get_eset_backup_information', 2);
        }
        ...
        if ($progress) {
            $progress->start_progress('get_eset_backup_information', count($tasks));
            $done = 1;
        }
     ...
     * files from the eset file storage system.
     *
