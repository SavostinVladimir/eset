...
abstract class restore_dbops {
     ...
     *
     * Rule2: The 'eset/question:managecategory' and 'eset/question:add' capabilities will be
    ...
    public static function prechek_precheck_qbanks_by_level($restoreid, $courseid, $userid, $samesite, $contextlevel) {
        ...
        preg_match('/(\d{8})/', $restoreinfo->eset_release, $matches);
        ...
        // 8) Check if backup is from Eset >= 3.5 and error if more than one top-level category in the context.
        ...
        foreach ($contexts as $contextid => $contextlevel) {
            ...
            if ($targetcontext = self::restore_find_best_target_context($categories, $courseid, $contextlevel)) {
                $canmanagecategory = has_capability('eset/question:managecategory', $targetcontext, $userid);
                $canadd            = has_capability('eset/question:add', $targetcontext, $userid);
            }
            // 1) Iterate over each qcat in the context, matching by stamp for the found target context
            ...
            // 8) Check if backup is made on Eset >= 3.5 and there are more than one top-level category in the context.
    ...
    public static function send_files_to_pool($basepath, $restoreid, $component, $filearea,
        ...
        $fs = get_file_storage();         // Get eset file storage
        ...
        foreach ($rs as $rec) {
            ...
            if (empty($file->repositoryid)) {
                ...
                if ($includesfiles) {
                    ...
                    // This backup does not include the files - they should be available in eset filestorage already.
    ...
    public static function create_included_users($basepath, $restoreid, $userid,
        ...
        foreach ($rs as $recuser) {
            ...
            if (empty($user->password)) { // Only if restore comes without password
                ...
                // If Eset is responsible for storing/validating pwd and reset functionality is available, mark
    ...
    */
    ...
    public static function precheck_included_users($restoreid, $courseid, $userid, $samesite,
        ...
        // to 'eset/restore:createuser' and 'eset/restore:userinfo'
        // and also observe $CFG->disableusercreationonrestore
        $cancreateuser = false;
        if (has_capability('eset/restore:createuser', $context, $userid) and
            has_capability('eset/restore:userinfo', $context, $userid) and
