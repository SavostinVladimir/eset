...
abstract class backup_cron_automated_helper {
     ...
     * @return eset_recordset The recordset of matching courses.
     */
    ...
    public static function launch_automated_backup($course, $starttime, $userid) {
        ...
        $bc = new backup_controller(backup::TYPE_1COURSE, $course->id, backup::FORMAT_ESET, backup::INTERACTIVE_NO,
        ...
        try {
            ...
            // This is a feature to prevent esetdata to be filled up and break a site when the admin misconfigured
    ...
    protected static function remove_excess_backups_from_directory($course, $now) {
        ...
        $filename = backup::FORMAT_ESET . '-' . backup::TYPE_1COURSE . '-' . $course->id . '-';
        ...
        foreach (scandir($dir) as $backupfile) {
            ...
            if ($bcinfo->format === backup::FORMAT_ESET &&
