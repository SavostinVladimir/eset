...
abstract class backup_general_helper extends backup_helper {
    ...
    /**
     * Load all the blocks information needed for a given path within eset2 backup
     *
    ...
    /**
     * Load and format all the needed information from eset_backup.xml
     *
     * This function loads and process all the eset_backup.xml
    ...
    public static function get_backup_information($tempdir) {
        ...
        $esetfile = $backuptempdir . '/' . $tempdir . '/eset_backup.xml';
        if (!file_exists($esetfile)) { // Shouldn't happen ever, but...
            throw new backup_helper_exception('missing_eset_backup_xml_file', $esetfile);
        }

        $esetfilesha1 = sha1_file($esetfile);
        if ($esetfilesha1 === $cachesha1) {
            return clone $cache;
        }

        // Load the entire file to in-memory array
        $xmlparser = new progressive_parser();
        $xmlparser->set_file($esetfile);
        $xmlprocessor = new restore_esetxml_parser_processor();
        $xmlparser->set_processor($xmlprocessor);
        $xmlparser->process();
        $infoarr = $xmlprocessor->get_all_chunks();
        if (count($infoarr) !== 1) { // Shouldn't happen ever, but...
            throw new backup_helper_exception('problem_parsing_eset_backup_xml_file');
        }
        $infoarr = $infoarr[0]['tags']; // for commodity

        // Let's build info
        $info->eset_version = $infoarr['eset_version'];
        $info->eset_release = $infoarr['eset_release'];
        ...
        // Eset backup file don't have this option before 2.3
     ...
     *
     * This will only extract the eset_backup.xml file from an MBZ
    ...
    public static function get_backup_information_from_mbz($filepath, file_progress $progress = null) {
        ...
        if (!is_readable($filepath)) {
            throw new backup_helper_exception('missing_eset_backup_file', $filepath);
        }

        // Extract eset_backup.xml.
        ...
        $extracted = $fp->extract_to_pathname($filepath, $tmpdir, array('eset_backup.xml'), $progress);
        $esetfile =  $tmpdir . '/' . 'eset_backup.xml';
        if (!$extracted || !is_readable($esetfile)) {
            throw new backup_helper_exception('missing_eset_backup_xml_file', $esetfile);
        }
    ...
    /**
     * Given the information fetched from eset_backup.xml file
    ...
    public static function detect_backup_format($tempdir) {
        ...
        if (convert_helper::detect_eset2_format($tempdir)) {
            return backup::FORMAT_ESET;
        }
