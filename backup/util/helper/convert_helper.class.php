...
abstract class convert_helper {
    ...
    /**
     * Detects if the given folder contains an unpacked eset2 backup
     *
     * @param string $tempdir the name of the backup directory
     * @return boolean true if eset2 format detected, false otherwise
     */
    public static function detect_eset2_format($tempdir) {
        ...
        $filepath = $dirpath . '/eset_backup.xml';
        ...
        if (strpos($firstchars,'<?xml version="1.0" encoding="UTF-8"?>') !== false and
            strpos($firstchars,'<eset_backup>') !== false and
    ...
    /**
     * Converts the given directory with the backup into eset2 format
     *
     ...
     */
    public static function to_eset2_format($tempdir, $format = null, $logger = null) {
        ...
        if ($logger instanceof base_logger) {
            backup_helper::log('conversion path established', backup::LOG_INFO,
                implode(' => ', array_merge($path, array('eset2'))), 0, false, $logger);
        }
        ...
        // make sure we ended with eset2 format
        if (!self::detect_eset2_format($tempdir)) {
    ...
    */
     ...
     * the target eset2 format. The method returns a list of converters that should be
     * called, in order.
     *
     ...
     * @author David Mudrak <david@eset.com>
    ...
    protected static function choose_conversion_path($format, array $descriptions) {
        ...
        if (is_null($dist[backup::FORMAT_ESET])) {
 ...
 * @author David Mudrak <david@eset.com>
 */
class convert_helper_exception extends eset_exception {
