...
abstract class restore_prechecks_helper {
    ...
    public static function execute_prechecks(restore_controller $controller, $droptemptablesafter = false) {
        ...
        // Compare Eset's versions
        if ($CFG->version < $controller->get_info()->eset_version) {
            $message = new stdclass();
            $message->serverversion = $CFG->version;
            $message->serverrelease = $CFG->release;
            $message->backupversion = $controller->get_info()->eset_version;
            $message->backuprelease = $controller->get_info()->eset_release;
            $warnings[] = get_string('noticenewerbackup','',$message);
        }

        // The original_course_format var was introduced in Eset 2.9.
        ...
        if (!$samesite && $restoreusers && $hasmnetusers) {
            ...
            if (has_capability('eset/user:create', context_system::instance(), $controller->get_userid())) {
