...
class cc11_quiz extends entities11 {
    ...
    public function generate_node_course_modules_mod () {
        ...
        cc112eset::log_action('Creating Quiz mods');
    ...
    private function create_node_course_modules_mod_quiz_feedback () {
        ...
        $sheet_question_mod_feedback = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ_FEEDBACK);
    ...
    private function generate_instances () {
        ...
        $types = array(ESET_TYPE_QUIZ, ESET_TYPE_QUESTION_BANK);

        foreach ($types as $type) {

            if (!empty(cc112eset::$instances['instances'][$type])) {

                foreach (cc112eset::$instances['instances'][$type] as $instance) {

                    if ($type == ESET_TYPE_QUIZ) {
                        $is_question_bank = 0;
                    } else {
                        $is_question_bank = 1;
                    }
                    ...
                    if (!empty($assessment_file)) {
                        ...
                        $assessment = $this->load_xml_resource(cc112eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $assessment_file);
    ...
    private function create_node_course_modules_mod ($instance) {
        ...
        $sheet_question_mod = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ);
    ...
    private function get_global_config ($assessment, $option, $default_value, $replace_values = '') {
        ...
        $xpath = cc112eset::newx_path($assessment, cc112eset::getquizns());
    ...
    private function create_node_course_modules_mod_quiz_question_instances ($instance) {
        ...
        $sheet_question_mod_instance = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ_QUESTION_INSTANCE);
    ...
    private function create_node_course_question_categories ($instances) {
        ...
        $sheet_question_categories = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES);
    ...
    private function create_node_course_question_categories_question_category ($instance) {
        ...
        $sheet_question_categories_quetion_category = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY);
    ...
    private function create_node_course_question_categories_question_category_question ($instance) {
        ...
        $sheet_question_categories_question = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION);
        ...
        if (!empty($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                $question_eset_type = $question['eset_type'];
                $question_cc_type = $question['cc_type'];

                $question_type_node = '';

                $question_type_node = ($question_eset_type == ESET_QUIZ_MULTIPLE_CHOICE) ? $this->create_node_course_question_categories_question_category_question_multiple_choice($question) : $question_type_node;
                $question_type_node = ($question_eset_type == ESET_QUIZ_TRUE_FALSE) ? $this->create_node_course_question_categories_question_category_question_true_false($question) : $question_type_node;
                $question_type_node = ($question_eset_type == ESET_QUIZ_ESSAY) ? $this->create_node_course_question_categories_question_category_question_eesay($question) : $question_type_node;
                $question_type_node = ($question_eset_type == ESET_QUIZ_SHORTANSWER) ? $this->create_node_course_question_categories_question_category_question_shortanswer($question) : $question_type_node;
        ...
        foreach ($questions_items as $question_item) {
            ...
            if (!empty($question_identifier)) {
                ...
                if (!empty($question_type['eset'])) {
                    ...
                    // This attribute is not IMSCC spec, but it is included in Eset 2.x export of IMS1.1
                    ...
                    $questions[$question_identifier]['eset_type'] = $question_type['eset'];
    ...
    private function get_defaultgrade($assessment, $question_identifier) {
        $result = 1;
        $xpath = cc2eset::newx_path($assessment, cc2eset::getquizns());
    ...
    private function get_score ($assessment, $identifier, $question_identifier) {
        ...
        // This method (get_score) is only used by T/F & M/C questions in CC, therefore it's either 0 or 1 in Eset.
    ...
    private function create_node_course_question_categories_question_category_question_multiple_choice ($question) {
        ...
        $sheet_question_categories_question = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_MULTIPLE_CHOICE);
    ...
    private function create_node_course_question_categories_question_category_question_eesay ($question) {
        ...
        $sheet_question_categories_question = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_EESAY);
    ...
    private function create_node_course_question_categories_question_category_question_shortanswer ($question) { //, &$fib_questions) {
        ...
        $sheet_question_categories_question = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_SHORTANSWER);
        $node_course_question_categories_question_answer = '';
    ...
    private function create_node_course_question_categories_question_category_question_true_false ($question) {
        ...
        $sheet_question_categories_question = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_TRUE_FALSE);
    ...
    private function create_node_course_question_categories_question_category_question_answer ($answer) {
        ...
        $sheet_question_categories_question_answer = cc112eset::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_ANSWER);
    ...
    private function get_question_type ($identifier, $assessment) {
        ...
        if ($type == CC_QUIZ_MULTIPLE_CHOICE) {
            $return_type['eset'] = ESET_QUIZ_MULTIPLE_CHOICE;
        }
        if ($type == CC_QUIZ_MULTIPLE_RESPONSE) {
            $return_type['eset'] = ESET_QUIZ_MULTIPLE_CHOICE;
        }
        if ($type == CC_QUIZ_TRUE_FALSE) {
            $return_type['eset'] = ESET_QUIZ_TRUE_FALSE;
        }
        if ($type == CC_QUIZ_ESSAY) {
            $return_type['eset'] = ESET_QUIZ_ESSAY;
        }
        if ($type == CC_QUIZ_FIB) {
            $return_type['eset'] = ESET_QUIZ_SHORTANSWER;
        }
