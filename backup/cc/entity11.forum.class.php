...
class cc11_forum extends entities11 {
    ...
    public function generate_node () {
        ...
        cc2eset::log_action('Creating Forum mods');

        $response = '';

        if (!empty(cc2eset::$instances['instances'][ESET_TYPE_FORUM])) {
            foreach (cc2eset::$instances['instances'][ESET_TYPE_FORUM] as $instance) {
    ...
    private function create_node_course_modules_mod_forum ($instance) {
        ...
        $sheet_mod_forum = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_FORUM);
    ...
    public function get_topic_data ($instance) {
        ...
        if (!empty($topic_file)) {
            ...
            $topic_file_path = cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $topic_file;
            $topic_file_dir = dirname($topic_file_path);
            $topic = $this->load_xml_resource($topic_file_path);

            if (!empty($topic)) {

                $xpath = cc2eset::newx_path($topic, cc112eset::$forumns);
