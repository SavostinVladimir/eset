...
class cc11_resource extends entities11 {
    ...
    public function generate_node () {
        ...
        cc112eset::log_action('Creating Resource mods');

        $response = '';
        $sheet_mod_resource = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_RESOURCE);

        if (!empty(cc112eset::$instances['instances'][ESET_TYPE_RESOURCE])) {
            foreach (cc112eset::$instances['instances'][ESET_TYPE_RESOURCE] as $instance) {
    ...
    private function create_node_course_modules_mod_resource ($sheet_mod_resource, $instance) {
        ...
        $mod_alltext = '';
        $mod_summary = '';
        $xpath = cc112eset::newx_path(cc112eset::$manifest, cc112eset::$namespaces);

        if ($instance['common_cartriedge_type'] == cc112eset::CC_TYPE_WEBCONTENT || $instance['common_cartriedge_type'] == cc112eset::CC_TYPE_ASSOCIATED_CONTENT) {
        ...
        if ($instance['common_cartriedge_type'] == cc112eset::CC_TYPE_WEBLINK) {
            ...
            if ($external_resource) {
                ...
                $resource = $this->load_xml_resource(cc112eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $external_resource);

                if (!empty($resource)) {
                    $xpath = cc112eset::newx_path($resource, cc112eset::$resourcens);
        ...
        if (!empty($link) && ($instance['common_cartriedge_type'] == cc112eset::CC_TYPE_WEBCONTENT)) {
            $ext = strtolower(pathinfo($link, PATHINFO_EXTENSION));
            if (in_array($ext, array('html', 'htm', 'xhtml'))) {
                $mod_type = 'html';
                //extract the content of the file
                $rootpath = realpath(cc112eset::$path_to_manifest_folder);
