...
class cc_resource extends entities {
    ...
    public function generate_node () {
        ...
        cc2eset::log_action('Creating Resource mods');

        $response = '';
        $sheet_mod_resource = cc2eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_RESOURCE);

        if (!empty(cc2eset::$instances['instances'][ESET_TYPE_RESOURCE])) {
            foreach (cc2eset::$instances['instances'][ESET_TYPE_RESOURCE] as $instance) {
    ...
    private function create_node_course_modules_mod_resource ($sheet_mod_resource, $instance) {
        ...
        $mod_alltext = '';
        $mod_summary = '';
        $xpath = cc2eset::newx_path(cc2eset::$manifest, cc2eset::$namespaces);

        if ($instance['common_cartriedge_type'] == cc2eset::CC_TYPE_WEBCONTENT || $instance['common_cartriedge_type'] == cc2eset::CC_TYPE_ASSOCIATED_CONTENT) {
        ...
        if ($instance['common_cartriedge_type'] == cc2eset::CC_TYPE_WEBLINK) {
            ...
            if ($external_resource) {
                ...
                $resource = $this->load_xml_resource(cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $external_resource);

                if (!empty($resource)) {
                    $xpath = cc2eset::newx_path($resource, cc2eset::getresourcens());
        ...
        if (!empty($link) && ($instance['common_cartriedge_type'] == cc2eset::CC_TYPE_WEBCONTENT)) {
