...
function cc_convert ($dir) {
    ...
    if (is_readable($manifest_file) && !is_readable($eset_file)) {
        ...
        if ($is_cc) {
            ...
            echo get_string('cc2eset_checking_schema', 'imscc') . '<br />';
            ...
            if ($cc_manifest->load($manifest_file)) {
                if ($cc_manifest->schemaValidate($schema_file)) {
                    ...
                    echo get_string('cc2eset_valid_schema', 'imscc') . '<br />';

                    $cc2eset = new cc2eset($manifest_file);

                    if (!$cc2eset->is_auth()) {
                        return $cc2eset->generate_eset_xml();
                    } else {
                        echo $OUTPUT->notification(get_string('cc2eset_req_auth', 'imscc'));
                        return false;
                    }

                } else {
                    echo $OUTPUT->notification(get_string('cc2eset_invalid_schema', 'imscc'));
                    return false;
                }

            } else {
                echo $OUTPUT->notification(get_string('cc2eset_manifest_dont_load', 'imscc'));
                return false;
            }
        }
    }
