...
abstract class cc_convert_eset2 {
    ...
    public static function convert($packagedir, $outdir) {
        ...
        if ($doc->load($coursefile)) {
            ...
            $eset_backup = $dir . DIRECTORY_SEPARATOR . 'eset_backup.xml';
            $secp = new XMLGenericDocument();
            $docp = new XMLGenericDocument();
            if ($docp->load($eset_backup)) {
                //sections
                $sections = array();
                ...
                $section_list = $docp->nodeList('/eset_backup/information/contents/sections/section');
    ...
    */
    protected static function process_sequence(cc_i_item &$item, cc_i_manifest &$manifest, array $sequence, $packageroot, $outdir) {
        $eset_backup = $packageroot . DIRECTORY_SEPARATOR . 'eset_backup.xml';
        $doc = new XMLGenericDocument();
        if(!$doc->load($eset_backup)) {
            return;
        }
        $activities = $doc->nodeList('/eset_backup/information/contents/activities/activity');
        if (!empty($activities)) {
