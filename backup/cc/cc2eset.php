...
class cc2eset {
    ...
    public function generate_eset_xml () {
        ...
        if (array_key_exists("index", self::$instances)) {
            ...
            if (!file_put_contents(static::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'eset.xml', $result_xml)) {
                static::log_action('Cannot save the eset manifest file: ' . static::$path_to_tmp_folder . DIRECTORY_SEPARATOR . 'eset.xml', true);
            } else {
                $status = true;
            }
    ...
    protected function create_node_course_sections_section () {
        ...
        if (!empty($topics)) {
            ...
            foreach ($topics as $topic) {
                ...
                if ($topic['eset_type'] == ESET_TYPE_LABEL) {
    ...
    */
    ...
    protected function create_node_course_sections_section_mods_mod ($root_parent) {
        ...
        if ($childs) {
            ...
            foreach ($childs as $child) {
                ...
                if ($child['eset_type'] == ESET_TYPE_LABEL) {
    ...
    protected function create_code_info_details_mod () {
        ...
        $count_quiz = $this->count_instances(ESET_TYPE_QUIZ);
        $count_forum = $this->count_instances(ESET_TYPE_FORUM);
        $count_resource = $this->count_instances(ESET_TYPE_RESOURCE);
        $count_label = $this->count_instances(ESET_TYPE_LABEL);

        $sheet_info_details_mod_instances_instance = static::loadsheet(SHEET_INFO_DETAILS_MOD_INSTANCE);

        if ($count_resource > 0) {
            $resource_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_resource, static::$instances['instances'][ESET_TYPE_RESOURCE]);
        }
        if ($count_quiz > 0) {
            $quiz_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_quiz, static::$instances['instances'][ESET_TYPE_QUIZ]);
        }
        if ($count_forum > 0) {
            $forum_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_forum, static::$instances['instances'][ESET_TYPE_FORUM]);
        }
        if ($count_label > 0) {
            $label_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_label, static::$instances['instances'][ESET_TYPE_LABEL]);
        }

        $resource_mod = $count_resource ? $this->create_mod_info_details_mod(ESET_TYPE_RESOURCE, $resource_instance) : '';
        $quiz_mod = $count_quiz ? $this->create_mod_info_details_mod(ESET_TYPE_QUIZ, $quiz_instance) : '';
        $forum_mod = $count_forum ? $this->create_mod_info_details_mod(ESET_TYPE_FORUM, $forum_instance) : '';
        $label_mod = $count_label ? $this->create_mod_info_details_mod(ESET_TYPE_LABEL, $label_instance) : '';
        ...
        if ($items) {
            ...
            foreach ($items as $item) {
                ...
                if ($item->nodeName == "item")  {
                    ...
                    $eset_type = $this->convert_to_eset_type($cc_type);
                    //Fix the label issue - MDL-33523
                    if (empty($identifierref) && empty($title)) {
                      $eset_type = TYPE_UNKNOWN;
                    }
                }
                elseif ($item->nodeName == "resource")  {
                    ...
                    $title = 'Quiz Bank ' . ($this->count_instances($eset_type) + 1);
                ...
                static::$instances['index'][$array_index]['eset_type'] = $eset_type;
                ...
                static::$instances['index'][$array_index]['instance'] = $this->count_instances($eset_type);
                static::$instances['index'][$array_index]['resource_indentifier'] = $identifierref;

                static::$instances['instances'][$eset_type][] = array('title' => $title,
        ...
        if (array_key_exists('index', static::$instances)) {
            if (static::$instances['index'] && $type) {
                ...
                foreach (static::$instances['index'] as $instance) {
                    if (!empty($instance['eset_type'])) {
                        $types[] = $instance['eset_type'];
                    }
                }
    ...
    public function convert_to_eset_type ($cc_type) {
        $type = TYPE_UNKNOWN;

        if ($cc_type == static::CC_TYPE_FORUM) {
            $type = ESET_TYPE_FORUM;
        }
        ...
        if ($cc_type == static::CC_TYPE_QUESTION_BANK) {
            $type = ESET_TYPE_QUESTION_BANK;
        }
        //TODO: label
        if ($cc_type == static::CC_TYPE_EMPTY) {
            $type = ESET_TYPE_LABEL;
        }
    ...
    public static function log_file() {
        return static::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'cc2eset.log';
    }
