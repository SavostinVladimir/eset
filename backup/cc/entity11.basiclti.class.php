...
class cc11_basiclti extends entities11 {
    ...
    public function generate_node () {
        ...
        cc2eset::log_action('Creating BasicLTI mods');

        $response = '';

        if (!empty(cc2eset::$instances['instances'][ESET_TYPE_BASICLTI])) {
            foreach (cc2eset::$instances['instances'][ESET_TYPE_BASICLTI] as $instance) {
    ...
    private function create_node_course_modules_mod_basiclti ($instance) {
        ...
        $sheet_mod_basiclti = cc112eset::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_BASICLTI);
    ...
    public function get_basiclti_data($instance) {
        ...
        if (!empty($basiclti_file)) {
            $basiclti_file_path = cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $basiclti_file;
            $basiclti_file_dir = dirname($basiclti_file_path);
            $basiclti = $this->load_xml_resource($basiclti_file_path);
            if (!empty($basiclti)) {
                $xpath = cc2eset::newx_path($basiclti, cc112eset::$basicltins);
