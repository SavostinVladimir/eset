...
class entities {
    ...
    public function load_xml_resource($path_to_file) {
        ...
        cc2eset::log_action('Load the XML resource file: '.$path_to_file);

        if (!$resource->load($path_to_file)) {
            cc2eset::log_action('Cannot load the XML resource file: ' . $path_to_file, true);
        }
    ...
    public function move_files($files, $destination_folder) {
        ...
        if (!empty($files)) {
            ...
            foreach ($files as $file) {
                $source = cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $file;
                $destination = $destination_folder . DIRECTORY_SEPARATOR . $file;

                $destination_directory = dirname($destination);

                cc2eset::log_action('Copy the file: ' . $source . ' to ' . $destination);
                ...
                if (!$copy_success) {
                    ...
                    cc2eset::log_action('Cannot copy the file ' . $source . ' to ' . $destination, false);
                }
            }
        }
    }
    ...
    protected function get_all_files () {
        ...
        foreach (cc2eset::$restypes as $type) {
        ...
        if (!empty($labels) && ($labels->length > 0)) {
            $tname = 'course_files';
            $dpath = cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $tname;
            $rfpath = 'files.gif';
    ...
    public function move_all_files() {
        ...
        if (!empty($files)) {
            $this->move_files($files, cc2eset::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'course_files', true);
        }
