...
require_once($CFG->dirroot . '/backup/cc/cc2eset.php');
...
class cc112eset extends cc2eset {
    ...
    public function convert_to_eset_type ($cc_type) {
        $type = parent::convert_to_eset_type($cc_type);
    ...
    protected function create_code_info_details_mod () {
        ...
        $count_blti = $this->count_instances(ESET_TYPE_LTI);
        ...
        if ($count_blti > 0) {
            $blti_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_blti, static::$instances['instances'][ESET_TYPE_LTI]);
            $blti_mod = $blti_instance ? $this->create_mod_info_details_mod(ESET_TYPE_LTI, $blti_instance) : '';
        }
    ...
    /**
    * (non-PHPdoc)
    * @see cc2eset::get_module_visible()
    */
