...
require_once($CFG->dirroot.'/backup/cc/cc112eset.php');
...
class imscc11_converter extends base_converter {
    ...
    public static function detect_format($tempdir) {
        ...
        $manifest = cc112eset::get_manifest($filepath);
     ...
     * 'from' - the supported source format, eg. backup::FORMAT_ESET1
     * 'to'   - the supported target format, eg. backup::FORMAT_ESET
    ...
    protected function execute() {
        ...
        $manifest = cc112eset::get_manifest($this->get_tempdir_path());
        if (empty($manifest)) {
        ...
        $cc112eset = new cc112eset($manifest);
        if ($cc112eset->is_auth()) {
            throw new imscc11_convert_exception('protected_cc_not_supported');
        }
        $status = $cc112eset->generate_eset_xml();
        // Final cleanup.
        ...
        if ($mdoc->load($manifestdir.'/eset.xml', LIBXML_NONET)) {
            $mdoc->save($this->get_workdir_path().'/eset.xml', LIBXML_NOEMPTYTAG);
        } else {
            $xml_error->collect();
            $this->log('validation error(s): '.PHP_EOL.error_messages::instance(), backup::LOG_DEBUG, null, 2);
            throw new imscc11_convert_exception(error_messages::instance()->to_string(true));
