...
class imscc11_store_backup_file extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        list($dinfo, $cinfo, $sinfo) = backup_controller_dbops::get_eset_backup_information($this->get_backupid());
...
class imscc11_backup_convert extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        if (mkdir($tempdir, $CFG->directorypermissions, true)) {
            ...
            cc_convert_eset2::convert($basepath, $tempdir);
            //Switch the directories
            if (empty($CFG->keeptempdirectoriesonbackup)) {
                fulldelete($basepath);
            } else {
                if (!rename($basepath, $basepath  . '_eset2_source')) {
