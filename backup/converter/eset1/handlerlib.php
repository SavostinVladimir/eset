...
/**
 * Defines Eset 1.9 backup conversion handlers
 *
 ...
 */
abstract class eset1_handlers_factory {

    /**
     * @param eset1_converter the converter requesting the converters
     * @return list of all available conversion handlers
     */
    public static function get_handlers(eset1_converter $converter) {

        $handlers = array(
            new eset1_root_handler($converter),
            new eset1_info_handler($converter),
            new eset1_course_header_handler($converter),
            new eset1_course_outline_handler($converter),
            new eset1_roles_definition_handler($converter),
            new eset1_question_bank_handler($converter),
            new eset1_scales_handler($converter),
            new eset1_outcomes_handler($converter),
            new eset1_gradebook_handler($converter),
        );
        ...
        foreach ($handlers as $handler) {
            if (!$handler instanceof eset1_handler) {
                throw new eset1_convert_exception('wrong_handler_class', get_class($handler));
            }
        }
     ...
     * @param eset1_converter $converter the converter requesting the handler
     * @throws eset1_convert_exception
     * @return array of {@link eset1_handler} instances
     */
    protected static function get_plugin_handlers($type, eset1_converter $converter) {
        global $CFG;

        $handlers = array();
        $plugins = core_component::get_plugin_list($type);
        foreach ($plugins as $name => $dir) {
            $handlerfile  = $dir . '/backup/eset1/lib.php';
            $handlerclass = "eset1_{$type}_{$name}_handler";
            if (file_exists($handlerfile)) {
                require_once($handlerfile);
            } elseif ($type == 'block') {
                $handlerclass = "eset1_block_generic_handler";
            } else {
                continue;
            }

            if (!class_exists($handlerclass)) {
                throw new eset1_convert_exception('missing_handler_class', $handlerclass);
            }
 ...
 */
abstract class eset1_handler implements loggable {

    /** @var eset1_converter */
    protected $converter;

    /**
     * @param eset1_converter $converter the converter that requires us
     */
    public function __construct(eset1_converter $converter) {
        $this->converter = $converter;
    }

    /**
     * @return eset1_converter the converter that required this handler
     */
 ...
 */
abstract class eset1_xml_handler extends eset1_handler {
    ...
    protected function open_xml_writer($filename) {
        ...
        if (!is_null($this->xmlfilename) and $filename !== $this->xmlfilename) {
            throw new eset1_convert_exception('xml_writer_already_opened_for_other_file', $this->xmlfilename);
        }
        ...
        if (!$this->xmlwriter instanceof xml_writer) {
            ...
            if (!check_dir_exists($directory)) {
                throw new eset1_convert_exception('unable_create_target_directory', $directory);
            }
            $this->xmlwriter = new xml_writer(new file_xml_output($fullpath), new eset1_xml_transformer());
    ...
    protected function write_xml($element, array $data, array $attribs = array(), $parent = '/') {
        ...
        if (!$this->has_xml_writer()) {
            throw new eset1_convert_exception('write_xml_without_writer');
        }
 ...
 */
class eset1_root_handler extends eset1_xml_handler {

    public function get_paths() {
        return array(new convert_path('root_element', '/ESET_BACKUP'));
    }
    ...
    public function on_root_element_start() {
        ...
        $fileshandler = new eset1_files_handler($this->converter);
        $fileshandler->process();
    }

    /**
     * This is executed at the end of the eset.xml parsing
     */
    public function on_root_element_end() {
        ...
        ////////////////////////////////////////////////////////////////////////
        // write eset_backup.xml
        ////////////////////////////////////////////////////////////////////////
        $this->open_xml_writer('eset_backup.xml');

        $this->xmlwriter->begin_tag('eset_backup');
        $this->xmlwriter->begin_tag('information');

        // eset_backup/information
        $this->xmlwriter->full_tag('name', $backupinfo['name']);
        $this->xmlwriter->full_tag('eset_version', $backupinfo['eset_version']);
        $this->xmlwriter->full_tag('eset_release', $backupinfo['eset_release']);
        ...
        // eset_backup/information/contents
        $this->xmlwriter->begin_tag('contents');

        // eset_backup/information/contents/activities
        ...
        // eset_backup/information/contents/sections
        ...
        foreach ($this->converter->get_stash_itemids('sectioninfo') as $sectionid) {
            ...
            $sectionsettings[] = array(
                ...
                'value'     => 0); // @todo how to detect this from eset.xml?
        ...
        // eset_backup/information/contents/course
        ...
        $this->xmlwriter->end_tag('eset_backup');
        ...
        // missing file may happen if the watched tag is not present in eset.xml (for example
        // QUESTION_CATEGORIES is optional in eset.xml but questions.xml must exist in
        // eset2 format) or the handler has not been implemented yet.
 ...
 */
class eset1_files_handler extends eset1_xml_handler {
...
class eset1_info_handler extends eset1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('info', '/ESET_BACKUP/INFO'),
            new convert_path('info_details', '/ESET_BACKUP/INFO/DETAILS'),
            new convert_path('info_details_mod', '/ESET_BACKUP/INFO/DETAILS/MOD'),
            new convert_path('info_details_mod_instance', '/ESET_BACKUP/INFO/DETAILS/MOD/INSTANCES/INSTANCE'),
        );
    }

    /**
     * Stashes the backup info for later processing by {@link eset1_root_handler}
     */
    ...
    public function on_info_details_mod_end($data) {
        ...
        // keep only such modules that seem to have the support for eset1 implemented
        $modname = $this->currentmod['name'];
        if (file_exists($CFG->dirroot.'/mod/'.$modname.'/backup/eset1/lib.php')) {
    ...
    /**
     * Stashes the list of activity module types for later processing by {@link eset1_root_handler}
     */
...
/**
 * Handles the conversion of /ESET_BACKUP/COURSE/HEADER paths
 */
class eset1_course_header_handler extends eset1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'course_header', '/ESET_BACKUP/COURSE/HEADER',
                array(
                    'newfields' => array(
                        'summaryformat'          => 1,
                        'legacyfiles'            => 2,
            ...
            new convert_path(
                'course_header_category', '/ESET_BACKUP/COURSE/HEADER/CATEGORY',
                array(
                    'newfields' => array(
                        'description' => null,
                    )
    ...
    public function on_course_header_end() {
        ...
        $this->course['summary'] = eset1_converter::migrate_referenced_files($this->course['summary'], $fileman);
 ...
 */
class eset1_course_outline_handler extends eset1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('course_sections', '/ESET_BACKUP/COURSE/SECTIONS'),
            new convert_path(
                'course_section', '/ESET_BACKUP/COURSE/SECTIONS/SECTION',
                array(
                    'newfields' => array(
                        'name'          => null,
                        'summaryformat' => 1,
            ...
            new convert_path(
                'course_module', '/ESET_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD',
                array(
                    'newfields' => array(
                        'completion'                => 0,
            ...
            new convert_path('course_modules', '/ESET_BACKUP/COURSE/MODULES'),
            // todo new convert_path('course_module_roles_overrides', '/ESET_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_OVERRIDES'),
            // todo new convert_path('course_module_roles_assignments', '/ESET_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_ASSIGNMENTS'),
        );
    }
    ...
    public function on_course_section_end() {
        ...
        $this->currentsection['summary'] = eset1_converter::migrate_referenced_files($this->currentsection['summary'], $fileman);
    ...
    public function on_course_modules_end() {
        ...
        foreach ($this->converter->get_stash('modnameslist') as $modname) {
            ...
            foreach ($modinfo['instances'] as $modinstanceid => $modinstance) {
                ...
                $this->write_xml('grade_letters', array()); // no grade_letters in module context in Eset 1.9
 ...
 */
class eset1_roles_definition_handler extends eset1_xml_handler {

    /**
     * Where the roles are defined in the source eset.xml
     */
    public function get_paths() {
        return array(
            new convert_path('roles', '/ESET_BACKUP/ROLES'),
            new convert_path(
                'roles_role', '/ESET_BACKUP/ROLES/ROLE',
                array(
                    'newfields' => array(
                        'description'   => '',
                        'sortorder'     => 0,
    ...
    /**
     * If there are any roles defined in eset.xml, convert them to roles.xml
     */
    ...
    public function on_roles_end() {
        ...
        if (!$this->has_xml_writer()) {
            // no roles defined in eset.xml so {link self::process_roles_role()}
            // was never executed
            $this->open_xml_writer('roles.xml');
...
/**
 * Handles the conversion of the question bank included in the eset.xml file
 */
class eset1_question_bank_handler extends eset1_xml_handler {
    ...
    /** @var eset1_file_manager instance used to convert question images */
    ...
    public function get_paths() {
        ...
        $paths = array(
            new convert_path('question_categories', '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES'),
            new convert_path(
                'question_category', '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY',
                array(
                    'newfields' => array(
                        'infoformat' => 0
                    )
                )),
            new convert_path('question_category_context', '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/CONTEXT'),
            new convert_path('questions', '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS'),
            // the question element must be grouped so we can re-dispatch it to the qtype handler as a whole
            new convert_path('question', '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION', array(), true),
        );
        ...
        foreach (array_keys($subpaths) as $subpath) {
            ...
            $path = '/ESET_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION/'.$subpath;
    ...
    public function process_question_category_context($data) {
        ...
        case 'course':
            ...
            break;
        case 'coursecategory':
            // this is a bit hacky. the source eset.xml defines COURSECATEGORYLEVEL as a distance
    ...
    public function process_question(array $data, array $raw) {
        ...
        $data['questiontext'] = eset1_converter::migrate_referenced_files($data['questiontext'], $this->fileman);

        // Migrate files in generalfeedback.
        $this->fileman->filearea  = 'generalfeedback';
        $data['generalfeedback'] = eset1_converter::migrate_referenced_files($data['generalfeedback'], $this->fileman);
        ...
        foreach (array(
            ...
            if (!array_key_exists($fieldname, $data)) {
                throw new eset1_convert_exception('missing_common_question_field', $fieldname);
            }
     ...
     * @return array|eset1_qtype_handler|bool
     */
    protected function get_qtype_handler($qtype) {
        ...
        if (is_null($this->qtypehandlers)) {
            ...
            foreach (core_component::get_plugin_list('qtype') as $qtypename => $qtypelocation) {
                $filename = $qtypelocation.'/backup/eset1/lib.php';
                if (file_exists($filename)) {
                    $classname = 'eset1_qtype_'.$qtypename.'_handler';
                    require_once($filename);
                    if (!class_exists($classname)) {
                        throw new eset1_convert_exception('missing_handler_class', $classname);
                    }
...
/**
 * Handles the conversion of the scales included in the eset.xml file
 */
class eset1_scales_handler extends eset1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('scales', '/ESET_BACKUP/COURSE/SCALES'),
            new convert_path(
                'scale', '/ESET_BACKUP/COURSE/SCALES/SCALE',
                array(
                    'renamefields' => array(
                        'scaletext' => 'scale',
                    ),
    ...
    public function process_scale(array $data, array $raw) {
        ...
        $this->fileman->itemid = $data['id'];
        $data['description'] = eset1_converter::migrate_referenced_files($data['description'], $this->fileman);
 ...
 */
class eset1_outcomes_handler extends eset1_xml_handler {

    /** @var eset1_file_manager instance used to convert images embedded into outcome descriptions */
    protected $fileman = null;
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook_grade_outcomes', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES'),
            new convert_path(
                'gradebook_grade_outcome', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES/GRADE_OUTCOME',
                array(
                    'addfields' => array(
                        'descriptionformat' => FORMAT_ESET,
                    ),
                )
            ),
        );
...
/**
 * Handles the conversion of the gradebook structures in the eset.xml file
 */
class eset1_gradebook_handler extends eset1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook', '/ESET_BACKUP/COURSE/GRADEBOOK'),
            new convert_path('gradebook_grade_letter', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_LETTERS/GRADE_LETTER'),
            new convert_path(
                'gradebook_grade_category', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_CATEGORIES/GRADE_CATEGORY',
                array(
                    'addfields' => array(
            ...
            new convert_path('gradebook_grade_item', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM'),
            new convert_path('gradebook_grade_item_grades', '/ESET_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM/GRADE_GRADES'),
        );
    }
     ...
     *
     * This should not be needed actually as the eset.xml contains just one GRADEBOOK
     ...
     *
     * In Eset 1.9, all grade_letters are from course context only. Therefore
     * we put them here.
     */
     ...
     *
     * Eset 1.9 backup does not store the grade_category's depth and path. This method is used
    ...
    protected function calculate_category_path($categoryid) {
        ...
        if (!array_key_exists($categoryid, $this->categoryparent)) {
            throw new eset1_convert_exception('gradebook_unknown_categoryid', null, $categoryid);
        }
        ...
        while (!is_null($parent)) {
            ...
            if (in_array($parent, $path)) {
                throw new eset1_convert_exception('circular_reference_in_categories_tree');
            }
        }
 ...
 */
abstract class eset1_plugin_handler extends eset1_xml_handler {
     ...
     */
    public function __construct(eset1_converter $converter, $plugintype, $pluginname) {
 ...
 */
abstract class eset1_qtype_handler extends eset1_plugin_handler {

    /** @var eset1_question_bank_handler */
     ...
     *
     * @param array $answers as parsed by the grouped parser in eset.xml
    ...
    protected function migrate_files($text, $component, $filearea, $itemid) {
        ...
        $text = eset1_converter::migrate_referenced_files($text, $fileman);
        return $text;
    }
     ...
     *
     * This structure is not present in eset.xml, we create a new artificial one here.
     *
    ...
    protected function get_default_numerical_options($oldquestiontextformat, $units) {
        ...
        if ($CFG->texteditors !== 'textarea' and $oldquestiontextformat == FORMAT_ESET) {
    ...
    public function __construct(eset1_question_bank_handler $qbankhandler, $qtype) {
    ...
    final public function get_paths() {
        throw new eset1_convert_exception('qtype_handler_get_paths');
    }
    ...
    final protected function open_xml_writer($filename) {
        throw new eset1_convert_exception('opening_xml_writer_forbidden');
    }
     ...
     *
     * @param array $old the parsed answer array in eset.xml
    ...
    private function convert_answer(array $old, $qtype) {
        ...
        if ($CFG->texteditors !== 'textarea') {
            ...
            $new['feedbackformat'] = FORMAT_ESET;
        }
 ...
 */
abstract class eset1_mod_handler extends eset1_plugin_handler {
     ...
     * {@link eset1_course_outline_handler::process_course_module()}
     *
 ...
 */
abstract class eset1_resource_successor_handler extends eset1_mod_handler {

    /**
     * Resource successors do not attach to paths themselves, they are called explicitely
     * by eset1_mod_resource_handler
     *
     * @return array
     */
    final public function get_paths() {
     ...
     *
     * Called by {@link eset1_mod_resource_handler::process_resource()}
     *
 ...
 */
abstract class eset1_block_handler extends eset1_plugin_handler {

    public function get_paths() {
        $blockname = strtoupper($this->pluginname);
        return array(
            new convert_path('block', "/ESET_BACKUP/COURSE/BLOCKS/BLOCK/{$blockname}"),
        );
    }
    ...
    protected function write_roles_xml($newdata, $data) {
        // This is an empty shell, as the eset1 converter doesn't handle user data.
 ...
 */
class eset1_block_generic_handler extends eset1_block_handler {
 ...
 */
abstract class eset1_submod_handler extends eset1_plugin_handler {

    /** @var eset1_mod_handler */
    protected $parenthandler;

    /**
     * @param eset1_mod_handler $parenthandler the handler of a module we are subplugin of
     * @param string $subplugintype the type of the subplugin
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(eset1_mod_handler $parenthandler, $subplugintype, $subpluginname) {
