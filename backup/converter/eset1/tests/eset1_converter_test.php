...
/**
 * Unit tests for the eset1 converter
 *
 * @package    core_backup
...
global $CFG;
require_once($CFG->dirroot . '/backup/converter/eset1/lib.php');


class core_backup_eset1_converter_testcase extends advanced_testcase {

    /** @var string the name of the directory containing the unpacked Eset 1.9 backup */
    protected $tempdir;

    /** @var string the full name of the directory containing the unpacked Eset 1.9 backup */
    ...
    protected function setUp() {
        ...
        copy(
            "$CFG->dirroot/backup/converter/eset1/tests/fixtures/eset.xml",
            "$this->tempdirpath/eset.xml"
        );
        copy(
            "$CFG->dirroot/backup/converter/eset1/tests/fixtures/icon.gif",
    ...
    public function test_detect_format() {
        $detected = eset1_converter::detect_format($this->tempdir);
        $this->assertEquals(backup::FORMAT_ESET1, $detected);
    }

    public function test_convert_factory() {
        $converter = convert_factory::get_converter('eset1', $this->tempdir);
        $this->assertInstanceOf('eset1_converter', $converter);
    }

    /**
     * @expectedException eset1_convert_storage_exception
     */
    ...
    /**
     * @expectedException eset1_convert_empty_storage_exception
     */
    public function test_stash_requiring_empty_stash() {
        ...
        try {
            ...
            throw new eset1_convert_empty_storage_exception('rethrowing');
        }
    }
    ...
    public function test_convert_path_implicit_recipes() {
        ...
        $this->assertEquals('esetr', $data['martin']);
...
as it is parsed from the backup file. <br /><br /><img border="0" width="110" vspace="0" hspace="0" height="92" title="News" alt="News" src="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif" /><a href="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif$@FORCEDOWNLOAD@$">download image</a><br />
    ...
    <br /><a href=\'$@FILEPHP@$$@SLASH@$MANUAL.DOC$@FORCEDOWNLOAD@$\'>download manual</a><br />';
        ...
        $files = eset1_converter::find_referenced_files($text);
        ...
        $text = eset1_converter::rewrite_filephp_usage($text, array('/pics/news.gif', '/another/file/notused.txt'));
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>';
        ...
        $text = eset1_converter::rewrite_filephp_usage($text, $files);
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>', $text);
    ...
    public function test_question_bank_conversion() {
        ...
        copy(
            "$CFG->dirroot/backup/converter/eset1/tests/fixtures/questions.xml",
            "$this->tempdirpath/eset.xml"
        );
        $converter = convert_factory::get_converter('eset1', $this->tempdir);
        $converter->convert();
