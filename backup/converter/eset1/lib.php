...
/**
 * Provides classes used by the eset1 converter
 *
 * @package    backup-convert
 * @subpackage eset1
 * @copyright  2011 Mark Nielsen <mark@esetrooms.com>
...
/**
 * Converter of Eset 1.9 backup into Eset 2.x format
 */
class eset1_converter extends base_converter {

    /** @var progressive_parser eset.xml file parser */
    protected $xmlparser;

    /** @var eset1_parser_processor */
    ...
    public function log($message, $level, $a = null, $depth = null, $display = false) {
        parent::log('(eset1) '.$message, $level, $a, $depth, $display);
    }

    /**
     * Detects the Eset 1.9 format of the backup directory
     *
     * @param string $tempdir the name of the backup directory
     * @return null|string backup::FORMAT_ESET1 if the Eset 1.9 is detected, null otherwise
     */
    public static function detect_format($tempdir) {
        global $CFG;

        $tempdirpath = make_backup_temp_directory($tempdir, false);
        $filepath = $tempdirpath . '/eset.xml';
        if (file_exists($filepath)) {
            ...
            if (strpos($first_chars,'<?xml version="1.0" encoding="UTF-8"?>') !== false and
                strpos($first_chars,'<ESET_BACKUP>') !== false and
                strpos($first_chars,'<INFO>') !== false) {

                return backup::FORMAT_ESET1;
            }
        }
    ...
    protected function init() {
        ...
        $this->xmlparser->set_file($this->get_tempdir_path() . '/eset.xml');
        $this->log('setting xml processor', backup::LOG_DEBUG, null, 1);
        $this->xmlprocessor = new eset1_parser_processor($this);
        $this->xmlparser->set_processor($this->xmlprocessor);

        // make sure that MOD and BLOCK paths are visited
        $this->xmlprocessor->add_path('/ESET_BACKUP/COURSE/MODULES/MOD');
        $this->xmlprocessor->add_path('/ESET_BACKUP/COURSE/BLOCKS/BLOCK');

        // register the conversion handlers
        foreach (eset1_handlers_factory::get_handlers($this) as $handler) {
    ...
    protected function execute() {
        ...
        $this->log('parsing eset.xml starts', backup::LOG_DEBUG);
        $this->xmlparser->process();
        $this->log('parsing eset.xml done', backup::LOG_DEBUG);
     ...
     */
    protected function register_handler(eset1_handler $handler, array $elements) {
        ...
        foreach ($elements as $element) {
            ...
            $path = preg_replace('/^\/ESET_BACKUP\/COURSE\/MODULES\/MOD\/(\w+)\//', '/ESET_BACKUP/COURSE/MODULES/MOD/', $path);
            $path = preg_replace('/^\/ESET_BACKUP\/COURSE\/BLOCKS\/BLOCK\/(\w+)\//', '/ESET_BACKUP/COURSE/BLOCKS/BLOCK/', $path);
    ...
    public function process_chunk($data) {
        ...
        if ($path === '/ESET_BACKUP/COURSE/MODULES/MOD') {
            $this->currentmod = strtoupper($data['tags']['MODTYPE']);
            $path = '/ESET_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod;

        } else if (strpos($path, '/ESET_BACKUP/COURSE/MODULES/MOD') === 0) {
            $path = str_replace('/ESET_BACKUP/COURSE/MODULES/MOD', '/ESET_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod, $path);
        }

        // expand the BLOCK paths so that they contain the module name
        if ($path === '/ESET_BACKUP/COURSE/BLOCKS/BLOCK') {
            $this->currentblock = strtoupper($data['tags']['NAME']);
            $path = '/ESET_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock;

        } else if (strpos($path, '/ESET_BACKUP/COURSE/BLOCKS/BLOCK') === 0) {
            $path = str_replace('/ESET_BACKUP/COURSE/BLOCKS/BLOCK', '/ESET_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock, $path);
        }
     ...
     * /ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP because the <MOD> must
     ...
     * /ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP/SUBMISSIONS because it is
    ...
    public function set_stash($stashname, $info, $itemid = 0) {
        try {
            ...
            throw new eset1_convert_storage_exception('unable_to_restore_stash', null, $e->getMessage());
        }
    }
     ...
     * @param string $stashname name of the stash
     * @param int $itemid optional id for multiple infos within the same stashname
     * @throws eset1_convert_empty_storage_exception if the info has not been stashed previously
    ...
    public function get_stash($stashname, $itemid = 0) {
        ...
        if (empty($record)) {
            throw new eset1_convert_empty_storage_exception('required_not_stashed_data', array($stashname, $itemid));
        } else {
    ...
    public function get_stash_or_default($stashname, $itemid = 0, $default = null) {
        ...
        } catch (eset1_convert_empty_storage_exception $e) {
            return $default;
        }
    }
     ...
     *
     * Eset 1.9 backups do not contain any context information. But we need them
     * in Eset 2.x format so here we generate fictive context id for every given
    ...
    public function get_file_manager($contextid = null, $component = null, $filearea = null, $itemid = 0, $userid = null) {
        return new eset1_file_manager($this, $contextid, $component, $filearea, $itemid, $userid);
    }
     ...
     * @return eset1_inforef_manager
     */
    public function get_inforef_manager($name, $id = 0) {
        return new eset1_inforef_manager($this, $name, $id);
    }
     ...
     * @param eset1_file_manager $fileman file manager to use for the file migration
     * @return string the original $text with $@FILEPHP@$ references replaced with the new @@PLUGINFILE@@
     */
    public static function migrate_referenced_files($text, eset1_file_manager $fileman) {
        ...
        if (!empty($files)) {
            foreach ($files as $file) {
                ...
                } catch (eset1_convert_exception $e) {
    ...
    public static function find_referenced_files($text) {
        ...
        if ($result === false) {
            throw new eset1_convert_exception('error_while_searching_for_referenced_files');
        }
        if ($result == 0) {
            return $files;
        }
    ...
    public static function description() {
        ...
        return array(
            'from'  => backup::FORMAT_ESET1,
            'to'    => backup::FORMAT_ESET,
            'cost'  => 10,
        );
    }
}
 ...
 */
class eset1_convert_exception extends convert_exception {
}


/**
 * Exception thrown by the temporary storage subsystem of eset1_converter
 */
class eset1_convert_storage_exception extends eset1_convert_exception {
}
 ...
 * Exception thrown by the temporary storage subsystem of eset1_converter
 */
class eset1_convert_empty_storage_exception extends eset1_convert_exception {
}


/**
 * XML parser processor used for processing parsed eset.xml
 */
class eset1_parser_processor extends grouped_parser_processor {
 ...
 */
class eset1_xml_transformer extends xml_contenttransformer {
    ...
    public function process($content) {
        ...
        if (is_array($content)) {
            debugging('Eset1 XML transformer should not process arrays but plain content always', DEBUG_DEVELOPER);
            ...
            debugging('Eset1 XML transformer should not process objects but plain content always', DEBUG_DEVELOPER);
...
class convert_path {
     ...
     * {@line eset1_course_outline_handler} is a good example that uses all of these.
     *
 ...
 */
class convert_path_exception extends eset_exception {
 ...
 *
 * The files in Eset 1.9 backup are stored in moddata, user_files, group_files,
 * course_files and site_files folders.
 */
class eset1_file_manager implements loggable {

    /** @var eset1_converter instance we serve to */
    public $converter;
     ...
     *
     * @param eset1_converter $converter the converter instance we serve to
     ...
     * @param int $userid initial user id of the files being migrated
     */
    public function __construct(eset1_converter $converter, $contextid = null, $component = null, $filearea = null, $itemid = 0, $userid = null) {
    ...
    public function migrate_file($sourcepath, $filepath = '/', $filename = null, $sortorder = 0, $timecreated = null, $timemodified = null) {
        ...
        if ($sourcepath !== clean_param($sourcepath, PARAM_PATH)) {
            throw new eset1_convert_exception('file_invalid_path', $sourcepath);
        }

        $sourcefullpath = $this->basepath.'/'.$sourcepath;

        if (!is_readable($sourcefullpath)) {
            throw new eset1_convert_exception('file_not_readable', $sourcefullpath);
        }
        ...
        if (core_text::strlen($filepath) > 255) {
            throw new eset1_convert_exception('file_path_longer_than_255_chars');
        }
        ...
        if ($filename === '') {
            throw new eset1_convert_exception('unsupported_chars_in_filename');
        }
    ...
    public function migrate_directory($rootpath, $relpath='/') {
        ...
        if (substr($rootpath, -1) === '/') {
            debugging('eset1_file_manager::migrate_directory() expects $rootpath without the trailing slash', DEBUG_DEVELOPER);
        ...
        foreach ($items as $item) {
            ...
            if ($item->isLink()) {
                throw new eset1_convert_exception('unexpected_symlink');
            }
    ...
    protected function add_file_to_pool($pathname) {
        ...
        if (!is_readable($pathname)) {
            throw new eset1_convert_exception('file_not_readable');
        }
        ...
        if (file_exists($hashfile)) {
            if (filesize($hashfile) !== $filesize) {
                ...
                throw new eset1_convert_exception('same_hash_different_size');
            }
            $newfile = false;
            ...
            if (!copy($pathname, $hashfile)) {
                throw new eset1_convert_exception('unable_to_copy_file');
            }

            if (filesize($hashfile) !== $filesize) {
                throw new eset1_convert_exception('filesize_different_after_copy');
            }
        }
...
class eset1_inforef_manager {
     ...
     *
     * @param eset1_converter $converter
     * @param string $name the name of the annotator (like course, section, activity, block)
     * @param int $id the id of the annotator if required
     */
    public function __construct(eset1_converter $converter, $name, $id = 0) {
        $this->annotator   = $name;
        $this->annotatorid = $id;
    }
