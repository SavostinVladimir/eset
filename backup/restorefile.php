...
/**
 * Import backup file or select existing backup file from eset
 * @package   esetcore
...
$url = new eset_url('/backup/restorefile.php', array('contextid'=>$contextid));
...
require_capability('eset/restore:restorecourse', $context);
...
if ($action == 'choosebackupfile') {
    if ($filearea == 'automated') {
        require_capability('eset/restore:viewautomatedfilearea', $context);
    }
    if ($fileinfo = $browser->get_file_info($filecontext, $component, $filearea, $itemid, $filepath, $filename)) {
        if (is_a($fileinfo, 'file_info_stored')) {
            ...
            $restore_url = new eset_url('/backup/restore.php', array('contextid' => $contextid,
            ...
            $restore_url = new eset_url('/backup/restore.php', array(
...
$data = $form->get_data();
if ($data && has_capability('eset/restore:uploadfile', $context)) {
    $filename = restore_controller::get_tempdir_name($courseid, $USER->id);
    $pathname = $tmpdir . '/' . $filename;
    if (!$form->save_file('backupfile', $pathname)) {
        throw new restore_ui_exception('errorcopyingbackupfile', null, $pathname);
    }
    $restore_url = new eset_url('/backup/restore.php', array('contextid'=>$contextid, 'filename'=>$filename));
    redirect($restore_url);
    die;
}
...
if (has_capability('eset/restore:uploadfile', $context)) {
