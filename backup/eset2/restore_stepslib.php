...
class restore_gradebook_structure_step extends restore_structure_step {
     ...
     */
     protected function execute_condition() {
        ...
        $backupbuild = 0;
        preg_match('/(\d{8})/', $this->get_task()->get_info()->eset_release, $matches);
        if (!empty($matches[1])) {
            $backupbuild = (int) $matches[1]; // The date of Eset build at the time of the backup.
        }
    ...
    protected function check_minmaxtouse() {
        ...
        if ($minmaxtouse === false &&
            ...
            $version = $this->get_task()->get_info()->eset_version;
...
class restore_groups_structure_step extends restore_structure_step {
    ...
    public function process_group($data) {
        ...
        if (isset($data->idnumber) and has_capability('eset/course:changeidnumber', $context, $this->task->get_userid())) {
...
class restore_scales_structure_step extends restore_structure_step {
    ...
    protected function process_scale($data) {
        ...
        if (!$scadb = $DB->get_record_sql("SELECT *
            ...
            if ($data->courseid == 0 && !has_capability('eset/course:managescales', $systemctx , $this->task->get_userid())) {
                $data->courseid = $this->get_courseid();
            }
...
class restore_outcomes_structure_step extends restore_structure_step {
    ...
    protected function process_outcome($data) {
        ...
        if (!$outdb = $DB->get_record_sql('SELECT *
            ...
            $systemctx = context_system::instance();
            if (is_null($data->courseid) && !has_capability('eset/grade:manageoutcomes', $systemctx , $this->task->get_userid())) {
                $data->courseid = $this->get_courseid();
            }
...
class restore_section_structure_step extends restore_structure_step {
    ...
    public function process_section($data) {
        ...
        // by a lot of people to "hide" sections on purpose (so this remains as used to be in Eset 1.x)
     ...
     * in Eset 2.7+ but we still support restore.
     *
     ...
     * exist in Eset 2.7+ but we still support restore.
     *
     * @param stdClass $data Record data
     */
...
class restore_course_structure_step extends restore_structure_step {
    ...
    public function process_course($data) {
        ...
        $canchangeidnumber = $isnewcourse || has_capability('eset/course:changeidnumber', $context, $userid);
        $canchangesummary = $isnewcourse || has_capability('eset/course:changesummary', $context, $userid);
        $canforcelanguage = has_capability('eset/course:setforcedlanguage', $context, $userid);
    ...
    protected function after_execute() {
        ...
        if ($this->legacyrestrictmodules) {
            ...
            list($roleids) = get_roles_with_cap_in_context($context, 'eset/course:manageactivities');
            list($managerroleids) = get_roles_with_cap_in_context($context, 'eset/site:config');
...
class restore_fix_restorer_access_step extends restore_execution_step {
    protected function define_execution() {
        ...
        if (is_enrolled($context, $userid, 'eset/course:update', true) or is_viewing($context, $userid, 'eset/course:update')) {
            // Current user may access the course (admin, category manager or restored teacher enrolment usually)
            return;
        }

        // Try to add role only - we do not need enrolment if user has eset/course:view or is already enrolled
...
class restore_move_module_questions_categories extends restore_execution_step {
    ...
    protected function define_execution() {
        ...
        preg_match('/(\d{8})/', $this->task->get_info()->eset_release, $matches);
        $backupbuild = (int)$matches[1];
        $after35 = false;
        if (version_compare($backuprelease, '3.5', '>=') && $backupbuild > 20180205) {
            $after35 = true;
 ...
 *
 * @copyright 2012 David Mudrak <david@eset.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
        ...
        foreach ($rs as $record) {
            ...
            if ($info->oldfile->repositorytype === 'local' or $info->oldfile->repositorytype === 'coursefiles') {
                ...
                if ($source) {
                    ...
                    // This is a reference to some eset file that was not contained in the backup
...
abstract class restore_questions_activity_structure_step extends restore_activity_structure_step {
    ...
    /**
     * Attach below $element (usually attempts) the needed restore_path_elements
     * to restore question attempt data from Eset 2.0.
     *
