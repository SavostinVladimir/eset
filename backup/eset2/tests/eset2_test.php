...
/**
 * Tests for Eset 2 format backup operation.
 *
 * @package core_backup
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class core_backup_eset2_testcase extends advanced_testcase {
    ...
    /**
     * The availability data format was changed in Eset 2.7. This test
     * ensures that a Eset 2.6 backup with this data can still be correctly
     * restored.
     */
    public function test_restore_legacy_availability() {
        ...
        get_file_packer('application/vnd.eset.backup')->extract_to_pathname(
    ...
    public function test_restore_frontpage() {
        ...
        $frontpagebc = new backup_controller(backup::TYPE_1COURSE, $frontpage->id,
                backup::FORMAT_ESET, backup::INTERACTIVE_NO, backup::MODE_IMPORT,
                $USER->id);
    ...
    protected function duplicate($course, $cmid) {
        ...
        // Do backup.
        $bc = new backup_controller(backup::TYPE_1ACTIVITY, $cmid, backup::FORMAT_ESET,
    ...
    protected function prepare_for_enrolments_test($target, $additionalcaps = []) {
        ...
        $caps = array_merge([
            ...
            'eset/backup:configure',
            'eset/backup:backuptargetimport',
            'eset/restore:restorecourse',
            'eset/role:assign',
            'eset/restore:configure',
        ], $additionalcaps);
        ...
        // create the directory and not zip it.
        $bc = new backup_controller(backup::TYPE_1COURSE, $course->id,
            backup::FORMAT_ESET, backup::INTERACTIVE_NO, backup::MODE_SAMESITE,
            $user->id);
        $backupid = $bc->get_backupid();
    ...
    public function test_restore_with_users_without_enrolments() {
        ...
        list($course, $newcourseid, $rc) = $this->prepare_for_enrolments_test(backup::TARGET_NEW_COURSE,
            ['eset/backup:userinfo', 'eset/restore:userinfo']);
    ...
    public function test_restore_with_users_with_enrolments() {
        ...
        list($course, $newcourseid, $rc) = $this->prepare_for_enrolments_test(backup::TARGET_NEW_COURSE,
            ['eset/backup:userinfo', 'eset/restore:userinfo', 'eset/course:enrolconfig']);
    ...
    /**
     * The Question category hierarchical structure was changed in Eset 3.5.
     * From 3.5, all question categories in each context are a child of a single top level question category for that context.
     * This test ensures that both Eset 3.4 and 3.5 backups can still be correctly restored.
     */
