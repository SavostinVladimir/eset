...
class backup_course_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        // Add 'numsections' in order to be able to restore in previous versions of Eset.
        // Even though Eset does not officially support restore into older verions of Eset from the
 ...
 * using one specialised nested_element, will copy them form eset storage
 * to backup storage
 */
...
/**
 * Structure step in charge of creating the main eset_backup.xml file
...
class backup_main_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $info['eset_version'] = $CFG->version;
        $info['eset_release'] = $CFG->release;
        ...
        list($dinfo, $cinfo, $sinfo) = backup_controller_dbops::get_eset_backup_information(
                $this->get_backupid(), $this->get_task()->get_progress());

        // Define elements

        $eset_backup = new backup_nested_element('eset_backup');

        $information = new backup_nested_element('information', null, array(
            'name', 'eset_version', 'eset_release', 'backup_version',
        ...
        $eset_backup->add_child($information);
        ...
        // Prepare some information to be sent to main eset_backup.xml file
        return $eset_backup;
    }
...
class backup_zip_contents extends backup_execution_step implements file_progress {
    ...
    protected function define_execution() {
        ...
        if (file_exists($logfilepath)) {
             $files['eset_backup.log'] = $logfilepath;
        }
        ...
        // Get the zip packer
        $zippacker = get_file_packer('application/vnd.eset.backup');
