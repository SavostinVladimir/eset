...
require_capability('eset/restore:restoretargetimport', $context);

// Set up the page
$PAGE->set_title($course->shortname . ': ' . get_string('import'));
$PAGE->set_heading($course->fullname);
$PAGE->set_url(new eset_url('/backup/import.php', array('id'=>$courseid)));
...
if ($importcourseid === false || $searchcourses) {
    ...
    $url = new eset_url('/backup/import.php', array('id'=>$courseid));
...
require_capability('eset/backup:backuptargetimport', $importcontext);
...
if (!($bc = backup_ui::load_controller($backupid))) {
    $bc = new backup_controller(backup::TYPE_1COURSE, $importcourse->id, backup::FORMAT_ESET,
    ...
    if (!$rc->execute_precheck()) {
        ...
        if (is_array($precheckresults)) {
            if (!empty($precheckresults['errors'])) { // If errors are found, terminate the import.
                ...
                echo $OUTPUT->continue_button(new eset_url('/course/view.php', array('id'=>$course->id)));
                echo $OUTPUT->footer();
                die();
            }
