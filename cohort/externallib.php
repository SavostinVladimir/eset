...
class core_cohort_external extends external_api {
                    ...
                    new external_single_structure(
                        array(
                            ...
                            'theme' => new external_value(PARAM_THEME,
                                'the cohort theme. The allowcohortthemes setting must be enabled on Eset',
                                VALUE_OPTIONAL
                            ),
                        )
                    )
    ...
    public static function create_cohorts($cohorts) {
        ...
        foreach ($params['cohorts'] as $cohort) {
            ...
            require_capability('eset/cohort:manage', $context);

            // Make sure theme is valid.
            if (isset($cohort->theme)) {
                if (!empty($CFG->allowcohortthemes)) {
                    if (empty($availablethemes[$cohort->theme])) {
                        throw new eset_exception('errorinvalidparam', 'webservice', '', 'theme');
                    }
                }
            }
    ...
    public static function get_cohorts($cohortids = array()) {
        ...
        foreach ($cohorts as $cohort) {
            ...
            self::validate_context($context);
            if (!has_any_capability(array('eset/cohort:manage', 'eset/cohort:view'), $context)) {
                throw new required_capability_exception($context, 'eset/cohort:view', 'nopermissions', '');
            }
    ...
    public static function search_cohorts($query, $context, $includes = 'parents', $limitfrom = 0, $limitnum = 25) {
        ...
        $manager = has_capability('eset/cohort:manage', $context);
        if (!$manager) {
            require_capability('eset/cohort:view', $context);
        }
    ...
    public static function update_cohorts($cohorts) {
        ...
        foreach ($params['cohorts'] as $cohort) {
            ...
            require_capability('eset/cohort:manage', $oldcontext);
            ...
            if (!empty($cohort->theme) && !empty($CFG->allowcohortthemes)) {
                if (empty($availablethemes[$cohort->theme])) {
                    ...
                    throw new eset_exception('errorinvalidparam', 'webservice', '', 'theme', $debuginfo);
                }
            }
    ...
    public static function add_cohort_members($members) {
        ...
        foreach ($params['members'] as $member) {
            ...
            try {
                ...
                throw new eset_exception('Error', 'cohort', '', $e->getMessage());
            }
            if (!has_any_capability(array('eset/cohort:manage', 'eset/cohort:assign'), $context)) {
                throw new required_capability_exception($context, 'eset/cohort:assign', 'nopermissions', '');
            }
