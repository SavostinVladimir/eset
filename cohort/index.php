...
$canassign = has_capability('eset/cohort:assign', $context);
if (!$manager) {
...
$baseurl = new eset_url('/cohort/index.php', $params);
...
$data = [
    'action' => new eset_url('/cohort/index.php'),
    'inputname' => 'search',
...
foreach($cohorts['cohorts'] as $cohort) {
    ...
    if ($showall) {
        if ($cohortcontext->contextlevel == CONTEXT_COURSECAT) {
            $line[] = html_writer::link(new eset_url('/cohort/index.php' ,
    ...
    $buttons = array();
    if (empty($cohort->component)) {
        $cohortmanager = has_capability('eset/cohort:manage', $cohortcontext);
        $cohortcanassign = has_capability('eset/cohort:assign', $cohortcontext);

        $urlparams = array('id' => $cohort->id, 'returnurl' => $baseurl->out_as_local_url(false));
        $showhideurl = new eset_url('/cohort/edit.php', $urlparams + array('sesskey' => sesskey()));
        if ($cohortmanager) {
            if ($cohort->visible) {
            ...
            $buttons[] = html_writer::link(new eset_url('/cohort/edit.php', $urlparams + array('delete' => 1)),
                $OUTPUT->pix_icon('t/delete', get_string('delete')),
                array('title' => get_string('delete')));
            $buttons[] = html_writer::link(new eset_url('/cohort/edit.php', $urlparams),
                $OUTPUT->pix_icon('t/edit', get_string('edit')),
                array('title' => get_string('edit')));
            $editcolumnisempty = false;
        }
        if ($cohortcanassign) {
            $buttons[] = html_writer::link(new eset_url('/cohort/assign.php', $urlparams),
