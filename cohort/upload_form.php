 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cohort_upload_form extends esetform {
    ...
    protected function get_context_options() {
        if ($this->contextoptions === null) {
            ...
            $displaylist = core_course_category::make_categories_list('eset/cohort:manage');
            // We need to index the options array by context id instead of category id and add option for system context.
            $syscontext = context_system::instance();
            if (has_capability('eset/cohort:manage', $syscontext)) {
    ...
    protected function get_cohort_file($draftid) {
        global $USER;
        // We can not use esetform::get_file_content() method because we need the content before the form is validated.
        if (!$draftid) {
            return null;
        }
        $fs = get_file_storage();
    ...
    protected function preview_uploaded_cohorts() {
        ...
        foreach ($previewdrows as $idx) {
            ...
            foreach ($columns as $key => $value) {
                if ($key === 'contextid') {
                    $text = html_writer::link(new eset_url('/cohort/index.php', array('contextid' => $context->id)),
