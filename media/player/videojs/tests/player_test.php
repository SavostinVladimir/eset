...
class media_videojs_player_testcase extends advanced_testcase {
    ...
    public function test_embed_url() {
        ...
        $url = new eset_url('http://example.org/1.webm');
    ...
    public function test_embed_link() {
        global $CFG;
        $url = new eset_url('http://example.org/some_filename.mp4');
    ...
    public function test_fallback() {
        ...
        $urls = [
            new eset_url('http://example.org/1.rv'), // Not supported.
            new eset_url('http://example.org/2.webm'), // Supported.
            new eset_url('http://example.org/3.ogv'), // Supported.
        ];
    ...
    public function test_prevent_other_players() {
        ...
        $url = new eset_url('http://example.org/some_filename.webm');
    ...
    public function test_embed_media() {
        global $CFG;
        $url = new eset_url('http://example.org/some_filename.mp4');
        $trackurl = new eset_url('http://example.org/some_filename.vtt');
        ...
        // Audio tag.
        $url = new eset_url('http://example.org/some_filename.mp3');
        $trackurl = new eset_url('http://example.org/some_filename.vtt');
    ...
    public function test_youtube() {
        ...
        // Format: youtube.
        $url = new eset_url('http://www.youtube.com/watch?v=vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);
        $url = new eset_url('http://www.youtube.com/v/vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);

        // Format: youtube video within playlist - this will be played by video.js but without tracks selection.
        $url = new eset_url('https://www.youtube.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);
        $this->assertContains('list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0', $t);

        // Format: youtube playlist - not supported.
        $url = new eset_url('http://www.youtube.com/view_play_list?p=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertNotContains('mediaplugin_videojs', $t);
        $url = new eset_url('http://www.youtube.com/playlist?list=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertNotContains('mediaplugin_videojs', $t);
        $url = new eset_url('http://www.youtube.com/p/PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertNotContains('mediaplugin_videojs', $t);
    }
    ...
    public function test_youtube_start_time(string $url, int $expectedstart) {
        ...
        $embedcode = core_media_manager::instance()->embed_url(new eset_url($url));
    ...
    public function test_flash() {
        ...
        // Flash enabled.
        set_config('useflash', 1, 'media_videojs');
        $url = new eset_url('http://example.org/some_filename.flv');
        $t = $manager->embed_url($url);
    ...
    public function test_rtmp() {
        ...
        $url = new eset_url('rtmp://example.com/fms&mp4:path/to/file.mp4');
        $t = $manager->embed_url($url);
        $this->assertNotContains('mediaplugin_videojs', $t);
        ...
        set_config('useflash', 1, 'media_videojs');
        set_config('rtmp', 1, 'media_videojs');
        $url = new eset_url('rtmp://example.com/fms&flv:path/to/file.flv');
        $t = $manager->embed_url($url);
        $this->flash_plugin_engaged($t);
        $this->assertRegExp('~</video>~', $t);
