...
class media_youtube_testcase extends advanced_testcase {
    ...
    public function test_supported() {
        ...
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $url = new eset_url('http://m.youtube.com/watch?v=vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        ...
        $url = new eset_url('https://www.youtube.com/watch?v=JNJMF1l3udM&t=1h11s');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertContains('start=3611', $t);

        // Format: youtube video within playlist with start time.
        $url = new eset_url('https://www.youtube.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0&t=1m5s');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertContains('list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0', $t);
        ...
        $url = new eset_url('https://www.youtube.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_">');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
        $this->assertNotContains('list=PLxcO_', $t); // We shouldn't get a list param as input was invalid.
        $url = new eset_url('https://www.youtube.com/watch?v=JNJMF1l3udM&t=">');
        $t = $manager->embed_url($url);
        $this->assertContains('</iframe>', $t);
