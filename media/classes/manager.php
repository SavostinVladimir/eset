...
final class core_media_manager {
    ...
    /** @var eset_page page this instance was initialised for */
    private $page;

    /**
     * Returns a singleton instance of a manager
     *
     * Note as of Eset 3.3, this will call setup for you.
     *
     ...
     *
     * @param eset_page $page The page we are going to add requirements to.
    ...
    /**
     * @deprecated since Eset 3.3. The setup is now done in ::instance() so there is no need to call this.
     */
     ...
     *
     * @param eset_url $url Full URL of media file
     ...
     */
    public function embed_url(eset_url $url, $name = '', $width = 0, $height = 0,
                              $options = array()) {
        ...
        if (preg_match('/[?#]d=([\d]{1,4}%?)x([\d]{1,4}%?)/', $rawurl, $matches)) {
            $width = $matches[1];
            $height = $matches[2];
            $url = new eset_url(str_replace($matches[0], '', $rawurl));
        }
     ...
     *
     * @param array $alternatives Array of eset_url to media files
     * @param string $name Optional user-readable name to display in download link
     * @param int $width Width in pixels (optional)
     * @param int $height Height in pixels (optional)
     * @param array $options Array of key/value pairs
     ...
     *
     * @param eset_url $url URL of media file
     * @param array $options Options (same as when embedding)
     * @return bool True if file can be embedded
     */
    public function can_embed_url(eset_url $url, $options = array()) {
     ...
     * link.
     *
     * @param array $urls URL of media file and any alternatives (eset_url)
     * @param array $options Options (same as when embedding)
     * @return bool True if file can be embedded
     */
     ...
     * it into an array of eset_url objects suitable for using when calling
     * embed_alternatives.
     *
     ...
     * @return array Array of 1 or more eset_url objects
     */
    public function split_alternatives($combinedurl, &$width, &$height) {
        ...
        foreach ($urls as $url) {
            ...
            // Turn it into eset_url object.
            $returnurls[] = new eset_url($url);
        }
     ...
     * @param eset_url $url URL
     */
    public function get_extension(eset_url $url) {
    ...
    /**
     * Obtains the filename from the eset_url.
     * @param eset_url $url URL
     * @return string Filename only (not escaped)
     */
    public function get_filename(eset_url $url) {
    ...
    /**
     * Guesses MIME type for a eset_url based on file extension.
     * @param eset_url $url URL
     * @return string MIME type
     */
    public function get_mimetype(eset_url $url) {
