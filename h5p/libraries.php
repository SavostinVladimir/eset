 ...
 *
 * @package    core_h5p
 * @copyright  2019 Amaia Anabitarte <amaia@eset.com>
...
require_capability('eset/h5p:updatelibraries', $context);

$pagetitle = get_string('h5pmanage', 'core_h5p');
$url = new \eset_url("/h5p/libraries.php");
...
if ($deletelibrary) {
    ...
    if ($confirm) {
        ...
        redirect(new eset_url('/h5p/libraries.php'));
    }
    ...
    echo $OUTPUT->confirm(
        ...
        new eset_url($PAGE->url, ['deletelibrary' => $deletelibrary, 'confirm' => 1]),
        new eset_url('/h5p/libraries.php')
    );
    echo $OUTPUT->footer();
    die();
}
