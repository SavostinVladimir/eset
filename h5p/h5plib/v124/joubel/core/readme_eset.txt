...
2. As the mbstring extension is optional in Eset, the following changes have been hardcoded to the library:
...
4. Replace the $_SESSION references to $SESSION. That implies that the information is saved to backends, so only the Eset one should be used by core (core should be free from $_SESSION and always use $SESSION).
h5p.classes.php file:
...
The point 2 from above won't be needed once the mbstring extension becomes mandatory in Eset. A request has been
sent to MDL-65809.
