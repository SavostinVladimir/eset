...
class helper {
     ...
     * @param stored_file $file Eset file instance
    ...
    public static function can_deploy_package(\stored_file $file): bool {
        ...
        if (has_capability('eset/h5p:deploy', $context, $file->get_userid())) {
            return true;
        }
    ...
    public static function can_update_library(\stored_file $file): bool {
        ...
        $context = \context::instance_by_id($file->get_contextid());
        if (has_capability('eset/h5p:updatelibraries', $context, $file->get_userid())) {
            return true;
        }
        ...
        if ($task = \core\task\manager::get_scheduled_task('\core\task\h5p_get_content_types_task')) {
            ...
            if ($status && \core\task\manager::is_runnable() && get_config('tool_task', 'enablerunnow')) {
                $statusaction = \html_writer::link(
                    new \eset_url('/admin/tool/task/schedule_task.php',
     ...
     * @param \eset_url $link The URL to management page
    ...
    public static function get_core_assets(): array {
        ...
        foreach (core::$styles as $style) {
            ...
            $PAGE->requires->css(new \eset_url($liburl . $style . $cachebuster));
        }
        // Add core JavaScript.
     ...
     * @param  \eset_url $url The URL of the exported file.
     * @param  factory $factory The \core_h5p\factory object
     * @return array|null The information export file otherwise null.
     */
    public static function get_export_info(string $exportfilename, \eset_url $url = null, ?factory $factory = null): ?array {
