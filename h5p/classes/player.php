 ...
 *
 * @package    core_h5p
 * @copyright  2019 Sara Arjona <sara@eset.com>
...
class player {
     ...
     * @param string $component optional eset component to sent xAPI tracking
     */
    public function __construct(string $url, \stdClass $config, bool $preventredirect = true, string $component = '') {
        if (empty($url)) {
            throw new \eset_exception('h5pinvalidurl', 'core_h5p');
        }
    ...
    public static function display(string $url, \stdClass $config, bool $preventredirect = true,
        ...
        $fileurl = new \eset_url('/h5p/embed.php', $params);
    ...
    public function add_assets_to_page() {
        ...
        $contenturl = \eset_url::make_pluginfile_url($systemcontext->id, \core_h5p\file_storage::COMPONENT,
        ...
        $contentsettings = [
            ...
            'exportUrl'       => ($exporturl instanceof \eset_url) ? $exporturl->out(false) : '',
     ...
     *
     * @return \eset_url|null The URL of the exported file.
     */
    private function get_export_settings(bool $downloadenabled): ?\eset_url {
        ...
        if (strpos($this->url, '/webservice/pluginfile.php')) {
            $url  = \eset_url::make_webservice_pluginfile_url(
                $systemcontext->id,
    ...
    private function get_assets(): array {
        ...
        $settings['esetLibraryPaths'] = $this->core->get_dependency_roots($this->h5pid);
        // Add also the Eset component where the results will be tracked.
        $settings['esetComponent'] = $this->component;
        ...
        if ($this->embedtype === 'div') {
            ...
            // Schedule JavaScripts for loading through Eset.
            foreach ($files['scripts'] as $script) {
                ...
                $this->jsrequires[] = new \eset_url($isexternal ? $url : $CFG->wwwroot . $url);
            }

            // Schedule stylesheets for loading through Eset.
            foreach ($files['styles'] as $style) {
                ...
                $this->cssrequires[] = new \eset_url($isexternal ? $url : $CFG->wwwroot . $url);
            }
     ...
     * Get the encoded URL for embeding this H5P content.
     * @param  string $url The URL of the .h5p file.
     * @param string $component optional Eset component to send xAPI tracking
     *
     * @return \eset_url The embed URL.
     */
    public static function get_embed_url(string $url, string $component = ''): \eset_url {
        $params = ['url' => $url];
        if (!empty($component)) {
        ...
        return new \eset_url('/h5p/embed.php', $params);
    }
