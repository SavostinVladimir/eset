...
class core extends \H5PCore {
    ...
    public function get_dependency_roots(int $id): array {
        ...
        foreach ($dependencies as $dependency) {
            $library = $this->find_library($dependency);
            $roots[self::libraryToString($dependency, true)] = (eset_url::make_pluginfile_url(
                $context->id,
                'core_h5p',
                'libraries',
                $library->id,
    ...
    public static function get_scripts(): array {
        ...
        $urls[] = new eset_url("/h5p/js/h5p_overrides.js", [
            'ver' => $jsrev,
        ]);
     ...
     * Get H5P endpoints.
     *
     * If $endpoint = 'content' and $library is null, eset_url is the endpoint of the latest version of the H5P content
     * types; however, if $library is the machine name of a content type, eset_url is the endpoint to download the content type.
     ...
     * @return eset_url The endpoint eset_url object.
     */
    public function get_api_endpoint(?string $library = null, string $endpoint = 'content'): eset_url {
        if ($endpoint == 'site') {
