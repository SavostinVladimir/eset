...
class api {
    ...
    public static function create_content_from_pluginfile_url(string $url, \stdClass $config, factory $factory,
        ...
        if ($h5p) {
            ...
            if ($displayoptions != $h5p->displayoptions && has_capability('eset/h5p:setdisplayoptions', $context)) {
            ...
            if (!$h5pid && $file->get_userid() != $USER->id && has_capability('eset/h5p:updatelibraries', $context)) {
    ...
    protected static function get_pluginfile_hash(string $url, bool $preventredirect = true) {
        ...
        $url = new \eset_url(urldecode($url));
        ...
        try {
            ...
            throw new \eset_exception('invalidcontextid', 'core_h5p');
        }
        ...
        if ($context->contextlevel == CONTEXT_USER && $USER->id !== $context->instanceid) {
            throw new \eset_exception('h5pprivatefile', 'core_h5p');
        }
