...
abstract class handler {
     ...
     */
    public static function get_h5p_core_library_url(?string $filepath = null, ?array $params = null): ?\eset_url {
        return new \eset_url(static::get_h5p_core_library_base($filepath), $params);
    }
     ...
     * @param string $filepath The path within the h5p root.
     * @param array $params These params override current params or add new.
     * @return null|\eset_url The eset_url to a file in the H5P Editor library.
     */
    public static function get_h5p_editor_library_url(?string $filepath = null, ?array $params = null): ?\eset_url {
        return new \eset_url(static::get_h5p_editor_library_base($filepath), $params);
    }
    ...
    public static function get_h5p_string(string $identifier, string $language): ?string {
        ...
        // Composed code languages, such as 'Spanish, Mexican' are different in H5P and Eset:
        // - In H5P, they use '-' to separate language from the country. For instance: es-mx.
        // - However, in Eset, they have '_' instead of '-'. For instance: es_mx.
        $language = str_replace('-', '_', $language);
        if (get_string_manager()->string_exists($identifier, $component)) {
            $defaultesetlang = 'en';
            // In Eset, all the English strings always will exist because they have to be declared in order to let users
            ...
            if ($language === $defaultesetlang || $string->out($language) !== $string->out($defaultesetlang)) {
