...
class core_files_renderer extends plugin_renderer_base {
    ...
    public function render_files_tree_viewer(files_tree_viewer $tree) {
        $html = $this->output->heading_with_help(get_string('coursefiles'), 'courselegacyfiles', 'eset');
        ...
        $html .= $this->output->single_button(new eset_url('/files/coursefilesedit.php', array('contextid'=>$tree->context->id)), get_string('coursefilesedit'), 'get');
    ...
    public function render_form_filemanager($fm) {
        ...
        $module = array(
            ...
            'requires' => array('eset-core-notification-dialogue', 'core_filepicker', 'base', 'io-base', 'node', 'json', 'core_dndupload', 'panel', 'resize-plugin', 'dd-plugin'),
            'strings' => array(
                array('error', 'eset'), array('info', 'eset'), array('confirmdeletefile', 'repository'),
                ...
                array('confirmrenamefile', 'repository'), array('newfolder', 'repository'), array('edit', 'eset'),
                ...
                ['confirmdeleteselectedfile', 'repository'], ['selectall', 'eset'], ['deselectall', 'eset'],
...
<div class="filemanager fp-mkdir-dlg" role="dialog" aria-live="assertive" aria-labelledby="fp-mkdir-dlg-title">
    ...
    protected function fm_print_restrictions($fm) {
        ...
        if ($hasmaxfiles && $hasarealimit) {
            $maxsize = get_string('maxsizeandattachmentsandareasize', 'eset', $strparam);
        } else if ($hasmaxfiles) {
            $maxsize = get_string('maxsizeandattachments', 'eset', $strparam);
        } else if ($hasarealimit) {
            $maxsize = get_string('maxsizeandareasize', 'eset', $strparam);
        } else {
            $maxsize = get_string('maxfilesize', 'eset', $maxbytes);
        }
...
<div class="file-picker fp-msg" role="alertdialog" aria-live="assertive" aria-labelledby="fp-msg-labelledby">
    ...
    protected function create_license_help_icon_context() : stdClass {
        ...
        $iconcontext->alt = get_string('helpprefix2', 'eset', get_string('chooselicense', 'repository'));
...
/**
 * Data structure representing a general eset file tree viewer
 *
 * @copyright 2010 Dongsheng Cai
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Eset 2.0
...
class files_tree_viewer implements renderable {
    ...
    /**
     * Constructor of eset_file_tree_viewer class
    ...
    public function __construct(file_info $file_info, array $options = null) {
        ...
        while ($level) {
            ...
            unset($params['itemid']);
            $url = new eset_url('/files/index.php', $params);
