 ...
 * @copyright 2018 David Matamoros <davidmc@eset.com>
...
abstract class handler {
    ...
    public static function get_handler(string $component, string $area, int $itemid = 0) : handler {
        ...
        throw new \eset_exception('unknownhandler', 'core_customfield', (object)$a);
    }
     ...
     * @return \eset_url
     */
    abstract public function get_configuration_url() : \eset_url;
    ...
    protected function validate_category(category_controller $category) : category_controller {
        ...
        if (!array_key_exists($category->get('id'), $categories)) {
            throw new \eset_exception('categorynotfound', 'core_customfield');
        }
    ...
    protected function validate_field(field_controller $field) : field_controller {
        if (!array_key_exists($field->get('categoryid'), $this->get_categories_with_fields())) {
            throw new \eset_exception('fieldnotfound', 'core_customfield');
        }
     ...
     *
     * This method is called from esetform::definition_after_data and allows to tweak
     * mform with some data coming directly from the field plugin data controller.
     *
     * @param \EsetQuickForm $mform
     * @param int $instanceid
     */
    public function instance_form_definition_after_data(\EsetQuickForm $mform, int $instanceid = 0) {
    ...
    /**
     * Validates the given data for custom fields, used in esetform validation() function
     *
     * Example:
     ...
     */
    public function instance_form_definition(\EsetQuickForm $mform, int $instanceid = 0,
     ...
     * @param \EsetQuickForm $mform
     */
    public function config_form_definition(\EsetQuickForm $mform) {
    }
    ...
    public function setup_edit_page(field_controller $field) : string {
        ...
        $url = new \eset_url('/customfield/edit.php',
