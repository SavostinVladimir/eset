...
/**
 * Library functions to facilitate the use of JavaScript in Eset.
 *
...
class page_requirements_manager {
    ...
    /**
     * @var array of eset_url Theme sheets, initialised only from core_renderer
     */
    protected $cssthemeurls = array();

    /**
     * @var array of eset_url List of custom theme sheets, these are strongly discouraged!
    ...
    public function __construct() {
        ...
        // For important information on patching YUI modules, please see http://docs.eset.org/dev/YUI/Patching.
        ...
        $configname = $this->YUI_config->set_config_source('lib/yui/config/eset.js');
        $this->YUI_config->add_group('eset', array(
            'name' => 'eset',
        ...
        if ($CFG->debugdeveloper) {
            ...
            // However we do want the DEBUG versions of our Eset-specific modules.
            // To debug a YUI-specific issue, change the yui3loader->filter value to DEBUG.
            $this->YUI_config->filter = 'RAW';
            $this->YUI_config->groups['eset']['filter'] = 'DEBUG';
            ...
            $this->YUI_config->groups['eset']['filter'] = null;
        ...
        // Add the eset group's module data.
        $this->YUI_config->add_eset_metadata();
     ...
     */
    public function get_config_for_javascript(eset_page $page, renderer_base $renderer) {
        global $CFG;
    ...
    /**
     * Initialise with the bits of JavaScript that every Eset page should have.
     *
     * @param eset_page $page
     * @param core_renderer $renderer
     */
    protected function init_requirements_data(eset_page $page, core_renderer $renderer) {
        global $CFG;
        ...
        $this->string_for_js('cancel', 'eset');
        $this->string_for_js('yes', 'eset');
        ...
        if ($page->user_is_editing()) {
            ...
            $this->strings_for_js(array('movecontent',
                                        ...
                                        'emptydragdropregion'),
                                  'eset');
            $page->requires->yui_module('eset-core-blocks', 'M.core_blocks.init_dragdrop', array($params), null, true);
        }
     ...
     * to once.
     *
     * @param string|eset_url $url The path to the .js file, relative to $CFG->dirroot / $CFG->wwwroot.
     *      For example '/mod/mymod/customscripts.js'; use eset_url for external scripts
     ...
     *
     * NOTE: this should not be used in official Eset distribution!
     *
     * {@see http://docs.eset.org/dev/jQuery}
     */
     ...
     *   function theme_yyy_page_init(eset_page $page) {
     *       $page->requires->jquery();
     *       $page->requires->jquery_plugin('ui');
     *       $page->requires->jquery_plugin('ui-css');
     *   }
    ...
    public function jquery_plugin($plugin, $component = 'core') {
        ...
        if ($component !== 'core' and in_array($plugin, array('jquery', 'ui', 'ui-css'))) {
            debugging("jQuery plugin '$plugin' is included in Eset core, other components can not use the same name.", DEBUG_DEVELOPER);
            $component = 'core';
        ...
        if (isset($this->jqueryplugins[$plugin])) {
            // No problem, we already have something, first Eset plugin to register the jQuery plugin wins.
            return true;
        }
        ...
        foreach ($plugins[$plugin]['files'] as $file) {
            ...
            if (!empty($CFG->slasharguments)) {
                $url = new eset_url("/theme/jquery.php");
                ...
                if (strpos($path, $CFG->dirroot) === 0) {
                    ...
                    debugging("Eset jQuery integration requires 'slasharguments' setting to be enabled.");
                    continue;
                }
            }
     ...
     *
     * @param eset_url|string $url full eset url, or shortened path to script
     * @return eset_url
     */
    protected function js_fix_url($url) {
        ...
        if ($url instanceof eset_url) {
            ...
            if (substr($url, -3) === '.js') {
                $jsrev = $this->get_jsrev();
                if (empty($CFG->slasharguments)) {
                    return new eset_url('/lib/javascript.php', array('rev'=>$jsrev, 'jsfile'=>$url));
                } else {
                    $returnurl = new eset_url('/lib/javascript.php');
                    $returnurl->set_slashargument('/'.$jsrev.$url);
                    return $returnurl;
                }
            } else {
                return new eset_url($url);
            }
        } else {
            throw new coding_exception('Invalid JS url, it has to be shortened url starting with / or eset_url instance.', $url);
        }
    }
            ...
            switch($component) {
                case 'core_filepicker':
                    $module = array('name'     => 'core_filepicker',
                                    ...
                                    'requires' => array(
                                        ...
                                        'escape', 'eset-core_filepicker', 'eset-core-notification-dialogue'
                                    ),
                                    'strings'  => array(array('lastmodified', 'eset'), array('name', 'eset'), array('type', 'repository'), array('size', 'repository'),
                                                        array('invalidjson', 'repository'), array('error', 'eset'), array('info', 'eset'),
                ...
                case 'core_comment':
                    $module = array('name'     => 'core_comment',
                                    ...
                                    'strings' => array(array('confirmdeletecomments', 'admin'), array('yes', 'eset'), array('no', 'eset'))
                                );
                    break;
                case 'core_role':
                ...
                case 'core_dndupload':
                    $module = array('name'     => 'core_dndupload',
                                    ...
                                    'strings'  => array(array('uploadformlimit', 'eset'), array('droptoupload', 'eset'), array('maxfilesreached', 'eset'),
                                                        array('dndenabled_inbox', 'eset'), array('fileexists', 'eset'), array('maxbytesfile', 'error'),
                                                        array('sizegb', 'eset'), array('sizemb', 'eset'), array('sizekb', 'eset'), array('sizeb', 'eset'),
                                                        array('maxareabytesreached', 'eset'), array('serverconnection', 'error'),
                                                        array('changesmadereallygoaway', 'eset'), array('complete', 'eset')
                                                    ));
                    break;
            }
    ...
    public function js_module($module) {
        ...
        if (!empty($module['strings'])) {
            foreach ($module['strings'] as $string) {
                $identifier = $string[0];
                $component = isset($string[1]) ? $string[1] : 'eset';
        ...
        // eset modules to require each other.
    ...
    public function css($stylesheet) {
        ...
        if ($stylesheet instanceof eset_url) {
            // ok
        } else if (strpos($stylesheet, '/') === 0) {
            $stylesheet = new eset_url($stylesheet);
        } else {
     ...
     *
     * @param eset_url $stylesheet
     * @return void
     */
    public function css_theme(eset_url $stylesheet) {
    ...
    public function yui_module($modules, $function, array $arguments = null, $galleryversion = null, $ondomready = false) {
        ...
        if ($galleryversion != null) {
            debugging('The galleryversion parameter to yui_module has been deprecated since Eset 2.3.');
        }
     ...
     * So, for example, after a call to $PAGE->requires->string_for_js('course', 'eset');
     * then the JavaScript variable M.str.eset.course will be 'Course', or the
     ...
     *     $PAGE->requires->string_for_js('fullnamedisplay', 'eset', $USER);
     *     // Use the result of the substitution in Javascript.
     *     alert(M.str.eset.fullnamedisplay);
     *
     ...
     *     $PAGE->requires->string_for_js('fullnamedisplay', 'eset');
     *     // Provide the values on the fly in Javascript.
     *     user = { firstname : 'Harry', lastname : 'Potter' }
     *     alert(M.util.get_string('fullnamedisplay', 'eset', user);
     *
     ...
     * <pre>
     *      $PAGE->requires->data_for_js('mydata', array('name' => 'Eset'));
     * </pre>
     * then in JavsScript mydata.name will be 'Eset'.
     *
     * @deprecated
    ...
    protected function get_amd_footercode() {
        ...
        $jsloader = new eset_url('/lib/javascript.php');
        $jsloader->set_slashargument('/' . $jsrev . '/');
        $requirejsloader = new eset_url('/lib/requirejs.php');
        $requirejsloader->set_slashargument('/' . $jsrev . '/');

        $requirejsconfig = file_get_contents($CFG->dirroot . '/lib/requirejs/eset-config.js');
    ...
    protected function get_yui3lib_headcss() {
        ...
        if ($this->yui3loader->combine) {
            ...
            $code .= '<link rel="stylesheet" type="text/css" href="'.$this->yui3loader->local_comboBase.'rollup/'.$CFG->yui3version.'/yui-esetsimple' . $yuiformat . '.css" />';
    ...
    protected function get_yui3lib_headcode() {
        ...
        $format = '-min';
        if ($this->YUI_config->groups['eset']['filter'] === 'DEBUG') {
            $format = '-debug';
        }
        ...
        $baserollups = array(
            'rollup/' . $rollupversion . "/yui-esetsimple{$yuiformat}.js",
        );
    ...
    protected function get_css_code() {
        ...
        // Add the YUI code first. We want this to be overridden by any Eset CSS.
     ...
     */
    public function get_head_code(eset_page $page, core_renderer $renderer) {
        global $CFG;
        ...
        // Add a subset of Eset configuration to the M namespace.
    ...
    public function get_top_of_body_code(renderer_base $renderer) {
        ...
        // Add hacked jQuery support, it is not intended for standard Eset distribution!
...
class YUI_config {
    ...
    public function update_group($name, $config) {
        if (!isset($this->groups[$name])) {
            throw new coding_exception('The Eset YUI module does not exist. You must define the eset module config using YUI_config->add_module_config first.');
        }
        $this->groups[$name] = $config;
    }
    ...
    /**
     * Add the eset YUI module metadata for the eset group to the YUI_config instance.
     *
     ...
     * @return void
     */
    public function add_eset_metadata() {
        global $CFG;
        if (!isset($this->groups['eset'])) {
            throw new coding_exception('The Eset YUI module does not exist. You must define the eset module config using YUI_config->add_module_config first.');
        }

        if (!isset($this->groups['eset']['modules'])) {
            $this->groups['eset']['modules'] = array();
        }

        $cache = cache::make('core', 'yuimodules');
        if (!isset($CFG->jsrev) || $CFG->jsrev == -1) {
            $metadata = array();
            $metadata = $this->get_eset_metadata();
        ...
        $this->groups['eset']['modules'] = array_merge($this->groups['eset']['modules'],
                $metadata);
    }

    /**
     * Determine the module metadata for all eset YUI modules.
     *
     ...
     */
    private function get_eset_metadata() {
        $esetmodules = array();
        // Core isn't a plugin type or subsystem - handle it seperately.
        if ($module = $this->get_eset_path_metadata(core_component::get_component_directory('core'))) {
            $esetmodules = array_merge($esetmodules, $module);
        }
        ...
        foreach ($subsystems as $subsystem => $path) {
            ...
            if ($module = $this->get_eset_path_metadata($path)) {
                $esetmodules = array_merge($esetmodules, $module);
            }
        }
     ...
     */
    private function get_eset_path_metadata($path) {
