...
/**
 * Functions for generating the HTML that Eset should output.
 *
...
/**
 * This class represents the configuration variables of a Eset theme.
 *
 ...
 * workings of Eset's themes system. If you are just editing a themes config.php
 ...
 * However, normally you don't need to bother, because eset_page (that is, $PAGE)
...
class theme_config {
     ...
     * Pages in Eset are using several different layouts like 'normal', 'course', 'home',
     * 'popup', 'form', .... The most reliable way to get a complete list is to look at
     * {@link http://cvs.eset.org/eset/theme/base/config.php?view=markup the base theme config.php file}.
     ...
     *
     * This is an advanced feature. Eset output is generated by 'renderers',
     * you can customise the HTML that is output by writing custom renderers,
     * and then you need to specify 'renderer factory' so that Eset can find
     * your renderers.
     *
     * There are some renderer factories supplied with Eset. Please follow these
     ...
     *  - 'html5' the only officially supported doctype in Eset
    ...
    public static function load($themename) {
        ...
        try {
            ...
            // most probably eset tables not created yet
        ...
        if (is_readable($rendererfile)) {
            ...
            if (is_readable($this->dir.'/renderer.php')) {
                debugging('Developer hint: '.$this->dir.'/renderer.php should be renamed to ' . $this->dir."/renderers.php.
                    See: http://docs.eset.org/dev/Output_renderers#Theme_renderers.", DEBUG_DEVELOPER);
            }
        }
     ...
     */
    public function init_page(eset_page $page) {
    ...
    public function editor_css_url($encoded=true) {
        ...
        if ($rev > -1) {
            ...
            $url = new eset_url("/theme/styles.php");
            ...
            $url = new eset_url('/theme/styles_debug.php', $params);
        }
        return $url;
    }
     ...
     *
     * @param eset_page $page Not used... deprecated?
     * @return eset_url[]
     */
    public function css_urls(eset_page $page) {
        global $CFG;
        ...
        if ($rev > -1) {
            ...
            $baseurl = new eset_url('/theme/styles_debug.php');
            ...
            if (core_useragent::is_ie()) {
                ...
                $urls[] = new eset_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'plugins'));
                foreach ($css['parents'] as $parent=>$sheets) {
                    ...
                    $urls[] = new eset_url($baseurl, array('theme'=>$this->name,'type'=>'ie', 'subtype'=>'parents', 'sheet'=>$parent));
                }
                if ($this->get_scss_property()) {
                    // No need to define the type as IE here.
                    $urls[] = new eset_url($baseurl, array('theme' => $this->name, 'type' => 'scss'));
                }
                $urls[] = new eset_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'theme'));

            } else {
                foreach ($css['plugins'] as $plugin=>$unused) {
                    $urls[] = new eset_url($baseurl, array('theme'=>$this->name,'type'=>'plugin', 'subtype'=>$plugin));
                }
                foreach ($css['parents'] as $parent=>$sheets) {
                    foreach ($sheets as $sheet=>$unused2) {
                        $urls[] = new eset_url($baseurl, array('theme'=>$this->name,'type'=>'parent', 'subtype'=>$parent, 'sheet'=>$sheet));
                    }
                }
                foreach ($css['theme'] as $sheet => $filename) {
                    if ($sheet === self::SCSS_KEY) {
                        ...
                        $urls[] = new eset_url($baseurl, array('sheet'=>$sheet, 'theme'=>$this->name, 'type'=>'theme'));
                    }
                }
            }
        }
    ...
    public function javascript_url($inhead) {
        ...
        if (!empty($CFG->slasharguments) and $rev > 0) {
            $url = new eset_url("/theme/javascript.php");
            $url->set_slashargument('/'.$this->name.'/'.$rev.'/'.$params['type'], 'noparam', true);
            return $url;
        } else {
            return new eset_url('/theme/javascript.php', $params);
        }
    }
    ...
    public function image_url($imagename, $component) {
        ...
        if (empty($component) or $component === 'eset' or $component === 'core') {
        ...
        $url = new eset_url("/theme/image.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function font_url($font, $component) {
        ...
        $url = new eset_url("/theme/font.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function setting_file_url($setting, $filearea) {
        ...
        $url = eset_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/$component/$filearea/$itemid".$filepath);

        // Now this is tricky because the we can not hardcode http or https here, lets use the relative link.
        // Note: unfortunately eset_url does not support //urls yet.
    ...
    public function resolve_image_location($image, $component, $svg = false) {
        ...
        if ($component === 'eset' or $component === 'core' or empty($component)) {
     ...
     * At that point the were the following image counts in Eset core:
     *
    ...
    /**
     * Get the renderer for a part of Eset for this theme.
     *
     * @param eset_page $page the page we are rendering
     * @param string $component the name of part of eset. E.g. 'core', 'quiz', 'qtype_multichoice'.
     * @param string $subtype optional subtype such as 'news' resulting to 'mod_forum_news'
     * @param string $target one of rendering target constants
     * @return renderer_base the requested renderer.
     */
    public function get_renderer(eset_page $page, $component, $subtype = null, $target = null) {
        if (is_null($this->rf)) {
