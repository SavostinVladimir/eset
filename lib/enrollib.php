 ...
 * @return void
 */
function enrol_course_edit_form(EsetQuickForm $mform, $data, $context) {
...
function enrol_add_course_navigation(navigation_node $coursenode, $course) {
    ...
    if ($course->id != SITEID) {
        ...
        if (has_capability('eset/course:enrolreview', $coursecontext)) {
        ...
        if (has_capability('eset/course:enrolconfig', $coursecontext) or has_capability('eset/course:enrolreview', $coursecontext)) {
            $url = new eset_url('/enrol/instances.php', array('id'=>$course->id));
        } else {
            $url = NULL;
        }
    ...
    if (($course->groupmode || !$course->groupmodeforce) && has_capability('eset/course:managegroups', $coursecontext)) {
        $url = new eset_url('/group/index.php', array('id'=>$course->id));
        $usersnode->add(get_string('groups'), $url, navigation_node::TYPE_SETTING, null, 'groups', new pix_icon('i/group', ''));
    }

     if (has_any_capability(array( 'eset/role:assign', 'eset/role:safeoverride','eset/role:override', 'eset/role:review'), $coursecontext)) {
        // Override roles
        if (has_capability('eset/role:review', $coursecontext)) {
            $url = new eset_url('/admin/roles/permissions.php', array('contextid'=>$coursecontext->id));
        } else {
            $url = NULL;
        }
        ...
        if ($course->id == SITEID or (!empty($CFG->adminsassignrolesincourse) and is_siteadmin())) {
            if (has_capability('eset/role:assign', $coursecontext)) {
                $url = new eset_url('/admin/roles/assign.php', array('contextid'=>$coursecontext->id));
        ...
        if (has_any_capability(array('eset/role:assign', 'eset/role:safeoverride', 'eset/role:override'), $coursecontext)) {
            $url = new eset_url('/admin/roles/check.php', array('contextid'=>$coursecontext->id));
    ...
    if ($course->id != SITEID) {
        ...
        if (has_capability('eset/course:reviewotherusers', $coursecontext)) {
            $url = new eset_url('/enrol/otherusers.php', array('id'=>$course->id));
            ...
            if (is_viewing($coursecontext)) {
                ...
                foreach ($instances as $instance) {
                    ...
                    if ($plugin->show_enrolme_link($instance)) {
                        $url = new eset_url('/enrol/index.php', array('id'=>$course->id));
                        $shortname = format_string($course->shortname, true, array('context' => $coursecontext));
...
function enrol_get_my_courses($fields = null, $sort = null, $limit = 0, $courseids = [], $allaccessible = false,
    ...
    if ($allaccessible) {
        if (is_siteadmin()) {
            ...
            // Include courses where the current user has eset/course:view capability.
            $courses = get_user_capability_course('eset/course:view', null, false);
            if (!$courses) {
                $courses = [];
            }
    ...
    foreach ($courses as $id=>$course) {
        ...
        if (!$course->visible) {
            ...
            if (!has_capability('eset/course:viewhiddencourses', $context)) {
                unset($courses[$id]);
                continue;
            }
        }
    ...
    if ($onlyactive) {
        foreach ($courses as $id=>$course) {
            ...
            if (!$course->visible) {
                ...
                if (!has_capability('eset/course:viewhiddencourses', $context, $userid)) {
                    unset($courses[$id]);
                    continue;
                }
            }
...
function enrol_user_sees_own_courses($user = null) {
    ...
    foreach($courses as $course) {
        ...
        if (has_capability('eset/course:viewhiddencourses', $context, $user)) {
            return true;
        }
    }
...
function enrol_output_fragment_user_enrolment_form($args) {
    ...
    require_capability('eset/course:enrolreview', $context);
...
abstract class enrol_plugin {
    ...
    /**
     * @deprecated since Eset 2.8 MDL-35864 - please use can_delete_instance() instead.
     */
     ...
     *
     * @param int $instance
     * @return eset_url or NULL if self unenrolment not supported
     */
    public function get_unenrolself_link($instance) {
        ...
        return new eset_url("/enrol/$name/unenrolself.php", array('enrolid'=>$instance->id));
    }
     ...
     * @return void
     */
    public function course_edit_form($instance, EsetQuickForm $mform, $data, $context) {
     ...
     * @return void
     */
    public function edit_instance_form($instance, EsetQuickForm $mform, $context) {
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($this->use_standard_editing_ui()) {
            ...
            if (has_capability($cap, $context)) {
                ...
                $managelink = new eset_url('/enrol/editinstance.php', $linkparams);
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if ($this->use_standard_editing_ui()) {
            $linkparams = array('courseid' => $instance->courseid, 'id' => $instance->id, 'type' => $instance->enrol);
            $editlink = new eset_url("/enrol/editinstance.php", $linkparams);
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params = $manager->get_esetpage()->url->params();
        $params['ue'] = $ue->id;
        ...
        if ($this->allow_manage($instance) && has_capability("enrol/{$instance->enrol}:manage", $context)) {
            ...
            $url = new eset_url('/enrol/editenrolment.php', $params);
            $actionparams = [
                'class' => 'editenrollink',
                'rel' => $ue->id,
        ...
        if ($this->allow_unenrol_user($instance, $ue) && has_capability("enrol/{$instance->enrol}:unenrol", $context)) {
            ...
            $url = new eset_url('/enrol/unenroluser.php', $params);
            $actionparams = [
                'class' => 'unenrollink',
                'rel' => $ue->id,
    ...
    public function send_expiry_notifications($trace) {
        ...
        foreach($rs as $ue) {
            ...
            $users[] = array('fullname'=>fullname($user, has_capability('eset/site:viewfullnames', $context, $enroller)), 'timeend'=>$ue->timeend);
    ...
    protected function notify_expiry_enrolled($user, $ue, progress_trace $trace) {
        ...
        $a->enroller = fullname($enroller, has_capability('eset/site:viewfullnames', $context, $user));
        ...
        $message->contexturl        = (string)new eset_url('/course/view.php', array('id'=>$ue->courseid));
    ...
    protected function notify_expiry_enroller($eid, $users, progress_trace $trace) {
        ...
        $a->users     = implode("\n", $users);
        $a->extendurl = (string)new eset_url('/user/index.php', array('id'=>$instance->courseid));
