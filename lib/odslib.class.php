...
class EsetODSWorkbook {
     ...
     * @return EsetODSWorksheet
     */
    public function add_worksheet($name = '') {
        $ws = new EsetODSWorksheet($name, $this->worksheets);
        $this->worksheets[] = $ws;
        return $ws;
    }

    /**
     * Create one Eset Format.
     *
     ...
     * @return EsetODSFormat
     */
    public function add_format($properties = array()) {
        return new EsetODSFormat($properties);
    }
    ...
    public function close() {
        ...
        $writer = new EsetODSWriter($this->worksheets);
...
class EsetODSWorksheet {
    ...
    public function write_string($row, $col, $str, $format = null) {
        if (!isset($this->data[$row][$col])) {
            $this->data[$row][$col] = new EsetODSCell();
        }
        if (is_array($format)) {
            $format = new EsetODSFormat($format);
        }
    ...
    public function merge_cells($first_row, $first_col, $last_row, $last_col) {
        ...
        if (!isset($this->data[$first_row][$first_col])) {
            $this->data[$first_row][$first_col] = new EsetODSCell();
        }
...
<office:document-meta xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0"
                      ...
                      office:version="1.2">
    <office:meta>
        <meta:generator>Eset '.$CFG->release.'</meta:generator>
