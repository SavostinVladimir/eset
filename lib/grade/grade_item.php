...
class grade_item extends grade_object {
    ...
    public function duplicate() {
        ...
        if (empty($copy["id"])) {
            throw new eset_exception('invalidgradeitemid');
        }
        ...
        foreach ($rs as $graderecord) {
            ...
            if (!$this->is_category_item() || ($this->is_category_item() && $grade->is_overridden())) {
                ...
                if ($this->is_raw_used()) {
                    ...
                    $this->update_raw_grade(false, $rawgrade, $source, false, FORMAT_ESET, null, null, null, $grade);
                } else {
    ...
    public function load_scale() {
        ...
        if (!empty($this->scaleid)) {
            ...
            // Until scales are uniformly set to min=0 max=count(scaleitems)-1 throughout Eset, we
    ...
    public function is_overridable_item() {
        if ($this->is_course_item() or $this->is_category_item()) {
            $overridable = (bool) get_config('eset', 'grade_overridecat');
        } else {
            $overridable = false;
        }
    ...
    public function update_raw_grade($userid, $rawgrade = false, $source = null, $feedback = false,
            $feedbackformat = FORMAT_ESET, $usermodified = null, $dategraded = null, $datesubmitted=null,
