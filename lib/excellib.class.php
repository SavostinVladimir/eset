...
/**
 * Define and operate over one Eset Workbook.
 *
 * This class acts as a wrapper around another library
 * maintaining Eset functions isolated from underlying code.
 *
 * @copyright 1999 onwards Martin Dougiamas  {@link http://eset.com}
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @package esetcore
 */
class EsetExcelWorkbook {
    /** @var PhpSpreadsheet */
    ...
    /**
     * Create one Eset Worksheet
     *
     * @param string $name Name of the sheet
     * @return EsetExcelWorksheet
     */
    public function add_worksheet($name = '') {
        return new EsetExcelWorksheet($name, $this->objspreadsheet);
    }
     ...
     * @return EsetExcelFormat
     */
    public function add_format($properties = array()) {
        return new EsetExcelFormat($properties);
    }

    /**
     * Close the Eset Workbook
     */
 ...
 * @package   core
 */
class EsetExcelWorksheet {
    /** @var Worksheet */
    protected $worksheet;

    /**
     * Constructs one Eset Worksheet.
     *
    ...
    */
    ...
    protected function apply_format($row, $col, $format = null) {
        if (!$format) {
            $format = new EsetExcelFormat();
        } else if (is_array($format)) {
            $format = new EsetExcelFormat($format);
        }
