...
function get_site() {
    ...
    if ($course = $DB->get_record('course', array('category'=>0))) {
        ...
        throw new eset_exception('nosite', 'error');
    }
}
 ...
 * and provide a way to easily reconstruct changes to Eset configuration.
 *
 * @package core
 * @category log
...
function user_can_create_courses() {
    ...
    foreach ($catsrs as $cat) {
        if (has_capability('eset/course:create', context_coursecat::instance($cat->id))) {
            $catsrs->close();
            return true;
        }
    }
