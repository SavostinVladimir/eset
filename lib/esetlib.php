...
/**
 * esetlib.php - Eset main library
 *
 * Main library file of miscellaneous general-purpose Eset functions.
...
/**
 * PARAM_CAPABILITY - A capability name, like 'eset/role:manage'. Actually
...
/**
 * PAGE_COURSE_VIEW is a definition of a page type. For more information on the page class see eset/lib/pagelib.php.
 */
...
/** True if module supports backup/restore of eset2 format */
define('FEATURE_BACKUP_ESET2', 'backup_eset2');
...
/**
 * URL of the Eset sites registration portal.
 */
defined('HUB_ESETORGHUBURL') || define('HUB_ESETORGHUBURL', 'https://stats.eset.org');

/**
 * Eset mobile app service name
 */
define('ESET_OFFICIAL_MOBILE_SERVICE', 'eset_mobile_app');
...
function required_param($parname, $type) {
    ...
    if (is_array($param)) {
        ...
        // TODO: switch to fatal error in Eset 2.3.
...
function optional_param($parname, $default, $type) {
    ...
    if (is_array($param)) {
        debugging('Invalid array parameter detected in required_param(): '.$parname);
        // TODO: switch to $default in Eset 2.3.
...
function get_config($plugin, $name = null) {
    ...
    if ($plugin === 'eset' || $plugin === 'core' || empty($plugin)) {
 ...
 * @param    stdClass|int|null $user  A eset user object or id, null means current user
 * @throws   coding_exception
 ...
 *             if 99 then default user's timezone is used {@link http://docs.eset.org/dev/Time_API#Timezone}
 ...
 *        {@link http://docs.eset.org/dev/Time_API#Timezone}
 ...
 *
 * @param int $date the timestamp - since Eset 2.9 this is a real UTC timestamp
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if (get_user_preferences('auth_forcepasswordchange') && !\core\session\manager::is_loggedinas()) {
        ...
        if ($userauth->can_change_password() and !$preventredirect) {
            ...
            if ($changeurl = $userauth->change_password_url()) {
                ...
                // Use eset internal method.
                redirect($CFG->wwwroot .'/login/change_password.php');
            }
        } else if ($userauth->can_change_password()) {
            throw new eset_exception('forcepasswordchangenotice');
        } else {
            throw new eset_exception('nopasswordchangeforced', 'auth');
        }
    }
    ...
    if ($usernotfullysetup) {
        if ($preventredirect) {
            throw new eset_exception('usernotfullysetup');
        }
        if ($setwantsurltome) {
            $SESSION->wantsurl = qualified_me();
        }
    ...
    if (!$USER->policyagreed && !is_siteadmin() && !NO_SITEPOLICY_CHECK) {
        ...
        if ($policyurl = $manager->get_redirect_url(isguestuser())) {
            if ($preventredirect) {
                throw new eset_exception('sitepolicynotagreed', 'error', '', $policyurl->out());
            }
            if ($setwantsurltome) {
                $SESSION->wantsurl = qualified_me();
            }
    ...
    if (!empty($CFG->maintenance_enabled) and !has_capability('eset/site:maintenanceaccess', $sysctx)) {
        if ($preventredirect) {
    ...
    if ($course->id == SITEID) {
        ...
        if (is_role_switched($course->id)) {
            ...
            if (!$course->visible and !has_capability('eset/course:viewhiddencourses', $coursecontext)) {
                ...
                navigation_node::override_active_url(new eset_url('/'));
    ...
    if ($cm && $cm->deletioninprogress) {
        if ($preventredirect) {
            throw new eset_exception('activityisscheduledfordeletion');
        }
...
function get_extra_user_fields($context, $already = array()) {
    ...
    if (!has_capability('eset/site:viewuseridentity', $context)) {
        return array();
    }
    ...
    if ($hiddenidentifiers) {
        if ($context->get_course_context(false)) {
            ...
            $canviewhiddenuserfields = has_capability('eset/course:viewhiddenuserfields', $context);

        } else {
            // We are not inside a course.
            $canviewhiddenuserfields = has_capability('eset/user:viewhiddendetails', $context);
        }
        ...
        if (!empty($CFG->passwordpolicycheckonlogin)) {
            ...
            if (!$passed) {
                ...
                if ($authplugin->can_change_password()) {
                    ...
                    $internalchangeurl = new eset_url('/login/change_password.php');
                    ...
                    redirect(new eset_url('/login/forgot_password.php'));
                } else {
 ...
 *
 * @throws eset_exception If a problem occurs while generating the hash.
 */
function hash_internal_user_password($password, $fasthash = false) {
    ...
    if ($generatedhash === false || $generatedhash === null) {
        throw new eset_exception('Failed to generate password hash.');
    }
 ...
 */
function eset_process_email($modargs, $body) {
    global $DB;
 ...
 * @return eset_phpmailer|null mailer instance if 'get' used or nothing
 */
function get_mailer($action='get') {
    global $CFG;

    /** @var eset_phpmailer $mailer */
    static $mailer  = null;
    static $counter = 0;
...
/**
 * Generate a unique email Message-ID using the eset domain and install path
 *
...
function generate_email_messageid($localpart = null) {
    ...
    // If multiple esets are on the same domain we want to tell them
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    // header with details of where exactly in eset the email was triggered from,
    ...
    if (ini_get('mail.add_x_header')) {
        ...
        $mail->addCustomHeader('X-Eset-Originating-Script: ' . $originheader);
    }
    ...
    if ($mail->send()) {
        ...
        $event->trigger();
        if (CLI_SCRIPT) {
            mtrace('Error: lib/esetlib.php email_to_user(): '.$mail->ErrorInfo);
        }
        if (!empty($mail->SMTPDebug)) {
            echo '</pre>';
        }
...
function send_confirmation_email($user, $confirmationurl = null) {
    ...
    $confirmationurl = new eset_url($confirmationurl);
    ...
    // Because of this custom encoding we can't use eset_url directly.
...
function get_file_packer($mimetype='application/zip') {
    ...
    switch ($mimetype) {
        ...
        case 'application/vnd.eset.profiling':
        ...
        case 'application/vnd.eset.backup':
 ...
 * 5. by the Eset admin in $CFG->maxbytes
...
function get_user_max_upload_file_size($context, $sitebytes = 0, $coursebytes = 0, $modulebytes = 0, $user = null,
    ...
    if (has_capability('eset/course:ignorefilesizelimits', $context, $user)) {
 ...
 * to retrieve it.<br/>
 * If you open eset/lang/en/eset.php and look near line 278
 ...
 * different approach. Looking in eset/lang/en/calendar.php you find
 * around line 75:
 * <code>
 ...
 *
 * In Eset 2.3 there is a new argument to this function $lazyload.
 ...
 *      If none is specified then eset.php is used.
 ...
 *
 * Example usage of this function when the string is in the eset.php file:<br/>
 * <code>
 * echo '<strong>';
 * print_string('course');
 * echo '</strong>';
 * </code>
 *
 * Example usage of this function when the string is not in the eset.php file:<br/>
 * <code>
 * echo '<h1>';
 ...
 * @category string
 * @param string $identifier The key identifier for the localized string
 * @param string $component The module where the key identifier is stored. If none is specified then eset.php is used.
...
class emoticon_manager {
    ...
    /**
     * Returns default set of emoticons supported by Eset
     *
 ...
 *
 * This function was originally used for standard Eset plugins, please use
...
/**
 * Determine if eset installation requires update.
 *
 ...
 *
 * Eset version numbers consist of three numbers separated by a dot, for
 ...
 */
function eset_major_version($fromdisk = false) {
    global $CFG;
 ...
 */
function eset_setlocale($locale='') {
    global $CFG;
 ...
 * replace an existing eset script
 *
 ...
 * is in esetlib because it does not rely on loading any of the MNET code.
 *
 * @param object $user A valid user object
 * @return bool        True if the user is from a remote Eset.
 */
...
/**
 * This function will search for browser prefereed languages, setting Eset
...
function set_mnet_remote_client($client) {
    if (!defined('MNET_SERVER')) {
        throw new eset_exception('notinxmlrpcserver', 'mnet');
    }
    global $MNET_REMOTE_CLIENT;
 ...
 * @param stdClass|core_course_list_element $course Eset course object
 ...
 *     $string = get_string('yes', 'eset', null, true);
