...
class auth_plugin_base {
    ...
    /**
     * Eset custom fields to sync with.
     * @var array()
     */
     ...
     *
     * @return eset_url url of the profile page or null if standard used
     */
     ...
     *
     * Internal plugins use password hashes from Eset user table for authentication.
     *
     * @return bool
     */
    ...
    /**
     * Indicates if eset should automatically update internal user
     ...
     *
     * In previous versions of Eset, the function
     ...
     * @return eset_form A form which edits a record from the user table.
     */
     ...
     * this information to eset user-table you should honour synchronisation flags
     *
     ...
     * string keys:
     *
     * - url => (eset_url|string) URL of the page to send the user to for authentication
     * - name => (string) Human readable name of the IdP
     * - iconurl => (eset_url|string) URL of the icon representing the IdP (since Eset 3.3)
     *
     ...
     * Post logout hook.
     *
     * This method is used after eset logout by auth classes to execute server logout.
     *
     ...
     * This is a lighter version of the one in esetlib -- won't do
    ...
    public static function prepare_identity_providers_for_output($identityproviders, renderer_base $output) {
        ...
        foreach ($identityproviders as $idp) {
            ...
            if ($idp['iconurl'] instanceof eset_url) {
                $idp['iconurl'] = $idp['iconurl']->out(false);
            }
            unset($idp['icon']);
            if ($idp['url'] instanceof eset_url) {
    ...
    public function get_password_change_info(stdClass $user) : array {
        ...
        if (!empty($data->link) and has_capability('eset/user:changeownpassword', $systemcontext, $user->id)) {
...
function signup_validate_data($data, $files) {
    ...
    if (! validate_email($data['email'])) {
        ...
        // Emails in Eset as case-insensitive and accents-sensitive. Such a combination can lead to very slow queries
        ...
        if ($DB->record_exists_sql($sql, $params)) {
            $forgotpasswordurl = new eset_url('/login/forgot_password.php');
            $forgotpasswordlink = html_writer::link($forgotpasswordurl, get_string('emailexistshintlink'));
            $errors['email'] = get_string('emailexists') . ' ' . get_string('emailexistssignuphint', 'eset', $forgotpasswordlink);
        }
    }
 ...
 * @param stdclass $settings Eset admin settings instance
...
function display_auth_lock_options($settings, $auth, $userfields, $helptext, $mapremotefields, $updateremotefields, $customfields = array()) {
    ...
    foreach ($userfields as $field) {
        ...
        if ($fieldnametoolong) {
            ...
            $url = new eset_url('/user/profile/index.php');
