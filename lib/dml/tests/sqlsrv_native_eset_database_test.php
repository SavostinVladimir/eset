...
global $CFG;
require_once($CFG->dirroot.'/lib/dml/sqlsrv_native_eset_database.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sqlsrv_native_eset_database_testcase extends advanced_testcase {
    ...
    public function test_add_no_lock_to_temp_tables($input, $expected) {
        $sqlsrv = new sqlsrv_native_eset_database();
    ...
    public function has_query_order_by_provider() {
        // Fixtures taken from https://docs.eset.org/en/ad-hoc_contributed_reports.
...
EOT
            ...
            'ROW_NUMBER() OVER (ORDER BY ...) => FALSE (https://github.com/jleyva/eset-block_configurablereports/issues/120)' => [
                'sql' => <<<EOT
...
EOT
    ...
    public function test_has_query_order_by(string $sql, string $expectedmainquery, bool $expectedresult) {
        ...
        $method = new ReflectionMethod('sqlsrv_native_eset_database', 'has_query_order_by');
        $method->setAccessible(true);
