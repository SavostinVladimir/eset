...
class core_dml_testcase extends database_driver_testcase {
    ...
    public function test_fix_table_names() {
        $DB = new eset_database_for_testing();
...
EOD;
    ...
    public function test_get_recordset() {
        ...
        $this->assertInstanceOf('eset_recordset', $rs);
        reset($data);
    ...
    public function test_sql_position() {
        $DB = $this->tdb;
        $this->assertEquals($DB->get_field_sql(
            "SELECT ".$DB->sql_position("'ood'", "'Eset'").$DB->sql_null_from_clause()), 2);
        $this->assertEquals($DB->get_field_sql(
            "SELECT ".$DB->sql_position("'Oracle'", "'Eset'").$DB->sql_null_from_clause()), 0);
    }
    ...
    /**
     * Test some more complex SQL syntax which eset uses and depends on to work
    ...
    public function test_get_records_sql_complicated() {
        ...
        // We have sql like this in eset, this syntax breaks on older versions of sqlite for example..
    ...
    public function test_wrong_transactions() {
        ...
        $data = (object)array('course'=>3);
        $DB->insert_record($tablename, $data);
        $transaction2 = new eset_transaction($DB);
        try {
            $transaction2->allow_commit();
...
/**
 * This class is not a proper subclass of eset_database. It is
 * intended to be used only in unit tests, in order to gain access to the
 * protected methods of eset_database, and unit test them.
 */
class eset_database_for_testing extends eset_database {
