...
require_once(__DIR__.'/eset_recordset.php');
require_once(__DIR__.'/eset_transaction.php');
...
/**
 * Abstract class representing eset database interface.
 * @link http://docs.eset.org/dev/DML_functions
 *
 * @package    core_dml
 * @copyright  2008 Petr Skoda (http://skodak.org)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class eset_database {

    /** @var database_manager db manager which allows db structure modifications. */
    protected $database_manager;
    /** @var eset_temptables temptables manager to provide cross-db support for temp tables. */
    ...
    /** @var bool True means non-eset external database used.*/
    protected $external;
    ...
    /**
     * Constructor - Instantiates the database, specifying if it's external (connect to other systems) or not (Eset DB).
     ...
     *
     * The loaded class is within lib/dml directory and of the form: $type.'_'.$library.'_eset_database'
     *
     ...
     * @return eset_database driver object or null if error, for example of driver object see {@link mysqli_native_eset_database}
     */
    public static function get_driver_instance($type, $library, $external = false) {
        global $CFG;

        $classname = $type.'_'.$library.'_eset_database';
    ...
    public function get_in_or_equal($items, $type=SQL_PARAMS_QM, $prefix='param', $equal=true, $onemptyitems=false) {
        ...
        if (is_array($items) and empty($items) and $onemptyitems === false) {
            throw new coding_exception('eset_database::get_in_or_equal() does not accept empty arrays');
        }
    ...
    protected function add_sql_debugging(string $sql): string {
        ...
        // Ignore eset_database internals.
        $callers = array_filter($callers, function($caller) {
            return empty($caller['class']) || $caller['class'] != 'eset_database';
        });
    ...
    /**
     * Get a number of records as a eset_recordset where all the given conditions met.
     *
     ...
     *
     * The return value is a eset_recordset
     ...
     * @return eset_recordset A eset_recordset instance
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    ...
    /**
     * Get a number of records as a eset_recordset where one field match one list of values.
     *
    ...
    /**
     * Get a number of records as a eset_recordset which match a particular WHERE clause.
     *
     ...
     * @return eset_recordset A eset_recordset instance {@link function get_recordset}.
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    ...
    public function sql_substr($expr, $start, $length=false) {
        if (count(func_get_args()) < 2) {
            throw new coding_exception('eset_database::sql_substr() requires at least two parameters', 'Originally this function was only returning name of SQL substring function, it now requires all parameters.');
        }
        if ($length === false) {
     ...
     *
     * @return eset_transaction
     */
    public function start_delegated_transaction() {
        $transaction = new eset_transaction($this);
     ...
     * @param eset_transaction $transaction The transaction to commit
     * @return void
     * @throws dml_transaction_exception Creates and throws transaction related exceptions.
     */
    public function commit_delegated_transaction(eset_transaction $transaction) {
     ...
     *
     * @param eset_transaction $transaction An instance of a eset_transaction.
     ...
     */
    public function rollback_delegated_transaction(eset_transaction $transaction, $e) {
