 ...
 * cross-db compatibility in the Eset 2.x DB API with some operations
 * not natively supported by Oracle, namely:
 *  - locking: Application locks used by Eset DB sessions. It uses
 *             the DBMS_LOCK package so execution must be granted
 *             to the Eset DB user by SYS to work properly.
 ...
 *
 * Eset will not parse this file correctly if it uses Windows line endings.
 */

CREATE OR REPLACE PACKAGE ESETLIB AS
...
CREATE OR REPLACE PACKAGE BODY ESETLIB AS
...
BEGIN
    RETURN ESETLIB.BITOR(value1,value2) - BITAND(value1,value2);
END BITXOR;
...
BEGIN
    ...
    stringresult := CONCAT(CONCAT(ESETLIB.UNDO_DIRTY_HACK(string1), ESETLIB.UNDO_DIRTY_HACK(string2)), ESETLIB.UNDO_DIRTY_HACK(string3));
