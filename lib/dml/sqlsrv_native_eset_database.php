...
/**
 * Native sqlsrv class representing eset database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/sqlsrv_native_eset_recordset.php');
require_once(__DIR__.'/sqlsrv_native_eset_temptables.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v2 or later
 */
class sqlsrv_native_eset_database extends eset_database {

    protected $sqlsrv = null;
    protected $last_error_reporting; // To handle SQL*Server-Native driver default verbosity
    protected $temptables; // Control existing temptables (sqlsrv_eset_temptables object)
    ...
    /**
     * Constructor - instantiates the database, specifying if it's external (connect to other systems) or no (Eset DB)
     ...
     * @param mixed $prefix string|bool The eset db table name's prefix. false is used for external databases where prefix not used
    ...
    public function connect($dbhost, $dbuser, $dbpass, $dbname, $prefix, array $dboptions=null) {
        ...
        $this->temptables = new sqlsrv_native_eset_temptables($this);
    ...
    protected function fetch_columns(string $table): array {
        ...
        while ($rawcolumn = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
            ...
            $info->meta_type = $this->sqlsrvtype2esettype($info->type);
    ...
    /**
     * Provides mapping between sqlsrv native data types and eset_database - database_column_info - ones)
     *
     * @param string $sqlsrv_type native sqlsrv data type
     * @return string 1-char database_column_info data type
     */
    private function sqlsrvtype2esettype($sqlsrv_type) {
        $type = null;
     ...
     *
     * @param mixed $result
     * @return sqlsrv_native_eset_recordset
     */
    protected function create_recordset($result) {
        $rs = new sqlsrv_native_eset_recordset($result, $this);
        $this->recordsets[] = $rs;
        return $rs;
    }
     ...
     * @internal
     * @param sqlsrv_native_eset_recordset $rs
     */
    public function recordset_closed(sqlsrv_native_eset_recordset $rs) {
    ...
    public function sql_substr($expr, $start, $length = false) {
        if (count(func_get_args()) < 2) {
            throw new coding_exception('eset_database::sql_substr() requires at least two parameters',
