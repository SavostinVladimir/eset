...
/**
 * Native oci class representing eset database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/oci_native_eset_recordset.php');
require_once(__DIR__.'/oci_native_eset_temptables.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class oci_native_eset_database extends eset_database {
     ...
     * @param mixed $prefix string means eset db prefix, false used for external databases where prefix not used
    ...
    public function connect($dbhost, $dbuser, $dbpass, $dbname, $prefix, array $dboptions=null) {
        ...
        // Make sure eset package is installed - now required.
        if (!$this->oci_package_installed()) {
            ...
            if (!$this->oci_package_installed()) {
                throw new dml_exception('dbdriverproblem', 'Oracle PL/SQL Eset support package ESETLIB is not installed! Database administrator has to execute /lib/dml/oci_native_eset_package.sql script.');
            }
        }
        ...
        $this->temptables = new oci_native_eset_temptables($this, $this->unique_session_id);
    ...
    protected function tweak_param_names($sql, array $params) {
        ...
        uksort($searcharr, array('oci_native_eset_database', 'compare_by_length_desc'));
    ...
    private function oracle_dirty_hack ($table, $field, $value) {
        ...
        // !! This paragraph explains behaviour before Eset 2.0:
        //
        // For Oracle DB, empty strings are converted to NULLs in DB
        // and this breaks a lot of NOT NULL columns currently Eset. In the future it's
        ...
        // !! These paragraphs explain the rationale about the change for Eset 2.5:
        //
        // Before Eset 2.0, we only used to apply this DIRTY HACK to NOT NULL columns, as
        ...
        //
        // So, starting with Eset 2.0, we are going to apply the DIRTY HACK to all the
        ...
        // !! Conclusions:
        //
        // From Eset 2.5 onwards, ALL empty strings in Oracle DBs will be stored as
            ...
            foreach($params as $key => $value) {
                ...
                if (is_array($value)) { // Let's go to bind special cases (lob descriptors)
                    if (isset($value['clob'])) {
                        ...
                        if ($descriptors === null) {
                            throw new coding_exception('eset_database::bind_params() $descriptors not specified for clob');
                        }
                        $descriptors[] = $lob;
    ...
    public function execute($sql, array $params=null) {
        ...
        if (strpos($sql, ';') !== false) {
            throw new coding_exception('eset_database::execute() Multiple sql statements found or bound parameters not used properly in query!');
        }
    ...
    /**
     * Get a number of records as a eset_recordset using a SQL statement.
     *
     ...
     * @param int $limitfrom return a subset of records, starting at this point (optional, required if $limitnum is set).
     * @param int $limitnum return a subset comprising this many records (optional, required if $limitfrom is set).
     * @return eset_recordset instance
     * @throws dml_exception A DML specific exception is thrown for any errors.
     */
    ...
    protected function create_recordset($stmt) {
        return new oci_native_eset_recordset($stmt);
    }
    ...
    public function get_records_sql($sql, array $params=null, $limitfrom=0, $limitnum=0) {
        ...
        foreach ($records as $row) {
            ...
            array_walk($row, array('oci_native_eset_database', 'onespace2empty'));
            $id = reset($row);
            if (isset($return[$id])) {
                $colname = key($row);
    ...
    public function get_fieldset_sql($sql, array $params=null) {
        ...
        array_walk($return, array('oci_native_eset_database', 'onespace2empty'));
    ...
    public function insert_record_raw($table, $params, $returnid=true, $bulk=false, $customsequence=false) {
        ...
        if ($customsequence) {
            if (!isset($params['id'])) {
                throw new coding_exception('eset_database::insert_record_raw() id field must be specified if custom sequences used.');
            }
            $returnid = false;
        } else {
            unset($params['id']);
        ...
        if (empty($params)) {
            throw new coding_exception('eset_database::insert_record_raw() no fields found.');
        }
    ...
    public function update_record_raw($table, $params, $bulk=false) {
        ...
        if (!isset($params['id'])) {
            throw new coding_exception('eset_database::update_record_raw() id field must be specified.');
        }

        if (empty($params)) {
            throw new coding_exception('eset_database::update_record_raw() no fields found.');
        }
    ...
    public function sql_bitor($int1, $int2) {
        return 'ESETLIB.BITOR(' . $int1 . ', ' . $int2 . ')';
    }

    public function sql_bitxor($int1, $int2) {
        return 'ESETLIB.BITXOR(' . $int1 . ', ' . $int2 . ')';
    }
    ...
    public function sql_concat() {
        ...
        return " ESETLIB.UNDO_MEGA_HACK($s) ";
    }
     ...
     *
     * Method overriding {@link eset_database::get_in_or_equal} to be able to get
    ...
    protected function recursive_concat(array $args) {
        ...
        return "ESETLIB.TRICONCAT($first, $second, $third)";
    }
    ...
    /**
     * Try to add required eset package into oracle server.
     */
    protected function attempt_oci_package_install() {
        $sqls = file_get_contents(__DIR__.'/oci_native_eset_package.sql');
    ...
    public function get_session_lock($rowid, $timeout) {
        ...
        $sql = 'SELECT ESETLIB.GET_LOCK(:lockname, :locktimeout) FROM DUAL';
    ...
    public function release_session_lock($rowid) {
        ...
        $sql = 'SELECT ESETLIB.RELEASE_LOCK(:lockname) FROM DUAL';
        $this->query_start($sql, $params, SQL_QUERY_AUX);
        $stmt = $this->parse_query($sql);
        $this->bind_params($stmt, $params);
        $result = oci_execute($stmt, $this->commit_status);
