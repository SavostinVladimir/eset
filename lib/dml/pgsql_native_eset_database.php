...
/**
 * Native pgsql class representing eset database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/pgsql_native_eset_recordset.php');
require_once(__DIR__.'/pgsql_native_eset_temptables.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pgsql_native_eset_database extends eset_database {
    ...
    public function raw_connect(string $dbhost, string $dbuser, string $dbpass, string $dbname, $prefix, array $dboptions=null): bool {
        ...
        $this->temptables = new pgsql_native_eset_temptables($this);
    ...
    protected function query_end($result) {
        ...
        try {
            ...
            if ($this->savepointpresent and $this->last_type != SQL_QUERY_AUX and $this->last_type != SQL_QUERY_SELECT) {
                $res = @pg_query($this->pgsql, "RELEASE SAVEPOINT eset_pg_savepoint; SAVEPOINT eset_pg_savepoint");
                if ($res) {
                    pg_free_result($res);
                }
            }
            ...
            if ($this->savepointpresent) {
                $res = @pg_query($this->pgsql, "ROLLBACK TO SAVEPOINT eset_pg_savepoint; SAVEPOINT eset_pg_savepoint");
                if ($res) {
                    pg_free_result($res);
                }
            }
    ...
    public function get_recordset_sql($sql, array $params=null, $limitfrom=0, $limitnum=0) {
        ...
        return new pgsql_native_eset_recordset($result, $this, $cursorname);
    }
    ...
    public function get_session_lock($rowid, $timeout) {
        ...
        //       multiple instances of eset, we could try to use pg_advisory_lock(int, int),
    ...
    protected function begin_transaction() {
        ...
        $sql = "BEGIN ISOLATION LEVEL READ COMMITTED; SAVEPOINT eset_pg_savepoint";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
    ...
    protected function commit_transaction() {
        ...
        $sql = "RELEASE SAVEPOINT eset_pg_savepoint; COMMIT";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
    ...
    protected function rollback_transaction() {
        $this->savepointpresent = false;
        $sql = "RELEASE SAVEPOINT eset_pg_savepoint; ROLLBACK";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
