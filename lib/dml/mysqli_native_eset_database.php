...
/**
 * Native mysqli class representing eset database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/eset_read_slave_trait.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mysqli_native_eset_database extends eset_database {
    use eset_read_slave_trait {
     ...
     * Eset is designed to work best on ACID compliant databases
    ...
    public function get_dbengine() {
        ...
        if ($engine === 'MyISAM') {
            // we really do not want MyISAM for Eset, InnoDB or XtraDB is a reasonable defaults if supported
    ...
    public function raw_connect(string $dbhost, string $dbuser, string $dbpass, string $dbname, $prefix, array $dboptions=null): bool {
        ...
        // With strict mode enforced, Eset DB layer will be consistently throwing
        ...
        $this->temptables = new mysqli_native_eset_temptables($this);
    ...
    /**
     * Returns eset column info for raw column from information schema.
    ...
    private function get_column_info(stdClass $rawcolumn) {
        ...
        $info->meta_type      = $this->mysqltype2esettype($rawcolumn->data_type);
        ...
        if ($info->meta_type === 'C') {
            ...
            if ("$rawcolumn->character_maximum_length" === '4294967295') { // watch out for PHP max int limits!
                // means maximum eset size for text column, in other drivers it may also mean unknown size
                $info->max_length = -1;
            } else {
                $info->max_length = $rawcolumn->character_maximum_length;
            }
     ...
     */
    private function mysqltype2esettype($mysql_type) {
        $type = null;
    ...
    protected function create_recordset($result) {
        return new mysqli_native_eset_recordset($result);
    }
