...
/**
 * pgsql specific eset recordset class
 *
 * @package    core_dml
 * @copyright  2008 Petr Skoda (http://skodak.org)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pgsql_native_eset_recordset extends eset_recordset {
    ...
    /** @var pgsql_native_eset_database Postgres database resource */
    protected $db;
     ...
     * @param pgsql_native_eset_database $db Database object (only required when using cursors)
     * @param string $cursorname Name of cursor or '' if none
     */
    public function __construct($result, pgsql_native_eset_database $db = null, $cursorname = '') {
        if ($cursorname && !$db) {
