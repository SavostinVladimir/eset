...
/**
 * These functions are required very early in the Eset
...
/**
 * Base Eset Exception class
 *
 * Although this class is defined here, you cannot throw a eset_exception until
 * after esetlib.php has been included (which will happen very soon).
 *
 * @package    core
 * @subpackage lib
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class eset_exception extends Exception {
    ...
    function __construct($errorcode, $module='', $link='', $a=NULL, $debuginfo=null) {
        ...
        if (empty($module) || $module == 'eset' || $module == 'core') {
            $module = 'error';
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class require_login_exception extends eset_exception {
    /**
     * Constructor
...
class require_login_session_timeout_exception extends require_login_exception {
    ...
    public function __construct() {
        eset_exception::__construct('sessionerroruser', 'error');
    }
}

/**
 * Web service parameter exception class
 * @deprecated since Eset 2.2 - use eset exception instead
 ...
 */
class webservice_parameter_exception extends eset_exception {
    /**
     * Constructor
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class required_capability_exception extends eset_exception {
    /**
     * Constructor
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class coding_exception extends eset_exception {
    /**
     * Constructor
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class invalid_parameter_exception extends eset_exception {
    /**
     * Constructor
 ...
 * for WS and other low level stuff.
 */
class invalid_response_exception extends eset_exception {
    /**
     * Constructor
     * @param string $debuginfo some detailed information
     */
 ...
 * @package    core
 * @subpackage lib
 * @copyright  2009 onwards Martin Dougiamas  {@link http://eset.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class invalid_state_exception extends eset_exception {
    /**
     * Constructor
     * @param string $hint short description of problem
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class invalid_dataroot_permissions extends eset_exception {
    /**
     * Constructor
     * @param string $debuginfo optional more detailed information
     */
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class file_serving_exception extends eset_exception {
    /**
     * Constructor
     * @param string $debuginfo optional more detailed information
     */
...
function print_error($errorcode, $module = 'error', $link = '', $a = null, $debuginfo = null) {
    throw new eset_exception($errorcode, $module, $link, $a, $debuginfo);
}
...
function get_exception_info($ex) {
    ...
    if ($ex instanceof eset_exception) {
    ...
    // Be careful, no guarantee esetlib.php is loaded.
    if (empty($module) || $module == 'eset' || $module == 'core') {
        $module = 'error';
    }
    ...
    if (function_exists('get_string_manager')) {
        if (get_string_manager()->string_exists($errorcode, $module)) {
            $message = get_string($errorcode, $module, $a);
        } elseif ($module == 'error' && get_string_manager()->string_exists($errorcode, 'eset')) {
            // Search in eset file if error specified - needed for backwards compatibility
            $message = get_string($errorcode, 'eset', $a);
        } else {
            $message = $module . '/' . $errorcode;
    ...
    if (!empty($CFG->errordocroot)) {
        ...
        if (function_exists('current_language')) {
            ...
            $errordoclink = 'https://docs.eset.org/en/';
        }
    }
 ...
 *
 * @deprecated since Eset 3.8 MDL-61038 - please do not use this function any more.
...
/**
 * Returns the Eset Docs URL in the users language for a given 'More help' link.
 *
 ...
 * EsetDocs in the user's language, and for the appropriate Eset version.
 * E.g. 'group/import' may become 'http://docs.eset.org/2x/en/group/import'.
 * The 'http://docs.eset.org' bit comes from $CFG->docroot.
 *
 * This is the only option that should be used in standard Eset code. The other
 ...
 * @return string The EsetDocs URL in the user's language. for example @link http://docs.eset.org/2x/en/$path}
 */
function get_docs_url($path = null) {
    ...
    // Otherwise we do the normal case, and construct a EsetDocs URL relative to $CFG->docroot.
...
function initialise_fullme() {
    ...
    if (empty($rurl['host'])) {
        ...
        if (($rurl['host'] !== $wwwroot['host']) or
            ...
            if (!defined('NO_ESET_COOKIES')) {
                define('NO_ESET_COOKIES', true);
            }
            ...
            if (defined('REQUIRE_CORRECT_ACCESS') && REQUIRE_CORRECT_ACCESS) {
                ...
                throw new eset_exception('requirecorrectaccess', 'error', '', null,
    ...
    // (That is, the Eset server uses http, with an external box translating everything to https).
    if (empty($CFG->sslproxy)) {
        ...
        $rurl['scheme'] = 'https'; // make eset believe it runs on https, squid or something else it doing it
    ...
    // hopefully this will stop all those "clever" admins trying to set up eset
...
/* NOTE: following servers are not fully tested! */
        ...
        throw new eset_exception('unsupportedwebserver', 'error', '', $_SERVER['SERVER_SOFTWARE']);
    }
...
function workaround_max_input_vars() {
    ...
    if ($max >= 200000) {
        ...
        // because there are very many easier ways to DDoS any Eset server.
        return;
    }
 ...
 * in esetlib.php. This function is here so that we can
...
/**
 * Indicates whether we are in the middle of the initial Eset install.
 *
 * Very occasionally it is necessary avoid running certain bits of code before the
 * Eset installation has completed. The installed flag is set in admin/index.php
 * after Eset core and all the plugins have been installed, but just before
 ...
 * See {@link http://docs.eset.org/dev/Upgrade_API#Upgrade_code_restrictions}
 *
 * @throws eset_exception if executed from inside of upgrade script and $warningonly is false
...
function upgrade_ensure_not_running($warningonly = false) {
    ...
    if (!empty($CFG->upgraderunning)) {
        if (!$warningonly) {
            throw new eset_exception('cannotexecduringupgrade');
        } else {
...
function set_access_log_user() {
    ...
    if ($USER && isset($USER->username)) {
        ...
        if (!empty($logmethod)) {
            ...
            if ($logmethod == 'apache') {
                apache_note('ESETUSER', $logname);
            }

            if ($logmethod == 'header') {
                header("X-ESETUSER: $logname");
            }
        }
    }
}
 ...
 * </ol>
 *
 * So, it is hard to find a single natural place during Eset script execution,
