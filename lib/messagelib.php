 ...
 *  fullmessageformat int the format if the full message (FORMAT_ESET, FORMAT_HTML, ..)
...
function message_send(\core\message\message $eventdata) {
    ...
    if (!$eventdata->notification && !$eventdata->convid) {
        ...
        if (empty($CFG->messaging) || $eventdata->component !== 'eset' || $eventdata->name !== 'instantmessage') {
 ...
 *
 * @param string $component For example 'eset', 'mod_forum' or 'block_quiz_results'
 * @return boolean True on success
 */
function message_update_providers($component='eset') {
    global $DB;
 ...
 *
 * @param  string $component   The name of component (e.g. eset, mod_forum, etc.)
 ...
 * @param string $component A eset component like 'eset', 'mod_forum', 'block_quiz_results'
 * @return array An array of message providers
 */
