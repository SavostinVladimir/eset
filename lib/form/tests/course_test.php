...
/**
 * Unit tests for EsetQuickForm_course.
 *
 ...
 * Unit tests for EsetQuickForm_course
 *
 * Contains test cases for testing EsetQuickForm_course.
 *
 * @package    core_form
 * @category   test
 * @copyright  2020 Ruslan Kabalin
...
class core_form_course_testcase extends basic_testcase {
    ...
    public function test_constructor_attributes() {
        $attributes = [
            'exclude' => [1, 2],
            'requiredcapabilities' => ['eset/course:update'],
        ];

        $element = new EsetQuickForm_course('testel', null, $attributes);
        $html = $element->toHtml();
        $this->assertContains('data-exclude="1,2"', $html);
        $this->assertContains('data-requiredcapabilities="eset/course:update"', $html);
