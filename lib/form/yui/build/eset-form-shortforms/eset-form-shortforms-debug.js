YUI.add('eset-form-shortforms', function (Y, NAME) {

/**
 * Provides the form shortforms class.
 *
 * @module eset-form-shortforms
 */
...
Y.extend(SHORTFORMS, Y.Base, {
    ...
    initializer: function() {
        var form = Y.one('#' + this.get('formid')),
            ...
            idlist;
        if (!form) {
            Y.log('Could not locate the form', 'warn', 'eset-form-shortforms');
            return;
        }
    ...
    set_state: function(fieldset, collapsed) {
        ...
        if (!statuselement) {
            Y.log("M.form.shortforms::switch_state was called on an fieldset without a status field: '" +
                fieldset.get('id') + "'", 'debug', 'eset-form-shortforms');
            return this;
        }
    ...
    update_btns: function() {
        ...
        if (expandbtn) {
            ...
            btn.setHTML(M.util.get_string('expandall', 'eset'));
        } else {
            btn.addClass(CSS.COLLAPSEALL);
            btn.setHTML(M.util.get_string('collapseall', 'eset'));
        }
...
}, '@VERSION@', {"requires": ["node", "base", "selector-css3", "eset-core-event"]});
