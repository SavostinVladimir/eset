...
function ldap_isgroupmember($ldapconnection, $userid, $group_dns, $member_attrib) {
    ...
    foreach ($group_dns as $group) {
        ...
        if (!empty($search) && ldap_count_entries($ldapconnection, $search)) {
            $info = ldap_get_entries_eset($ldapconnection, $search);
            if (count($info) > 0 ) {
 ...
 */
function ldap_connect_eset($host_url, $ldap_version, $user_type, $bind_dn, $bind_pw, $opt_deref, &$debuginfo, $start_tls=false) {
...
function ldap_normalise_objectclass($objectclass, $default = '*') {
    if (empty($objectclass)) {
        ...
        //
        // For example, we could select the users whose objectClass is 'user' and have the 'enabledEsetUser'
        // attribute, with something like:
        //
        // (&(objectClass=user)(enabledEsetUser=1))
        //
 ...
 */
function ldap_get_entries_eset($ldapconnection, $searchresult) {
...
function ldap_paged_results_supported($ldapversion, $ldapconnection = null) {
    ...
    $entries = ldap_get_entries_eset($ldapconnection, $sr);
    if (empty($entries)) {
        return false;
    }
    $info = $entries[0];
