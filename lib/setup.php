...
/**
 * Holds the core settings that affect how Eset works. Some of its fields
 * are set in config.php, and the rest are loaded from the config table.
 *
 * Some typical settings in the $CFG global:
 *  - $CFG->wwwroot  - Path to eset index directory in url format.
 *  - $CFG->dataroot - Path to eset data files directory on server's filesystem.
 *  - $CFG->dirroot  - Path to eset's library folder on server's filesystem.
 *  - $CFG->libdir   - Path to eset's library folder on server's filesystem.
 *  - $CFG->backuptempdir  - Path to eset's backup temp file directory on server's filesystem.
 *  - $CFG->tempdir  - Path to eset's temp file directory on server's filesystem.
 *  - $CFG->cachedir - Path to eset's cache directory on server's filesystem (shared by cluster nodes).
 *  - $CFG->localcachedir - Path to eset's local cache directory (not shared by cluster nodes).
 *
 * @global object $CFG
 * @name $CFG
 */
...
if (!isset($CFG->wwwroot) or $CFG->wwwroot === 'http://example.com/eset') {
    if (isset($_SERVER['REMOTE_ADDR'])) {
        header($_SERVER['SERVER_PROTOCOL'] . ' 503 Service Unavailable');
    }
...
if (!defined('NO_DEBUG_DISPLAY')) {
    if (defined('AJAX_SCRIPT') and AJAX_SCRIPT) {
        // Eset AJAX scripts are expected to return json data, any PHP notices or errors break it badly,
...
// When set to true MUC (Eset caching) will be disabled as much as possible.
...
// When set to true MUC (Eset caching) will not use any of the defined or default stores.
...
if (version_compare(PHP_VERSION, '5.6.5') < 0) {
    ...
    echo "Eset 3.2 or later requires at least PHP 5.6.5 (currently using version $phpversion).\n";
...
if (!defined('ESET_INTERNAL')) { // Necessary because cli installer has to define it earlier.
    /** Used by library scripts to check they are being called by Eset. */
    define('ESET_INTERNAL', true);
}
...
if (defined('ABORT_AFTER_CONFIG')) {
    if (!defined('ABORT_AFTER_CONFIG_CANCEL')) {
        ...
        if (NO_DEBUG_DISPLAY) {
            // Some parts of Eset cannot display errors and debug at all.
...
if (!empty($CFG->earlyprofilingenabled)) {
    require_once($CFG->libdir . '/xhprof/xhprof_eset.php');
    profiling_start();
}
...
/**
 * Eset's wrapper round PHP's $_SESSION.
 *
...
/**
 * Relative eset script path '/course/view.php'
...
//point pear include path to esets lib/pear so that includes and requires will search there for files before anywhere else
...
require_once($CFG->libdir .'/esetlib.php');       // Other general-purpose functions
require_once($CFG->libdir .'/enrollib.php');        // Enrolment related functions
require_once($CFG->libdir .'/pagelib.php');         // Library that defines the eset_page class, used for $PAGE
...
// this is a security problem because some eset scripts may
// disclose sensitive information.
if (ini_get_bool('display_errors')) {
    define('WARN_DISPLAY_ERRORS_ENABLED', true);
}
// If we want to display Eset errors, then try and set PHP errors to match.
...
} else if (!defined('NO_ESET_COOKIES')) {
...
if ($forcelang = optional_param('forcelang', '', PARAM_SAFEDIR)) {
    if (isloggedin()
        ...
        && has_capability('eset/site:forcelanguage', context_system::instance())) {
...
if (!empty($CFG->esetpageclass)) {
    if (!empty($CFG->esetpageclassfile)) {
        require_once($CFG->esetpageclassfile);
    }
    $classname = $CFG->esetpageclass;
} else {
    $classname = 'eset_page';
}
$PAGE = new $classname();
...
// LogFormat to get the current logged in username in eset.
// Alternatvely for other web servers a header X-ESETUSER can be set which
