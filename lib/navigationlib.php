...
/**
 * This file contains classes used to manage the navigation structures within Eset.
 *
 * @since      Eset 2.0
 * @package    core
 ...
 *
 * This class is used to represent a node in a navigation tree within Eset,
...
class navigation_node implements renderable {
    ...
    /** @var eset_url|action_link|null An action for the node (link) */
    public $action = null;
        ...
        if (is_array($properties)) {
            ...
            if (array_key_exists('action', $properties)) {
                ...
                if (is_string($this->action)) {
                    $this->action = new eset_url($this->action);
                }
    ...
    public function check_if_active($strength=URL_MATCH_EXACT) {
        ...
        if (self::$fullmeurl == null) {
            if ($PAGE->has_set_url()) {
                self::override_active_url(new eset_url($PAGE->url));
            } else {
                self::override_active_url(new eset_url($FULLME));
            }
        }

        // Compare the action of this node against the fullmeurl
        if ($this->action instanceof eset_url && $this->action->compare(self::$fullmeurl, $strength)) {
            $this->make_active();
            return true;
        }
        return false;
     ...
     *
     * @param eset_url $url The url to use for the fullmeurl.
     * @param bool $loadadmintree use true if the URL point to administration tree
     */
    public static function override_active_url(eset_url $url, $loadadmintree = false) {
        // Clone the URL, in case the calling script changes their URL later.
        self::$fullmeurl = new eset_url($url);
     ...
     * @param string $text
     * @param eset_url|action_link $action
     * @param int $type
    ...
    /**
     * Sets the title for this node and forces Eset to utilise it.
     * @param string $title
     */
    ...
    public function action() {
        if ($this->action instanceof eset_url) {
            return $this->action;
    ...
    protected function add_context_locking_node(\navigation_node $node, \context $context) {
        ...
        if (!empty($CFG->contextlocking) && has_capability('eset/site:managecontextlocks', $context)) {
 ...
 *
 * See
 * {@link lib/pagelib.php} {@link eset_page::initialise_theme_and_output()}
...
class global_navigation extends navigation_node {
    /** @var eset_page The Eset page this navigation object belongs to. */
    protected $page;
     ...
     *
     * @param eset_page $page The page this navigation object belongs to
     */
    public function __construct(eset_page $page) {
        ...
        if (get_home_page() == HOMEPAGE_SITE) {
            ...
            // We are using the users my eset for the root element
            $properties = array(
                'key' => 'myhome',
                'type' => navigation_node::TYPE_SYSTEM,
                'text' => get_string('myhome'),
                'action' => new eset_url('/my/'),
    ...
    public function initialise() {
        ...
        if (get_home_page() == HOMEPAGE_SITE) {
            // The home element should be my eset because the root element is the site
            if (isloggedin() && !isguestuser()) {  // Makes no sense if you aren't logged in
                $this->rootnodes['home'] = $this->add(get_string('myhome'), new eset_url('/my/'),
                    self::TYPE_SETTING, null, 'myhome', new pix_icon('i/dashboard', ''));
                $this->rootnodes['home']->showinflatnavigation = true;
            }
        } else {
            // The home element should be the site because the root node is my eset
            $this->rootnodes['home'] = $this->add(get_string('sitehome'), new eset_url('/'),
        ...
        $this->rootnodes['courses'] = $this->add(get_string('courses'), new eset_url('/course/index.php'), self::TYPE_ROOTNODE, null, 'courses');
        ...
        // in {@link http://docs.eset.org/dev/Navigation_2.0_structure}
        if (!isloggedin()) {
    ...
    protected function current_user_is_parent_role() {
        ...
        if ($this->useridtouseforparentchecks && $this->useridtouseforparentchecks != $USER->id) {
            ...
            if (!has_capability('eset/user:viewdetails', $usercontext)) {
                return false;
            }
    ...
    /**
     * Loads the courses in Eset into the navigation.
     *
     * @global eset_database $DB
        ...
        if ($this->show_categories()) {
            ...
            if (count($fullfetch)) {
                ...
                foreach ($coursesrs as $course) {
                    ...
                    if (!$course->visible && !is_role_switched($course->id) && !has_capability('eset/course:viewhiddencourses', context_course::instance($course->id))) {
                        continue;
                    }
            ...
            if ($categoryids) {
                ...
                foreach ($categories as &$category) {
                    if (array_key_exists($category->key, $excessivecategories) && !$this->can_add_more_courses_to_category($category)) {
                        $url = new eset_url('/course/index.php', array('categoryid' => $category->key));
    ...
    protected function add_category(stdClass $category, navigation_node $parent, $nodetype = self::TYPE_CATEGORY) {
        ...
        $url = $canview ? new eset_url('/course/index.php', array('categoryid' => $category->id)) : null;
    ...
    protected function load_section_activities(navigation_node $sectionnode, $sectionnumber, array $activities, $course = null) {
        ...
        foreach ($activities as $activity) {
            ...
            $action = new eset_url($activity->url);
    ...
    protected function load_for_user($user=null, $forceforcontext=false) {
        ...
        if (!$issitecourse) {
            ...
            $userviewurl = new eset_url('/user/view.php', $baseargs);
            ...
            if (course_can_view_participants($coursecontext)) {
                $usersnode->action = new eset_url('/user/index.php', array('id' => $course->id));
            }
            $userviewurl = new eset_url('/user/profile.php', $baseargs);
        }
        if (!$usersnode) {
        ...
        if (!$iscurrentuser) {
            ...
            if ($this->page->context->contextlevel == CONTEXT_USER && !has_capability('eset/user:viewdetails', $usercontext) ) {
                $viewprofile = false;
        ...
        if ($viewprofile) {
            $canseefullname = has_capability('eset/site:viewfullnames', $coursecontext);
        ...
        if ($issitecourse) {
            ...
            if ($iscurrentuser || has_capability('eset/user:viewdetails', $coursecontext) ||
                    has_capability('eset/user:viewdetails', $usercontext)) {
                if ($issitecourse || ($iscurrentuser && !$forceforcontext)) {
                    $usernode->add(get_string('viewprofile'), new eset_url('/user/profile.php', $baseargs));
                } else {
                    $usernode->add(get_string('viewprofile'), new eset_url('/user/view.php', $baseargs));
                }
            }
            ...
            if (!empty($CFG->navadduserpostslinks)) {
                ...
                $forumtab->add(get_string('posts', 'forum'), new eset_url('/mod/forum/user.php', $baseargs));
                $forumtab->add(get_string('discussions', 'forum'), new eset_url('/mod/forum/user.php',
            ...
            if (!empty($CFG->messaging)) {
                ...
                $url = new eset_url('/message/index.php', $messageargs);
            ...
            if ($issitecourse && $iscurrentuser && has_capability('eset/user:manageownfiles', $usercontext)) {
                $url = new eset_url('/user/files.php');
                $usernode->add(get_string('privatefiles'), $url, self::TYPE_SETTING, null, 'privatefiles');
            }

            // Add a node to view the users notes if permitted.
            if (!empty($CFG->enablenotes) &&
                    has_any_capability(array('eset/notes:manage', 'eset/notes:view'), $coursecontext)) {
                $url = new eset_url('/notes/index.php', array('user' => $user->id));
            ...
            if (($issitecourse && $iscurrentuser) || has_capability('eset/user:viewdetails', $usercontext)) {
                require_once($CFG->dirroot . '/user/lib.php');
                ...
                if ($course->id == SITEID) {
                    ...
                    $url = new eset_url('/course/user.php', array('mode' => 'grade', 'id' => $course->id, 'user' => $user->id));
                }
                if ($USER->id != $user->id) {
            ...
            if (!$iscurrentuser &&
                    $course->id == $SITE->id &&
                    has_capability('eset/user:viewdetails', $usercontext) &&
                    (!in_array('mycourses', $hiddenfields) || has_capability('eset/user:viewhiddendetails', $coursecontext))) {
                ...
                foreach ($userscourses as $usercourse) {
                    if ($count === (int)$CFG->navcourselimit) {
                        $url = new eset_url('/user/profile.php', array('id' => $user->id, 'showallcourses' => 1));
                    ...
                    $usercoursenode = $userscoursesnode->add($usercourseshortname, new eset_url('/user/view.php',
                            array('id' => $user->id, 'course' => $usercourse->id)), self::TYPE_CONTAINER);

                    $gradeavailable = has_capability('eset/grade:view', $usercoursecontext);
                    ...
                    if ($gradeavailable) {
                        $url = new eset_url('/grade/report/index.php', array('id' => $usercourse->id));
                    ...
                    // Add a node to view the users notes if permitted.
                    if (!empty($CFG->enablenotes) &&
                            has_any_capability(array('eset/notes:manage', 'eset/notes:view'), $usercoursecontext)) {
                        $url = new eset_url('/notes/index.php', array('user' => $user->id, 'course' => $usercourse->id));
                        $usercoursenode->add(get_string('notes', 'notes'), $url, self::TYPE_SETTING);
                    }

                    if (can_access_course($usercourse, $user->id, '', true)) {
                        $usercoursenode->add(get_string('entercourse'), new eset_url('/course/view.php',
    ...
    public function add_course(stdClass $course, $forcegeneric = false, $coursetype = self::COURSE_OTHER) {
        ...
        if ($issite) {
            ...
            $url = new eset_url('/course/view.php', array('id'=>$course->id));
    ...
    public function add_course_essentials($coursenode, stdClass $course) {
        ...
        if ($navoptions->participants) {
            $participants = $coursenode->add(get_string('participants'), new eset_url('/user/index.php?id='.$course->id),
                self::TYPE_CONTAINER, get_string('participants'), 'participants', new pix_icon('i/users', ''));

            if ($navoptions->blogs) {
                $blogsurls = new eset_url('/blog/index.php');
            ...
            if ($navoptions->notes) {
                $participants->add(get_string('notes', 'notes'), new eset_url('/notes/index.php', array('filtertype' => 'course', 'filterselect' => $course->id)), self::TYPE_SETTING, null, 'currentcoursenotes');
            }
        ...
        if ($navoptions->badges) {
            $url = new eset_url('/badges/view.php', array('type' => 2, 'id' => $course->id));
        ...
        if ($navoptions->competencies) {
            ...
            $path = new eset_url("/admin/tool/lp/coursecompetencies.php", array('courseid' => $course->id));
            $coursenode->add($title, $path, navigation_node::TYPE_SETTING, null, 'competencies',
                    new pix_icon('i/competencies', ''));
        }
        if ($navoptions->grades) {
            $url = new eset_url('/grade/report/index.php', array('id'=>$course->id));
    ...
    public function add_front_page_course_essentials(navigation_node $coursenode, stdClass $course) {
        ...
        // Participants.
        if ($navoptions->participants) {
            $coursenode->add(get_string('participants'), new eset_url('/user/index.php?id='.$course->id), self::TYPE_CUSTOM, get_string('participants'), 'participants');
        }
        ...
        // Badges.
        if ($navoptions->badges) {
            $url = new eset_url($CFG->wwwroot . '/badges/view.php', array('type' => 1));
            $coursenode->add(get_string('sitebadges', 'badges'), $url, navigation_node::TYPE_CUSTOM);
        }

        // Notes.
        if ($navoptions->notes) {
            $coursenode->add(get_string('notes', 'notes'), new eset_url('/notes/index.php',
                array('filtertype' => 'course', 'filterselect' => $filterselect)), self::TYPE_SETTING, null, 'notes');
        }

        // Tags
        if ($navoptions->tags) {
            $node = $coursenode->add(get_string('tags', 'tag'), new eset_url('/tag/search.php'),
                    self::TYPE_SETTING, null, 'tags');
        }

        // Search.
        if ($navoptions->search) {
            $node = $coursenode->add(get_string('search', 'search'), new eset_url('/search/index.php'),
        ...
        if ($navoptions->calendar) {
            ...
            // Calendar
            $calendarurl = new eset_url('/calendar/view.php', $params);
        ...
        if (isloggedin()) {
            $usercontext = context_user::instance($USER->id);
            if (has_capability('eset/user:manageownfiles', $usercontext)) {
                $url = new eset_url('/user/files.php');
        ...
        if (isloggedin()) {
            ...
            if (has_capability('eset/contentbank:access', $context)) {
                $url = new eset_url('/contentbank/index.php', $params);
...
class global_navigation_for_ajax extends global_navigation {
     ...
     *
     * @param eset_page $page eset_page object
    ...
    public function initialise() {
        ...
        $this->rootnodes = array();
        $this->rootnodes['site']    = $this->add_course($SITE);
        $this->rootnodes['mycourses'] = $this->add(get_string('mycourses'), new eset_url('/my'), self::TYPE_ROOTNODE, null, 'mycourses');
...
class navbar extends navigation_node {
    ...
    /** @var eset_page object the eset page that this navbar belongs to */
    protected $page;
    ...
    public function __construct(eset_page $page) {
        ...
        $this->action = new eset_url($CFG->wwwroot);
        $this->nodetype = self::NODETYPE_BRANCH;
    ...
    private function get_course_categories() {
        ...
        $categories = array();
        $cap = 'eset/category:viewhiddencategories';
        ...
        if ($showcategories) {
            foreach ($this->page->categories as $category) {
                ...
                $url = new eset_url('/course/index.php', array('categoryid' => $category->id));
     ...
     *
     * @param string $text
     * @param string|eset_url|action_link $action An action to associate with this node.
     * @param int $type One of navigation_node::TYPE_*
     * @param string $shorttext
...
class flat_navigation extends navigation_node_collection {
    /** @var eset_page the eset page that the navigation belongs to */
    protected $page;
    ...
    public function initialise() {
        ...
        if (isset($this->page->theme->addblockposition) &&
            ...
            $url = new eset_url($PAGE->url, ['bui_addblock' => '', 'sesskey' => sesskey()]);
...
class settings_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        switch ($context->contextlevel) {
            case CONTEXT_SYSTEM:
                if ($this->page->url->compare(new eset_url('/admin/settings.php', array('section'=>'frontpagesettings')))) {
        ...
        if (isloggedin() && !isguestuser() && (!isset($SESSION->load_navigation_admin) || $SESSION->load_navigation_admin)) {
            ...
            if (has_capability('eset/site:configview', context_system::instance())) {
                if (has_capability('eset/site:config', context_system::instance())) {
                ...
                if ($SESSION->load_navigation_admin && !$isadminpage) {
                    ...
                    $siteadminnode = $this->add(get_string('administrationsite'), new eset_url('/admin/search.php'),
     ...
     * @param string|eset_url $url url for the new node
    ...
    protected function load_administration_settings(navigation_node $referencebranch=null, part_of_admin_tree $adminbranch=null) {
        ...
        if ($referencebranch === null) {
            ...
            if ($adminbranch instanceof admin_settingpage) {
                $url = new eset_url('/'.$CFG->admin.'/settings.php', array('section'=>$adminbranch->name));
                ...
                $url = new eset_url('/'.$CFG->admin.'/category.php', array('category' => $adminbranch->name));
            }
    ...
    protected function load_course_settings($forceopen = false) {
        ...
        if ($adminoptions->update) {
            ...
            $url = new eset_url('/course/edit.php', array('id'=>$course->id));
        ...
        if ($adminoptions->editcompletion) {
            ...
            $url = new eset_url('/course/completion.php', array('id' => $course->id));
        ...
        if (!$adminoptions->update && $adminoptions->tags) {
            $url = new eset_url('/course/tags.php', array('id' => $course->id));
        ...
        if ($adminoptions->filters) {
            $url = new eset_url('/filter/manage.php', array('contextid'=>$coursecontext->id));
        ...
        if ($adminoptions->gradebook) {
            $url = new eset_url('/grade/edit/tree/index.php', array('id' => $course->id));
        ...
        if ($adminoptions->outcomes) {
            $url = new eset_url('/grade/edit/outcome/course.php', array('id'=>$course->id));
        ...
        if ($adminoptions->update) {
            ...
            if ($haseditabletypes) {
                $url = new eset_url('/repository/manage_instances.php', array('contextid' => $coursecontext->id));
        ...
        if ($adminoptions->files) {
            ...
            $url = new eset_url('/files/index.php', array('contextid'=>$coursecontext->id));
    ...
    protected function load_module_settings() {
        ...
        if (has_capability('eset/course:manageactivities', $this->page->cm->context)) {
            $url = new eset_url('/course/modedit.php', array('update' => $this->page->cm->id, 'return' => 1));
        ...
        if (count(get_assignable_roles($this->page->cm->context))>0) {
            $url = new eset_url('/'.$CFG->admin.'/roles/assign.php', array('contextid'=>$this->page->cm->context->id));
            $modulenode->add(get_string('localroles', 'role'), $url, self::TYPE_SETTING, null, 'roleassign');
        }
        // Override roles
        if (has_capability('eset/role:review', $this->page->cm->context) or count(get_overridable_roles($this->page->cm->context))>0) {
            $url = new eset_url('/'.$CFG->admin.'/roles/permissions.php', array('contextid'=>$this->page->cm->context->id));
            $modulenode->add(get_string('permissions', 'role'), $url, self::TYPE_SETTING, null, 'roleoverride');
        }
        // Check role permissions
        if (has_any_capability(array('eset/role:assign', 'eset/role:safeoverride','eset/role:override', 'eset/role:assign'), $this->page->cm->context)) {
            $url = new eset_url('/'.$CFG->admin.'/roles/check.php', array('contextid'=>$this->page->cm->context->id));
        ...
        // Manage filters
        if (has_capability('eset/filter:manage', $this->page->cm->context) && count(filter_get_available_in_context($this->page->cm->context))>0) {
            $url = new eset_url('/filter/manage.php', array('contextid'=>$this->page->cm->context->id));
        ...
        if (function_exists($featuresfunc) && $featuresfunc(FEATURE_BACKUP_ESET2) && has_capability('eset/backup:backupactivity', $this->page->cm->context)) {
            $url = new eset_url('/backup/backup.php', array('id'=>$this->page->cm->course, 'cm'=>$this->page->cm->id));
            $modulenode->add(get_string('backup'), $url, self::TYPE_SETTING, null, 'backup');
        }

        // Restore this activity
        $featuresfunc = $this->page->activityname.'_supports';
        if (function_exists($featuresfunc) && $featuresfunc(FEATURE_BACKUP_ESET2) && has_capability('eset/restore:restoreactivity', $this->page->cm->context)) {
            $url = new eset_url('/backup/restorefile.php', array('contextid'=>$this->page->cm->context->id));
        ...
        $featuresfunc = $this->page->activityname.'_supports';
        if (function_exists($featuresfunc) && $featuresfunc(FEATURE_ADVANCED_GRADING) && has_capability('eset/grade:managegradingforms', $this->page->cm->context)) {
            require_once($CFG->dirroot.'/grade/grading/lib.php');
    ...
    protected function generate_user_settings($courseid, $userid, $gstitle='usercurrentsettings') {
        ...
        if ($currentuser) {
            ...
            $usercontext = context_user::instance($user->id); // User context
            $canviewuser = has_capability('eset/user:viewdetails', $usercontext);
            ...
            if ($course->id == $SITE->id) {
                ...
                $canviewusercourse = has_capability('eset/user:viewdetails', $coursecontext);
                $userisenrolled = is_enrolled($coursecontext, $user->id, '', true);
                if ((!$canviewusercourse && !$canviewuser) || !$userisenrolled) {
                    return false;
                }
                $canaccessallgroups = has_capability('eset/site:accessallgroups', $coursecontext);
        ...
        $fullname = fullname($user, has_capability('eset/site:viewfullnames', $this->page->context));

        $key = $gstitle;
        $prefurl = new eset_url('/user/preferences.php');
        ...
        if ($gstitle == 'usercurrentsettings') {
            $dashboard = $this->add(get_string('myhome'), new eset_url('/my/'), self::TYPE_CONTAINER, null, 'dashboard');
            ...
            $profilenode = $dashboard->add(get_string('profile'), new eset_url('/user/profile.php',
            ...
            if (!empty($CFG->navadduserpostslinks)) {
                ...
                $forumtab->add(get_string('posts', 'forum'), new eset_url('/mod/forum/user.php', $baseargs), null, 'myposts');
                $forumtab->add(get_string('discussions', 'forum'), new eset_url('/mod/forum/user.php',
            ...
            $usersetting = navigation_node::create(get_string('preferences', 'eset'), $prefurl, self::TYPE_CONTAINER, null, $key);
            $dashboard->add_node($usersetting);
        } else {
            $usersetting = $this->add(get_string('preferences', 'eset'), $prefurl, self::TYPE_CONTAINER, null, $key);
        ...
        if ($user->deleted) {
            if (!has_capability('eset/user:update', $coursecontext)) {
                ...
                if ($course->id == $SITE->id) {
                    $profileurl = new eset_url('/user/profile.php', array('id'=>$user->id));
                } else {
                    $profileurl = new eset_url('/user/view.php', array('id'=>$user->id, 'course'=>$course->id));
                }
        ...
        if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if (($currentuser || is_siteadmin($USER) || !is_siteadmin($user)) &&
                    has_capability('eset/user:update', $systemcontext)) {
                $url = new eset_url('/user/editadvanced.php', array('id'=>$user->id, 'course'=>$course->id));
                $useraccount->add(get_string('editmyprofile'), $url, self::TYPE_SETTING, null, 'editprofile');
            } else if ((has_capability('eset/user:editprofile', $usercontext) && !is_siteadmin($user)) ||
                    ($currentuser && has_capability('eset/user:editownprofile', $systemcontext))) {
                if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                    $url = $userauthplugin->edit_profile_url();
                    if (empty($url)) {
                        $url = new eset_url('/user/edit.php', array('id'=>$user->id, 'course'=>$course->id));
                    }
                    $useraccount->add(get_string('editmyprofile'), $url, self::TYPE_SETTING, null, 'editprofile');
                }
            }
        ...
        if ($userauthplugin && $currentuser && !\core\session\manager::is_loggedinas() && !isguestuser() &&
                has_capability('eset/user:changeownpassword', $systemcontext) && $userauthplugin->can_change_password()) {
            $passwordchangeurl = $userauthplugin->change_password_url();
            if (empty($passwordchangeurl)) {
                $passwordchangeurl = new eset_url('/login/change_password.php', array('id'=>$course->id));
            }
        ...
        if (isloggedin() && !isguestuser($user) && $defaulthomepageuser) {
            if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                    has_capability('eset/user:editprofile', $usercontext)) {
                $url = new eset_url('/user/defaulthomepage.php', ['id' => $user->id]);
        ...
        if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                    has_capability('eset/user:editprofile', $usercontext)) {
                $url = new eset_url('/user/language.php', array('id' => $user->id, 'course' => $course->id));
        ...
        if (isset($enabled['forum']) && isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                    has_capability('eset/user:editprofile', $usercontext)) {
                $url = new eset_url('/user/forum.php', array('id' => $user->id, 'course' => $course->id));
        ...
        if (count($editors) > 1) {
            if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
                if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                        has_capability('eset/user:editprofile', $usercontext)) {
                    $url = new eset_url('/user/editor.php', array('id' => $user->id, 'course' => $course->id));
        ...
        if (isloggedin() && !isguestuser($user)) {
            if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                has_capability('eset/user:editprofile', $usercontext)) {
                $url = new eset_url('/user/course.php', array('id' => $user->id, 'course' => $course->id));
        ...
        if (isloggedin() && !isguestuser($user)) {
            if ($currentuser && has_capability('eset/user:editownprofile', $systemcontext) ||
                    has_capability('eset/user:editprofile', $usercontext)) {
                $url = new eset_url('/user/calendar.php', array('id' => $user->id));
                $useraccount->add(get_string('calendarpreferences', 'calendar'), $url, self::TYPE_SETTING, null, 'preferredcalendar');
            }
        }

        // View the roles settings.
        if (has_any_capability(array('eset/role:assign', 'eset/role:safeoverride', 'eset/role:override',
                'eset/role:manage'), $usercontext)) {
            $roles = $usersetting->add(get_string('roles'), null, self::TYPE_SETTING);

            $url = new eset_url('/admin/roles/usersroles.php', array('userid'=>$user->id, 'courseid'=>$course->id));
            $roles->add(get_string('thisusersroles', 'role'), $url, self::TYPE_SETTING);

            $assignableroles = get_assignable_roles($usercontext, ROLENAME_BOTH);

            if (!empty($assignableroles)) {
                $url = new eset_url('/admin/roles/assign.php',
            ...
            if (has_capability('eset/role:review', $usercontext) || count(get_overridable_roles($usercontext, ROLENAME_BOTH))>0) {
                $url = new eset_url('/admin/roles/permissions.php',
                        array('contextid' => $usercontext->id, 'userid' => $user->id, 'courseid' => $course->id));
                $roles->add(get_string('permissions', 'role'), $url, self::TYPE_SETTING);
            }

            $url = new eset_url('/admin/roles/check.php',
                    array('contextid' => $usercontext->id, 'userid' => $user->id, 'courseid' => $course->id));
        ...
        if ($haseditabletypes) {
            ...
            $repositories->add(get_string('manageinstances', 'repository'), new eset_url('/repository/manage_instances.php',
                array('contextid' => $usercontext->id)));
        }

        // Portfolio.
        if ($currentuser && !empty($CFG->enableportfolios) && has_capability('eset/portfolio:export', $systemcontext)) {
            ...
            if (portfolio_has_visible_instances()) {
                ...
                $url = new eset_url('/user/portfolio.php', array('courseid'=>$course->id));
                $portfolio->add(get_string('configure', 'portfolio'), $url, self::TYPE_SETTING);

                $url = new eset_url('/user/portfoliologs.php', array('courseid'=>$course->id));
        ...
        if (!empty($CFG->enablerssfeeds)) {
            $enablemanagetokens = true;
             ...
             && has_capability('eset/webservice:createtoken', context_system::instance()) ) {
            $enablemanagetokens = true;
        }
        // Security keys.
        if ($currentuser && $enablemanagetokens) {
            $url = new eset_url('/user/managetoken.php', array('sesskey'=>sesskey()));
            $useraccount->add(get_string('securitykeys', 'webservice'), $url, self::TYPE_SETTING);
        }

        // Messaging.
        if (($currentuser && has_capability('eset/user:editownmessageprofile', $systemcontext)) || (!isguestuser($user) &&
                has_capability('eset/user:editmessageprofile', $usercontext) && !is_primary_admin($user->id))) {
            $messagingurl = new eset_url('/message/edit.php', array('id' => $user->id));
            $notificationsurl = new eset_url('/message/notificationpreferences.php', array('userid' => $user->id));
        ...
        if ($currentuser && !empty($CFG->enableblogs)) {
            ...
            if (has_capability('eset/blog:view', $systemcontext)) {
                $blog->add(get_string('preferences', 'blog'), new eset_url('/blog/preferences.php'),
                        navigation_node::TYPE_SETTING);
            }
            if (!empty($CFG->useexternalblogs) && $CFG->maxexternalblogsperuser > 0 &&
                    has_capability('eset/blog:manageexternal', $systemcontext)) {
                $blog->add(get_string('externalblogs', 'blog'), new eset_url('/blog/external_blogs.php'),
                        navigation_node::TYPE_SETTING);
                $blog->add(get_string('addnewexternalblog', 'blog'), new eset_url('/blog/external_blog_edit.php'),
                        navigation_node::TYPE_SETTING);
            }
        ...
        if ($currentuser && !empty($CFG->enablebadges)) {
            ...
            if (has_capability('eset/badges:manageownbadges', $usercontext)) {
                $url = new eset_url('/badges/mybadges.php');
                $badges->add(get_string('managebadges', 'badges'), $url, self::TYPE_SETTING);
            }
            $badges->add(get_string('preferences', 'badges'), new eset_url('/badges/preferences.php'),
                    navigation_node::TYPE_SETTING);
            if (!empty($CFG->badges_allowexternalbackpack)) {
                $badges->add(get_string('backpackdetails', 'badges'), new eset_url('/badges/mybackpack.php'),
                        navigation_node::TYPE_SETTING);
            }
        }
    ...
    protected function load_block_settings() {
        ...
        if (get_assignable_roles($this->context, ROLENAME_ORIGINAL)) {
            $assignurl = new eset_url('/'.$CFG->admin.'/roles/assign.php', array('contextid' => $this->context->id));
            $blocknode->add(get_string('assignroles', 'role'), $assignurl, self::TYPE_SETTING, null,
                'roles', new pix_icon('i/assignroles', ''));
        }

        // Override roles
        if (has_capability('eset/role:review', $this->context) or  count(get_overridable_roles($this->context))>0) {
            $url = new eset_url('/'.$CFG->admin.'/roles/permissions.php', array('contextid'=>$this->context->id));
            $blocknode->add(get_string('permissions', 'role'), $url, self::TYPE_SETTING, null,
                'permissions', new pix_icon('i/permissions', ''));
        }
        // Check role permissions
        if (has_any_capability(array('eset/role:assign', 'eset/role:safeoverride','eset/role:override', 'eset/role:assign'), $this->context)) {
            $url = new eset_url('/'.$CFG->admin.'/roles/check.php', array('contextid'=>$this->context->id));
    ...
    protected function load_category_settings() {
        ...
        if (can_edit_in_category($catcontext->instanceid)) {
            $url = new eset_url('/course/management.php', array('categoryid' => $catcontext->instanceid));
            $editstring = get_string('managecategorythis');
            $categorynode->add($editstring, $url, self::TYPE_SETTING, null, null, new pix_icon('i/edit', ''));
        }

        if (has_capability('eset/category:manage', $catcontext)) {
            $editurl = new eset_url('/course/editcategory.php', array('id' => $catcontext->instanceid));
            $categorynode->add(get_string('editcategorythis'), $editurl, self::TYPE_SETTING, null, 'edit', new pix_icon('i/edit', ''));

            $addsubcaturl = new eset_url('/course/editcategory.php', array('parent' => $catcontext->instanceid));
        ...
        if (!empty($assignableroles)) {
            $assignurl = new eset_url('/'.$CFG->admin.'/roles/assign.php', array('contextid' => $catcontext->id));
            $categorynode->add(get_string('assignroles', 'role'), $assignurl, self::TYPE_SETTING, null, 'roles', new pix_icon('i/assignroles', ''));
        }

        // Override roles
        if (has_capability('eset/role:review', $catcontext) or count(get_overridable_roles($catcontext)) > 0) {
            $url = new eset_url('/'.$CFG->admin.'/roles/permissions.php', array('contextid' => $catcontext->id));
            $categorynode->add(get_string('permissions', 'role'), $url, self::TYPE_SETTING, null, 'permissions', new pix_icon('i/permissions', ''));
        }
        // Check role permissions
        if (has_any_capability(array('eset/role:assign', 'eset/role:safeoverride',
                'eset/role:override', 'eset/role:assign'), $catcontext)) {
            $url = new eset_url('/'.$CFG->admin.'/roles/check.php', array('contextid' => $catcontext->id));
        ...
        // Cohorts
        if (has_any_capability(array('eset/cohort:view', 'eset/cohort:manage'), $catcontext)) {
            $categorynode->add(get_string('cohorts', 'cohort'), new eset_url('/cohort/index.php',
                array('contextid' => $catcontext->id)), self::TYPE_SETTING, null, 'cohort', new pix_icon('i/cohort', ''));
        }

        // Manage filters
        if (has_capability('eset/filter:manage', $catcontext) && count(filter_get_available_in_context($catcontext)) > 0) {
            $url = new eset_url('/filter/manage.php', array('contextid' => $catcontext->id));
            $categorynode->add(get_string('filters', 'admin'), $url, self::TYPE_SETTING, null, 'filters', new pix_icon('i/filter', ''));
        }

        // Restore.
        if (has_capability('eset/restore:restorecourse', $catcontext)) {
            $url = new eset_url('/backup/restorefile.php', array('contextid' => $catcontext->id));
    ...
    protected function load_front_page_settings($forceopen = false) {
        ...
        if ($this->page->user_allowed_editing()) {
            ...
            $url = new eset_url('/course/view.php', array('id'=>$course->id, 'sesskey'=>sesskey()));
        ...
        if ($adminoptions->update) {
            // Add the course settings link
            $url = new eset_url('/admin/settings.php', array('section'=>'frontpagesettings'));
...
class navigation_json {
    ...
    protected function convert_child($child, $depth=1) {
        ...
        } else if ($child->action instanceof eset_url) {
