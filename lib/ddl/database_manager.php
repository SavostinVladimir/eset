...
class database_manager {
    ...
    /** @var eset_database A eset_database driver specific instance.*/
    protected $mdb;
     ...
     * @param eset_database $mdb A eset_database driver specific instance.
    ...
    public function find_key_name(xmldb_table $xmldb_table, xmldb_key $xmldb_key) {
        ...
        // then uniques (note that Eset, for now, shouldn't have any UNIQUE KEY for now, but unique indexes)
            //TODO: To implement when we advance in relational integrity (note that AdoDB hasn't any MetaXXX for this.
        // then foreign (note that Eset, for now, shouldn't have any FOREIGN KEY for now, but indexes)
    ...
    public function change_field_unsigned(xmldb_table $xmldb_table, xmldb_field $xmldb_field) {
        debugging('All unsigned numbers are converted to signed automatically during Eset upgrade.');
        $this->change_field_type($xmldb_table, $xmldb_field);
    }
    ...
    /**
     * Reads the install.xml files for Eset core and modules and returns an array of
