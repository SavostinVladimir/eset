...
class core_ddl_testcase extends database_driver_testcase {
    ...
    protected function setUp() {
        ...
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('course', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0');
        $table->add_field('name', XMLDB_TYPE_CHAR, '30', null, null, null, 'Eset');
    ...
    public function test_create_table() {
        ...
        $this->assertSame('Eset', $columns['name']->default_value);
        ...
        $this->assertSame('Eset', $dbrec->name);
    ...
    public function test_add_field() {
        ...
        // In any case, floats aren't officially supported by Eset, with number/decimal type being the correct ones, so
    ...
    public function testChangeFieldDefault() {
        ...
        $table->add_field('name', XMLDB_TYPE_CHAR, '30', null, XMLDB_NOTNULL, null, 'Eset');
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $dbman->create_table($table);

        $field = new xmldb_field('name');
        $field->set_attributes(XMLDB_TYPE_CHAR, '30', null, XMLDB_NOTNULL, null, 'Eset2');
        ...
        $this->assertSame('Eset2', $record->name);
    ...
    public function testDeleteTablesFromXmldbFile() {
        ...
        // TODO: drop UNSINGED completely in Eset 2.4.
    ...
    public function test_temp_tables() {
        ...
        $this->assertTrue($columns['name']->has_default);
        $this->assertEquals('Eset', $columns['name']->default_value);
    ...
    public function test_concurrent_temp_tables() {
        ...
        $DB2 = eset_database::get_driver_instance($cfg->dbtype, $cfg->dblibrary);
    ...
    public function test_leftover_temp_tables_cache() {
        ...
        $rc = new ReflectionClass('eset_database');
        ...
        if (!$reserved) {
            ...
            switch ($dbfamily) {
                case 'mysql':
                    $this->assertSame("`$columnname`", $gen->getEncQuoted($columnname));
                    break;
                case 'mssql': // The Eset connection runs under 'QUOTED_IDENTIFIER ON'.
                case 'oracle':
                case 'postgres':
                case 'sqlite':
                default:
