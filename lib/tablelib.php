...
class flexible_table {
    ...
    /**
     * Sets $this->baseurl.
     * @param eset_url|string $url the url with params needed to call up this page
     */
    function define_baseurl($url) {
        $this->baseurl = new eset_url($url);
    }
    ...
    function col_fullname($row) {
        ...
        $name = fullname($row, has_capability('eset/site:viewfullnames', $this->get_context()));
        if ($this->download) {
            return $name;
        }
        ...
        if ($COURSE->id == SITEID) {
            $profileurl = new eset_url('/user/profile.php', array('id' => $userid));
        } else {
            $profileurl = new eset_url('/user/view.php',
     ...
     * options are changed.
     */
    function format_text($text, $format=FORMAT_ESET, $options=NULL, $courseid=NULL) {
        ...
        foreach ($this->columns as $column => $index) {
            ...
            switch ($column) {
                ...
                case 'fullname':
                    ...
                    if (has_capability('eset/site:viewfullnames', $this->get_context())) {
...
class table_sql extends flexible_table {
    ...
    public function close_recordset() {
        if ($this->rawdata && ($this->rawdata instanceof \core\dml\recordset_walk ||
                $this->rawdata instanceof eset_recordset)) {
            $this->rawdata->close();
            $this->rawdata = null;
        }
    }
