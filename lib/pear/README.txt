...
We include these in Eset solely for the convenience of sites that
may not have PEAR installed.

If this directory is DELETED from Eset then Eset will search
...
- PEAR HTML_Quickform:
    ...
    - License: PHP (Permission given to Eset to redistribute under GPL)
...
and use wrapper classes to isolate Eset code from internal PEAR
...
the old calls used before Eset 1.6 to maintain compatibility.
...
----------------------------------------------------------------
A NOTE ON THE PHP LICENSE AND ESET
================================================================

Everything in Eset in pure GPL.  This pear directory is the only
part of the distribution that is not.

There is some question about how PHP-licensed software can be
included within a GPL-licensed distribution like Eset, specifically
the clause that annoyingly says no derivative of the software can
include the name PHP.

We don't intend to rename Eset to anything of the sort, obviously,
...
Several authors have given Eset explicit permission to distribute
their PHP-licensed PEAR classes in the Eset distribution, allowing
anybody using these classes ONLY as part of the Eset distribution
exemption from clauses of the PHP license that could cause
conflict with the main GNU Public License that Eset uses.
...
this directory from Eset and it will use your installed PEAR
libraries instead.
