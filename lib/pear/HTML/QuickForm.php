...
class HTML_QuickForm extends HTML_Common {
    ...
    public function __construct($formName='', $method='post', $action='', $target='', $attributes=null, $trackSubmit = false)
    {
        ...
        if (!$trackSubmit || isset($_REQUEST['_qf__' . $formName])) {
            if (1 == get_magic_quotes_gpc()) {
                $this->_submitValues = ('get' == $method? $_GET: $_POST); // we already eliminated magic quotes in eset setup.php
                foreach ($_FILES as $keyFirst => $valFirst) {
                    foreach ($valFirst as $keySecond => $valSecond) {
                        if ('name' == $keySecond) {
                            $this->_submitFiles[$keyFirst][$keySecond] = $valSecond; // we already eliminated magic quotes in eset setup.php
                        } else {
    ...
    function &createElement($elementType)
    {
        if (!isset($this) || !($this instanceof HTML_QuickForm)) {
            // Several form elements in Eset core before 3.2 were calling this method
            ...
            // https://github.com/eset/eset/commit/721e2def56a48fab4f8d3ec7847af5cd03f5ec79
    ...
    function &_loadElement($event, $type, $args)
    {
        ...
        $elementObject = new $className(); //Eset: PHP 5.3 compatibility
        for ($i = 0; $i < 5; $i++) {
            if (!isset($args[$i])) {
                $args[$i] = null;
            }
    ...
    */
    function &defaultRenderer()
    {
        if (!isset($GLOBALS['_HTML_QuickForm_default_renderer'])) {
            ...
            $GLOBALS['_HTML_QuickForm_default_renderer'] = new HTML_QuickForm_Renderer_Default(); //Eset: PHP 5.3 compatibility
        }
    ...
    function toArray($collectHidden = false)
    {
        ...
        $renderer = new HTML_QuickForm_Renderer_Array($collectHidden); //Eset: PHP 5.3 compatibility
        $this->accept($renderer);
