...
abstract class xml_database_exporter extends database_exporter {
    ...
    /**
     * Callback function. Outputs open XML PI and eset_database opening tag.
     *
     * @param float $version the version of the system which generating the data
     * @param string $release eset release info
    ...
    public function begin_database_export($version, $release, $timestamp, $description) {
        ...
        $this->output('<eset_database version="'.$version.'" release="'.$release.'" timestamp="'.$timestamp.'"'.(empty ($description) ? '' : ' comment="'.htmlspecialchars($description, ENT_QUOTES, 'UTF-8').'"').'>');
    }
    ...
    /**
     * Callback function. Outputs eset_database closing tag.
     */
    public function finish_database_export() {
        $this->output('</eset_database>');
    }
