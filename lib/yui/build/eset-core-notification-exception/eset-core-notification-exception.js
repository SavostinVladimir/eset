YUI.add('eset-core-notification-exception', function (Y, NAME) {
 ...
 * @submodule eset-core-notification-exception
 */

var EXCEPTION_NAME = 'Eset exception',
    EXCEPTION;
...
Y.extend(EXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h1 id="eset-dialogue-' + config.COUNT + '-header-text">' + Y.Escape.html(config.name) + '</h1>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="eset-exception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="eset-exception-message">' + Y.Escape.html(this.get('message')) + '</div>'))
                .append(Y.Node.create('<div class="eset-exception-param hidden param-filename"><label>' +
                        M.util.get_string('file', 'eset') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('fileName')) + '</div>'))
                .append(Y.Node.create('<div class="eset-exception-param hidden param-linenumber"><label>' +
