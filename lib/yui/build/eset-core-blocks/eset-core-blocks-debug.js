YUI.add('eset-core-blocks', function (Y, NAME) {
 ...
 *
 * @module eset-core-blockdraganddrop
 */
...
var SELECTOR = {
    DRAGHANDLE: '.' + CSS.HEADER + ' .commands .eset-core-dragdrop-draghandle'
};
...
M.core.blockdraganddrop.is_using_blocks_render_method = function() {
    if (this._isusingnewblocksmethod === null) {
        ...
        if (goodregions > 0 && allregions > 0 && goodregions !== allregions) {
            Y.log('Both core_renderer::blocks and core_renderer::blocks_for_region have been used.', 'warn', 'eset-core_blocks');
        }
    }
...
M.core.blockdraganddrop.init = function(params) {
    if (this.is_using_blocks_render_method()) {
        Y.log('Block drag and drop initialised for the blocks method.', 'info', 'eset-core_blocks');
        new MANAGER(params);
    } else {
        Y.log('Block drag and drop initialised with the legacy manager (blocks_for_region used).', 'info', 'eset-core_blocks');
