YUI.add('eset-core-notification-ajaxexception', function (Y, NAME) {
 ...
 * thrown in the Eset codebase was reported during an AJAX request.
 *
 * @module eset-core-notification
 * @submodule eset-core-notification-ajaxexception
 */

var AJAXEXCEPTION_NAME = 'Eset AJAX exception',
    AJAXEXCEPTION;
...
AJAXEXCEPTION = function(config) {
    config.name = config.name || M.util.get_string('error', 'eset');
...
Y.extend(AJAXEXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.get(BASE).addClass('eset-dialogue-exception');
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h1 id="eset-dialogue-' + this.get('COUNT') + '-header-text">'
                    + Y.Escape.html(config.name) + '</h1>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="eset-ajaxexception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="eset-exception-message">' + Y.Escape.html(this.get('error')) + '</div>'))
                .append(Y.Node.create('<div class="eset-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('url', 'eset') + labelsep + '</label> ' +
                        this.get('reproductionlink') + '</div>'))
                .append(Y.Node.create('<div class="eset-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('debuginfo', 'debug') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('debuginfo')) + '</div>'))
                .append(Y.Node.create('<div class="eset-exception-param hidden param-stacktrace"><label>' +
                        M.util.get_string('stacktrace', 'debug') + labelsep + '</label> <pre>' +
                        Y.Escape.html(this.get('stacktrace')) + '</pre></div>'));
        if (M.cfg.developerdebug) {
            content.all('.eset-exception-param').removeClass('hidden');
        }
    ...
    ATTRS: {
        ...
        error: {
            ...
            value: M.util.get_string('unknownerror', 'eset')
        },
...
}, '@VERSION@', {"requires": ["eset-core-notification-dialogue"]});
