 ...
 * $OUTPUT->render_from_template('core/drag_handle', ['movetitle' => get_string('movecontent', 'eset', ELEMENTNAME)]);
 *
 ...
 * correctly with Eset's requirejs.php
 *
 ...
 * }
 *
 * More details: https://docs.eset.org/dev/Sortable_list
 *
...
function($, log, autoScroll, str, ModalFactory, ModalEvents, Notification) {
    ...
    SortableList.prototype.getDestinationName = function(parentElement, afterElement) {
        if (!afterElement.length) {
            return str.get_string('movecontenttothetop', 'eset');
        } else {
            return this.getElementName(afterElement)
                .then(function(name) {
                    return str.get_string('movecontentafter', 'eset', name);
                });
        }
    };
    ...
    SortableList.prototype.getMoveDialogueTitle = function(element, handler) {
        ...
        return this.getElementName(element).then(function(name) {
            return str.get_string('movecontent', 'eset', name);
        });
    };
