...
class processor implements  \core_analytics\classifier, \core_analytics\regressor, \core_analytics\packable {
    ...
    protected function is_webserver_ready() {
        if (empty($this->pathtopython)) {
            $settingurl = new \eset_url('/admin/settings.php', array('section' => 'systempaths'));
            return get_string('pythonpathnotdefined', 'mlbackend_python', $settingurl->out());
        }

        // Check the installed pip package version.
        $cmd = "{$this->pathtopython} -m esetmlbackend.version";
    ...
    public function train_classification($uniqueid, \stored_file $dataset, $outputdir) {
        ...
        if (!$resultobj = json_decode($result)) {
            throw new \eset_exception('errorpredictwrongformat', 'analytics', '', json_last_error_msg());
        }
    ...
    public function evaluate_classification($uniqueid, $maxdeviation, $niterations, \stored_file $dataset,
        ...
        if (!$this->useserver) {
            ...
            if (!mkdir($rundir, $CFG->directorypermissions, true)) {
                throw new \eset_exception('errorexportmodelresult', 'analytics');
            }
            ...
            if (!$success) {
                ...
                throw new \eset_exception('errorpredictionsprocessor', 'analytics', '', $a);
            }
    ...
    public function import(string $uniqueid, string $modeldir, string $importdir) : bool {
        ...
        if (!$this->useserver) {
            ...
            if ($exitcode != 0) {
                throw new \eset_exception('errorimportmodelresult', 'analytics');
            }
    ...
    protected function get_file_path(\stored_file $file) {
        // From eset filesystem to the local file system.
        // This is not ideal, but there is no read access to eset filesystem files.
    ...
    protected function exec_command(string $modulename, array $params, string $errorlangstr) {
        ...
        $cmd = $this->pathtopython . ' -m esetmlbackend.' . $modulename . ' ';
        ...
        if (!$result) {
            throw new \eset_exception($errorlangstr, 'analytics');
        }
     ...
     *
     * @param  \eset_url      $url            The requested url in the python ML server
    ...
    protected function server_request($url, string $method, array $requestparams, ?array $options = null) {
        ...
        if ($curl->info['http_code'] !== 200) {
            throw new \eset_exception('errorserver', 'mlbackend_python', '',
     ...
     *
     * @param  string $actual   Actual esetmlbackend version
    ...
    private function version_check_return($actual, $vercheck) {
        ...
        if (!$this->useserver) {
            ...
            // value is 'python'.
            $cmd = "python -m esetmlbackend.version";
        }
        return get_string('pythonpackagenotinstalled', 'mlbackend_python', $cmd);
    }
