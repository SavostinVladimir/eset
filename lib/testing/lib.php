 ...
 * @param string $esetpath starting with "/", ex: "/admin/tool/cli/init.php"
 * @return string path relative to current directory or absolute path
 */
function testing_cli_argument_path($esetpath) {
    global $CFG;

    if (isset($CFG->admin) and $CFG->admin !== 'admin') {
        $esetpath = preg_replace('|^/admin/|', "/$CFG->admin/", $esetpath);
    }
    ...
    $path = realpath($CFG->dirroot.$esetpath);
...
function testing_update_composer_dependencies(bool $selfupdate = true, bool $updatedependencies = true): void {
    ...
    // Switch to Eset's dirroot for easier path handling.
    chdir($dirroot);
 ...
 * OS. ESET_DIRECTORY_SEPARATOR should only be used when parsing
