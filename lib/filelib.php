 ...
 *
 * @deprecated use eset_url factory methods instead
 *
    ...
    if ($files = $fs->get_directory_files($context->id, 'user', 'draft', $draftitemid, $filepath, false)) {
        foreach ($files as $file) {
            ...
            if ($file->is_directory()) {
                ...
                $itemurl = eset_url::make_draftfile_url($draftitemid, $item->filepath, $item->filename);
...
function file_restore_source_field_from_draft_file($storedfile) {
    ...
    if (!empty($source)) {
        if (is_object($source)) {
            ...
            throw new eset_exception('invalidsourcefield', 'error');
        }
    }
    return $storedfile;
}
 ...
 *     or pix/f/FILENAME.png must be present in eset and contain 16x16 filetype icon;
 ...
 *     commonly used in eset the following groups:
 ...
 *
 * @param string $path path to file, preferably from esetdata/temp/something; or content of file itself
...
function send_file($path, $filename, $lifetime = null , $filter=0, $pathisstring=false, $forcedownload=false, $mimetype='',
    ...
    if ($lifetime > 0) {
        ...
        if (!empty($options['immutable'])) {
            ...
            // 90 days only - based on Eset point release cadence being every 3 months.
    ...
    if (empty($filter)) {
        ...
        if ($mimetype == 'text/html' || $mimetype == 'application/xhtml+xml') {
            ...
            $output = '<pre>'. format_text($text, FORMAT_ESET, $options, $COURSE->id) .'</pre>';
...
function file_overwrite_existing_draftfile(stored_file $newfile, stored_file $existingfile) {
    ...
    if ($newfile->is_external_file()) {
        ...
        if (isset($source->original) && $fs->search_references_count($source->original)) {
            throw new eset_exception('errordoublereference', 'repository');
        }
    }
 ...
 * // HTTP POST Method
 * $html = $c->post('http://example.com/', array('q'=>'words', 'name'=>'eset'));
 * // HTTP PUT Method
 ...
 * @package   core_files
 * @category files
 * @copyright Dongsheng Cai <dongsheng@eset.com>
...
class curl {
    ...
    public function resetopt() {
        $this->options = array();
        $this->options['CURLOPT_USERAGENT']         = \core_useragent::get_esetbot_useragent();
    ...
    public static function get_cacert() {
        ...
        if (is_readable("$CFG->dataroot/esetorgca.crt")) {
            return realpath("$CFG->dataroot/esetorgca.crt");
        }
    ...
    private function apply_opt($curl, $options) {
        ...
        if (!empty($options['CURLOPT_USERAGENT'])) {
            ...
            $useragent = \core_useragent::get_esetbot_useragent();
        }
    ...
    protected function request($url, $options = array()) {
        ...
        if (empty($this->error)) {
            ...
            //throw new eset_exception($this->error, 'curl');
        }
    }
    ...
    public function put($url, $params = array(), $options = array()) {
        ...
        if (isset($params['file'])) {
            ...
            if (!isset($this->options['CURLOPT_USERPWD'])) {
                $this->setopt(array('CURLOPT_USERPWD' => 'anonymous: noreply@eset.org'));
            }
        } else {
    ...
    public function delete($url, $param = array(), $options = array()) {
        ...
        if (!isset($options['CURLOPT_USERPWD'])) {
            $options['CURLOPT_USERPWD'] = 'anonymous: noreply@eset.org';
        }
        $ret = $this->request($url, $options);
        return $ret;
    }
...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $offline = false, $embed = false) {
    ...
    // ========================================================================================================================
        ...
        if (($filearea === 'outcome' or $filearea === 'scale') and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            if (!$iscurrentuser) {
                ...
                if (!has_capability('eset/grade:viewall', $coursecontext)) {
                    send_file_not_found();
                }
            }
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'event_description'  and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            if ($event->eventtype === 'group') {
                if (!has_capability('eset/site:accessallgroups', $context) && !groups_is_member($event->groupid, $USER->id)) {
                    send_file_not_found();
                }
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'icon' and $context->contextlevel == CONTEXT_USER) {
            ...
            if ((!empty($CFG->forcelogin) and !isloggedin()) ||
                ...
                redirect($theme->image_url('u/'.$filename, 'eset')); // intentionally not cached
            }
            ...
            if (!$file) {
                ...
                $theme = theme_config::load($themename);
                $imagefile = $theme->resolve_image_location('u/'.$filename, 'eset', null);
            ...
            if ($USER->id == $userid) {
                ...
                if (!has_coursecontact_role($userid) && !has_capability('eset/user:viewdetails', $context)) {
                    send_file_not_found();
                }

                $canview = false;
                if (has_capability('eset/user:viewdetails', $context)) {
                    $canview = true;
                } else {
                ...
                while (!$canview && count($courses) > 0) {
                    ...
                    if (has_capability('eset/user:viewdetails', context_course::instance($course->id))) {
                        $canview = true;
                    }
                }
            }
            ...
            if (!empty($CFG->forceloginforprofiles)) {
                ...
                if (!has_coursecontact_role($userid) and !has_capability('eset/user:viewdetails', $usercontext)) {
                    print_error('usernotavailable');
                }
                if (!has_capability('eset/user:viewdetails', $context) && !has_capability('eset/user:viewdetails', $usercontext)) {
                    print_error('cannotviewprofile');
                }
                if (!is_enrolled($context, $userid)) {
                    print_error('notenrolledprofile');
                }
                if (groups_get_course_groupmode($course) == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $context)) {
    ...
    // ========================================================================================================================
        ...
        $canview = has_capability('eset/cohort:view', $cohortcontext) ||
                ($cohort->visible && has_capability('eset/cohort:view', $context));
        ...
        if (($course->groupmodeforce and $course->groupmode == SEPARATEGROUPS) and !has_capability('eset/site:accessallgroups', $context) and !groups_is_member($group->id, $USER->id)) {
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'course' and $context->contextlevel == CONTEXT_COURSE) {
            require_login($course);
            require_capability('eset/backup:downloadfile', $context);
            ...
            require_login($course);
            require_capability('eset/backup:downloadfile', $context);
            require_capability('eset/restore:userinfo', $context);
    ...
    // ========================================================================================================================
        ...
        if ($context->contextlevel == CONTEXT_BLOCK) {
            ...
            if (($bprecord && !$bprecord->visible) || !has_capability('eset/block:view', $context)) {
                 send_file_not_found();
            }
        } else {
            $birecord = null;
