 ...
 * 3. User approved the request, and get back to eset
...
class oauth_helper {
    ...
    function __construct($args) {
        ...
        if (!empty($args['oauth_callback'])) {
            $this->oauth_callback = new eset_url($args['oauth_callback']);
        }
    ...
    public function request_token() {
        ...
        if (empty($result['oauth_token'])) {
            throw new eset_exception('oauth1requesttoken', 'core_error', '', null, $content);
        }
    ...
    public function get_access_token($token, $secret, $verifier='') {
        ...
        if (empty($keys['oauth_token']) || empty($keys['oauth_token_secret'])) {
            throw new eset_exception('oauth1accesstoken', 'core_error', '', null, $content);
        }
    ...
    public function parse_result($str) {
        if (empty($str)) {
            throw new eset_exception('error');
        }
        $parts = explode('&', $str);
        $result = array();
...
abstract class oauth2_client extends curl {
    ...
    /** @var eset_url $returnurl URL to return to after authenticating */
     ...
     * @param eset_url $returnurl
     * @param string $scope
     */
    public function __construct($clientid, $clientsecret, eset_url $returnurl, $scope) {
        parent::__construct();
     ...
     *
     * @return eset_url url of callback
     */
    public static function callback_url() {
        global $CFG;

        return new eset_url('/admin/oauth2callback.php');
    }
    ...
    public function get_login_url() {
        ...
        return new eset_url($this->auth_url(), $params);
    }
    ...
    public function upgrade_token($code) {
        ...
        if ($this->info['http_code'] !== 200) {
            throw new eset_exception('Could not upgrade oauth token');
        }

        $r = json_decode($response);

        if (is_null($r)) {
            throw new eset_exception("Could not decode JSON token response");
        }

        if (!empty($r->error)) {
            throw new eset_exception($r->error . ' ' . $r->error_description);
        }
