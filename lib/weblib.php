 ...
 *
 * Library of all general-purpose Eset PHP functions and constants
 * that produce HTML output
 *
 * Other main libraries:
 * - datalib.php - functions that access the database.
 * - esetlib.php - general-purpose Eset functions.
 *
 * @package    core
 * @subpackage lib
...
/**
 * A eset_url comparison using this flag will return true if the base URLs match, params are ignored.
 */
define('URL_MATCH_BASE', 0);

/**
 * A eset_url comparison using this flag will return true if the base URLs match and the params of url1 are part of url2.
 */
define('URL_MATCH_PARAMS', 1);

/**
 * A eset_url comparison using this flag will return true if the two URLs are identical, except for the order of the params.
 */
...
/**
 * Determines whether or not the Eset site is being served over HTTPS.
 *
 ...
 *
 * It can be used in eset pages where config.php has been included without any further includes.
 *
 ...
 * and / or to process form data. A eset_url object :
 ...
 * @link http://docs.eset.org/dev/lib/weblib.php_eset_url See short write up here
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @package core
 */
class eset_url {
    ...
    /**
     * Create new instance of eset_url.
     *
     * @param eset_url|string $url - eset_url means make a copy of another
     *      eset_url and change parameters, string means full url or shortened
    ...
    public function __construct($url, array $params = null, $anchor = null) {
        ...
        if ($url instanceof eset_url) {
            ...
            $parts = parse_url($url);
            if ($parts === false) {
                throw new eset_exception('invalidurl');
            }
    ...
    public function out($escaped = true, array $overrideparams = null) {
        ...
        if (isset($CFG->urlrewriteclass) && !isset($CFG->upgraderunning)) {
            ...
            if ($pluginurl instanceof eset_url) {
                $url = $pluginurl;
            }
        }
    ...
    public static function make_file_url($urlbase, $path, $forcedownload = false) {
        ...
        $url = new eset_url($urlbase, $params);
 ...
 *
 * Used on most forms in Eset to check for data
 ...
 * @return string
 */
function page_get_doc_link_path(eset_page $page) {
    global $CFG;

    if (empty($CFG->docroot) || during_initial_install()) {
        return '';
    }
    if (!has_capability('eset/site:doclinks', $page->context)) {
        return '';
    }
...
function validate_email($address) {
    global $CFG;
    require_once($CFG->libdir.'/phpmailer/eset_phpmailer.php');

    return eset_phpmailer::validateAddress($address) && !preg_match('/[<>]/', $address);
}
...
function format_text_menu() {
    return array (FORMAT_ESET => get_string('formattext'),
 ...
 *            [FORMAT_ESET, FORMAT_HTML, FORMAT_PLAIN, FORMAT_MARKDOWN]
 ...
 * @return string
 */
function format_text($text, $format = FORMAT_ESET, $options = null, $courseiddonotuse = null) {
    global $CFG, $DB, $PAGE;
    ...
    switch ($format) {
        ...
        case FORMAT_WIKI:
            ...
            $text = '<p>NOTICE: Wiki-like formatting has been removed from Eset.  You should not be seeing
                     this message as all texts should have been converted to Markdown format instead.
                     Please post a bug report to http://eset.org/bugs with information about where you
        ...
        default:  // FORMAT_ESET or anything else.
 ...
 * @param boolean $striplinks To strip any link in the result text. Eset 1.8 default changed from false to true! MDL-8713
 ...
 * @param string $text The text to be formatted. This is raw text originally from user input.
 * @param int $format Identifier of the text format to be used
 *            [FORMAT_ESET, FORMAT_HTML, FORMAT_PLAIN, FORMAT_WIKI, FORMAT_MARKDOWN]
 * @return string
 */
...
/**
 * Removes the usage of Eset files from a text.
 *
 * In some rare cases we need to re-use a text that already has embedded links
 * to some files hosted within Eset. But the new area in which we will push
...
function trusttext_trusted($context) {
    return (trusttext_active() and has_capability('eset/site:trustcontent', $context));
}
 ...
 * Eset pages through XSS attacks.
 *
 ...
 * @param int|string $format deprecated parameter, should always contain FORMAT_HTML or FORMAT_ESET
...
function clean_text($text, $format = FORMAT_HTML, $options = array()) {
    ...
    // rawurlencode(), htmlentities(), htmlspecialchars(), p(), s(), eset_url, html_writer and friends!!!
...
function purify_html($text, $options = array()) {
    ...
    if (empty($purifiers[$type])) {
        ...
        $config->set('HTML.DefinitionID', 'esethtml');
 ...
 * by {@link format_text()} to convert FORMAT_ESET to HTML. You are supposed
 ...
 * @param int|false $contentformat False for strings or the text format: FORMAT_ESET/FORMAT_HTML/FORMAT_PLAIN/FORMAT_MARKDOWN
...
function content_to_text($content, $contentformat) {
    ...
    switch ($contentformat) {
        ...
        case FORMAT_MARKDOWN:
            ...
            break;
        default:
            // FORMAT_HTML, FORMAT_ESET and $contentformat = false, the later one are strings usually formatted through
...
/**
 * Send the HTTP headers that Eset requires.
 *
...
function send_headers($contenttype, $cacheable = true) {
    ...
    // The Eset app must be allowed to embed content always.
    if (empty($CFG->allowframembedding) && !core_useragent::is_eset_app()) {
...
function print_group_picture($group, $courseid, $large = false, $return = false, $link = true, $includetoken = false) {
    ...
    $output = '';
    if ($link or has_capability('eset/site:accessallgroups', $context)) {
        $linkurl = new eset_url('/user/index.php', ['id' => $courseid, 'group' => $group->id]);
 ...
 *                 If the group picture is included in an e-mail or some other location where the audience is a specific
 *                 user who will not be logged in when viewing, then we use a token to authenticate the user.
 * @return eset_url Returns the url for the group picture.
 */
function get_group_picture_url($group, $courseid, $large = false, $includetoken = false) {
    ...
    if ($group->hidepicture and !has_capability('eset/course:managegroups', $context)) {
        return;
    }
    ...
    $grouppictureurl = eset_url::make_pluginfile_url(
...
function print_recent_activity_note($time, $user, $text, $link, $return=false, $viewfullnames=null) {
    ...
    if (is_null($viewfullnames)) {
        ...
        $viewfullnames = has_capability('eset/site:viewfullnames', $context);
    }
    ...
    foreach ($modinfo->cms as $mod) {
        ...
        if ($mod->sectionnum >= 0 and $section != $mod->sectionnum) {
            ...
            if ($thissection->visible or
                    ...
                    has_capability('eset/course:viewhiddensections', $coursecontext)) {
...
function print_grade_menu($courseid, $name, $current, $includenograde=true, $return=false) {
    ...
    $link = new eset_url('/course/scales.php', array('id' => $courseid, 'list' => 1));
 ...
 * @param eset_url|string $link The link to use for the continue button
 ...
 *
 * @param eset_url|string $url A eset_url to redirect to. Strings are not to be trusted!
...
function redirect($url, $message='', $delay=null, $messagetype = \core\output\notification::NOTIFY_INFO) {
    ...
    if (CLI_SCRIPT or AJAX_SCRIPT) {
        ...
        throw new eset_exception('redirecterrordetected', 'error');
    }
    ...
    if ($PAGE) {
        ...
        $PAGE->set_title(get_string('pageshouldredirect', 'eset'));
    }
    ...
    // Sanitise url - we can not rely on eset_url or our URL cleaning
    ...
    if ($PAGE) {
        $CFG->docroot = false; // To prevent the link to eset docs from being displayed on redirect page.
...
function print_maintenance_message() {
    ...
    header($_SERVER['SERVER_PROTOCOL'] . ' 503 Eset under maintenance');
    header('Status: 503 Eset under maintenance');
 ...
 *
 * It is not recommended to use this function in Eset 2.5 but it is left for backward
 * compartibility.
 *
 ...
 *
 * @todo Use a central function to create the popup calls all over Eset and
...
function get_formatted_help_string($identifier, $component, $ajax = false, $a = null) {
    ...
    if ($sm->string_exists($identifier . '_help', $component)) {
        ...
        if ($sm->string_exists($helplink, $component)) {  // Link to further info in Eset docs.
            $link = get_string($helplink, $component);
            $linktext = get_string('morehelp');

            $data->doclink = new stdClass();
            $url = new eset_url(get_docs_url($link));
            if ($ajax) {
