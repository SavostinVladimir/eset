...
function badges_notify_badge_award(badge $badge, $userid, $issued, $filepathhash) {
    ...
    $issuedlink = html_writer::link(new eset_url('/badges/badge.php', array('hash' => $issued)), $badge->name);
    ...
    $eventdata->customdata        = [
        'notificationiconurl' => eset_url::make_pluginfile_url(
...
function badges_add_course_navigation(navigation_node $coursenode, stdClass $course) {
    ...
    $canmanage = has_any_capability(array('eset/badges:viewawarded',
                                          ...
                                          'eset/badges:deletebadge'), $coursecontext);
    ...
    if (!empty($CFG->enablebadges) && !empty($CFG->badges_allowcoursebadges) && !$isfrontpage && $canmanage) {
        ...
        $url = new eset_url('/badges/index.php', array('type' => BADGE_TYPE_COURSE, 'id' => $course->id));

        $coursenode->get('coursebadges')->add(get_string('managebadges', 'badges'), $url,
            navigation_node::TYPE_SETTING, null, 'coursebadges');

        if (has_capability('eset/badges:createbadge', $coursecontext)) {
            $url = new eset_url('/badges/newbadge.php', array('type' => BADGE_TYPE_COURSE, 'id' => $course->id));
...
function print_badge_image(badge $badge, stdClass $context, $size = 'small') {
    ...
    $imageurl = eset_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', $fsize, false);
...
function badges_bake($hash, $badgeid, $userid = 0, $pathhash = false) {
    ...
    $fileurl = eset_url::make_pluginfile_url($user_context->id, 'badges', 'userbadge', $badge->id, '/', $hash, true);
    return $fileurl;
}
...
function badges_check_backpack_accessibility() {
    ...
    $fakeassertion = new eset_url('/badges/assertion.php', array('b' => 'abcd1234567890'));
...
function badges_setup_backpack_js() {
    ...
    if (!empty($CFG->badges_allowexternalbackpack)) {
        if (badges_open_badges_backpack_api() == OPEN_BADGES_V1) {
            ...
            $PAGE->requires->js(new eset_url(BADGE_BACKPACKURL . '/issuer.js'), true);
...
function badges_create_site_backpack($data) {
    ...
    require_capability('eset/badges:manageglobalsettings', $context);
...
function badges_get_default_issuer() {
    ...
    $issuerid = new eset_url('/badges/issuer_json.php');
 ...
 * @param string $internalid The id for this object on the Eset site.
...
function badges_send_verification_email($email, $backpackid, $backpackpassword) {
    ...
    $verificationurl = new eset_url($verificationurl);
...
function badge_assemble_notification(stdClass $badge) {
    ...
    if ($msgs = $DB->get_records_select('badge_issued', 'issuernotified IS NULL AND badgeid = ?', array($badge->id))) {
        ...
        foreach ($msgs as $msg) {
            $issuedlink = html_writer::link(new eset_url('/badges/badge.php', array('hash' => $msg->uniquehash)), $badge->name);
...
function badges_verify_backpack(int $backpackid) {
    ...
    if (empty($backpack->apiversion) || ($backpack->apiversion == OPEN_BADGES_V2)) {
        ...
        if (empty($result) || !empty($result->error)) {
            ...
            $backpackurl = (new eset_url('/badges/backpacks.php', $params))->out(false);

            $message = get_string('sitebackpackwarning', 'badges', ['url' => $backpackurl, 'warning' => $warning]);
            $icon = $OUTPUT->pix_icon('i/warning', get_string('warning', 'eset'));
