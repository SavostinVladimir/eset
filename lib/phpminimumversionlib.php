...
// ESET_INTERNAL check intentionally missing to allow this to be used more widely!

/**
 * A set of PHP-compatible convenience functions to check Eset minimum PHP version in
 * a unified place.
 *
 * PLEASE NOTE: This file is made to be both php-version compatible and without requirement on
 * any eset functions or installation so it can be used in installer or incompatible PHP versions.
 *
 * @package    core
 * @copyright  2017 Dan Poltawski <dan@eset.com>
 ...
 */
function eset_require_minimum_php_version() {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP!
    eset_minimum_php_version_is_met(true);
}

/**
 * Tests the current PHP version against Eset's minimum requirement. When requirement
 ...
 */
function eset_minimum_php_version_is_met($haltexecution = false) {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP.
    // Do not use modern php features or Eset convenience functions (e.g. localised strings).

    $minimumversion = '7.1.0';
    $esetrequirementchanged = '3.7';

    if (version_compare(PHP_VERSION, $minimumversion) < 0) {
        if ($haltexecution) {
            $error = "Eset ${esetrequirementchanged} or later requires at least PHP ${minimumversion} "
