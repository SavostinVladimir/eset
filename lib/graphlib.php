 ...
 * as part of Eset (http://eset.com).  Modified lines
 * are marked with "Eset".
 */
...
class graph {
  ...
  var $offset_relation   =   null;         // array of offsets for different sets of data
    ...
    function init() {
      ...
      /// Eset mods:  overrides the font path and encodings
    ...
    function draw_set($order, $set, $offset) {
      ...
      foreach ($this->x_data as $index => $x) {
        ...
        if (($bar!='none') && (string)$thisY != 'none') {
            if ($relatedset = $this->offset_relation[$set]) {                               // Eset
                $yoffset = $this->calculated['y_plot'][$relatedset][$index];                // Eset
            } else {                                                                        // Eset
                $yoffset = 0;                                                               // Eset
            }                                                                               // Eset
            //$this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set);           // Eset
            $this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set, $yoffset);   // Eset
        }
    ...
    function find_range($data, $min, $max, $resolution) {
      ...
      if ($factor > 0.1) { // To avoid some wierd rounding errors (Eset)
        $factor = round($factor * 1000.0) / 1000.0; // To avoid some wierd rounding errors (Eset)
      } // To avoid some wierd rounding errors (Eset)
    ...
    function bar($x, $y, $type, $size, $colour, $offset, $index, $yoffset) {
      ...
      if ($this->parameter['zero_axis'] != 'none') {
        ...
        // Eset addition, plus the function parameter yoffset
        if ($yoffset) {                                           // Eset
            $yoffset = $yoffset - round(($bottom - $v) / 2.0);    // Eset
            $bottom -= $yoffset;                                  // Eset
            $v      -= $yoffset;                                  // Eset
        }                                                         // Eset
