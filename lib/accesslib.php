...
/** Capability allow management of trusts - NOT IMPLEMENTED YET - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
define('RISK_MANAGETRUST', 0x0001);
/** Capability allows changes in system configuration - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
define('RISK_CONFIG',      0x0002);
/** Capability allows user to add scripted content - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
define('RISK_XSS',         0x0004);
/** Capability allows access to personal user information - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
define('RISK_PERSONAL',    0x0008);
/** Capability allows users to add content others may see - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
define('RISK_SPAM',        0x0010);
/** capability allows mass delete of data belonging to other users - see {@link http://docs.eset.org/dev/Hardening_new_Roles_system} */
...
function has_capability($capability, context $context, $user = null, $doanything = true) {
    ...
    if (strpos($capability, 'eset/legacy:') === 0) {
    ...
    if (!empty($CFG->contextlocking)) {
        if ($capinfo->captype === 'write' && $context->locked) {
            ...
            // It does not apply to eset/site:managecontextlocks - this is to allow context locking to be unlocked.
            if ($capinfo->name !== 'eset/site:managecontextlocks') {
...
function guess_if_creator_will_have_course_capability($capability, context $context, $user = null) {
    ...
    if (!has_capability('eset/course:create', $context, $user)) {
        return false;
    }
    ...
    if ($context->contextlevel == CONTEXT_COURSE) {
        if (is_viewing($context, $user, 'eset/role:assign') or is_enrolled($context, $user, 'eset/role:assign')) {
            return false;
        }
    } else {
        if (has_capability('eset/course:view', $context, $user) and has_capability('eset/role:assign', $context, $user)) {
            return false;
        }
    }
 ...
 *
 * NOTE before Eset 2.0, this function attempted to make an appropriate
...
function reload_all_capabilities() {
    ...
    foreach ($sw as $path => $roleid) {
        if ($record = $DB->get_record('context', array('path'=>$path))) {
            ...
            if (has_capability('eset/role:switchroles', $context)) {
...
function create_role($name, $shortname, $description, $archetype = '') {
    ...
    if (strpos($archetype, 'eset/legacy:') !== false) {
...
function is_guest(context $context, $user = null) {
    ...
    if (has_capability('eset/course:view', $coursecontext, $user)) {
...
/**
 * Returns true if the user has eset/course:view capability in the course,
...
function can_access_course(stdClass $course, $user = null, $withcapability = '', $onlyactive = false) {
    ...
    if (!$course->visible and !has_capability('eset/course:viewhiddencourses', $coursecontext, $userid)) {
        return false;
    }
 ...
 *
 * @access private
 * @param string $component full plugin name, examples: 'eset', 'mod_forum'
 ...
 *
 * @access private
 * @param string $component - examples: 'eset', 'mod_forum'
 * @return array array of capabilities
 */
function get_cached_capabilities($component = 'eset') {
    global $DB;
 ...
 * If no parameters are given, the function updates the core eset
 * capabilities.
 *
 ...
 *
 * @access private
 * @param string $component examples: 'eset', 'mod_forum', 'block_quiz_results'
 * @return boolean true if success, exception in case of any problems
 */
function update_capabilities($component = 'eset') {
    global $DB, $OUTPUT;
...
/**
 * Return a link to eset docs for a given capability name
 *
 * @param stdClass $capability a capability - a row from the mdl_capabilities table.
 * @return string the human-readable capability name as a link to Eset Docs.
 */
...
function get_component_string($component, $contextlevel) {
    ...
    if ($component === 'eset' || $component === 'core') {
...
function get_profile_roles(context $context) {
    ...
    if (has_capability('eset/role:assign', $context)) {
    ...
    if ($roles = $DB->get_records_sql($sql, $params)) {
        ...
        foreach ($roles as $roleid => $unused) {
            if (isset($viewableroles[$roleid])) {
                $url = new eset_url('/user/index.php', ['contextid' => $context->id, 'roleid' => $roleid]);
 ...
 * to cause an out-of-memory error on large Eset sites, so this combination is deprecated and
...
function get_users_roles(context $context, $userids = [], $checkparentcontexts = true, $order = 'c.contextlevel DESC, r.sortorder ASC') {
    ...
    if (!$userids && $checkparentcontexts) {
        debugging('Please do not call get_users_roles() with $checkparentcontexts = true ' .
                'and $userids array not set. This combination causes large Eset sites ' .
...
function get_assignable_roles(context $context, $rolenamedisplay = ROLENAME_ALIAS, $withusercounts = false, $user = null) {
    ...
    if (!has_capability('eset/role:assign', $context, $userid)) {
        if ($withusercounts) {
 ...
 * test the eset/role:switchroles to see if the user is allowed to switch in the first place.
 *
...
function get_overridable_roles(context $context, $rolenamedisplay = ROLENAME_ALIAS, $withcounts = false) {
    ...
    if (!has_any_capability(array('eset/role:safeoverride', 'eset/role:override'), $context)) {
        if ($withcounts) {
            return array(array(), array(), array());
        } else {
            return array();
...
function get_with_capability_join(context $context, $capability, $useridcolumn) {
    ...
    if (!empty($CFG->contextlocking) && $context->locked) {
        ...
        $excludelockedcapsparams['managelockscap'] = 'eset/site:managecontextlocks';
    }
 ...
 *               have capability both $capability and eset/site:accessallgroups
...
function get_users_by_capability(context $context, $capability, $fields = '', $sort = '', $limitfrom = '', $limitnum = '',
    ...
    if ($groups) {
        ...
        if ($useviewallgroups) {
            $viewallgroupsusers = get_users_by_capability($context, 'eset/site:accessallgroups', 'u.id, u.id', '', '', '', '', $exceptions);
 ...
 *
 * Note that eset is based on capabilities and it is usually better
...
/**
 * Basic eset context abstraction class.
 *
 * Google confirms that no other important framework is using "context" class,
 * we could use something else like mcontext or eset_context, but we need to type
...
abstract class context extends stdClass implements IteratorAggregate {
    ...
    protected function __construct(stdClass $record) {
        ...
        } else if (!during_initial_install() && !eset_needs_upgrading()) {
            debugging('Locked value missing. Code is possibly not usings the getter properly.', DEBUG_DEVELOPER);
        }
    }
...
class context_user extends context {
    ...
    public function get_url() {
        ...
        if ($COURSE->id == SITEID) {
            $url = new eset_url('/user/profile.php', array('id'=>$this->_instanceid));
        } else {
            $url = new eset_url('/user/view.php', array('id'=>$this->_instanceid, 'courseid'=>$COURSE->id));
        }
        return $url;
    }
    ...
    public function get_capabilities() {
        ...
        $extracaps = array('eset/grade:viewall');
...
class context_coursecat extends context {
     ...
     */
    public function get_url() {
        return new eset_url('/course/index.php', array('categoryid' => $this->_instanceid));
    }
...
class context_course extends context {
    ...
    public function get_url() {
        if ($this->_instanceid != SITEID) {
            return new eset_url('/course/view.php', array('id'=>$this->_instanceid));
        }
...
class context_module extends context {
    ...
    public function get_url() {
        ...
        if ($modname = $DB->get_field_sql("SELECT md.name AS modname
                                             ...
                                             JOIN {modules} md ON md.id = cm.module
                                            WHERE cm.id = ?", array($this->_instanceid))) {
            return new eset_url('/mod/' . $modname . '/view.php', array('id'=>$this->_instanceid));
        }
...
class context_block extends context {
    ...
    public function get_context_name($withprefix = true, $short = false) {
        ...
        if ($blockinstance = $DB->get_record('block_instances', array('id'=>$this->_instanceid))) {
            global $CFG;
            require_once("$CFG->dirroot/blocks/esetblock.class.php");
