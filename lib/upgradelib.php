 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class upgrade_exception extends eset_exception {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class downgrade_exception extends eset_exception {
    function __construct($plugin, $oldversion, $newversion) {
        global $CFG;
        $plugin = is_null($plugin) ? 'eset' : $plugin;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class upgrade_requires_exception extends eset_exception {
    function __construct($plugin, $pluginversion, $currenteset, $requireeset) {
        global $CFG;
        $a = new stdClass();
        $a->pluginname     = $plugin;
        $a->pluginversion  = $pluginversion;
        $a->currenteset  = $currenteset;
        $a->requireeset  = $requireeset;
        parent::__construct('pluginrequirementsnotmet', 'error', "$CFG->wwwroot/$CFG->admin/index.php", $a);
    }
}

/**
 * Exception thrown when attempting to install a plugin that declares incompatibility with eset version
 *
 * @package    core
 * @subpackage upgrade
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class plugin_incompatible_exception extends eset_exception {
    /**
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class plugin_defective_exception extends eset_exception {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class plugin_misplaced_exception extends eset_exception {
    /**
     * Constructor.
...
function upgrade_stale_php_files_present() {
    ...
    $someexamplesofremovedfiles = array(
        ...
        '/course/yui/build/eset-course-modchooser/eset-course-modchooser-min.js',
        ...
        // Removed in 3.5.
        '/lib/dml/mssql_native_eset_database.php',
        '/lib/dml/mssql_native_eset_recordset.php',
        '/lib/dml/mssql_native_eset_temptables.php',
        // Removed in 3.4.
        '/auth/README.txt',
        '/calendar/set.php',
        '/enrol/users.php',
        ...
        '/calendar/yui/src/info/assets/skins/sam/eset-calendar-info.css',
    ...
    foreach ($plugs as $plug=>$fullplug) {
        ...
        if (!empty($plugin->requires)) {
            if ($plugin->requires > $CFG->version) {
                ...
                throw new plugin_defective_exception($component, 'Plugin is not compatible with Eset 2.x or later.');
            }
        }

        // Throw exception if plugin is incompatible with eset version.
 ...
 *
 * @param string $component name of component (eset, mod_assignment, etc.)
 * @return void
 */
...
function external_update_descriptions($component) {
    ...
    foreach ($dbservices as $dbservice) {
        ...
        if (isset($service['shortname']) and
            ...
            throw new eset_exception('installserviceshortnameerror', 'webservice', '', $service['shortname']);
        }
        if ($dbservice->shortname != $service['shortname']) {
            ...
            if (isset($service['shortname'])) { //we currently accepts multiple shortname == null
                ...
                if (!empty($existingservice)) {
                    throw new eset_exception('installexistingserviceshortnameerror', 'webservice', '', $service['shortname']);
                }
            }
    ...
    foreach ($services as $name => $service) {
        ...
        if (isset($service['shortname'])) { //we currently accepts multiple shortname == null
            $existingservice = $DB->get_record('external_services',
                    array('shortname' => $service['shortname']));
            if (!empty($existingservice)) {
                throw new eset_exception('installserviceshortnameerror', 'webservice');
            }
        }
    ...
    foreach ($functions as $function) {
        ...
        foreach ($services as $serviceshortname) {
            ...
            if (!empty($servicescache[$serviceshortname])) {
                ...
                // We don't allow functions to inject themselves in services created by an user in Eset.
...
function upgrade_started($preinstall=false) {
    ...
    if ($preinstall) {
        ...
        if (!CLI_SCRIPT and !$PAGE->headerprinted) {
            ...
            $PAGE->set_title($strupgrade.' - Eset '.$CFG->target_release);
...
function print_upgrade_part_start($plugin, $installation, $verbose) {
    global $OUTPUT;
    if (empty($plugin) or $plugin == 'eset') {
...
/**
 * Install core eset tables and initialize
...
function install_core($version, $verbose) {
    ...
    try {
        core_php_time_limit::raise(600);
        print_upgrade_part_start('eset', true, $verbose); // does not store upgrade running flag
        ...
        log_update_descriptions('eset');
        external_update_descriptions('eset');
        \core\task\manager::reset_scheduled_tasks_for_component('eset');
        \core_analytics\manager::update_default_models_for_component('eset');
        message_update_providers('eset');
        \core\message\inbound\manager::update_handlers_for_component('eset');
        core_tag_area::reset_definitions_for_component('eset');
...
function upgrade_core($version, $verbose) {
    ...
    try {
        ...
        print_upgrade_part_start('eset', false, $verbose);
        ...
        print_upgrade_part_end('eset', false, $verbose);
    } catch (Exception $ex) {
        upgrade_handle_exception($ex);
    } catch (Throwable $ex) {
        // Engine errors in PHP7 throw exceptions of type Throwable (this "catch" will be ignored in PHP5).
...
/**
 * Upgrade/install other parts of eset
 * @param bool $verbose
 * @return void, may throw exception
 */
        ...
        foreach ($data['methods'] as $method) {
            ...
            if (!empty($dataobject->classname)) {
                if (!class_exists($dataobject->classname)) {
                    throw new eset_exception('installnosuchmethod', 'mnet', '', (object)array('method' => $dataobject->functionname, 'class' => $dataobject->classname));
                }
                ...
                if (!array_key_exists($key, $cachedclasses)) { // look to see if we've already got a reflection object
                    try {
                        ...
                        throw new eset_exception('installreflectionclasserror', 'mnet', '', (object)array('method' => $dataobject->functionname, 'class' => $dataobject->classname, 'error' => $e->getMessage()));
                    }
                }
                $r =& $cachedclasses[$key];
                ...
                if (!function_exists($dataobject->functionname)) {
                    throw new eset_exception('installnosuchfunction', 'mnet', '', (object)array('method' => $dataobject->functionname, 'file' => $dataobject->filename));
                }
                try {
                    $functionreflect = new ReflectionFunction($dataobject->functionname);
                } catch (ReflectionException $e) { // catch these and rethrow them to something more helpful
                    throw new eset_exception('installreflectionfunctionerror', 'mnet', '', (object)array('method' => $dataobject->functionname, '' => $dataobject->filename, 'error' => $e->getMessage()));
                }
            }
    ...
    if (isset($CFG->config_php_settings['upgradekey'])) {
        if ($upgradekeyhash === null or $upgradekeyhash !== sha1($CFG->config_php_settings['upgradekey'])) {
            if (!$PAGE->headerprinted) {
                ...
                echo $output->upgradekey_form_page(new eset_url('/admin/index.php', array('cache' => 0)));
                die();
            } else {
 ...
 * @param eset_url|string|null $continue URL to proceed with installation at the validation screen
 * @param eset_url|string|null $return URL to go back on cancelling at the validation screen
 */
function upgrade_install_plugins(array $installable, $confirmed, $heading='', $continue=null, $return=null) {
    ...
    if ($confirmed) {
        ...
        if (!$pluginman->install_plugins($installable, true, true)) {
            throw new eset_exception('install_plugins_failed', 'core_plugin', $return);
        }
        ...
        // URL we must go to.
        $mustgoto = new eset_url('/admin/index.php', array('cache' => 0, 'confirmplugincheck' => 0));
