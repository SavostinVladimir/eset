...
class course_modinfo {
     ...
     * @throws eset_exception If the course-module does not exist
     */
    public function get_cm($cmid) {
        if (empty($this->cms[$cmid])) {
            throw new eset_exception('invalidcoursemodule', 'error');
        }
    ...
    public function get_section_info($sectionnumber, $strictness = IGNORE_MISSING) {
        if (!array_key_exists($sectionnumber, $this->sectioninfo)) {
            if ($strictness === MUST_EXIST) {
                throw new eset_exception('sectionnotexist');
            } else {
                return null;
            }
        }
     ...
     * @throws eset_exception if course is not found
     */
     ...
     * @throws eset_exception If the lock cannot be obtained
     */
    protected static function get_course_cache_lock($courseid) {
        ...
        if (!$lock) {
            throw new eset_exception('locktimeout', '', '', null,
     ...
     * @throws eset_exception if course is not found (if $course object misses some of the
 ...
 * Eset APIs such as filters, user information, output renderers and we
 ...
 * @property-read eset_url $url URL to link to for this module, or null if it doesn't have a view page - calculated on request
...
class cm_info implements IteratorAggregate {
    ...
    /**
     * @var eset_url full external url pointing to icon image for activity
     */
    private $iconurl;
    ...
    /**
     * List of methods with no arguments that were public prior to Eset 2.6.
     *
    ...
    private static $standardmethods = array(
        ...
        // Method obtain_dynamic_data() should not be called from outside of this class but it was public before Eset 2.6.
        'obtain_dynamic_data',
    );

    /**
     * Magic method to call functions that are now declared as private but were public in Eset before 2.6.
    ...
    /**
     * @return eset_url URL to link to for this module, or null if it doesn't have a view page
     */
    ...
    /**
     * @param eset_core_renderer $output Output render to use, or null for default (global)
     * @return eset_url Icon URL for a suitable icon to put beside this cm
     */
    ...
    public function get_grouping_label($textclasses = '') {
        ...
        if ($this->effectivegroupmode != NOGROUPS && !empty($this->groupingid) &&
                has_capability('eset/course:managegroups', context_course::instance($this->course))) {
     ...
     *
     * @param eset_url $iconurl full external url pointing to icon image for activity
     * @return void
     */
    public function set_icon_url(eset_url $iconurl) {
    ...
    public function __construct(course_modinfo $modinfo, $notused1, $mod, $notused2) {
        ...
        // iconurl may be stored as either string or instance of eset_url.
        $this->iconurl          = isset($mod->iconurl) ? new eset_url($mod->iconurl) : '';
        ...
        $this->url = $modviews[$this->modname]
                ? new eset_url('/mod/' . $this->modname . '/view.php', array('id'=>$this->id))
                : null;
    }
    ...
    private function update_user_visible() {
        ...
        if ((!$this->visible && !has_capability('eset/course:viewhiddenactivities', $this->get_context(), $userid)) ||
                (!$this->get_available() &&
                !has_capability('eset/course:ignoreavailabilityrestrictions', $this->get_context(), $userid))) {
        ...
        $this->uservisibleoncoursepage = $this->uservisible &&
            ($this->visibleoncoursepage ||
                has_capability('eset/course:manageactivities', $this->get_context(), $userid) ||
                has_capability('eset/course:activityvisibility', $this->get_context(), $userid));
 ...
 * @throws eset_exception when course is not found (nothing is thrown if resetting)
 */
 ...
 * @throws eset_exception If the item doesn't exist or is of wrong module name
 */
...
function get_course_and_cm_from_instance($instanceorid, $modulename, $courseorid = 0, $userid = 0) {
    ...
    if (!array_key_exists($instanceid, $instances)) {
        throw new eset_exception('invalidmoduleid', 'error', $instanceid);
    }
...
class cached_cm_info {
    ...
    /**
     * Component for icon for this activity, as per image_url; leave blank to use default 'eset'
     * component
     * @see renderer_base::image_url()
     * @var string
     */
...
class section_info implements IteratorAggregate {
    ...
    private function get_uservisible() {
        ...
        if (!$this->_visible || !$this->get_available()) {
            ...
            if (!$this->_visible && !has_capability('eset/course:viewhiddensections', $coursecontext, $userid) ||
                    (!$this->get_available() &&
                    !has_capability('eset/course:ignoreavailabilityrestrictions', $coursecontext, $userid))) {
