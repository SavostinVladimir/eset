...
/**
 * This file keeps track of upgrades to Eset.
 *
...
/**
 * Main upgrade tasks to be executed on Eset version bump
 *
 * This function is automatically executed after one bump in the Eset core
 ...
 *     // help you here. See {@link http://docs.eset.org/dev/XMLDB_editor}.
 *     upgrade_main_savepoint(true, XXXXXXXXXX.XX);
 * }
 *
 * All plugins within Eset (modules, blocks, reports...) support the existence of
 * their own upgrade.php file, using the "Frankenstyle" component name as
 * defined at {@link http://docs.eset.org/dev/Frankenstyle}, for example:
 ...
 *     - Data definition API: {@link http://docs.eset.org/dev/Data_definition_API}
...
function xmldb_main_upgrade($oldversion) {
    ...
    if ($oldversion < 2018110700.01) {
        ...
        unset_config('showcourseimages', 'esetcourse');
    ...
    if ($oldversion < 2019021500.01) {
        $insights = $DB->get_record('message_providers', ['component' => 'eset', 'name' => 'insights']);
        if (!empty($insights)) {
    ...
    if ($oldversion < 2019041800.01) {
        ...
        foreach ($legacyselfmessagesrs as $message) {
            ...
            $tabledata->fullmessageformat = $message->fullmessageformat ?? FORMAT_ESET;
    ...
    if ($oldversion < 2019042300.01) {
        $sql = "UPDATE {capabilities}
                   SET name = ?,
                       contextlevel = ?
                 WHERE name = ?";
        $DB->execute($sql, ['eset/category:viewcourselist', CONTEXT_COURSECAT, 'eset/course:browse']);

        $sql = "UPDATE {role_capabilities}
                   SET capability = ?
                 WHERE capability = ?";
        $DB->execute($sql, ['eset/category:viewcourselist', 'eset/course:browse']);
    ...
    if ($oldversion < 2019072500.01) {
        ...
        $oldloggedinconfig = get_config('message', 'message_provider_eset_messagecontactrequests_loggedin');
        $oldloggedoffconfig = get_config('message', 'message_provider_eset_messagecontactrequests_loggedoff');
        ...
        set_config('message_provider_eset_messagecontactrequests_loggedin', $newloggedinconfig, 'message');
        set_config('message_provider_eset_messagecontactrequests_loggedoff', $newloggedoffconfig, 'message');
    ...
    if ($oldversion < 2019083000.01) {
        ...
        if (!file_exists($CFG->dirroot . '/blocks/community/communitycourse.php')) {
            ...
            // Remove Eset-level community based capabilities.
    ...
    if ($oldversion < 2019100800.02) {
        // Rename the official eset sites directory the site is registered with.
        $DB->execute("UPDATE {registration_hubs}
                         SET hubname = ?, huburl = ?
                       WHERE huburl = ?", ['eset', 'https://stats.eset.org', 'https://eset.net']);

        // Convert the hub site specific settings to the new naming format without the hub URL in the name.
        $hubconfig = get_config('hub');

        if (!empty($hubconfig)) {
            foreach (upgrade_convert_hub_config_site_param_names($hubconfig, 'https://eset.net') as $name => $value) {
    ...
    if ($oldversion < 2020060500.01) {
        // Define field esetnetprofile to be added to user.
        $table = new xmldb_table('user');
        $field = new xmldb_field('esetnetprofile', XMLDB_TYPE_CHAR, '255', null, null, null, null, 'alternatename');

        // Conditionally launch add field esetnetprofile.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    ...
    if ($oldversion < 2020061500.02) {
        ...
        if (get_config('eset', 'agedigitalconsentmap') === $oldageofdigitalconsentmap) {
