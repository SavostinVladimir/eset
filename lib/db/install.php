 ...
 *
 * This function is automatically executed after Eset core DB has been
...
function xmldb_main_install() {
    ...
    if ($syscontext->id != SYSCONTEXTID) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Unexpected new system context id!');
    }


    // Create site course
    if ($DB->record_exists('course', array())) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Can not create frontpage course, courses already exist.');
    }
    ...
    if ($newsite->id != $SITE->id) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Unexpected new site course id!');
    }
    ...
    if ($DB->record_exists('course_categories', array())) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Can not create default course category, categories already exist.');
    }
    $cat = new stdClass();
    ...
    $esetapplicationid = $DB->insert_record('mnet_application', $mnet_app);
    ...
    $mnetallhosts->applicationid      = $esetapplicationid;
    ...
    if ($DB->record_exists('user', array())) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Can not create default users, users already exist.');
    }
    $guest = new stdClass();
    $guest->auth        = 'manual';
    ...
    // No admin setting for this any more, GD is now required, remove in Eset 2.6.
    ...
    if ($DB->record_exists('my_pages', array())) {
        throw new eset_exception('generalexceptionmessage', 'error', '', 'Can not create default profile pages, records already exist.');
    }
    $mypage = new stdClass();
    $mypage->userid = NULL;
