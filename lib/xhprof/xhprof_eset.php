...
function profiling_stop() {
    ...
    // config.php before Eset is installed. Rare but...
...
/**
 * Export profiling runs to a .mpr (eset profile runs) file.
 *
 ...
 *     - eset_profiling_runs.xml: Metadata about the information
 *         exported. Contains some header information (version and
 *         release of eset, database, git hash - if available, date
 ...
 * profiling table for each run (eset version, database, git hash...).
 *
...
/**
 * Import a .mpr (eset profile runs) file into eset.
 *
...
function profiling_import_runs($file, $commentprefix = '') {
    ...
    $fp = get_file_packer('application/vnd.eset.profiling');
    $status = $fp->extract_to_pathname($file, $tmpdir);

    // Look for master file and verify its format.
    if ($status) {
        $mfile = $tmpdir . '/eset_profiling_runs.xml';
...
function profiling_export_generate(array $runids, $tmpdir) {
    ...
    // Calculate the header information to be sent to eset_profiling_runs.xml.
    $release = $CFG->release;
    $version = $CFG->version;
    $dbtype = $CFG->dbtype;
    ...
    $mainxo = new file_xml_output($tmpdir . '/eset_profiling_runs.xml');
    $mainxw = new xml_writer($mainxo);

    // Output begins.
    $mainxw->start();
    $mainxw->begin_tag('eset_profiling_runs');
    ...
    foreach ($runids as $runid) {
        ...
        $runxw->start();
        $runxw->begin_tag('eset_profiling_run');
        ...
        $runxw->end_tag('eset_profiling_run');
        $runxw->stop();
    }
    $mainxw->end_tag('runs');
    $mainxw->end_tag('eset_profiling_runs');
    $mainxw->stop();
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="eset_profiling_runs">
    <xs:complexType>
      <xs:sequence>
        <xs:element ref="info"/>
        <xs:element ref="runs"/>
...
function profiling_get_import_run_schema() {
    $schema = <<<EOS
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="eset_profiling_run">
    <xs:complexType>
      <xs:sequence>
 ...
 */
class eset_xhprofrun implements iXHProfRuns {
...
class xhprof_table_sql extends table_sql {
    ...
    protected function col_url($row) {
        ...
        $scripturl = new eset_url('/admin/tool/profiling/index.php', array('script' => $row->url, 'listurl' => $row->url));
        ...
        if ($this->listurlmode) {
            ...
            $detailsurl = new eset_url('/admin/tool/profiling/index.php', array('listurl' => $row->url));
    ...
    protected function col_timecreated($row) {
        ...
        $url = new eset_url('/admin/tool/profiling/index.php', array('runid' => $row->runid, 'listurl' => $row->url));
