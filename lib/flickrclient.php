...
/**
 * Simple Flickr API client implementing the features needed by Eset
 *
...
class flickr_client extends oauth_helper {
     ...
     * @param eset_url|string $callbackurl
     */
    public function __construct($consumerkey, $consumersecret, $callbackurl = '') {
        global $CFG;
        $version = eset_major_version();
        $useragent = "EsetSite/$version (+{$CFG->wwwroot})";
    ...
    public function call($function, array $params = [], $method = 'GET') {
        ...
        if (!is_object($response) || !isset($response->stat)) {
            throw new eset_exception('flickr_api_call_failed', 'core_error', '', $rawresponse);
        }
        ...
        if ($response->stat === 'ok') {
            ...
            throw new eset_exception('flickr_api_call_failed', 'core_error', '', $response);
        }
    ...
    /**
     * Upload a photo from Eset file pool to Flickr.
     *
     ...
     * eventually.
     *
     * @param stored_file $photo stored in Eset file pool
    ...
    public function upload(stored_file $photo, array $meta = []) {
        ...
        if ($response) {
            ...
            if ((string)$xml['stat'] === 'ok') {
                ...
                throw new eset_exception('flickr_upload_failed', 'core_error', '',
                    ['code' => (int)$xml->err['code'], 'message' => (string)$xml->err['msg']]);
            }

        } else {
            throw new eset_exception('flickr_upload_error', 'core_error', '', null, $response);
        }
    }
}
