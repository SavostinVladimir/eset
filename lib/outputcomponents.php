 ...
 *
 * Please see http://docs.eset.org/en/Developement:How_Eset_outputs_HTML
 * for an overview.
 *
 * @package core
 * @category output
...
class file_picker implements renderable {
    ...
    public function __construct(stdClass $options) {
        ...
        if (!empty($options->itemid)) {
            ...
            if (!empty($file)) {
                $options->currentfile = html_writer::link(eset_url::make_draftfile_url($file->get_itemid(), $file->get_filepath(), $file->get_filename()), $file->get_filename());
            }
        }
...
class user_picture implements renderable {
     ...
     * @return eset_url
     */
    public function get_url(eset_page $page, renderer_base $renderer = null) {
        global $CFG;
        ...
        if ($this->user->picture > 0) {
            ...
            $url = eset_url::make_pluginfile_url(
        ...
        if ($this->user->picture == 0 and !empty($CFG->enablegravatar)) {
            ...
            // https gravatar page.
            if (is_https()) {
                return new eset_url("https://secure.gravatar.com/avatar/{$md5}", array('s' => $size, 'd' => $gravatardefault));
            } else {
                return new eset_url("http://www.gravatar.com/avatar/{$md5}", array('s' => $size, 'd' => $gravatardefault));
            }
        }
...
class help_icon implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data->url = (new eset_url('/help.php', $options))->out(false);
...
class pix_icon implements renderable, templatable {
     ...
     */
    public function __construct($pix, $alt, $component='eset', array $attributes = null) {
        global $PAGE;
...
class pix_emoticon extends pix_icon implements renderable {
     ...
     */
    public function __construct($pix, $alt, $component = 'eset', array $attributes = array()) {
...
class single_button implements renderable {
     ...
     */
    public function __construct(eset_url $url, $label, $method='post', $primary=false, $attributes = []) {
        $this->url    = clone($url);
        $this->label  = $label;
        $this->method = $method;
...
class single_select implements renderable, templatable {
    ...
    /**
     * @var eset_url Target url - includes hidden fields
     */
    var $url;
     ...
     * @param string $formid
     */
    public function __construct(eset_url $url, $name, array $options, $selected = '', $nothing = array('' => 'choosedots'), $formid = null) {
        $this->url      = $url;
        $this->name     = $name;
     ...
     */
    public function set_old_help_icon($helppage, $title, $component = 'eset') {
     ...
     * @param string $component
     */
    public function set_help_icon($identifier, $component = 'eset') {
...
class url_select implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $data->action = (new eset_url('/course/jumpto.php'))->out(false);
...
class action_link implements renderable {
     ...
     */
    public function __construct(eset_url $url,
                                $text,
...
class html_writer {
     ...
     *
     * @param string|eset_url $url The URL
     ...
     * @return string HTML fragment
     */
    public static function input_hidden_params(eset_url $url, array $exclude = null) {
     ...
     * @param eset_url|string $url optional url of the external script, $code ignored if specified
...
class html_table {
    ...
    /**
     * @deprecated since Eset 2.0. Styling should be in the CSS.
...
class paging_bar implements renderable, templatable {
    ...
    /**
     * @var string|eset_url If this  is a string then it is the url which will be appended with $pagevar,
     * an equals sign and the page number.
     * If this is a eset_url object then the pagevar param will be replaced by
     ...
     */
    public function prepare(renderer_base $output, eset_page $page, $target) {
        ...
        if ($this->totalcount > $this->perpage) {
            ...
            if ($this->page > 0) {
                $this->previouslink = html_writer::link(new eset_url($this->baseurl, array($this->pagevar=>$pagenum)), get_string('previous'), array('class'=>'previous'));
            }
            ...
            if ($this->page > round(($this->maxdisplay/3)*2)) {
                ...
                $this->firstlink = html_writer::link(new eset_url($this->baseurl, array($this->pagevar=>0)), '1', array('class'=>'first'));
            } else {
                $currpage = 0;
            }
            ...
            while ($displaycount < $this->maxdisplay and $currpage < $lastpage) {
                ...
                if ($this->page == $currpage) {
                    ...
                    $pagelink = html_writer::link(new eset_url($this->baseurl, array($this->pagevar=>$currpage)), $displaypage);
            ...
            if ($currpage < $lastpage) {
                ...
                $this->lastlink = html_writer::link(new eset_url($this->baseurl, array($this->pagevar=>$lastpageactual)), $lastpage, array('class'=>'last'));
            }

            $pagenum = $this->page + 1;

            if ($pagenum != $lastpage) {
                $this->nextlink = html_writer::link(new eset_url($this->baseurl, array($this->pagevar=>$pagenum)), get_string('next'), array('class'=>'next'));
            }
        }
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if ($this->page > 0) {
            $data->previous = [
                'page' => $this->page,
                'url' => (new eset_url($this->baseurl, [$this->pagevar => $this->page - 1]))->out(false)
            ];
        }
        ...
        if ($this->page > round(($this->maxdisplay / 3) * 2)) {
            ...
            $data->first = [
                'page' => 1,
                'url' => (new eset_url($this->baseurl, [$this->pagevar => 0]))->out(false)
            ];
        }
        ...
        while ($displaycount < $this->maxdisplay and $currpage < $lastpage) {
            ...
            $link = new eset_url($this->baseurl, [$this->pagevar => $currpage]);
        ...
        if ($currpage < $lastpage) {
            $data->last = [
                'page' => $lastpage,
                'url' => (new eset_url($this->baseurl, [$this->pagevar => $lastpage - 1]))->out(false)
            ];
        }
 ...
 *
 * contents should probably be generated using a eset_block_..._renderer.
 *
...
class custom_menu_item implements renderable, templatable {
    ...
    /**
     * @var eset_url The link to give the icon if it has no children
     */
    protected $url;
     ...
     *
     * @param string $text
     * @param eset_url $url A eset url to apply as the link for this item [Optional]
     ...
     */
    public function __construct($text, eset_url $url=null, $title=null, $sort = null, custom_menu_item $parent = null) {
        $this->text = $text;
        $this->url = $url;
        $this->title = $title;
        $this->sort = (int)$sort;
     ...
     */
    public function add($text, eset_url $url = null, $title = null, $sort = null) {
     ...
     * @param eset_url $url
     */
    public function set_url(eset_url $url) {
        $this->url = $url;
    }
...
class custom_menu extends custom_menu_item {
     ...
     *
     * Example structure:
     *     First level first item|http://www.eset.com/
     *     -Second level first item|http://www.eset.com/partners/
     *     -Second level second item|http://www.eset.com/hq/
     *     --Third level first item|http://www.eset.com/jobs/
     *     -Second level third item|http://www.eset.com/development/
     *     First level second item|http://www.eset.com/feedback/
     *     First level third item
     *     English only|http://eset.com|English only item|en
     *     German only|http://eset.de|Deutsch|de,de_du,de_kids
     *
     *
     * @static
            ...
            foreach ($settings as $i => $setting) {
                ...
                if (!empty($setting)) {
                    switch ($i) {
                        ...
                        case 1: // URL.
                            try {
                                $itemurl = new eset_url($setting);
                            } catch (eset_exception $exception) {
...
class context_header implements renderable {
     ...
     *                       url => Link for the button to head to. Should be a eset_url.
    ...
    protected function format_button_images() {
        ...
        foreach ($this->additionalbuttons as $buttontype => $button) {
            ...
            if (!isset($button['image'])) {
                ...
                // Check to see if this is an internal Eset icon.
                $internalimage = $page->theme->resolve_image_location('t/' . $button['image'], 'eset');
                if ($internalimage) {
...
class action_menu implements renderable, templatable {
    ...
    public function __construct(array $actions = array()) {
        ...
        $this->attributes = array(
            ...
            'class' => 'eset-actionmenu',
            'data-enhance' => 'eset-core-actionmenu'
        );
     ...
     * @param eset_page $page
     */
    public function initialise_js(eset_page $page) {
        static $initialised = false;
        if (!$initialised) {
            $page->requires->yui_module('eset-core-actionmenu', 'M.core.actionmenu.init');
            $initialised = true;
        }
    }
    ...
    public function get_primary_actions(core_renderer $output = null) {
        ...
        if (!empty($this->menutrigger)) {
            ...
            $title = new lang_string('actionsmenu', 'eset');
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!empty($this->menutrigger)) {
            ...
            $actionicon = new pix_icon('t/edit_menu', '', 'eset', $iconattributes);
        }
...
class action_menu_link extends action_link implements renderable {
     ...
     * Constructs the object.
     *
     * @param eset_url $url The URL for the action.
     ...
     */
    public function __construct(eset_url $url, pix_icon $icon = null, $text, $primary = true, array $attributes = array()) {
...
class action_menu_link_primary extends action_menu_link {
     ...
     * @param array $attributes
     */
    public function __construct(eset_url $url, pix_icon $icon = null, $text, array $attributes = array()) {
...
class progress_bar implements renderable, templatable {
    ...
    private function _update($percent, $msg) {
        ...
        if (is_numeric($estimate)) {
            $estimatemsg = get_string('secondsleft', 'eset', round($estimate, 2));
        }
