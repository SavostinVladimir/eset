 ...
 * tests of required versions and libraries to run Eset. It can be
 ...
 */
function check_eset_environment($version, $env_select = ENV_SELECT_NEWER) {
...
/**
 * This function will return the list of Eset versions available
 *
...
function get_list_of_environment_versions($contents) {
    ...
    if (isset($contents['COMPATIBILITY_MATRIX']['#']['ESET'])) {
        foreach ($contents['COMPATIBILITY_MATRIX']['#']['ESET'] as $version) {
...
/**
 * This function will return the xmlized data belonging to one Eset version
 *
 * @param string $version top version from which we start to look backwards
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
 * @return mixed the xmlized structure or false on error
...
/// Detect available versions
    ...
    // be matched against any version of Eset.
...
/// We now we have it. Extract from full contents.
    ...
    return $contents['COMPATIBILITY_MATRIX']['#']['ESET'][$fl_arr[$version]];
}
...
/// Only run the eset versions checker on upgrade, not on install
    if (!empty($CFG->version)) {
        $results[] = environment_check_eset($version, $env_select);
    }
...
/// Get current Eset version (release) for later compare
...
/**
 * This function will check if Eset requirements are satisfied
 *
 * @uses NO_VERSION_DATA_FOUND
 * @param string $version xml version we are going to use to test this server
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
 ...
 */
function environment_check_eset($version, $env_select) {

    $result = new environment_results('eset');
...
/// Extract the eset part
    if (!isset($data['@']['requires'])) {
        $needed_version = '1.0'; /// Default to 1.0 if no eset requires is found
    } else {
    /// Extract required eset version
...
/**
 * Looks for buggy PCRE implementation, we need unicode support in Eset...
 * @param string $version xml version we are going to use to test this server
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
