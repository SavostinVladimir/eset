 ...
 *
 * Customised version of SimplePie for Eset
 */
...
/**
 * Eset Customised version of the SimplePie class
 *
 * This class extends the stock SimplePie class
 * in order to make sensible configuration choices,
 * such as using the Eset cache directory and
 * curl functions/proxy config for making http
 * requests in line with eset configuration.
 *
 * @copyright 2009 Dan Poltawski <talktodan@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Eset 2.0
 */
class eset_simplepie extends SimplePie {
    /**
    ...
    public function __construct($feedurl = null, $timeout = 2) {
        $cachedir = eset_simplepie::get_cache_directory();
        check_dir_exists($cachedir);

        parent::__construct();

        // Use the Eset class for http requests
        $this->set_file_class('eset_simplepie_file');

        // Use html purifier for text cleaning.
        $this->set_sanitize_class('eset_simplepie_sanitize');
        $this->sanitize = new eset_simplepie_sanitize();
...
/**
 * Eset Customised version of the SimplePie_File class
 *
 * This class extends the stock SimplePie class
 * in order to utilise Esets own curl class for making
 * http requests. By using the eset curl class
 * we ensure that the correct proxy configuration is used.
 */
class eset_simplepie_file extends SimplePie_File {

    /**
     * The constructor is a copy of the stock simplepie File class which has
     * been modified to add in use the Eset curl class rather than php curl
     * functions.
     */
 ...
 */
class eset_simplepie_sanitize extends SimplePie_Sanitize {
