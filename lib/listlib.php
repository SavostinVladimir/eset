...
//------------------------------------------------------
     ...
     * @param eset_url $pageurl url for this page
    ...
    public function __construct($type='ul', $attributes='', $editable = false, $pageurl=null, $page = 0, $pageparamname = 'page', $itemsperpage = 20) {
        ...
        if ($pageurl === null) {
            $this->pageurl = new eset_url($PAGE->url);
...
abstract class list_item {
    ...
    /** @var eset_list Set if there are any children of this listitem. */
    public $children;
    ...
    public function set_icon_html($first, $last, $lastitem) {
        ...
        if (isset($this->parentlist->parentitem)) {
            ...
            $url = new eset_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'left'=>$this->id)));
        ...
        if (!$first) {
            $url = new eset_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'moveup'=>$this->id)));
        ...
        if (!$last) {
            $url = new eset_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'movedown'=>$this->id)));
        ...
        if (!empty($lastitem)) {
            ...
            $url = new eset_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'right'=>$this->id)));
    ...
    /**
     * Recurse down tree creating list_items, called from eset_list::list_from_records
     *
     * @param array $records
