 ...
 * @since     Eset 2.0
 */
class block_not_on_page_exception extends eset_exception {
    /**
     * Constructor
...
/**
 * This class keeps track of the block that should appear on a eset_page.
 *
...
/// Field declarations =========================================================
    ...
    /**
     * the eset_page we are managing blocks for.
     * @var eset_page
     */
    protected $page;
...
/// Constructor ================================================================
    ...
    /**
     * Constructor.
     * @param object $page the eset_page object object we are managing the blocks for,
...
/// Getter methods =============================================================
    ...
    protected function get_move_target_url($region, $weight) {
        return new eset_url($this->page->url, array('bui_moveid' => $this->movingblock,
...
/// Inner workings =============================================================
    ...
    public function ensure_content_created($region, $output) {
        ...
        if (!has_capability('eset/block:view', $this->page->context) ) {
...
/// Process actions from the URL ===============================================
    ...
    public function edit_controls($block) {
        ...
        if ($this->page->user_can_edit_blocks()) {
            ...
            $controls[] = new action_menu_link_primary(
                new eset_url($actionurl, array('bui_moveid' => $block->instance->id)),
                new pix_icon('t/move', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
                $str,
        ...
        if ($this->page->user_can_edit_blocks() || $block->user_can_edit()) {
            ...
            $controls[] = new action_menu_link_secondary(
                new eset_url($actionurl, array('bui_editid' => $block->instance->id)),
                new pix_icon('t/edit', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
                $str,
        ...
        if ($this->page->user_can_edit_blocks() && $block->instance_can_be_hidden()) {
            ...
            if ($block->instance->visible) {
                ...
                $url = new eset_url($actionurl, array('bui_hideid' => $block->instance->id));
                $icon = new pix_icon('t/hide', $str, 'eset', array('class' => 'iconsmall', 'title' => ''));
                $attributes = array('class' => 'editing_hide');
            } else {
                $str = new lang_string('showblock', 'block', $blocktitle);
                $url = new eset_url($actionurl, array('bui_showid' => $block->instance->id));
                $icon = new pix_icon('t/show', $str, 'eset', array('class' => 'iconsmall', 'title' => ''));
        ...
        if (get_assignable_roles($block->context, ROLENAME_SHORT)) {
            $rolesurl = new eset_url('/admin/roles/assign.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = new lang_string('assignrolesinblock', 'block', $blocktitle);
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/assignroles', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
                $str, array('class' => 'editing_assignroles')
            );
        }

        // Permissions.
        if (has_capability('eset/role:review', $block->context) or get_overridable_roles($block->context)) {
            $rolesurl = new eset_url('/admin/roles/permissions.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = get_string('permissions', 'role');
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/permissions', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
                $str, array('class' => 'editing_permissions')
            );
        }

        // Change permissions.
        if (has_any_capability(array('eset/role:safeoverride', 'eset/role:override', 'eset/role:assign'), $block->context)) {
            $rolesurl = new eset_url('/admin/roles/check.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = get_string('checkpermissions', 'role');
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/checkpermissions', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
        ...
        if ($this->user_can_delete_block($block)) {
            ...
            $controls[] = new action_menu_link_secondary(
                new eset_url($actionurl, array('bui_deleteid' => $block->instance->id)),
                new pix_icon('t/delete', $str, 'eset', array('class' => 'iconsmall', 'title' => '')),
                $str,
                array('class' => 'editing_delete')
            );
        }

        if (!empty($CFG->contextlocking) && has_capability('eset/site:managecontextlocks', $block->context)) {
            ...
            if (empty($parentcontext) || empty($parentcontext->locked)) {
                ...
                $controls[] = new action_menu_link_secondary(
                    ...
                    new pix_icon($lockicon, $lockstring, 'eset', array('class' => 'iconsmall', 'title' => '')),
                    $lockstring,
                    ['class' => 'editing_lock']
                );
            }
    ...
    public function process_url_add() {
        ...
        if (!$this->page->user_can_edit_blocks()) {
            throw new eset_exception('nopermissions', '', $this->page->url->out(), get_string('addblock'));
        }
        ...
        if ($blocktype === '') {
            ...
            if (!$addableblocks) {
                ...
                $url = new eset_url($addpage->url, array('sesskey' => sesskey()));
        ...
        if (!array_key_exists($blocktype, $addableblocks)) {
            throw new eset_exception('cannotaddthisblocktype', '', $this->page->url->out(), $blocktype);
        }
    ...
    public function process_url_delete() {
        ...
        if (!$this->user_can_delete_block($block)) {
            throw new eset_exception('nopermissions', '', $this->page->url->out(), get_string('deleteablock'));
        }

        if (!$confirmdelete) {
            $deletepage = new eset_page();
            ...
            $PAGE = $deletepage;
            //some functions like EsetQuickForm::addHelpButton use $OUTPUT so we need to replace that too
            ...
            $PAGE->set_heading($site->fullname);
            echo $OUTPUT->header();
            $confirmurl = new eset_url($deletepage->url, array('sesskey' => sesskey(), 'bui_deleteid' => $block->instance->id, 'bui_confirm' => 1));
            $cancelurl = new eset_url($deletepage->url);
    ...
    public function process_url_show_hide() {
        ...
        if (!$this->page->user_can_edit_blocks()) {
            throw new eset_exception('nopermissions', '', $this->page->url->out(), get_string('hideshowblocks'));
    ...
    public function process_url_edit() {
        ...
        if (!$block->user_can_edit() && !$this->page->user_can_edit_blocks()) {
            throw new eset_exception('nopermissions', '', $this->page->url->out(), get_string('editblock'));
        }

        $editpage = new eset_page();
        ...
        // overwrite global $PAGE ready for this. (Formslib refers to it.)
        $PAGE = $editpage;
        //some functions like EsetQuickForm::addHelpButton use $OUTPUT so we need to replace that to
        ...
        if ($mform->is_cancelled()) {
            ...
            if (has_capability('eset/site:manageblocks', $parentcontext)) { // Check permissions in destination
            ...
            $strheading = get_string('blockconfiga', 'eset', $block->get_title());
            ...
            if ($bits[0] == 'tag' && !empty($this->page->subpage)) {
                ...
                $editpage->navbar->add(get_string('tags'), new eset_url('/tag/'));
    ...
    public function process_url_move() {
        ...
        if (!$this->is_known_region($newregion)) {
            throw new eset_exception('unknownblockregion', '', $this->page->url, $newregion);
        }
 ...
 * @param eset_page $page the page this block is appearing on.
 * @return block_base the requested block instance.
 */
...
/**
 * Generates the page type list for the my eset page
 *
 * @param string $pagetype
 * @param stdClass $parentcontext
 * @param stdClass $currentcontext
...
function block_add_block_ui($page, $output) {
    ...
    $actionurl = new eset_url($page->url, array('sesskey'=>sesskey()));
 ...
 * @param eset_page $page the back to set the visibility with respect to.
