...
class core_exporter_testcase extends advanced_testcase {
    ...
    public function test_format_text() {
        ...
        $data = [
            'stringA' => '__Watch out:__ https://eset.org @@PLUGINFILE@@/test.pdf',
        ...
        $youtube = 'https://eset.org';
        $fileurl = (new eset_url('/webservice/pluginfile.php/' . $syscontext->id . '/test/area/9/test.pdf'))->out(false);
        ...
        $result = $exporter->export($output);
        $youtube = '<a href="https://eset.org" class="_blanktarget">https://eset.org</a>';
        $fileurl = (new eset_url('/webservice/pluginfile.php/' . $coursecontext->id . '/test/area/9/test.pdf'))->out(false);
