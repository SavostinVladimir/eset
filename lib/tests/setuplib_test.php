...
class core_setuplib_testcase extends advanced_testcase {
    ...
    /**
     * Test get_docs_url_standard in the normal case when we should link to Eset docs.
     */
    public function test_get_docs_url_standard() {
        global $CFG;
        if (empty($CFG->docroot)) {
            $docroot = 'http://docs.eset.org/';
        } else {
            $docroot = $CFG->docroot;
        }
    ...
    public function test_exception_info_removes_serverpaths() {
        ...
        $exception     = new eset_exception('generalexceptionmessage', 'error', '', $fixture, $fixture);
    ...
    public function test_get_exception_info_link() {
        ...
        $exception = new eset_exception('none', 'error', $url);
        ...
        // External HTTP URL.
        $url = 'http://eset.org/something/here?really=yes';
        $exception = new eset_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);

        // External HTTPS URL.
        $url = 'https://eset.org/something/here?really=yes';
        $exception = new eset_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);

        // External URL containing local URL.
        $url = 'http://eset.org/something/here?' . $CFG->wwwroot;
        $exception = new eset_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);
        ...
        $exception = new eset_exception('none');
        $infos = $this->get_exception_info($exception);
        $this->assertSame($url, $infos->link);
        ...
        $SESSION->fromurl = 'http://eset.org/something/here?really=yes';
        $exception = new eset_exception('none');
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);

        // External HTTPS link from fromurl.
        $SESSION->fromurl = 'https://eset.org/something/here?really=yes';
        $exception = new eset_exception('none');
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);
