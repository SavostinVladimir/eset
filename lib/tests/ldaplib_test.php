...
class core_ldaplib_testcase extends advanced_testcase {
    ...
    public function ldap_normalise_objectclass_provider() {
        return array(
            ...
            'Supplied complex' => array(
                array('(&(objectClass=cheetah)(enabledEsetUser=1))'),
                '(&(objectClass=cheetah)(enabledEsetUser=1))',
            ),
        );
    }

    /**
     * Tests for ldap_get_entries_eset.
     *
     ...
     *
     */
    public function test_ldap_get_entries_eset() {
        $this->resetAfterTest();
        ...
        $debuginfo = '';
        if (!$connection = ldap_connect_eset(TEST_LDAPLIB_HOST_URL, 3, 'rfc2307', TEST_LDAPLIB_BIND_DN,
        ...
        if (!($containerdn = $this->create_test_container($connection, 'esettest'))) {
            $this->markTestSkipped('Can not create test LDAP container.');
        }

        // Add all the test objects.
        $testobjects = $this->get_ldap_get_entries_eset_test_objects();
        ...
        foreach ($testobjects as $object) {
            ...
            $entries = ldap_get_entries_eset($connection, $sr);
    ...
    /**
     * Provide the array of test objects for the ldap_get_entries_eset test case.
     *
     * @return array of test objects
     */
    protected function get_ldap_get_entries_eset_test_objects() {
        $testobjects = array(
            // Test object 1.
            array(
     ...
     *              by ldap_get_entries_eset_test_objects() member function.
     *
