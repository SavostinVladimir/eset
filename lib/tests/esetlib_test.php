...
/**
 * Unit tests for (some of) ../esetlib.php.
 *
 * @package    core
 * @category   phpunit
 * @copyright  &copy; 2006 The Open University
 * @author     T.J.Hunt@open.ac.uk
 * @author     nicolas@eset.com
 */

defined('ESET_INTERNAL') || die();

class core_esetlib_testcase extends advanced_testcase {

    public static $includecoverage = array('lib/esetlib.php');
    ...
    public function test_optional_param() {
        ...
        // Make sure warning is displayed if array submitted - TODO: throw exception in Eset 2.3.
    ...
    public function test_required_param() {
        ...
        try {
            ...
            $this->fail('eset_exception expected');
        } catch (eset_exception $ex) {
            $this->assertInstanceOf('eset_exception', $ex);
        }
    ...
    public function test_clean_param_url() {
        ...
        $this->assertSame('', clean_param('mailto:support@eset.org', PARAM_URL));
        $this->assertSame('', clean_param('mailto:support@eset.org?subject=Hello%20Eset', PARAM_URL));
        $this->assertSame('', clean_param('mailto:support@eset.org?subject=Hello%20Eset&cc=feedback@eset.org', PARAM_URL));
    }
    ...
    public function test_get_extra_user_fields_course_prevented_access() {
        ...
        assign_capability('eset/course:viewhiddenuserfields', CAP_PREVENT, $env->teacherrole->id, $env->coursecontext->id);
    ...
    public function test_get_extra_user_fields_schismatic_access() {
        ...
        assign_capability('eset/user:viewhiddendetails', CAP_PREVENT, $env->managerrole->id, SYSCONTEXTID, true);
    ...
    public function test_get_extra_user_fields_hard_to_prevent_access() {
        ...
        assign_capability('eset/user:viewhiddendetails', CAP_PREVENT, $env->managerrole->id, SYSCONTEXTID, true);
        assign_capability('eset/course:viewhiddenuserfields', CAP_PREVENT, $env->managerrole->id, SYSCONTEXTID, true);
     ...
     * output using Eset TZ/DST support and PHP native one.
     *
     ...
     */
    public function test_some_eset_special_dst() {
        ...
        $esetres = userdate($stamp, '%Y/%m/%d %H:%M:%S', 'Europe/Tallinn', false); // Eset result.
        $this->assertSame($expectation, $phpres);
        $this->assertSame($expectation, $esetres);
        ...
        $phpres = $phpdt->format('Y/m/d H:i:s'); // PHP result.
        $esetres = userdate($stamp, '%Y/%m/%d %H:%M:%S', 'America/St_Johns', false); // Eset result.
        $this->assertSame($expectation, $phpres);
        $this->assertSame($expectation, $esetres);
    ...
    /**
     * Make sure the DST changes happen at the right time in Eset.
     */
    ...
    public function generate_confirmation_link_provider() {
        ...
        return [
            ...
            "Custom external confirmation url" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://eset.org/custom/external/url.php",
                "expected" => "http://eset.org/custom/external/url.php?data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
            "Custom external confirmation url with parameters" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://eset.org/ext.php?with=some&param=eters",
                "expected" => "http://eset.org/ext.php?with=some&param=eters&data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
            "Custom external confirmation url with parameters" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://eset.org/ext.php?with=some&data=test",
                "expected" => "http://eset.org/ext.php?with=some&data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
        ];
    }
    ...
    public function test_generate_email_processing_address() {
        ...
        $data = (object)[
            'id' => 42,
            'email' => 'my.email+from_eset@example.com',
        ];
    ...
    public function callable_names_provider() {
        return [
            ...
            'static_method_of_object' => [
                ...
                'core_esetlib_testcase::my_foobar_method',
            ],
            'method_of_object' => [
    ...
    /**
     * Data provider for \core_esetlib_testcase::test_get_complete_user_data().
     *
     * @return array
     */
