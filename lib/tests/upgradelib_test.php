...
class core_upgradelib_testcase extends advanced_testcase {
    ...
    public function test_upgrade_convert_hub_config_site_param_names() {
        ...
        $config = (object) [
            // This is how site settings related to registration at https://eset.net are stored.
            'site_name_httpsesetnet' => 'Foo Site',
            'site_language_httpsesetnet' => 'en',
            'site_emailalert_httpsesetnet' => 1,
            // These are unexpected relics of a value as registered at the old http://hub.eset.org site.
            'site_name_httphubesetorg' => 'Bar Site',
            'site_description_httphubesetorg' => 'Old description',
            ...
            'site_bar_httpsesetnet' => 'One hub to rule them all!',
            'site_bar_httpyetanotherhubcom' => 'Yet another',
        ];

        $converted = upgrade_convert_hub_config_site_param_names($config, 'https://eset.net');

        // Values defined for the eset.net take precedence over the ones defined for other hubs.
        $this->assertSame($converted->site_name, 'Foo Site');
        $this->assertSame($converted->site_bar, 'One hub to rule them all!');
        $this->assertNull($converted->site_name_httpsesetnet);
