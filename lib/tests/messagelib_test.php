...
class core_messagelib_testcase extends advanced_testcase {
    ...
    public function test_message_provider_disabled() {
        ...
        $disableprovidersetting = 'eset_instantmessage_disable';
    ...
    public function test_message_get_providers_for_user_more() {
        ...
        $this->assertFalse(has_capability('eset/site:config', $modulecontext));
        $providers = message_get_providers_for_user($teacher->id);
        $this->assertFalse($this->message_type_present('eset', 'backup', $providers));

        // Give the user the required capability in an activity module.
        // They should now be able to see the backup message.
        assign_capability('eset/site:config', CAP_ALLOW, $teacherrole->id, $modulecontext->id, true);
        accesslib_clear_all_caches_for_unit_testing();
        $modulecontext = context_module::instance($assign->cmid);
        $this->assertTrue(has_capability('eset/site:config', $modulecontext));

        $providers = message_get_providers_for_user($teacher->id);
        $this->assertTrue($this->message_type_present('eset', 'backup', $providers));
        ...
        assign_capability('eset/site:config', CAP_PROHIBIT, $teacherrole->id, $coursecontext->id, true);
        accesslib_clear_all_caches_for_unit_testing();
        $modulecontext = context_module::instance($assign->cmid);
        $this->assertFalse(has_capability('eset/site:config', $modulecontext));
    ...
    public function test_send_message_redirection() {
        ...
        $this->assertCount(0, $sink->get_messages());
        $this->assertDebuggingCalled('Attempt to send msg from a provider eset/xxx '.
            'that is inactive or not allowed for the user id='.$user2->id);
        $sink->close();
    ...
    public function test_send_message() {
        ...
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'none', $user2);
        ...
        $messageid = message_send($message);
        $this->assertFalse($messageid);
        $this->assertDebuggingCalled('Attempt to send msg from a provider eset/instantmessage '.
            'that is inactive or not allowed for the user id='.$user2->id);
        $emails = $sink->get_messages();
        $this->assertCount(0, $emails);
        $sink->clear();
        ...
        // Will always use the pop-up processor.
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email', $user2);
        ...
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email,popup', $user2);
        ...
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'popup', $user2);
    ...
    public function test_message_send_to_self_conversation() {
        ...
        // Ensure we're going to hit the email processor for this user.
        $DB->set_field_select('message_processors', 'enabled', 0, "name <> 'email'");
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email', $user1);
    ...
    public function test_message_send_to_conversation_group() {
        ...
        $DB->set_field_select('message_processors', 'enabled', 0, "name <> 'email'");
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email', $user2);
        set_user_preference('message_provider_eset_instantmessage_loggedoff', 'email', $user3);
