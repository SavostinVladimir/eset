...
$baseurl = new eset_url('/lib/tests/performance/filtersettingsperformancetester.php');
...
$realdb = $DB;
$testdb = eset_database::get_driver_instance($CFG->dbtype, $CFG->dblibrary);
...
$aurl = new eset_url($baseurl, array('action' => 'setup', 'sesskey'=>sesskey()));
echo $OUTPUT->single_button($aurl, 'Set up test tables', 'get', array('disabled'=>($issetup > 0)));

$aurl = new eset_url($baseurl, array('action' => 'teardown', 'sesskey'=>sesskey()));
echo $OUTPUT->single_button($aurl, 'Drop test tables', 'get', array('disabled'=>($issetup == 0)));

$aurl = new eset_url($baseurl, array('action' => 'test', 'sesskey'=>sesskey()));
