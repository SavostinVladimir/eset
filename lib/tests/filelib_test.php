...
class core_filelib_testcase extends advanced_testcase {
    ...
    public function test_curl_post() {
        ...
        // Test post request.
        $curl = new curl();
        $contents = $curl->post($testurl, 'data=esettest');
        $response = $curl->getResponse();
        $this->assertSame('200 OK', reset($response));
        $this->assertSame(0, $curl->get_errno());
    ...
    public function test_delete_original_file_from_draft() {
        ...
        // The file reference should be a regular eset file now.
...
Set-Cookie: BBC-UID=c4de2e109c8df6a51de627cee11b214bd4fb6054a030222488317afb31b343360EsetBot/1.0; expires=Mon, 07-Dec-15 14:44:53 GMT; path=/; domain=bbc.co.uk
X-Cache-Action: MISS
X-Cache-Age: 0
...
EOF;
    ...
    public function test_curl_useragent() {
        ...
        $esetbot = \core_useragent::get_esetbot_useragent();

        $curl->call_apply_opt($options);
        $this->assertTrue(in_array("User-Agent: $esetbot", $curl->header));
        ...
        $this->assertFalse(in_array("User-Agent: $esetbot", $curl->header));
    ...
    public function test_file_rewrite_pluginfile_urls_includetoken() {
        ...
        $expectedurl = new \eset_url("/tokenpluginfile.php/{$token}/{$syscontext->id}/user/private/0/image.png");
    ...
    public function test_file_rewrite_pluginfile_urls_includetoken_no_slashargs() {
        ...
        $token = get_user_key('core_files', $USER->id);
        $expectedurl = new \eset_url("/tokenpluginfile.php");
