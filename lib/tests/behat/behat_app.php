...
class behat_app extends behat_base {
     ...
     *
     * This updates Eset configuration and starts Ionic running, if it isn't already.
     */
    ...
    /**
     * Opens the Eset app in the browser.
     *
     * Requires JavaScript.
     *
     * @Given /^I enter the app$/
     * @throws DriverException Issue with configuration or feature file
     * @throws dml_exception Problem with Eset setup
    ...
    /**
     * Fixes the Eset admin settings to allow mobile app use (if not already correct).
     *
     * @throws dml_exception If there is any problem changing Eset settings
     */
    protected function fix_eset_setup() {
        global $CFG, $DB;

        // Configure Eset settings to enable app web services.
        ...
        role_change_permission($userroleid, $systemcontext, 'eset/webservice:createtoken', CAP_ALLOW);
        ...
        $service = $webservicemanager->get_external_service_by_shortname(
                ESET_OFFICIAL_MOBILE_SERVICE, MUST_EXIST);
        if (!$service->enabled) {
            $service->enabled = 1;
    ...
    protected function prepare_browser(string $url) {
        ...
        $this->spin(
                function($context, $args) {
                    ...
                    if ($title) {
                        $text = $title->getHtml();
                        if ($text === 'Eset Desktop') {
                            return true;
                        }
                    }
                    throw new DriverException('Eset app not found in browser');
                }, false, 60);

        // Run the scripts to install Eset 'pending' checks.
        ...
        $situation = $this->spin(
                function($context, $args) {
                    ...
                    throw new DriverException('Eset app login URL prompt not found');
    ...
    public function login(string $username) {
        ...
        $this->spin(
                function($context, $args) {
                    ...
                    throw new DriverException('Eset app main page not loaded after login');
                }, false, 30);
