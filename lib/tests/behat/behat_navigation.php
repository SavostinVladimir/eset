...
class behat_navigation extends behat_base {
    ...
    public function i_expand_node($nodetext) {
        ...
        // This step is useless with Javascript disabled as Eset auto expands
    ...
    public function i_open_my_profile_in_edit_mode() {
        ...
        if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if (is_siteadmin($user) ||  has_capability('eset/user:update', $systemcontext)) {
                $url = new eset_url('/user/editadvanced.php', array('id' => $user->id, 'course' => SITEID,
                    'returnto' => 'profile'));
            } else if (has_capability('eset/user:editownprofile', $systemcontext)) {
                ...
                if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                    ...
                    if (empty($url)) {
                        if (empty($course)) {
                            ...
                            $url = new eset_url('/user/edit.php', array('id' => $user->id, 'course' => $courseid,
                                'returnto' => 'profile'));
                        }
                    }
     ...
     *
     * @param string $page the type of page. E.g. 'Admin notifications' or 'core_user > Preferences'.
     * @return eset_url the corresponding URL.
     */
    protected function resolve_page_helper(string $page): eset_url {
     ...
     * @return eset_url the corresponding URL.
     */
    protected function resolve_page_instance_helper(string $identifier, string $pagetype): eset_url {
     ...
     */
    protected function resolve_core_page_url(string $name): eset_url {
        switch ($name) {
            case 'Homepage':
                return new eset_url('/');

            case 'Admin notifications':
                return new eset_url('/admin/');
     ...
     * @throws Exception with a meaningful error message if the specified page cannot be found.
     */
    protected function resolve_core_page_instance_url(string $type, string $identifier): eset_url {
        global $DB;
        ...
        switch ($type) {
            case 'Category page':
                ...
                return new eset_url('/course/category.php', ['id' => $categoryid]);
            ...
            case 'Course':
                ...
                return new eset_url('/course/view.php', ['id' => $courseid]);
    ...
    public function i_am_on_course_homepage($coursefullname) {
        ...
        $url = new eset_url('/course/view.php', ['id' => $course->id]);
        $this->execute('behat_general::i_visit', [$url]);
    }
    ...
    protected function find_header_administration_menu($mustexist = false) {
        $menuxpath = '//header[@id=\'page-header\']//div[contains(@class,\'eset-actionmenu\')]';
        if ($mustexist) {
