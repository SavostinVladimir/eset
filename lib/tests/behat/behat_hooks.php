...
class behat_hooks extends behat_base {
    ...
    /**
     * Gives access to eset codebase, ensures all is ready and sets up the test lock.
     *
     * Includes config.php to use eset codebase with $CFG->behat_* instead of $CFG->prefix and $CFG->dataroot, called
     * once per suite.
     *
     * @BeforeSuite
    ...
    public static function before_suite_hook(BeforeSuiteScope $scope) {
        ...
        // Defined only when the behat CLI command is running, the eset init setup process will
        ...
        if (!behat_util::is_test_data_updated()) {
            ...
            $message = <<<EOF
Your behat test site is outdated, please run the following command from your Eset dirroot to drop, and reinstall the Behat test site.
...
EOF;
    ...
    /**
     * Gives access to eset codebase, to keep track of feature start time.
     *
    ...
    /**
     * Gives access to eset codebase, to keep track of feature end time.
     *
    ...
    /**
     * Gives access to eset codebase, to keep track of suite timings.
     *
...
EOF;
    ...
    public function before_scenario_hook(BeforeScenarioScope $scope) {
        ...
        // If scenario requires the Eset app to be running, set this up.
        if ($this->has_tag('app')) {
    ...
    public function before_step(BeforeStepScope $scope) {
        ...
        if (!$this->scenariorunning) {
            ...
            // Checking that the root path is a Eset test site.
            if (self::is_first_scenario()) {
     ...
     * @see Eset\BehatExtension\EventDispatcher\Tester\ChainedStepTester
     */
