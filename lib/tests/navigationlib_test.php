...
class core_navigationlib_testcase extends advanced_testcase {
    ...
    protected function setup_node() {
        ...
        $activeurl = $PAGE->url;
        $inactiveurl = new eset_url('http://www.eset.com/');
    ...
    public function test_node__construct() {
        ...
        $fakeproperties = array(
            ...
            'action' => new eset_url('http://www.eset.org/'));
    ...
    public function test_node_add() {
        ...
        $node1 = $this->node->add('test_add_1', 'http://www.eset.org/', navigation_node::TYPE_COURSE, 'testadd1', 'key', new pix_icon('i/course', ''));
        // Add a node with the minimum args required.
        $node2 = $this->node->add('test_add_2', null, navigation_node::TYPE_CUSTOM, 'testadd2');
        $node3 = $this->node->add(str_repeat('eset ', 15), str_repeat('eset', 15));
    ...
    public function test_node_check_if_active() {
        ...
        // Demo1 -> action is http://www.eset.org/, thus should be true.
        ...
        // Demo2 -> action is http://www.eset.com/, thus should be false.
    ...
    public function test_node_make_active() {
        ...
        $node2 = $this->node->add('active node 2', new eset_url($CFG->wwwroot), navigation_node::TYPE_COURSE, null, 'anode2');
        $node1->make_active();
    ...
    public function test_navbar_prepend_and_add() {
        ...
        $page->set_url(new eset_url('/course/view.php', array('id' => $course->id)));
     ...
     * @param $node
     */
    public function test_navbar_has_items(eset_page $page) {
        $this->resetAfterTest();
    ...
    public function test_cache_set() {
        ...
        $cache->set('software', 'Eset');
        $this->assertTrue($cache->cached('software'));
        $this->assertEquals($cache->software, 'Eset');
    }
...
class exposed_global_navigation extends global_navigation {
    ...
    public function __construct(eset_page $page=null) {
        global $PAGE;
        if ($page === null) {
            $page = $PAGE;
        }
...
class navigation_exposed_eset_page extends eset_page {
