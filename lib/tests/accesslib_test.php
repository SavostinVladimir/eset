...
class core_accesslib_testcase extends advanced_testcase {
    ...
    public function test_capabilities_mutation() {
        $oldcap = get_capability_info('eset/site:config');
        $cap = get_capability_info('eset/site:config');
        unset($cap->name);
        $newcap = get_capability_info('eset/site:config');
    ...
    public function test_is_safe_capability() {
        ...
        $capability = $DB->get_record('capabilities', array('name'=>'eset/site:config'), '*', MUST_EXIST);
    ...
    public function test_assign_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupcourse'))); // Any capability assigned to student by default.
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse')));
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse')));

        $this->setUser($user);
        $result = assign_capability('eset/backup:backupcourse', CAP_ALLOW, $student->id, $frontcontext->id);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse'));
        ...
        $this->setUser(0);
        $result = assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $student->id, $frontcontext->id, false);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse'));
        $this->assertNotEmpty($permission);
        $this->assertEquals(CAP_ALLOW, $permission->permission);
        ...
        $result = assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $student->id, $frontcontext->id, true);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse'));
        $this->assertNotEmpty($permission);
        ...
        $result = assign_capability('eset/backup:backupcourse', CAP_INHERIT, $student->id, $frontcontext->id);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'eset/backup:backupcourse'));
        ...
        $capability = 'eset/backup:backupcourse';
    ...
    public function test_unassign_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupcourse'))); // Any capability assigned to manager by default.
        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $manager->id, $frontcontext->id);

        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));

        $result = unassign_capability('eset/backup:backupcourse', $manager->id, $syscontext->id);
        $this->assertTrue($result);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));
        unassign_capability('eset/backup:backupcourse', $manager->id, $frontcontext);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));

        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $manager->id, $syscontext->id);
        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $manager->id, $frontcontext->id);
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));

        $result = unassign_capability('eset/backup:backupcourse', $manager->id);
        $this->assertTrue($result);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'eset/backup:backupcourse')));
    ...
    public function test_role_assign() {
        ...
        $this->assertInstanceOf('eset_url', $event->get_url());
    ...
    public function test_get_roles_with_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupcourse'))); // Any capability is ok.
        $DB->delete_records('role_capabilities', array('capability'=>'eset/backup:backupcourse'));

        $roles = get_roles_with_capability('eset/backup:backupcourse');
        $this->assertEquals(array(), $roles);

        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $manager->id, $syscontext->id);
        assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $manager->id, $frontcontext->id);
        assign_capability('eset/backup:backupcourse', CAP_PREVENT, $teacher->id, $frontcontext->id);

        $roles = get_roles_with_capability('eset/backup:backupcourse');
        $this->assertEquals(array($teacher->id, $manager->id), array_keys($roles), '', 0, 10, true);

        $roles = get_roles_with_capability('eset/backup:backupcourse', CAP_ALLOW);
        $this->assertEquals(array($manager->id), array_keys($roles), '', 0, 10, true);

        $roles = get_roles_with_capability('eset/backup:backupcourse', null, $syscontext);
        $this->assertEquals(array($manager->id), array_keys($roles), '', 0, 10, true);
    }
    ...
    public function test_core_role_set_assign_allowed() {
        ...
        $mode = 'assign';
        $baseurl = new eset_url('/admin/roles/allow.php', array('mode' => $mode));
    ...
    public function test_get_overridable_roles() {
        ...
        $DB->insert_record('role_names', $teacherename);
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupcourse'))); // Any capability is ok.
        assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $teacherrole->id, $coursecontext->id);
        ...
        foreach ($users as $user) {
            ...
            foreach ($contexts as $contextid => $unused) {
                ...
                foreach ($allroles as $roleid => $role) {
                    $hascap = has_any_capability(array('eset/role:safeoverride', 'eset/role:override'), $context);
                    if (is_siteadmin()) {
                        $this->assertTrue(isset($roles[$roleid]));
                    } else {
                        $parents = $context->get_parent_context_ids(true);
    ...
    public function test_has_capability_and_friends() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupsection')));
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'eset/site:approvecourse')));

        $sca = array('eset/backup:backupsection', 'eset/backup:backupcourse', 'eset/site:approvecourse');
        $sc = array('eset/backup:backupsection', 'eset/backup:backupcourse');

        $this->setUser(0);
        $this->assertFalse(has_capability('eset/backup:backupsection', $coursecontext));
        $this->assertFalse(has_capability('eset/backup:backupcourse', $coursecontext));
        $this->assertFalse(has_capability('eset/site:approvecourse', $coursecontext));
        $this->assertFalse(has_any_capability($sca, $coursecontext));
        $this->assertFalse(has_all_capabilities($sca, $coursecontext));

        $this->assertTrue(has_capability('eset/backup:backupsection', $coursecontext, $teacher));
        $this->assertTrue(has_capability('eset/backup:backupcourse', $coursecontext, $teacher));
        $this->assertFalse(has_capability('eset/site:approvecourse', $coursecontext, $teacher));
        ...
        $this->assertTrue(has_capability('eset/backup:backupsection', $coursecontext, $admin));
        $this->assertTrue(has_capability('eset/backup:backupcourse', $coursecontext, $admin));
        $this->assertTrue(has_capability('eset/site:approvecourse', $coursecontext, $admin));
        ...
        $this->assertFalse(has_capability('eset/backup:backupsection', $coursecontext, $admin, false));
        $this->assertFalse(has_capability('eset/backup:backupcourse', $coursecontext, $admin, false));
        $this->assertFalse(has_capability('eset/site:approvecourse', $coursecontext, $admin, false));
        ...
        $this->setUser($teacher);
        $this->assertTrue(has_capability('eset/backup:backupsection', $coursecontext));
        $this->assertTrue(has_capability('eset/backup:backupcourse', $coursecontext));
        $this->assertFalse(has_capability('eset/site:approvecourse', $coursecontext));
        ...
        $this->assertTrue(has_capability('eset/backup:backupsection', $coursecontext));
        $this->assertTrue(has_capability('eset/backup:backupcourse', $coursecontext));
        $this->assertTrue(has_capability('eset/site:approvecourse', $coursecontext));
        $this->assertTrue(has_any_capability($sca, $coursecontext));
        $this->assertTrue(has_all_capabilities($sc, $coursecontext));
        $this->assertTrue(has_all_capabilities($sca, $coursecontext));

        $this->assertFalse(has_capability('eset/backup:backupsection', $coursecontext, 0));
        $this->assertFalse(has_capability('eset/backup:backupcourse', $coursecontext, 0));
        $this->assertFalse(has_capability('eset/site:approvecourse', $coursecontext, 0));
    ...
    public function test_fake_capability() {
        ...
        $fakecapname = 'eset/fake:capability';
    ...
    public function test_fake_capability_assign() {
        ...
        $capability = 'eset/fake:capability';
    ...
    public function test_role_definition_caching() {
        ...
        // Alter a role.
        role_change_permission($studentrole->id, $coursecontext, 'eset/course:tag', CAP_ALLOW);
    ...
    public function test_get_user_capability_course() {
        ...
        $cap = 'eset/course:view';

        // The structure being created here is this:
        //
        // All tests work with the single capability 'eset/course:view'.
        //
    ...
    public function test_guess_if_creator_will_have_course_capability() {
        ...
        $this->assertFalse(has_capability('eset/course:view', $categorycontext, $creator));
        $this->assertFalse(has_capability('eset/role:assign', $categorycontext, $creator));
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext, $creator));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext, $creator));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, $creator));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, $creator));

        $this->assertTrue(has_capability('eset/role:assign', $categorycontext, $manager));
        $this->assertTrue(has_capability('eset/course:visibility', $categorycontext, $manager));
        $this->assertTrue(has_capability('eset/course:visibility', $coursecontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, $manager->id));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, $manager->id));

        $this->assertEquals(0, $USER->id);
        $this->assertFalse(has_capability('eset/course:view', $categorycontext));
        $this->assertFalse(has_capability('eset/role:assign', $categorycontext));
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext));

        $this->setUser($manager);
        $this->assertTrue(has_capability('eset/role:assign', $categorycontext));
        $this->assertTrue(has_capability('eset/course:visibility', $categorycontext));
        $this->assertTrue(has_capability('eset/course:visibility', $coursecontext));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext));
        ...
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext, $creator));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext, $creator));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, $creator));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, $creator));

        $this->setUser($creator);
        $this->assertFalse(has_capability('eset/role:assign', $categorycontext, null));
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext, null));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext, null));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, null));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, null));
        $this->setUser(0);
        ...
        role_change_permission($managerrole->id, $categorycontext, 'eset/course:visibility', CAP_PREVENT);
        role_assign($creatorrole->id, $manager->id, $categorycontext);

        $this->assertTrue(has_capability('eset/course:view', $categorycontext, $manager));
        $this->assertTrue(has_capability('eset/course:view', $coursecontext, $manager));
        $this->assertTrue(has_capability('eset/role:assign', $categorycontext, $manager));
        $this->assertTrue(has_capability('eset/role:assign', $coursecontext, $manager));
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext, $manager));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext, $manager));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, $manager));
        $this->assertFalse(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, $manager));

        role_change_permission($managerrole->id, $categorycontext, 'eset/course:view', CAP_PREVENT);
        $this->assertTrue(has_capability('eset/role:assign', $categorycontext, $manager));
        $this->assertFalse(has_capability('eset/course:visibility', $categorycontext, $manager));
        $this->assertFalse(has_capability('eset/course:visibility', $coursecontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $categorycontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('eset/course:visibility', $coursecontext, $manager));
    ...
    public function test_require_capability() {
        ...
        try {
            require_capability('eset/site:config', $syscontext);
            $this->fail('Exception expected from require_capability()');
        } catch (eset_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
        $this->setAdminUser();
        $this->assertFalse(has_capability('eset/site:config', $syscontext, 0));
        try {
            require_capability('eset/site:config', $syscontext, 0);
            $this->fail('Exception expected from require_capability()');
        } catch (eset_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
        $this->assertFalse(has_capability('eset/site:config', $syscontext, null, false));
        try {
            require_capability('eset/site:config', $syscontext, null, false);
            $this->fail('Exception expected from require_capability()');
        } catch (eset_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
    ...
    public function test_permission_evaluation() {
        ...
        foreach ($testcontexts as $context) {
            ...
            $this->assertInstanceOf('eset_url', $context->get_url());
        ...
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertFalse($rc);
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $allroles['teacher'], $frontpagecontext->id);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertEquals(CAP_ALLOW, $rc->permission);
        assign_capability('eset/site:accessallgroups', CAP_PREVENT, $allroles['teacher'], $frontpagecontext->id);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertEquals(CAP_ALLOW, $rc->permission);
        assign_capability('eset/site:accessallgroups', CAP_PREVENT, $allroles['teacher'], $frontpagecontext, true);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertEquals(CAP_PREVENT, $rc->permission);

        assign_capability('eset/site:accessallgroups', CAP_INHERIT, $allroles['teacher'], $frontpagecontext);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertFalse($rc);
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $allroles['teacher'], $frontpagecontext);
        unassign_capability('eset/site:accessallgroups', $allroles['teacher'], $frontpagecontext, true);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'eset/site:accessallgroups'));
        $this->assertFalse($rc);
        unassign_capability('eset/site:accessallgroups', $allroles['teacher'], $frontpagecontext->id, true);
        unset($rc);
        ...
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $CFG->defaultuserroleid, $frontpageblockcontext, true);
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpageblockcontext, true);
        assign_capability('eset/block:view', CAP_PROHIBIT, $allroles['guest'], $frontpageblockcontext, true);
        ...
        assign_capability('eset/site:accessallgroups', CAP_PREVENT, $CFG->defaultuserroleid, $frontpagepagecontext, true);
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpagepagecontext, true);
        ...
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $CFG->defaultuserroleid, $frontpagecontext, true);
        assign_capability('eset/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpagecontext, true);
        ...
        assign_capability('eset/course:update', CAP_PROHIBIT, $allroles['teacher'], context_course::instance($testcourses[17]), true);
        ...
        $this->assertFalse(has_capability('eset/block:view', $frontpageblockcontext, $guestid));
        ...
        $this->assertFalse(has_capability('eset/block:view', $frontpageblockcontext, 0));
        ...
        $this->assertFalse(has_capability('eset/course:create', $systemcontext, $testusers[11]));
        $this->assertTrue(has_capability('eset/course:create', context_coursecat::instance($testcategories[2]), $testusers[11]));
        $this->assertFalse(has_capability('eset/course:create', context_course::instance($testcourses[1]), $testusers[11]));
        $this->assertTrue(has_capability('eset/course:create', context_course::instance($testcourses[19]), $testusers[11]));

        $this->assertFalse(has_capability('eset/course:update', context_course::instance($testcourses[1]), $testusers[9]));
        $this->assertFalse(has_capability('eset/course:update', context_course::instance($testcourses[19]), $testusers[9]));
        $this->assertFalse(has_capability('eset/course:update', $systemcontext, $testusers[9]));

        // Test prohibits.
        $this->assertTrue(has_capability('eset/course:update', context_system::instance(), $testusers[19]));
        $ids = get_users_by_capability(context_system::instance(), 'eset/course:update', 'u.id');
        $this->assertArrayHasKey($testusers[19], $ids);
        $this->assertFalse(has_capability('eset/course:update', context_course::instance($testcourses[17]), $testusers[19]));
        $ids = get_users_by_capability(context_course::instance($testcourses[17]), 'eset/course:update', 'u.id');
        ...
        $enrolled = get_enrolled_users($coursecontext, 'eset/course:update');
        $this->assertCount(1, $enrolled);
        ...
        load_all_capabilities();
        $coursecontext = context_course::instance($course1->id);
        $this->assertTrue(has_capability('eset/course:update', $coursecontext));
        ...
        $this->assertFalse(has_capability('eset/course:update', $coursecontext));
        ...
        $this->assertTrue(has_capability('eset/course:update', $blockcontext));
        role_switch($allroles['student'], $coursecontext);
        $this->assertEquals($allroles['student'], $USER->access['rsw'][$coursecontext->path]);
        $this->assertFalse(has_capability('eset/course:update', $blockcontext));
        reload_all_capabilities();
        $this->assertFalse(has_capability('eset/course:update', $blockcontext));
        load_all_capabilities();
        $this->assertTrue(has_capability('eset/course:update', $blockcontext));
        ...
        $this->assertFalse(has_capability('eset/course:update', $coursecontext, $userid));
        load_temp_course_role($coursecontext, $roleid);
        reload_all_capabilities();
        $this->assertFalse(has_capability('eset/course:update', $coursecontext, $userid));
        $USER = new stdClass();
    ...
    public function test_context_module_caps_returned_by_get_capabilities_in_sys_context() {
        ...
        $expectedcapabilities = ['eset/site:accessallgroups', 'eset/site:viewfullnames',
    ...
    public function test_update_capabilities() {
        ...
        $this->assertFalse(isset($existingcaps['eset/site:restore']));         // Moved to new 'eset/restore:restorecourse'.
        $this->assertTrue(isset($existingcaps['eset/restore:restorecourse'])); // New cap from 'eset/site:restore'.
        $this->assertTrue(isset($existingcaps['eset/site:sendmessage']));      // New capability.
        $this->assertTrue(isset($existingcaps['eset/backup:backupcourse']));
        $this->assertTrue(isset($existingcaps['eset/backup:backupsection']));  // Cloned from 'eset/backup:backupcourse'.
        $this->assertTrue(isset($existingcaps['eset/site:approvecourse']));    // Updated bitmask.
        $this->assertTrue(isset($existingcaps['eset/course:manageactivities']));
        $this->assertTrue(isset($existingcaps['mod/page:addinstance']));         // Cloned from core 'eset/course:manageactivities'.

        // Fake state before upgrade.
        $DB->set_field('capabilities', 'name', 'eset/site:restore', array('name'=>'eset/restore:restorecourse'));
        $DB->set_field('role_capabilities', 'capability', 'eset/site:restore', array('capability'=>'eset/restore:restorecourse'));
        assign_capability('eset/site:restore', CAP_PROHIBIT, $teacher->id, $froncontext->id, true);
        $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'eset/site:restore', 'roleid'=>$teacher->id), 'contextid, permission', 'contextid, permission'));

        $DB->delete_records('role_capabilities', array('capability'=>'eset/site:sendmessage'));
        $DB->delete_records('capabilities', array('name'=>'eset/site:sendmessage'));

        $DB->delete_records('role_capabilities', array('capability'=>'eset/backup:backupsection'));
        $DB->delete_records('capabilities', array('name'=>'eset/backup:backupsection'));
        assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $student->id, $froncontext->id, true);
        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $teacher->id, $froncontext->id, true);

        $DB->set_field('capabilities', 'riskbitmask', 0, array('name'=>'eset/site:approvecourse'));
        ...
        assign_capability('eset/course:manageactivities', CAP_PROHIBIT, $student->id, $froncontext->id, true);
        assign_capability('eset/course:manageactivities', CAP_ALLOW, $teacher->id, $froncontext->id, true);

        // Execute core.
        update_capabilities('eset');

        // Only core should be upgraded.
        $caps = $DB->get_records('capabilities', array(), 'id', 'name, captype, contextlevel, component, riskbitmask');

        $this->assertFalse(isset($existingcaps['eset/site:restore']));
        $this->assertTrue(isset($caps['eset/restore:restorecourse']));
        $this->assertEquals($existingcaps['eset/restore:restorecourse'], $caps['eset/restore:restorecourse']);
        $perms2 = array_values($DB->get_records('role_capabilities', array('capability'=>'eset/restore:restorecourse', 'roleid'=>$teacher->id), 'contextid, permission', 'contextid, permission'));
        $this->assertEquals($perms1, $perms2);

        $this->assertTrue(isset($caps['eset/site:sendmessage']));
        $this->assertEquals($existingcaps['eset/site:sendmessage'], $caps['eset/site:sendmessage']);

        $this->assertTrue(isset($caps['eset/backup:backupsection']));
        $this->assertEquals($existingcaps['eset/backup:backupsection'], $caps['eset/backup:backupsection']);
        $roles = $DB->get_records_sql('SELECT DISTINCT roleid AS id FROM {role_capabilities} WHERE capability=? OR capability=?', array('eset/backup:backupcourse', 'eset/backup:backupsection'));
        foreach ($roles as $role) {
            $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'eset/backup:backupcourse', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
            $perms2 = array_values($DB->get_records('role_capabilities', array('capability'=>'eset/backup:backupsection', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
            $this->assertEquals($perms1, $perms2);
        }

        $this->assertTrue(isset($caps['eset/site:approvecourse']));
        $this->assertEquals($existingcaps['eset/site:approvecourse'], $caps['eset/site:approvecourse']);
        ...
        $roles = $DB->get_records_sql('SELECT DISTINCT roleid AS id FROM {role_capabilities} WHERE capability=? OR capability=?', array('eset/course:manageactivities', 'mod/page:addinstance'));
        foreach ($roles as $role) {
            $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'eset/course:manageactivities', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
    ...
    public function test_get_users_by_capability() {
        ...
        $users = get_users_by_capability($coursecontext, 'eset/backup:backupcourse');
        ...
        $users = get_users_by_capability($coursecontext, 'eset/site:approvecourse');
        ...
        // Test role override.
        assign_capability('eset/backup:backupcourse', CAP_PROHIBIT, $teacherrole->id, $coursecontext, true);
        assign_capability('eset/backup:backupcourse', CAP_ALLOW, $studentrole->id, $coursecontext, true);
    ...
    public function test_get_with_capability_sql() {
        ...
        list($sql, $params) = get_with_capability_sql($coursecontext, 'eset/backup:backupcourse');
        $users = $DB->get_records_sql($sql, $params);
        ...
        list($sql, $params) = get_with_capability_sql($coursecontext, 'eset/site:approvecourse');
        $users = $DB->get_records_sql($sql, $params);
    ...
    public function get_get_with_capability_join_override_cases() {
        return [
                'no overrides' => [true, []],
                'one override' => [true, ['eset/course:viewscales']],
                'both overrides' => [false, ['eset/course:viewscales', 'eset/question:flag']],
        ];
    }
    ...
    public function test_get_with_capability_join_when_overrides_present(
        ...
        $coursecontext = context_course::instance($course->id);
        $this->assertTrue(has_capability('eset/course:viewscales', $coursecontext, $student));
        $this->assertTrue(has_capability('eset/question:flag', $coursecontext, $student));
        ...
        $sqljoin = get_with_capability_join($coursecontext,
                ['eset/course:viewscales', 'eset/question:flag'], 'u.id');
    ...
    public function test_require_all_capabilities() {
        ...
        $sca = array('eset/backup:backupsection', 'eset/backup:backupcourse');
