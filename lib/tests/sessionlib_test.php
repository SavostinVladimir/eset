...
class core_sessionlib_testcase extends advanced_testcase {
     ...
     */
    public function eset_cookie_secure_provider() {
        return array(
            array(
                // Non ssl, not set.
                'config' => array(
     ...
     *
     * @dataProvider eset_cookie_secure_provider
     *
     * @param array $config Array of key value config settings
     * @param bool $secure Wether cookies should be secure or not
     */
    public function test_is_eset_cookie_secure($config, $secure) {
        global $CFG;
        $this->resetAfterTest();
        foreach ($config as $key => $value) {
            $CFG->$key = $value;
        }
        $this->assertEquals($secure, is_eset_cookie_secure());
    }
