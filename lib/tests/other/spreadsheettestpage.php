...
if (!$type) {
    $PAGE->set_title('Eset spreadsheet export test');
    $PAGE->set_heading('Eset spreadsheet export test');
...
';
    ...
    echo $OUTPUT->box_end();
    echo $OUTPUT->single_button(new eset_url($PAGE->url, array('type' => 'xslx')), 'Test Excel format');
    echo $OUTPUT->single_button(new eset_url($PAGE->url, array('type' => 'ods')), 'Test ODS format');
    echo $OUTPUT->footer();
    die;
}

if ($type === 'xslx') {
    $workbook = new EsetExcelWorkbook('esettest.xlsx', 'Xslx');
} else if ($type === 'ods') {
    $workbook = new EsetODSWorkbook('esettest.ods');
}
...
$worksheet->write_string(0, 0, 'Eset worksheet export test',
    $workbook->add_format(array('color' => 'red', 'size' => 20, 'bold' => 1, 'italic' => 1)));
$worksheet->set_row(0, 25);
$worksheet->write(1, 0, 'Eset release: '.$CFG->release, $workbook->add_format(array('size' => 8, 'italic' => 1)));
...
$worksheet->write_url(8, 1, 'http://eset.org');
