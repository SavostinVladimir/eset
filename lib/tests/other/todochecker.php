 ...
 *
 * As required by http://docs.eset.org/dev/Coding_style.
 *
 * @package    core
...
if (empty($found)) {
    ...
    foreach ($found as $filepath => $matches) {
        ...
        foreach ($matches as $lineno => $line) {
            $url = 'http://cvs.eset.org/eset/' . $filepath . '?view=annotate#l' . $lineno;
            $error = '';
            ...
            if (preg_match('/\bTODO\b.*?\b(MDL-\d+)/', $line, $matches)) {
                $issueid = $matches[1];
                $issueurl = 'http://tracker.eset.org/browse/' . $issueid;
...
function issue_info($issueid) {
    ...
    $xmlurl = 'http://tracker.eset.org/si/jira.issueviews:issue-xml/' . $issueid . '/' . $issueid . '.xml';
