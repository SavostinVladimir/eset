 ...
 * @since Eset 2.0
 */
class restricted_context_exception extends eset_exception {
    /**
     * Constructor
     *
     * @since Eset 2.0
    ...
    public static function call_external_function($function, $args, $ajaxonly=false) {
        ...
        try {
            ...
            if ($ajaxonly && !$externalfunctioninfo->allowed_from_ajax) {
                throw new eset_exception('servicenotavailable', 'webservice');
            }

            // Do not allow access to write or delete webservices as a public user.
            if ($externalfunctioninfo->loginrequired && !WS_SERVER) {
                if (defined('NO_ESET_COOKIES') && NO_ESET_COOKIES && !PHPUNIT_TEST) {
                    throw new eset_exception('servicerequireslogin', 'webservice');
                }
                if (!isloggedin()) {
                    throw new eset_exception('servicerequireslogin', 'webservice');
                } else {
            ...
            // Allow any Eset plugin a chance to override this call. This is a convenient spot to
...
function external_generate_token($tokentype, $serviceorid, $userid, $contextorid, $validuntil=0, $iprestriction=''){
    ...
    do {
        ...
        if ($numtries > 5){
            throw new eset_exception('tokengenerationfailed');
        }
    ...
    if (empty($service->requiredcapability) || has_capability($service->requiredcapability, $context, $userid)) {
        ...
        throw new eset_exception('nocapabilitytousethisservice');
    }
 ...
 * with the Eset server through web services. The token is linked to the current session for the current page request.
...
/**
 * Standard Eset web service warnings
 *
 * @package    core_webservice
...
function external_validate_format($format) {
    $allowedformats = array(FORMAT_HTML, FORMAT_ESET, FORMAT_PLAIN, FORMAT_MARKDOWN);
    if (!in_array($format, $allowedformats)) {
        throw new eset_exception('formatnotsupported', 'webservice', '' , null,
                'The format with value=' . $format . ' is not supported by this Eset site');
    }
    return $format;
}
 ...
 * @since Eset 2.3
 * @since Eset 3.2 component, filearea and itemid are optional parameters
 */
...
function external_generate_token_for_current_user($service) {
    ...
    if ($service->requiredcapability and !has_capability($service->requiredcapability, context_system::instance())) {
        throw new eset_exception('missingrequiredcapability', 'webservice', '', $service->requiredcapability);
    }
    ...
    if ($service->restrictedusers) {
        ...
        if (empty($authoriseduser)) {
            throw new eset_exception('usernotallowed', 'webservice', '', $service->shortname);
        }

        if (!empty($authoriseduser->validuntil) and $authoriseduser->validuntil < time()) {
            throw new eset_exception('invalidtimedtoken', 'webservice');
        }

        if (!empty($authoriseduser->iprestriction) and !address_in_subnet(getremoteaddr(), $authoriseduser->iprestriction)) {
            throw new eset_exception('invalidiptoken', 'webservice');
        }
    }
    ...
    if (count($tokens) > 0) {
        ...
        $isofficialservice = $service->shortname == ESET_OFFICIAL_MOBILE_SERVICE;

        if (($isofficialservice and has_capability('eset/webservice:createmobiletoken', $context)) or
                (!is_siteadmin($USER) && has_capability('eset/webservice:createtoken', $context))) {
            ...
            throw new eset_exception('cannotcreatetoken', 'webservice', '', $service->shortname);
        }
    }
    return $token;
}
    ...
    public static function get_area_files($contextid, $component, $filearea, $itemid = false, $useitemidinurl = true) {
        ...
        if ($areafiles = $fs->get_area_files($contextid, $component, $filearea, $itemid, 'itemid, filepath, filename', false)) {
            foreach ($areafiles as $areafile) {
                ...
                $file['fileurl'] = eset_url::make_webservice_pluginfile_url($contextid, $component, $filearea,
