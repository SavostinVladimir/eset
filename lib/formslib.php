...
/**
 * formslib.php - library of classes for creating forms in Eset, based on PEAR QuickForms.
 *
 ...
 * extend esetform overriding abstract classes definition and optionally defintion_after_data
 * and validation.
 *
...
/**
 * Wrapper that separates quickforms syntax from eset code
 *
 * Eset specific wrapper that separates quickforms syntax from eset code. You won't directly
 * use this class you should write a class definition which extends this class or a more specific
 * subclass such a esetform_mod for each form you want to display and/or process with formslib.
 *
 ...
 */
abstract class esetform {
    /** @var string name of the form */
    protected $_formname;       // form name

    /** @var EsetQuickForm quickform object definition */
    protected $_form;
     ...
     * the name you gave the class extending esetform. You should call your class something
     * like
     *
     * @param mixed $action the action attribute for the form. If empty defaults to auto detect the
     *              current url. If a eset_url object then outputs params as hidden variables.
    ...
    public function __construct($action=null, $customdata=null, $method='post', $target='', $attributes=null, $editable=true,
                                $ajaxformdata=null) {
        global $CFG, $FULLME;
        // no standard mform in eset should allow autocomplete with the exception of user signup
        if (empty($attributes)) {
        ...
        $this->_form = new EsetQuickForm($this->_formname, $method, $action, $target, $attributes, $ajaxformdata);
        if (!$editable){
     ...
     */
    public function esetform($action=null, $customdata=null, $method='post', $target='', $attributes=null, $editable=true) {
    ...
    function validate_defined_fields($validateonnosubmit=false) {
        ...
        if ($this->no_submit_button_pressed() && empty($validateonnosubmit)){
            ...
            $eset_val = $this->validation($data, $files);
            if ((is_array($eset_val) && count($eset_val)!==0)) {
                // non-empty array means errors
                foreach ($eset_val as $element=>$msg) {
                    $mform->setElementError($element, $msg);
                }
                $eset_val = false;
            ...
            $this->_validated = ($internal_val and $element_val and $eset_val and $file_val);
        }
     ...
     *
     * @deprecated since Eset 2.0
     * @todo MDL-31294 remove this api
     * @see esetform::save_stored_file()
     * @see esetform::save_file()
    ...
    function get_new_filename($elname=null) {
        ...
        if ($element instanceof EsetQuickForm_filepicker || $element instanceof EsetQuickForm_filemanager) {
     ...
     * This is a protected method which will be used only inside esetforms
     *
     * @param string $elname name of element
    ...
    function save_stored_file($elname, $newcontextid, $newcomponent, $newfilearea, $newitemid, $newfilepath='/',
        ...
        if ($element instanceof EsetQuickForm_filepicker) {
            $values = $this->_form->exportValues($elname);
            if (empty($values[$elname])) {
                return false;
            }
     ...
     * IntermediateClass extends esetform'.
     *
    ...
    function add_checkbox_controller($groupid, $text = null, $attributes = null, $originalValue = 0) {
        ...
        if (!is_null($contollerbutton) || is_null($selectvalue)) {
            foreach ($mform->_elements as $element) {
                if (($element instanceof EsetQuickForm_advcheckbox) &&
        ...
        $PAGE->requires->yui_module('eset-form-checkboxcontroller', 'M.form.checkboxcontroller',
                array(
        ...
        $submitlink = new EsetQuickForm_submit($checkboxcontrollername, $attributes);
     ...
     * @deprecated since Eset 3.3 MDL-57471
     */
    ...
    /**
     * Used by tests to generate valid submit keys for eset forms that are
     * submitted with ajax data.
     *
     * @throws \eset_exception If called outside unit test environment
    ...
    public static function mock_generate_submit_keys($data = []) {
        if (!defined('PHPUNIT_TEST') || !PHPUNIT_TEST) {
            throw new \eset_exception("This function can only be used for unit testing.");
        }
...
/**
 * EsetQuickForm implementation
 *
 * You never extend this class directly. The class methods of this class are available from
 * the private $this->_form property on esetform and its children. You generally only
 * call methods on this class from within abstract methods that you override on esetform such
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class EsetQuickForm extends HTML_QuickForm_DHTMLRulesTableless {
    ...
    /**
     * String with the html for hidden params passed in as part of a eset_url
     ...
     * @param string|eset_url $action Form's action
     ...
     * @deprecated since Eset 3.1
     */
    public function EsetQuickForm($formName, $method, $action, $target='', $attributes=null) {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
    ...
    */
     ...
     *        {@link lib/esetlib.php} for defined parameter types
     */
    ...
    public function getCleanedValue($value, $type) {
        if (is_array($type) && is_array($value)) {
            ...
            throw new coding_exception('Unexpected type or value received in EsetQuickForm::getCleanedValue()');
        }
        return $value;
    }
     ...
     * no data is loaded in using esetform::set_data()
     *
     * note: $slashed param removed
     *
     ...
     */
    function addHelpButton($elementname, $identifier, $component = 'eset', $linktext = '', $suppresscheck = false) {
        global $OUTPUT;
                ...
                if ('client' == $rule['validation']) {
                    ...
                    if (is_object($element) && $element->getType() == 'editor') {
                        if ($element->getType() == 'editor') {
                            ...
                            // Add format to rule as esetform check which format is supported by browser
...
';
      ...
      if (!ret && !first_focus) {
        first_focus = true;
        Y.use(\'eset-core-event\', function() {
        ...
        if (is_a($element, 'HTML_QuickForm_group')) {
            ...
            foreach ($elsInGroup as $elInGroup){
                if (is_a($elInGroup, 'HTML_QuickForm_group')) {
                    ...
                    // We will be appending group name again in EsetQuickForm_group::export_for_template().
...
/**
 * EsetQuickForm renderer
 *
 * A renderer for EsetQuickForm that only uses XHTML and CSS and no
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class EsetQuickForm_Renderer extends HTML_QuickForm_Renderer_Tableless{
     ...
     * @deprecated since Eset 3.1
     */
    public function EsetQuickForm_Renderer() {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
        self::__construct();
    }
     ...
     *
     * @param EsetQuickForm $form reference of the form
     */
    function startForm(&$form){
        ...
        if ($form->is_form_change_checker_enabled()) {
            $PAGE->requires->yui_module('eset-core-formchangechecker',
            ...
            $PAGE->requires->string_for_js('changesmadereallygoaway', 'eset');
        }
        if (!empty($this->_collapsibleElements)) {
            if (count($this->_collapsibleElements) > 1) {
                ...
                $PAGE->requires->strings_for_js(array('collapseall', 'expandall'), 'eset');
            }
            $PAGE->requires->yui_module('eset-form-shortforms', 'M.form.shortforms', array(array('formid' => $formid)));
        }
     ...
     *
     * @param EsetQuickForm_group $group Passed by reference
     ...
     *
     * @global eset_page $PAGE
     * @param esetform $form Passed by reference
     */
    function finishForm(&$form){
        ...
        if (!$form->isFrozen()) {
            ...
            if (count($args[1]) > 0) {
                $PAGE->requires->js_init_call('M.form.initFormDependencies', $args, true, esetform::get_js_module());
            }
        }
    }
   /**
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class EsetQuickForm_Rule_Required extends HTML_QuickForm_Rule {
    /**
 ...
 */
$GLOBALS['_HTML_QuickForm_default_renderer'] = new EsetQuickForm_Renderer();

/** Please keep this list in alphabetical order. */
EsetQuickForm::registerElementType('advcheckbox', "$CFG->libdir/form/advcheckbox.php", 'EsetQuickForm_advcheckbox');
EsetQuickForm::registerElementType('autocomplete', "$CFG->libdir/form/autocomplete.php", 'EsetQuickForm_autocomplete');
EsetQuickForm::registerElementType('button', "$CFG->libdir/form/button.php", 'EsetQuickForm_button');
EsetQuickForm::registerElementType('cancel', "$CFG->libdir/form/cancel.php", 'EsetQuickForm_cancel');
EsetQuickForm::registerElementType('course', "$CFG->libdir/form/course.php", 'EsetQuickForm_course');
EsetQuickForm::registerElementType('cohort', "$CFG->libdir/form/cohort.php", 'EsetQuickForm_cohort');
EsetQuickForm::registerElementType('searchableselector', "$CFG->libdir/form/searchableselector.php", 'EsetQuickForm_searchableselector');
EsetQuickForm::registerElementType('checkbox', "$CFG->libdir/form/checkbox.php", 'EsetQuickForm_checkbox');
EsetQuickForm::registerElementType('date_selector', "$CFG->libdir/form/dateselector.php", 'EsetQuickForm_date_selector');
EsetQuickForm::registerElementType('date_time_selector', "$CFG->libdir/form/datetimeselector.php", 'EsetQuickForm_date_time_selector');
EsetQuickForm::registerElementType('duration', "$CFG->libdir/form/duration.php", 'EsetQuickForm_duration');
EsetQuickForm::registerElementType('editor', "$CFG->libdir/form/editor.php", 'EsetQuickForm_editor');
EsetQuickForm::registerElementType('filemanager', "$CFG->libdir/form/filemanager.php", 'EsetQuickForm_filemanager');
EsetQuickForm::registerElementType('filepicker', "$CFG->libdir/form/filepicker.php", 'EsetQuickForm_filepicker');
EsetQuickForm::registerElementType('filetypes', "$CFG->libdir/form/filetypes.php", 'EsetQuickForm_filetypes');
EsetQuickForm::registerElementType('float', "$CFG->libdir/form/float.php", 'EsetQuickForm_float');
EsetQuickForm::registerElementType('grading', "$CFG->libdir/form/grading.php", 'EsetQuickForm_grading');
EsetQuickForm::registerElementType('group', "$CFG->libdir/form/group.php", 'EsetQuickForm_group');
EsetQuickForm::registerElementType('header', "$CFG->libdir/form/header.php", 'EsetQuickForm_header');
EsetQuickForm::registerElementType('hidden', "$CFG->libdir/form/hidden.php", 'EsetQuickForm_hidden');
EsetQuickForm::registerElementType('listing', "$CFG->libdir/form/listing.php", 'EsetQuickForm_listing');
EsetQuickForm::registerElementType('defaultcustom', "$CFG->libdir/form/defaultcustom.php", 'EsetQuickForm_defaultcustom');
EsetQuickForm::registerElementType('modgrade', "$CFG->libdir/form/modgrade.php", 'EsetQuickForm_modgrade');
EsetQuickForm::registerElementType('modvisible', "$CFG->libdir/form/modvisible.php", 'EsetQuickForm_modvisible');
EsetQuickForm::registerElementType('password', "$CFG->libdir/form/password.php", 'EsetQuickForm_password');
EsetQuickForm::registerElementType('passwordunmask', "$CFG->libdir/form/passwordunmask.php", 'EsetQuickForm_passwordunmask');
EsetQuickForm::registerElementType('questioncategory', "$CFG->libdir/form/questioncategory.php", 'EsetQuickForm_questioncategory');
EsetQuickForm::registerElementType('radio', "$CFG->libdir/form/radio.php", 'EsetQuickForm_radio');
EsetQuickForm::registerElementType('recaptcha', "$CFG->libdir/form/recaptcha.php", 'EsetQuickForm_recaptcha');
EsetQuickForm::registerElementType('select', "$CFG->libdir/form/select.php", 'EsetQuickForm_select');
EsetQuickForm::registerElementType('selectgroups', "$CFG->libdir/form/selectgroups.php", 'EsetQuickForm_selectgroups');
EsetQuickForm::registerElementType('selectwithlink', "$CFG->libdir/form/selectwithlink.php", 'EsetQuickForm_selectwithlink');
EsetQuickForm::registerElementType('selectyesno', "$CFG->libdir/form/selectyesno.php", 'EsetQuickForm_selectyesno');
EsetQuickForm::registerElementType('static', "$CFG->libdir/form/static.php", 'EsetQuickForm_static');
EsetQuickForm::registerElementType('submit', "$CFG->libdir/form/submit.php", 'EsetQuickForm_submit');
EsetQuickForm::registerElementType('tags', "$CFG->libdir/form/tags.php", 'EsetQuickForm_tags');
EsetQuickForm::registerElementType('text', "$CFG->libdir/form/text.php", 'EsetQuickForm_text');
EsetQuickForm::registerElementType('textarea', "$CFG->libdir/form/textarea.php", 'EsetQuickForm_textarea');
EsetQuickForm::registerElementType('url', "$CFG->libdir/form/url.php", 'EsetQuickForm_url');
EsetQuickForm::registerElementType('warning', "$CFG->libdir/form/warning.php", 'EsetQuickForm_warning');

EsetQuickForm::registerRule('required', null, 'EsetQuickForm_Rule_Required', "$CFG->libdir/formslib.php");
