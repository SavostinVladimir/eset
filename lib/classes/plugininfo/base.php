...
use core_component, core_plugin_manager, eset_url, coding_exception;
...
abstract class base {
    ...
    public $versiondb;
    /** @var int|float|string required version of Eset core  */
    public $versionrequires;
    /** @var array explicitly supported branches of Eset core  */
    public $pluginsupported;
    /** @var int first incompatible branch of Eset core  */
     ...
     * of the current Eset version, false otherwise.
     *
     * @return bool
     */
    ...
    /**
     * Returns true if the the given Eset version is enough to run this plugin
     *
     * @param string|int|double $esetversion
     * @return bool
     */
    public function is_core_dependency_satisfied($esetversion) {

        if (empty($this->versionrequires)) {
            return true;

        } else {
            return (double)$this->versionrequires <= (double)$esetversion;
        }
    }

    /**
     * Returns true if the the given eset branch is not stated incompatible with the plugin
     *
     * @param int $branch the eset branch number
     * @return bool true if not incompatible with eset branch
     */
    ...
    public function get_status() {
        ...
        if (is_null($this->versiondb) and is_null($this->versiondisk)) {
            ...
            // $version = pi(); and similar funny jokes - hopefully Donald E. Knuth will never contribute to Eset ;-)
    ...
    public function get_settings_url() {
        ...
        if ($settings && $settings instanceof \admin_settingpage) {
            return new eset_url('/admin/settings.php', array('section' => $section));
        } else if ($settings && $settings instanceof \admin_externalpage) {
            return new eset_url($settings->url);
        } else {
            return null;
        }
    }
     ...
     * @param bool $hassiteconfig whether the current user has eset/site:config capability
     */
    ...
    public function get_return_url_after_uninstall($return) {
        ...
        return new eset_url('/admin/plugins.php#plugin_type_cell_'.$this->type);
    }
    ...
    public final function get_default_uninstall_url($return = 'overview') {
        return new eset_url('/admin/plugins.php', array(
            'sesskey' => sesskey(),
