    ...
    public function get_url() {
        if ($this->courseid) {
            if ($this->contextlevel == CONTEXT_MODULE) {
                return new \eset_url('/question/preview.php', ['cmid' => $this->contextinstanceid, 'id' => $this->objectid]);
            }
            return new \eset_url('/question/preview.php', ['courseid' => $this->courseid, 'id' => $this->objectid]);
        }
        // Lets try editing from the frontpage for contexts above course.
        return new \eset_url('/question/preview.php', ['courseid' => SITEID, 'id' => $this->objectid]);
    }
}
