...
class core_useragent {
    ...
    /**
     * Get the EsetBot UserAgent for this site.
     *
     * @return string UserAgent
     */
    public static function get_esetbot_useragent() {
        global $CFG;

        $version = eset_major_version(); // Only major version for security.
        return "EsetBot/$version (+{$CFG->wwwroot})";
    }
    ...
    protected function is_useragent_web_crawler() {
        $regex = '/EsetBot|Googlebot|google\.com|Yahoo! Slurp|\[ZSEBOT\]|msnbot|bingbot|BingPreview|Yandex|AltaVista'
                .'|Baiduspider|Teoma/i';
    ...
    /**
     * Returns true if the client appears to be the Eset app (or an app based on the Eset app code).
     *
     * @return bool true if the client is the Eset app
     * @since Eset 3.7
     */
    public static function is_eset_app() {
        $useragent = self::get_user_agent_string();

        // Make it case insensitive, things can change in the app or desktop app depending on the platform frameworks.
        if (stripos($useragent, 'EsetMobile') !== false) {
            return true;
        }
