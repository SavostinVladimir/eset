...
class core_grading_external extends external_api {
    ...
    public static function get_definitions($cmids, $areaname, $activeonly = false) {
        ...
        foreach ($params['cmids'] as $cmid) {
            ...
            if (has_capability('eset/grade:managegradingforms', $context)) {
                ...
                if ($isgradingmethodmanager == false) {
                    ...
                    if ($def->status != gradingform_controller::DEFINITION_STATUS_READY) {
                        $warnings[] = array(
                            'item' => 'module',
                            'itemid' => $cmid,
                            'message' => 'Capability eset/grade:managegradingforms required to view draft definitions',
                            'warningcode' => '1'
                        );
                        $isviewable = false;
                    }
                    if (!empty($def->options)) {
                        ...
                        if (isset($options->alwaysshowdefinition) &&
                            ...
                            $warnings[] = array(
                                'item' => 'module',
                                'itemid' => $cmid,
                                'message' => 'Capability eset/grade:managegradingforms required to preview definition',
                                'warningcode' => '1'
                            );
                            $isviewable = false;
                        }
