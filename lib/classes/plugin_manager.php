 ...
 * Eset. It is mainly used by the plugins management admin page and the
...
class core_plugin_manager {
    ...
    /** the plugin is shipped with standard Eset distribution */
    ...
    /** the current Eset version is too high for plugin. */
    ...
    /** the eset version is explicitly supported */
    const VERSION_SUPPORTED = 'supported';
    /** the eset version is not explicitly supported */
    ...
    /** @var \core\update\api client instance to use for accessing download.eset.org/api/ */
    ...
    public function plugin_name($component) {
        ...
        if (is_null($pluginfo)) {
            throw new eset_exception('err_unknown_plugin', 'core_plugin', '', array('plugin' => $component));
        }
     ...
     *
     * @param int $esetversion the version from version.php.
     * @param array $failedplugins to return the list of plugins with non-satisfied dependencies
     * @param int $branch the current eset branch, null if not provided
     * @return bool true if all the dependencies are satisfied for all plugins.
     */
    public function all_plugins_ok($esetversion, &$failedplugins = array(), $branch = null) {
        global $CFG;
        if (empty($branch)) {
            $branch = $CFG->branch;
            if (empty($branch)) {
        ...
        foreach ($this->get_plugins() as $type => $plugins) {
            foreach ($plugins as $plugin) {
                ...
                if (!$plugin->is_core_dependency_satisfied($esetversion)) {
                    $return = false;
     ...
     * @param null|string|int|double $esetversion explicit eset core version to check against, defaults to $CFG->version
     * @param null|string|int $esetbranch explicit eset core branch to check against, defaults to $CFG->branch
     * @return array of objects
     */
    public function resolve_requirements(\core\plugininfo\base $plugin, $esetversion=null, $esetbranch=null) {
        global $CFG;
        ...
        if ($esetversion === null) {
            $esetversion = $CFG->version;
        }

        if ($esetbranch === null) {
            $esetbranch = $CFG->branch;
        }

        $reqs = array();
        $reqcore = $this->resolve_core_requirements($plugin, $esetversion, $esetbranch);
        ...
        foreach ($plugin->get_other_required_plugins() as $reqplug => $reqver) {
            $reqs[$reqplug] = $this->resolve_dependency_requirements($plugin, $reqplug, $reqver, $esetbranch);
        }
    ...
    /**
     * Helper method to resolve plugin's requirements on the eset core.
     *
     * @param \core\plugininfo\base $plugin the plugin we are checking
     * @param string|int|double $esetversion eset core branch to check against
     * @return stdObject
     */
    protected function resolve_core_requirements(\core\plugininfo\base $plugin, $esetversion, $esetbranch) {
        ...
        if (isset($plugin->pluginincompatible) && $plugin->pluginincompatible != null) {
            if (!$plugin->is_core_compatible_satisfied($esetbranch)) {
     ...
     * @param string|int $esetbranch explicit eset core branch to check against, defaults to $CFG->branch
     * @return stdClass
     */
    protected function resolve_dependency_requirements(\core\plugininfo\base $plugin, $otherpluginname,
            $requiredversion, $esetbranch) {
    ...
    /**
     * Helper method to determine whether a eset version is explicitly supported.
     *
     * @param \core\plugininfo\base $plugin the plugin we are checking
     * @param int $branch the eset branch to check support for
     * @return string
     */
     ...
     * directory name would become the plugin name once extracted to the Eset
     * dirroot.
     *
     ...
     *
     * If used for installation of remote plugins from the Eset Plugins
    ...
    public function install_plugins(array $plugins, $confirmed, $silent) {
        ...
        foreach ($plugins as $plugin) {
            ...
            $validator->assert_eset_version($CFG->version);
     ...
     * @return eset_url uninstall URL, null if uninstall not supported
     */
    ...
    /**
     * Defines a list of all plugins that were originally shipped in the standard Eset distribution,
    ...
    public static function is_deleted_standard_plugin($type, $name) {
        ...
        // not supported as source versions for upgrade any more. For example, at ESET_23_STABLE
        // branch, listed should be no plugins that were removed at 1.9.x - 2.1.x versions as
        // Eset 2.3 supports upgrades from 2.2.x only.
        $plugins = array(
    ...
    /**
     * Defines a white list of all plugins shipped in the standard Eset distribution
     *
     * @param string $type
    ...
    public static function standard_plugins_list($type) {
        ...
        $standard_plugins = array(
            ...
            'tinymce' => array(
                'ctrlhelp', 'managefiles', 'esetemoticon', 'esetimage',
                'esetmedia', 'esetnolink', 'pdw', 'spellchecker', 'wrap'
            ),
            ...
            'tool' => array(
                ...
                'mobile', 'esetnet', 'multilangupgrade', 'monitor', 'oauth2', 'phpunit', 'policy', 'profiling', 'recyclebin',
    ...
    public function remove_plugin_folder(\core\plugininfo\base $plugin) {
        ...
        if (!$this->is_plugin_folder_removable($plugin->component)) {
            throw new eset_exception('err_removing_unremovable_folder', 'core_plugin', '',
     ...
     * real heart of Eset. They should be followed by other plugin types that are
    ...
    /**
     * Returns a client for https://download.eset.org/api/
     *
