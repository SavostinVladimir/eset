...
class manager {
    ...
    public static function start() {
        ...
        if (empty($DB) or empty($CFG->version) or !defined('NO_ESET_COOKIES') or NO_ESET_COOKIES or CLI_SCRIPT) {
            self::$sessionactive = false;
    ...
    protected static function prepare_cookies() {
        ...
        $cookiesecure = is_eset_cookie_secure();
        ...
        $sessionname = 'EsetSession'.$CFG->sessioncookie;
        ...
        ini_set('session.serialize_handler', 'php');  // We can move to 'php_serialize' after we require PHP 5.5.4 form Eset.

        // Eset does normal session timeouts, this is for leftovers only.
    ...
    private static function should_use_samesite_none(): bool {
        ...
        if (\core_useragent::is_chrome() && \core_useragent::check_chrome_version('78') && is_eset_cookie_secure()) {
            return true;
        }
        return false;
    }
            ...
            if (!self::$handler->requires_write_lock()) {
                ...
                if ($arraydiff) {
                    if (isset($arraydiff['cachestore_session'])) {
                        throw new \eset_exception('The session store can not be in the session when '
     ...
     *
     * All login forms generated from Eset must include a login token
     ...
     * the use of stdClasses in Eset sessions.
     *
     * @param array $array1
     * @param array $array2
     * @return array
