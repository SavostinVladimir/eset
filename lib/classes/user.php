...
class core_user {
    ...
    /** @var int Indicates that user profile view should be allowed even if Eset would prevent it */
     ...
     *
     * If you have eset/user:viewdetails at system level, you can search everyone.
     * Otherwise we check which courses you *do* have that permission and search everyone who is
     * enrolled on those courses.
     *
     * Normally you can only search the user's name. If you have the eset/site:viewuseridentity
    ...
    public static function search($query, \context_course $coursecontext = null,
        ...
        $viewsystem = has_capability('eset/user:viewdetails', $systemcontext);
        if ($viewsystem) {
        ...
        $identitysystem = has_capability('eset/site:viewuseridentity', $systemcontext);
    ...
    public static function require_active_user($user, $checksuspended = false, $checknologin = false) {
        ...
        if ($user->deleted) {
            throw new eset_exception('userdeleted');
        }
        ...
        if (isguestuser($user)) {
            throw new eset_exception('guestsarenotallowed', 'error');
        }

        if ($checksuspended and $user->suspended) {
            throw new eset_exception('suspended', 'auth');
        }
    ...
    protected static function fill_preferences_cache() {
        ...
        $preferences['auth_forcepasswordchange'] = array('type' => PARAM_INT, 'null' => NULL_NOT_ALLOWED, 'choices' => array(0, 1),
            'permissioncallback' => function($user, $preferencename) {
                global $USER;
                $systemcontext = context_system::instance();
                return ($USER->id != $user->id && (has_capability('eset/user:update', $systemcontext) ||
                        ($user->timecreated > time() - 10 && has_capability('eset/user:create', $systemcontext))));
            });
        ...
        $preferences['blogpagesize'] = array('type' => PARAM_INT, 'null' => NULL_NOT_ALLOWED, 'default' => 10,
            'permissioncallback' => function($user, $preferencename) {
                global $USER;
                return $USER->id == $user->id && has_capability('eset/blog:view', context_system::instance());
            });
    ...
    protected static function default_preference_permission_check($user, $preferencename) {
        ...
        if ($user->id == $USER->id) {
            // Editing own profile.
            $systemcontext = context_system::instance();
            return has_capability('eset/user:editownprofile', $systemcontext);
        } else  {
            // Teachers, parents, etc.
            $personalcontext = context_user::instance($user->id);
            if (!has_capability('eset/user:editprofile', $personalcontext)) {
                return false;
            }
