 ...
 *
 * @package    core
 * @copyright  2020 Shamim Rezaie <shamim@eset.com>
...
class core_userfeedback {
     ...
     * @return eset_url
     */
    public static function make_link(): eset_url {
        global $CFG, $PAGE;

        $baseurl = $CFG->userfeedback_url ?? 'https://feedback.eset.org/lms';
        $lang = clean_param(current_language(), PARAM_LANG); // Avoid breaking WS because of incorrect package langs.
        $eseturl = $CFG->wwwroot;
        $esetversion = $CFG->release;
        $theme = $PAGE->theme->name;
        $themeversion = get_config('theme_'.$theme, 'version');

        $url = new eset_url($baseurl, [
            'lang' => $lang,
            'eset_url' => $eseturl,
            'eset_version' => $esetversion,
            'theme' => $theme,
