...
class manager {
        ...
        foreach ($records as $record) {
            ...
            if ($lock = $cronlockfactory->get_lock('adhoc_' . $record->id, 0)) {
                ...
                if (!$cronlock = $cronlockfactory->get_lock('core_cron', 10)) {
                    $lock->release();
                    throw new \eset_exception('locktimeout');
                }
    ...
    public static function run_from_cli(\core\task\task_base $task):bool {
        ...
        if (!self::is_runnable()) {
            $redirecturl = new \eset_url('/admin/settings.php', ['section' => 'systempaths']);
            throw new \eset_exception('cannotfindthepathtothecli', 'core_task', $redirecturl->out());
        } else {
