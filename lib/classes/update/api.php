 ...
 *
 * @package     core
 * @copyright   2015 David Mudrak <david@eset.com>
...
/**
 * General purpose client for https://download.eset.org/api/
 *
 * The API provides proxy access to public information about plugins available
 * in the Eset Plugins directory. It is used when we are checking for
 ...
 * - locate the most suitable plugin version for the given Eset branch
 *
 * TODO:
 *
...
class api {
    ...
    const APIROOT = 'https://download.eset.org/api';
     ...
     * Eset plugins directory. This is typically used to handle the
     ...
     * name, that suits best for the given Eset core branch. Minimal required
     ...
     * @param int $branch eset core branch such as 29, 30, 31 etc, defaults to $CFG->branch
     * @return \core\update\remote_info|bool
     */
    ...
    public function validate_pluginfo_format($data) {
        ...
        if (!empty($data->version)) {
            ...
            $versionproperties = array('id' => 1, 'version' => 1, 'release' => 0, 'maturity' => 0,
                ...
                'vcsrepositoryurl' => 0, 'vcsbranch' => 0, 'vcstag' => 0, 'supportedesets' => 0);
            ...
            if (!empty($data->version->supportedesets)) {
                if (!is_array($data->version->supportedesets)) {
                    return false;
                }
                foreach ($data->version->supportedesets as $supportedeset) {
                    if (!is_object($supportedeset)) {
                        return false;
                    }
                    if (empty($supportedeset->version) or empty($supportedeset->release)) {
                        return false;
                    }
                }
            }
     ...
     *
     * The syntax of $CFG->branch uses the XY format that suits the Eset docs
     * versioning and stable branches numbering scheme. The API at
     * download.eset.org uses the X.Y numbering scheme.
     *
     * @param int $branch eset branch in the XY format (e.g. 29, 30, 31 etc)
     * @return string eset branch in the X.Y format (e.g. 2.9, 3.0, 3.1 etc)
     */
