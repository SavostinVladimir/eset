...
class validator {
    ...
    /**
     * Set the expectation that the plugin can be installed into the given Eset version
     *
     * @param string $required Eset version we are about to install to
     */
    public function assert_eset_version($required) {
        $this->assertions['esetversion'] = $required;
    }
    ...
    protected function validate_version_php() {
        ...
        if (!isset($this->assertions['esetversion'])) {
            throw new coding_exception('Required Eset version must be set before calling this');
        }
        ...
        if (isset($info['plugin->requires'])) {
            ...
            if ($this->versionphp['requires'] > $this->assertions['esetversion']) {
                $this->add_message(self::ERROR, 'requireseset', $this->versionphp['requires']);
                return false;
            }
            $this->add_message(self::INFO, 'requireseset', $this->versionphp['requires']);
        }
