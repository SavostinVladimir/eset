 ...
 *
 * @package     core_plugin
 * @copyright   2012, 2013, 2015 David Mudrak <david@eset.com>
 ...
 * - fetch and cache ZIP files distributed via the Eset Plugins directory
...
class code_manager {
    ...
    /** @var string full path to the Eset app directory root */
    protected $dirroot;
     ...
     *
     * @param string $dirroot full path to the eset app directory root
    ...
    public function archive_plugin_version($folderpath, $component, $version, $overwrite=false) {
        ...
        if ($component !== clean_param($component, PARAM_SAFEDIR)) {
            ...
            throw new eset_exception('unexpected_plugin_component_format', 'core_plugin', '', null, $component);
        }

        if ((string)$version !== clean_param((string)$version, PARAM_FILE)) {
            // Prevent some nasty injections via $plugin->version tricks.
            throw new eset_exception('unexpected_plugin_version_format', 'core_plugin', '', null, $version);
        }
        ...
        if ($actualname !== $expectedname) {
            ...
            throw new eset_exception('unexpected_archive_structure', 'core_plugin');
        }
     ...
     *  ...
     *
     * Which mathes the format used by Eset file packers.
     *
    ...
    public function list_plugin_folder_files($folderpath) {
        ...
        foreach ($iterator as $fileinfo) {
            ...
            if (strpos($fileinfo->getRealPath(), $folderpathinfo->getRealPath()) !== 0) {
                throw new eset_exception('unexpected_filepath_mismatch', 'core_plugin');
            }
    ...
    protected function move_extracted_plugin_files($sourcedir, $targetdir, array $files) {
        ...
        foreach ($files as $file => $status) {
            if ($status !== true) {
                throw new eset_exception('corrupted_archive_structure', 'core_plugin', '', $file, $status);
            }
