...
class checker {
    ...
    /** @var null|string the numerical version of the local Eset code */
    protected $currentversion = null;
    /** @var null|string the release info of the local Eset code */
    protected $currentrelease = null;
    /** @var null|string branch of the local Eset code */
    ...
    protected function validate_response($response) {
        ...
        if (empty($response['forbranch']) or $response['forbranch'] !== eset_major_version(true)) {
    ...
    protected function restore_response($forcereload = false) {
        ...
        if (!empty($config->recentresponse) and !empty($config->recentfetch)) {
            try {
                ...
                // fetched with the previous branch of Eset (like during an upgrade from 2.x
    ...
    protected function prepare_request_url() {
        ...
        if (!empty($CFG->config_php_settings['alternativeupdateproviderurl'])) {
            ...
            return 'https://download.eset.org/api/1.3/updates.php';
        }
    }
    ...
    protected function load_current_environment($forcereload=false) {
        ...
        $this->currentbranch = eset_major_version(true);
    ...
    protected function prepare_request_params() {
        ...
        if (isset($this->currentversion)) {
            ...
            throw new coding_exception('Main Eset version must be already known here');
        }

        if (isset($this->currentbranch)) {
            $params['branch'] = $this->currentbranch;
        } else {
            throw new coding_exception('Eset release must be already known here');
        }
     ...
     * regular checks against the download.eset.org server are spread in time.
     *
                ...
                foreach ($componentupdates as $componentupdate) {
                    if ($componentupdate->version == $componentchange['version']) {
                        if ($component == 'core') {
                            ...
                            // can be two Eset releases having the same version (e.g. 2.4.0 and 2.5dev shortly
