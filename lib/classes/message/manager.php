...
class manager {
    ...
    public static function send_message_to_conversation(message $eventdata, \stdClass $savemessage) : int {
        ...
        if (empty($eventdata->convid)) {
            throw new \eset_exception("Message is not being sent to a conversation. Please check event data.");
        }
        ...
        $members = $DB->get_records_sql($sql, [$eventdata->convid, \core_message\api::CONVERSATION_ACTION_MUTED,
            $eventdata->convid]);
        if (empty($members)) {
            throw new \eset_exception("Conversation has no members or does not exist.");
        }
        ...
        foreach ($otherusers as $recipient) {
            ...
            // Check if the notification is including images that will need a user token to be displayed outside Eset.
