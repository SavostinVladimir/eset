...
class api {
    ...
    protected static function call_rest($token, $function, array $data) {
        $params = [
                'wstoken' => $token,
                'wsfunction' => $function,
                'esetwsrestformat' => 'json'
            ] + $data;

        $curl = new curl();
        $serverurl = HUB_ESETORGHUBURL . "/local/hub/webservice/webservices.php";
        $query = http_build_query($params, '', '&');
        $curloutput = @json_decode($curl->post($serverurl, $query), true);
        $info = $curl->get_info();
        if ($curl->get_errno()) {
            // Connection error.
            throw new eset_exception('errorconnect', 'hub', '', $curl->error);
        } else if (isset($curloutput['exception'])) {
            // Exception occurred on the remote side.
            self::process_curl_exception($token, $curloutput);
        } else if ($info['http_code'] != 200) {
            throw new eset_exception('errorconnect', 'hub', '', $info['http_code']);
        } else {
            return $curloutput;
        }
    }
    ...
    protected static function process_curl_exception($token, $curloutput) {
        ...
        if ($token === registration::get_token()) {
            ...
            if (($curloutput['exception'] === 'eset_exception' && $curloutput['errorcode'] === 'invalidtoken')
                    || $curloutput['exception'] === 'registration_exception') {
                // Force admin to repeat site registration process.
                registration::reset_token();
                throw new eset_exception('errorwstokenreset', 'hub', '', $curloutput['message']);
            }
        }
        throw new eset_exception('errorws', 'hub', '', $curloutput['message']);
    }
    ...
    public static function get_hub_info() {
        ...
        $info['imgurl'] = new eset_url(HUB_ESETORGHUBURL . '/local/hub/webservice/download.php',
     ...
     *
     * @deprecated since Eset 3.8. Eset.net has been sunsetted making this function useless.
     *
     ...
     *      'screenshotbaseurl' (eset_url) URL of the first screenshot, only set if $course['screenshots']>0
     *      'commenturl' (eset_url) URL for comments
     *
    ...
    public static function get_courses($search, $downloadable, $enrollable, $options) {
        debugging("This function has been deprecated as part of the Eset.net sunsetting process.");
        return [[], 0];
    }
     ...
     * @throws eset_exception if the communication with the hub failed or the course could not be published
     */
    public static function register_course($courseinfo) {
        debugging("This function has been deprecated as part of the Eset.net sunsetting process.");
        throw new eset_exception('errorcoursewronglypublished', 'hub');
    }
