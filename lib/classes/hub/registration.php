...
/**
 * Methods to use when registering the site at the eset sites directory.
 *
 * @package    core
 * @copyright  2017 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class registration {
    ...
    protected static function get_registration($confirmed = true) {
        ...
        if (self::$registration === null) {
            self::$registration = $DB->get_record('registration_hubs', ['huburl' => HUB_ESETORGHUBURL]) ?: null;
        }
    ...
    public static function require_registration() {
        ...
        if (has_capability('eset/site:config', context_system::instance())) {
            throw new eset_exception('registrationwarning', 'admin', new eset_url('/admin/registration/index.php'));
        } else {
            throw new eset_exception('registrationwarningcontactadmin', 'admin');
        }
    }
    ...
    public static function get_site_info($defaults = []) {
        ...
        // Version and url.
        $siteinfo['esetrelease'] = $CFG->release;
    ...
    public static function get_stats_summary($siteinfo) {
        ...
        $esetrelease = $siteinfo['esetrelease'];
        if (preg_match('/^(\d+\.\d.*?)[\. ]/', $esetrelease, $matches)) {
            $esetrelease = $matches[1];
        }
        $senddata = [
            'esetrelease' => get_string('sitereleasenum', 'hub', $esetrelease),
    ...
    public static function confirm_registration($token, $newtoken, $hubname) {
        ...
        if (!$registration || $registration->token !== $token) {
            throw new eset_exception('wrongtoken', 'hub', new eset_url('/admin/registration/index.php'));
        }
    ...
    public static function register($returnurl) {
        ...
        // We will also update registration after we receive confirmation from eset.net.
        $url = new eset_url(HUB_ESETORGHUBURL . '/local/hub/siteregistration.php',
    ...
    public static function reset_site_identifier($token) {
        ...
        $registration = self::get_registration(false);
        if (!$registration || $registration->token != $token) {
            throw new eset_exception('wrongtoken', 'hub',
     ...
     * {
     *     "courses": 384,
     *     "description": "Official eset sites directory",
     ...
     *     "name": "eset",
     *     "sites": 274175,
     *     "url": "https://stats.eset.org",
     *     "imgurl": "https://stats.eset.org/local/hub/webservice/download.php?filetype=hubscreenshot"
     * }
     *
     * @return array|null
     */
    public static function get_esetnet_info() {
        try {
    ...
    public static function registration_reminder($url) {
        ...
        if (self::show_after_install() || self::get_new_registration_fields()) {
            $returnurl = new eset_url($url);
            redirect(new eset_url('/admin/registration/index.php', ['returnurl' => $returnurl->out_as_local_url(false)]));
        }
    }
}
