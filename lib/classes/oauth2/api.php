...
class api {
    ...
    public static function init_standard_issuer($type) {
        ...
        if ($type == 'google') {
            ...
            throw new eset_exception('OAuth 2 service type not recognised: ' . $type);
        }
    }
    ...
    public static function create_standard_issuer($type, $baseurl = false) {
        ...
        if ($type == 'google') {
            ...
            if (!$baseurl) {
                throw new eset_exception('Nextcloud service type requires the baseurl parameter.');
            }
     ...
     * @throws eset_exception Request for token upgrade failed for technical reasons
     */
     ...
     * @param eset_url $currenturl The url to the current page.
     * @param string $additionalscopes The additional scopes required for authorization.
     * @return \core\oauth2\client
     */
    public static function get_user_oauth_client(issuer $issuer, eset_url $currenturl, $additionalscopes = '') {
    ...
    /**
     * Get the list of defined mapping from OAuth user fields to eset user fields.
     *
     * @param \core\oauth2\issuer $issuer The desired OAuth issuer
     * @return \core\oauth2\user_field_mapping[]
     */
    ...
    protected static function discover_endpoints($issuer) {
        ...
        if (!$json = $curl->get($url)) {
            ...
            throw new eset_exception($msg);
        }

        if ($msg = $curl->error) {
            throw new eset_exception('Could not discover service endpoints: ' . $msg);
        }
     ...
     *
     * Requires eset/site:config capability at the system context.
     *
     ...
     *
     * @param \core\oauth2\issuer $issuer
     * @param eset_url $returnurl The url to the current page (we will be redirected back here after authentication).
     * @return boolean
     */
