 ...
 *   -it defines the necessary objects and interfaces to build the Eset
 *    admin hierarchy
 *   -it defines the admin_externalpage_setup()
 *
 *  ADMIN_SETTING OBJECTS
 *
 *  Eset settings are represented by objects that inherit from the admin_setting
 ...
 *  appear in the Eset admin tree block. All interaction with admin_settingpage
 *  objects is handled by the admin/settings.php file.
 *
 *  ADMIN_EXTERNALPAGE OBJECTS
 *
 *  There are some settings in Eset that are too complex to (efficiently) handle
...
/**
 * Returns names of all known tables == tables that eset knows about.
 *
...
function is_dataroot_insecure($fetchtest=false) {
    ...
    foreach($rp as $r) {
        if (strpos($siteroot, '/'.$r.'/') === 0) {
            $siteroot = substr($siteroot, strlen($r)+1); // eset web in subdirectory
        } else {
...
class admin_externalpage implements part_of_admin_tree {
     ...
     * @param mixed $req_capability The role capability/permission a user must have to access this external page. Defaults to 'eset/site:config'.
     ...
     */
    public function __construct($name, $visiblename, $url, $req_capability='eset/site:config', $hidden=false, $context=NULL) {
        $this->name        = $name;
        $this->visiblename = $visiblename;
        $this->url         = $url;
...
class admin_settingpage implements part_of_admin_tree {
     ...
     *      if you specify something other than system or front page. Defaults to system.
     */
    public function __construct($name, $visiblename, $req_capability='eset/site:config', $hidden=false, $context=NULL) {
...
abstract class admin_setting {
    ...
    private function parse_setting_name($name) {
        $bits = explode('/', $name);
        if (count($bits) > 2) {
            throw new eset_exception('invalidadminsettingname', '', '', $name);
        }
    ...
    public function get_defaultsetting() {
        ...
        if (!empty($adminroot->custom_defaults)) {
            $plugin = is_null($this->plugin) ? 'eset' : $this->plugin;
        ...
        if ($parentvalidation === true) {
            if ($this->maxlength > 0) {
                ...
                if ($length > $this->maxlength) {
                    return get_string('maximumchars', 'eset',  $this->maxlength);
                }
                return true;
            } else {
 ...
 * get_users_from_config($CFG->mysetting, $capability); function in esetlib.php.
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class admin_setting_emoticons extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $context->reseturl = new eset_url('/admin/resetemoticons.php');
...
class admin_settings_num_course_sections extends admin_setting_configselect {
    ...
    /** Lazy-load the available choices for the select box */
    public function load_choices() {
        $max = get_config('esetcourse', 'maxsections');
 ...
 *
 * @copyright 2015 Adrian Greeve <adrian@eset.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class admin_setting_manageenrols extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $url = new eset_url('/admin/enrol.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allenrols as $enrol => $unused) {
            ...
            if (isset($active_enrols[$enrol])) {
                $aurl = new eset_url($url, array('action'=>'disable', 'enrol'=>$enrol));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= $OUTPUT->pix_icon('t/hide', $strdisable) . '</a>';
                $enabled = true;
                $displayname = $name;
            } else if (isset($enrols_available[$enrol])) {
                $aurl = new eset_url($url, array('action'=>'enable', 'enrol'=>$enrol));
                $hideshow = "<a href=\"$aurl\">";
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'enrol_' . $name, false)) {
                ...
                $icon = $OUTPUT->pix_icon('spacer', '', 'eset', array('class' => 'icon pluginicon noicon'));
            }
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new eset_url($url, array('action'=>'up', 'enrol'=>$enrol));
                    $updown .= "<a href=\"$aurl\">";
                ...
                if ($updowncount < $enrolcount) {
                    $aurl = new eset_url($url, array('action'=>'down', 'enrol'=>$enrol));
                    $updown .= "<a href=\"$aurl\">";
            ...
            if (!empty($enrols_available[$enrol]) and method_exists($enrols_available[$enrol], 'test_settings')) {
                $testsettingsurl = new eset_url('/enrol/test_settings.php', array('enrol'=>$enrol, 'sesskey'=>sesskey()));
...
class admin_page_managemessageoutputs extends admin_externalpage {
    ...
    public function __construct() {
        ...
        parent::__construct('managemessageoutputs',
            ...
            new eset_url('/admin/message.php')
        );
    }
 ...
 *
 * @deprecated since Eset 3.7 MDL-64495. Please use admin_page_managemessageoutputs instead.
 * @todo       MDL-64866 This will be deleted in Eset 3.11.
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_page_defaultmessageoutputs extends admin_page_managemessageoutputs {
    ...
    public function __construct() {
        ...
        admin_externalpage::__construct('defaultmessageoutputs', get_string('defaultmessageoutputs', 'message'), new eset_url('/message/defaultoutputs.php'));
    }
}
...
class admin_page_manageqbehaviours extends admin_externalpage {
    ...
    public function __construct() {
        ...
        parent::__construct('manageqbehaviours', get_string('manageqbehaviours', 'admin'),
                new eset_url('/admin/qbehaviours.php'));
    }
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($displayauths as $auth => $name) {
            ...
            if (!empty($authplugins[$auth]) and method_exists($authplugins[$auth], 'test_settings')) {
                $testurl = new eset_url('/auth/test_settings.php', array('auth'=>$auth, 'sesskey'=>sesskey()));
    ...
    public function output_html($data, $query='') {
        ...
        // hide/show link
            ...
            if (file_exists($CFG->dirroot.'/lib/editor/'.$editor.'/settings.php')) {
                $eurl = new eset_url('/admin/settings.php', array('section'=>'editorsettings'.$editor));
...
class admin_setting_manageantiviruses extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $baseurl = new eset_url('/admin/antiviruses.php', array('sesskey' => sesskey()));
        foreach ($antivirusesavailable as $antivirus => $name) {
            ...
            if (file_exists($CFG->dirroot.'/lib/antivirus/'.$antivirus.'/settings.php')) {
                $eurl = new eset_url('/admin/settings.php', array('section' => 'antivirussettings'.$antivirus));
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @deprecated since Eset 3.9 MDL-45184. Please use \tool_licensemanager\manager instead.
 * @todo MDL-45184 This class will be deleted in Eset 4.1.
 */
class admin_setting_managelicenses extends admin_setting {
     ...
     * Always returns true, does nothing
     *
     * @deprecated since Eset 3.9 MDL-45184.
     * @todo MDL-45184 This method will be deleted in Eset 4.1
     *
     * @return true
     */
    public function get_setting() {
...
class admin_setting_manageformats extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $cnt = 0;
        $defaultformat = get_config('esetcourse', 'format');
        $spacer = $OUTPUT->pix_icon('spacer', '', 'eset', array('class' => 'iconsmall'));
        foreach ($formats as $format) {
            $url = new eset_url('/admin/courseformats.php',
            ...
            if ($format->is_enabled()) {
                ...
                if ($defaultformat === $format->name) {
                    ...
                    $hideshow = html_writer::link($url->out(false, array('action' => 'disable')),
                            $OUTPUT->pix_icon('t/hide', $txt->disable, 'eset', array('class' => 'iconsmall')));
                }
            } else {
                $strformatname = $format->displayname;
                $class = 'dimmed_text';
                $hideshow = html_writer::link($url->out(false, array('action' => 'enable')),
                    $OUTPUT->pix_icon('t/show', $txt->enable, 'eset', array('class' => 'iconsmall')));
            }
            $updown = '';
            if ($cnt) {
                $updown .= html_writer::link($url->out(false, array('action' => 'up')),
                    $OUTPUT->pix_icon('t/up', $txt->up, 'eset', array('class' => 'iconsmall'))). '';
            } else {
                $updown .= $spacer;
            }
            if ($cnt < count($formats) - 1) {
                $updown .= '&nbsp;'.html_writer::link($url->out(false, array('action' => 'down')),
                    $OUTPUT->pix_icon('t/down', $txt->down, 'eset', array('class' => 'iconsmall')));
            } else {
                $updown .= $spacer;
            }
            $cnt++;
        ...
        $return .= html_writer::table($table);
        $link = html_writer::link(new eset_url('/admin/settings.php', array('section' => 'coursesettings')), new lang_string('coursesettings'));
...
class admin_setting_managecustomfields extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($fields as $field) {
            $url = new eset_url('/admin/customfields.php',
...
class admin_setting_managedataformats extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($formats as $format) {
            ...
            $url = new eset_url('/admin/dataformats.php',
...
abstract class admin_setting_manage_plugins extends admin_setting {
    ...
    protected function get_manage_url() {
        return new eset_url('/admin/updatesetting.php');
    }
    ...
    public function output_html($data, $query = '') {
        ...
        $context = (object) [
            'manageurl' => new eset_url($this->get_manage_url(), [
        ...
        foreach ($plugins as $key => $plugin) {
            $pluginlink = new eset_url($context->manageurl, ['plugin' => $key]);
            ...
            // Enable/Disable link.
            $togglelink = new eset_url($pluginlink);
            if ($plugin->is_enabled()) {
                ...
                if (count($context->plugins)) {
                    ...
                    $pluginkey->moveuplink = new eset_url($pluginlink, ['action' => 'up']);
                }

                if (count($enabled) > count($context->plugins) + 1) {
                    // This is not the last plugin.
                    $pluginkey->movedownlink = new eset_url($pluginlink, ['action' => 'down']);
                }
...
class admin_setting_managemediaplayers extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $updowncount = 1;
        $url = new eset_url('/admin/media.php', array('sesskey' => sesskey()));
        $printed = array();
        $spacer = $OUTPUT->pix_icon('spacer', '', 'eset', array('class' => 'iconsmall'));
        ...
        foreach ($plugins as $name => $plugin) {
            ...
            if (!$plugininfo->is_installed_and_upgraded()) {
                ...
                if ($enabled) {
                    $hideshow = html_writer::link(new eset_url($url, array('action' => 'disable')),
                        $OUTPUT->pix_icon('t/hide', $strdisable, 'eset', array('class' => 'iconsmall')));
                } else {
                    $hideshow = html_writer::link(new eset_url($url, array('action' => 'enable')),
                        $OUTPUT->pix_icon('t/show', $strenable, 'eset', array('class' => 'iconsmall')));
                    $class = 'dimmed_text';
                }
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    $updown = html_writer::link(new eset_url($url, array('action' => 'up')),
                        $OUTPUT->pix_icon('t/up', $strup, 'eset', array('class' => 'iconsmall')));
                } else {
                    $updown = $spacer;
                }
                if ($updowncount < count($enabledplugins)) {
                    $updown .= html_writer::link(new eset_url($url, array('action' => 'down')),
                        $OUTPUT->pix_icon('t/down', $strdown, 'eset', array('class' => 'iconsmall')));
                } else {
                    $updown .= $spacer;
                }
                ++$updowncount;
 ...
 *
 * @copyright  2020 Amaia Anabitarte <amaia@eset.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class admin_setting_managecontentbankcontenttypes extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($types as $type) {
            $url = new eset_url('/admin/contentbank.php',
...
function admin_externalpage_setup($section, $extrabutton = '', array $extraurlparams = null, $actualurl = '', array $options = array()) {
    ...
    if ($PAGE->user_allowed_editing()) {
        if ($PAGE->user_is_editing()) {
            ...
            $url = new eset_url($PAGE->url, array('adminedit'=>'0', 'sesskey'=>sesskey()));
        } else {
            $caption = get_string('blocksediton');
            $url = new eset_url($PAGE->url, array('adminedit'=>'1', 'sesskey'=>sesskey()));
        }
...
function admin_search_settings_html($query) {
    ...
    foreach ($findings as $found) {
        ...
        if ($page instanceof admin_externalpage) {
            $headingurl = new eset_url($page->url);
        } else if ($page instanceof admin_settingpage) {
            $headingurl = new eset_url('/admin/settings.php', ['section' => $page->name]);
        } else {
            continue;
        }
...
/** @var string */
    ...
    /**
     * Helper function that generates a eset_url object
     * relevant to the repository
     */

    function repository_action_url($repository) {
        return new eset_url($this->baseurl, array('sesskey'=>sesskey(), 'repos'=>$repository));
    }
        ...
        if (!empty($repositorytypes)) {
            ...
            foreach ($repositorytypes as $i) {
                ...
                if (!empty($typeoptionnames) || !empty($instanceoptionnames)) {
                    // Calculate number of instances in order to display them for the Eset administrator
...
class admin_setting_enablemobileservice extends admin_setting_configcheckbox {
    ...
    public function get_setting() {
        ...
        $mobileservice = $webservicemanager->get_external_service_by_shortname(ESET_OFFICIAL_MOBILE_SERVICE);
    ...
    public function write_setting($data) {
        ...
        $servicename = ESET_OFFICIAL_MOBILE_SERVICE;
        ...
        if ((string)$data === $this->yes) {
             ...
             $otherenabledservices = $DB->get_records_select('external_services',
                     'enabled = :enabled AND (shortname != :shortname OR shortname IS NULL)', array('enabled' => 1,
                         'shortname' => ESET_OFFICIAL_MOBILE_SERVICE));
...
class admin_setting_webservicesoverview extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        /// One system controlling Eset with Token
        ...
        $row = array();
        $url = new eset_url("/admin/search.php?query=enablewebservices");
        ...
        /// 2. Enable protocols
        $row = array();
        $url = new eset_url("/admin/settings.php?section=webserviceprotocols");
        ...
        $row = array();
        $url = new eset_url("/user/editadvanced.php?id=-1");
        ...
        $row = array();
        $url = new eset_url("/admin/roles/check.php?contextid=1");
        ...
        $row = array();
        $url = new eset_url("/admin/settings.php?section=externalservices");
        ...
        $row = array();
        $url = new eset_url("/admin/webservice/tokens.php?sesskey=" . sesskey() . "&action=create");
        ...
        $row = array();
        $url = new eset_url("/admin/search.php?query=enablewsdocumentation");
        ...
        /// 10. Test the service
        $row = array();
        $url = new eset_url("/admin/webservice/testclient.php");
...
class admin_setting_managewebservicetokens extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $baseurl = new eset_url('/' . $CFG->admin . '/settings.php?section=webservicetokens');

        $return = $OUTPUT->box_start('generalbox webservicestokenui');

        if (has_capability('eset/webservice:managealltokens', context_system::instance())) {
...
class admin_setting_configcolourpicker extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        $icon = new pix_icon('i/loading', get_string('loading', 'admin'), 'eset', ['class' => 'loadingicon']);
        $context = (object) [
            'id' => $this->get_id(),
            'name' => $this->get_full_name(),
            'value' => $data,
...
class admin_setting_searchsetupinfo extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $row = array();
        $url = new eset_url('/admin/settings.php?section=manageglobalsearch#admin-searchengine');
        ...
        // Available areas.
        $row = array();
        $url = new eset_url('/admin/searchareas.php');
        ...
        if (empty($CFG->searchengine)) {
            ...
            $url = new eset_url('/admin/settings.php?section=search' . $CFG->searchengine);
        ...
        $row = array();
        $url = new eset_url("/admin/search.php?query=enableglobalsearch");
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2016 Dan Poltawski <dan@eset.com>
 */
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2018 Mihail Geshoski <mihail@eset.com>
 */
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2019 Bas Brands <bas@eset.com>
 */
