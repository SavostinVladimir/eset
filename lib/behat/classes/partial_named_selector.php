...
class behat_partial_named_selector extends \Behat\Mink\Selector\PartialNamedSelector {
    ...
    public function __construct() {
        ...
        foreach (static::$esetselectors as $name => $xpath) {
     ...
     * will be replaced by the locator. These are Eset's own xpaths.
     *
     * @var array XPaths for eset elements.
     */
    protected static $esetselectors = array(
        'activity' => <<<XPATH
...
XPATH
        , 'dialogue' => <<<XPATH
.//div[contains(concat(' ', normalize-space(@class), ' '), ' eset-dialogue ') and
    normalize-space(descendant::div[
        contains(concat(' ', normalize-space(@class), ' '), ' eset-dialogue-hd ')
        ]) = %locator%] |
...
XPATH
    ...
    protected static $customreplacements = [
        ...
        '%ariaLabelMatch%' => [
            'eset' => 'contains(./@aria-label, %locator%)',
        ],
    ];
