...
class behat_field_manager {
    ...
    public static function get_form_field_from_label($label, RawMinkContext $context) {
        ...
        // There are eset form elements that are not directly related with
     ...
     *
     * Not all the fields are part of a eset form, in this
    ...
    public static function get_form_field(NodeElement $fieldnode, Session $session) {
        ...
        // Get the field type if is part of a esetform.
        if (self::is_esetform_field($fieldnode)) {
        ...
        // If is not a esetforms field use the base field type.
        if (empty($type)) {
            $type = 'field';
        }
    ...
    public static function get_field_instance($type, NodeElement $fieldnode, Session $session) {
        ...
        // If the field is not part of a esetform, we should still try to find out
     ...
     * esetform-specific elements have been detected.
     *
     * @param NodeElement $fieldnode
    ...
    /**
     * Detects when the field is a esetform field type.
     *
     * Note that there are fields inside esetforms that are not
     * esetform element; this method can not detect this, this will
     ...
     * @return bool
     */
    protected static function is_esetform_field(NodeElement $fieldnode) {

        // We already waited when getting the NodeElement and we don't want an exception if it's not part of a esetform.
    ...
    protected static function get_field_node_type(NodeElement $fieldnode, Session $session) {
        ...
        if ($class = $parentnode->getAttribute('class')) {
            ...
            // Stop propagation through the DOM, if it does not have a felement is not part of a eset form.
     ...
     * the value of form elements.
     *
     * @deprecated since Eset 2.6 MDL-39634 - please do not use this function any more.
     * @todo MDL-XXXXX This will be deleted in Eset 2.8
