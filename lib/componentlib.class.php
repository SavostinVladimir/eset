 ...
 * download and install feature of Eset, used to keep updated some
 * items like languages, pear, enviroment... i.e, components.
 *
 * It has been developed harcoding some important limits that are
 * explained below:
 *    - It only can check, download and install items under esetdata.
 ...
 *     if ($cd = new component_installer('https://download.eset.org', 'langpack/2.0',
 ...
 *                     $a = new stdClass();
 *                     $a->url = 'https://download.eset.org/langpack/2.0/es.zip';
 ...
 * download.eset.org to the esetdata directory.
 *
...
class component_installer {
    ...
    var $destpath;     /// Relative path (from esetdata) where the .zip
     ...
     *               than zipfilename)
     * @param string $destpath Relative path (from esetdata) where the .zip file will
     *               be expanded (default='' = esetdataitself)
     * @return object
     */
    ...
    /// Check for correct sourcebase (this will be out in the future)
        if (!PHPUNIT_TEST and $this->sourcebase != 'https://download.eset.org') {
...
class lang_installer {
    ...
    /** @var string this Eset major version */
    protected $version;
    ...
    public function __construct($langcode = '') {
        ...
        $this->version = eset_major_version(true);
    ...
    public function lang_pack_url($langcode = '') {
        ...
        if (empty($langcode)) {
            return 'https://download.eset.org/langpack/'.$this->version.'/';
        } else {
            return 'https://download.eset.org/download.php/langpack/'.$this->version.'/'.$langcode.'.zip';
        }
    }

    /**
     * Returns the list of available language packs from download.eset.org
     *
     * @return array|bool false if can not download
     */
    public function get_remote_list_of_languages() {
        $source = 'https://download.eset.org/langpack/' . $this->version . '/languages.md5';
    ...
    protected function install_language_pack($langcode) {
        ...
        $installer = new component_installer('https://download.eset.org', 'download.php/direct/langpack/' . $this->version,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class lang_installer_exception extends eset_exception {
