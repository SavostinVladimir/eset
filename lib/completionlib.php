...
function completion_can_view_data($userid, $course = null) {
    ...
    if (has_capability('eset/user:viewuseractivitiesreport', $personalcontext)) {
        return true;
...
class completion_info {
     ...
     *
     * @param stdClass $course Eset course object.
     */
     ...
     *
     * @deprecated since Eset 2.0 - Use display_help_icon instead.
     */
    ...
    /**
     * @deprecated since Eset 2.8 MDL-46290.
     */
    ...
    public function user_can_override_completion($user) {
        return has_capability('eset/course:overridecompletion', context_course::instance($this->course_id), $user);
    }
     ...
     * @return void
     * @throws eset_exception if trying to override without permission.
     */
        ...
        if ($override) {
            if (!$this->user_can_override_completion($USER)) {
                throw new required_capability_exception(context_course::instance($this->course_id),
                                                        'eset/course:overridecompletion', 'nopermission', '');
            }
        }
    ...
    public function is_tracked_user($userid) {
        return is_enrolled(context_course::instance($this->course->id), $userid, 'eset/course:isincompletionreports', true);
    }
    ...
    public function get_num_tracked_users($where = '', $whereparams = array(), $groupid = 0) {
        ...
        list($enrolledsql, $enrolledparams) = get_enrolled_sql(
                context_course::instance($this->course->id), 'eset/course:isincompletionreports', $groupid, true);
    ...
    public function get_tracked_users($where = '', $whereparams = array(), $groupid = 0,
        ...
        list($enrolledsql, $params) = get_enrolled_sql(
                ...
                'eset/course:isincompletionreports', $groupid, true);
     ...
     * @throws eset_exception Exception with the error string as debug info
     */
    public function internal_systemerror($error) {
        global $CFG;
        throw new eset_exception('err_system','completion',
