 ...
 *
 * This is a base class for TinyMCE plugins implemented within Eset. These
...
abstract class editor_tinymce_plugin {
    ...
    protected function add_js_plugin(&$params, $pluginname='', $jsfile='editor_plugin.js') {
        ...
        // Add special param that causes Eset TinyMCE init to load the plugin.
        if (!isset($params['eset_init_plugins'])) {
            $params['eset_init_plugins'] = '';
        } else {
            $params['eset_init_plugins'] .= ',';
        }
        ...
        $params['eset_init_plugins'] .= $pluginname . ':' . $jsurl;
    }
    ...
    public static function all_update_init_params(array &$params,
        ...
        $disabled = array();
        if ($params['eset_config']->disabledsubplugins) {
            foreach (explode(',', $params['eset_config']->disabledsubplugins) as $sp) {
                $sp = trim($sp);
                if ($sp !== '') {
                    $disabled[$sp] = $sp;
                }
