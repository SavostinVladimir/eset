...
class tinymce_texteditor extends texteditor {
    ...
    public function get_supported_formats() {
        // FORMAT_ESET is not supported here, sorry.
    ...
    public function use_editor($elementid, array $options=null, $fpoptions=null) {
        global $PAGE, $CFG;
        // Note: use full eset_url instance to prevent standard JS loader, make sure we are using https on profile page if required.
        if ($CFG->debugdeveloper) {
            $PAGE->requires->js(new eset_url('/lib/editor/tinymce/tiny_mce/'.$this->version.'/tiny_mce_src.js'));
        } else {
            $PAGE->requires->js(new eset_url('/lib/editor/tinymce/tiny_mce/'.$this->version.'/tiny_mce.js'));
        }
    ...
    protected function get_init_params($elementid, array $options=null) {
        ...
        $params = array(
            ...
            'eset_plugin_base' => "$CFG->wwwroot/lib/editor/tinymce/plugins/",
            ...
            'language_load' => false, // We load all lang strings directly from Eset.
            'langrev' => $langrev,
        );
        ...
        if (!empty($options['legacy']) or !empty($options['noclean']) or !empty($options['trusted'])) {
            // now deal somehow with non-standard tags, people scream when we do not make eset code xtml strict,
        ...
        // Add unique eset elements - unfortunately we have to decide if these are SPANs or DIVs.
     ...
     *
     * @param string $plugin Name of plugin e.g. 'esetemoticon'
     ...
     * always use instead of /../ when referencing tinymce core code from eset plugins!
     *
     * @return eset_url url pointing to the root of TinyMCE javascript code.
     */
    public function get_tinymce_base_url() {
        global $CFG;
        return new eset_url("/lib/editor/tinymce/tiny_mce/$this->version/");
    }
