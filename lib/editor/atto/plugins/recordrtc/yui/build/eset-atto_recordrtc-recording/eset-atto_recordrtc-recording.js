YUI.add('eset-atto_recordrtc-recording', function (Y, NAME) {
        ...
        cm.uploadBtn.on('click', function() {
            ...
            if (cm.chunks.length === 0) {
                ...
                cm.upload_to_server(cm.recType, function(progress, fileURLOrError) {
                    ...
                    } else if (progress === 'upload-failed-404') { // 404 error = File too large in Eset.
                        cm.uploadBtn.set('disabled', false);
...
M.atto_recordrtc.abstractmodule = {
    // A helper for making a Eset alert appear.
...
}, '@VERSION@', {"requires": ["eset-atto_recordrtc-button"]});
