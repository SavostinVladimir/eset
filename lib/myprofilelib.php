...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
        if (($iscurrentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('eset/user:update',
                    $systemcontext)) {
            $url = new eset_url('/user/editadvanced.php', array('id' => $user->id, 'course' => $courseid,
                'returnto' => 'profile'));
            ...
            $tree->add_node($node);
                   ...
                   || ($iscurrentuser && has_capability('eset/user:editownprofile', $systemcontext))) {
            ...
            if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                ...
                if (empty($url)) {
                    if (empty($course)) {
                        $url = new eset_url('/user/edit.php', array('id' => $user->id, 'returnto' => 'profile'));
                    } else {
    ...
    if (!$iscurrentuser && $PAGE->settingsnav->can_view_user_preferences($user->id)) {
        $url = new eset_url('/user/preferences.php', array('userid' => $user->id));
        $title = get_string('preferences', 'eset');
    ...
    if (!$user->deleted && !$iscurrentuser &&
                !\core\session\manager::is_loggedinas() && has_capability('eset/user:loginas',
                $courseorsystemcontext) && !is_siteadmin($user->id)) {
        $url = new eset_url('/course/loginas.php',
    ...
    // Contact details.
    if (has_capability('eset/user:viewhiddendetails', $courseorusercontext)) {
        $hiddenfields = array();
    } else {
        $hiddenfields = array_flip(explode(',', $CFG->hiddenuserfields));
    }
    $canviewuseridentity = has_capability('eset/site:viewuseridentity', $courseorusercontext);
    ...
    if ($iscurrentuser
        or (!isset($hiddenfields['email']) and (
            ...
            or has_capability('eset/course:useremail', $courseorusercontext) // TODO: Deprecate/remove for MDL-37479.
        ))
    ...
    if (!isset($hiddenfields['esetnetprofile']) && $user->esetnetprofile) {
        $node = new core_user\output\myprofile\node('contact', 'esetnetprofile', get_string('esetnetprofile', 'user'), null,
                null, $user->esetnetprofile);
        $tree->add_node($node);
    }
        ...
        if ($mycourses = enrol_get_all_users_courses($user->id, true, null)) {
            ...
            foreach ($mycourses as $mycourse) {
                if ($mycourse->category) {
                    ...
                    if (!isset($course) || $mycourse->id != $course->id) {
                        ...
                        if ($mycourse->visible == 0) {
                            if (!has_capability('eset/course:viewhiddencourses', $ccontext)) {
                                continue;
                            }
                        ...
                        $url = new eset_url('/user/view.php', $params);
                ...
                if (!$showallcourses && $shown == $CFG->navcourselimit) {
                    $url = null;
                    if (isset($course)) {
                        $url = new eset_url('/user/view.php',
    ...
    if (!empty($course)) {
        ...
        if (!isset($hiddenfields['groups']) && !empty($course)) {
            $accessallgroups = has_capability('eset/site:accessallgroups', $courseorsystemcontext);
    ...
    if ($user->icq && !isset($hiddenfields['icqnumber'])) {
        $imurl = new eset_url('http://web.icq.com/wwp', array('uin' => $user->icq) );
        $iconurl = new eset_url('http://web.icq.com/whitepages/online', array('icq' => $user->icq, 'img' => '5'));
    ...
    if ($user->skype && !isset($hiddenfields['skypeid'])) {
        ...
        $iconurl = new eset_url('http://mystatus.skype.com/smallicon/'.urlencode($user->skype));
        if (is_https()) {
    ...
    if ($user->yahoo && !isset($hiddenfields['yahooid'])) {
        $imurl = new eset_url('http://edit.yahoo.com/config/send_webmesg', array('.target' => $user->yahoo, '.src' => 'pg'));
        $iconurl = new eset_url('http://opi.yahoo.com/online', array('u' => $user->yahoo, 'm' => 'g', 't' => '0'));
        $statusicon = html_writer::tag('img', '',
    ...
    // Last ip.
    if (has_capability('eset/user:viewlastip', $usercontext) && !isset($hiddenfields['lastip'])) {
        if ($user->lastip) {
            $iplookupurl = new eset_url('/iplookup/index.php', array('ip' => $user->lastip, 'user' => $user->id));
