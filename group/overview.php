...
$url = new eset_url('/group/overview.php', array('id'=>$courseid));
if ($groupid !== 0) {
...
$popupurl = new eset_url($rooturl.'&group='.$groupid);
...
$popupurl = new eset_url($rooturl.'&grouping='.$groupingid);
...
foreach ($members as $gpgid=>$groupdata) {
    ...
    foreach ($groupdata as $gpid=>$users) {
        ...
        foreach ($users as $user) {
            ...
            $fullnames[] = html_writer::link(new eset_url('/user/view.php', ['id' => $user->id, 'course' => $course->id]),
                $displayname);
        }
...
if (count($hoverevents)>0) {
    $PAGE->requires->string_for_js('description', 'eset');
