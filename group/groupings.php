...
navigation_node::override_active_url(new eset_url('/group/index.php', array('id'=>$courseid)));
...
if ($groupings = $DB->get_records('groupings', array('courseid'=>$course->id), 'name')) {
    $canchangeidnumber = has_capability('eset/course:changeidnumber', $context);
    ...
    foreach($groupings as $grouping) {
        ...
        $url = new eset_url('/group/grouping.php', array('id' => $grouping->id));
        ...
        if (empty($grouping->idnumber) || $canchangeidnumber) {
            ...
            $url = new eset_url('/group/grouping.php', array('id' => $grouping->id, 'delete' => 1));
        ...
        $url = new eset_url('/group/assign.php', array('id' => $grouping->id));
...
echo $OUTPUT->single_button(new eset_url('grouping.php', array('courseid'=>$courseid)), $srtnewgrouping);
