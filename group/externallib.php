...
class core_group_external extends external_api {
    ...
    public static function delete_group_members($members) {
        ...
        foreach ($params['members'] as $member) {
            ...
            if (!groups_remove_member_allowed($group, $user)) {
                ...
                throw new eset_exception('errorremovenotpermitted', 'group', '', $fullname);
            }
    ...
    public static function get_course_user_groups($courseid = 0, $userid = 0, $groupingid = 0) {
        ...
        foreach ($courses as $course) {
            ...
            if ($userid != $USER->id && !has_capability('eset/course:managegroups', $course->context)) {
                $warnings[] = array(
                    'item' => 'course',
                    'itemid' => $course->id,
    ...
    public static function get_activity_allowed_groups($cmid, $userid = 0) {
        ...
        core_user::require_active_user($user);
         ...
         // Check if we have permissions for retrieve the information.
        if ($user->id != $USER->id) {
            if (!has_capability('eset/course:managegroups', $context)) {
        ...
        $results = array(
            'groups' => $usergroups,
            'canaccessallgroups' => has_capability('eset/site:accessallgroups', $context, $user),
            'warnings' => $warnings
        );
        return $results;
    }
