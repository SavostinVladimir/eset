...
$url = new eset_url('/group/grouping.php');
if ($id) {
    $url->param('id', $id);
...
navigation_node::override_active_url(new eset_url('/group/index.php', array('id' => $course->id)));

$returnurl = $CFG->wwwroot.'/group/groupings.php?id='.$course->id;

if ($id and $delete) {
    if (!empty($grouping->idnumber) && !has_capability('eset/course:changeidnumber', $context)) {
    ...
    if (!$confirm) {
        ...
        $formcontinue = new single_button(new eset_url('grouping.php', $optionsyes), get_string('yes'), 'get');
        $formcancel = new single_button(new eset_url('groupings.php', $optionsno), get_string('no'), 'get');
...
if ($editform->is_cancelled()) {
    ...
    $success = true;
    if (!has_capability('eset/course:changeidnumber', $context)) {
...
$PAGE->navbar->add($strparticipants, new eset_url('/user/index.php', array('id'=>$courseid)));
$PAGE->navbar->add($strgroupings, new eset_url('/group/groupings.php', array('id'=>$courseid)));
