 ...
 *
 * @package    core_group
 * @copyright  2017 Adrian Greeve <adrian@eset.com>
...
class group_details implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!empty($this->group->description) || (!empty($this->group->picture) && empty($this->group->hidepicture))) {
            ...
            $descriptionformat = $this->group->descriptionformat ?? FORMAT_ESET;
            $options = [
                'overflowdiv' => true,
                'context'     => $context
            ];
            ...
            if (has_capability('eset/course:managegroups', $context)) {
                $url = new eset_url('/group/group.php', ['id' => $this->group->id, 'courseid' => $this->group->courseid]);
