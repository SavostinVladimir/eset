...
See also http://docs.eset.org/dev/Local_customisation for more
information.
...
* applications that extend eset at the system level (hub server, amos server, etc.)
...
version tells Eset to invalidate all caches, do db upgrades if necessary,
...
$plugin->requires = 2010021900;   // Requires this Eset version
...
NOTE: events are not yet fully implemented in current Eset 2.0dev.
...
from /local/nicehack/db/services.php and are automatically installed/updated in Eset.
...
<?php
$defaults['eset']['forcelogin'] = 1;  // new default for $CFG->forcelogin
$defaults['scorm']['maxgrade'] = 20;    // default for get_config('scorm', 'maxgrade')
$defaults['esetcourse']['numsections'] = 11;
$defaults['eset']['hiddenuserfields'] = array('city', 'country');
...
---------------------------------
Eset supports other type of local customisation of standard language
...
esetdata/lang/en_local
<?php
  $string['login'] = 'Sign in';

See also http://docs.eset.org/en/Language_editing
...
Examples:
* disable one specific eset page without code modification
