...
class workshop_comments_strategy implements workshop_strategy {
     ...
     *
     * @param eset_url $actionurl URL of form handler, defaults to auto detect the current url
     * @param string $mode          Mode to open the form in: preview/assessment
     * @param stdClass $assessment  The current assessment
     * @param bool $editable
     * @param array $options
     */
    public function get_assessment_form(eset_url $actionurl=null, $mode='preview', stdclass $assessment=null, $editable=true, $options=array()) {
        global $CFG;    // needed because the included files use it
        global $PAGE;
        global $DB;
    ...
    public function save_assessment(stdclass $assessment, stdclass $data) {
        ...
        for ($i = 0; $i < $data->nodims; $i++) {
            ...
            $grade->peercommentformat = FORMAT_ESET;
            if (empty($grade->id)) {
                // new grade
