 ...
 */
class eset1_mod_workshop_handler extends eset1_mod_handler {
    ...
    /** @var eset1_file_manager for the current workshop */
    protected $fileman = null;

    /** @var eset1_inforef_manager */
    protected $inforefman = null;

    /** @var array list of eset1_workshopform_handler instances */
     ...
     *
     * Note that the path /ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP does not
    ...
    public function get_paths() {
        return array(
            new convert_path('workshop', '/ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP'),
            new convert_path('workshop_elements', '/ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS'),
            new convert_path(
                'workshop_element', '/ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT',
                array(
                    'dropfields' => array(
                        'stddev',
                        'totalassessments',
            ...
            new convert_path('workshop_element_rubric', '/ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT/RUBRICS/RUBRIC'),
        );
    }

    /**
     * This is executed every time we have one /ESET_BACKUP/COURSE/MODULES/MOD/WORKSHOP
     * data available
     */
    public function process_workshop($data, $raw) {
        ...
        $this->currentworkshop['intro'] = eset1_converter::migrate_referenced_files($this->currentworkshop['intro'], $this->fileman);
    ...
    /**
     * Processes one <ELEMENT> tag from eset.xml
     */
    ...
    /**
     * Processes one <RUBRIC> tag from eset.xml
     */
     ...
     * @return eset1_workshopform_handler the instance of the handler
     */
    protected function get_strategy_handler($strategy) {
        ...
        if (is_null($this->strategyhandlers)) {
            ...
            foreach ($subplugins as $name => $dir) {
                ...
                $handlerclass = "eset1_workshopform_{$name}_handler";
                ...
                if (!$this->strategyhandlers[$name] instanceof eset1_workshopform_handler) {
                    throw new eset1_convert_exception('wrong_handler_class', get_class($this->strategyhandlers[$name]));
                }
            }
        }

        if (!isset($this->strategyhandlers[$strategy])) {
            throw new eset1_convert_exception('usupported_subplugin', 'workshopform_'.$strategy);
        }
 ...
 */
abstract class eset1_workshopform_handler extends eset1_submod_handler {

    /**
     * @param eset1_mod_handler $workshophandler the handler of a module we are subplugin of
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(eset1_mod_handler $workshophandler, $subpluginname) {
