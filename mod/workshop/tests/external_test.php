...
class mod_workshop_external_testcase extends externallib_advanced_testcase {
    ...
    public function setUp() {
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $data['description__idx_'.$i.'_editor'] = array('text' => "Content $i", 'format' => FORMAT_ESET);
    ...
    public function test_mod_workshop_get_workshops_by_courses() {
        ...
        // Execute real Eset enrolment as we'll call unenrol() method on the instance later.
    ...
    public function test_view_workshop_user_student() {
        ...
        $esetworkshop = new \eset_url('/mod/workshop/view.php', array('id' => $this->cm->id));
        $this->assertEquals($esetworkshop, $event->get_url());
    ...
    public function test_add_submission() {
        ...
        $result = mod_workshop_external::add_submission($this->workshop->id, $title, $content, FORMAT_ESET, $draftidinlineattach,
            $draftidattach);
    ...
    public function test_add_submission_already_added() {
        ...
        $result = mod_workshop_external::add_submission($this->workshop->id, 'My submission', '', FORMAT_ESET, 0, $draftidattach);
        $result = external_api::clean_returnvalue(mod_workshop_external::add_submission_returns(), $result);
    ...
    public function test_update_submission() {
        ...
        $result = mod_workshop_external::update_submission($submissionid, $title, $content, FORMAT_ESET, $draftidinlineattach,
            $draftidattach);
    ...
    public function test_get_submissions_from_student_with_feedback_from_teacher() {
        ...
        $record->feedbackauthorformat = FORMAT_ESET;
        $record->published = 1;
    ...
    public function test_view_submission() {
        ...
        $this->assertEquals($this->context, $event->get_context());
        $esetworkshop = new \eset_url('/mod/workshop/submission.php', array('id' => $firstsubmissionid,
            'cmid' => $this->cm->id));
        $this->assertEquals($esetworkshop, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
