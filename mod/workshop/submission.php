...
if ($deletable and $delete) {
    ...
    echo $output->confirm($prompt, new eset_url($PAGE->url, ['delete' => 1, 'confirm' => 1]), $workshop->view_url());
}
...
if (!$delete) {
    ...
    if ($editable) {
        if ($submission->id) {
            $btnurl = new eset_url($PAGE->url, array('edit' => 'on', 'id' => $submission->id));
            $btntxt = get_string('editsubmission', 'workshop');
        } else {
            $btnurl = new eset_url($PAGE->url, array('edit' => 'on'));
    ...
    if ($submission->id and $deletable) {
        $url = new eset_url($PAGE->url, array('delete' => 1));
    ...
    if ($submission->id and !$edit and !$isreviewer and $canallocate and $workshop->assessing_allowed($USER->id)) {
        $url = new eset_url($PAGE->url, array('assess' => 1));
