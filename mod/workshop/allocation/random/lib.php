...
class workshop_random_allocator implements workshop_allocator {
    ...
    public function ui() {
        ...
        // the nasty hack follows to bypass the sad fact that eset quickforms do not allow to actually
    ...
    protected function add_new_allocations(array $newallocations, array $dataauthors, array $datareviewers) {
        ...
        foreach ($newallocations as $newallocation) {
            ...
            if (!isset($submissions[$authorid])) {
                throw new eset_exception('unabletoallocateauthorwithoutsubmission', 'workshop');
            }
        ...
        if (is_array($submissions)) {
            foreach ($submissions as $submissionid => $submission) {
                if (isset($byauthor[$submission->authorid])) {
                    throw new eset_exception('moresubmissionsbyauthor', 'workshop');
                }
     ...
     *
     * The algorithm of this function has been described at http://eset.org/mod/forum/discuss.php?d=128473
    ...
    protected function random_allocation($authors, $reviewers, $assessments, $result, array $options) {
        ...
        if (workshop_random_allocator_setting::NUMPER_SUBMISSION == $numper) {
            ...
            throw new eset_exception('unknownusertypepassed', 'workshop');
        }
