...
class mod_workshop_renderer extends plugin_renderer_base {
    ...
    protected function render_workshop_user_plan(workshop_user_plan $plan) {
        ...
        foreach ($plan->phases as $phasecode => $phase) {
            ...
            if ($phase->active) {
                ...
                $icon = $this->output->pix_icon('i/marked', '', 'eset', ['role' => 'presentation']);
    ...
    protected function render_workshop_assessment(workshop_assessment $assessment) {
        ...
        if (($assessment->url instanceof eset_url) and ($this->page->url != $assessment->url)) {
        ...
        if (!is_null($assessment->form)) {
            ...
            $o .= $this->output->container(self::esetform($assessment->form), 'assessment-form');
    ...
    protected function helper_submission_attachments($submissionid, $format = 'html') {
        ...
        foreach ($files as $file) {
            ...
            $fileurl    = eset_url::make_pluginfile_url($ctx->id, 'mod_workshop', 'submission_attachment',
                            $submissionid, $filepath, $filename, true);
            $embedurl   = eset_url::make_pluginfile_url($ctx->id, 'mod_workshop', 'submission_attachment',
                            $submissionid, $filepath, $filename, false);
            $embedurl   = new eset_url($embedurl, array('preview' => 'bigthumb'));
            $type       = $file->get_mimetype();
            $image      = $this->output->pix_icon(file_file_icon($file), get_mimetype_description($file), 'eset', array('class' => 'icon'));
    ...
    protected function helper_grading_report_submission(stdclass $participant) {
        ...
        if (is_null($participant->submissionid)) {
            ...
            $url = new eset_url('/mod/workshop/submission.php',
    ...
    protected function helper_grading_report_assessment($assessment, $shownames, array $userinfo, $separator) {
        ...
        $url = new eset_url('/mod/workshop/assessment.php',
    ...
    /**
     * Helper method dealing with the fact we can not just fetch the output of esetforms
     *
     * @param esetform $mform
     * @return string HTML
     */
    protected static function esetform(esetform $mform) {
