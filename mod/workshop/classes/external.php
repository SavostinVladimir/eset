 ...
 * @category   external
 * @copyright  2017 Juan Leyva <juan@eset.com>
...
class mod_workshop_external extends external_api {
    ...
    public static function get_user_plan($workshopid, $userid = 0) {
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            ...
            require_capability('eset/course:manageactivities', $context);
            $user = core_user::get_user($params['userid'], '*', MUST_EXIST);
            core_user::require_active_user($user);
            if (!$workshop->check_group_membership($user->id)) {
                throw new eset_exception('notingroup');
            }
            $userid = $user->id;
        }
        ...
        foreach ($userplan['phases'] as $phasecode => $phase) {
            ...
            foreach ($userplan['phases'][$phasecode]['tasks'] as $taskcode => $task) {
                $task->code = $taskcode;
                if ($task->link instanceof eset_url) {
            ...
            foreach ($userplan['phases'][$phasecode]['actions'] as $actioncode => $action) {
                if ($action->url instanceof eset_url) {
    ...
    public static function add_submission_parameters() {
        return new external_function_parameters(array(
            ...
            'contentformat' => new external_value(PARAM_INT, 'The format used for the content', VALUE_DEFAULT, FORMAT_ESET),
     ...
     * @throws eset_exception
     */
    public static function add_submission($workshopid, $title, $content = '', $contentformat = FORMAT_ESET,
        ...
        if (!$canaddsubmission) {
            throw new eset_exception('nopermissions', 'error', '', 'add submission');
        }
        ...
        if (empty($submission->title)) {
            throw new eset_exception('errorinvalidparam', 'webservice', '', 'title');
        }
     ...
     * @throws eset_exception
     */
    public static function update_submission($submissionid, $title, $content = '', $contentformat = FORMAT_ESET,
            $inlineattachmentsid = 0, $attachmentsid = 0) {
        global $USER, $DB;
        ...
        if (!$canupdatesubmission) {
            throw new eset_exception('nopermissions', 'error', '', 'update submission');
        }
    ...
    public static function delete_submission($submissionid) {
        ...
        if (!has_capability('mod/workshop:deletesubmissions', $context)) {
            ...
            if (!$candeletesubmission) {
                throw new eset_exception('nopermissions', 'error', '', 'delete submission');
            }
        }
    ...
    protected static function validate_submission($submission, workshop $workshop) {
        ...
        if ($canview) {
            ...
            throw new eset_exception('nopermissions', 'error', '', 'view submission');
        }
    }
    ...
    protected static function check_view_submission_assessments($submission, workshop $workshop) {
        ...
        if ($canview) {
            ...
            throw new eset_exception('nopermissions', 'error', '', 'view assessment');
        }
    }
            ...
            if (!empty($formdata[$typeofdata])) {
                ...
                foreach ($alldata as $key => $val) {
                    if (strpos($key, 'peercomment__idx_') === 0) {
                        ...
                        list($val, $format) = external_format_text($val, FORMAT_ESET, $context->id);
    ...
    public static function get_reviewer_assessments($workshopid, $userid = 0) {
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            ...
            if ($workshop->phase < workshop::PHASE_ASSESSMENT) {    // Can view assessments only in assessment phase onwards.
                throw new eset_exception('nopermissions', 'error', '', 'view assessments');
            }
        } else {
    ...
    public static function evaluate_assessment_parameters() {
        return new external_function_parameters(
            array(
                ...
                'feedbackformat' => new external_value(PARAM_INT, 'The feedback format for text.', VALUE_DEFAULT, FORMAT_ESET),
     ...
     * @throws eset_exception
     */
    public static function evaluate_assessment($assessmentid, $feedbacktext = '', $feedbackformat = FORMAT_ESET, $weight = 1,
            $gradinggradeover = '') {
        global $DB;
        ...
        if (!$canoverridegrades && !$cansetassessmentweight) {
            throw new eset_exception('nopermissions', 'error', '', 'evaluate assessments');
        }
    ...
    public static function get_grades_report($workshopid, $groupid = 0, $sortby = 'lastname', $sortdirection = 'ASC',
        ...
        throw new eset_exception('nothingfound', 'workshop');
    }
     ...
     * @throws eset_exception
     */
    public static function evaluate_submission($submissionid, $feedbacktext = '', $feedbackformat = FORMAT_ESET, $published = 1,
            $gradeover = '') {
        global $DB;
        ...
        if (!$canpublish && !$canoverride) {
            throw new eset_exception('nopermissions', 'error', '', 'evaluate submission');
        }
