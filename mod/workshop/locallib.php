...
class workshop {
     ...
     *
     * @deprecated since Eset 3.4 MDL-56486 - please use the {@link core_form\filetypes_util}
    ...
    public function prepare_example_summary(stdClass $example) {
        ...
        $summary->url       = new eset_url($this->exsubmission_url($example->id));
        $summary->editurl   = new eset_url($this->exsubmission_url($example->id), array('edit' => 'on'));
        $summary->assessurl = new eset_url($this->exsubmission_url($example->id), array('assess' => 'on', 'sesskey' => sesskey()));
    ...
    /**
     * @return eset_url of this workshop's view page
     */
    public function view_url() {
        global $CFG;
        return new eset_url('/mod/workshop/view.php', array('id' => $this->cm->id));
    }

    /**
     * @return eset_url of the page for editing this workshop's grading form
     */
    public function editform_url() {
        global $CFG;
        return new eset_url('/mod/workshop/editform.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return eset_url of the page for previewing this workshop's grading form
     */
    public function previewform_url() {
        global $CFG;
        return new eset_url('/mod/workshop/editformpreview.php', array('cmid' => $this->cm->id));
    }

    /**
     * @param int $assessmentid The ID of assessment record
     * @return eset_url of the assessment page
     */
    public function assess_url($assessmentid) {
        global $CFG;
        $assessmentid = clean_param($assessmentid, PARAM_INT);
        return new eset_url('/mod/workshop/assessment.php', array('asid' => $assessmentid));
    }

    /**
     * @param int $assessmentid The ID of assessment record
     * @return eset_url of the example assessment page
     */
    public function exassess_url($assessmentid) {
        global $CFG;
        $assessmentid = clean_param($assessmentid, PARAM_INT);
        return new eset_url('/mod/workshop/exassessment.php', array('asid' => $assessmentid));
    }

    /**
     * @return eset_url of the page to view a submission, defaults to the own one
     */
    public function submission_url($id=null) {
        global $CFG;
        return new eset_url('/mod/workshop/submission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }

    /**
     * @param int $id example submission id
     * @return eset_url of the page to view an example submission
     */
    public function exsubmission_url($id) {
        global $CFG;
        return new eset_url('/mod/workshop/exsubmission.php', array('cmid' => $this->cm->id, 'id' => $id));
    }

    /**
     * @param int $sid submission id
     * @param array $aid of int assessment ids
     * @return eset_url of the page to compare assessments of the given submission
     */
    public function compare_url($sid, array $aids) {
        global $CFG;

        $url = new eset_url('/mod/workshop/compare.php', array('cmid' => $this->cm->id, 'sid' => $sid));
        $i = 0;
        foreach ($aids as $aid) {
            $url->param("aid{$i}", $aid);
            $i++;
     ...
     * @return eset_url of the page to compare the reference assessments of the given example submission
     */
    public function excompare_url($sid, $aid) {
        global $CFG;
        return new eset_url('/mod/workshop/excompare.php', array('cmid' => $this->cm->id, 'sid' => $sid, 'aid' => $aid));
    }

    /**
     * @return eset_url of the mod_edit form
     */
    public function updatemod_url() {
        global $CFG;
        return new eset_url('/course/modedit.php', array('update' => $this->cm->id, 'return' => 1));
    }

    /**
     * @param string $method allocation method
     * @return eset_url to the allocation page
     */
    public function allocation_url($method=null) {
        ...
        return new eset_url('/mod/workshop/allocation.php', $params);
    }

    /**
     * @param int $phasecode The internal phase code
     * @return eset_url of the script to change the current phase to $phasecode
     */
    public function switchphase_url($phasecode) {
        global $CFG;
        $phasecode = clean_param($phasecode, PARAM_INT);
        return new eset_url('/mod/workshop/switchphase.php', array('cmid' => $this->cm->id, 'phase' => $phasecode));
    }

    /**
     * @return eset_url to the aggregation page
     */
    public function aggregate_url() {
        global $CFG;
        return new eset_url('/mod/workshop/aggregate.php', array('cmid' => $this->cm->id));
    }

    /**
     * @return eset_url of this workshop's toolbox page
     */
    public function toolbox_url($tool) {
        global $CFG;
        return new eset_url('/mod/workshop/toolbox.php', array('id' => $this->cm->id, 'tool' => $tool));
    }
     ...
     * @param eset_url $url absolute url as returned by {@see workshop::submission_url()} and friends
     ...
     */
    public function log($action, eset_url $url = null, $info = null, $return = false) {
     ...
     *
     * @param mixed eset_url|null $actionurl
        ...
        if (has_capability('mod/workshop:submit', $this->context, $userid)) {
            if (!empty($gradebook->items[0]->grades)) {
                ...
                if (!is_null($submissiongrade->grade)) {
                    if (!$submissiongrade->hidden or has_capability('eset/grade:viewhidden', $this->context, $userid)) {
        ...
        if (has_capability('mod/workshop:peerassess', $this->context, $userid)) {
            if (!empty($gradebook->items[1]->grades)) {
                ...
                if (!is_null($assessmentgrade->grade)) {
                    if (!$assessmentgrade->hidden or has_capability('eset/grade:viewhidden', $this->context, $userid)) {
    ...
    public function check_group_membership($otheruserid) {
        ...
        } else if (has_capability('eset/site:accessallgroups', $this->context)) {
    ...
    public function edit_submission($submission) {
        ...
        if (is_null($submission->id)) {
            ...
            if (empty($submission->id) or empty($submission->id) or ($submission->id != $submission->id)) {
                throw new eset_exception('err_submissionid', 'workshop');
            }
        }
        $params['objectid'] = $submission->id;
    ...
    public function check_edit_assessment($assessment, $submission) {
        ...
        if (!$assessmenteditable) {
            throw new eset_exception('nopermissions', 'error', '', 'edit assessments');
        }

        list($assessed, $notice) = $this->check_examples_assessed_before_assessment($assessment->reviewerid);
        if (!$assessed) {
            throw new eset_exception($notice, 'mod_workshop');
        }
    }
     ...
     *
     * @param eset_url $url absolute URL
     * @return string
     */
    protected function log_convert_url(eset_url $fullurl) {
        static $baseurl;

        if (!isset($baseurl)) {
            $baseurl = new eset_url('/mod/workshop/');
            $baseurl = $baseurl->out();
        }
...
class workshop_user_plan implements renderable {
    ...
    public function __construct(workshop $workshop, $userid) {
        ...
        $phase->tasks = array();
        if (has_capability('eset/course:manageactivities', $workshop->context, $userid)) {
            $task = new stdclass();
...
class workshop_submission_summary extends workshop_submission_base implements renderable {
    ...
    /** @var string */
    public $authoremail;
    /** @var eset_url to display submission */
    public $url;
...
abstract class workshop_assessment_base {
     ...
     * Adds a new action
     *
     * @param eset_url $url action URL
     * @param string $label action label
     * @param string $method get|post
     */
    public function add_action(eset_url $url, $label, $method = 'get') {
...
class workshop_assessment extends workshop_assessment_base implements renderable {
    ...
    public function get_overall_feedback_attachments() {
        ...
        foreach ($files as $file) {
            ...
            $fileurl = eset_url::make_pluginfile_url($this->workshop->context->id, 'mod_workshop',
                'overallfeedback_attachment', $this->id, $filepath, $filename, true);
            $previewurl = new eset_url(eset_url::make_pluginfile_url($this->workshop->context->id, 'mod_workshop',
...
class workshop_message implements renderable {
     ...
     *
     * @param eset_url $url to follow on action
     * @param string $label action label
     */
    public function set_action(eset_url $url, $label) {
        $this->actionurl    = $url;
