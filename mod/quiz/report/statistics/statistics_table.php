...
class quiz_statistics_table extends flexible_table {
     ...
     * @param eset_url $reporturl the URL to redisplay this report.
    ...
    public function  wrap_html_start() {
        // Horrible Eset 2.0 wide-content work-around.
        if (!$this->is_downloading()) {
    ...
    protected function col_name($questionstat) {
        ...
        $baseurl = new eset_url($this->baseurl);
        if (!is_null($questionstat->variant)) {
            if ($questionstat->subquestion) {
                // Variant of a sub-question.
                $url = new eset_url($baseurl, array('qid' => $questionstat->questionid, 'variant' => $questionstat->variant));
                ...
                $url = new eset_url($baseurl, array('slot' => $questionstat->slot, 'variant' => $questionstat->variant));
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysisforvariant',
                                                                                   'quiz_statistics',
                                                                                   $questionstat->variant)));
            }
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $url = new eset_url($baseurl, array('qid' => $questionstat->questionid));
                ...
                $url = new eset_url($baseurl, array('slot' => $questionstat->slot));
    ...
    protected function format_percentage(float $number, bool $fraction = true, int $decimals = 2) {
        ...
        return get_string('percents', 'eset', format_float($number * $coefficient, $decimals));
    }
