...
class quiz_statistics_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        $reporturl = new eset_url('/mod/quiz/report.php', $pageoptions);
        ...
        if ($everything) { // Implies is downloading.
            ...
            if ($variantno === null &&
                ...
                $this->table->define_baseurl(new eset_url($reporturl, array('slot' => $slot)));
     ...
     *
     * @param eset_url $reporturl the base URL of the report.
     * @return string HTML.
     */
    protected function everything_download_options(eset_url $reporturl) {
        global $OUTPUT;
     ...
     *                                   QUIZ_GRADEAVERAGE, QUIZ_GRADEHIGHEST, QUIZ_ATTEMPTLAST or QUIZ_ATTEMPTFIRST
     *                                   we calculate stats based on which attempts would affect the grade for each student.
     * @param eset_url $reporturl url for this report
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $quizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        ...
        $recalcualteurl = new eset_url($reporturl,
