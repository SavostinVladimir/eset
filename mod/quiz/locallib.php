 ...
 * Functions that are also called by core Eset are in {@link lib.php}
...
function quiz_create_attempt(quiz $quizobj, $attemptnumber, $lastattempt, $timenow, $ispreview = false, $userid = null) {
    ...
    if ($quiz->sumgrades < 0.000005 && $quiz->grade > 0.000005) {
        throw new eset_exception('cannotstartgradesmismatch', 'quiz',
                new eset_url('/mod/quiz/view.php', array('q' => $quiz->id)),
...
function quiz_get_user_timeclose($courseid) {
    ...
    if (has_capability('eset/course:update', context_course::instance($courseid))) {
...
function quiz_question_edit_button($cmid, $question, $returnurl, $contentaftericon = '') {
    ...
    // Build the icon.
    if ($action) {
        if ($returnurl instanceof eset_url) {
            $returnurl = $returnurl->out_as_local_url(false);
        }
        $questionparams = array('returnurl' => $returnurl, 'cmid' => $cmid, 'id' => $question->id);
        $questionurl = new eset_url("$CFG->wwwroot/question/question.php", $questionparams);
 ...
 * @param object $question the question
 * @param int $variant which question variant to preview (optional).
 * @return eset_url to preview this question with the options from this quiz.
 */
...
function quiz_get_flag_option($attempt, $context) {
    global $USER;
    if (!has_capability('eset/question:flag', $context)) {
...
function quiz_get_review_options($quiz, $attempt, $context) {
    ...
    if (!empty($attempt->id)) {
        $options->questionreviewlink = new eset_url('/mod/quiz/reviewquestion.php',
    ...
    if (!empty($attempt->id) && $attempt->state == quiz_attempt::FINISHED && !$attempt->preview &&
        ...
        $options->manualcommentlink = new eset_url('/mod/quiz/comment.php',
                array('attempt' => $attempt->id));
    }

    if (!is_null($context) && !$attempt->preview &&
            has_capability('mod/quiz:viewreports', $context) &&
            has_capability('eset/grade:viewhidden', $context)) {
...
function quiz_send_notification_messages($course, $quiz, $attempt, $context, $cm) {
    ...
    if (is_array($groups) && count($groups) > 0) {
        ...
        // 'eset/site:accessallgroups' get notified.
        $groups = -1;
    } else {
        $groups = '';
    }
...
function quiz_get_js_module() {
    ...
    return array(
        ...
        'requires' => array('base', 'dom', 'event-delegate', 'event-key',
                'core_question_engine', 'eset-core-formchangechecker'),
        'strings' => array(
            array('cancel', 'eset'),
            ...
            array('timesup', 'quiz'),
            array('changesmadereallygoaway', 'eset'),
        ),
    );
}
...
function quiz_add_random_questions($quiz, $addonpage, $categoryid, $number,
    ...
    require_capability('eset/question:useall', $catcontext);
    ...
    if ($lastattempt && ($lastattempt->state == quiz_attempt::IN_PROGRESS ||
        ...
        if ($lastattempt->state == quiz_attempt::ABANDONED || $lastattempt->state == quiz_attempt::FINISHED) {
            if ($redirect) {
                ...
                throw new eset_quiz_exception($quizobj, 'attemptalreadyclosed');
            }
        }
...
function quiz_create_attempt_handling_errors($attemptid, $cmid = null) {
    try {
        ...
        if (!empty($cmid)) {
            ...
            $continuelink = new eset_url('/mod/quiz/view.php', array('id' => $cmid));
            $context = context_module::instance($cm->id);
            if (has_capability('mod/quiz:preview', $context)) {
                throw new eset_exception('attempterrorcontentchange', 'quiz', $continuelink);
            } else {
                throw new eset_exception('attempterrorcontentchangeforuser', 'quiz', $continuelink);
            }
        } else {
            throw new eset_exception('attempterrorinvalid', 'quiz');
        }
    }
    if (!empty($cmid) && $attempobj->get_cmid() != $cmid) {
        throw new eset_exception('invalidcoursemodule');
    } else {
        return $attempobj;
    }
}
