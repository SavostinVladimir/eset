...
class restore_quiz_activity_structure_step extends restore_questions_activity_structure_step {
     ...
     * Eset 2.9 quiz sections did not exist, so if the file being restored
    ...
    protected function process_quiz($data) {
        ...
        // Process Safe Exam Browser settings for backups taken in Eset < 3.9.
        if (!empty($addsebrule)) {
    ...
    protected function process_quiz_question_instance($data) {
        ...
        if (isset($data->questioncategoryid)) {
            ...
            // Backward compatibility for backups created using Eset 3.4 or earlier.
