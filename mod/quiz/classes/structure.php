...
class structure {
    ...
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            ...
            throw new \eset_exception('cannoteditafterattempts', 'quiz',
                    new \eset_url('/mod/quiz/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if (empty($movingslot)) {
            throw new \eset_exception('Bad slot ID ' . $idmove);
        }
     ...
     * It is only possible to add a random question if the user has the eset/question:useall capability
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            ...
            $usablecontexts = $relatedcontexts->having_cap('eset/question:useall');
