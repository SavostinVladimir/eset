 ...
 * @package    mod_quiz
 * @category   external
 * @copyright  2016 Juan Leyva <juan@eset.com>
...
class mod_quiz_external extends external_api {
            ...
            array(
                'quizzes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Eset primary key.'),
    ...
    public static function start_attempt($quizid, $preflightdata = array(), $forcenew = false) {
        ...
        if (!$quizobj->has_questions()) {
            throw new eset_quiz_exception($quizobj, 'noquestionsfound');
        }
        ...
        if (!$quizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new eset_quiz_exception($quizobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == quiz_attempt::OVERDUE) {
                    throw new eset_quiz_exception($quizobj, 'stateoverdue');
                } else {
                    throw new eset_quiz_exception($quizobj, 'attemptstillinprogress');
                }
            }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new eset_quiz_exception($attemptobj->get_quizobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new eset_quiz_exception($attemptobj->get_quizobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == quiz_attempt::OVERDUE) {
            throw new eset_quiz_exception($attemptobj->get_quizobj(), 'stateoverdue');
        }
        ...
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'noquestionsfound');
            }
        }
     ...
     * @since  Eset 3.1
     * @since Eset 3.2 blockedbyprevious parameter added.
     */
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new eset_quiz_exception($attemptobj->get_quizobj(), 'noreview', null, '',
                    $attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new eset_quiz_exception($attemptobj->get_quizobj(), 'noreviewattempt');
        }
    ...
    public static function get_quiz_feedback_for_grade($quizid, $grade) {
        ...
        $result = array();
        $result['feedbacktext'] = '';
        $result['feedbacktextformat'] = FORMAT_ESET;
