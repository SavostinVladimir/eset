YUI.add('eset-mod_quiz-toolboxes', function (Y, NAME) {
...
},
{
    ...
    ATTRS: {
        /**
         * The ID of the Eset Course being edited.
         *
         * @attribute courseid
         * @default 0
         * @type Number
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Eset.mod_quiz.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Eset.mod_quiz.util.slot.remove(element);
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.Eset.mod_quiz.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.Eset.mod_quiz.util.slot.getId(slot);
            slots.push(slot);
        });
        ...
        confirm.on('complete-yes', function() {
            ...
            this.send_request(data, spinner, function(response) {
                ...
                if (response.deleted) {
                    // Actually remove the element.
                    Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                        Y.Eset.mod_quiz.util.slot.remove(node.ancestor('li.activity'));
                    });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.Eset.mod_quiz.util.slot.getId(activity)
            };
        ...
        this.send_request(data, null, function(response) {
            ...
            var editinstructions = Y.Node.create('<span class="' + CSS.EDITINSTRUCTIONS + '" id="id_editinstructions" />')
                .set('innerHTML', M.util.get_string('edittitleinstructions', 'eset'));
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Eset.mod_quiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Eset.mod_quiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Eset.mod_quiz.util.page.SELECTORS.PAGE);
                    Y.Eset.mod_quiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Eset.mod_quiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Eset.mod_quiz.util.slot.reorderSlots();
        Y.Eset.mod_quiz.util.slot.reorderPageBreaks();
        Y.Eset.mod_quiz.util.page.reorderPages();
        Y.Eset.mod_quiz.util.slot.updateOneSlotSections();
        Y.Eset.mod_quiz.util.slot.updateAllDependencyIcons();
    },
 ...
 *
 * @module eset-mod_quiz-toolboxes
