YUI.add('eset-mod_quiz-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for Quiz sections and slots.
 *
 * @module eset-mod-quiz-dragdrop
 */
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    setup_for_section: function(baseselector) {
        Y.Node.all(baseselector).each(function(sectionnode) {
            ...
            var sectionid = Y.Eset.core_course.util.section.getId(sectionnode);
            ...
            if (sectionid > 0) {
                ...
                // Add dragger icon
                var title = M.util.get_string('movesection', 'eset', sectionid);
    ...
    drop_hit: function(e) {
        ...
        var dragnode = drag.get('node'),
            dragnodeid = Y.Eset.core_course.util.section.getId(dragnode),
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'eset-mod_quiz-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'eset-mod_quiz-dragdrop');
                ...
                success: function(tid, response) {
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Eset.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                        Y.Eset.core_course.util.section.getId(sectionlist.item(index))) {
                                Y.log("Swapping " + Y.Eset.core_course.util.section.getId(sectionlist.item(index - 1)) +
                                        " with " + Y.Eset.core_course.util.section.getId(sectionlist.item(index)),
                                        "debug", "eset-mod_quiz-dragdrop");
                                // Swap section id.
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    setup_for_resource: function(baseselector) {
        Y.Node.all(baseselector).each(function(resourcesnode) {
            ...
            if (move) {
                var resourcedraghandle = this.get_drag_handle(M.util.get_string('move', 'eset'),
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Eset.mod_quiz.util.slot.getId(dragnode));
        params.sectionId = Y.Eset.core_course.util.section.getId(dragnode.ancestor('li.section', true));

        var previousslot = dragnode.previous(SELECTOR.SLOT);
        if (previousslot) {
            params.previousid = Number(Y.Eset.mod_quiz.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Eset.mod_quiz.util.page.getId(previouspage));
        }
...
M.mod_quiz.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "dd-scroll",
        "eset-core-dragdrop",
        "eset-core-notification",
        "eset-mod_quiz-quizbase",
        "eset-mod_quiz-util-base",
        "eset-mod_quiz-util-page",
        "eset-mod_quiz-util-slot",
        "eset-course-util"
    ]
});
