YUI.add('eset-mod_quiz-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module eset-mod_quiz-util
 * @submodule eset-mod_quiz-util-slot
 */

Y.namespace('Eset.mod_quiz.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Eset.mod_quiz.util.slot
 * @static
 */
Y.Eset.mod_quiz.util.slot = {
    CSS: {
        SLOT: 'slot',
    ...
    CONSTANTS: {
        SLOTIDPREFIX: 'slot-',
        QUESTION: M.util.get_string('question', 'eset')
    },
    SELECTORS: {
        SLOT: 'li.slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Eset.mod_quiz.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Eset.mod_quiz.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    remove: function(slot) {
        var page = Y.Eset.mod_quiz.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Eset.mod_quiz.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Eset.mod_quiz.util.page.remove(page);
    },
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '';
            var iconname = '';
            if (Y.Eset.mod_quiz.util.page.isPage(nextitem)) {
...
}, '@VERSION@', {"requires": ["node", "eset-mod_quiz-util-base"]});
