YUI.add('eset-mod_quiz-autosave', function (Y, NAME) {
...
M.mod_quiz.autosave = {
    ...
    init: function(delay) {
        ...
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'eset-mod_quiz-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof window.tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'eset-mod_quiz-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'eset-mod_quiz-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'eset-mod_quiz-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'eset-mod_quiz-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'eset-mod_quiz-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'eset-mod_quiz-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'eset-mod_quiz-autosave');
        if (typeof window.tinyMCE !== 'undefined') {
    ...
    save_done: function(transactionid, response) {
        ...
        if (autosavedata.status !== 'OK') {
            // Because IIS is useless, Eset can't send proper HTTP response
        ...
        if (typeof autosavedata.timeleft !== 'undefined') {
            Y.log('Updating timer: ' + autosavedata.timeleft + ' seconds remain.', 'debug', 'eset-mod_quiz-timer');
            M.mod_quiz.timer.updateEndTime(autosavedata.timeleft);
        }

        Y.log('Save completed.', 'debug', 'eset-mod_quiz-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'eset-mod_quiz-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'eset-mod_quiz-autosave');
        this.save_transaction = null;
