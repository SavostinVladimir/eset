...
class quiz_settings extends persistent {
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new eset_exception('noconfigfilefound', 'quizaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $quizurl = new eset_url($CFG->wwwroot . "/mod/quiz/view.php", ['id' => $this->get('cmid')]);
    ...
    /**
     * Use the boolean map to add Eset boolean setting to config PList.
     */
     ...
     *
     * @return array Eset setting as key, SEB setting as value.
     */
