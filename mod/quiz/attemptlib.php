 ...
 * constructor for a eset_exception.
 *
 * @copyright 2008 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Eset 2.0
 */
class eset_quiz_exception extends eset_exception {
    /**
     * Constructor.
     *
...
class quiz {
    ...
    public function show_only_active_users() {
        return !has_capability('eset/course:viewsuspendedusers', $this->get_context());
    }
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new eset_exception('cannotstartmissingquestion', 'quiz', $this->view_url());
            }
     ...
     * @return eset_url the URL of this quiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new eset_url('/mod/quiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new eset_url('/mod/quiz/review.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    public function summary_url($attemptid) {
        return new eset_url('/mod/quiz/summary.php', array('attempt' => $attemptid, 'cmid' => $this->get_cmid()));
    }
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new eset_quiz_exception($this, 'questionnotloaded', $id);
        }
    }
...
class quiz_attempt {
    ...
    public function is_review_allowed() {
        ...
        $cm = $this->get_cm();
        if ($this->has_capability('eset/site:accessallgroups') ||
     ...
     * @param eset_url $thispageurl to return to after the editing form is
    ...
    /**
     * @return eset_url the URL of this quiz's summary page.
     */
    public function summary_url() {
        return new eset_url('/mod/quiz/summary.php', array('attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()));
    }
    ...
    public function processattempt_url() {
        return new eset_url('/mod/quiz/processattempt.php');
    }
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new eset_url(
     ...
     * @param eset_url $thispageurl the URL of the page this question is being printed on.
     ...
     * The array values are either a {@link eset_url} with the attempt parameter
     ...
     * @return mod_quiz_links_to_other_attempts|bool containing array int => null|eset_url.
     *      False if none.
     */
    public function links_to_other_attempts(eset_url $url) {
        ...
        foreach ($attempts as $at) {
            if ($at->id == $this->attempt->id) {
                ...
                $links->links[$at->attempt] = new eset_url($url, array('attempt' => $at->id));
            }
        }
        return $links;
    }
     ...
     * The array values are either a {@link eset_url} with the slot parameter
     ...
     * @param eset_url $baseurl the base URL to modify to generate each link.
     * @return mod_quiz_links_to_other_attempts|null containing array int => null|eset_url,
     *      or null if the question in this slot has not been redone.
     */
    public function links_to_other_redos($slot, eset_url $baseurl) {
        $originalslot = $this->get_original_slot($slot);
        ...
        foreach ($qas as $qa) {
            if ($qa->get_slot() == $slot) {
                ...
                $url = new eset_url($baseurl, array('slot' => $qa->get_slot()));
     ...
     * @param bool $studentisonline is the student currently interacting with Eset?
     */
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        if ($questiondata->qtype != 'random') {
            ...
            if ($newqusetionid === null) {
                throw new eset_exception('notenoughrandomquestions', 'quiz',
     ...
     * @return eset_url The requested URL.
     */
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            return new eset_url($fragment);

        } else {
            $url = new eset_url('/mod/quiz/' . $script . '.php' . $fragment,
        ...
        if (!$finishattempt) {
            ...
            if (!$toolate) {
                try {
                    ...
                    throw new eset_exception('submissionoutofsequencefriendlymessage', 'question',
                    ...
                    throw new eset_exception('errorprocessingresponses', 'question',
...
class quiz_nav_question_button implements renderable {
    ...
    public $flagged;
    /** @var eset_url the link this button goes to, or null if there should not be a link. */
    public $url;
...
abstract class quiz_nav_panel_base {
    ...
    protected function render_restart_preview_link($output) {
        ...
        return $output->restart_preview_button(new eset_url(
