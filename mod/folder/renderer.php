...
class mod_folder_renderer extends plugin_renderer_base {
    ...
    public function display_folder(stdClass $folder) {
        ...
        if ($downloadable) {
            $downloadbutton = $this->output->single_button(
                new eset_url('/mod/folder/download_folder.php', array('id' => $cm->id)),
        ...
        if (has_capability('mod/folder:managefiles', $context) &&
            ($folder->display != FOLDER_DISPLAY_INLINE || !has_capability('eset/course:manageactivities', $context))) {
            $editbutton = $this->output->single_button(
                new eset_url('/mod/folder/edit.php', array('id' => $cm->id)),
                get_string('edit')
            );
    ...
    protected function htmllize_tree($tree, $dir) {
        ...
        foreach ($dir['subdirs'] as $subdir) {
            $image = $this->output->pix_icon(file_folder_icon(24), $subdir['dirname'], 'eset');
        ...
        foreach ($dir['files'] as $file) {
            ...
            $url = eset_url::make_pluginfile_url($file->get_contextid(), $file->get_component(),
            ...
            if (file_extension_in_typegroup($filename, 'web_image')) {
                ...
                $image = $this->output->pix_icon(file_file_icon($file, 24), $filenamedisplay, 'eset');
            }
            $filename = html_writer::tag('span', $image, array('class' => 'fp-icon')).
