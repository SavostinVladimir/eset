...
function resource_display_embed($resource, $cm, $course, $file) {
    ...
    $eseturl = eset_url::make_pluginfile_url($context->id, 'mod_resource', 'content', $resource->revision,
    ...
    if (file_mimetype_in_typegroup($mimetype, 'web_image')) {  // It's an image
        $code = resourcelib_embed_image($eseturl->out(), $title);

    } else if ($mimetype === 'application/pdf') {
        // PDF document
        $code = resourcelib_embed_pdf($eseturl->out(), $title, $clicktoopen);

    } else if ($mediamanager->can_embed_url($eseturl, $embedoptions)) {
        // Media (audio/video) file.
        $code = $mediamanager->embed_url($eseturl, $title, 0, 0, $embedoptions);

    } else {
        // We need a way to discover if we are loading remote docs inside an iframe.
        $eseturl->param('embed', 1);

        // anything else - just try object tag enlarged as much as possible
        $code = resourcelib_embed_general($eseturl, $title, $clicktoopen, $mimetype);
    }
