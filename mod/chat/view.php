...
if (has_capability('mod/chat:chat', $context)) {
    ...
    $params['id'] = $chat->id;
    $chattarget = new eset_url("/mod/chat/gui_$CFG->chat_method/index.php", $params);
    echo '<p>';
    ...
    $params['id'] = $chat->id;
    $link = new eset_url('/mod/chat/gui_basic/index.php', $params);
    ...
    if ($chat->studentlogs or has_capability('mod/chat:readlog', $context)) {
        if ($msg = chat_get_session_messages($chat->id, $currentgroup)) {
            echo '<p>';
            echo html_writer::link(new eset_url('/mod/chat/report.php', array('id' => $cm->id)),
...
if ($chatusers = chat_get_users($chat->id, $currentgroup, $cm->groupingid)) {
    ...
    foreach ($chatusers as $chatuser) {
        ...
        $url = new eset_url('/user/view.php', array('id' => $chatuser->id, 'course' => $chat->course));
