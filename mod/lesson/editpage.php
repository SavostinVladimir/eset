...
if (!empty($returnto)) {
    $returnto = new eset_url($returnto);
} else {
    $returnto = new eset_url('/mod/lesson/edit.php', array('id' => $id));
...
// in esetform::_process_submission because the array key check doesn't
...
if ($edit) {
    ...
    $mform->set_data($data);
    $PAGE->navbar->add(get_string('edit'), new eset_url('/mod/lesson/edit.php', array('id'=>$id)));
