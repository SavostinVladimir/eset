...
$url = new eset_url('/mod/lesson/essay.php', array('id'=>$id));
if ($mode !== 'display') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
switch ($mode) {
    ...
    case 'update':
        ...
        if ($form = $mform->get_data()) {
            ...
            redirect(new eset_url('/mod/lesson/essay.php', array('id'=>$cm->id)));
        } else {
    ...
    case 'email':
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($essayinfo->graded && !$essayinfo->sent) {
                ...
                // Context url.
                $contexturl = new eset_url('/grade/report/user/index.php', array('id' => $course->id));
...
switch ($mode) {
    case 'display':
        ...
        foreach (array_keys($studentessays) as $userid) {
            ...
            foreach ($studentessays[$userid] as $page => $tries) {
                ...
                foreach($tries as $try) {
                    ...
                    // link for each essay
                    $url = new eset_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'grade','attemptid'=>$essay->id,'sesskey'=>sesskey()));
            ...
            $url = new eset_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'email','userid'=>$userid,'sesskey'=>sesskey()));
        ...
        $url = new eset_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'email','sesskey'=>sesskey()));
