...
class lesson_page_type_branchtable extends lesson_page {
    ...
    public function get_jumps() {
        ...
        if ($answers = $this->get_answers()) {
            ...
            // We get here is the lesson was created on a Eset 1.9 site and
    ...
    public function display($renderer, $attempt) {
        ...
        foreach ($this->get_answers() as $answer) {
            ...
            $url = new eset_url('/mod/lesson/continue.php', $params);
            $buttons[] = $renderer->single_button($url, strip_tags(format_text($answer->answer, FORMAT_ESET, $options)));
            $i++;
        }
        // Set the orientation
    ...
    public function report_answers($answerpage, $answerdata, $useranswer, $pagestats, &$i, &$n) {
        ...
        foreach ($answers as $answer) {
            $data = "<input type=\"button\" class=\"btn btn-secondary\" name=\"$answer->id\" " .
                    "value=\"".s(strip_tags(format_text($answer->answer, FORMAT_ESET, $formattextdefoptions)))."\" " .
                    "disabled=\"disabled\"> ";
    ...
    public function add_page_link($previd) {
        global $PAGE, $CFG;
        $addurl = new eset_url('/mod/lesson/editpage.php', array('id'=>$PAGE->cm->id, 'pageid'=>$previd, 'qtype'=>LESSON_PAGE_BRANCHTABLE));
