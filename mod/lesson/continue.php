...
$url = new eset_url('/mod/lesson/continue.php', array('id'=>$cm->id));
$PAGE->set_url($url);
...
if (!$canmanage) {
    ...
    if (!$lesson->check_time($timer)) {
        redirect(new eset_url('/mod/lesson/view.php', array('id' => $cm->id, 'pageid' => LESSON_EOL, 'outoftime' => 'normal')));
...
if ($result->nodefaultresponse || $result->inmediatejump) {
    ...
    redirect(new eset_url('/mod/lesson/view.php', array('id'=>$cm->id,'pageid'=>$result->newpageid)));
}
...
if (!$reviewmode) {
    echo format_text($result->feedback, FORMAT_ESET, array('context' => $context, 'noclean' => true));
}
...
if (isset($USER->modattempts[$lesson->id])) {
    ...
    $url = new eset_url('/mod/lesson/view.php', array('id' => $cm->id, 'pageid' => LESSON_EOL));
...
if (!$result->correctanswer && !$result->noanswer && !$result->isessayquestion && !$reviewmode && $lesson->review && !$result->maxattemptsreached) {
    $url = new eset_url('/mod/lesson/view.php', array('id' => $cm->id, 'pageid' => $page->id));
    echo $OUTPUT->single_button($url, get_string('reviewquestionback', 'lesson'));
}

$url = new eset_url('/mod/lesson/view.php', array('id'=>$cm->id, 'pageid'=>$result->newpageid));
