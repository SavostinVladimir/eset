...
/**
 * Eset renderer used to display special elements of the lesson module
 *
 * @package mod_lesson
...
class mod_lesson_renderer extends plugin_renderer_base {
    ...
    public function continue_links(lesson $lesson, $lastpageseenid) {
        ...
        $yeslink = html_writer::link(new eset_url('/mod/lesson/view.php', array('id' => $this->page->cm->id,
        ...
        $nolink = html_writer::link(new eset_url('/mod/lesson/view.php', array('id' => $this->page->cm->id,
    ...
    public function display_edit_collapsed(lesson $lesson, $pageid) {
        ...
        while ($pageid != 0) {
            ...
            $data = array();
            $url = new eset_url('/mod/lesson/edit.php', array(
                'id'     => $this->page->cm->id,
                'mode'   => 'single',
                'pageid' => $page->id
            ));
    ...
    public function add_page_links(lesson $lesson, $prevpageid=false) {
        ...
        $importquestionsurl = new eset_url('/mod/lesson/import.php',array('id'=>$this->page->cm->id, 'pageid'=>$prevpageid));
        ...
        $addquestionurl = new eset_url('/mod/lesson/editpage.php', array('id'=>$this->page->cm->id, 'pageid'=>$prevpageid));
    ...
    public function add_first_page_links(lesson $lesson) {
        ...
        $addquestionurl = new eset_url('/mod/lesson/editpage.php', array('id'=>$this->page->cm->id, 'pageid'=>$prevpageid, 'firstpage'=>1));
    ...
    public function page_action_links(lesson_page $page, $printmove, $printaddpage=false) {
        ...
        if ($printmove) {
            $url = new eset_url('/mod/lesson/lesson.php',
        ...
        $url = new eset_url('/mod/lesson/editpage.php', array('id' => $this->page->cm->id, 'pageid' => $page->id, 'edit' => 1));
        $label = get_string('updatepagenamed', 'lesson', format_string($page->title));
        $img = $this->output->pix_icon('t/edit', $label);
        $actions[] = html_writer::link($url, $img, array('title' => $label));

        // Duplicate action.
        $url = new eset_url('/mod/lesson/lesson.php', array('id' => $this->page->cm->id, 'pageid' => $page->id,
        ...
        $url = new eset_url('/mod/lesson/view.php', array('id' => $this->page->cm->id, 'pageid' => $page->id));
        ...
        if ($printaddpage) {
            ...
            $addpageurl = new eset_url('/mod/lesson/editpage.php', array('id'=>$this->page->cm->id, 'pageid'=>$page->id, 'sesskey'=>sesskey()));
    ...
    public function display_eol_page(lesson $lesson, $data) {
        ...
        if (has_capability('gradereport/user:view', context_course::instance($course->id))
            ...
            $url = new eset_url('/grade/index.php', array('id' => $course->id));
