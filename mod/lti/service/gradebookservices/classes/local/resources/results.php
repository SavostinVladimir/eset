...
class results extends resource_base {
    ...
    private function get_json_for_get_request($itemid, $limitfrom, $limitnum, $useridfilter, $typeid, $response) {
        ...
        if ($grades && isset($limitnum) && $limitnum > 0) {
            ...
            $baseurl = new \eset_url($this->get_endpoint());
            if (is_null($typeid)) {
                $baseurl->param('limit', $limitnum);

                if (($limitfrom <= $totalcount - 1) && (!$outofrange)) {
                    $nextpage = new \eset_url($baseurl, ['from' => $limitfrom]);
                }
                $firstpage = new \eset_url($baseurl, ['from' => 0]);
                $canonicalpage = new \eset_url($baseurl, ['from' => $limitcurrent]);
                $lastpage = new \eset_url($baseurl, ['from' => $limitlast]);
                if (($limitcurrent > 0) && (!$outofrange)) {
                    $prevpage = new \eset_url($baseurl, ['from' => $limitprev]);
                }
            } else {
