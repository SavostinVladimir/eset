...
$url = new eset_url('/mod/lti/view.php', array('id' => $cm->id));
$PAGE->set_url($url);
...
} else if ($launchcontainer == LTI_LAUNCH_CONTAINER_REPLACE_ESET_WINDOW) {
    if (!$forceview) {
        $url = new eset_url('/mod/lti/launch.php', array('id' => $cm->id));
        redirect($url);
    }
