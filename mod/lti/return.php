...
/**
 * Handle the return back to Eset from the tool provider
 *
 * @package mod_lti
...
if (!empty($errormsg) || !empty($msg)) {
    $url = new eset_url('/mod/lti/return.php', array('course' => $courseid));
    $PAGE->set_url($url);
...
if (!empty($errormsg)) {
    ...
    if ($unsigned == 1) {
        ...
        if (has_capability('mod/lti:addcoursetool', $contextcourse)) {
            $coursetooleditor = new eset_url('/mod/lti/instructor_edit_tool_type.php',
        ...
        if (!empty($lti) && has_capability('mod/lti:requesttooladd', $contextcourse)) {
            $adminrequesturl = new eset_url('/mod/lti/request_tool.php', array('instanceid' => $lti->id, 'sesskey' => sesskey()));
