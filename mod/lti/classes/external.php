...
class mod_lti_external extends external_api {
    ...
    public static function create_tool_proxy($name, $registrationurl, $capabilityoffered, $serviceoffered) {
        ...
        if (!empty($duplicates)) {
            throw new eset_exception('duplicateregurl', 'mod_lti');
        }
     ...
     * @since Eset 3.1
     * @throws eset_exception If the tool type could not be created
     */
    public static function create_tool_type($cartridgeurl, $key, $secret) {
        ...
        if (!empty($cartridgeurl)) {
            ...
            if (empty($data->lti_toolurl)) {
                throw new eset_exception('unabletocreatetooltype', 'mod_lti');
            } else {
    ...
    public static function update_tool_type($id, $name, $description, $state) {
        ...
        if (empty($type)) {
            throw new eset_exception('unabletofindtooltype', 'mod_lti', '', array('id' => $id));
        }
        ...
        if (!empty($state)) {
            ...
            if (in_array($state, array(1, 2, 3))) {
                ...
                throw new eset_exception("Invalid state: $state - must be 1, 2, or 3");
            }
        }
