...
define('LTI_LAUNCH_CONTAINER_REPLACE_ESET_WINDOW', 5);
...
function lti_get_launch_data($instance, $nonce = '') {
    ...
    $url = new \eset_url('/mod/lti/return.php', $returnurlparams);
    ...
    switch($launchcontainer) {
        ...
        case LTI_LAUNCH_CONTAINER_EMBED_NO_BLOCKS:
            $target = 'iframe';
            break;
        case LTI_LAUNCH_CONTAINER_REPLACE_ESET_WINDOW:
            $target = 'frame';
            break;
    ...
    if ((!empty($key) && !empty($secret)) || ($ltiversion === LTI_VERSION_1P3)) {
        ...
        $endpointurl = new \eset_url($endpoint);
...
function lti_build_registration_request($toolproxy) {
    ...
    $url = new \eset_url('/mod/lti/externalregistrationreturn.php', $returnurlparams);
    $returnurl = $url->out(false);
...
function lti_build_request($instance, $typeconfig, $course, $typeid = null, $islti2 = false) {
    ...
    if (!empty($instance->id) && !empty($instance->servicesalt) && ($islti2 ||
        ...
        $serviceurl = new \eset_url('/mod/lti/service.php');
 ...
 *
 * @return array                    Request details
 * @deprecated since Eset 3.7 MDL-62599 - please do not use this function any more.
...
function lti_build_standard_message($instance, $orgid, $ltiversion, $messagetype = 'basic-lti-launch-request') {
    ...
    $requestparams['ext_lms'] = 'eset-2';
    $requestparams['tool_consumer_info_product_family_code'] = 'eset';
 ...
 * @param eset_url $returnurl The return URL in the tool consumer (TC) that the tool provider (TP)
 ...
 * @throws eset_exception When the LTI tool type does not exist.`
 * @throws coding_exception For invalid media type and presentation target parameters.
 */
function lti_build_content_item_selection_request($id, $course, eset_url $returnurl, $title = '', $text = '', $mediatypes = [],
    ...
    // Validate parameters.
    if (!$tool) {
        throw new eset_exception('errortooltypenotfound', 'mod_lti');
    }
    ...
    if (!empty($typeconfig['toolurl_ContentItemSelectionRequest'])) {
        $toolurl = new eset_url($typeconfig['toolurl_ContentItemSelectionRequest']);
    } else {
        $toolurl = new eset_url($typeconfig['toolurl']);
    }
...
function lti_verify_oauth_signature($typeid, $consumerkey) {
    ...
    if ($consumerkey !== $key) {
        throw new eset_exception('errorincorrectconsumerkey', 'mod_lti');
    }
...
function lti_verify_with_keyset($jwtparam, $keyseturl, $clientid) {
    ...
    try {
        if (empty($keyset)) {
            throw new eset_exception('errornocachedkeysetfound', 'mod_lti');
        }
...
function lti_verify_jwt_signature($typeid, $consumerkey, $jwtparam) {
    ...
    if (isset($tool->toolproxyid)) {
        throw new eset_exception('JWT security not supported with LTI 2');
    }
    ...
    if (empty($typeconfig['keytype']) || $typeconfig['keytype'] === LTI_RSA_KEY) {
        ...
        if (empty($publickey)) {
            throw new eset_exception('No public key configured');
        }
        ...
        if (empty($keyseturl)) {
            throw new eset_exception('No public keyset configured');
        }
        // Attempts to verify jwt with jwk keyset.
        lti_verify_with_keyset($jwtparam, $keyseturl, $tool->clientid);
    } else {
        throw new eset_exception('Invalid public key type');
    }
...
function lti_tool_configuration_from_content_item($typeid, $messagetype, $ltiversion, $consumerkey, $contentitemsjson) {
    ...
    if (empty($items)) {
        throw new eset_exception('errorinvaliddata', 'mod_lti', '', $contentitemsjson);
    }
    if (!isset($items->{'@graph'}) || !is_array($items->{'@graph'}) || (count($items->{'@graph'}) > 1)) {
        throw new eset_exception('errorinvalidresponseformat', 'mod_lti');
    }
    ...
    if (!empty($items->{'@graph'})) {
        ...
        if (isset($item->icon->{'@id'})) {
            $iconurl = new eset_url($item->icon->{'@id'});
    ...
    if (!empty($tools)) {
        ...
        foreach ($tools as $type) {
            ...
            if (empty($type->toolproxyid)) {
                $baseurl = new \eset_url('/mod/lti/typessettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
                ...
                $baseurl = new \eset_url('/mod/lti/toolssettings.php', array(
                        'action' => 'accept',
                        'id' => $type->id,
                        'sesskey' => sesskey(),
                        'tab' => $id
...
EOD;
        foreach ($toolproxies as $toolproxy) {
            ...
            $baseurl = new \eset_url('/mod/lti/registersettings.php', array(
                    'action' => 'accept',
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => $id
            ...
            $registerurl = new \eset_url('/mod/lti/register.php', array(
                    'id' => $toolproxy->id,
                    'sesskey' => sesskey(),
                    'tab' => 'tool_proxy'
                ));
...
function lti_get_ims_role($user, $cmid, $courseid, $islti2) {
    ...
    if (empty($cmid)) {
        ...
        if (has_capability('eset/course:manageactivities', $context, $user)) {
...
function lti_get_configured_types($courseid, $sectionreturn = 0) {
    ...
    foreach ($admintypes as $ltitype) {
        ...
        $type->link = new eset_url('/course/modedit.php', array('add' => 'lti', 'return' => 0, 'course' => $courseid,
...
function lti_get_launch_container($lti, $toolconfig) {
    ...
    if ($devicetype === core_useragent::DEVICETYPE_MOBILE || $devicetype === core_useragent::DEVICETYPE_TABLET ) {
        $launchcontainer = LTI_LAUNCH_CONTAINER_REPLACE_ESET_WINDOW;
    }
...
function lti_log_request($rawbody) {
    if ($tempdir = make_temp_directory('mod_lti', false)) {
        if ($tempfile = tempnam($tempdir, 'mod_lti_request'.date('YmdHis'))) {
            ...
            foreach (eset\mod\lti\OAuthUtil::get_headers() as $header => $value) {
...
function get_tool_type_edit_url(stdClass $type) {
    $url = new eset_url('/mod/lti/typessettings.php',
...
function get_tool_proxy_edit_url(stdClass $proxy) {
    $url = new eset_url('/mod/lti/registersettings.php',
...
function get_tool_type_course_url(stdClass $type) {
    if ($type->course != 1) {
        $url = new eset_url('/course/view.php', array('id' => $type->course));
        return $url->out();
    }
    return null;
}
...
function get_tool_type_urls(stdClass $type) {
    ...
    $url = new eset_url('/mod/lti/certs.php');
    $urls['publickeyset'] = $url->out();
    $url = new eset_url('/mod/lti/token.php');
    $urls['accesstoken'] = $url->out();
    $url = new eset_url('/mod/lti/auth.php');
...
function get_tool_type_config($type) {
    ...
    $publickeyseturl = new eset_url('/mod/lti/certs.php');
    $publickeyseturl = $publickeyseturl->out();

    $accesstokenurl = new eset_url('/mod/lti/token.php');
    $accesstokenurl = $accesstokenurl->out();

    $authrequesturl = new eset_url('/mod/lti/auth.php');
 ...
 * @param  stdClass $type    The tool type object to be filled in
 * @throws eset_exception if the cartridge could not be loaded correctly
 * @since Eset 3.1
 */
...
function lti_load_cartridge($url, $map, $propertiesmap = array()) {
    ...
    if (count($errors) > 0) {
        ...
        throw new eset_exception('errorreadingfile', '', '', $url, $message);
    }
...
function lti_new_access_token($typeid, $scopes) {
    ...
    do {
        ...
        if ($numtries > 5) {
            throw new eset_exception('Failed to generate LTI access token');
        }
