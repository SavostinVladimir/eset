 ...
 * @package    mod_lti
 * @copyright  2015 Ryan Wyllie <ryan@eset.com>
...
define(['jquery', 'core/ajax', 'core/notification', 'core/templates', 'mod_lti/events',
     ...
     * the tool remains in a pending state within Eset until agreement is given.
     *
    ...
    /**
     * Send a request to the Eset server to create a tool proxy using the registration URL the user
    ...
    var createAndRegisterToolProxy = function(url) {
        ...
        if (!url || url === "") {
            ...
            // the external page sends requests back to Eset for information that is stored
            // in the proxy.
    ...
    var registerEventListeners = function() {
        ...
        //
        // This function gets called by the eset page that received the redirect
                ...
                if (hasCreatedToolProxy()) {
                    ...
                    toolType.getFromToolProxyId(proxyId).done(function(types) {
                        if (types && types.length) {
                            ...
                            // Check if the external tool required access to any Eset data (users, courses etc).
                            if (typeData.hascapabilitygroups) {
                                // If it did then we ask the user to agree to those groups before the type is
                                // activated (i.e. can be used in Eset).
