 ...
 *
 * It contains the great majority of functions defined by Eset
...
function wiki_search_form($cm, $search = '', $subwiki = null) {
    ...
    $data = [
        'action' => new eset_url('/mod/wiki/search.php'),
...
function wiki_extend_navigation(navigation_node $navref, $course, $module, $cm) {
    ...
    if (wiki_can_create_pages($context)) {
        $link = new eset_url('/mod/wiki/create.php', array('action' => 'new', 'swid' => $swid));
...
function wiki_get_extra_capabilities() {
    return array('eset/comment:view', 'eset/comment:post', 'eset/comment:delete');
}
...
function mod_wiki_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \eset_url('/mod/wiki/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
