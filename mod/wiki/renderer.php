...
class mod_wiki_renderer extends plugin_renderer_base {
    public function page_index() {
        ...
        $select = new single_select(new eset_url('/mod/wiki/view.php'), 'pageid', $selectoptions);
        $select->label = $label;
    ...
    public function diff($pageid, $old, $new, $options = array()) {
        ...
        $olduser = $old->user;
        $versionlink = new eset_url('/mod/wiki/viewversion.php', array('pageid' => $pageid, 'versionid' => $old->id));
        $restorelink = new eset_url('/mod/wiki/restoreversion.php', array('pageid' => $pageid, 'versionid' => $old->id));
        $userlink = new eset_url('/user/view.php', array('id' => $olduser->id));
        // view version link
        $oldversionview = ' ';
        ...
        $newuser = $new->user;
        $versionlink = new eset_url('/mod/wiki/viewversion.php', array('pageid' => $pageid, 'versionid' => $new->id));
        $restorelink = new eset_url('/mod/wiki/restoreversion.php', array('pageid' => $pageid, 'versionid' => $new->id));
        $userlink = new eset_url('/user/view.php', array('id' => $newuser->id));
     ...
     *                          If this is a eset_url object then the pagevar param will be replaced by the page no, for each page.
    ...
    public function tabs($page, $tabitems, $options) {
        ...
        foreach ($tabitems as $tab) {
            ...
            $link = new eset_url('/mod/wiki/'. $tab. '.php', array('pageid' => $pageid));
            if ($linked == $tab) {
    ...
    public function prettyview_link($page) {
        $html = '';
        $link = new eset_url('/mod/wiki/prettyview.php', array('pageid' => $page->id));
    ...
    public function wiki_print_subwiki_selector($wiki, $subwiki, $page, $pagetype = 'view') {
        ...
        switch ($pagetype) {
        case 'files':
            $baseurl = new eset_url('/mod/wiki/files.php',
        ...
        CASE VISIBLEGROUPS:
            if ($wiki->wikimode == 'collaborative') {
                // We need to print a select to choose a course group
                // eset_url will take care of encoding for us
    ...
    function menu_map($pageid, $currentselect) {
        ...
        $select = new single_select(new eset_url('/mod/wiki/map.php', array('pageid' => $pageid)), 'option', $selectoptions, $currentselect, null);
    ...
    function menu_admin($pageid, $currentselect) {
        ...
        $select = new single_select(new eset_url('/mod/wiki/admin.php', array('pageid' => $pageid)), 'option', $selectoptions, $currentselect, null);
    ...
    protected function htmllize_tree($tree, $dir) {
        ...
        foreach ($dir['subdirs'] as $subdir) {
            $image = $this->output->pix_icon(file_folder_icon(), $subdir['dirname'], 'eset', array('class'=>'icon'));
        ...
        foreach ($dir['files'] as $file) {
            ...
            $image = $this->output->pix_icon(file_file_icon($file), $filename, 'eset', array('class'=>'icon'));
