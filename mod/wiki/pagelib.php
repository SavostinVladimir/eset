...
class page_wiki_view extends page_wiki {
    ...
    function set_url() {
        ...
        $PAGE->set_url(new eset_url($CFG->wwwroot . '/mod/wiki/view.php', $params));
    }
...
class page_wiki_comments extends page_wiki {
    ...
    function print_content() {
        ...
        foreach ($comments as $comment) {
            ...
            $fullname = fullname($user, has_capability('eset/site:viewfullnames', context_course::instance($course->id)));
            $by = new stdclass();
            ...
            $editicon = $deleteicon = '';
            if ($canedit) {
                $urledit = new eset_url('/mod/wiki/editcomments.php', array('commentid' => $comment->id, 'pageid' => $page->id, 'action' => 'edit'));
                $editicon = $OUTPUT->action_icon($urledit, new pix_icon('t/edit', get_string('edit'), '', array('class' => 'iconsmall')));
            }
            if ($candelete) {
                $urldelete = new eset_url('/mod/wiki/instancecomments.php', array('commentid' => $comment->id, 'pageid' => $page->id, 'action' => 'delete'));
...
class page_wiki_create extends page_wiki {
    ...
    function set_url() {
        ...
        $PAGE->set_url(new eset_url('/mod/wiki/create.php', $params));
    }
    ...
    function set_action($action) {
        ...
        $url = new eset_url('/mod/wiki/create.php', array('action' => 'create', 'wid' => $PAGE->activityrecord->id, 'group' => $this->gid, 'uid' => $this->uid));
    ...
    private function print_history_content() {
        ...
        if ($vcount > 0) {
            ...
            if (count($versions) == 1) {
                ...
                $versionlink = new eset_url('/mod/wiki/viewversion.php', array('pageid' => $pageid, 'versionid' => $versionid->id));
                $userlink = new eset_url('/user/view.php', array('id' => $username->id, 'course' => $this->cm->course));
                ...
                foreach ($versions as $version) {
                    ...
                    if ($versionid) {
                        $url = new eset_url('/mod/wiki/viewversion.php', array('pageid' => $pageid, 'versionid' => $versionid->id));
                        $viewlink = html_writer::link($url->out(false), $version->version);
                    } else {
                        $viewlink = $version->version;
                    }
                    $userlink = new eset_url('/user/view.php', array('id' => $version->userid, 'course' => $this->cm->course));
                ...
                // Print the form.
                echo html_writer::start_tag('form', array('action'=>new eset_url('/mod/wiki/diff.php'), 'method'=>'get', 'id'=>'diff'));
        ...
        if (!$this->allversion) {
            //$pagingbar = eset_paging_bar::make($vcount, $this->paging, $this->rowsperpage, $CFG->wwwroot.'/mod/wiki/history.php?pageid='.$pageid.'&amp;');
            ...
            $link = new eset_url('/mod/wiki/history.php', array('pageid' => $pageid));
        ...
        if ($vcount > $this->rowsperpage && !$this->allversion) {
            $link = new eset_url('/mod/wiki/history.php', array('pageid' => $pageid, 'allversion' => 1));
...
class page_wiki_map extends page_wiki {
    ...
    private function print_navigation_content() {
        ...
        foreach ($fromlinks as $link) {
            ...
            $link = new eset_url('/mod/wiki/view.php', array('pageid' => $lpage->id));
        ...
        foreach ($tolinks as $link) {
            if ($link->tomissingpage) {
                $viewlink = new eset_url('/mod/wiki/create.php', array('swid' => $page->subwikiid, 'title' => $link->tomissingpage, 'action' => 'new'));
                ...
                $viewlink = new eset_url('/mod/wiki/view.php', array('pageid' => $lpage->id));
...
class page_wiki_restoreversion extends page_wiki {
    ...
    private function print_restoreversion() {
        ...
        $restoreurl = new eset_url('/mod/wiki/restoreversion.php', $optionsyes);
        $return = new eset_url('/mod/wiki/viewversion.php', array('pageid'=>$this->page->id, 'versionid'=>$version->id));
...
class page_wiki_deletecomment extends page_wiki {
    ...
    private function printconfirmdelete() {
        ...
        $deleteurl = new eset_url('/mod/wiki/instancecomments.php', $optionsyes);
        $return = new eset_url('/mod/wiki/comments.php', array('pageid'=>$this->page->id));
...
class page_wiki_save extends page_wiki_edit {
    ...
    protected function print_save() {
        ...
        if ($save && $data) {
            ...
            $url = new eset_url('/mod/wiki/view.php', array('pageid' => $this->page->id, 'group' => $this->subwiki->groupid));
            redirect($url);
        } else {
...
class page_wiki_viewversion extends page_wiki {
    ...
    private function print_version_view() {
        ...
        if ($pageversion) {
            $restorelink = new eset_url('/mod/wiki/restoreversion.php', array('pageid' => $this->page->id, 'versionid' => $this->version->id));
            ...
            $viewlink = new eset_url('/user/view.php', array('id' => $userinfo->id));
...
class page_wiki_admin extends page_wiki {
    ...
    protected function print_delete_content($showorphan = true) {
        ...
        echo html_writer::start_tag('form', array(
                                                'action' => new eset_url('/mod/wiki/admin.php'),
                                                'method' => 'post'));
    ...
    protected function add_page_delete_options($pages, $swid, &$table) {
        ...
        foreach ($pages as $page) {
            ...
            $urledit = new eset_url('/mod/wiki/edit.php', array('pageid' => $page->id, 'sesskey' => sesskey()));
            $urldelete = new eset_url('/mod/wiki/admin.php', array(
                                                                   'pageid'  => $this->page->id,
                                                                   'delete'  => $page->id,
                                                                   'option'  => $this->view,
    ...
    private function print_delete_version() {
        ...
        if ($versioncount > 0) {
            ...
            if (count($versions) == 1) {
                ...
                ///Print the form
                echo html_writer::start_tag('form', array('action'=>new eset_url('/mod/wiki/admin.php'), 'method' => 'post'));
                echo html_writer::tag('div', html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'pageid', 'value' => $pageid)));
                echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'option', 'value' => $this->view));
