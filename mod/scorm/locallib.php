...
function scorm_print_launch ($user, $scorm, $action, $cm) {
    ...
    if ($orgs = $DB->get_records_select_menu('scorm_scoes', 'scorm = ? AND '.
        ...
        if (count($orgs) > 1) {
            $select = new single_select(new eset_url($action), 'organization', $orgs, $organization, null);
...
function scorm_simple_play($scorm, $user, $context, $cmid) {
    ...
    if ($scoes) {
        ...
        if ($scorm->skipview >= SCORM_SKIPVIEW_FIRST) {
            ...
            $url = new eset_url('/mod/scorm/player.php', array('a' => $scorm->id, 'currentorg' => $orgidentifier));
 ...
 * @param object $scorm a eset scrom object - mdl_scorm
...
function scorm_get_attempt_status($user, $scorm, $cm='') {
    ...
    if (!empty($cm)) {
        ...
        if (has_capability('mod/scorm:deleteownresponses', $context) &&
            ...
            $deleteurl = new eset_url($PAGE->url, array('action' => 'delete', 'sesskey' => sesskey()));
...
function scorm_get_toc($user, $scorm, $cmid, $toclink=TOCJSLINK, $currentorg='', $scoid='', $mode='normal',
    ...
    if ($scorm->hidetoc == SCORM_TOC_POPUP) {
        ...
        $url = new eset_url('/mod/scorm/player.php?a='.$scorm->id.'&currentorg='.$currentorg.$modestr);
...
function scorm_require_available($scorm, $checkviewreportcap = false, $context = null) {
    ...
    if (!$available) {
        ...
        throw new eset_exception($reason, 'scorm', '', $warnings[$reason]);
    }
...
function scorm_get_sco_and_launch_url($scorm, $scoid, $context) {
    ...
    if (scorm_external_link($sco->launch)) {
        ...
        // Note: do not convert this to use eset_url().
        // SCORM does not work without slasharguments and eset_url() encodes querystring vars.
