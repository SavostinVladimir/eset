...
$url = new eset_url('/mod/scorm/report/userreporttracks.php', array('id' => $id,
    'user' => $userid,
    'attempt' => $attempt,
    'scoid' => $scoid));
...
if (!groups_user_groups_visible($course, $userid, $cm)) {
    throw new eset_exception('nopermissiontoshow');
}
...
$PAGE->navbar->add($strreport, new eset_url('/mod/scorm/report.php', array('id' => $cm->id)));
$PAGE->navbar->add("$strattempt $attempt - ".fullname($user),
    new eset_url('/mod/scorm/report/userreport.php', array('id' => $id, 'user' => $userid, 'attempt' => $attempt)));
