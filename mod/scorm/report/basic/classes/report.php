...
class report extends \mod_scorm\report {
    ...
    public function display($scorm, $cm, $course, $download) {
        ...
        $PAGE->set_url(new \eset_url($PAGE->url, array('attemptsmode' => $attemptsmode)));
        ...
        if ($groupmode = groups_get_activity_groupmode($cm)) { // Groups are being used.
            if (!$download) {
                groups_print_activity_menu($cm, new \eset_url($PAGE->url, $displayoptions));
            }
        }
        ...
        if ( !$nostudents ) {
            ...
            if (!$download) {
                ...
                $filename .= ".xls";
                // Creating a workbook.
                $workbook = new \EsetExcelWorkbook("-");
                // Sending HTTP headers.
                $workbook->send($filename);
                // Creating the first worksheet.
                $sheettitle = get_string('report', 'scorm');
            ...
            if ($attempts) {
                foreach ($attempts as $scouser) {
                    ...
                    if (!$download) {
                        $url = new \eset_url('/user/view.php', array('id' => $scouser->userid, 'course' => $course->id));
                    ...
                    if (empty($timetracks->start)) {
                        ...
                        if (!$download) {
                            $url = new \eset_url('/mod/scorm/report/userreport.php', array('id' => $cm->id,
                        ...
                        foreach ($scoes as $sco) {
                            if ($sco->launch != '') {
                                if ($trackdata = scorm_get_tracks($sco->id, $scouser->userid, $scouser->attempt)) {
                                    ...
                                    if (!$download) {
                                        $url = new \eset_url('/mod/scorm/report/userreporttracks.php', array('id' => $cm->id,
                ...
                if (!$download) {
                    ...
                    if (!empty($attempts)) {
                        ...
                        echo $OUTPUT->single_button(new \eset_url($PAGE->url,
