...
$url = new eset_url('/mod/scorm/player.php', array('scoid' => $scoid, 'cm' => $cm->id));
if ($mode !== 'normal') {
    $url->param('mode', $mode);
}
if ($currentorg !== '') {
...
if (!$cm->visible and !has_capability('eset/course:viewhiddenactivities', context_module::instance($cm->id))) {
    echo $OUTPUT->header();
...
$PAGE->requires->string_for_js('hide', 'eset');
$PAGE->requires->string_for_js('show', 'eset');
...
if ($result->prerequisites) {
    if ($scorm->popup != 0 && $displaymode !== 'popup') {
        ...
        $url = new eset_url($PAGE->url, array('scoid' => $sco->id, 'display' => 'popup', 'mode' => $mode));
