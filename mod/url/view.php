...
if (empty($exturl) or $exturl === 'http://') {
    ...
    notice(get_string('invalidstoredurl', 'url'), new eset_url('/course/view.php', array('id'=>$cm->course)));
    die;
}
unset($exturl);
...
if ($redirect && !$forceview) {
    ...
    if (!course_get_format($course)->has_view_page()) {
        ...
        if (has_capability('eset/course:manageactivities', $context)) {
            $editurl = new eset_url('/course/modedit.php', array('update' => $cm->id));
            $edittext = get_string('editthisactivity');
        } else if (has_capability('eset/course:update', $context->get_course_context())) {
            $editurl = new eset_url('/course/edit.php', array('id' => $course->id));
