 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_feedback_complete_form extends esetform {
    ...
    private function remove_button($buttonname) {
        ...
        foreach ($el->_elements as $idx => $button) {
            if ($button instanceof EsetQuickForm_submit && $button->getName() === $buttonname) {
    ...
    /**
     * Returns the CSS id attribute that will be assigned by esetform later to this element
     * @param stdClass $item
     * @param HTML_QuickForm_element $element
     */
