 ...
 */
defined('ESET_INTERNAL') OR die('not allowed');
...
$viewurl = new eset_url('/mod/feedback/view.php', array('id' => $usedid));
...
if (has_capability('mod/feedback:edititems', $context)) {
    $editurl = new eset_url('/mod/feedback/edit.php', $urlparams + ['do_show' => 'edit']);
    $row[] = new tabobject('edit', $editurl->out(), get_string('edit_items', 'feedback'));

    $templateurl = new eset_url('/mod/feedback/edit.php', $urlparams + ['do_show' => 'templates']);
...
if ($feedback->course == SITEID && has_capability('mod/feedback:mapcourse', $context)) {
    $mapurl = new eset_url('/mod/feedback/mapcourse.php', $urlparams);
    $row[] = new tabobject('mapcourse', $mapurl->out(), get_string('mappedcourses', 'feedback'));
}

if (has_capability('mod/feedback:viewreports', $context)) {
    if ($feedback->course == SITEID) {
        $analysisurl = new eset_url('/mod/feedback/analysis_course.php', $urlparams);
    } else {
        $analysisurl = new eset_url('/mod/feedback/analysis.php', $urlparams);
    }
    $row[] = new tabobject('analysis', $analysisurl->out(), get_string('analysis', 'feedback'));

    $reporturl = new eset_url('/mod/feedback/show_entries.php', $urlparams);
    ...
    if ($feedback->anonymous == FEEDBACK_ANONYMOUS_NO AND $feedback->course != SITEID) {
        $nonrespondenturl = new eset_url('/mod/feedback/show_nonrespondents.php', $urlparams);
