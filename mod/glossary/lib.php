...
function glossary_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid = 0, $groupid = 0) {
    ...
    /*
    $accessallgroups = has_capability('eset/site:accessallgroups', $context);
...
function glossary_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    echo html_writer::link(new eset_url('/mod/glossary/view.php', $urlparams),
            strip_tags($activity->content->concept), $class);
    echo html_writer::end_tag('div');

    $url = new eset_url('/user/view.php', array('course'=>$courseid, 'id'=>$activity->user->id));
...
function glossary_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($entries as $entry) {
        if ($entrycount < GLOSSARY_RECENT_ACTIVITY_LIMIT) {
            ...
            $link = new eset_url($CFG->wwwroot.'/mod/glossary/view.php' , $urlparams);
...
/**
 * Function to be run periodically according to the eset cron
...
function glossary_get_entries_search($concept, $courseid) {
    ...
    if (has_capability('eset/course:viewhiddenactivities', context_system::instance())) {
...
function glossary_print_entry_icons($course, $cm, $glossary, $entry, $mode='',$hook='', $type = 'print') {
    ...
    if ($entry->approved || has_capability('mod/glossary:approve', $context)) {
        ...
        $return .= \html_writer::link(
            new \eset_url('/mod/glossary/showentry.php', ['eid' => $entry->id]),
...
function  glossary_print_entry_approval($cm, $entry, $mode, $align="right", $insidetable=true) {
    ...
    if ($mode == 'approval' and !$entry->approved) {
        ...
        echo $OUTPUT->action_icon(
            new eset_url('approve.php', array('eid' => $entry->id, 'mode' => $mode, 'sesskey' => sesskey())),
...
function glossary_print_categories_menu($cm, $glossary, $hook, $category) {
    ...
    $fmtoptions = array(
     ...
     if (has_capability('mod/glossary:managecategories', $context)) {
             ...
             $options['mode'] = 'cat';
             $options['hook'] = $hook;
             echo $OUTPUT->single_button(new eset_url("editcategories.php", $options), get_string("editcategories","glossary"), "get");
     }
     echo '</td>';
     ...
     $select = new single_select(new eset_url("/mod/glossary/view.php", array('id'=>$cm->id, 'mode'=>'cat')), 'hook', $menu, $selected, null, "catmenu");
 ...
 * Fully compatible with Eset's print_paging_bar() function. Perhaps some day this
    ...
    foreach ($showtabs as $showtabkey => $showtabvalue) {
        ...
        switch($showtabvalue) {
            case GLOSSARY_STANDARD :
                $navigation->add(get_string('standardview', 'glossary'), new eset_url('/mod/glossary/view.php',
                        array('id' => $cm->id, 'mode' => 'letter')));
                break;
            case GLOSSARY_CATEGORY :
                $navigation->add(get_string('categoryview', 'glossary'), new eset_url('/mod/glossary/view.php',
                        array('id' => $cm->id, 'mode' => 'cat')));
                break;
            case GLOSSARY_DATE :
                $navigation->add(get_string('dateview', 'glossary'), new eset_url('/mod/glossary/view.php',
                        array('id' => $cm->id, 'mode' => 'date')));
                break;
            case GLOSSARY_AUTHOR :
                $navigation->add(get_string('authorview', 'glossary'), new eset_url('/mod/glossary/view.php',
...
function glossary_extend_settings_navigation(settings_navigation $settings, navigation_node $glossarynode) {
    ...
    if (has_capability('mod/glossary:import', $PAGE->cm->context)) {
        $glossarynode->add(get_string('importentries', 'glossary'), new eset_url('/mod/glossary/import.php', array('id'=>$PAGE->cm->id)));
    }

    if (has_capability('mod/glossary:export', $PAGE->cm->context)) {
        $glossarynode->add(get_string('exportentries', 'glossary'), new eset_url('/mod/glossary/export.php', array('id'=>$PAGE->cm->id, 'mode'=>$mode, 'hook'=>$hook)));
    }
    ...
    if (has_capability('mod/glossary:approve', $PAGE->cm->context) && ($hiddenentries = $DB->count_records('glossary_entries', array('glossaryid'=>$PAGE->cm->instance, 'approved'=>0)))) {
        $glossarynode->add(get_string('waitingapproval', 'glossary'), new eset_url('/mod/glossary/view.php', array('id'=>$PAGE->cm->id, 'mode'=>'approval')));
    }

    if (has_capability('mod/glossary:write', $PAGE->cm->context)) {
        $glossarynode->add(get_string('addentry', 'glossary'), new eset_url('/mod/glossary/edit.php', array('cmid'=>$PAGE->cm->id)));
    }
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->glossary_enablerssfeeds) && $glossary->rsstype && $glossary->rssarticles && has_capability('mod/glossary:view', $PAGE->cm->context)) {
        ...
        $url = new eset_url(rss_get_url($PAGE->cm->context->id, $USER->id, 'mod_glossary', $glossary->id));
...
function mod_glossary_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \eset_url('/mod/glossary/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
