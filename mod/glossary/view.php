...
$url = new eset_url('/mod/glossary/view.php', array('id'=>$cm->id));
if (isset($mode)) {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
/// The print icon
    if ( $showcommonelements and $mode != 'search') {
        if (has_capability('mod/glossary:manageentries', $context) or $glossary->allowprintview) {
            ...
            $printurl = new eset_url('/mod/glossary/print.php', $params);
...
if ($showcommonelements ) {
    ...
    $data = [
        'action' => new eset_url('/mod/glossary/view.php'),
...
if ($allentries) {
    ...
    //Build paging bar
    $baseurl = new eset_url('/mod/glossary/view.php', ['id' => $id, 'mode' => $mode, 'hook' => $hook,
        ...
        if ($printpivot) {
            ...
            if ($currentpivot != $upperpivot) {
                ...
                // printing the user icon if defined (only when browsing authors)
                    ...
                    $pivottoshow = fullname($user, has_capability('eset/site:viewfullnames', context_course::instance($course->id)));
                } else {
                    echo '<th >';
                }
    ...
    if ($allentries instanceof eset_recordset) {
        $allentries->close();
    }
}
if ( !$entriesshown ) {
