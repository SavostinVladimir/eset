...
$url = new eset_url('/mod/forum/index.php', array('id' => $id));
if ($subscribe !== null) {
    require_sesskey();
...
if (!is_null($subscribe)) {
    if (isguestuser() or !$showsubscriptioncolumns) {
        ...
        redirect(
                new eset_url('/mod/forum/index.php', array('id' => $id)),
    ...
    foreach ($modinfo->get_instances_of('forum') as $forumid => $cm) {
        ...
        if (!\mod_forum\subscriptions::is_forcesubscribed($forum)) {
            ...
            $canmanageactivities = has_capability('eset/course:manageactivities', $coursecontext, $USER->id);
    ...
    $returnto = forum_go_back_to(new eset_url('/mod/forum/index.php', array('id' => $course->id)));
        ...
        if ($usetracking) {
            if ($forum->trackingtype == FORUM_TRACKING_OFF) {
                ...
                if (($forum->trackingtype == FORUM_TRACKING_FORCED) && ($CFG->forum_allowforcedreadtracking)) {
                    ...
                    $aurl = new eset_url('/mod/forum/settracking.php', array(
                            'id' => $forum->id,
                            'sesskey' => sesskey(),
                        ));
                    if (!isset($untracked[$forum->id])) {
            ...
            if ($usetracking) {
                if ($forum->trackingtype == FORUM_TRACKING_OFF) {
                    ...
                    if (($forum->trackingtype == FORUM_TRACKING_FORCED) && ($CFG->forum_allowforcedreadtracking)) {
                        ...
                        $aurl = new eset_url('/mod/forum/settracking.php', array('id' => $forum->id));
                        if (!isset($untracked[$forum->id])) {
                            $trackedlink = $OUTPUT->single_button($aurl, $stryes, 'post', array('title' => $strnotrackforum));
                        } else {
                            $trackedlink = $OUTPUT->single_button($aurl, $strno, 'post', array('title' => $strtrackforum));
...
if (!isguestuser() && isloggedin() && $showsubscriptioncolumns) {
    ...
    $subscriptionlink = new eset_url('/mod/forum/index.php', [
        'id'        => $course->id,
        'sesskey'   => sesskey(),
    ]);
