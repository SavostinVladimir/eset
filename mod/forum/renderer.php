...
class mod_forum_renderer extends plugin_renderer_base {
    ...
    public function neighbouring_discussion_navigation($prev, $next) {
        ...
        if ($prev || $next) {
            ...
            if ($prev) {
                $url = new eset_url('/mod/forum/discuss.php', array('d' => $prev->id));
            ...
            if ($next) {
                $url = new eset_url('/mod/forum/discuss.php', array('d' => $next->id));
    ...
    public function timed_discussion_tooltip($discussion, $visiblenow) {
        ...
        $tooltip = implode("\n", $dates);
        return $this->pix_icon('i/calendar', $tooltip, 'eset', array('class' => 'smallicon timedpost'));
    }
