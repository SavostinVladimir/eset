...
function forum_supports($feature) {
    switch($feature) {
        ...
        case FEATURE_BACKUP_ESET2:          return true;
...
function forum_user_outline($course, $user, $mod, $forum) {
    ...
    if (!empty($grades->items[0]->grades)) {
        ...
        if (!$grade->hidden || has_capability('eset/grade:viewhidden', context_course::instance($course->id))) {
...
/**
 * @deprecated since Eset 3.3, when the block_course_overview block was removed.
 */
...
function forum_is_user_group_discussion(cm_info $cm, $discussiongroupid) {
    ...
    if (has_capability('eset/site:accessallgroups', context_module::instance($cm->id)) ||
...
function forum_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($printposts as $post) {
        ...
        $discussionurl = new eset_url('/mod/forum/discuss.php', ['d' => $post->discussion]);
...
function forum_get_readable_forums($userid, $courseid=0) {
    ...
    foreach ($courses as $course) {
        ...
        foreach ($modinfo->instances['forum'] as $forumid => $cm) {
         ...
         /// group access
            if (groups_get_activity_groupmode($cm, $course) == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $context)) {
...
function forum_get_discussions($cm, $forumsort="", $fullpost=true, $unused=-1, $limit=-1,
    ...
    if ($groupmode) {
        ...
        if ($groupmode == VISIBLEGROUPS or has_capability('eset/site:accessallgroups', $modcontext)) {
            if ($currentgroup) {
...
function forum_get_discussion_neighbours($cm, $discussion, $forum) {
    ...
    $groupselect = '';
    if ($groupmode) {
        if ($groupmode == VISIBLEGROUPS || has_capability('eset/site:accessallgroups', $modcontext)) {
            if ($currentgroup) {
...
function forum_rating_validate($params) {
    ...
    if ($discussion->groupid > 0 and $groupmode = groups_get_activity_groupmode($cm, $course)) {   // Groups are being used
        ...
        if (!groups_is_member($discussion->groupid) and !has_capability('eset/site:accessallgroups', $context)) {
...
function forum_print_discussion_header(&$post, $forum, $group = -1, $datestring = "",
    ...
    $canviewfullnames = has_capability('eset/site:viewfullnames', $modcontext);
...
function forum_get_discussion_subscription_icon($forum, $discussionid, $returnurl = null, $includetext = false) {
    ...
    $subscriptionlink = new eset_url('/mod/forum/subscribe.php', array(
        'sesskey' => sesskey(),
        'id' => $forum->id,
        'd' => $discussionid,
        'returnurl' => $returnurl,
...
function forum_print_mode_form($id, $mode, $forumtype='') {
    ...
    if ($forumtype == 'single') {
        $select = new single_select(
            new eset_url("/mod/forum/view.php",
            array('f' => $id)),
            'mode',
        ...
        $select = new single_select(
            new eset_url("/mod/forum/discuss.php",
            array('d' => $id)),
            'mode',
            forum_get_layout_modes($useexperimentalui),
            $mode,
...
function forum_print_attachments($post, $cm, $type) {
    ...
    if ($files) {
        ...
        foreach ($files as $file) {
            ...
            $iconimage = $OUTPUT->pix_icon(file_file_icon($file), get_mimetype_description($file), 'eset', array('class' => 'icon'));
...
function forum_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    ...
    if (!has_capability('eset/course:managefiles', $context) && $storedfile->get_userid() != $USER->id) {
        return null;
    }
    // Make sure groups allow this user to see this file
    if ($discussion->groupid > 0 && !has_capability('eset/site:accessallgroups', $context)) {
        $groupmode = groups_get_activity_groupmode($cm, $course);
...
function forum_add_new_post($post, $mform, $unused = null) {
    ...
    // Let Eset know that assessable content is uploaded (eg for plagiarism detection)
...
function forum_post_subscription($fromform, $forum, $discussion) {
    ...
    if (\mod_forum\subscriptions::is_forcesubscribed($forum)) {
        ...
        if ($subscribed && !has_capability('eset/course:manageactivities', context_course::instance($forum->course), $USER->id)) {
...
function forum_get_subscribe_link($forum, $context, $messages = array(), $cantaccessagroup = false, $unused1 = true,
    ...
    if (\mod_forum\subscriptions::is_forcesubscribed($forum)) {
        ...
        $url = new eset_url('/mod/forum/subscribe.php', $options);
...
// $forum is an object
    ...
    if (!$groupmode or has_capability('eset/site:accessallgroups', $context)) {
        return true;
    }
...
function forum_user_can_post($forum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    ...
    if (has_capability('eset/site:accessallgroups', $context)) {
        return true;
    }

    if ($groupmode == VISIBLEGROUPS) {
        if ($discussion->groupid == -1) {
            // allow students to reply to all participants discussions - this was not possible in Eset <1.8
            return true;
        }
...
function forum_user_can_see_group_discussion($discussion, $cm, $context) {
    ...
    if ($discussion->groupid > 0) {
        $groupmode = groups_get_activity_groupmode($cm);
        if ($groupmode == SEPARATEGROUPS) {
            return groups_is_member($discussion->groupid) || has_capability('eset/site:accessallgroups', $context);
        }
    }
...
function forum_user_can_see_post($forum, $discussion, $post, $user = null, $cm = null, $checkdeleted = true) {
    ...
    if (!$canviewdiscussion && !has_all_capabilities(array('eset/user:viewdetails', 'eset/user:readuserposts'), context_user::instance($post->userid))) {
        return false;
    }
...
function forum_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0)  {
    ...
    $accessallgroups = has_capability('eset/site:accessallgroups', $cm_context);
...
function forum_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    $discussionurl = new eset_url('/mod/forum/discuss.php', ['d' => $content->discussion]);
    ...
    if ($authorhidden) {
        ...
        $userurl = new eset_url('/user/view.php');
...
/**
 * This function is used by the reset_course_userdata function in esetlib.
...
function forum_get_extra_capabilities() {
    return ['eset/rating:view', 'eset/rating:viewany', 'eset/rating:viewall', 'eset/rating:rate'];
}
...
function forum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    ...
    if (isloggedin() && !isguestuser()) {
        ...
        foreach ($reportnames as $reportname) {
            if (has_capability("forumreport/{$reportname}:view", $PAGE->cm->context)) {
                ...
                $reportlink = new eset_url("/mod/forum/report/{$reportname}/index.php", $reportlinkparams);
    ...
    if ($canmanage) {
        ...
        $allowchoice = $mode->add(get_string('subscriptionoptional', 'forum'), new eset_url('/mod/forum/subscribe.php', array('id'=>$forumobject->id, 'mode'=>FORUM_CHOOSESUBSCRIBE, 'sesskey'=>sesskey())), navigation_node::TYPE_SETTING);
        $forceforever = $mode->add(get_string("subscriptionforced", "forum"), new eset_url('/mod/forum/subscribe.php', array('id'=>$forumobject->id, 'mode'=>FORUM_FORCESUBSCRIBE, 'sesskey'=>sesskey())), navigation_node::TYPE_SETTING);
        $forceinitially = $mode->add(get_string("subscriptionauto", "forum"), new eset_url('/mod/forum/subscribe.php', array('id'=>$forumobject->id, 'mode'=>FORUM_INITIALSUBSCRIBE, 'sesskey'=>sesskey())), navigation_node::TYPE_SETTING);
        $disallowchoice = $mode->add(get_string('subscriptiondisabled', 'forum'), new eset_url('/mod/forum/subscribe.php', array('id'=>$forumobject->id, 'mode'=>FORUM_DISALLOWSUBSCRIBE, 'sesskey'=>sesskey())), navigation_node::TYPE_SETTING);
    ...
    if ($cansubscribe) {
        ...
        $url = new eset_url('/mod/forum/subscribe.php', array('id'=>$forumobject->id, 'sesskey'=>sesskey()));
        ...
        if (isset($discussionid)) {
            ...
            $url = new eset_url('/mod/forum/subscribe.php', array(
                    'id' => $forumobject->id,
                    'sesskey' => sesskey(),
                    'd' => $discussionid,
                    'returnurl' => $PAGE->url->out(),
    ...
    if (has_capability('mod/forum:viewsubscribers', $PAGE->cm->context)){
        $url = new eset_url('/mod/forum/subscribers.php', array('id'=>$forumobject->id));
    ...
    if ($enrolled && forum_tp_can_track_forums($forumobject)) { // keep tracking info for users with suspended enrolments
        if ($forumobject->trackingtype == FORUM_TRACKING_OPTIONAL
            ...
            $url = new eset_url('/mod/forum/settracking.php', array(
                    'id' => $forumobject->id,
                    'sesskey' => sesskey(),
                ));
            $forumnode->add($linktext, $url, navigation_node::TYPE_SETTING);
    ...
    if ($enablerssfeeds && $forumobject->rsstype && $forumobject->rssarticles && $hascourseaccess) {
        ...
        $url = new eset_url(rss_get_url($PAGE->cm->context->id, $userid, "mod_forum", $forumobject->id));
    ...
    if ($capabilitymanager->can_export_forum($USER)) {
        $url = new eset_url('/mod/forum/export.php', ['id' => $forumobject->id]);
 ...
 *
 * @global eset_database $DB The database connection
...
function forum_get_posts_by_user($user, array $courses, $musthaveaccess = false, $discussionsonly = false, $limitfrom = 0, $limitnum = 50) {
    ...
    $hascapsonuser = $hascapsonuser && has_all_capabilities(array('eset/user:viewdetails', 'eset/user:readuserposts'), $usercontext);
    ...
    foreach ($courses as $course) {
        ...
        if ($iscurrentuser || $hascapsonuser) {
            ...
            if (groups_get_course_groupmode($course) == SEPARATEGROUPS && $course->groupmodeforce
              && !has_capability('eset/site:accessallgroups', $coursecontext) && !has_capability('eset/site:accessallgroups', $coursecontext, $user->id)) {
    ...
    foreach ($return->courses as $course) {
        ...
        foreach ($modinfo->get_instances_of('forum') as $forumid => $cm) {
            ...
            if (!$iscurrentuser && !$hascapsonuser) {
                ...
                if (groups_get_activity_groupmode($cm, $course) == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $cm->context)) {
...
function forum_set_user_maildigest($forum, $maildigest, $user = null) {
    ...
    if (!isset($digestoptions[$maildigest])) {
        throw new eset_exception('invaliddigestsetting', 'mod_forum');
    }
...
function mod_forum_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $postsurl = new eset_url('/mod/forum/user.php', array('id' => $user->id));
    if (!empty($course)) {
    ...
    $discussionssurl = new eset_url('/mod/forum/user.php', array('id' => $user->id, 'mode' => 'discussions'));
    if (!empty($course)) {
...
function mod_forum_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \eset_url('/mod/forum/view.php', ['id' => $cm->id]),
        $itemcount,
        true
    );
}
