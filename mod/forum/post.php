...
$prefilledpostformat = optional_param('postformat', FORMAT_ESET, PARAM_INT);
...
if (!empty($forum)) {
        ...
        if (!isguestuser()) {
            if (!is_enrolled($coursecontext)) {
                if (enrol_selfenrol_available($course->id)) {
                    ...
                    redirect(new eset_url('/enrol/index.php', array('id' => $course->id,
    ...
    if (!$cm->visible and !has_capability('eset/course:viewhiddenactivities', $modcontext)) {
        redirect(
    ...
    if (!$capabilitymanager->can_reply_to_post($USER, $discussionentity, $parententity)) {
        if (!isguestuser()) {
            ...
            if (forum_discussion_is_locked($forum, $discussion)) {
                redirect(new eset_url('/mod/forum/discuss.php', array('d' => $discussion->id)));
            }
        }
        print_error('nopostforum', 'forum');
    }
    ...
    if ($groupmode == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $modcontext)) {
    ...
    if ($prunemform->is_cancelled()) {
        ...
        $PAGE->navbar->add($subjectstr, new eset_url('/mod/forum/discuss.php', array('d' => $discussion->id)));
...
if ($mformpost->is_cancelled()) {
    ...
    if ($fromform->edit) {
        ...
        $redirectto = new eset_url('/mod/forum/view.php', array('f' => $fromform->forum));
