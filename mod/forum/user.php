...
$url = new eset_url('/mod/forum/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
$hasparentaccess = $isparent && has_all_capabilities(array('eset/user:viewdetails', 'eset/user:readuserposts'), $usercontext);
...
if (empty($result->posts)) {
    ...
    if ($iscurrentuser) {
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'forum'), new eset_url('/mod/forum/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'forum'), new eset_url('/mod/forum/user.php',
