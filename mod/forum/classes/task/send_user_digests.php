...
/**
 * Adhoc task to send eset forum digests for the specified user.
 *
 * @package    mod_forum
 * @copyright  2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class send_user_digests extends \core\task\adhoc_task {
    ...
    public function execute() {
        ...
        foreach ($this->discussions as $discussion) {
            ...
            if (!isset($this->viewfullnames[$forum->id])) {
                $this->viewfullnames[$forum->id] = has_capability('eset/site:viewfullnames', $modcontext, $this->recipient->id);
            }
    ...
    protected function add_message_header() {
        ...
        $headerdata = (object) [
            ...
            'userprefs' => (new \eset_url('/user/forum.php', [
    ...
    protected function add_discussion_header($discussion, $forum, $course) {
        ...
        $this->discussiontext .= "\n";
        $this->discussiontext .= new \eset_url('/mod/forum/discuss.php', [
                'd' => $discussion->id,
            ]);
        $this->discussiontext .= "\n";
