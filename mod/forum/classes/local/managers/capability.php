...
class capability {
    ...
    public function can_access_all_groups(stdClass $user) : bool {
        return has_capability('eset/site:accessallgroups', $this->get_context(), $user);
    }
        ...
        switch ($this->forum->get_type()) {
            ...
            case 'single':
                if ($discussion->is_first_post($post)) {
                    return has_capability('eset/course:manageactivities', $context, $user);
                }
                break;
        }
    ...
    public function validate_delete_post(stdClass $user, discussion_entity $discussion, post_entity $post,
        ...
        if ($forum->get_type() == 'single' && $discussion->is_first_post($post)) {
            ...
            throw new eset_exception('cannotdeletepost', 'forum');
        }
        ...
        if ($post->get_total_score()) {
            throw new eset_exception('couldnotdeleteratings', 'rating');
        }

        if ($hasreplies && !has_capability('mod/forum:deleteanypost', $context, $user)) {
            throw new eset_exception('couldnotdeletereplies', 'forum');
        }
    }
    ...
    public function can_manage_forum(stdClass $user) {
        return has_capability('eset/course:manageactivities', $this->get_context(), $user);
    }
    ...
    public function can_manage_tags(stdClass $user) : bool {
        return has_capability('eset/tag:manage', context_system::instance(), $user);
    }
