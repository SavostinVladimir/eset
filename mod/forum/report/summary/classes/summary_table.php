...
class summary_table extends table_sql {
    ...
    public function col_export(\stdClass $data): string {
        ...
        $buttoncontext = [
            'url' => new \eset_url('/mod/forum/export.php', $params),
    ...
    protected function get_filter_groups(array $groups): array {
        ...
        foreach ($this->cms as $cm) {
            ...
            if (!$allgroups) {
                ...
                $aag = has_capability('eset/site:accessallgroups', $this->forumcontexts[$cm->id]);
    ...
    public function wrap_html_finish(): void {
        ...
        $perpageselect = new \single_select(new \eset_url(''), 'perpage',
                $perpageoptions, $selected, null, 'selectperpage');
        $perpageselect->set_label(get_string('perpage', 'eset'));
