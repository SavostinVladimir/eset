...
$url = new eset_url('/mod/forum/subscribe.php', array('id'=>$id));
if (!is_null($mode)) {
    $url->param('mode', $mode);
}
if ($user !== 0) {
...
if ($groupmode && !$issubscribed && !has_capability('eset/site:accessallgroups', $context)) {
...
if (is_null($mode) and !is_enrolled($context, $USER, '', true)) {   // Guests and visitors can't subscribe - only enrolled
    ...
    if (isguestuser()) {
        ...
        echo $OUTPUT->confirm(get_string('subscribeenrolledonly', 'forum').'<br /><br />'.get_string('liketologin'),
                     get_login_url(), new eset_url('/mod/forum/view.php', array('f'=>$id)));
        echo $OUTPUT->footer();
        exit;
    } else {
        // There should not be any links leading to this place, just redirect.
        redirect(
                new eset_url('/mod/forum/view.php', array('f'=>$id)),
...
if ($issubscribed) {
    if (is_null($sesskey)) {
        ...
        $viewurl = new eset_url('/mod/forum/view.php', array('f' => $id));
        if ($discussionid) {
            $a = new stdClass();
