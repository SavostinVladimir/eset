...
if (!$search || $showform) {
    ...
    $PAGE->navbar->add($strforums, new eset_url('/mod/forum/index.php', array('id'=>$course->id)));
...
$PAGE->navbar->add($strsearch, new eset_url('/mod/forum/search.php', array('id'=>$course->id)));
...
$url    = new eset_url("/mod/forum/search.php", $params);
foreach ($tags as $tag) {
    $url .= "&tags[]=$tag";
}
...
$url = new eset_url('search.php', array('search' => $search, 'id' => $course->id, 'perpage' => $perpage));
