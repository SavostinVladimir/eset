...
/**
 * @deprecated since Eset 1.1 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 1.5 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 1.6 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 1.7 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 1.9 MDL-13303 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.0 MDL-21657 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.0 MDL-14632 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.0 MDL-14113 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.0 MDL-23479 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.3 MDL-33166 - please do not use this function any more.
 */
...
/**
 * @deprecated since Eset 2.4 use forum_user_can_see_post() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::is_subscribed() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::subscribe_user() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::unsubscribe_user() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::fetch_subscribed_users() instead
  */
 ...
 *
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::is_forcesubscribed() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::set_subscription_mode() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::get_subscription_mode() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::is_subscribed in combination wtih
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::get_unsubscribable_forums() instead
 */
...
/**
 * @deprecated since Eset 2.8 use \mod_forum\subscriptions::get_potential_subscribers() instead
 */
 ...
 * @deprecated since Eset 3.0 use \mod_forum\output\forum_post_email instead
 */
function forum_make_mail_text($course, $cm, $forum, $discussion, $post, $userfrom, $userto, $bare = false, $replyaddress = null) {
    ...
    $renderable->viewfullnames = has_capability('eset/site:viewfullnames', $modcontext, $userto->id);
...
function forum_print_post($post, $discussion, $forum, &$cm, $course, $ownpost=false, $reply=false, $link=false,
    ...
    if (!isset($cm->cache->caps)) {
        ...
        $cm->cache->caps['eset/site:viewfullnames']  = has_capability('eset/site:viewfullnames', $modcontext);
    ...
    $discussionlink = new eset_url('/mod/forum/discuss.php', array('d'=>$post->discussion));
    ...
    $postuser->fullname    = fullname($postuser, $cm->cache->caps['eset/site:viewfullnames']);
    $postuser->profilelink = new eset_url('/user/view.php', array('id'=>$post->userid, 'course'=>$course->id));
    ...
    // Add a permalink.
    $permalink = new eset_url($discussionlink);
    ...
    if ($istracked && $CFG->forum_usermarksread && isloggedin()) {
        $url = new eset_url($discussionlink, array('postid'=>$post->id, 'mark'=>'unread'));
        $text = $str->markunread;
        if (!$postisread) {
            $url->param('mark', 'read');
            $text = $str->markread;
    ...
    if ($post->parent) {
        $url = new eset_url($discussionlink);
        if ($str->displaymode == FORUM_MODE_THREADED) {
            $url->param('parent', $post->parent);
        } else {
    ...
    if ($forum->type == 'single' and $discussion->firstpost == $post->id) {
        if (has_capability('eset/course:manageactivities', $modcontext)) {
            // The first post in single simple is the forum description.
            $commands[] = array('url'=>new eset_url('/course/modedit.php', array('update'=>$cm->id, 'sesskey'=>sesskey(), 'return'=>1)), 'text'=>$str->edit);
        }
    } else if (($ownpost && $age < $CFG->maxeditingtime) || $cm->cache->caps['mod/forum:editanypost']) {
        $commands[] = array('url'=>new eset_url('/mod/forum/post.php', array('edit'=>$post->id)), 'text'=>$str->edit);
    }

    if ($cm->cache->caps['mod/forum:splitdiscussions'] && $post->parent && $forum->type != 'single') {
        $commands[] = array('url'=>new eset_url('/mod/forum/post.php', array('prune'=>$post->id)), 'text'=>$str->prune, 'title'=>$str->pruneheading);
    }
    ...
    if ($forum->type == 'single' and $discussion->firstpost == $post->id) {
        ...
        $commands[] = array('url'=>new eset_url('/mod/forum/post.php', array('delete'=>$post->id)), 'text'=>$str->delete);
    }

    if ($reply) {
        $commands[] = array('url'=>new eset_url('/mod/forum/post.php#mformforum', array('reply'=>$post->id)), 'text'=>$str->reply);
    }
    ...
    if ($shortenpost) {
        ...
        $postcontent .= html_writer::tag('div', '('.get_string('numwords', 'eset', count_words($post->message)).')',
        ...
        if (!empty($forum->displaywordcount)) {
            $postcontent .= html_writer::tag('div', get_string('numwords', 'eset', count_words($postcontent)),
                array('class'=>'post-word-count'));
        }
    ...
    if ($link) {
        ...
        if (!empty($discussion->unread) && $discussion->unread !== '-') {
            ...
            $unreadlink = new eset_url($discussionlink, null, 'unread');
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            if ($depth > 0) {
                ...
                echo html_writer::link(new eset_url('/mod/forum/discuss.php', [
                        'd' => $post->discussion,
                        'parent' => $post->id,
                    ]), $subject);
                echo $byline;
...
function forum_print_latest_discussions($course, $forum, $maxdiscussions = -1, $displayformat = 'plain', $sort = '',
    ...
    if ($canstart) {
        ...
        $button = new single_button(new eset_url('/mod/forum/post.php', ['forum' => $forum->id]), $buttonadd, 'get');
        ...
        $ignore = true;
    } else if ($groupmode and !has_capability('eset/site:accessallgroups', $context)) {
 ...
 * @return int
 * @deprecated since Eset 3.7
 * @todo MDL-65252 This will be removed in Eset 3.11
 */
