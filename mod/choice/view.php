...
$url = new eset_url('/mod/choice/view.php', array('id'=>$id));
if ($action !== '') {
...
if (!$choiceformshown) {
    ...
    if (isguestuser()) {
        ...
        echo $OUTPUT->confirm(get_string('noguestchoose', 'choice').'<br /><br />'.get_string('liketologin'),
                     get_login_url(), new eset_url('/course/view.php', array('id'=>$course->id)));
        ...
        echo $OUTPUT->single_button(new eset_url('/enrol/index.php?', array('id'=>$course->id)), get_string('enrolme', 'core_enrol', $courseshortname));
