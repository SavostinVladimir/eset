 ...
 * pages.
 *
 * This file is part of the Database module for Eset
 *
...
$PAGE->set_url(new eset_url('/mod/data/preset.php', array('d'=>$data->id)));
...
foreach ($presets as &$preset) {
    if (!empty($preset->userid)) {
        ...
        if (data_user_can_delete_preset($context, $preset) && $preset->name != 'Image gallery') {
            $delurl = new eset_url('/mod/data/preset.php', array('d'=> $data->id, 'action'=>'confirmdelete', 'fullname'=>$preset->userid.'/'.$preset->shortname, 'sesskey'=>sesskey()));
...
if (optional_param('sesskey', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if ($formdata = $form_importexisting->get_data()) {
        ...
        if ($action == 'confirmdelete') {
            ...
            echo $OUTPUT->confirm($strwarning, new eset_url('preset.php', $optionsyes), new eset_url('preset.php', $optionsno));
            echo $OUTPUT->footer();
            exit(0);
