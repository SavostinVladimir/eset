...
class mod_data_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_view_database() {
        ...
        $esetdata = new \eset_url('/mod/data/view.php', array('id' => $this->cm->id));
        $this->assertEquals($esetdata, $event->get_url());
    ...
    public function test_add_entry() {
        ...
        foreach ($result['entry']['contents'] as $content) {
            ...
            if ($field->type == 'textarea') {
                ...
                $this->assertEquals(FORMAT_ESET, $content['content1']);
                continue;
            }
            if ($field->type == 'url') {
                $this->assertEquals('https://eset.org', $content['content']);
                continue;
            }
            $this->assertEquals('multimenu1##multimenu4', $content['content']);
        }
