...
// Using the mod/data capability "viewalluserpresets" existing in Eset 1.9.x.
// In Eset >= 2, new roles may be introduced and used instead.
...
class data_field_base {     // Base class for Database Field Types (see field/*/field.class.php)
    ...
    function image() {
        ...
        $link = new eset_url('/mod/data/field.php', $params);
        $str = '<a href="'.$link->out().'">';
...
function data_generate_tag_form($recordid = false, $selected = []) {
    ...
    if (has_capability('eset/tag:manage', context_system::instance()) && $showstandard) {
        $url = new eset_url('/tag/manage.php', array('tc' => core_tag_area::get_collection('mod_data',
            'data_records')));
 ...
 * @param int $page
 * @param bool $return
 * @param object $jumpurl a eset_url by which to jump back to the record list (can be null)
 * @return mixed
 */
function data_print_template($template, $records, $data, $search='', $page=0, $return=false, eset_url $jumpurl=null) {
    global $CFG, $DB, $OUTPUT;
    ...
    if (!$jumpurl) {
        $jumpurl = new eset_url('/mod/data/view.php', array('d' => $data->id));
    }
    $jumpurl = new eset_url($jumpurl, array('page' => $page, 'sesskey' => sesskey()));
    ...
    // Replacing special tags (##Edit##, ##Delete##, ##More##)
        ...
        if (has_capability('mod/data:approve', $context) && ($data->approval) && (!$record->approved)) {
            $approveurl = new eset_url($jumpurl, array('approve' => $record->id));
        ...
        $patterns[]='##disapprove##';
        if (has_capability('mod/data:approve', $context) && ($data->approval) && ($record->approved)) {
            $disapproveurl = new eset_url($jumpurl, array('disapprove' => $record->id));
...
function data_rating_validate($params) {
    ...
    if ($groupid > 0 and $groupmode = groups_get_activity_groupmode($cm, $course)) {   // Groups are being used
        ...
        if (!groups_is_member($groupid) and !has_capability('eset/site:accessallgroups', $context)) {
...
function data_convert_to_roles($data, $teacherroles=array(), $studentroles=array(), $cmid=NULL) {
    ...
    switch ($cm->groupmode) {
        ...
        case SEPARATEGROUPS:
            foreach ($studentroles as $studentrole) {
                assign_capability('eset/site:accessallgroups', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('eset/site:accessallgroups', CAP_ALLOW, $teacherrole->id, $context->id);
            }
            break;
        case VISIBLEGROUPS:
            foreach ($studentroles as $studentrole) {
                assign_capability('eset/site:accessallgroups', CAP_ALLOW, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('eset/site:accessallgroups', CAP_ALLOW, $teacherrole->id, $context->id);
            }
...
abstract class data_preset_importer {
    ...
    /**
     * Retreive the contents of a file. That file may either be in a conventional directory of the Eset file storage
     ...
     * @param string $dir the directory to look in. null if using the Eset file storage
    ...
    public function get_preset_settings() {
        ...
        if (!is_directory_a_preset($this->directory)) {
            //maybe the user requested a preset stored in the Eset file storage
...
function data_get_extra_capabilities() {
    return ['eset/rating:view', 'eset/rating:viewany', 'eset/rating:viewall', 'eset/rating:rate',
            'eset/comment:view', 'eset/comment:post', 'eset/comment:delete'];
}
    ...
    if (empty($readcount)) {
        ...
        while ($record = $cir->next()) {
            ...
            if ($recordid = data_add_record($data, 0, $authorid)) {  // Add instance to data_record.
                ...
                $recordsadded++;
                print get_string('added', 'eset', $recordsadded) . ". " . get_string('entry', 'data') . " (ID $recordid)<br />\n";
            }
        }
        $cir->close();
        $cir->cleanup(true);
...
function data_export_xls($export, $dataname, $count) {
    ...
    $filearg = '-';
    $workbook = new EsetExcelWorkbook($filearg);
...
function data_export_ods($export, $dataname, $count) {
    ...
    $filename .= '.ods';
    $filearg = '-';
    $workbook = new EsetODSWorkbook($filearg);
    $workbook->send($filename);
    $worksheet = array();
    $worksheet[0] = $workbook->add_worksheet('');
    $rowno = 0;
...
function data_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    ...
    if ($record->groupid) {
        ...
        if ($groupmode == SEPARATEGROUPS and !has_capability('eset/site:accessallgroups', $context)) {
...
function data_extend_navigation($navigation, $course, $module, $cm) {
    ...
    $navigation->add(get_string('list', 'data'), new eset_url('/mod/data/view.php', array('d'=>$cm->instance)));
    if (!empty($rid)) {
        $navigation->add(get_string('single', 'data'), new eset_url('/mod/data/view.php', array('d'=>$cm->instance, 'rid'=>$rid)));
    } else {
        $navigation->add(get_string('single', 'data'), new eset_url('/mod/data/view.php', array('d'=>$cm->instance, 'mode'=>'single')));
    }
    $navigation->add(get_string('search', 'data'), new eset_url('/mod/data/view.php', array('d'=>$cm->instance, 'mode'=>'asearch')));
}
...
function data_extend_settings_navigation(settings_navigation $settings, navigation_node $datanode) {
    ...
    if (data_user_can_add_entry($data, $currentgroup, $groupmode, $PAGE->cm->context)) { // took out participation list here!
        ...
        $datanode->add($addstring, new eset_url('/mod/data/edit.php', array('d'=>$PAGE->cm->instance)));
    }

    if (has_capability(DATA_CAP_EXPORT, $PAGE->cm->context)) {
        // The capability required to Export database records is centrally defined in 'lib.php'
        // and should be weaker than those required to edit Templates, Fields and Presets.
        $datanode->add(get_string('exportentries', 'data'), new eset_url('/mod/data/export.php', array('d'=>$data->id)));
    }
    if (has_capability('mod/data:manageentries', $PAGE->cm->context)) {
        $datanode->add(get_string('importentries', 'data'), new eset_url('/mod/data/import.php', array('d'=>$data->id)));
    }
    ...
    if (has_capability('mod/data:managetemplates', $PAGE->cm->context)) {
        ...
        foreach ($templatelist as $template) {
            $templates->add(get_string($template, 'data'), new eset_url('/mod/data/templates.php', array('d'=>$data->id,'mode'=>$template)));
        }

        $datanode->add(get_string('fields', 'data'), new eset_url('/mod/data/field.php', array('d'=>$data->id)));
        $datanode->add(get_string('presets', 'data'), new eset_url('/mod/data/preset.php', array('d'=>$data->id)));
    }
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        $url = new eset_url(rss_get_url($PAGE->cm->context->id, $USER->id, 'mod_data', $data->id));
...
function mod_data_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('add', 'data'),
        new \eset_url('/mod/data/view.php', array('id' => $cm->id)),
        1,
        $actionable
    );
}
