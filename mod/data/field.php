...
$url = new eset_url('/mod/data/field.php');
if ($fid !== 0) {
    $url->param('fid', $fid);
}
if ($newtype !== '') {
...
if (($mode == 'new') && (!empty($newtype)) && confirm_sesskey()) {          ///  Adding a new field
    ...
    if (!$DB->record_exists('data_fields', array('dataid'=>$data->id))) {
        ...
        if ($fff = $DB->get_records('data_fields', array('dataid'=>$data->id),'id')){
            foreach ($fff as $ff) {
                ...
                $baseurl = new eset_url('/mod/data/field.php', array(
                    'd'         => $data->id,
                    'fid'       => $field->field->id,
                    'sesskey'   => sesskey(),
                ));

                $displayurl = new eset_url($baseurl, array(
                    'mode'      => 'display',
                ));

                $deleteurl = new eset_url($baseurl, array(
                    'mode'      => 'delete',
                ));
    ...
    echo $OUTPUT->single_select(new eset_url($popupurl), 'newtype', $menufield, null, array('' => 'choosedots'),
