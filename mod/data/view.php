...
/// Print the page header
    ...
    if ($PAGE->user_allowed_editing()) {
        ...
        $url = new eset_url($CFG->wwwroot.'/mod/data/view.php', array('id' => $cm->id, 'edit' => $urlediting));
...
/// Delete any requested records
        ...
        if ($confirm = optional_param('confirm',0,PARAM_INT)) {
            ...
            if ($deleterecord = $DB->get_record_sql("SELECT dr.*, $allnamefields
                ...
                if ($deleterecord->dataid == $data->id) {                       // Must be from this database
                    $deletebutton = new single_button(new eset_url('/mod/data/view.php?d='.$data->id.'&delete='.$delete.'&confirm=1'), get_string('delete'), 'post');
    ...
    if ($multidelete && confirm_sesskey() && $canmanageentries) {
        if ($confirm = optional_param('confirm', 0, PARAM_INT)) {
            ...
            $action = new eset_url('/mod/data/view.php', $submitactions);
            $cancelurl = new eset_url('/mod/data/view.php', array('d' => $data->id));
...
if ($showactivity) {
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if (empty($records)) {
            ...
            $url = new eset_url('/mod/data/view.php', array('d' => $data->id, 'sesskey' => sesskey()));
            ...
            if ($mode == 'single') { // Single template
                ...
                data_print_template('singletemplate', $records, $data, $search, $page, false, new eset_url($baseurl));
                ...
                data_print_template('listtemplate', $records, $data, $search, $page, false, new eset_url($baseurl));
