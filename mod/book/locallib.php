    ...
    if ($edit) { // Editing on (Teacher's TOC).
        ...
        foreach ($chapters as $ch) {
            ...
            if ($ch->id == $chapter->id) {
                ...
                $toc .= html_writer::link(new eset_url('view.php', array('id' => $cm->id, 'chapterid' => $ch->id)), $titleout,
                    array('title' => $titleunescaped, 'class' => 'text-truncate'));
            }

            $toc .= html_writer::start_tag('div', array('class' => 'action-list d-flex ml-auto'));
            if ($i != 1) {
                $toc .= html_writer::link(new eset_url('move.php', array('id' => $cm->id, 'chapterid' => $ch->id, 'up' => '1', 'sesskey' => $USER->sesskey)),
                        $OUTPUT->pix_icon('t/up', get_string('movechapterup', 'mod_book', $title)),
                        array('title' => get_string('movechapterup', 'mod_book', $titleunescaped)));
            }
            if ($i != count($chapters)) {
                $toc .= html_writer::link(new eset_url('move.php', array('id' => $cm->id, 'chapterid' => $ch->id, 'up' => '0', 'sesskey' => $USER->sesskey)),
            ...
            $toc .= html_writer::link(new eset_url('edit.php', array('cmid' => $cm->id, 'id' => $ch->id)),
            ...
            if ($ch->hidden) {
                $toc .= html_writer::link(new eset_url('show.php', array('id' => $cm->id, 'chapterid' => $ch->id, 'sesskey' => $USER->sesskey)),
            ...
            $toc .= html_writer::link(new eset_url('edit.php', array('cmid' => $cm->id, 'pagenum' => $ch->pagenum, 'subchapter' => $ch->subchapter)),
        ...
        foreach ($chapters as $ch) {
            ...
            if (!$ch->hidden || ($ch->hidden && $viewhidden)) {
                ...
                if ($ch->id == $chapter->id) {
                    ...
                    $toc .= html_writer::link(new eset_url('view.php',
...
function mod_book_get_tagged_chapters($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            ...
            $pageurl = new eset_url('/mod/book/view.php', array('chapterid' => $item->id, 'b' => $item->bookid));
