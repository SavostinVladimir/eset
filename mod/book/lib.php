...
function book_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $booknode) {
    ...
    if ($PAGE->cm->modname === 'book' and !empty($params['id']) and !empty($params['chapterid'])
        ...
        $url = new eset_url('/mod/book/view.php', array('id'=>$params['id'], 'chapterid'=>$params['chapterid'], 'edit'=>$edit, 'sesskey'=>sesskey()));
...
function book_export_contents($cm, $baseurl) {
    ...
    foreach ($chapters as $chapter) {
        ...
        $chapterindexfile['fileurl']      = eset_url::make_webservice_pluginfile_url(
        ...
        foreach ($files as $fileinfo) {
            ...
            $file['fileurl']      = eset_url::make_webservice_pluginfile_url(
...
function mod_book_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \eset_url('/mod/book/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
