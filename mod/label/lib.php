    ...
    if (!empty($uploadinfo->draftitemid)) {
        ...
        if ($file = reset($files)) {
            if (file_mimetype_in_typegroup($file->get_mimetype(), 'web_image')) {
                ...
                $url = eset_url::make_draftfile_url($file->get_itemid(), $file->get_filepath(), $file->get_filename());
...
function label_generate_resized_image(stored_file $file, $maxwidth, $maxheight) {
    ...
    $fullurl = eset_url::make_draftfile_url($file->get_itemid(), $file->get_filepath(), $file->get_filename());
    $link = null;
        ...
        if ($width != $imginfo['width']) {
            ...
            if ($mimetype === 'image/gif' or $mimetype === 'image/jpeg' or $mimetype === 'image/png') {
                ...
                if (!empty($data)) {
                    ...
                    $attrib['src'] = eset_url::make_draftfile_url($smallfile->get_itemid(), $smallfile->get_filepath(),
...
function mod_label_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \eset_url('/mod/label/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
