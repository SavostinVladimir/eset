...
class assign_admin_page_manage_assign_plugins extends admin_externalpage {
    ...
    public function __construct($subtype) {
        ...
        $url = new eset_url('/mod/assign/adminmanageplugins.php', array('subtype'=>$subtype));
...
class assign_plugin_manager {
     ...
     */
    public function __construct($subtype) {
        $this->pageurl = new eset_url('/mod/assign/adminmanageplugins.php', array('subtype'=>$subtype));
        $this->subtype = $subtype;
    }
    ...
    private function format_icon_link($action, $plugin, $icon, $alt) {
        ...
        return $OUTPUT->action_icon(new eset_url($url,
                array('action' => $action, 'plugin'=> $plugin, 'sesskey' => sesskey())),
                new pix_icon($icon, $alt, 'eset', array('title' => $alt)),
    ...
    private function view_plugins_table() {
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            if ($row[1] != '' && $exists) {
                $row[] = html_writer::link(new eset_url('/admin/settings.php',
    ...
    private function check_permissions() {
        ...
        require_capability('eset/site:config', $systemcontext);
    }
