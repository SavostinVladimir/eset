...
/**
 * This file contains the eset hooks for the assign module.
 *
 ...
 * @param esetform $mform form passed by reference
 */
...
/**
 * Return the list if Eset features this module supports
 *
...
function assign_extend_settings_navigation(settings_navigation $settings, navigation_node $navref) {
    ...
    if (has_capability('mod/assign:manageoverrides', $PAGE->cm->context)) {
        $url = new eset_url('/mod/assign/overrides.php', array('cmid' => $PAGE->cm->id));
        $node = navigation_node::create(get_string('groupoverrides', 'assign'),
            new eset_url($url, array('mode' => 'group')),
        ...
        $node = navigation_node::create(get_string('useroverrides', 'assign'),
            new eset_url($url, array('mode' => 'user')),
    ...
    if (has_capability('gradereport/grader:view', $cm->context) &&
            has_capability('eset/grade:viewall', $cm->context)) {
        $link = new eset_url('/grade/report/grader/index.php', array('id' => $course->id));
    ...
    if (has_any_capability(array('mod/assign:grade', 'mod/assign:viewgrades'), $context)) {
        $link = new eset_url('/mod/assign/view.php', array('id' => $cm->id, 'action'=>'grading'));
        $node = $navref->add(get_string('viewgrading', 'assign'), $link, navigation_node::TYPE_SETTING);

        $link = new eset_url('/mod/assign/view.php', array('id' => $cm->id, 'action'=>'downloadall'));
    ...
    if (has_capability('mod/assign:revealidentities', $context)) {
        ...
        if ($assignment && $assignment->blindmarking && !$assignment->revealidentities) {
            ...
            $url = new eset_url('/mod/assign/view.php', $urlparams);
    ...
    foreach ($submissions as $submission) {
        ...
        if (empty($showrecentsubmissions)) {
            if (!array_key_exists($cm->id, $grader)) {
                $grader[$cm->id] = has_capability('eset/grade:viewall', $context);
            }
            if (!$grader[$cm->id]) {
                continue;
            }
        ...
        if ($groupmode == SEPARATEGROUPS &&
                !has_capability('eset/site:accessallgroups',  $context)) {
            if (isguestuser()) {
...
function assign_get_recent_mod_activity(&$activities,
    ...
    $grader          = has_capability('eset/grade:viewall', $cmcontext);
    $accessallgroups = has_capability('eset/site:accessallgroups', $cmcontext);
    $viewfullnames   = has_capability('eset/site:viewfullnames', $cmcontext);
...
function assign_get_extra_capabilities() {
    return ['gradereport/grader:view', 'eset/grade:viewall'];
}
...
function assign_get_file_info($browser,
    ...
    if ($filearea === ASSIGN_INTROATTACHMENT_FILEAREA) {
        if (!has_capability('eset/course:managefiles', $context)) {
...
function assign_user_outline($course, $user, $coursemodule, $assignment) {
    ...
    $result = new stdClass();
    if (!$gradingitem->hidden || has_capability('eset/grade:viewhidden', context_course::instance($course->id))) {
 ...
 *
 * This is used to determine global visibility of an event in all places throughout Eset. For example,
...
function mod_assign_core_calendar_provide_event_action(calendar_event $event,
    ...
    if ($event->eventtype == ASSIGN_EVENT_TYPE_GRADINGDUE) {
        ...
        $url = new \eset_url('/mod/assign/view.php', [
            'id' => $cm->id,
            'action' => 'grader'
        ]);
