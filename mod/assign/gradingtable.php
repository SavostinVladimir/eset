...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment,
        ...
        $url = new eset_url($CFG->wwwroot . '/mod/assign/view.php', $urlparams);
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $viewfullnames = has_capability('eset/site:viewfullnames', $this->assignment->get_context());
        ...
        if ($this->is_downloading()) {
            if (isset($markers[$row->allocatedmarker])) {
                return fullname($markers[$row->allocatedmarker],
                        has_capability('eset/site:viewfullnames', $this->assignment->get_context()));
            } else {
                return '';
            }
        }
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $link = new eset_url('/user/view.php', array('id' => $row->id, 'course' => $courseid));
    ...
    public function can_view_all_grades() {
        ...
        return has_capability('gradereport/grader:view', $context) &&
               has_capability('eset/grade:viewall', $context);
    }
