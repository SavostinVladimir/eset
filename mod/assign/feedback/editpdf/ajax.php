...
if ($action === 'pollconversions') {
    ...
    if (in_array($response->status, $completestatuslist)) {
        ...
        foreach ($pages as $id => $pagefile) {
            ...
            $page->url = eset_url::make_pluginfile_url($context->id,
    ...
    $response->url = '';
    if ($file) {
        $url = eset_url::make_pluginfile_url($assignment->get_context()->id,
                                               'assignfeedback_editpdf',
    ...
    $page = new stdClass();
    $page->url = eset_url::make_pluginfile_url($context->id, document_services::COMPONENT, $filearea,
