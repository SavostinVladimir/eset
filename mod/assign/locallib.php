...
class assign {
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new eset_url('/mod/assign/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new eset_url('/mod/assign/index.php', array('id' => $this->get_course()->id));
        }
    ...
    public function view($action='', $args = array()) {
        ...
        if ($action == 'redirect') {
            $nextpageurl = new eset_url('/mod/assign/view.php', $nextpageparams);
            $messages = '';
     ...
     * @param EsetQuickForm $mform
     * @param stdClass $data
     * @param int $userid - The userid we are grading
     * @return void
     */
    protected function add_plugin_grade_elements($grade, EsetQuickForm $mform, stdClass $data, $userid) {
        foreach ($this->feedbackplugins as $plugin) {
     ...
     * @param EsetQuickForm $mform The form to add the configuration settings to.
     ...
     * @return void
     */
    protected function add_plugin_settings(assign_plugin $plugin, EsetQuickForm $mform, & $pluginsenabled) {
        global $CFG;
     ...
     * @return void
     */
    public function add_all_plugin_settings(EsetQuickForm $mform) {
     ...
     *
     * Cron function to be run periodically according to the eset cron.
     *
     * @return bool
     */
     ...
     *
     * @param esetform $mform - Used for validation of the submitted data
     * @return string
     */
    ...
    public function get_submission_group_members($groupid, $onlyids, $excludesuspended = false) {
        ...
        if ($excludesuspended || !has_capability('eset/course:viewsuspendedusers', $this->context)) {
    ...
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('assign', $course->id, 'm.duedate')) {
            $o .= $this->get_renderer()->notification(get_string('thereareno', 'eset', $strplural));
            $o .= $this->get_renderer()->continue_button(new eset_url('/course/view.php', array('id' => $course->id)));
            return $o;
        }
    ...
    public function can_view_grades($groupid = null) {
        ...
        if ($this->get_course_module()->effectivegroupmode == SEPARATEGROUPS) {
            ...
            $groupflag = has_capability('eset/site:accessallgroups', $this->get_context());
    ...
    protected function download_submissions($userids = false) {
        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    ...
                    $fullname = fullname($student, has_capability('eset/site:viewfullnames', $this->get_context()));
                    ...
                    foreach ($this->submissionplugins as $plugin) {
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
                            if ($downloadasfolders) {
                                ...
                                // This is the default behavior for version of Eset >= 3.1.
                                ...
                                // This was the default behavior for version of Eset < 3.1.
        ...
        if (count($filesforzipping) == 0) {
            ...
            $url = new eset_url('/mod/assign/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
     ...
     *             (see http://docs.eset.org/dev/Migrating_logging_calls_in_plugins).
     *
    ...
    protected function view_single_grading_panel($args) {
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $viewfullnames = has_capability('eset/site:viewfullnames', $this->get_context());
    ...
    protected function view_remove_submission_confirm() {
        ...
        $urlparams = array('id' => $this->get_course_module()->id,
                           ...
                           'userid' => $userid,
                           'sesskey' => sesskey());
        $confirmurl = new eset_url('/mod/assign/view.php', $urlparams);

        $urlparams = array('id' => $this->get_course_module()->id,
                           'action' => 'view');
        $cancelurl = new eset_url('/mod/assign/view.php', $urlparams);
    ...
    protected function view_return_links() {
        ...
        $url = new eset_url('/mod/assign/view.php', $params);
    ...
    protected function view_grading_table() {
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
                has_capability('eset/grade:viewall', $this->get_course_context())) {
        ...
        $showonlyactiveenrolopt = has_capability('eset/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('assign_downloadasfolders', 1);
        ...
        if ($markingallocation) {
            ...
            $viewfullnames = has_capability('eset/site:viewfullnames', $this->context);
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new eset_url('/mod/assign/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
        ...
        if ($this->is_blind_marking()) {
            ...
            if ($hasviewblind) {
                return get_string('participant', 'assign') . ' ' . $uniqueid . ' (' .
                        fullname($user, has_capability('eset/site:viewfullnames', $this->get_context())) . ')';
            } else {
                return get_string('participant', 'assign') . ' ' . $uniqueid;
            }
        } else {
            return fullname($user, has_capability('eset/site:viewfullnames', $this->get_context()));
        }
    }
    ...
    public function can_view_submission($userid) {
        ...
        if (!$this->is_active_user($userid) && !has_capability('eset/course:viewsuspendedusers', $this->context)) {
            return false;
        }
     ...
     *
     * @param esetform $mform Set to a grading batch operations form
     ...
     *
     * @param esetform $mform - null unless form validation has failed
     * @return string
     */
    ...
    protected function view_submission_page() {
        ...
        if ($this->can_view_grades()) {
            ...
            $currenturl = new eset_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id));
     ...
     *
     * @param esetform|null $mform If validation failed when submitting this form - this is the esetform.
     *               It can be null.
     ...
     * Save extension date.
     *
     * @param esetform $mform The submitted form
     * @return boolean
     */
     ...
     * @return void
     */
    public function add_grade_form_elements(EsetQuickForm $mform, stdClass $data, $params) {
        global $USER, $CFG, $SESSION;
        ...
        $capabilitylist = array('gradereport/grader:view', 'eset/grade:viewall');
        if (has_all_capabilities($capabilitylist, $this->get_course_context())) {
            $urlparams = array('id'=>$this->get_course()->id);
            $url = new eset_url('/grade/report/grader/index.php', $urlparams);
            $usergrade = '-';
     ...
     * @return void
     */
    public function add_submission_form_elements(EsetQuickForm $mform, stdClass $data) {
        global $USER;
    ...
    protected function process_save_grade(&$mform) {
        ...
        if (!$userid) {
            if (empty($SESSION->mod_assign_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new eset_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id));
                throw new eset_exception('useridlistnotcached', 'mod_assign', $url);
            }
        ...
        if (is_null($this->showonlyactiveenrol)) {
            ...
            if (!is_null($this->context)) {
                $this->showonlyactiveenrol = $this->showonlyactiveenrol ||
                            !has_capability('eset/course:viewsuspendedusers', $this->context);
            }
        }
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/assign:grade', $this->get_context()) && get_config('assign', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \eset_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        if ($confirm) {
            ...
            $continue = new \eset_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \eset_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id));
    ...
    protected function get_submissionstatement($adminconfig, $instance, $context) {
        ...
        if (!$instance->teamsubmission) {
            ...
            if (!empty($adminconfig->submissionstatement)) {
                ...
                $submissionstatement = format_text($adminconfig->submissionstatement, FORMAT_ESET, $options);
            }
