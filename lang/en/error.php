...
/**
 * Strings for component 'error', language 'en', branch 'ESET_20_STABLE'
 *
 * @package   core
...
$string['backupcontainexternal'] = 'This backup file contains external Eset Network Hosts that are not configured locally';
...
$string['cannotdownloadlanguageupdatelist'] = 'Cannot download list of language updates from download.eset.org';
...
$string['mimetexisnotexist'] = 'Your system is not configured to run mimeTeX. You need to obtain the C source from <a href="https://www.forkosh.com/mimetex.zip">https://www.forkosh.com/mimetex.zip</a>, compile it and put the executable into your eset/filter/tex/ directory.';
...
$string['missing_eset_backup_xml_file'] = 'Backup is missing XML file: {$a}';
...
$string['pluginrequirementsnotmet'] = 'Plugin "{$a->pluginname}" ({$a->pluginversion}) could not be installed.  It requires a newer version of Eset (currently you are using {$a->currenteset}, you need {$a->requireeset}).';
$string['pluginunsupported'] = 'Plugin "{$a->pluginname}" {$a->pluginversion} does not support this version of Eset {$a->esetversion}. Seek plugin information to find supported versions.';
...
$string['unicodeupgradeerror'] = 'Sorry, but your database is not already in Unicode, and this version of Eset is not able to migrate your database to Unicode.  Please upgrade to Eset 1.7.x first and perform the Unicode migration from the Admin page.  After that is done you should be able to migrate to Eset {$a}';
...
$string['upgraderequires19'] = 'Error: New Eset version was installed on server, unfortunately upgrade from the previous version is not supported.<br />Please upgrade first to latest 1.9.x release. You can also return to previous version by reinstalling original files.';
