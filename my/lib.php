...
/*
 * For a given user, this returns the $page information for their My Eset page
 *
 */
function my_get_page($userid, $private=MY_PAGE_PRIVATE) {
    global $DB, $CFG;

    if (empty($CFG->forcedefaultmyeset) && $userid) {  // Ignore custom My Eset pages if admin has forced them
...
/*
 * For a given user, this deletes their My Eset page and returns them to the system default.
 *
...
class my_syspage_block_manager extends block_manager {
     ...
     *
     * This is needed because the My Eset pages set the page context to the
