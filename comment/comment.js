    ...
    init: function(Y, options) {
        ...
        Y.extend(CommentHelper, Y.Base, {
            ...
            post: function() {
                ...
                if (value && value != M.util.get_string('addcomment', 'eset')) {
                    ...
                    this.request({
                        ...
                        callback: function(id, obj, args) {
                            ...
                            if (linkText) {
                                linkText.set('innerHTML', M.util.get_string('commentscount', 'eset', obj.count));
                            }
                            for(var i in ids) {
                                var attributes = {
                                    color: { to: '#06e' },
            ...
            render: function(list, newcmt) {
                ...
                for(var i in list) {
                    ...
                    if (list[i]['delete']||newcmt) {
                        ...
                        var deleteStr = Y.Escape.html(M.util.get_string('deletecommentbyon', 'eset', tokens));
                ...
                this.request({
                    ...
                    callback: function(id, ret, args) {
                        ...
                        if (ret.count && linkText) {
                            linkText.set('innerHTML', M.util.get_string('commentscount', 'eset', ret.count));
                        }
                        ...
                        if (ret.error == 'require_login') {
                            var result = {};
                            result.html = M.util.get_string('commentsrequirelogin', 'eset');
                        } else {
            ...
            dodelete: function(id) { // note: delete is a reserved word in javascript, chrome and safary do not like it at all here!
                ...
                function remove_dom(type, anim, cmt) {
                    ...
                    if (linkText && comments) {
                        linkText.set('innerHTML', M.util.get_string('commentscount', 'eset', comments.size()));
                    }
                }
                this.request({
                    action: 'delete',
            ...
            toggle_textarea: function(focus) {
                ...
                if (focus) {
                    if (t.get('value') == M.util.get_string('addcomment', 'eset')) {
                        t.set('value', '');
                        t.setStyle('color', 'black');
                    }
                }else{
                    if (t.get('value') == '') {
                        t.set('value', M.util.get_string('addcomment', 'eset'));
