...
/**
 * Comment is helper class to add/delete comments anywhere in eset
 *
 * @package   core
 * @category  comment
 * @copyright 2010 Dongsheng Cai {@link http://dongsheng.org}
...
class comment {
     ...
     *
     * @param eset_page $page The page object to initialise comments within
     *                          If not provided the global $PAGE is used
     */
    public static function init(eset_page $page = null) {
        global $PAGE;
    ...
    private function check_permissions() {
        $this->postcap = has_capability('eset/comment:post', $this->context);
        $this->viewcap = has_capability('eset/comment:view', $this->context);
        if (!empty($this->plugintype)) {
     ...
     * @return eset_url
     */
    public function get_nojslink(eset_page $page = null) {
        if ($page === null) {
            global $PAGE;
            $page = $PAGE;
        }

        $link = new eset_url($page->url, array(
            'nonjscomment'    => true,
     ...
     *
     * @param eset_page $page The eset page object that the JavaScript should be
     *                          initialised for.
     */
    public function initialise_javascript(eset_page $page) {
    ...
    public function get_comments($page = '', $sortdirection = 'DESC') {
        ...
        foreach ($rs as $u) {
            ...
            $url = new eset_url('/user/view.php', array('id'=>$u->id, 'course'=>$this->courseid));
        ...
        if (!empty($this->plugintype)) {
            // eset module will filter comments
     ...
     * @return stdClass
     */
    public function add($content, $format = FORMAT_ESET) {
        ...
        if (!empty($cmt_id)) {
            ...
            $url = new eset_url('/user/view.php', array('id' => $USER->id, 'course' => $this->courseid));
    ...
    public function print_comments($page = 0, $return = true, $nonjs = true) {
        ...
        if ($nonjs && $this->can_post()) {
            ...
            $html .= html_writer::start_tag('form', array('method' => 'post', 'action' => new eset_url('/comment/comment_post.php')));
            // Comment parameters
    ...
    public function print_comment($cmt, $nonjs = true) {
        ...
        if (!empty($cmt->delete) && empty($nonjs)) {
            $strdelete = get_string('deletecommentbyon', 'eset', (object)['user' => $cmt->fullname, 'time' => $cmt->time]);
    ...
    public function can_delete($comment) {
        ...
        $hascapability = has_capability('eset/comment:delete', $this->context);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class comment_exception extends eset_exception {
}
