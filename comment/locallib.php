...
/**
 * comment_manager is helper class to manage eset comments in admin page (Reports->Comments)
 *
 * @package   core
 * @copyright 2010 Dongsheng Cai {@link http://dongsheng.org}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class comment_manager {
    ...
    function get_comments($page) {
        ...
        foreach ($rs as $item) {
            ...
            $item->content = format_text($item->content, FORMAT_ESET, $formatoptions);
    ...
    public function print_comments($page = 0) {
        ...
        if (count($comments) == 0) {
            echo $OUTPUT->notification(get_string('nocomments', 'eset'));
            return false;
        }
        ...
        $link = new eset_url('/comment/index.php', array('action' => 'delete', 'sesskey' => sesskey()));
        foreach ($comments as $c) {
            $userdata = html_writer::link(new eset_url('/user/profile.php', ['id' => $c->userid]), $c->fullname);
            $this->setup_plugin($c);
            ...
            $action = html_writer::link(new eset_url($link, array('commentid' => $c->id)), get_string('delete'));
