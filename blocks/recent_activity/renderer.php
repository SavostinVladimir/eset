...
class block_recent_activity_renderer extends plugin_renderer_base {
    ...
    public function recent_activity($course, $timestart, $recentenrolments, $structuralchanges,
        ...
        $output .= html_writer::tag('div',
                html_writer::link(new eset_url('/course/recent.php', array('id' => $course->id)),
        ...
        if ($recentenrolments) {
            ...
            foreach ($recentenrolments as $user) {
                $output .= html_writer::tag('li',
                        html_writer::link(new eset_url('/user/view.php', array('id' => $user->id, 'course' => $course->id)),
    ...
    protected function structural_change($change) {
        ...
        switch ($change['action']) {
            case 'delete mod':
                $text = get_string('deletedactivity', 'eset', $cm->modfullname);
                break;
            case 'add mod':
                $text = get_string('added', 'eset', $cm->modfullname). '<br />'.
                    html_writer::link($cm->url, format_string($cm->name, true));
                break;
            case 'update mod':
                $text = get_string('updated', 'eset', $cm->modfullname). '<br />'.
                    html_writer::link($cm->url, format_string($cm->name, true));
                break;
            default:
                return '';
