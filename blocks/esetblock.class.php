...
/**
 * This file contains the parent class for eset blocks, block_base.
 *
 * @package    core_block
...
/**
 * Class for describing a eset block, all Eset blocks derive from this class
 *
...
/// Class Functions
     ...
     */
    function is_empty() {
        if ( !has_capability('eset/block:view', $this->context) ) {
            return true;
        }
...
/*        if ($this->get_content() === NULL) {
     ...
     *
     * @deprecated since Eset 2.9 MDL-49385 - Please use Admin Settings functionality to save block configuration.
     */
     ...
     * @param eset_page $the page this block is on.
     */
    ...
    function user_can_edit() {
        ...
        if (has_capability('eset/block:edit', $this->context)) {
            return true;
        }

        // The blocks in My Eset are a special case.  We want them to inherit from the user context.
        if (!empty($USER->id)
            ...
            return has_capability('eset/my:manageblocks', $this->page->context);
        }
    ...
    function user_can_addto($page) {
        ...
        // The blocks in My Eset are a special case and use a different capability.
        ...
        if (array_key_exists($page->pagetype, $mypagetypes)) { // Ensure we are on a page with a my page type.
            ...
            if ((isset($formats['my']) && $formats['my'] == false)
                ...
                return $this->has_add_block_capability($page, $capability)
                       && has_capability('eset/my:manageblocks', $page->context);
            }
        }
        ...
        if ($this->has_add_block_capability($page, $capability)
                && has_capability('eset/block:edit', $page->context)) {
            return true;
        }
    ...
    static function get_extra_capabilities() {
        return array('eset/block:view', 'eset/block:edit');
    }
