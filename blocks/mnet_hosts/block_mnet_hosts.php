...
class block_mnet_hosts extends block_list {
    ...
    function applicable_formats() {
        if (has_capability('eset/site:mnetlogintoremote', context_system::instance(), NULL, false)) {
        ...
        if ($hosts) {
            foreach ($hosts as $host) {
                if ($host->id == $USER->mnethostid) {
                    $url = new \eset_url($host->wwwroot);
                } else {
                    $url = new \eset_url('/auth/mnet/jump.php', array('hostid' => $host->id));
                }
