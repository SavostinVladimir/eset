...
class block_social_activities extends block_list {
    ...
    function get_content() {
        ...
        $isediting = $this->page->user_is_editing() && has_capability('eset/course:manageactivities', $context);
            ...
            foreach ($modinfo->sections[0] as $modnumber) {
                ...
                if (!$ismoving) {
                    ...
                    $actions = array('move' => new action_menu_link_primary(
                        new eset_url('/course/mod.php', array('sesskey' => sesskey(), 'copy' => $mod->id)),
                        new pix_icon('t/move', $strmove, 'eset', array('class' => 'iconsmall', 'title' => '')),
                        $strmove
                    )) + $actions;
                ...
                if ($mod->visible || has_capability('eset/course:viewhiddenactivities', $mod->context)) {
                    if ($ismoving) {
