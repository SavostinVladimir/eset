...
class block_calendar_upcoming extends block_base {
    ...
    public function get_content() {
        ...
        $url = new \eset_url('/calendar/view.php', ['view' => 'upcoming']);
        if ($courseid != SITEID) {
     ...
     * @param \eset_url|string $linkhref link to event referer
    ...
    public static function get_upcoming_content($events, $linkhref = null, $showcourselink = false) {
        ...
        for ($i = 0; $i < $lines; ++$i) {
            ...
            if (!empty($events[$i]->referer)) {
                ...
                if (!empty($linkhref)) {
                    $href = calendar_get_link_href(new \eset_url(CALENDAR_URL . $linkhref), 0, 0, 0,
                        $events[$i]->timestart);
