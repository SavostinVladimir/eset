...
class block_admin_bookmarks extends block_base {
    ...
    function get_content() {
        ...
        if (get_user_preferences('admin_bookmarks')) {
            ...
            foreach($bookmarks as $bookmark) {
                ...
                if ($temp instanceof admin_settingpage) {
                    $contenturl = new eset_url('/admin/settings.php', array('section'=>$bookmark));
                    ...
                    $contenturl = new eset_url($temp->url);
                    $contentlink = html_writer::link($contenturl, $temp->visiblename);
                    $contents[] = html_writer::tag('li', $contentlink);
                } else if ($temp instanceof admin_category) {
                    $contenturl = new eset_url('/admin/category.php', array('category' => $bookmark));
                    $contentlink = html_writer::link($contenturl, $temp->visiblename);
                    $contents[] = html_writer::tag('li', $contentlink);
                }
            }
        ...
        if ($section == 'search' || empty($section)){
            ...
            $deleteurl = new eset_url('/blocks/admin_bookmarks/delete.php', array('section'=>$section, 'sesskey'=>sesskey()));
            $this->content->footer =  html_writer::link($deleteurl, get_string('unbookmarkthispage','admin'));
        } else {
            $createurl = new eset_url('/blocks/admin_bookmarks/create.php', array('section'=>$section, 'sesskey'=>sesskey()));
