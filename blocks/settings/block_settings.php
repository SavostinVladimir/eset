...
/**
 * This file contains classes used to manage the navigation structures in Eset
 * and was introduced as part of the changes occuring in Eset 2.0
 *
 * @since Eset 2.0
 ...
 *
 * Used to produce the settings navigation block new to Eset 2.0
 *
 * @package block_settings
 * @copyright 2009 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class block_settings extends block_base {
    ...
    function get_content() {
        ...
        // only do search if you have eset/site:config
        if (!empty($this->content->text)) {
            if (has_capability('eset/site:config',context_system::instance()) ) {
                $this->content->footer = $renderer->search_form(new eset_url("$CFG->wwwroot/$CFG->admin/search.php"), optional_param('query', '', PARAM_RAW));
            } else {
                $this->content->footer = '';
            }
